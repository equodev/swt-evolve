 ...
 * @since 3.0
 */
public final class SwtTextLayout extends SwtResource implements ITextLayout {
    ...
    public SwtTextLayout(Device device, TextLayout api) {
        ...
        if (layout == 0)
            SWT.error(SWT.ERROR_NO_HANDLES);
        OS.pango_layout_set_font_description(layout, ((SwtDevice) device.getImpl()).systemFont.handle);
        OS.pango_layout_set_wrap(layout, OS.PANGO_WRAP_WORD_CHAR);
        OS.pango_layout_set_tabs(layout, ((SwtDevice) device.getImpl()).emptyTab);
    ...
    void computeRuns() {
        ...
        Font defaultFont = font != null ? font : ((SwtDevice) device.getImpl()).systemFont;
        for (int i = 0; i < stylesCount - 1; i++) {
            ...
            if (style.underline) {
                    ...
                    case SWT.UNDERLINE_LINK:
                        {
                            if (style.foreground == null) {
                                ...
                                attr = OS.pango_attr_foreground_new((short) (linkRGBA.red * 0xFFFF), (short) (linkRGBA.green * 0xFFFF), (short) (linkRGBA.blue * 0xFFFF));
                                OS.memmove(attribute, attr, PangoAttribute.sizeof);
                                attribute.start_index = byteStart;
                                attribute.end_index = byteEnd;
                                OS.memmove(attr, attribute, PangoAttribute.sizeof);
                ...
                if (style.underlineColor != null) {
                    ...
                    attr = OS.pango_attr_underline_color_new((short) (rgba.red * 0xFFFF), (short) (rgba.green * 0xFFFF), (short) (rgba.blue * 0xFFFF));
                    if (attr != 0) {
                        OS.memmove(attribute, attr, PangoAttribute.sizeof);
                        attribute.start_index = byteStart;
                        attribute.end_index = byteEnd;
            ...
            if (style.strikeout) {
                ...
                if (style.strikeoutColor != null) {
                    ...
                    attr = OS.pango_attr_strikethrough_color_new((short) (rgba.red * 0xFFFF), (short) (rgba.green * 0xFFFF), (short) (rgba.blue * 0xFFFF));
                    if (attr != 0) {
                        OS.memmove(attribute, attr, PangoAttribute.sizeof);
                        attribute.start_index = byteStart;
                        attribute.end_index = byteEnd;
            ...
            if (foreground != null && !foreground.isDisposed()) {
                ...
                attr = OS.pango_attr_foreground_new((short) (rgba.red * 0xFFFF), (short) (rgba.green * 0xFFFF), (short) (rgba.blue * 0xFFFF));
                OS.memmove(attribute, attr, PangoAttribute.sizeof);
                attribute.start_index = byteStart;
                attribute.end_index = byteEnd;
                OS.memmove(attr, attribute, PangoAttribute.sizeof);
            ...
            if (background != null && !background.isDisposed()) {
                ...
                attr = OS.pango_attr_background_new((short) (rgba.red * 0xFFFF), (short) (rgba.green * 0xFFFF), (short) (rgba.blue * 0xFFFF));
                OS.memmove(attribute, attr, PangoAttribute.sizeof);
                attribute.start_index = byteStart;
                attribute.end_index = byteEnd;
                OS.memmove(attr, attribute, PangoAttribute.sizeof);
    ...
    void drawInPixels(GC gc, int x, int y, int selectionStart, int selectionEnd, Color selectionForeground, Color selectionBackground, int flags) {
        ...
        if (!hasSelection) {
            ...
            if (fullSelection) {
                ...
                drawWithCairo(gc, x, y, 0, C.strlen(ptr), yExtent, fullSelection, selectionForeground.handle, selectionBackground.handle);
                if ((data.style & SWT.MIRRORED) != 0) {
                    Cairo.cairo_restore(cairo);
                }
            } else {
                ...
                drawWithCairo(gc, x, y, byteSelStart, byteSelEnd, yExtent, fullSelection, selectionForeground.handle, selectionBackground.handle);
                if ((data.style & SWT.MIRRORED) != 0) {
                    Cairo.cairo_restore(cairo);
                }
            }
        ...
        for (int i = 0; i < stylesCount - 1; i++) {
            ...
            if (drawBorder && !((SwtTextStyle) style.getImpl()).isAdherentBorder(styles[i + 1].style)) {
                ...
                if (rgn != 0) {
                    ...
                    SwtRegion.cairo_region_get_rectangles(rgn, rects, nRects);
                    cairo_rectangle_int_t rect = new cairo_rectangle_int_t();
    ...
    public int getAlignment() {
        ...
        switch(align) {
            case OS.PANGO_ALIGN_LEFT:
                return rtl ? SWT.RIGHT : SWT.LEFT;
            case OS.PANGO_ALIGN_RIGHT:
	...
	*/
    ...
    public FontMetrics getLineMetrics(int lineIndex) {
        ...
        int ascentInPoints;
        if (line.runs == 0) {
            long font = this.font != null ? this.font.handle : ((SwtDevice) device.getImpl()).systemFont.handle;
	...
	*/
    ...
    public void setFont(Font font) {
        ...
        if (oldFont != null && oldFont.equals(font))
            return;
        OS.pango_layout_set_font_description(layout, font != null ? font.handle : ((SwtDevice) device.getImpl()).systemFont.handle);
    }
