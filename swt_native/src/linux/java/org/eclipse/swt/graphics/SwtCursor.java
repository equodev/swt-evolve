...
public final class SwtCursor extends SwtResource implements ICursor {
    ...
    public SwtCursor(Device device, int style, Cursor api) {
        ...
        for (String name : new String[] { name1, name2, "left_ptr", "default" }) {
            if (name == null)
                continue;
            if (GTK.GTK4) {
                getApi().handle = GDK.gdk_cursor_new_from_name(name, 0);
            } else {
                getApi().handle = GDK.gdk_cursor_new_from_name(GDK.gdk_display_get_default(), name);
            }
            if (getApi().handle != 0)
                break;
        }
    ...
    private void setupCursorFromImageData(ImageData source, int hotspotX, int hotspotY) {
        ...
        if (!palette.isDirect || source.depth != 24 || stride != source.bytesPerLine || palette.redMask != 0xFF000000 || palette.greenMask != 0xFF0000 || palette.blueMask != 0xFF00) {
            ...
            if (palette.isDirect) {
                ImageData.blit(source.data, source.depth, source.bytesPerLine, source.getByteOrder(), source.width, source.height, palette.redMask, palette.greenMask, palette.blueMask, buffer, 32, source.width * 4, ImageData.MSB_FIRST, source.width, source.height, 0xFF000000, 0xFF0000, 0xFF00, false, false);
            } else {
                ...
                ImageData.blit(source.width, source.height, source.data, source.depth, source.bytesPerLine, source.getByteOrder(), srcReds, srcGreens, srcBlues, buffer, 32, source.width * 4, ImageData.MSB_FIRST, 0xFF000000, 0xFF0000, 0xFF00);
            }
        ...
        if (GTK.GTK4) {
            ...
            getApi().handle = GDK.gdk_cursor_new_from_texture(texture, hotspotX, hotspotY, 0);
            OS.g_object_unref(texture);
        } else {
            getApi().handle = GDK.gdk_cursor_new_from_pixbuf(display, pixbuf, hotspotX, hotspotY);
        }
