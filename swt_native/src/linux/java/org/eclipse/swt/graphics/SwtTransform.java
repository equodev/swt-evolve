...
public class SwtTransform extends SwtResource implements ITransform {
    ...
    public void getElements(float[] elements) {
        ...
        if (elements.length < 6)
            SWT.error(SWT.ERROR_INVALID_ARGUMENT);
        elements[0] = (float) getApi().handle[0];
    ...
    public void identity() {
        if (isDisposed())
            SWT.error(SWT.ERROR_GRAPHIC_DISPOSED);
        Cairo.cairo_matrix_init(getApi().handle, 1, 0, 0, 1, 0, 0);
    }
    ...
    public void invert() {
        if (isDisposed())
            SWT.error(SWT.ERROR_GRAPHIC_DISPOSED);
        if (Cairo.cairo_matrix_invert(getApi().handle) != 0) {
    ...
    public void multiply(Transform matrix) {
        ...
        if (matrix.isDisposed())
            SWT.error(SWT.ERROR_INVALID_ARGUMENT);
        Cairo.cairo_matrix_multiply(getApi().handle, matrix.handle, getApi().handle);
    }
    ...
    public void rotate(float angle) {
        if (isDisposed())
            SWT.error(SWT.ERROR_GRAPHIC_DISPOSED);
        Cairo.cairo_matrix_rotate(getApi().handle, angle * (float) Math.PI / 180);
    }
    ...
    public void scale(float scaleX, float scaleY) {
        if (isDisposed())
            SWT.error(SWT.ERROR_GRAPHIC_DISPOSED);
        Cairo.cairo_matrix_scale(getApi().handle, scaleX, scaleY);
    }
    ...
    public void shear(float shearX, float shearY) {
        ...
        Cairo.cairo_matrix_multiply(getApi().handle, matrix, getApi().handle);
    }
