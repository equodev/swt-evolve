...
public final class SwtFont extends SwtResource implements IFont {
    ...
    public SwtFont(Device device, FontData fd, Font api) {
        ...
        init(fd.getName(), ((SwtFontData) fd.getImpl()).getHeightF(), fd.getStyle(), fd.string);
        init();
    }
    ...
    @Override
    void destroy() {
        OS.pango_font_description_free(getApi().handle);
        getApi().handle = 0;
    }
    ...
    public FontData[] getFontData() {
        ...
        long family = OS.pango_font_description_get_family(getApi().handle);
        int length = C.strlen(family);
        byte[] buffer = new byte[length];
        C.memmove(buffer, family, length);
        String name = new String(Converter.mbcsToWcs(buffer));
        float height = (float) OS.pango_font_description_get_size(getApi().handle) / OS.PANGO_SCALE;
        ...
        int pangoStyle = OS.pango_font_description_get_style(getApi().handle);
        int pangoWeight = OS.pango_font_description_get_weight(getApi().handle);
        ...
        if (pangoWeight >= OS.PANGO_WEIGHT_BOLD)
            style |= SWT.BOLD;
        long fontString = OS.pango_font_description_to_string(getApi().handle);
    ...
    void init(String name, float height, int style, byte[] fontString) {
        ...
        if (fontString != null) {
            getApi().handle = OS.pango_font_description_from_string(fontString);
            if (getApi().handle == 0)
                SWT.error(SWT.ERROR_NO_HANDLES);
        } else {
            getApi().handle = OS.pango_font_description_new();
            if (getApi().handle == 0)
                SWT.error(SWT.ERROR_NO_HANDLES);
            byte[] buffer = Converter.wcsToMbcs(name, true);
            OS.pango_font_description_set_family(getApi().handle, buffer);
            if (size > 0) {
                OS.pango_font_description_set_size(getApi().handle, (int) (0.5f + size * OS.PANGO_SCALE));
            }
            OS.pango_font_description_set_stretch(getApi().handle, OS.PANGO_STRETCH_NORMAL);
            ...
            OS.pango_font_description_set_style(getApi().handle, pangoStyle);
            OS.pango_font_description_set_weight(getApi().handle, pangoWeight);
        }
    }
