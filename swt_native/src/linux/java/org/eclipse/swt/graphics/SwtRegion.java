...
public final class SwtRegion extends SwtResource implements IRegion {
    ...
    public SwtRegion(Device device, Region api) {
        super(device, api);
        getApi().handle = Cairo.cairo_region_create();
        if (getApi().handle == 0)
	...
	*/
        ...
        Cairo.cairo_region_union(getApi().handle, polyRgn);
    ...
    void addInPixels(int x, int y, int width, int height) {
        ...
        rect.width = width;
        rect.height = height;
        Cairo.cairo_region_union_rectangle(getApi().handle, rect);
    }
    ...
    public void add(Region region) {
        ...
        if (region.isDisposed())
            SWT.error(SWT.ERROR_INVALID_ARGUMENT);
        Cairo.cairo_region_union(getApi().handle, region.handle);
    }
    ...
    boolean containsInPixels(int x, int y) {
        return Cairo.cairo_region_contains_point(getApi().handle, x, y);
    }
    ...
    @Override
    void destroy() {
        Cairo.cairo_region_destroy(getApi().handle);
        getApi().handle = 0;
    }
    ...
    Rectangle getBoundsInPixels() {
        cairo_rectangle_int_t rect = new cairo_rectangle_int_t();
        Cairo.cairo_region_get_extents(getApi().handle, rect);
    ...
    void intersectInPixels(int x, int y, int width, int height) {
        ...
        Cairo.cairo_region_intersect(getApi().handle, rectRgn);
    ...
    public void intersect(Region region) {
        ...
        if (region.isDisposed())
            SWT.error(SWT.ERROR_INVALID_ARGUMENT);
        Cairo.cairo_region_intersect(getApi().handle, region.handle);
    }
    ...
    boolean intersectsInPixels(int x, int y, int width, int height) {
        ...
        rect.width = width;
        rect.height = height;
        return Cairo.cairo_region_contains_rectangle(getApi().handle, rect) != Cairo.CAIRO_REGION_OVERLAP_OUT;
    }
    ...
    public boolean isEmpty() {
        if (isDisposed())
            SWT.error(SWT.ERROR_GRAPHIC_DISPOSED);
        return Cairo.cairo_region_is_empty(getApi().handle);
    }
	...
	*/
        if (pointArray.length < 6)
            return;
        long polyRgn = gdk_region_polygon(pointArray, pointArray.length / 2, GDK.GDK_EVEN_ODD_RULE);
        Cairo.cairo_region_subtract(getApi().handle, polyRgn);
        Cairo.cairo_region_destroy(polyRgn);
    }
    ...
    void subtractInPixels(int x, int y, int width, int height) {
        ...
        rect.height = height;
        long rectRgn = Cairo.cairo_region_create_rectangle(rect);
        Cairo.cairo_region_subtract(getApi().handle, rectRgn);
        Cairo.cairo_region_destroy(rectRgn);
    }
    ...
    public void subtract(Region region) {
        ...
        if (region.isDisposed())
            SWT.error(SWT.ERROR_INVALID_ARGUMENT);
        Cairo.cairo_region_subtract(getApi().handle, region.handle);
    }
    ...
    void translateInPixels(int x, int y) {
        Cairo.cairo_region_translate(getApi().handle, x, y);
    }
