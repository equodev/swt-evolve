...
*/
    ...
    static synchronized Device findDevice(long xDisplay) {
        for (int i = 0; i < Devices.length; i++) {
            ...
            if (device != null && ((SwtDevice) device.getImpl()).xDisplay == xDisplay) {
                return device;
            }
        }
        return null;
        ...
        for (int i = 0; i < n_families[0]; i++) {
            ...
            if (match) {
                ...
                for (int j = 0; j < n_faces[0]; j++) {
                    ...
                    Font font = SwtFont.gtk_new(this.getApi(), fontDesc);
    ...
    public void init() {
        ...
        systemFont = SwtFont.gtk_new(this.getApi(), defaultFont);
        overrideThemeValues();
    }
    ...
    static long XErrorProc(long xDisplay, long xErrorEvent) {
        ...
        if (device != null) {
            if (((SwtDevice) device.getImpl()).warningLevel == 0) {
                if (Device.DEBUG || ((SwtDevice) device.getImpl()).debug) {
