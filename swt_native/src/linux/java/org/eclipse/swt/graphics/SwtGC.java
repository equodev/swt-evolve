...
public final class SwtGC extends SwtResource implements IGC {
    ...
    public static GC gtk_new(long handle, GCData data) {
        ...
        ((SwtGC) gc.getImpl()).init(null, data, handle);
        return gc;
    }
    ...
    public static GC gtk_new(Drawable drawable, GCData data) {
        ...
        ((SwtGC) gc.getImpl()).init(drawable, data, gdkGC);
        return gc;
    }
        ...
        if ((state & (BACKGROUND | FOREGROUND)) != 0) {
            ...
            if (pattern != null) {
                if ((data.style & SWT.MIRRORED) != 0 && ((SwtPattern) pattern.getImpl()).surface != 0) {
                    long newPattern = Cairo.cairo_pattern_create_for_surface(((SwtPattern) pattern.getImpl()).surface);
                    if (newPattern == 0)
                        SWT.error(SWT.ERROR_NO_HANDLES);
    ...
    long convertRgn(long rgn, double[] matrix) {
        ...
        for (int i = 0; i < nRects[0]; i++) {
            ...
            long polyRgn = SwtRegion.gdk_region_polygon(pointArray, pointArray.length / 2, GDK.GDK_EVEN_ODD_RULE);
    ...
    void copyAreaInPixels(int srcX, int srcY, int width, int height, int destX, int destY, boolean paint) {
        ...
        if (data.image != null) {
            ...
            Cairo.cairo_rectangle(getApi().handle, destX, destY, width, height);
            Cairo.cairo_clip(getApi().handle);
            Cairo.cairo_translate(getApi().handle, deltaX, deltaY);
            Cairo.cairo_set_operator(getApi().handle, Cairo.CAIRO_OPERATOR_SOURCE);
            Cairo.cairo_push_group(getApi().handle);
            if (!GTK.GTK4)
                GDK.gdk_cairo_set_source_window(getApi().handle, drawable, 0, 0);
            Cairo.cairo_paint(getApi().handle);
            Cairo.cairo_pop_group_to_source(getApi().handle);
            Cairo.cairo_rectangle(getApi().handle, destX - deltaX, destY - deltaY, width, height);
            Cairo.cairo_clip(getApi().handle);
            Cairo.cairo_paint(getApi().handle);
    ...
    void drawFocusInPixels(int x, int y, int width, int height) {
        long cairo = data.cairo;
        checkGC(FOREGROUND);
        long context = GTK.gtk_widget_get_style_context(((SwtDevice) data.device.getImpl()).shellHandle);
    ...
    public int getAntialias() {
        ...
        switch(antialias) {
            case Cairo.CAIRO_ANTIALIAS_DEFAULT:
                return SWT.DEFAULT;
    ...
    public Color getBackground() {
        if (getApi().handle == 0)
            SWT.error(SWT.ERROR_GRAPHIC_DISPOSED);
        return SwtColor.gtk_new(data.device, data.backgroundRGBA);
    }
    ...
    public Color getForeground() {
        ...
        return SwtColor.gtk_new(data.device, data.foregroundRGBA);
    }
    ...
    void getSize(int[] width, int[] height) {
        ...
        long surface = Cairo.cairo_get_target(getApi().handle);
    ...
    void init(Drawable drawable, GCData data, long gdkGC) {
        ...
        this.data = data;
        getApi().handle = gdkGC;
        long cairo = data.cairo = getApi().handle;
 ...
 */
    ...
    void setString(String string, int flags) {
        ...
        OS.pango_layout_set_tabs(layout, (flags & SWT.DRAW_TAB) != 0 ? 0 : ((SwtDevice) data.device.getImpl()).emptyTab);
        data.string = string;
        data.stringWidth = data.stringHeight = -1;
    ...
    public void setXORMode(boolean xor) {
        if (getApi().handle == 0)
            SWT.error(SWT.ERROR_GRAPHIC_DISPOSED);
        Cairo.cairo_set_operator(getApi().handle, xor ? Cairo.CAIRO_OPERATOR_DIFFERENCE : Cairo.CAIRO_OPERATOR_OVER);
        data.xorMode = xor;
    }
