 ...
 * @since 3.1
 */
public class SwtPattern extends SwtResource implements IPattern {
    ...
    public SwtPattern(Device device, Image image, Pattern api) {
        ...
        Cairo.cairo_pattern_set_extend(getApi().handle, Cairo.CAIRO_EXTEND_REPEAT);
    ...
    public SwtPattern(Device device, float x1, float y1, float x2, float y2, Color color1, int alpha1, Color color2, int alpha2, Pattern api) {
        ...
        if (color2.isDisposed())
            SWT.error(SWT.ERROR_INVALID_ARGUMENT);
        getApi().handle = Cairo.cairo_pattern_create_linear(x1, y1, x2, y2);
        if (getApi().handle == 0)
            SWT.error(SWT.ERROR_NO_HANDLES);
        SwtGC.setCairoPatternColor(getApi().handle, 0, color1, alpha1);
        SwtGC.setCairoPatternColor(getApi().handle, 1, color2, alpha2);
        Cairo.cairo_pattern_set_extend(getApi().handle, Cairo.CAIRO_EXTEND_REPEAT);
        init();
    }
    ...
    @Override
    void destroy() {
        Cairo.cairo_pattern_destroy(getApi().handle);
        getApi().handle = surface = 0;
    }
