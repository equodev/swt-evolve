...
public final class SwtImage extends SwtResource implements Drawable, IImage {
    ...
    public SwtImage(Device device, Image srcImage, int flag, Image api) {
        ...
        this.imageDataProvider = ((SwtImage) srcImage.getImpl()).imageDataProvider;
        this.imageFileNameProvider = ((SwtImage) srcImage.getImpl()).imageFileNameProvider;
        this.imageGcDrawer = ((SwtImage) srcImage.getImpl()).imageGcDrawer;
        ...
        if (flag != SWT.IMAGE_COPY) {
            int stride = Cairo.cairo_image_surface_get_stride(getApi().surface);
            long data = Cairo.cairo_image_surface_get_data(getApi().surface);
            int oa, or, og, ob;
            if (OS.BIG_ENDIAN) {
                oa = 0;
                or = 1;
    ...
    void createFromPixbuf(int type, long pixbuf) {
        ...
        getApi().surface = Cairo.cairo_image_surface_create(format, pixbufWidth, pixbufHeight);
        if (getApi().surface == 0)
            SWT.error(SWT.ERROR_NO_HANDLES);
        Cairo.cairo_surface_set_device_scale(getApi().surface, scaleFactor, scaleFactor);
        long data = Cairo.cairo_image_surface_get_data(getApi().surface);
        int cairoStride = Cairo.cairo_image_surface_get_stride(getApi().surface);
        ...
        Cairo.cairo_surface_mark_dirty(getApi().surface);
    }
    ...
    void createMask() {
        ...
        int stride = Cairo.cairo_image_surface_get_stride(getApi().surface);
        long surfaceData = Cairo.cairo_image_surface_get_data(getApi().surface);
    ...
    public boolean equals(Object object) {
        ...
        if (imageDataProvider != null && ((SwtImage) image.getImpl()).imageDataProvider != null) {
            ...
            return getApi().surface == image.surface;
        }
    }
    ...
    public ImageData getImageDataAtCurrentZoom() {
        if (isDisposed())
            SWT.error(SWT.ERROR_GRAPHIC_DISPOSED);
        long surface = ImageList.convertSurface(this.getApi());
    ...
    public static Image gtk_new_from_pixbuf(Device device, int type, long pixbuf) {
        Image image = new Image(device);
        ((SwtImage) image.getImpl()).createFromPixbuf(type, pixbuf);
        image.type = type;
        return image;
    }
    ...
    void init(int width, int height) {
        ...
        /* Create the pixmap */
        if (GTK.GTK4) {
            getApi().surface = Cairo.cairo_image_surface_create(Cairo.CAIRO_FORMAT_RGB24, width, height);
        } else {
            getApi().surface = GDK.gdk_window_create_similar_surface(GDK.gdk_get_default_root_window(), Cairo.CAIRO_CONTENT_COLOR, width, height);
        }
        if (getApi().surface == 0)
            SWT.error(SWT.ERROR_NO_HANDLES);
        ...
        currentDeviceZoom = 100;
        Cairo.cairo_surface_set_device_scale(getApi().surface, 1f, 1f);
        long cairo = Cairo.cairo_create(getApi().surface);
        if (cairo == 0)
            SWT.error(SWT.ERROR_NO_HANDLES);
    ...
    private void init(ImageData image, int zoom) {
        ...
        getApi().surface = Cairo.cairo_image_surface_create(format, imageDataWidth, imageDataHeight);
        if (getApi().surface == 0)
            SWT.error(SWT.ERROR_NO_HANDLES);
        Cairo.cairo_surface_set_device_scale(getApi().surface, scaleFactor, scaleFactor);
        int stride = Cairo.cairo_image_surface_get_stride(getApi().surface);
    ...
    public long internal_new_GC(GCData data) {
        ...
        long gc = Cairo.cairo_create(getApi().surface);
        if (data != null) {
            ...
            data.device = device;
            data.foregroundRGBA = SwtDevice.COLOR_BLACK.handle;
            data.backgroundRGBA = SwtDevice.COLOR_WHITE.handle;
