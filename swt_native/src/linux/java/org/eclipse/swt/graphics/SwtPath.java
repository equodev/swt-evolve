...
public class SwtPath extends SwtResource implements IPath {
    ...
    public SwtPath(Device device, Path api) {
        ...
        getApi().handle = Cairo.cairo_create(surface);
    ...
    public SwtPath(Device device, Path path, float flatness, Path api) {
        ...
        if (copy == 0) {
            Cairo.cairo_destroy(getApi().handle);
            SWT.error(SWT.ERROR_NO_HANDLES);
        }
        Cairo.cairo_append_path(getApi().handle, copy);
    ...
    public void addArc(float x, float y, float width, float height, float startAngle, float arcAngle) {
        ...
        if (width == height) {
            ...
            if (closed)
                Cairo.cairo_move_to(getApi().handle, (x + width / 2f) + width / 2f * Math.cos(angle), (y + height / 2f) + height / 2f * Math.sin(angle));
            if (arcAngle >= 0) {
                Cairo.cairo_arc_negative(getApi().handle, x + width / 2f, y + height / 2f, width / 2f, angle, -(startAngle + arcAngle) * (float) Math.PI / 180);
            } else {
                Cairo.cairo_arc(getApi().handle, x + width / 2f, y + height / 2f, width / 2f, angle, -(startAngle + arcAngle) * (float) Math.PI / 180);
            }
        } else {
            Cairo.cairo_save(getApi().handle);
            Cairo.cairo_translate(getApi().handle, x + width / 2f, y + height / 2f);
            Cairo.cairo_scale(getApi().handle, width / 2f, height / 2f);
            float angle = -startAngle * (float) Math.PI / 180;
            if (closed)
                Cairo.cairo_move_to(getApi().handle, Math.cos(angle), Math.sin(angle));
            if (arcAngle >= 0) {
                Cairo.cairo_arc_negative(getApi().handle, 0, 0, 1, angle, -(startAngle + arcAngle) * (float) Math.PI / 180);
            } else {
                Cairo.cairo_arc(getApi().handle, 0, 0, 1, angle, -(startAngle + arcAngle) * (float) Math.PI / 180);
            }
            Cairo.cairo_restore(getApi().handle);
        }
        closed = false;
    ...
    public void addPath(Path path) {
        ...
        Cairo.cairo_append_path(getApi().handle, copy);
        Cairo.cairo_path_destroy(copy);
        closed = ((SwtPath) path.getImpl()).closed;
    }
    ...
    public void addRectangle(float x, float y, float width, float height) {
        if (isDisposed())
            SWT.error(SWT.ERROR_GRAPHIC_DISPOSED);
        moved = false;
        Cairo.cairo_rectangle(getApi().handle, x, y, width, height);
        closed = true;
    }
    ...
    public void addString(String string, float x, float y, Font font) {
        ...
        moved = false;
        SwtGC.addCairoString(getApi().handle, string, x, y, scaledFont);
        closed = true;
        // Dispose the scaled up font
        scaledFont.dispose();
    }
    ...
    public void close() {
        if (isDisposed())
            SWT.error(SWT.ERROR_GRAPHIC_DISPOSED);
        Cairo.cairo_close_path(getApi().handle);
        moved = false;
        closed = true;
    }
    ...
    public void cubicTo(float cx1, float cy1, float cx2, float cy2, float x, float y) {
        ...
        if (!moved) {
            ...
            Cairo.cairo_get_current_point(getApi().handle, currentX, currentY);
            Cairo.cairo_move_to(getApi().handle, currentX[0], currentY[0]);
            moved = true;
        }
        Cairo.cairo_curve_to(getApi().handle, cx1, cy1, cx2, cy2, x, y);
        closed = false;
    }
    ...
    public void getCurrentPoint(float[] point) {
        ...
        Cairo.cairo_get_current_point(getApi().handle, x, y);
        point[0] = (float) x[0];
        point[1] = (float) y[0];
    }
    ...
    public void lineTo(float x, float y) {
        ...
        Cairo.cairo_line_to(getApi().handle, x, y);
        closed = false;
    }
	...
	*/
        moved = true;
        Cairo.cairo_move_to(getApi().handle, x, y);
        closed = true;
    }
