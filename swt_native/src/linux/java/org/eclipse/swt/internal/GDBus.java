...
public class GDBus {
    ...
    public static class GDBusMethod {
        ...
        public GDBusMethod(String name, String[][] inputArgs, String[][] outputArgs, Function<Object[], Object[]> userFunction) {
            ...
            for (String[] dataType : outputArgs) {
                // I haven't tested output args, but should work if types get converted properly.
    ...
    public static void init(GDBusMethod[] methods, String appName) {
        ...
        int owner_id = OS.g_bus_own_name(OS.G_BUS_TYPE_SESSION, Converter.javaStringToCString(serviceName), // Allow name to be used by other eclipse instances.
        OS.G_BUS_NAME_OWNER_FLAGS_REPLACE | OS.G_BUS_NAME_OWNER_FLAGS_ALLOW_REPLACEMENT, onBusAcquired.getAddress(), // name_acquired_handler
        onNameAcquired.getAddress(), // name_lost_handler
        onNameLost.getAddress(), // user_data
     ...
     */
    // Callback only called directly by JNI.
    ...
    private static long onBusAcquired(long gDBusConnection, long const_gchar_name, long user_data) {
        ...
        }
        {
            ...
            OS.g_dbus_connection_register_object(gDBusConnection, Converter.javaStringToCString(DBUS_OBJECT_NAME), interface_info, vtable, // user_data
            0, // user_data_free_func
            0, error);
            if (error[0] != 0) {
