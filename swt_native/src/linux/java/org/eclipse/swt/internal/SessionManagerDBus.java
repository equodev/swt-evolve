...
public class SessionManagerDBus {
    ...
    public SessionManagerDBus() {
        ...
        boolean isDisabled = System.getProperty("org.eclipse.swt.internal.SessionManagerDBus.disable") != null || System.getenv("org.eclipse.swt.internal.SessionManagerDBus.disable") != null;
        if (isDisabled)
            return;
        start();
    }
    ...
    private synchronized void stop() {
        ...
        if ((sessionManagerProxy != 0) && (clientObjectPath != null)) {
            long args = OS.g_variant_new(//$NON-NLS-1$
            Converter.javaStringToCString("(o)"), Converter.javaStringToCString(clientObjectPath));
            long[] error = new long[1];
            OS.g_dbus_proxy_call_sync(sessionManagerProxy, //$NON-NLS-1$
            Converter.javaStringToCString("UnregisterClient"), args, OS.G_DBUS_CALL_FLAGS_NONE, dbusTimeoutMsec, 0, error);
            if (error[0] != 0) {
                System.err.format("SWT SessionManagerDBus: Failed to UnregisterClient: %s%n", extractFreeGError(error[0]));
            }
    ...
    private void sendEndSessionResponse(boolean is_ok, String reason, long responseID) {
        ...
        long args = OS.g_variant_new(//$NON-NLS-1$
        Converter.javaStringToCString("(bs)"), is_ok, Converter.javaStringToCString(reason));
        long[] error = new long[1];
        OS.g_dbus_proxy_call(clientProxy, //$NON-NLS-1$
        Converter.javaStringToCString("EndSessionResponse"), args, OS.G_DBUS_CALL_FLAGS_NONE, dbusTimeoutMsec, 0, 0, error);
        if (error[0] != 0) {
            System.err.format("SWT SessionManagerDBus: Failed to EndSessionResponse: %s%n", extractFreeGError(error[0]));
        }
    }
    ...
    private long connectSessionManager(String dbusName, String objectPath, String interfaceName) {
        int sessionManagerFlags = OS.G_DBUS_PROXY_FLAGS_DO_NOT_AUTO_START | OS.G_DBUS_PROXY_FLAGS_DO_NOT_LOAD_PROPERTIES | OS.G_DBUS_PROXY_FLAGS_DO_NOT_CONNECT_SIGNALS;
        long[] error = new long[1];
        long proxy = OS.g_dbus_proxy_new_for_bus_sync(OS.G_BUS_TYPE_SESSION, sessionManagerFlags, 0, Converter.javaStringToCString(dbusName), Converter.javaStringToCString(objectPath), Converter.javaStringToCString(interfaceName), 0, error);
    ...
    private boolean connectSessionManager() {
        long proxyGnome = connectSessionManager(//$NON-NLS-1$
        ...
        long proxyXFCE = connectSessionManager(//$NON-NLS-1$
    ...
    private String registerClient(String appID, String clientStartupID) {
        long args = OS.g_variant_new(//$NON-NLS-1$
        Converter.javaStringToCString("(ss)"), Converter.javaStringToCString(appID), Converter.javaStringToCString(clientStartupID));
        long[] error = new long[1];
        long clientInfo = OS.g_dbus_proxy_call_sync(sessionManagerProxy, //$NON-NLS-1$
        Converter.javaStringToCString("RegisterClient"), args, OS.G_DBUS_CALL_FLAGS_NONE, dbusTimeoutMsec, 0, error);
        if (clientInfo == 0)
    ...
    private boolean registerClient() {
        ...
        if (//$NON-NLS-1$
        !isGnome && errorText.startsWith("GDBus.Error:org.freedesktop.DBus.Error.UnknownMethod: "))
            return false;
    ...
    private boolean connectClientSignal() {
        ...
        long[] error = new long[1];
        clientProxy = OS.g_dbus_proxy_new_for_bus_sync(OS.G_BUS_TYPE_SESSION, 0, 0, Converter.javaStringToCString(dbusName), Converter.javaStringToCString(clientObjectPath), Converter.javaStringToCString(interfaceName), 0, error);
        if (clientProxy == 0) {
            System.err.format("SWT SessionManagerDBus: Failed to connect to Client: %s%n", extractFreeGError(error[0]));
            return false;
        }
        // Finally, we're ready to connect 'g-signal'.
        // The rest of the code merely prepares for this.
        //$NON-NLS-1$
        g_signal_callback = new Callback(this, "g_signal_handler", 5);
        g_signal_callbackid = OS.g_signal_connect(clientProxy, //$NON-NLS-1$
        Converter.javaStringToCString("g-signal"), g_signal_callback.getAddress(), 0);
        return true;
    }
}
