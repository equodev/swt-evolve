...
public class SWTCTabFolder extends SWTComposite implements ICTabFolder {
    ...
    SWTCTabItem[] items = new SWTCTabItem[0];
    ...
    SWTToolBar chevronTb;

    SWTToolItem chevronItem;
     ...
     * @see #getStyle()
     */
    public SWTCTabFolder(SWTComposite parent, int style) {
    ...
    static int checkStyle(SWTComposite parent, int style) {
    ...
    Rectangle[] computeControlBounds(Point size, boolean[][] position) {
        ...
        if (showChevron) {
            ...
            if (lastIndex != -1) {
                SWTCTabItem lastItem = (SWTCTabItem) (items[lastIndex]);
                int w = lastItem.x + lastItem.width + SPACING;
                if (!simple && lastIndex == selectedIndex)
                    w -= (renderer.curveIndent - 7);
...
*/
    ...
    Image createButtonImage(SWTDisplay display, int button) {
        return new Image(display, (ImageDataProvider) zoom -> {
            GC tempGC = new GC(SWTCTabFolder.this);
    ...
    private void notifyItemCountChange() {
        CTabFolderEvent e = new CTabFolderEvent(CTabFolder.getInstance(this));
        for (CTabFolder2Listener listener : folderListeners) {
            listener.itemsCount(e);
        }
    }

    void createItem(SWTCTabItem item, int index) {
        if (0 > index || index > getItemCount())
            SWT.error(SWT.ERROR_INVALID_RANGE);
        item.parent = this;
        SWTCTabItem[] newItems = new SWTCTabItem[items.length + 1];
    ...
    void destroyItem(SWTCTabItem item) {
        if (inDispose)
            return;
        ...
        if (index == -1)
            return;
        if (items.length == 1) {
            items = new SWTCTabItem[0];
            priority = new int[0];
            firstIndex = -1;
            selectedIndex = -1;
            SWTControl control = item.control;
        ...
        if (selectedIndex == index) {
            SWTControl control = (SWTControl) (item.getControl());
            selectedIndex = -1;
    ...
    SWTToolBar getChevron() {
        if (chevronTb == null) {
            chevronTb = new SWTToolBar(this, SWT.FLAT);
            initAccessibleChevronTb();
            addTabControl(chevronTb, SWT.TRAIL, -1, false);
        }
        if (chevronItem == null) {
            chevronItem = new SWTToolItem(chevronTb, SWT.PUSH);
	 ...
	 */
        ...
        if (size.x <= trim.width)
            return null;
        for (int element : priority) {
            SWTCTabItem item = (SWTCTabItem) (items[element]);
	 ...
	 */
        //checkWidget();
        SWTCTabItem[] tabItems = new SWTCTabItem[items.length];
 ...
 */
     ...
     * </ul>
     */
    public int indexOf(ICTabItem item_) {
    ...
    void initAccessible() {
        ...
        accessible.addAccessibleListener(new AccessibleAdapter() {
            ...
            public void getName(AccessibleEvent e) {
                SWTCTabItem item = (SWTCTabItem) (null);
        ...
        accessible.addAccessibleControlListener(new AccessibleControlAdapter() {
            ...
            public void getLocation(AccessibleControlEvent e) {
                ...
                if (childID == ACC.CHILDID_SELF) {
                    location = getBounds();
                    pt = ((SWTComposite) (getParent())).toDisplay(location.x, location.y);
                } else {
                ...
                if (!mru) {
                    ...
                    if (current + offset >= 0 && current + offset < idx) {
                        ...
                        if (showChevron) {
                            ...
                            chevronRect = event.display.map(Control.getInstance(chevronTb), Control.getInstance(this), chevronRect);
                            CTabFolderEvent e = new CTabFolderEvent(CTabFolder.getInstance(this));
                            e.time = event.time;
                            e.x = chevronRect.x;
                            e.y = chevronRect.y;
    ...
    void onMenuDetect(Event event) {
        if (event.detail == SWT.MENU_KEYBOARD) {
            if (selectedIndex != -1) {
                SWTCTabItem item = (SWTCTabItem) (items[selectedIndex]);
                Rectangle rect = ((SWTDisplay) (getDisplay())).map(this, null, item.getBounds());
    ...
    void onMouseDoubleClick(Event event) {
        if (event.button != 1 || (event.stateMask & SWT.BUTTON2) != 0 || (event.stateMask & SWT.BUTTON3) != 0)
            return;
        Event e = new Event();
        e.item = Widget.getInstance(((SWTCTabItem) (getItem(new Point(event.x, event.y)))));
        if (e.item != null) {
    ...
    void onMouse(Event event) {
        ...
        switch(event.type) {
            ...
            case SWT.MouseExit:
                {
                    for (int i = 0; i < items.length; i++) {
                        SWTCTabItem item = (SWTCTabItem) (items[i]);
                    ...
                    if (hoverTb && hoverRect.contains(x, y) && !hovering) {
                        ...
                        event.display.timerExec(2000, new Runnable() {
                            ...
                            public void run() {
                                ...
                                if (hovering) {
                                    SWTDisplay display = (SWTDisplay) (getDisplay());
                                    SWTControl c = (SWTControl) (display.getCursorControl());
                                    boolean reschedule = false;
                                    if (c != null) {
                                        for (SWTControl control : controls) {
                                            SWTControl temp = (SWTControl) (c);
                                            do {
                                                if (temp.equals(control)) {
                                                    reschedule = true;
                                                } else {
                                                    temp = ((SWTComposite) (temp.getParent()));
                                                    if (temp == null || temp.equals(SWTCTabFolder.this))
                                                        break;
                                                }
                                            } while (!reschedule);
                                            if (reschedule)
            ...
            case SWT.MouseUp:
                {
                    ...
                    if (item != null) {
                        if (item.closeRect.contains(x, y)) {
                            ...
                            CTabFolderEvent e = new CTabFolderEvent(CTabFolder.getInstance(this));
                            e.time = event.time;
                            e.item = Widget.getInstance(item);
                            e.doit = true;
                            for (CTabFolder2Listener listener : folderListeners) {
                                listener.close(e);
                            }
                            ...
                            if (!isDisposed() && item.isDisposed()) {
                                ...
                                SWTCTabItem nextItem = (SWTCTabItem) (getItem(pt));
                                if (nextItem != null) {
    ...
    void onSelection(Event event) {
        ...
        if (event.widget == Widget.getInstance(maxItem)) {
            CTabFolderEvent e = new CTabFolderEvent(CTabFolder.getInstance(this));
            e.time = event.time;
            for (CTabFolder2Listener folderListener : folderListeners) {
                if (maximized) {
        ...
        } else if (event.widget == Widget.getInstance(minItem)) {
            CTabFolderEvent e = new CTabFolderEvent(CTabFolder.getInstance(this));
            e.time = event.time;
            for (CTabFolder2Listener folderListener : folderListeners) {
                if (minimized) {
        ...
        } else if (event.widget == Widget.getInstance(chevronItem)) {
            Rectangle chevronRect = chevronItem.getBounds();
            chevronRect = event.display.map(Control.getInstance(chevronTb), Control.getInstance(this), chevronRect);
            CTabFolderEvent e = new CTabFolderEvent(CTabFolder.getInstance(this));
            e.time = event.time;
    ...
    void onTraverse(Event event) {
        ...
        switch(event.detail) {
            ...
            case SWT.TRAVERSE_TAB_PREVIOUS:
                SWTControl focusControl = (SWTControl) (getDisplay().getFocusControl());
    ...
    public void setBackground(Color[] colors, int[] percents, boolean vertical) {
        ...
        if (colors != null) {
            ...
            if (((SWTDisplay) (getDisplay())).getDepth() < 15) {
    ...
    void updateButtons() {
        ...
        if (showMax) {
            if (minMaxTb == null) {
                minMaxTb = new SWTToolBar(this, SWT.FLAT);
                initAccessibleMinMaxTb();
                addTabControl(minMaxTb, SWT.TRAIL, 0, false);
            }
            if (maxItem == null) {
                maxItem = new SWTToolItem(minMaxTb, SWT.PUSH);
                if (maxImage == null) {
        ...
        if (showMin) {
            ...
            if (minItem == null) {
                minItem = new SWTToolItem(minMaxTb, SWT.PUSH, 0);
                if (minImage == null) {
    ...
    private void updateChevronImage(boolean styleChange) {
        ...
        if (chevronImage != null)
            chevronImage.dispose();
        chevronImage = createButtonImage(((SWTDisplay) (getDisplay())), CTabFolderRenderer.PART_CHEVRON_BUTTON);
	...
	*/
        ...
        if (fixFocus) {
            SWTCTabItem item = (SWTCTabItem) (getSelection());
            if (item != null) {
                if (item.setFocus())
                    return true;
            }
    ...
    boolean isAncestor(SWTControl control) {
        while (control != null && control != this && !(control instanceof SWTShell)) {
            control = ((SWTComposite) (control.getParent()));
        }
        return control == this;
    }
     ...
     * </ul>
     */
    public void setInsertMark(ICTabItem item_, boolean after) {
        SWTCTabItem item = (SWTCTabItem) item_;
        checkWidget();
    }
    ...
    boolean setItemLocation(GC gc) {
        ...
        if (single) {
            // off screen
            int defaultX = ((SWTDisplay) (getDisplay())).getBounds().width + 10;
            for (int i = 0; i < items.length; i++) {
                SWTCTabItem item = (SWTCTabItem) (items[i]);
                if (i == selectedIndex) {
            ...
            for (int i = 0; i < priority.length; i++) {
                SWTCTabItem item = (SWTCTabItem) (items[priority[i]]);
                width += item.width;
    ...
    void setItemOrder(int[] indices) {
        ...
        SWTCTabItem[] temp = new SWTCTabItem[items.length];
    ...
    boolean setItemSize(GC gc) {
        ...
        if (size.x <= 0 || size.y <= 0)
            return changed;
        SWTToolBar chevron = (SWTToolBar) (getChevron());
        if (chevron != null)
        ...
        if (single) {
            ...
            if (selectedIndex != -1) {
                SWTCTabItem tab = (SWTCTabItem) (items[selectedIndex]);
        ...
        for (int i = 0; i < items.length; i++) {
            SWTCTabItem tab = (SWTCTabItem) (items[i]);
            int width = widths[i];
            changed = true;
            tab.shortenedText = null;
            tab.shortenedTextWidth = 0;
    ...
    public void setMaximized(boolean maximize) {
        ...
        if (minMaxTb != null && maxItem != null) {
            if (maxImage != null)
                maxImage.dispose();
            maxImage = createButtonImage(((SWTDisplay) (getDisplay())), CTabFolderRenderer.PART_MAX_BUTTON);
            maxItem.setImage(maxImage);
            //$NON-NLS-1$ //$NON-NLS-2$
            maxItem.setToolTipText(maximized ? SWT.getMessage("SWT_Restore") : SWT.getMessage("SWT_Maximize"));
        }
    ...
    public void setMinimized(boolean minimize) {
        ...
        if (minMaxTb != null && minItem != null) {
            if (minImage != null)
                minImage.dispose();
            minImage = createButtonImage(((SWTDisplay) (getDisplay())), CTabFolderRenderer.PART_MIN_BUTTON);
            minItem.setImage(minImage);
            //$NON-NLS-1$ //$NON-NLS-2$
            minItem.setToolTipText(minimized ? SWT.getMessage("SWT_Restore") : SWT.getMessage("SWT_Minimize"));
        }
     ...
     * </ul>
     */
    public void setSelection(ICTabItem item_) {
        SWTCTabItem item = (SWTCTabItem) item_;
        checkWidget();
        if (item == null)
            SWT.error(SWT.ERROR_NULL_ARGUMENT);
    ...
    public void setSelection(int index) {
        ...
        if (index < 0 || index >= items.length)
            return;
        SWTCTabItem selection = (SWTCTabItem) (items[index]);
        if (selectedIndex == index) {
            showItem(selection);
            return;
        }
        ...
        SWTControl newControl = selection.control;
        SWTControl oldControl = (SWTControl) (null);
        if (oldIndex != -1) {
    ...
    void setSelection(int index, boolean notify) {
        ...
        if (notify && selectedIndex != oldSelectedIndex && selectedIndex != -1) {
            ...
            event.item = Widget.getInstance(((SWTCTabItem) (getItem(selectedIndex))));
    ...
    public void setSelectionBackground(Color color) {
        ...
        if (selectionBackground == color)
            return;
        if (color == null)
            color = ((SWTDisplay) (getDisplay())).getSystemColor(SELECTION_BACKGROUND);
 ...
 */
    ...
    public void setSelectionForeground(Color color) {
        ...
        if (selectionForeground == color)
            return;
        if (color == null)
            color = ((SWTDisplay) (getDisplay())).getSystemColor(SELECTION_FOREGROUND);
     ...
     * @since 2.1
     */
    public void setTopRight(IControl control_) {
     ...
     * @since 3.0
     */
    public void setTopRight(IControl control_, int alignment) {
        SWTControl control = (SWTControl) control_;
        checkWidget();
        if (alignment != SWT.RIGHT && alignment != SWT.FILL && alignment != (SWT.RIGHT | SWT.WRAP)) {
            SWT.error(SWT.ERROR_INVALID_ARGUMENT);
        }
        if (control != null && (control.isDisposed() || ((SWTComposite) (control.getParent())) != this)) {
            SWT.error(SWT.ERROR_INVALID_ARGUMENT);
        }
     ...
     * @since 2.0
     */
    public void showItem(ICTabItem item_) {
        SWTCTabItem item = (SWTCTabItem) item_;
        checkWidget();
        if (item == null)
            SWT.error(SWT.ERROR_NULL_ARGUMENT);
    ...
    void showList(Rectangle rect) {
        ...
        for (SWTCTabItem tab : items) {
            if (tab.showing)
                continue;
            SWTMenuItem item = new SWTMenuItem(showMenu, SWT.NONE);
        ...
        Point location = ((SWTDisplay) (getDisplay())).map(this, null, x, y);
    ...
    public void showSelection() {
        checkWidget();
        if (selectedIndex != -1) {
            showItem(((SWTCTabItem) (getSelection())));
        }
    }
    ...
    boolean updateItems(int showIndex) {
        ...
        if (changed && getToolTipText() != null) {
            Point pt = ((SWTDisplay) (getDisplay())).getCursorLocation();
            pt = toControl(pt);
    ...
    void updateFolder(int flags) {
        ...
        ((SWTDisplay) (getDisplay())).asyncExec(updateRun);
    }
    ...
    void updateBkImages(boolean colorChanged) {
        if (controls != null && controls.length > 0) {
            ...
            for (int i = 0; i < controls.length; i++) {
                SWTControl control = (SWTControl) (controls[i]);
                if (!control.isDisposed()) {
                    if (hovering) {
                        if (control instanceof SWTComposite)
                            ((SWTComposite) control).setBackgroundMode(SWT.INHERIT_NONE);
                        control.setBackgroundImage(null);
                        control.setBackground(getBackground());
                    } else {
                        if (control instanceof SWTComposite)
                            ((SWTComposite) control).setBackgroundMode(SWT.INHERIT_DEFAULT);
                        ...
                        if (wrapped || gradientColors == null) {
                            ...
                            if (colorChanged || !bounds.equals(bkImageBounds[i])) {
                                ...
                                if (controlBkImages[i] != null)
                                    controlBkImages[i].dispose();
                                controlBkImages[i] = new Image(((SWTDisplay) (control.getDisplay())), bounds);
    ...
    String _getToolTip(int x, int y) {
        SWTCTabItem item = (SWTCTabItem) (getItem(new Point(x, y)));
        if (item == null)
            return null;
        if (!item.showing)
            return null;
     ...
     */
    /*public*/
    void addTabControl(SWTControl control, int flags) {
        checkWidget();
        addTabControl(control, flags, -1, true);
    }

    void addTabControl(SWTControl control, int flags, int index, boolean update) {
        switch(flags) {
            case SWT.TRAIL:
        ...
        if (control != null && ((SWTComposite) (control.getParent())) != this) {
            SWT.error(SWT.ERROR_INVALID_ARGUMENT);
        }
        //check for duplicates
        for (SWTControl ctrl : controls) {
            if (ctrl == control) {
                SWT.error(SWT.ERROR_INVALID_ARGUMENT);
            }
        }
        ...
        //Grow all 4 arrays
        SWTControl[] newControls = new SWTControl[length + 1];
     ...
     */
    /*public*/
    void removeTabControl(SWTControl control) {
        checkWidget();
        removeTabControl(control, true);
    }

    void removeTabControl(SWTControl control, boolean update) {
        if (control != null && ((SWTComposite) (control.getParent())) != this) {
            SWT.error(SWT.ERROR_INVALID_ARGUMENT);
        }
        int index = -1;
        ...
        if (controls.length == 1) {
            ...
            SWTControl[] newControls = new SWTControl[controls.length - 1];
