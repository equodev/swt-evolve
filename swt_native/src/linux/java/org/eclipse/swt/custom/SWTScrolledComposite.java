 ...
 */
public class SWTScrolledComposite extends SWTComposite implements IScrolledComposite {
     ...
     * @see #getStyle()
     */
    public SWTScrolledComposite(SWTComposite parent, int style) {
        super(parent, checkStyle(style));
        super.setLayout(new ScrolledCompositeLayout());
        SWTScrollBar hBar = ((SWTScrollBar) (getHorizontalBar()));
        if (hBar != null) {
            hBar.setVisible(false);
            hBar.addListener(SWT.Selection, e -> hScroll());
        }
        SWTScrollBar vBar = ((SWTScrollBar) (getVerticalBar()));
        if (vBar != null) {
        ...
        filter = event -> {
            if (event.type == SWT.FocusIn) {
                ...
                if (wi.delegate instanceof SWTControl w) {
                    showNextFocusedControl = ((SWTShell) (w.getDisplay().getActiveShell())) == ((SWTShell) (((SWTControl) w).getShell()));
                }
            }
        };
        addDisposeListener(e -> {
            ((SWTDisplay) (getDisplay())).removeFilter(SWT.FocusIn, filter);
            ((SWTDisplay) (getDisplay())).removeFilter(SWT.FocusOut, filter);
        });
    }
    ...
    boolean contains(SWTControl control) {
        if (control == null || control.isDisposed())
            return false;
        SWTComposite parent = (SWTComposite) (control.getParent());
        while (parent != null && !(parent instanceof SWTShell)) {
            if (this == parent)
                return true;
     ...
     * </ul>
     */
    public void setContent(IControl content_) {
        SWTControl content = (SWTControl) content_;
        checkWidget();
     ...
     * @since 3.4
     */
    public void showControl(IControl control_) {
        ...
        if (!contains(control))
            SWT.error(SWT.ERROR_INVALID_ARGUMENT);
        Rectangle itemRect = ((SWTDisplay) (getDisplay())).map(((SWTComposite) (control.getParent())), this, control.getBounds());
