...
public class SWTAnimatedProgress extends SWTCanvas implements IAnimatedProgress {
     ...
     * @see #getStyle()
     */
    public SWTAnimatedProgress(SWTComposite parent, int style) {
    ...
    void paintStripes(GC gc) {
        ...
        gc.setLineWidth(2);
        gc.setClipping(rect);
        Color color = ((SWTDisplay) (getDisplay())).getSystemColor(SWT.COLOR_LIST_SELECTION);
    ...
    public synchronized void start() {
        ...
        timer[0] = () -> {
            if (!active)
                return;
            GC gc = new GC(SWTAnimatedProgress.this);
            paintStripes(gc);
            gc.dispose();
