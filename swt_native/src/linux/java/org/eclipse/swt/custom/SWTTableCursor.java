 ...
 */
public class SWTTableCursor extends SWTCanvas implements ITableCursor {

    SWTTable table;

    SWTTableItem row = null;

    SWTTableColumn column = null;
     ...
     * @see Widget#getStyle()
     */
    public SWTTableCursor(SWTTable parent, int style) {
        super(parent, style);
        table = parent;
        setBackground(null);
        setForeground(null);
        ...
        SWTScrollBar hBar = ((SWTScrollBar) (table.getHorizontalBar()));
        if (hBar != null) {
            hBar.addListener(SWT.Selection, resizeListener);
        }
        SWTScrollBar vBar = ((SWTScrollBar) (table.getVerticalBar()));
        if (vBar != null) {
            ...
            case SWT.PAGE_UP:
                {
                    ...
                    if (index == rowIndex) {
                        ...
                        SWTTableItem item = (SWTTableItem) (table.getItem(index));
    ...
    void tableMouseDown(Event event) {
        ...
        SWTTableItem item = (SWTTableItem) (table.getItem(pt));
        if ((table.getStyle() & SWT.FULL_SELECTION) != 0) {
            ...
            for (int i = start; i < end; i++) {
                SWTTableItem nextItem = (SWTTableItem) (table.getItem(i));
        ...
        SWTTableColumn newColumn = (SWTTableColumn) (null);
        int columnCount = table.getColumnCount();
        if (columnCount == 0) {
            if ((table.getStyle() & SWT.FULL_SELECTION) == 0) {
            ...
            for (int i = 0; i < columnCount; i++) {
                ...
                rect.height += lineWidth;
                if (rect.contains(pt)) {
                    newColumn = ((SWTTableColumn) (table.getColumn(i)));
                    break;
                }
            }
            if (newColumn == null) {
                if ((table.getStyle() & SWT.FULL_SELECTION) == 0)
                    return;
                newColumn = ((SWTTableColumn) (table.getColumn(0)));
            }
        }
    ...
    void setRowColumn(int row, int column, boolean notify) {
        SWTTableItem item = (SWTTableItem) (row == -1 ? null : table.getItem(row));
        SWTTableColumn col = (SWTTableColumn) (column == -1 || table.getColumnCount() == 0 ? null : table.getColumn(column));
        setRowColumn(item, col, notify);
    }

    void setRowColumn(SWTTableItem row, SWTTableColumn column, boolean notify) {
     ...
     * </ul>
     */
    public void setSelection(ITableItem row_, int column) {
        SWTTableItem row = (SWTTableItem) row_;
        checkWidget();
        int columnCount = table.getColumnCount();
        int maxColumnIndex = columnCount == 0 ? 0 : columnCount - 1;
