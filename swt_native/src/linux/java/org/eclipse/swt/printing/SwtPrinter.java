...
public final class SwtPrinter extends SwtDevice implements IPrinter {
    ...
    //$NON-NLS-1$
    static boolean disablePrinting = System.getProperty("org.eclipse.swt.internal.gtk.disablePrinting") != null;
    ...
    public static PrinterData[] getPrinterList() {
        ...
        gtk_init();
        //$NON-NLS-1$
        Callback printerCallback = new Callback(SwtPrinter.class, "GtkPrinterFunc_List", 2);
	...
	*/
    ...
    public static PrinterData getDefaultPrinterData() {
        ...
        gtk_init();
        //$NON-NLS-1$
        Callback printerCallback = new Callback(SwtPrinter.class, "GtkPrinterFunc_Default", 2);
        GTK.gtk_enumerate_printers(printerCallback.getAddress(), 0, 0, true);
        printerCallback.dispose();
        return findData;
    }
    ...
    static long gtkPrinterFromPrinterData(PrinterData data) {
        gtk_init();
        //$NON-NLS-1$
        Callback printerCallback = new Callback(SwtPrinter.class, "GtkPrinterFunc_FindNamedPrinter", 2);
        findPrinter = 0;
        findData = data;
        GTK.gtk_enumerate_printers(printerCallback.getAddress(), 0, 0, true);
        printerCallback.dispose();
    ...
    static long GtkPrinterFunc_FindNamedPrinter(long printer, long user_data) {
        ...
        if ((pd.driver.equals(findData.driver) && pd.name.equals(findData.name)) || (pd.driver.equals(GTK_FILE_BACKEND)) && findData.printToFile && findData.driver == null && findData.name == null) {
...
*/
	 ...
	 */
        //$NON-NLS-1$
        GTK.gtk_page_setup_set_orientation(page_setup, restoreInt("orientation"));
        //$NON-NLS-1$
        GTK.gtk_page_setup_set_top_margin(page_setup, restoreDouble("top_margin"), GTK.GTK_UNIT_MM);
        //$NON-NLS-1$
        GTK.gtk_page_setup_set_bottom_margin(page_setup, restoreDouble("bottom_margin"), GTK.GTK_UNIT_MM);
        //$NON-NLS-1$
        GTK.gtk_page_setup_set_left_margin(page_setup, restoreDouble("left_margin"), GTK.GTK_UNIT_MM);
        //$NON-NLS-1$
        GTK.gtk_page_setup_set_right_margin(page_setup, restoreDouble("right_margin"), GTK.GTK_UNIT_MM);
        //$NON-NLS-1$
        byte[] name = restoreBytes("paper_size_name", true);
        //$NON-NLS-1$
        byte[] display_name = restoreBytes("paper_size_display_name", true);
        //$NON-NLS-1$
        byte[] ppd_name = restoreBytes("paper_size_ppd_name", true);
        //$NON-NLS-1$
        double width = restoreDouble("paper_size_width");
        //$NON-NLS-1$
        double height = restoreDouble("paper_size_height");
        //$NON-NLS-1$
        boolean custom = restoreBoolean("paper_size_is_custom");
        long paper_size = 0;
        if (custom) {
    ...
    public void init() {
        ...
        if (data.duplex != SWT.DEFAULT) {
            int duplex = data.duplex == PrinterData.DUPLEX_LONG_EDGE ? GTK.GTK_PRINT_DUPLEX_HORIZONTAL : data.duplex == PrinterData.DUPLEX_SHORT_EDGE ? GTK.GTK_PRINT_DUPLEX_VERTICAL : GTK.GTK_PRINT_DUPLEX_SIMPLEX;
