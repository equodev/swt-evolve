 ...
 */
public class SwtPrintDialog extends SwtDialog implements IPrintDialog {
    ...
    public SwtPrintDialog(Shell parent, int style, PrintDialog api) {
        super(parent, checkStyleBit(parent, style), api);
        checkSubclass();
    }
    ...
    public PrinterData open() {
        ...
        GTK.gtk_print_unix_dialog_set_manual_capabilities(handle, GTK.GTK_PRINT_CAPABILITY_COLLATE | GTK.GTK_PRINT_CAPABILITY_COPIES | GTK.GTK_PRINT_CAPABILITY_PAGE_SET);
        ...
        if (printerName == null && printerData.printToFile) {
            ...
            long printer = SwtPrinter.gtkPrinterFromPrinterData(printerData);
            if (printer != 0) {
                PrinterData data = SwtPrinter.printerDataFromGtkPrinter(printer);
        ...
        if ((printerData.printToFile || SwtPrinter.GTK_FILE_BACKEND.equals(printerData.driver)) && printerData.fileName != null) {
            // TODO: GTK_FILE_BACKEND is not GTK API (see gtk bug 345590)
            byte[] uri = SwtPrinter.uriFromFilename(printerData.fileName);
            if (uri != null) {
	 ...
	 */
        ...
        if (printerData.duplex != SWT.DEFAULT) {
            int duplex = printerData.duplex == PrinterData.DUPLEX_LONG_EDGE ? GTK.GTK_PRINT_DUPLEX_HORIZONTAL : printerData.duplex == PrinterData.DUPLEX_SHORT_EDGE ? GTK.GTK_PRINT_DUPLEX_VERTICAL : GTK.GTK_PRINT_DUPLEX_SIMPLEX;
        ...
        Display display = getParent() != null ? getParent().getDisplay() : SwtDisplay.getCurrent();
        int signalId = 0;
        long hookId = 0;
        ...
        //$NON-NLS-1$
        String key = "org.eclipse.swt.internal.gtk.externalEventLoop";
        ...
        if (response == GTK.GTK_RESPONSE_OK) {
            ...
            if (printer != 0) {
                ...
                // must unref
                settings = GTK.gtk_print_unix_dialog_get_settings(handle);
                // do not unref
                page_setup = GTK.gtk_print_unix_dialog_get_page_setup(handle);
                data = SwtPrinter.printerDataFromGtkPrinter(printer);
                ...
                // TODO: GTK_FILE_BACKEND is not GTK API (see gtk bug 345590)
                data.printToFile = SwtPrinter.GTK_FILE_BACKEND.equals(data.driver);
                if (data.printToFile) {
                ...
                data.duplex = duplex == GTK.GTK_PRINT_DUPLEX_HORIZONTAL ? PrinterData.DUPLEX_LONG_EDGE : duplex == GTK.GTK_PRINT_DUPLEX_VERTICAL ? PrinterData.DUPLEX_SHORT_EDGE : PrinterData.DUPLEX_NONE;
                ...
                // extra null terminator after print_settings and before page_setup
                index++;
			 ...
			 */
                //$NON-NLS-1$
                store("orientation", GTK.gtk_page_setup_get_orientation(page_setup));
                //$NON-NLS-1$
                store("top_margin", GTK.gtk_page_setup_get_top_margin(page_setup, GTK.GTK_UNIT_MM));
                //$NON-NLS-1$
                store("bottom_margin", GTK.gtk_page_setup_get_bottom_margin(page_setup, GTK.GTK_UNIT_MM));
                //$NON-NLS-1$
                store("left_margin", GTK.gtk_page_setup_get_left_margin(page_setup, GTK.GTK_UNIT_MM));
                //$NON-NLS-1$
                store("right_margin", GTK.gtk_page_setup_get_right_margin(page_setup, GTK.GTK_UNIT_MM));
                //$NON-NLS-1$
                long paper_size = GTK.gtk_page_setup_get_paper_size(page_setup);
                //$NON-NLS-1$
                storeBytes("paper_size_name", GTK.gtk_paper_size_get_name(paper_size));
                //$NON-NLS-1$
                storeBytes("paper_size_display_name", GTK.gtk_paper_size_get_display_name(paper_size));
                //$NON-NLS-1$
                storeBytes("paper_size_ppd_name", GTK.gtk_paper_size_get_ppd_name(paper_size));
                //$NON-NLS-1$
                store("paper_size_width", GTK.gtk_paper_size_get_width(paper_size, GTK.GTK_UNIT_MM));
                //$NON-NLS-1$
