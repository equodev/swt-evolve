 ...
 */
public class SWTDropTarget extends SWTWidget implements IDropTarget {
    ...
    static {
        //$NON-NLS-1$
        Drag_Motion = new Callback(SWTDropTarget.class, "Drag_Motion", 5);
        //$NON-NLS-1$
        Drag_Leave = new Callback(SWTDropTarget.class, "Drag_Leave", 3);
        //$NON-NLS-1$
        Drag_Data_Received = new Callback(SWTDropTarget.class, "Drag_Data_Received", 7);
        //$NON-NLS-1$
        Drag_Drop = new Callback(SWTDropTarget.class, "Drag_Drop", 5);
    }
     ...
     *  @see DND#DROP_LINK
     */
    public SWTDropTarget(SWTControl control, int style) {
        ...
        if (GTK.GTK4) {
            ...
            controlListener = event -> {
                if (!SWTDropTarget.this.isDisposed()) {
                    SWTDropTarget.this.dispose();
                }
            };
            ...
            dragOverHeartbeat = () -> {
                ...
                if (time < dragOverStart) {
                    ...
                    if (dropEffect != null) {
                        event.item = Widget.getInstance(dropEffect.getItem(dragOverEvent.x, dragOverEvent.y));
                    }
                ...
                control1 = SWTDropTarget.this.control;
                if (control1 == null || control1.isDisposed())
                    return;
                ((SWTDisplay) (control1.getDisplay())).timerExec(delay, dragOverHeartbeat);
            };
        }
    }
    ...
    static long Drag_Data_Received(long widget, long context, long x, long y, long data, long info, long time) {
        SWTDropTarget target = (SWTDropTarget) (FindDropTarget(widget));
        if (target == null)
            return 0;
    ...
    static SWTDropTarget FindDropTarget(long handle) {
        ...
        if (widget == null)
            return null;
        return (SWTDropTarget) widget.getData(DND.DROP_TARGET_KEY);
    }
	...
	*/
        if ((oldKeyOperation == -1) || !((Boolean) control.getData(IS_ACTIVE)).booleanValue()) {
            //drag enter
            selectedDataType = null;
            selectedOperation = DND.DROP_NONE;
        }
    ...
    public void setTransfer(Transfer... transferAgents) {
        if (GTK.GTK4) {
            ...
            if (control instanceof SWTCombo) {
    ...
    boolean setEventData(long context, int x, int y, int time, DNDEvent event) {
        ...
        if (dropEffect != null) {
            event.item = Widget.getInstance(dropEffect.getItem(coordinates.x, coordinates.y));
        }
        return true;
    }
