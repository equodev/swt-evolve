...
public class SwtClipboard implements IClipboard {
    ...
    private Object getContents_gtk4(Transfer transfer, int clipboards) {
        long contents = GTK4.gdk_clipboard_get_content(SwtClipboard.GTKCLIPBOARD);
        if (contents == 0)
            return null;
        ...
        if (transfer.getTypeIds()[0] == (int) GDK.GDK_TYPE_PIXBUF()) {
            ...
            if (pixbufObj != 0) {
                Image img = SwtImage.gtk_new_from_pixbuf(SwtDisplay.getCurrent(), SWT.BITMAP, pixbufObj);
    ...
    public String[] getAvailableTypeNames() {
        checkWidget();
        if (GTK.GTK4) {
            long formatsCStr = GTK4.gdk_content_formats_to_string(GTK4.gdk_clipboard_get_formats(SwtClipboard.GTKCLIPBOARD));
