...
class ContentProviders {
    ...
    private ContentProviders() {
        //$NON-NLS-1$
        gdkContentSerializeFunc = new Callback(this, "gdkContentSerializeFunc", void.class, new Type[] { long.class });
        gdkContentDeserializeFunc = new //$NON-NLS-1$
        Callback(//$NON-NLS-1$
        this, //$NON-NLS-1$
        "gdkContentDeserializeFunc", //$NON-NLS-1$
    ...
    private void gdkContentSerializeFunc(long pSerializer) {
        ...
        if (transferData.type == 0 || transfer == null || clipboardData == null) {
			 ...
			 */
            serializer.return_error(OS.g_error_new_literal(OS.g_io_error_quark(), OS.G_IO_ERROR_FAILED, "Could not convert data"));
            return;
        }
        ...
        if (transferData.result != 1) {
            serializer.return_error(OS.g_error_new_literal(OS.g_io_error_quark(), OS.G_IO_ERROR_FAILED, "Transfer object could not convert data"));
            return;
        }
        GAsyncReadyCallbackHelper.run(new Async() {
            ...
            public void async(long callback) {
                OS.g_output_stream_write_all_async(serializer.output_stream(), transferData.pValue, transferData.length, serializer.priority(), serializer.cancellable(), callback, 0);
            }
    ...
    private void gdkContentDeserializeFunc(long pDeserializer) {
        ...
        if (transferData.type == 0 || transfer == null) {
			 ...
			 * declared this as being able to convert the data
			 */
            deserializer.return_error(OS.g_error_new_literal(OS.g_io_error_quark(), OS.G_IO_ERROR_FAILED, "Could not convert data"));
        }
        ...
        GAsyncReadyCallbackHelper.run(new Async() {
            ...
            @Override
            public void async(long callback) {
                OS.g_output_stream_splice_async(memoryStream, deserializer.input_stream(), OS.G_OUTPUT_STREAM_SPLICE_CLOSE_SOURCE | OS.G_OUTPUT_STREAM_SPLICE_CLOSE_TARGET, deserializer.priority(), deserializer.cancellable(), callback, 0);
            }
            ...
            public void callback(long result) {
                ...
                if (size < 0) {
					 ...
					 * until here to error.
					 */
                    deserializer.return_error(OS.g_error_new_literal(OS.g_io_error_quark(), OS.G_IO_ERROR_MESSAGE_TOO_LARGE, "Could not convert data because transfer size was too big " + size));
                    return;
                }
    ...
    private String transferKey(Transfer transfer) {
        ...
        return "SwtTransfer_" + lowerCaseSafeClassName + "_" + ((SwtTransfer) transfer.getImpl()).id;
    }
