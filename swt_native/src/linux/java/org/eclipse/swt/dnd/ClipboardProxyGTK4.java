...
class ClipboardProxyGTK4 {
    ...
    void clear(Clipboard owner, int clipboards, boolean takeOwnership) {
        if ((clipboards & DND.CLIPBOARD) != 0) {
            if (takeOwnership || (activeClipboard == owner && GTK4.gdk_clipboard_is_local(SwtClipboard.GTKCLIPBOARD))) {
                GTK4.gdk_clipboard_set_content(SwtClipboard.GTKCLIPBOARD, 0);
        ...
        if (takeOwnership || ((clipboards & DND.SELECTION_CLIPBOARD) != 0 && activePrimaryClipboard == owner)) {
            if (GTK4.gdk_clipboard_is_local(SwtClipboard.GTKPRIMARYCLIPBOARD)) {
                GTK4.gdk_clipboard_set_content(SwtClipboard.GTKPRIMARYCLIPBOARD, 0);
    ...
    private void dispose() {
        ...
        CompletableFuture<Boolean> storeClipboardFuture = storeClipboard(SwtClipboard.GTKCLIPBOARD);
        CompletableFuture<Boolean> storeSelectionFuture = storeClipboard(SwtClipboard.GTKPRIMARYCLIPBOARD);
    ...
    boolean setData(Clipboard owner, Object[] data, Transfer[] dataTypes, int clipboards) {
        ...
        if ((clipboards & DND.CLIPBOARD) != 0 || (clipboards & DND.SELECTION_CLIPBOARD) != 0) {
            long clipboard = clipboards == DND.SELECTION_CLIPBOARD ? SwtClipboard.GTKPRIMARYCLIPBOARD : SwtClipboard.GTKCLIPBOARD;
