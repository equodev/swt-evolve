...
public class SwtLabel extends SwtControl implements ILabel {
    ...
    public void addRelation(Control control) {
        ...
        if (labelHandle == 0)
            return;
        if (control.getImpl() instanceof SwtControl) {
            ((SwtControl) control.getImpl())._getAccessible().addRelation(ACC.RELATION_LABELLED_BY, _getAccessible());
        }
	...
	*/
        boolean fixWrap = labelHandle != 0 && (getApi().style & SWT.WRAP) != 0 && GTK.gtk_widget_get_visible(labelHandle);
        ...
        if (fixWrap) {
            ...
            if (frameHandle != 0) {
                ...
                GTK.gtk_widget_get_size_request(getApi().handle, reqWidth, reqHeight);
                GTK.gtk_widget_set_size_request(getApi().handle, wHint, hHint);
                size = computeNativeSize(frameHandle, -1, -1, changed);
                GTK.gtk_widget_set_size_request(getApi().handle, reqWidth[0], reqHeight[0]);
            } else {
	...
	*/
    ...
    void createHandle(int index) {
        ...
        if ((getApi().style & SWT.SEPARATOR) != 0) {
            if ((getApi().style & SWT.HORIZONTAL) != 0) {
                getApi().handle = GTK.gtk_separator_new(GTK.GTK_ORIENTATION_HORIZONTAL);
                if (getApi().handle != 0) {
                    GTK.gtk_widget_set_valign(getApi().handle, GTK.GTK_ALIGN_CENTER);
                }
            } else {
                getApi().handle = GTK.gtk_separator_new(GTK.GTK_ORIENTATION_VERTICAL);
                if (getApi().handle != 0) {
                    GTK.gtk_widget_set_halign(getApi().handle, GTK.GTK_ALIGN_CENTER);
                }
            }
            if (getApi().handle == 0)
                error(SWT.ERROR_NO_HANDLES);
            ...
            if (GTK.GTK4) {
                ...
                GTK4.gtk_box_append(getApi().handle, imageHandle);
                gtk_box_set_child_packing(getApi().handle, labelHandle, true, true, 0, GTK.GTK_PACK_START);
                gtk_box_set_child_packing(getApi().handle, imageHandle, true, true, 0, GTK.GTK_PACK_START);
            } else {
                getApi().handle = GTK3.gtk_event_box_new();
                if (getApi().handle == 0)
                    error(SWT.ERROR_NO_HANDLES);
        ...
        if ((getApi().style & SWT.BORDER) != 0) {
            ...
            if (GTK.GTK4) {
                ...
                GTK4.gtk_frame_set_child(frameHandle, getApi().handle);
            } else {
                GTK3.gtk_container_add(fixedHandle, frameHandle);
                GTK3.gtk_container_add(frameHandle, getApi().handle);
    ...
    void resizeHandle(int width, int height) {
        ...
        long child = frameHandle != 0 ? frameHandle : getApi().handle;
    ...
    void setAlignment() {
        if ((getApi().style & SWT.LEFT) != 0) {
            //Aligns widget
            gtk_widget_set_align(labelHandle, GTK.GTK_ALIGN_START, GTK.GTK_ALIGN_START);
            //Aligns text inside the widget.
            gtk_label_set_align(0.0f, 0.0f);
        ...
        if ((getApi().style & SWT.CENTER) != 0) {
            //Aligns widget
            gtk_widget_set_align(labelHandle, GTK.GTK_ALIGN_CENTER, GTK.GTK_ALIGN_START);
            //Aligns text inside the widget.
            gtk_label_set_align(0.5f, 0.0f);
        ...
        if ((getApi().style & SWT.RIGHT) != 0) {
            //Aligns widget.
            gtk_widget_set_align(labelHandle, GTK.GTK_ALIGN_END, GTK.GTK_ALIGN_START);
            //Aligns text inside the widget.
            gtk_label_set_align(1.0f, 0.0f);
	...
	*/
    ...
    void setFontDescription(long font) {
        ...
        if (labelHandle != 0) {
            ...
            int originalDirection = (getApi().style & SWT.RIGHT_TO_LEFT) != 0 ? GTK.GTK_TEXT_DIR_RTL : GTK.GTK_TEXT_DIR_LTR;
            int tempDirection = (getApi().style & SWT.RIGHT_TO_LEFT) != 0 ? GTK.GTK_TEXT_DIR_LTR : GTK.GTK_TEXT_DIR_RTL;
    ...
    void setWidgetBackground() {
        ...
        super.setBackgroundGdkRGBA(getApi().handle, rgba);
    }
