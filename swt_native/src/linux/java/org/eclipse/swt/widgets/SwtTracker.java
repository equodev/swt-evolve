...
public class SwtTracker extends SwtWidget implements ITracker {
    ...
    Rectangle cachedCombinedDisplayResolution = SwtDisplay.getDefault().getBounds();

    Rectangle cachedUnion = new Rectangle(0, 0, 0, 0);

    // Note purposely lazy init.
    ...
    Point adjustMoveCursor() {
        ...
        if (GTK.GTK4) {
            ...
            ((SwtDisplay) display.getImpl()).getPointerPosition(actualXDouble, actualYDouble);
            actualX[0] = (int) actualXDouble[0];
            actualY[0] = (int) actualYDouble[0];
        } else {
            ((SwtDisplay) display.getImpl()).getWindowPointerPosition(window, actualX, actualY, state);
        }
    ...
    void drawRectangles(Rectangle[] rects) {
        ...
        if (parent != null) {
            long paintHandle = ((SwtControl) parent.getImpl()).paintHandle();
    ...
    boolean grab() {
        ...
        int result = gdk_pointer_grab(GTK.GTK4 ? surface : window, GDK.GDK_OWNERSHIP_NONE, false, GDK.GDK_POINTER_MOTION_MASK | GDK.GDK_BUTTON_RELEASE_MASK, 0, cursor, GDK.GDK_CURRENT_TIME);
    ...
    public boolean open() {
        ...
        if (parent != null) {
            window = gtk_widget_get_window(((SwtControl) parent.getImpl()).paintHandle());
        }
        if (window == 0)
            return false;
        cancelled = false;
        ...
        if (GTK.GTK4) {
            ...
            ((SwtDisplay) display.getImpl()).getPointerPosition(oldXDouble, oldYDouble);
            oldX[0] = (int) oldXDouble[0];
            oldY[0] = (int) oldYDouble[0];
        } else {
            ((SwtDisplay) display.getImpl()).getWindowPointerPosition(window, oldX, oldY, state);
        }
        /*
	...
	*/
        ...
        // In case resolution was changed during run time.
        cachedCombinedDisplayResolution = SwtDisplay.getDefault().getBounds();
        ...
        if (parent != null)
            GTK.gtk_window_set_transient_for(overlay, parent.getImpl().topHandle());
        ...
        ((SwtDisplay) display.getImpl()).tracker = this.getApi();
        try {
            while (tracking) {
                if (parent != null && parent.isDisposed())
                    break;
                ((SwtDisplay) display.getImpl()).runSkin();
                ((SwtDisplay) display.getImpl()).runDeferredLayouts();
                ...
                ((SwtDisplay) display.getImpl()).runAsyncMessages(false);
            }
        } finally {
            ((SwtDisplay) display.getImpl()).tracker = oldTracker;
        }
        ungrab();
        if (overlay != 0) {
    ...
    private void setTrackerBackground(boolean opaque) {
        ...
        if (opaque) {
            ...
            css = "window {  " + "border-top-color: transparent;" + "border-left-color: transparent;" + "border-right-color: transparent;" + "border-bottom-color: transparent;}";
        }
    ...
    boolean processEvent(long eventPtr) {
        ...
        switch(eventType) {
            ...
            case GDK.GDK_KEY_PRESS:
                gtk_key_press_event(widget, eventPtr);
                break;
            case GDK.GDK_KEY_RELEASE:
                gtk_key_release_event(widget, eventPtr);
                break;
