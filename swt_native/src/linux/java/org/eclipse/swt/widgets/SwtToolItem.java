...
public class SwtToolItem extends SwtItem implements IToolItem {
    ...
    void createHandle(int index) {
        ...
        switch(getApi().style & bits) {
            case SWT.SEPARATOR:
                if (GTK.GTK4) {
                    getApi().handle = GTK.gtk_separator_new(GTK.GTK_ORIENTATION_VERTICAL);
                    if (getApi().handle == 0)
                        error(SWT.ERROR_NO_HANDLES);
                } else {
                    getApi().handle = GTK3.gtk_separator_tool_item_new();
                    if (getApi().handle == 0)
                        error(SWT.ERROR_NO_HANDLES);
                    GTK3.gtk_separator_tool_item_set_draw(getApi().handle, true);
                }
                break;
            case SWT.DROP_DOWN:
                if (GTK.GTK4) {
                    getApi().handle = GTK.gtk_box_new(GTK.GTK_ORIENTATION_HORIZONTAL, 0);
                    if (getApi().handle == 0)
                        error(SWT.ERROR_NO_HANDLES);
                    ...
                    if (menuButton == 0)
                        error(SWT.ERROR_NO_HANDLES);
                    GTK4.gtk_box_append(getApi().handle, button);
                    GTK4.gtk_box_append(getApi().handle, menuButton);
                    arrowHandle = GTK4.gtk_widget_get_first_child(menuButton);
                    GTK4.gtk_menu_button_set_use_underline(menuButton, true);
                } else {
                    getApi().handle = GTK3.gtk_menu_tool_button_new(0, null);
                    if (getApi().handle == 0)
                        error(SWT.ERROR_NO_HANDLES);
                    long child = GTK3.gtk_bin_get_child(getApi().handle);
			...
			*/
            case SWT.CHECK:
                if (GTK.GTK4) {
                    getApi().handle = GTK.gtk_toggle_button_new();
                    if (getApi().handle == 0)
                        error(SWT.ERROR_NO_HANDLES);
                    boxHandle = GTK.gtk_box_new(GTK.GTK_ORIENTATION_VERTICAL, 0);
                    if (boxHandle == 0)
                        error(SWT.ERROR_NO_HANDLES);
                    GTK4.gtk_button_set_child(getApi().handle, boxHandle);
                    GTK.gtk_button_set_use_underline(getApi().handle, true);
                } else {
                    getApi().handle = GTK3.gtk_toggle_tool_button_new();
                    if (getApi().handle == 0)
                        error(SWT.ERROR_NO_HANDLES);
                }
                break;
            ...
            default:
                if (GTK.GTK4) {
                    getApi().handle = GTK.gtk_button_new();
                    if (getApi().handle == 0)
                        error(SWT.ERROR_NO_HANDLES);
                    boxHandle = GTK.gtk_box_new(GTK.GTK_ORIENTATION_VERTICAL, 0);
                    if (boxHandle == 0)
                    ...
                    getApi().handle = GTK3.gtk_tool_button_new(0, null);
                    if (getApi().handle == 0)
                        error(SWT.ERROR_NO_HANDLES);
                }
                break;
        ...
        if ((getApi().style & SWT.SEPARATOR) == 0) {
            if (GTK.GTK4) {
                ...
                if (imageHandle == 0)
                    error(SWT.ERROR_NO_HANDLES);
                GTK3.gtk_tool_button_set_icon_widget(getApi().handle, imageHandle);
                GTK3.gtk_tool_button_set_label_widget(getApi().handle, labelHandle);
                GTK3.gtk_tool_button_set_use_underline(getApi().handle, true);
            }
        }
        if ((parent.state & FONT) != 0) {
            long fontDesc = ((SwtControl) parent.getImpl()).getFontDescription();
	 ...
	 */
        if (GTK.GTK4) {
            ...
            GTK3.gtk_tool_item_set_homogeneous(getApi().handle, false);
        }
    }
    ...
    void createWidget(int index) {
        ...
        showWidget(index);
        ((SwtToolBar) parent.getImpl()).relayout();
    }
    ...
    public Rectangle getBounds() {
        ...
        if ((getApi().style & SWT.SEPARATOR) != 0 && control != null)
    ...
    public boolean getSelection() {
        ...
        boolean selection;
        if (GTK.GTK4) {
            selection = GTK.gtk_toggle_button_get_active(getApi().handle);
        } else {
            selection = GTK3.gtk_toggle_tool_button_get_active(getApi().handle);
        }
        return selection;
    }
        ...
        if (image != null) {
            ...
            if (imageList != null) {
				 ...
				 */
                    ...
                    OS.g_signal_connect(menuItem, OS.activate, SwtToolBar.menuItemSelectedFunc.getAddress(), getApi().handle);
                    return 1;
                }
            }
        }
            ...
            case GDK.GDK_BUTTON_PRESS:
                {
                    ...
                    if (eventButton[0] == 3) {
                        ...
                        ((SwtControl) parent.getImpl()).showMenu((int) eventRX[0], (int) eventRY[0]);
                    }
                    break;
                }
        }
    ...
    long gtk_focus_in_event(long widget, long event) {
        ((SwtToolBar) parent.getImpl()).hasChildFocus = true;
        ((SwtToolBar) parent.getImpl()).currentFocusItem = this.getApi();
        parent.getImpl().sendFocusEvent(SWT.FocusIn);
        return 0;
    }

    @Override
    long gtk_focus_out_event(long widget, long event) {
        ((SwtToolBar) parent.getImpl()).hasChildFocus = false;
        parent.getImpl().sendFocusEvent(SWT.FocusOut);
        return 0;
    }
    ...
    void hookEvents() {
        ...
        if (GTK.GTK4) {
			 ...
			 */
                OS.g_signal_connect_closure(GTK4.gtk_widget_get_first_child(getApi().handle), OS.clicked, ((SwtDisplay) display.getImpl()).getClosure(CLICKED), false);
            } else {
                OS.g_signal_connect_closure(getApi().handle, OS.clicked, ((SwtDisplay) display.getImpl()).getClosure(CLICKED), false);
            }
            long focusController = GTK4.gtk_event_controller_focus_new();
            GTK4.gtk_widget_add_controller(getApi().handle, focusController);
            ...
            OS.g_signal_connect(motionController, OS.enter, ((SwtDisplay) display.getImpl()).enterMotionProc, ENTER);
            OS.g_signal_connect(motionController, OS.leave, ((SwtDisplay) display.getImpl()).leaveProc, LEAVE);
            //TODO: event-after
		 ...
		 */
            eventHandle = GTK3.gtk_bin_get_child(getApi().handle);
            if ((getApi().style & SWT.DROP_DOWN) != 0) {
                long list = GTK3.gtk_container_get_children(eventHandle);
                eventHandle = OS.g_list_nth_data(list, 0);
                OS.g_list_free(list);
                if (arrowHandle != 0)
                    OS.g_signal_connect_closure(arrowHandle, OS.clicked, ((SwtDisplay) display.getImpl()).getClosure(CLICKED), false);
            }
            OS.g_signal_connect_closure(getApi().handle, OS.clicked, ((SwtDisplay) display.getImpl()).getClosure(CLICKED), false);
            OS.g_signal_connect_closure(getApi().handle, OS.create_menu_proxy, ((SwtDisplay) display.getImpl()).getClosure(CREATE_MENU_PROXY), false);
            OS.g_signal_connect_closure_by_id(eventHandle, ((SwtDisplay) display.getImpl()).signalIds[FOCUS_IN_EVENT], 0, ((SwtDisplay) display.getImpl()).getClosure(FOCUS_IN_EVENT), false);
            OS.g_signal_connect_closure_by_id(eventHandle, ((SwtDisplay) display.getImpl()).signalIds[FOCUS_OUT_EVENT], 0, ((SwtDisplay) display.getImpl()).getClosure(FOCUS_OUT_EVENT), false);
            OS.g_signal_connect_closure_by_id(eventHandle, ((SwtDisplay) display.getImpl()).signalIds[ENTER_NOTIFY_EVENT], 0, ((SwtDisplay) display.getImpl()).getClosure(ENTER_NOTIFY_EVENT), false);
            OS.g_signal_connect_closure_by_id(eventHandle, ((SwtDisplay) display.getImpl()).signalIds[LEAVE_NOTIFY_EVENT], 0, ((SwtDisplay) display.getImpl()).getClosure(LEAVE_NOTIFY_EVENT), false);
            /*
		...
		* to the parent.
		*/
            int mask = GDK.GDK_EXPOSURE_MASK | GDK.GDK_POINTER_MOTION_MASK | GDK.GDK_BUTTON_PRESS_MASK | GDK.GDK_BUTTON_RELEASE_MASK | GDK.GDK_ENTER_NOTIFY_MASK | GDK.GDK_LEAVE_NOTIFY_MASK | GDK.GDK_KEY_PRESS_MASK | GDK.GDK_KEY_RELEASE_MASK | GDK.GDK_FOCUS_CHANGE_MASK;
    ...
    void releaseWidget() {
        super.releaseWidget();
        if (((SwtToolBar) parent.getImpl()).currentFocusItem == this.getApi())
            ((SwtToolBar) parent.getImpl()).currentFocusItem = null;
        parent = null;
        control = null;
    ...
    void resizeHandle(int width, int height) {
        GTK.gtk_widget_set_size_request(getApi().handle, width, height);
        /*
	...
	*/
        ...
        gtk_widget_size_allocate(getApi().handle, allocation, -1);
    }
    ...
    void selectRadio() {
        ...
        int i = index - 1;
        while (i >= 0 && ((SwtToolItem) items[i].getImpl()).setRadioSelection(false)) --i;
        int j = index + 1;
        while (j < items.length && ((SwtToolItem) items[j].getImpl()).setRadioSelection(false)) j++;
        setSelection(true);
    }
    ...
    boolean setFocus() {
        return GTK.gtk_widget_child_focus(getApi().handle, GTK.GTK_DIR_TAB_FORWARD);
    }
    ...
    public void setHotImage(Image image) {
        ...
        if (image != null) {
            ImageList imageList = ((SwtToolBar) parent.getImpl()).imageList;
            if (imageList == null)
                imageList = ((SwtToolBar) parent.getImpl()).imageList = new ImageList();
	...
	*/
    ...
    public void setSelection(boolean selected) {
        ...
        if ((getApi().style & (SWT.CHECK | SWT.RADIO)) == 0)
            return;
        OS.g_signal_handlers_block_matched(getApi().handle, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, CLICKED);
        if (GTK.GTK4) {
            GTK.gtk_toggle_button_set_active(getApi().handle, selected);
        } else {
            GTK3.gtk_toggle_tool_button_set_active(getApi().handle, selected);
        }
        OS.g_signal_handlers_unblock_matched(getApi().handle, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, CLICKED);
    }
	 ...
	 */
        if ((parent.style & SWT.RIGHT) != 0) {
            if (!GTK.GTK4)
                GTK3.gtk_tool_item_set_is_important(getApi().handle, !string.isEmpty());
        }
        /*
	* If Text/Image of a tool-item changes, then it is
	* required to reset the proxy menu. Otherwise, the
	...
	*/
    ...
    public void setToolTipText(String string) {
        ...
        if (toolTipText == string || (toolTipText != null && toolTipText.equals(string)))
            return;
        if (((SwtControl) parent.getImpl()).toolTipText == null) {
            if (GTK.GTK4) {
                setToolTipText(getApi().handle, string);
            } else {
                long child = GTK3.gtk_bin_get_child(getApi().handle);
                if ((getApi().style & SWT.DROP_DOWN) != 0) {
                    long list = GTK3.gtk_container_get_children(child);
                ...
                setToolTipText(child != 0 ? child : getApi().handle, string);
            }
        }
        toolTipText = string;
        /*
	...
	*/
    ...
    void showWidget(int index) {
        if (GTK.GTK4) {
            if (index == 0) {
                GTK4.gtk_box_prepend(parent.handle, getApi().handle);
            } else if (index < 0) {
                ...
                for (long sibling = GTK4.gtk_widget_get_first_child(parent.handle); sibling != 0; sibling = GTK4.gtk_widget_get_next_sibling(sibling)) {
                    if (index == 1) {
                        GTK4.gtk_box_insert_child_after(parent.handle, getApi().handle, sibling);
                        break;
                    }
                    index--;
                }
    ...
    void updateStyle() {
        ...
        if (provider == 0) {
            ...
            if ((getApi().style & SWT.DROP_DOWN) != 0) {
                if (GTK.GTK4) {
                    ...
                    long box = GTK3.gtk_bin_get_child(getApi().handle);
                ...
                long context;
                if (GTK.GTK4) {
                    context = GTK.gtk_widget_get_style_context(getApi().handle);
                } else {
                    long child = GTK3.gtk_bin_get_child(getApi().handle);
        ...
        String css = "";
        if (foreground != null) {
            css += "button { color: " + ((SwtDisplay) display.getImpl()).gtk_rgba_to_css_string(foreground.handle) + "; }";
        } else {
            css += ((SwtToolBar) parent.getImpl()).cssForeground;
        }
        if (background != null) {
            css += "button { background-image: none; background-color: " + ((SwtDisplay) display.getImpl()).gtk_rgba_to_css_string(background.handle) + "; }";
        }
        if (GTK.GTK4) {
    ...
    long dpiChanged(long object, long arg0) {
        ...
        if (hotImage != null) {
            ((SwtImage) hotImage.getImpl()).internal_gtk_refreshImageForZoom();
            setHotImage(hotImage);
        }
        if (disabledImage != null) {
            ((SwtImage) disabledImage.getImpl()).internal_gtk_refreshImageForZoom();
    ...
    private void recreateMenuProxy() {
        if ((getApi().style & SWT.DROP_DOWN) != 0 || proxyMenuItem != 0) {
            if (GTK.GTK4) {
                ...
                proxyMenuItem = 0;
                proxyMenuItem = GTK3.gtk_tool_item_retrieve_proxy_menu_item(getApi().handle);
            }
        }
    }
