...
public abstract class SwtScrollable extends SwtControl implements IScrollable {
    ...
    ScrollBar createScrollBar(int style) {
        ...
        if ((style & SWT.H_SCROLL) != 0) {
            ...
            ((SwtScrollBar) bar.getImpl()).adjustmentHandle = GTK.gtk_scrolled_window_get_hadjustment(scrolledHandle);
        } else {
            bar.handle = GTK.gtk_scrolled_window_get_vscrollbar(scrolledHandle);
            ((SwtScrollBar) bar.getImpl()).adjustmentHandle = GTK.gtk_scrolled_window_get_vadjustment(scrolledHandle);
        }
        ((SwtScrollBar) bar.getImpl()).setOrientation(true);
        ((SwtScrollBar) bar.getImpl()).hookEvents();
    ...
    long gtk_draw(long widget, long cairo) {
		 ...
		 */
            ...
            if (overlayScrolling && OS.G_OBJECT_TYPE(widget) == OS.swt_fixed_get_type()) {
                if ((getApi().style & SWT.V_SCROLL) != 0 && verticalBar != null) {
                ...
                if ((getApi().style & SWT.H_SCROLL) != 0 && horizontalBar != null) {
	...
	*/
        if ((getApi().state & CANVAS) != 0) {
            ...
            if (!fetched) {
                ...
                if (deltasAvailable) {
                    if (delta_x[0] != 0) {
                        ...
                        if (scrollBar != null && !GTK.gtk_widget_get_visible(scrollBar.handle) && scrollBar.getEnabled()) {
                            ...
                            gtk_adjustment_get(((SwtScrollBar) scrollBar.getImpl()).adjustmentHandle, adjustment);
                            double delta = Math.pow(adjustment.page_size, 2.0 / 3.0) * delta_x[0];
                            int value = (int) Math.max(adjustment.lower, Math.min(adjustment.upper - adjustment.page_size, adjustment.value + delta));
                            GTK.gtk_adjustment_set_value(((SwtScrollBar) scrollBar.getImpl()).adjustmentHandle, value);
                            result = 1;
                        }
                    }
                    if (delta_y[0] != 0) {
                ...
                if (scrollBar != null && !GTK.gtk_widget_get_visible(scrollBar.handle) && scrollBar.getEnabled()) {
                    ...
                    int value = (int) Math.max(adjustment.lower, Math.min(adjustment.upper - adjustment.page_size, adjustment.value + wheel_delta));
                    GTK.gtk_adjustment_set_value(((SwtScrollBar) scrollBar.getImpl()).adjustmentHandle, value);
                    return 1;
                }
            }
    ...
    void setOrientation(boolean create) {
        ...
        if (horizontalBar != null)
            ((SwtScrollBar) horizontalBar.getImpl()).setOrientation(create);
        if (verticalBar != null)
            ((SwtScrollBar) verticalBar.getImpl()).setOrientation(create);
    }
    ...
    void resizeHandle(int width, int height) {
        ...
        long child = scrolledHandle != 0 ? scrolledHandle : getApi().handle;
