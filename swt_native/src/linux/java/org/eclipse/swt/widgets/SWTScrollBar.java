...
public class SWTScrollBar extends SWTWidget implements IScrollBar {
     ...
     */
    SWTScrollBar(SWTScrollable parent, int style) {
    ...
    Rectangle getThumbBoundsInPixels() {
        ...
        if (GTK.GTK4) {
            ...
            boolean success = GTK4.gtk_widget_translate_coordinates(parent.scrolledHandle, ((SWTShell) (parent.getShell())).shellHandle, 0, 0, origin_x, origin_y);
            if (success) {
                rect.x += origin_x[0];
                rect.y += origin_y[0];
            }
            success = GTK4.gtk_widget_translate_coordinates(parent.handle, ((SWTShell) (parent.getShell())).shellHandle, 0, 0, origin_x, origin_y);
            if (success) {
                rect.x -= origin_x[0];
                rect.y -= origin_y[0];
            }
    ...
    public boolean isEnabled() {
        checkWidget();
        return getEnabled() && ((SWTScrollable) (getParent())).getEnabled();
    }
    ...
    public boolean isVisible() {
        checkWidget();
        return getVisible() && ((SWTScrollable) (getParent())).isVisible();
    }
