...
public class SWTMenuItem extends SWTItem implements IMenuItem {
     ...
     * @see Widget#getStyle
     */
    public SWTMenuItem(SWTMenu parent, int style) {
     ...
     * @see Widget#getStyle
     */
    public SWTMenuItem(SWTMenu parent, int style, int index) {
        super(parent, checkStyle(style));
        this.parent = parent;
        ...
        if (GTK.GTK4) {
            ...
            if ((style & SWT.SEPARATOR) != 0) {
                ...
                for (int i = 0; i < itemsToMove; i++) {
                    SWTMenuItem removedItem = (SWTMenuItem) (selectedSection.sectionItems.remove(sectionRelativeIndex));
    ...
    void fixMenus(SWTDecorations newParent) {
    ...
    long getAccelGroup() {
        SWTMenu menu = (SWTMenu) (parent);
        while (menu != null && menu.cascade != null) {
            menu = menu.cascade.parent;
        }
        if (menu == null)
            return 0;
        SWTDecorations shell = menu.parent;
        if (shell == null)
            return 0;
    ...
    void releaseWidget() {
        super.releaseWidget();
        if (GTK.GTK4) {
            if (parent.actionGroup != 0 && actionName != null)
                OS.g_action_map_remove_action(parent.actionGroup, Converter.javaStringToCString(actionName));
        } else {
    ...
    void destroyWidget() {
        if (GTK.GTK4) {
            if ((style & SWT.SEPARATOR) != 0) {
                ...
                for (SWTMenuItem item : section.sectionItems) {
    ...
    void selectRadio() {
        int index = 0;
        SWTMenuItem[] items = (SWTMenuItem[]) (parent.getItems());
    ...
    private void _setEnabledOrDisabledImage() {
        if (!enabled) {
            if (defaultDisableImage == null && image != null) {
                defaultDisableImage = new Image(((SWTDisplay) (getDisplay())), image, SWT.IMAGE_DISABLE);
            }
     ...
     * </ul>
     */
    public void setMenu(IMenu menu__) {
        SWTMenu menu_ = (SWTMenu) menu__;
    ...
    public void setText(String string) {
        ...
        if (GTK.GTK4) {
            ...
            SWTMaskKeysym maskKeysym = (SWTMaskKeysym) (getMaskKeysym());
            if (maskKeysym != null) {
                OS.g_menu_item_set_attribute(handle, Converter.javaStringToCString("accel"), Converter.javaStringToCString("s"), GTK.gtk_accelerator_name(maskKeysym.keysym, maskKeysym.mask));
            }
            OS.g_menu_remove(section.getSectionHandle(), section.getItemPosition(this));
            OS.g_menu_insert_item(section.getSectionHandle(), section.getItemPosition(this), handle);
        } else {
    ...
    private static class SWTMaskKeysym {
    ...
    private SWTMaskKeysym getMaskKeysym() {
        if (text == null)
            return null;
        SWTMaskKeysym maskKeysym = new SWTMaskKeysym();
    ...
    boolean updateAcceleratorText(boolean show) {
        if (accelerator != 0)
            return false;
        SWTMaskKeysym maskKeysym = (SWTMaskKeysym) (null);
        if (show) {
