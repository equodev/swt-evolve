 ...
 */
public class SwtTableItem extends SwtItem implements ITableItem {
    ...
    SwtTableItem(Table parent, int style, int index, boolean create, TableItem api) {
        ...
        if (create) {
            ...
            GTK.gtk_tree_model_iter_nth_child(((SwtTable) parent.getImpl()).modelHandle, getApi().handle, 0, index);
        }
    }
    ...
    Color _getBackground() {
        long[] ptr = new long[1];
        GTK.gtk_tree_model_get(((SwtTable) parent.getImpl()).modelHandle, getApi().handle, SwtTable.BACKGROUND_COLUMN, ptr, -1);
        if (ptr[0] == 0)
    ...
    Color _getBackground(int index) {
        ...
        long[] ptr = new long[1];
        int modelIndex = ((SwtTable) parent.getImpl()).columnCount == 0 ? SwtTable.FIRST_COLUMN : ((SwtTableColumn) ((SwtTable) parent.getImpl()).columns[index].getImpl()).modelIndex;
        GTK.gtk_tree_model_get(((SwtTable) parent.getImpl()).modelHandle, getApi().handle, modelIndex + SwtTable.CELL_BACKGROUND, ptr, -1);
        if (ptr[0] == 0)
            return _getBackground();
        GdkRGBA gdkRGBA = new GdkRGBA();
        OS.memmove(gdkRGBA, ptr[0], GdkRGBA.sizeof);
    ...
    boolean _getChecked() {
        int[] ptr = new int[1];
        GTK.gtk_tree_model_get(((SwtTable) parent.getImpl()).modelHandle, getApi().handle, SwtTable.CHECKED_COLUMN, ptr, -1);
        return ptr[0] != 0;
    }

    Color _getForeground() {
        long[] ptr = new long[1];
        GTK.gtk_tree_model_get(((SwtTable) parent.getImpl()).modelHandle, getApi().handle, SwtTable.FOREGROUND_COLUMN, ptr, -1);
        if (ptr[0] == 0)
    ...
    Color _getForeground(int index) {
        ...
        long[] ptr = new long[1];
        int modelIndex = ((SwtTable) parent.getImpl()).columnCount == 0 ? SwtTable.FIRST_COLUMN : ((SwtTableColumn) ((SwtTable) parent.getImpl()).columns[index].getImpl()).modelIndex;
        GTK.gtk_tree_model_get(((SwtTable) parent.getImpl()).modelHandle, getApi().handle, modelIndex + SwtTable.CELL_FOREGROUND, ptr, -1);
        if (ptr[0] == 0)
            return _getForeground();
        GdkRGBA gdkRGBA = new GdkRGBA();
        OS.memmove(gdkRGBA, ptr[0], GdkRGBA.sizeof);
    ...
    Image _getImage(int index) {
        ...
        int modelIndex = ((SwtTable) parent.getImpl()).columnCount == 0 ? SwtTable.FIRST_COLUMN : ((SwtTableColumn) ((SwtTable) parent.getImpl()).columns[index].getImpl()).modelIndex;
        GTK.gtk_tree_model_get(((SwtTable) parent.getImpl()).modelHandle, getApi().handle, modelIndex + SwtTable.CELL_SURFACE, surfaceHandle, -1);
        if (surfaceHandle[0] == 0)
            return null;
        int imageIndex = ((SwtTable) parent.getImpl()).imageList.indexOf(surfaceHandle[0]);
        if (imageIndex == -1) {
            return null;
        } else {
            return ((SwtTable) parent.getImpl()).imageList.get(imageIndex);
        }
    }
    ...
    String _getText(int index) {
        ...
        long[] ptr = new long[1];
        int modelIndex = ((SwtTable) parent.getImpl()).columnCount == 0 ? SwtTable.FIRST_COLUMN : ((SwtTableColumn) ((SwtTable) parent.getImpl()).columns[index].getImpl()).modelIndex;
        GTK.gtk_tree_model_get(((SwtTable) parent.getImpl()).modelHandle, getApi().handle, modelIndex + SwtTable.CELL_TEXT, ptr, -1);
        if (ptr[0] == 0)
            return "";
    ...
    void clear() {
        if (((SwtTable) parent.getImpl()).currentItem == this.getApi())
            return;
        if (cached || (parent.style & SWT.VIRTUAL) == 0) {
            int columnCount = GTK.gtk_tree_model_get_n_columns(((SwtTable) parent.getImpl()).modelHandle);
            /* the columns before FOREGROUND_COLUMN contain int values, subsequent columns contain pointers */
            for (int i = SwtTable.CHECKED_COLUMN; i < SwtTable.FOREGROUND_COLUMN; i++) {
                GTK.gtk_list_store_set(((SwtTable) parent.getImpl()).modelHandle, getApi().handle, i, 0, -1);
            }
            for (int i = SwtTable.FOREGROUND_COLUMN; i < columnCount; i++) {
                GTK.gtk_list_store_set(((SwtTable) parent.getImpl()).modelHandle, getApi().handle, i, (long) 0, -1);
            }
        }
        cached = false;
        font = null;
    ...
    Rectangle getBoundsinPixels() {
        ...
        long textRenderer = ((SwtTable) parent.getImpl()).getTextRenderer(column);
        long pixbufRenderer = ((SwtTable) parent.getImpl()).getPixbufRenderer(column);
        if (textRenderer == 0 || pixbufRenderer == 0)
            return new Rectangle(0, 0, 0, 0);
        long path = GTK.gtk_tree_model_get_path(((SwtTable) parent.getImpl()).modelHandle, getApi().handle);
        GTK.gtk_widget_realize(parentHandle);
        boolean isExpander = GTK.gtk_tree_model_iter_n_children(((SwtTable) parent.getImpl()).modelHandle, getApi().handle) > 0;
        boolean isExpanded = GTK.gtk_tree_view_row_expanded(parentHandle, path);
        GTK.gtk_tree_view_column_cell_set_cell_data(column, ((SwtTable) parent.getImpl()).modelHandle, getApi().handle, isExpander, isExpanded);
        GdkRectangle rect = new GdkRectangle();
        GTK.gtk_tree_view_get_cell_area(parentHandle, path, column, rect);
        GTK.gtk_tree_path_free(path);
        if ((parent.getStyle() & SWT.MIRRORED) != 0)
            rect.x = ((SwtTable) parent.getImpl()).getClientWidth() - rect.width - rect.x;
        int right = rect.x + rect.width;
        int[] x = new int[1], w = new int[1];
        ((SwtTable) parent.getImpl()).ignoreSize = true;
        gtk_cell_renderer_get_preferred_size(textRenderer, parentHandle, w, null);
        ((SwtTable) parent.getImpl()).ignoreSize = false;
        rect.width = w[0];
    ...
    Rectangle getBoundsInPixels(int index) {
        ...
        long parentHandle = parent.handle;
        long column = 0;
        if (index >= 0 && index < ((SwtTable) parent.getImpl()).columnCount) {
            column = ((SwtTable) parent.getImpl()).columns[index].handle;
        } else {
        ...
        long path = GTK.gtk_tree_model_get_path(((SwtTable) parent.getImpl()).modelHandle, getApi().handle);
        GTK.gtk_widget_realize(parentHandle);
        GTK.gtk_tree_view_column_cell_set_cell_data(column, ((SwtTable) parent.getImpl()).modelHandle, getApi().handle, false, false);
        GdkRectangle rect = new GdkRectangle();
        GTK.gtk_tree_view_get_cell_area(parentHandle, path, column, rect);
        GTK.gtk_tree_path_free(path);
        ...
        if (index == 0 && (parent.style & SWT.CHECK) != 0) {
            int[] x = new int[1], w = new int[1];
            gtk_tree_view_column_cell_get_position(column, ((SwtTable) parent.getImpl()).checkRenderer, x, w);
            rect.x += x[0] + w[0];
            rect.width -= x[0] + w[0];
        }
        int width = GTK.gtk_tree_view_column_get_visible(column) ? rect.width + 1 : 0;
    ...
    public void setBackground(Color color) {
        ...
        GTK.gtk_list_store_set(((SwtTable) parent.getImpl()).modelHandle, getApi().handle, SwtTable.BACKGROUND_COLUMN, gdkRGBA, -1);
        cached = true;
    }
    ...
    public void setBackground(int index, Color color) {
        ...
        int modelIndex = ((SwtTable) parent.getImpl()).columnCount == 0 ? SwtTable.FIRST_COLUMN : ((SwtTableColumn) ((SwtTable) parent.getImpl()).columns[index].getImpl()).modelIndex;
        GdkRGBA gdkRGBA = color != null ? color.handle : null;
        GTK.gtk_list_store_set(((SwtTable) parent.getImpl()).modelHandle, getApi().handle, modelIndex + SwtTable.CELL_BACKGROUND, gdkRGBA, -1);
        cached = true;
        if (color != null) {
            boolean customDraw = (((SwtTable) parent.getImpl()).columnCount == 0) ? ((SwtTable) parent.getImpl()).firstCustomDraw : ((SwtTableColumn) ((SwtTable) parent.getImpl()).columns[index].getImpl()).customDraw;
            if (!customDraw) {
                if ((parent.style & SWT.VIRTUAL) == 0) {
                    long parentHandle = parent.handle;
                    long column = 0;
                    ...
                    if (column == 0)
                        return;
                    long textRenderer = ((SwtTable) parent.getImpl()).getTextRenderer(column);
                    long imageRenderer = ((SwtTable) parent.getImpl()).getPixbufRenderer(column);
                ...
                if (((SwtTable) parent.getImpl()).columnCount == 0) {
                    ((SwtTable) parent.getImpl()).firstCustomDraw = true;
                } else {
                    ((SwtTableColumn) ((SwtTable) parent.getImpl()).columns[index].getImpl()).customDraw = true;
                }
            }
        }
    }
    ...
    public void setChecked(boolean checked) {
        ...
        if (_getChecked() == checked)
            return;
        GTK.gtk_list_store_set(((SwtTable) parent.getImpl()).modelHandle, getApi().handle, SwtTable.CHECKED_COLUMN, checked, -1);
        /*
	...
	*/
        GTK.gtk_list_store_set(((SwtTable) parent.getImpl()).modelHandle, getApi().handle, SwtTable.GRAYED_COLUMN, !checked ? false : grayed, -1);
        cached = true;
    }
    ...
    public void setFont(Font font) {
        ...
        GTK.gtk_list_store_set(((SwtTable) parent.getImpl()).modelHandle, getApi().handle, SwtTable.FONT_COLUMN, fontHandle, -1);
        cached = true;
    }
    ...
    public void setFont(int index, Font font) {
        ...
        int modelIndex = ((SwtTable) parent.getImpl()).columnCount == 0 ? SwtTable.FIRST_COLUMN : ((SwtTableColumn) ((SwtTable) parent.getImpl()).columns[index].getImpl()).modelIndex;
        long fontHandle = font != null ? font.handle : 0;
        GTK.gtk_list_store_set(((SwtTable) parent.getImpl()).modelHandle, getApi().handle, modelIndex + SwtTable.CELL_FONT, fontHandle, -1);
        cached = true;
        if (font != null) {
            boolean customDraw = (((SwtTable) parent.getImpl()).columnCount == 0) ? ((SwtTable) parent.getImpl()).firstCustomDraw : ((SwtTableColumn) ((SwtTable) parent.getImpl()).columns[index].getImpl()).customDraw;
            if (!customDraw) {
    ...
    public void setForeground(Color color) {
        ...
        if (_getForeground().equals(color))
            return;
        GdkRGBA gdkRGBA = color != null ? color.handle : null;
        GTK.gtk_list_store_set(((SwtTable) parent.getImpl()).modelHandle, getApi().handle, SwtTable.FOREGROUND_COLUMN, gdkRGBA, -1);
        cached = true;
    }
    ...
    public void setForeground(int index, Color color) {
        ...
        int modelIndex = ((SwtTable) parent.getImpl()).columnCount == 0 ? SwtTable.FIRST_COLUMN : ((SwtTableColumn) ((SwtTable) parent.getImpl()).columns[index].getImpl()).modelIndex;
        GdkRGBA gdkRGBA = color != null ? color.handle : null;
        GTK.gtk_list_store_set(((SwtTable) parent.getImpl()).modelHandle, getApi().handle, modelIndex + SwtTable.CELL_FOREGROUND, gdkRGBA, -1);
        cached = true;
        if (color != null) {
            boolean customDraw = (((SwtTable) parent.getImpl()).columnCount == 0) ? ((SwtTable) parent.getImpl()).firstCustomDraw : ((SwtTableColumn) ((SwtTable) parent.getImpl()).columns[index].getImpl()).customDraw;
            if (!customDraw) {
	...
	*/
        int[] ptr = new int[1];
        GTK.gtk_tree_model_get(((SwtTable) parent.getImpl()).modelHandle, getApi().handle, SwtTable.CHECKED_COLUMN, ptr, -1);
        GTK.gtk_list_store_set(((SwtTable) parent.getImpl()).modelHandle, getApi().handle, SwtTable.GRAYED_COLUMN, ptr[0] == 0 ? false : grayed, -1);
        cached = true;
    }
    ...
    public void setImage(int index, Image image) {
        ...
        if (image != null) {
            ImageList imageList = ((SwtTable) parent.getImpl()).imageList;
            if (imageList == null)
                imageList = ((SwtTable) parent.getImpl()).imageList = new ImageList();
        ...
        if (!((SwtTable) parent.getImpl()).pixbufSizeSet) {
            if (image != null) {
                ...
                if (iWidth > currentWidth[0] || iHeight > currentHeight[0]) {
                    ...
                    ((SwtTable) parent.getImpl()).pixbufWidth = iWidth;
                    ((SwtTable) parent.getImpl()).pixbufSizeSet = true;
                }
            }
        } else {
            /*
		 ...
		 */
            if (((SwtTable) parent.getImpl()).pixbufWidth > Math.max(currentWidth[0], 0) || ((SwtTable) parent.getImpl()).pixbufHeight > Math.max(currentHeight[0], 0)) {
                GTK.gtk_cell_renderer_set_fixed_size(pixbufRenderer, ((SwtTable) parent.getImpl()).pixbufWidth, ((SwtTable) parent.getImpl()).pixbufHeight);
            }
        }
        int modelIndex = ((SwtTable) parent.getImpl()).columnCount == 0 ? SwtTable.FIRST_COLUMN : ((SwtTableColumn) ((SwtTable) parent.getImpl()).columns[index].getImpl()).modelIndex;
        GTK.gtk_list_store_set(((SwtTable) parent.getImpl()).modelHandle, getApi().handle, modelIndex + SwtTable.CELL_PIXBUF, pixbuf, -1);
        /*
	 ...
	 */
        ...
        GTK.gtk_list_store_set(((SwtTable) parent.getImpl()).modelHandle, getApi().handle, modelIndex + SwtTable.CELL_SURFACE, surface, -1);
        cached = true;
        /*
	 ...
	 */
        if (((SwtTable) parent.getImpl()).columnCount == 0) {
            ...
            ((SwtTable) parent.getImpl()).maxWidth = Math.max(((SwtTable) parent.getImpl()).maxWidth, ((SwtTable) parent.getImpl()).calculateWidth(column, this.getApi().handle));
        }
    }
    ...
    public void setText(int index, String string) {
        ...
        int modelIndex = ((SwtTable) parent.getImpl()).columnCount == 0 ? SwtTable.FIRST_COLUMN : ((SwtTableColumn) ((SwtTable) parent.getImpl()).columns[index].getImpl()).modelIndex;
        GTK.gtk_list_store_set(((SwtTable) parent.getImpl()).modelHandle, getApi().handle, modelIndex + SwtTable.CELL_TEXT, buffer, -1);
        cached = true;
        /*
	 * Bug 465056: single column Tables have a very small initial width.
	 * Fix: when text or an image is set for a Table, compute its
