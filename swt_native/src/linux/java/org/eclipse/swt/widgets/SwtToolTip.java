...
public class SwtToolTip extends SwtWidget implements IToolTip {
	 ...
	 */
        ...
        int x = point.x;
        int y = point.y;
        GTK3.gtk_window_resize(getApi().handle, w, h + TIP_HEIGHT);
        int[] polyline;
        ...
        if ((dest.width >= x + w) || (multipleMonitors && width >= x + w)) {
            if (dest.height >= y + h + TIP_HEIGHT) {
                ...
                GTK3.gtk_window_move(getApi().handle, Math.max(0, x - 17), y);
            } else {
                polyline = new int[] { 0, 5, 1, 5, 1, 3, 3, 1, 5, 1, 5, 0, w - 5, 0, w - 5, 1, w - 3, 1, w - 1, 3, w - 1, 5, w, 5, w, h - 5, w - 1, h - 5, w - 1, h - 3, w - 2, h - 3, w - 2, h - 2, w - 3, h - 2, w - 3, h - 1, w - 5, h - 1, w - 5, h, 35, h, 16, h + TIP_HEIGHT, 16, h, 5, h, 5, h - 1, 3, h - 1, 3, h - 2, 2, h - 2, 2, h - 3, 1, h - 3, 1, h - 5, 0, h - 5, 0, 5 };
                borderPolygon = new int[] { 0, 5, 1, 4, 1, 3, 3, 1, 4, 1, 5, 0, w - 6, 0, w - 5, 1, w - 4, 1, w - 2, 3, w - 2, 4, w - 1, 5, w - 1, h - 6, w - 2, h - 5, w - 2, h - 4, w - 4, h - 2, w - 5, h - 2, w - 6, h - 1, 35, h - 1, 17, h + TIP_HEIGHT - 2, 17, h - 1, 5, h - 1, 4, h - 2, 3, h - 2, 1, h - 4, 1, h - 5, 0, h - 6, 0, 5 };
                if ((parent.style & SWT.MIRRORED) != 0) {
                    x -= w - 36;
                ...
                GTK3.gtk_window_move(getApi().handle, Math.max(0, x - 17), y - h - TIP_HEIGHT);
            }
        } else {
            if (dest.height >= y + h + TIP_HEIGHT) {
                int t = TIP_HEIGHT;
                ...
                GTK3.gtk_window_move(getApi().handle, Math.max(dest.width - w, x - w + 17), y);
            } else {
                polyline = new int[] { 0, 5, 1, 5, 1, 3, 3, 1, 5, 1, 5, 0, w - 5, 0, w - 5, 1, w - 3, 1, w - 1, 3, w - 1, 5, w, 5, w, h - 5, w - 1, h - 5, w - 1, h - 3, w - 2, h - 3, w - 2, h - 2, w - 3, h - 2, w - 3, h - 1, w - 5, h - 1, w - 5, h, w - 16, h, w - 16, h + TIP_HEIGHT, w - 35, h, 5, h, 5, h - 1, 3, h - 1, 3, h - 2, 2, h - 2, 2, h - 3, 1, h - 3, 1, h - 5, 0, h - 5, 0, 5 };
                borderPolygon = new int[] { 0, 5, 1, 4, 1, 3, 3, 1, 4, 1, 5, 0, w - 6, 0, w - 5, 1, w - 4, 1, w - 2, 3, w - 2, 4, w - 1, 5, w - 1, h - 6, w - 2, h - 5, w - 2, h - 4, w - 4, h - 2, w - 5, h - 2, w - 6, h - 1, w - 17, h - 1, w - 17, h + TIP_HEIGHT - 2, w - 36, h - 1, 5, h - 1, 4, h - 2, 3, h - 2, 1, h - 4, 1, h - 5, 0, h - 6, 0, 5 };
                if ((parent.style & SWT.MIRRORED) != 0) {
                    x += w - 35;
                    polyline[42] = 35;
                ...
                GTK3.gtk_window_move(getApi().handle, Math.max(dest.width - w, x - w + 17), y - h - TIP_HEIGHT);
            }
        }
        GTK.gtk_widget_realize(getApi().handle);
    ...
    void createHandle(int index) {
        if ((getApi().style & SWT.BALLOON) != 0) {
            ...
            if (GTK.GTK4) {
                ...
                getApi().handle = GTK3.gtk_window_new(GTK.GTK_WINDOW_POPUP);
    ...
    Point getLocation() {
        ...
        if (x == -1 || y == -1) {
            if (GTK.GTK4) {
                ...
                ((SwtDisplay) display.getImpl()).getPointerPosition(px, py);
                x = (int) px[0];
                y = (int) py[0];
            } else {
    ...
    public boolean getVisible() {
        checkWidget();
        if ((getApi().style & SWT.BALLOON) != 0)
            return GTK.gtk_widget_get_visible(getApi().handle);
        return false;
    }
    ...
    void hookEvents() {
        if ((getApi().style & SWT.BALLOON) != 0) {
            OS.g_signal_connect_closure_by_id(getApi().handle, ((SwtDisplay) display.getImpl()).signalIds[EXPOSE_EVENT], 0, ((SwtDisplay) display.getImpl()).getClosure(EXPOSE_EVENT), true);
            OS.g_signal_connect_closure_by_id(getApi().handle, ((SwtDisplay) display.getImpl()).signalIds[EXPOSE_EVENT_INVERSE], 0, ((SwtDisplay) display.getImpl()).getClosure(EXPOSE_EVENT_INVERSE), true);
            if (GTK.GTK4) {
                ...
                GTK3.gtk_widget_add_events(getApi().handle, GDK.GDK_BUTTON_PRESS_MASK);
                OS.g_signal_connect_closure(getApi().handle, OS.button_press_event, ((SwtDisplay) display.getImpl()).getClosure(BUTTON_PRESS_EVENT), false);
            }
        }
    }
    ...
    public void setLocation(int x, int y) {
        ...
        this.y = y;
        if ((getApi().style & SWT.BALLOON) != 0) {
            if (GTK.gtk_widget_get_visible(getApi().handle))
                configure();
        }
    }
    ...
    public void setMessage(String string) {
        ...
        if (message.length() != 0) {
            ...
            layoutMessage = GTK.gtk_widget_create_pango_layout(getApi().handle, buffer);
    ...
    public void setText(String string) {
        ...
        if (text.length() != 0) {
            ...
            layoutText = GTK.gtk_widget_create_pango_layout(getApi().handle, buffer);
    ...
    public void setVisible(boolean visible) {
        ...
        if (visible) {
            ...
            if (autohide) {
                if (GTK.GTK4) {
                    timerId = OS.g_timeout_add(DELAY, ((SwtDisplay) display.getImpl()).windowTimerProc, getApi().handle);
                } else {
                    timerId = GDK.gdk_threads_add_timeout(DELAY, ((SwtDisplay) display.getImpl()).windowTimerProc, getApi().handle);
                }
            }
        } else {
            if ((getApi().style & SWT.BALLOON) != 0) {
