...
public class SwtTreeItem extends SwtItem implements ITreeItem {
    ...
    public SwtTreeItem(TreeItem parentItem, int style, TreeItem api) {
        this(((SwtTreeItem) checkNull(parentItem).getImpl()).parent, parentItem.handle, style, -1, 0, api);
    }
    ...
    public SwtTreeItem(TreeItem parentItem, int style, int index, TreeItem api) {
        this(((SwtTreeItem) checkNull(parentItem).getImpl()).parent, parentItem.handle, style, checkIndex(index), 0, api);
    }

    SwtTreeItem(Tree parent, long parentIter, int style, int index, long iter, TreeItem api) {
        super(parent, style, api);
        this.parent = parent;
        if (iter == 0) {
            ((SwtTree) parent.getImpl()).createItem(this.getApi(), parentIter, index);
        } else {
            assert getApi().handle == 0;
            getApi().handle = OS.g_malloc(GTK.GtkTreeIter_sizeof());
            if (getApi().handle == 0)
                error(SWT.ERROR_NO_HANDLES);
            C.memmove(getApi().handle, iter, GTK.GtkTreeIter_sizeof());
        }
    }
    ...
    Color _getBackground() {
        long[] ptr = new long[1];
        GTK.gtk_tree_model_get(((SwtTree) parent.getImpl()).modelHandle, getApi().handle, SwtTree.BACKGROUND_COLUMN, ptr, -1);
        if (ptr[0] == 0)
            return parent.getBackground();
        GdkRGBA gdkRGBA = new GdkRGBA();
        OS.memmove(gdkRGBA, ptr[0], GdkRGBA.sizeof);
        GDK.gdk_rgba_free(ptr[0]);
        return SwtColor.gtk_new(display, gdkRGBA);
    }
    ...
    Color _getBackground(int index) {
        ...
        long[] ptr = new long[1];
        int modelIndex = ((SwtTree) parent.getImpl()).columnCount == 0 ? SwtTree.FIRST_COLUMN : ((SwtTreeColumn) ((SwtTree) parent.getImpl()).columns[index].getImpl()).modelIndex;
        GTK.gtk_tree_model_get(((SwtTree) parent.getImpl()).modelHandle, getApi().handle, modelIndex + SwtTree.CELL_BACKGROUND, ptr, -1);
        if (ptr[0] == 0)
            return _getBackground();
        GdkRGBA gdkRGBA = new GdkRGBA();
        OS.memmove(gdkRGBA, ptr[0], GdkRGBA.sizeof);
    ...
    boolean _getChecked() {
        int[] ptr = new int[1];
        GTK.gtk_tree_model_get(((SwtTree) parent.getImpl()).modelHandle, getApi().handle, SwtTree.CHECKED_COLUMN, ptr, -1);
        return ptr[0] != 0;
    }

    Color _getForeground() {
        long[] ptr = new long[1];
        GTK.gtk_tree_model_get(((SwtTree) parent.getImpl()).modelHandle, getApi().handle, SwtTree.FOREGROUND_COLUMN, ptr, -1);
        if (ptr[0] == 0)
    ...
    Color _getForeground(int index) {
        ...
        long[] ptr = new long[1];
        int modelIndex = ((SwtTree) parent.getImpl()).columnCount == 0 ? SwtTree.FIRST_COLUMN : ((SwtTreeColumn) ((SwtTree) parent.getImpl()).columns[index].getImpl()).modelIndex;
        GTK.gtk_tree_model_get(((SwtTree) parent.getImpl()).modelHandle, getApi().handle, modelIndex + SwtTree.CELL_FOREGROUND, ptr, -1);
        if (ptr[0] == 0)
            return _getForeground();
        GdkRGBA gdkRGBA = new GdkRGBA();
        OS.memmove(gdkRGBA, ptr[0], GdkRGBA.sizeof);
    ...
    Image _getImage(int index) {
        ...
        int modelIndex = ((SwtTree) parent.getImpl()).columnCount == 0 ? SwtTree.FIRST_COLUMN : ((SwtTreeColumn) ((SwtTree) parent.getImpl()).columns[index].getImpl()).modelIndex;
        GTK.gtk_tree_model_get(((SwtTree) parent.getImpl()).modelHandle, getApi().handle, modelIndex + SwtTree.CELL_SURFACE, surfaceHandle, -1);
        if (surfaceHandle[0] == 0)
            return null;
        int imageIndex = ((SwtTree) parent.getImpl()).imageList.indexOf(surfaceHandle[0]);
        if (imageIndex == -1) {
            return null;
        } else {
            return ((SwtTree) parent.getImpl()).imageList.get(imageIndex);
        }
    }
    ...
    String _getText(int index) {
        ...
        long[] ptr = new long[1];
        int modelIndex = ((SwtTree) parent.getImpl()).columnCount == 0 ? SwtTree.FIRST_COLUMN : ((SwtTreeColumn) ((SwtTree) parent.getImpl()).columns[index].getImpl()).modelIndex;
        GTK.gtk_tree_model_get(((SwtTree) parent.getImpl()).modelHandle, getApi().handle, modelIndex + SwtTree.CELL_TEXT, ptr, -1);
        //$NON-NLS-1$
        if (ptr[0] == 0)
            return "";
    ...
    void clear() {
        if (((SwtTree) parent.getImpl()).currentItem == this.getApi())
            return;
        if (cached || (parent.style & SWT.VIRTUAL) == 0) {
            int columnCount = GTK.gtk_tree_model_get_n_columns(((SwtTree) parent.getImpl()).modelHandle);
            /* the columns before FOREGROUND_COLUMN contain int values, subsequent columns contain pointers */
            for (int i = SwtTree.CHECKED_COLUMN; i < SwtTree.FOREGROUND_COLUMN; i++) {
                GTK.gtk_tree_store_set(((SwtTree) parent.getImpl()).modelHandle, getApi().handle, i, 0, -1);
            }
            for (int i = SwtTree.FOREGROUND_COLUMN; i < columnCount; i++) {
                GTK.gtk_tree_store_set(((SwtTree) parent.getImpl()).modelHandle, getApi().handle, i, (long) 0, -1);
            }
        }
        cached = false;
        font = null;
    ...
    public void clear(int index, boolean all) {
        checkWidget();
        ((SwtTree) parent.getImpl()).clear(getApi().handle, index, all);
    }
    ...
    public void clearAll(boolean all) {
        checkWidget();
        ((SwtTree) parent.getImpl()).clearAll(all, getApi().handle);
    }

    @Override
    void destroyWidget() {
        ((SwtTree) parent.getImpl()).releaseItem(this.getApi(), false);
    ...
    Rectangle getBoundsInPixels(int index) {
        ...
        long column = 0;
        if (index >= 0 && index < ((SwtTree) parent.getImpl()).columnCount) {
            column = ((SwtTree) parent.getImpl()).columns[index].handle;
        } else {
        ...
        if ((parent.getStyle() & SWT.MIRRORED) != 0)
            rect.x = ((SwtTree) parent.getImpl()).getClientWidth() - rect.width - rect.x;
        GTK.gtk_tree_path_free(path);
        if (index == 0 && (parent.style & SWT.CHECK) != 0) {
            int[] x = new int[1], w = new int[1];
            gtk_tree_view_column_cell_get_position(column, ((SwtTree) parent.getImpl()).checkRenderer, x, w);
            rect.x += x[0] + w[0];
            rect.width -= x[0] + w[0];
        }
    ...
    Rectangle getBoundsInPixels() {
        ...
        if (column == 0)
            return new Rectangle(0, 0, 0, 0);
        long textRenderer = ((SwtTree) parent.getImpl()).getTextRenderer(column);
        long pixbufRenderer = ((SwtTree) parent.getImpl()).getPixbufRenderer(column);
        if (textRenderer == 0 || pixbufRenderer == 0)
            return new Rectangle(0, 0, 0, 0);
        long path = GTK.gtk_tree_model_get_path(((SwtTree) parent.getImpl()).modelHandle, getApi().handle);
        GTK.gtk_widget_realize(parentHandle);
        boolean isExpander = GTK.gtk_tree_model_iter_n_children(((SwtTree) parent.getImpl()).modelHandle, getApi().handle) > 0;
        boolean isExpanded = GTK.gtk_tree_view_row_expanded(parentHandle, path);
        GTK.gtk_tree_view_column_cell_set_cell_data(column, ((SwtTree) parent.getImpl()).modelHandle, getApi().handle, isExpander, isExpanded);
        GdkRectangle rect = new GdkRectangle();
        GTK.gtk_tree_view_get_cell_area(parentHandle, path, column, rect);
        if ((parent.getStyle() & SWT.MIRRORED) != 0)
            rect.x = ((SwtTree) parent.getImpl()).getClientWidth() - rect.width - rect.x;
        ...
        int[] x = new int[1], w = new int[1];
        ((SwtTree) parent.getImpl()).ignoreSize = true;
        gtk_cell_renderer_get_preferred_size(textRenderer, parentHandle, w, null);
        ((SwtTree) parent.getImpl()).ignoreSize = false;
        rect.width = w[0];
    ...
    public int getItemCount() {
        ...
        if (!((SwtTree) parent.getImpl()).checkData(this.getApi()))
            error(SWT.ERROR_WIDGET_DISPOSED);
        return GTK.gtk_tree_model_iter_n_children(((SwtTree) parent.getImpl()).modelHandle, getApi().handle);
    }
    ...
    public TreeItem getItem(int index) {
        ...
        try {
            if (!GTK.gtk_tree_model_iter_nth_child(((SwtTree) parent.getImpl()).modelHandle, iter, getApi().handle, index))
                error(SWT.ERROR_INVALID_RANGE);
            return ((SwtTree) parent.getImpl())._getItem(getApi().handle, iter, index);
        } finally {
            OS.g_free(iter);
        }
    }
    ...
    public TreeItem[] getItems() {
        ...
        if (!((SwtTree) parent.getImpl()).checkData(this.getApi()))
            error(SWT.ERROR_WIDGET_DISPOSED);
        return ((SwtTree) parent.getImpl()).getItems(getApi().handle);
    }
    ...
    public TreeItem getParentItem() {
        ...
        if (depth > 1) {
            ...
            long iter = OS.g_malloc(GTK.GtkTreeIter_sizeof());
            if (GTK.gtk_tree_model_get_iter(((SwtTree) parent.getImpl()).modelHandle, iter, path)) {
                item = ((SwtTree) parent.getImpl())._getItem(iter);
            }
            OS.g_free(iter);
        }
        GTK.gtk_tree_path_free(path);
    ...
    public int indexOf(TreeItem item) {
        ...
        long currentPath = GTK.gtk_tree_model_get_path(((SwtTree) parent.getImpl()).modelHandle, getApi().handle);
        long parentPath = GTK.gtk_tree_model_get_path(((SwtTree) parent.getImpl()).modelHandle, item.handle);
        ...
        if (!isParent)
            return index;
        long path = GTK.gtk_tree_model_get_path(((SwtTree) parent.getImpl()).modelHandle, item.handle);
        if (depth > 1) {
    ...
    void releaseChildren(boolean destroy) {
        if (destroy) {
            ((SwtTree) parent.getImpl()).releaseItems(getApi().handle);
        }
    ...
    public void removeAll() {
        checkWidget();
        long modelHandle = ((SwtTree) parent.getImpl()).modelHandle;
        int length = GTK.gtk_tree_model_iter_n_children(modelHandle, getApi().handle);
        if (length == 0)
            return;
        long iter = OS.g_malloc(GTK.GtkTreeIter_sizeof());
        if (iter == 0)
        ...
        int[] value = new int[1];
        while (GTK.gtk_tree_model_iter_children(modelHandle, iter, getApi().handle)) {
            GTK.gtk_tree_model_get(modelHandle, iter, SwtTree.ID_COLUMN, value, -1);
            TreeItem item = value[0] != -1 ? ((SwtTree) parent.getImpl()).items[value[0]] : null;
    ...
    public void setBackground(Color color) {
        ...
        GTK.gtk_tree_store_set(((SwtTree) parent.getImpl()).modelHandle, getApi().handle, SwtTree.BACKGROUND_COLUMN, gdkRGBA, -1);
        cached = true;
    }
    ...
    public void setBackground(int index, Color color) {
        ...
        int modelIndex = ((SwtTree) parent.getImpl()).columnCount == 0 ? SwtTree.FIRST_COLUMN : ((SwtTreeColumn) ((SwtTree) parent.getImpl()).columns[index].getImpl()).modelIndex;
        GdkRGBA gdkRGBA = color != null ? color.handle : null;
        GTK.gtk_tree_store_set(((SwtTree) parent.getImpl()).modelHandle, getApi().handle, modelIndex + SwtTree.CELL_BACKGROUND, gdkRGBA, -1);
        cached = true;
        updated = true;
        if (color != null) {
            boolean customDraw = (((SwtTree) parent.getImpl()).columnCount == 0) ? ((SwtTree) parent.getImpl()).firstCustomDraw : ((SwtTreeColumn) ((SwtTree) parent.getImpl()).columns[index].getImpl()).customDraw;
            if (!customDraw) {
                if ((parent.style & SWT.VIRTUAL) == 0) {
                    long parentHandle = parent.handle;
                    long column = 0;
                    ...
                    if (column == 0)
                        return;
                    long textRenderer = ((SwtTree) parent.getImpl()).getTextRenderer(column);
                    long imageRenderer = ((SwtTree) parent.getImpl()).getPixbufRenderer(column);
                    GTK.gtk_tree_view_column_set_cell_data_func(column, textRenderer, ((SwtDisplay) display.getImpl()).cellDataProc, parentHandle, 0);
                    GTK.gtk_tree_view_column_set_cell_data_func(column, imageRenderer, ((SwtDisplay) display.getImpl()).cellDataProc, parentHandle, 0);
                }
                if (((SwtTree) parent.getImpl()).columnCount == 0) {
                    ((SwtTree) parent.getImpl()).firstCustomDraw = true;
                } else {
                    ((SwtTreeColumn) ((SwtTree) parent.getImpl()).columns[index].getImpl()).customDraw = true;
                }
            }
        }
    }
    ...
    public void setChecked(boolean checked) {
        ...
        if (_getChecked() == checked)
            return;
        GTK.gtk_tree_store_set(((SwtTree) parent.getImpl()).modelHandle, getApi().handle, SwtTree.CHECKED_COLUMN, checked, -1);
        /*
	...
	*/
        GTK.gtk_tree_store_set(((SwtTree) parent.getImpl()).modelHandle, getApi().handle, SwtTree.GRAYED_COLUMN, !checked ? false : grayed, -1);
        cached = true;
    }
    ...
    public void setFont(Font font) {
        ...
        GTK.gtk_tree_store_set(((SwtTree) parent.getImpl()).modelHandle, getApi().handle, SwtTree.FONT_COLUMN, fontHandle, -1);
        cached = true;
    }
    ...
    public void setFont(int index, Font font) {
        ...
        int modelIndex = ((SwtTree) parent.getImpl()).columnCount == 0 ? SwtTree.FIRST_COLUMN : ((SwtTreeColumn) ((SwtTree) parent.getImpl()).columns[index].getImpl()).modelIndex;
        long fontHandle = font != null ? font.handle : 0;
        GTK.gtk_tree_store_set(((SwtTree) parent.getImpl()).modelHandle, getApi().handle, modelIndex + SwtTree.CELL_FONT, fontHandle, -1);
        cached = true;
        if (font != null) {
            boolean customDraw = (((SwtTree) parent.getImpl()).columnCount == 0) ? ((SwtTree) parent.getImpl()).firstCustomDraw : ((SwtTreeColumn) ((SwtTree) parent.getImpl()).columns[index].getImpl()).customDraw;
            if (!customDraw) {
    ...
    public void setForeground(Color color) {
        ...
        if (_getForeground().equals(color))
            return;
        GdkRGBA gdkRGBA = color != null ? color.handle : null;
        GTK.gtk_tree_store_set(((SwtTree) parent.getImpl()).modelHandle, getApi().handle, SwtTree.FOREGROUND_COLUMN, gdkRGBA, -1);
        cached = true;
    }
    ...
    public void setForeground(int index, Color color) {
        ...
        int modelIndex = ((SwtTree) parent.getImpl()).columnCount == 0 ? SwtTree.FIRST_COLUMN : ((SwtTreeColumn) ((SwtTree) parent.getImpl()).columns[index].getImpl()).modelIndex;
        GdkRGBA gdkRGBA = color != null ? color.handle : null;
        GTK.gtk_tree_store_set(((SwtTree) parent.getImpl()).modelHandle, getApi().handle, modelIndex + SwtTree.CELL_FOREGROUND, gdkRGBA, -1);
        cached = true;
        updated = true;
        if (color != null) {
            boolean customDraw = (((SwtTree) parent.getImpl()).columnCount == 0) ? ((SwtTree) parent.getImpl()).firstCustomDraw : ((SwtTreeColumn) ((SwtTree) parent.getImpl()).columns[index].getImpl()).customDraw;
	...
	*/
        int[] ptr = new int[1];
        GTK.gtk_tree_model_get(((SwtTree) parent.getImpl()).modelHandle, getApi().handle, SwtTree.CHECKED_COLUMN, ptr, -1);
        GTK.gtk_tree_store_set(((SwtTree) parent.getImpl()).modelHandle, getApi().handle, SwtTree.GRAYED_COLUMN, ptr[0] == 0 ? false : grayed, -1);
        cached = true;
    }
    ...
    public void setImage(int index, Image image) {
        ...
        if (image != null) {
            ImageList imageList = ((SwtTree) parent.getImpl()).imageList;
            if (imageList == null)
                imageList = ((SwtTree) parent.getImpl()).imageList = new ImageList();
        ...
        if (!((SwtTree) parent.getImpl()).pixbufSizeSet) {
            if (image != null) {
                ...
                if (iWidth > currentWidth[0] || iHeight > currentHeight[0]) {
                    ...
                    ((SwtTree) parent.getImpl()).pixbufHeight = iHeight;
                    ((SwtTree) parent.getImpl()).pixbufWidth = iWidth;
                    /*
		 ...
		 */
            if (((SwtTree) parent.getImpl()).pixbufWidth > Math.max(currentWidth[0], 0) || ((SwtTree) parent.getImpl()).pixbufHeight > Math.max(currentHeight[0], 0)) {
                GTK.gtk_cell_renderer_set_fixed_size(pixbufRenderer, ((SwtTree) parent.getImpl()).pixbufWidth, ((SwtTree) parent.getImpl()).pixbufHeight);
            }
        }
        GTK.gtk_tree_store_set(((SwtTree) parent.getImpl()).modelHandle, getApi().handle, modelIndex + SwtTree.CELL_PIXBUF, pixbuf, -1);
        /*
	 ...
	 */
        ...
        GTK.gtk_tree_store_set(((SwtTree) parent.getImpl()).modelHandle, getApi().handle, modelIndex + SwtTree.CELL_SURFACE, surface, -1);
        cached = true;
        updated = true;
    }
    ...
    public void setItemCount(int count) {
        ...
        ((SwtTree) parent.getImpl()).setItemCount(getApi().handle, count);
    }
    ...
    public void setText(int index, String string) {
        ...
        int modelIndex = ((SwtTree) parent.getImpl()).columnCount == 0 ? SwtTree.FIRST_COLUMN : ((SwtTreeColumn) ((SwtTree) parent.getImpl()).columns[index].getImpl()).modelIndex;
        GTK.gtk_tree_store_set(((SwtTree) parent.getImpl()).modelHandle, getApi().handle, modelIndex + SwtTree.CELL_TEXT, buffer, -1);
        cached = true;
        updated = true;
    }
