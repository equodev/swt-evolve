 ...
 */
public class SwtScrollBar extends SwtWidget implements IScrollBar {
     ...
     */
    SwtScrollBar(Scrollable parent, int style, ScrollBar api) {
    ...
    void deregister() {
        ...
        if (adjustmentHandle != 0)
            ((SwtDisplay) display.getImpl()).removeWidget(adjustmentHandle);
    }

    @Override
    void destroyWidget() {
        ((SwtScrollable) parent.getImpl()).destroyScrollBar(this.getApi());
        releaseHandle();
    }
    ...
    Rectangle getThumbBoundsInPixels() {
        ...
        if (GTK.GTK4) {
            ...
            boolean success = GTK4.gtk_widget_translate_coordinates(((SwtScrollable) parent.getImpl()).scrolledHandle, ((SwtShell) parent.getShell().getImpl()).shellHandle, 0, 0, origin_x, origin_y);
            if (success) {
                rect.x += origin_x[0];
                rect.y += origin_y[0];
            }
            success = GTK4.gtk_widget_translate_coordinates(parent.handle, ((SwtShell) parent.getShell().getImpl()).shellHandle, 0, 0, origin_x, origin_y);
            if (success) {
                rect.x -= origin_x[0];
                rect.y -= origin_y[0];
            }
            ...
            long window = gtk_widget_get_window(((SwtScrollable) parent.getImpl()).scrolledHandle);
            if (window != 0)
	 ...
	 */
        ...
        if (!GTK.GTK4) {
            ...
            GTK3.gtk_widget_style_get(getApi().handle, OS.has_backward_stepper, has_stepper, 0);
            hasB = has_stepper[0] != 0;
            GTK3.gtk_widget_style_get(getApi().handle, OS.has_secondary_backward_stepper, has_stepper, 0);
            hasB2 = has_stepper[0] != 0;
            GTK3.gtk_widget_style_get(getApi().handle, OS.has_forward_stepper, has_stepper, 0);
            hasF = has_stepper[0] != 0;
            GTK3.gtk_widget_style_get(getApi().handle, OS.has_secondary_forward_stepper, has_stepper, 0);
    ...
    public boolean getVisible() {
        checkWidget();
        long scrolledHandle = ((SwtScrollable) parent.getImpl()).scrolledHandle;
        int[] hsp = new int[1], vsp = new int[1];
        GTK.gtk_scrolled_window_get_policy(scrolledHandle, hsp, vsp);
        if ((getApi().style & SWT.HORIZONTAL) != 0) {
            return hsp[0] != GTK.GTK_POLICY_NEVER && hsp[0] != GTK.GTK_POLICY_EXTERNAL && GTK.gtk_widget_get_visible(getApi().handle);
        } else {
            return vsp[0] != GTK.GTK_POLICY_NEVER && vsp[0] != GTK.GTK_POLICY_EXTERNAL && GTK.gtk_widget_get_visible(getApi().handle);
        }
    }
    ...
    long gtk_value_changed(long range) {
        ...
        ((SwtScrollable) parent.getImpl()).updateScrollBarValue(this.getApi());
    ...
    void hookEvents() {
        super.hookEvents();
        OS.g_signal_connect_closure(adjustmentHandle, OS.value_changed, ((SwtDisplay) display.getImpl()).getClosure(VALUE_CHANGED), false);
        if (GTK.GTK4) {
            //TODO: GTK4 change-value moved to gtk_scroll_child in Composite, event-after
            long clickGesture = GTK4.gtk_gesture_click_new();
            GTK4.gtk_widget_add_controller(getApi().handle, clickGesture);
            OS.g_signal_connect(clickGesture, OS.pressed, ((SwtDisplay) display.getImpl()).gesturePressReleaseProc, GESTURE_PRESSED);
        } else {
            OS.g_signal_connect_closure(getApi().handle, OS.change_value, ((SwtDisplay) display.getImpl()).getClosure(CHANGE_VALUE), false);
            ...
            OS.g_signal_connect_closure_by_id(getApi().handle, ((SwtDisplay) display.getImpl()).signalIds[EVENT_AFTER], 0, ((SwtDisplay) display.getImpl()).getClosure(EVENT_AFTER), false);
        }
    }
    ...
    void register() {
        super.register();
        if (adjustmentHandle != 0)
            ((SwtDisplay) display.getImpl()).addWidget(adjustmentHandle, this.getApi());
    }
    ...
    void setOrientation(boolean create) {
        ...
        if ((parent.style & SWT.MIRRORED) != 0 || !create) {
            if ((parent.state & CANVAS) != 0) {
                if ((getApi().style & SWT.HORIZONTAL) != 0) {
                    GTK.gtk_range_set_inverted(getApi().handle, (parent.style & SWT.RIGHT_TO_LEFT) != 0);
                }
            }
        }
    }
    ...
    public void setVisible(boolean visible) {
        checkWidget();
        if (((SwtScrollable) parent.getImpl()).setScrollBarVisible(this.getApi(), visible)) {
    ...
    public long _adjustmentHandle() {
        return adjustmentHandle;
    }
