...
public class SwtProgressBar extends SwtControl implements IProgressBar {
    ...
    void createHandle(int index) {
        ...
        getApi().handle = GTK.gtk_progress_bar_new();
        if (getApi().handle == 0)
            error(SWT.ERROR_NO_HANDLES);
        if (GTK.GTK4) {
        ...
        if (vertical) {
            GTK.gtk_orientable_set_orientation(getApi().handle, GTK.GTK_ORIENTATION_VERTICAL);
            GTK.gtk_progress_bar_set_inverted(getApi().handle, true);
        } else {
            GTK.gtk_orientable_set_orientation(getApi().handle, GTK.GTK_ORIENTATION_HORIZONTAL);
            GTK.gtk_progress_bar_set_inverted(getApi().handle, false);
        }
 ...
 */
	...
	*/
    ...
    long timerProc(long widget) {
        if (isVisible())
            GTK.gtk_progress_bar_pulse(getApi().handle);
        return 1;
    }
