...
public class SwtSlider extends SwtControl implements ISlider {
    ...
    void createHandle(int index) {
        ...
        if ((getApi().style & SWT.HORIZONTAL) != 0) {
            getApi().handle = GTK.gtk_scrollbar_new(GTK.GTK_ORIENTATION_HORIZONTAL, adjustmentHandle);
        } else {
            getApi().handle = GTK.gtk_scrollbar_new(GTK.GTK_ORIENTATION_VERTICAL, adjustmentHandle);
        }
        if (getApi().handle == 0)
            error(SWT.ERROR_NO_HANDLES);
        /*
	 ...
	 */
        rangeHandle = GTK.GTK4 ? GTK4.gtk_widget_get_first_child(getApi().handle) : getApi().handle;
        if (GTK.GTK4) {
    ...
    void hookEvents() {
        super.hookEvents();
        OS.g_signal_connect_closure(rangeHandle, OS.change_value, ((SwtDisplay) display.getImpl()).getClosure(CHANGE_VALUE), false);
        OS.g_signal_connect_closure(rangeHandle, OS.value_changed, ((SwtDisplay) display.getImpl()).getClosure(VALUE_CHANGED), false);
    }
    ...
    void register() {
        super.register();
        if (GTK.GTK4) {
            ((SwtDisplay) display.getImpl()).addWidget(rangeHandle, this.getApi());
        } else {
            long hAdjustment = GTK.gtk_range_get_adjustment(getApi().handle);
            ((SwtDisplay) display.getImpl()).addWidget(hAdjustment, this.getApi());
        }
    }
    ...
    void deregister() {
        super.deregister();
        if (GTK.GTK4) {
            ((SwtDisplay) display.getImpl()).removeWidget(rangeHandle);
        } else {
            long hAdjustment = GTK.gtk_range_get_adjustment(getApi().handle);
            ((SwtDisplay) display.getImpl()).removeWidget(hAdjustment);
        }
    }
    ...
    void setOrientation(boolean create) {
        ...
        if ((getApi().style & SWT.RIGHT_TO_LEFT) != 0 || !create) {
            if ((getApi().style & SWT.HORIZONTAL) != 0) {
                GTK.gtk_range_set_inverted(rangeHandle, (getApi().style & SWT.RIGHT_TO_LEFT) != 0);
            }
        }
    }
    ...
    public long _rangeHandle() {
        return rangeHandle;
    }

    public int _scrollType() {
        return scrollType;
    }
