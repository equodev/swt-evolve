...
public class SWTExpandBar extends SWTComposite implements IExpandBar {
     ...
     * @see Widget#getStyle
     */
    public SWTExpandBar(SWTComposite parent, int style) {
        super(parent, style);
    }
    ...
    void createItem(SWTExpandItem item, int style, int index) {
        if (!(0 <= index && index <= itemCount))
            error(SWT.ERROR_INVALID_RANGE);
        if (itemCount == items.length) {
            SWTExpandItem[] newItems = new SWTExpandItem[itemCount + 4];
    ...
    void createWidget(int index) {
        ...
        items = new SWTExpandItem[4];
    }

    void destroyItem(SWTExpandItem item) {
        int index = 0;
    ...
    boolean forceFocus(long focusHandle) {
        ...
        for (int i = 0; i < itemCount; i++) {
            SWTExpandItem item = (SWTExpandItem) (items[i]);
            if (item.setFocus())
                return true;
        }
     ...
     * </ul>
     */
    public IExpandItem getItem(int index) {
        checkWidget();
        if (!(0 <= index && index < itemCount))
            error(SWT.ERROR_INVALID_RANGE);
        return items[index];
     ...
     * </ul>
     */
    public IExpandItem[] getItems() {
        checkWidget();
        SWTExpandItem[] result = new SWTExpandItem[itemCount];
    ...
    long gtk_key_press_event(long widget, long event) {
        ...
        while ((index = (index + offset + itemCount) % itemCount) != start) {
            SWTExpandItem item = (SWTExpandItem) (items[index]);
            if (item.setFocus())
                return result;
        }
        return result;
     ...
     * </ul>
     */
    public int indexOf(IExpandItem item_) {
        SWTExpandItem item = (SWTExpandItem) item_;
        checkWidget();
        if (item == null)
