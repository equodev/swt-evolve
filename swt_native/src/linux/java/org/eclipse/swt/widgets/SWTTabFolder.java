...
public class SWTTabFolder extends SWTComposite implements ITabFolder {
     ...
     * @see Widget#getStyle
     */
    public SWTTabFolder(SWTComposite parent, int style) {
	...
	*/
    ...
    void createWidget(int index) {
        ...
        items = new SWTTabItem[4];
    }

    void createItem(SWTTabItem item, int index) {
        int itemCount = 0;
        if (GTK.GTK4) {
        ...
        if (itemCount == items.length) {
            SWTTabItem[] newItems = new SWTTabItem[items.length + 4];
        ...
        if (itemCount == 1) {
            ...
            event.item = Widget.getInstance(items[0]);
            sendSelectionEvent(SWT.Selection, event, false);
            // the widget could be destroyed at this point
        }
    }

    void destroyItem(SWTTabItem item) {
        int index = 0;
        ...
        if (index == oldIndex) {
            ...
            if (newIndex != -1) {
                SWTControl control = (SWTControl) (items[newIndex].getControl());
                if (control != null && !control.isDisposed()) {
                    control.setBoundsInPixels(getClientAreaInPixels());
                    control.setVisible(true);
                }
                Event event = new Event();
                event.item = Widget.getInstance(items[newIndex]);
    ...
    IControl[] _getChildren() {
        ...
        for (int itemIndex = 0; itemIndex < itemCount; itemIndex++) {
            SWTTabItem tabItem = (SWTTabItem) (items[itemIndex]);
                ...
                if (GTK.GTK4) {
                    for (long child = GTK4.gtk_widget_get_first_child(parentHandle); child != 0; child = GTK4.gtk_widget_get_next_sibling(child)) {
                        ...
                        if (childWidget != null && childWidget instanceof SWTControl && childWidget != this) {
                            children[childrenCount] = (SWTControl) childWidget;
                            childrenCount++;
                        }
                    }
                } else {
                        ...
                        if (handle != 0) {
                            ...
                            if (widget != null && widget != this) {
                                if (widget instanceof SWTControl) {
                                    children[childrenCount++] = (SWTControl) widget;
                                }
                            }
                        }
                        OS.g_list_free(list);
    ...
    public ITabItem getItem(Point point) {
        ...
        for (int i = 0; i < itemCount; i++) {
            SWTTabItem item = (SWTTabItem) (items[i]);
    ...
    public ITabItem[] getItems() {
        ...
        SWTTabItem[] result = new SWTTabItem[count];
    ...
    public ITabItem[] getSelection() {
        ...
        int index = GTK.gtk_notebook_get_current_page(handle);
        if (index == -1)
            return new SWTTabItem[0];
        return new SWTTabItem[] { items[index] };
    }
    ...
    long gtk_switch_page(long notebook, long page, int page_num) {
        SWTTabItem item = (SWTTabItem) (items[page_num]);
        if (GTK.GTK4) {
            ...
            int index = GTK.gtk_notebook_get_current_page(handle);
            if (index != -1) {
                SWTControl control = (SWTControl) (items[index].getControl());
                if (control != null && !control.isDisposed()) {
     ...
     * </ul>
     */
    public int indexOf(ITabItem item_) {
        SWTTabItem item = (SWTTabItem) item_;
        checkWidget();
        if (item == null)
            error(SWT.ERROR_NULL_ARGUMENT);
        int index = -1;
    ...
    int setBounds(int x, int y, int width, int height, boolean move, boolean resize) {
        ...
        if ((result & RESIZED) != 0) {
            ...
            if (index != -1) {
                SWTTabItem item = (SWTTabItem) (items[index]);
    ...
    void setFontDescription(long font) {
        ...
        SWTTabItem[] items = (SWTTabItem[]) (getItems());
        for (int i = 0; i < items.length; i++) {
            if (items[i] != null) {
    ...
    void setSelection(int index, boolean notify) {
        ...
        if (oldIndex == index)
            return;
        if (oldIndex != -1) {
            SWTTabItem item = (SWTTabItem) (items[oldIndex]);
            SWTControl control = item.control;
            if (control != null && !control.isDisposed()) {
                control.setVisible(false);
            }
        ...
        int newIndex = GTK.gtk_notebook_get_current_page(handle);
        if (newIndex != -1) {
            SWTTabItem item = (SWTTabItem) (items[newIndex]);
            SWTControl control = item.control;
            if (control != null && !control.isDisposed()) {
                control.setBoundsInPixels(getClientAreaInPixels());
                control.setVisible(true);
     ...
     * @since 3.2
     */
    public void setSelection(ITabItem item_) {
        SWTTabItem item = (SWTTabItem) item_;
        if (item == null)
            error(SWT.ERROR_NULL_ARGUMENT);
        setSelection(new SWTTabItem[] { item });
    }
     ...
     * </ul>
     */
    public void setSelection(ITabItem[] items_) {
        SWTTabItem[] items = (SWTTabItem[]) items_;
        checkWidget();
        if (items == null)
            error(SWT.ERROR_NULL_ARGUMENT);
        if (items.length == 0) {
