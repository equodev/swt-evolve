...
public class SWTColorDialog extends SWTDialog implements IColorDialog {
     ...
     * @see Widget#getStyle
     */
    public SWTColorDialog(SWTShell parent) {
     ...
     * @see Widget#getStyle
     */
    public SWTColorDialog(SWTShell parent, int style) {
    ...
    public RGB open() {
        ...
        if (rgbs != null) {
            ...
            GTK.gtk_color_chooser_add_palette(handle, GTK.GTK_ORIENTATION_HORIZONTAL, colorsPerRow, rgbs.length, gdkRGBAS);
            GTK.gtk_color_chooser_set_rgba(handle, rgba);
        ...
        display.addIdleProc();
        SWTDialog oldModal = (SWTDialog) (null);
        ...
        int response;
        if (GTK.GTK4) {
            response = SyncDialogUtil.run(Display.getInstance(display), handle, false);
        } else {
