...
public class SwtMenuItem extends SwtItem implements IMenuItem {
    ...
    void addAccelerators(long accelGroup) {
        ...
        if (menu != null)
            ((SwtMenu) menu.getImpl()).addAccelerators(accelGroup);
    }
    ...
    void createHandle(int index) {
        ...
        if (GTK.GTK4) {
            switch(getApi().style & bits) {
                case SWT.SEPARATOR:
                    modelHandle = OS.g_menu_new();
                    getApi().handle = OS.g_menu_item_new_section(null, modelHandle);
                    break;
                case SWT.RADIO:
                    long stringVariantType = OS.g_variant_type_new(OS.G_VARIANT_TYPE_STRING);
                    actionHandle = OS.g_simple_action_new_stateful(Converter.javaStringToCString(String.valueOf(this.hashCode())), stringVariantType, OS.g_variant_new_string(Converter.javaStringToCString("untoggled")));
                    OS.g_action_map_add_action(((SwtMenu) parent.getImpl()).actionGroup, actionHandle);
                    actionName = String.valueOf(parent.hashCode()) + "." + String.valueOf(this.hashCode()) + "::toggled";
                    getApi().handle = OS.g_menu_item_new(null, Converter.javaStringToCString(actionName));
                    OS.g_variant_type_free(stringVariantType);
                    break;
                case SWT.CHECK:
                    long boolVariantType = OS.g_variant_type_new(OS.G_VARIANT_TYPE_BOOLEAN);
                    actionHandle = OS.g_simple_action_new_stateful(Converter.javaStringToCString(String.valueOf(this.hashCode())), 0, OS.g_variant_new_boolean(false));
                    OS.g_action_map_add_action(((SwtMenu) parent.getImpl()).actionGroup, actionHandle);
                ...
                case SWT.CASCADE:
                    modelHandle = OS.g_menu_new();
                    getApi().handle = OS.g_menu_item_new_submenu(Converter.javaStringToCString(""), modelHandle);
                    break;
                case SWT.PUSH:
                default:
            ...
            Section selectedSection = ((SwtMenu) parent.getImpl()).sections.getLast();
            for (Section section : ((SwtMenu) parent.getImpl()).sections) {
            ...
            if ((getApi().style & SWT.SEPARATOR) != 0) {
                section = ((SwtMenu) parent.getImpl()).new Section(this.getApi());
                ...
                for (int i = 0; i < itemsToMove; i++) {
                    ...
                    ((SwtMenuItem) removedItem.getImpl()).section = section;
                }
                int sectionInsertIndex = ((SwtMenu) parent.getImpl()).sections.indexOf(selectedSection) + 1;
                ((SwtMenu) parent.getImpl()).sections.add(sectionInsertIndex, section);
                OS.g_menu_insert_item(((SwtMenu) parent.getImpl()).modelHandle, sectionInsertIndex, getApi().handle);
            } else {
                section = selectedSection;
                selectedSection.sectionItems.add(sectionRelativeIndex, this.getApi());
                OS.g_menu_insert_item(selectedSection.getSectionHandle(), sectionRelativeIndex, getApi().handle);
            }
            ((SwtMenu) parent.getImpl()).items.add(index, this.getApi());
        } else {
            byte[] buffer = new byte[1];
            switch(getApi().style & bits) {
                case SWT.SEPARATOR:
                    getApi().handle = GTK3.gtk_separator_menu_item_new();
                    if (getApi().handle == 0)
				...
				*/
                    ...
                    getApi().handle = GTK3.gtk_radio_menu_item_new(group);
                    if (getApi().handle == 0)
                        error(SWT.ERROR_NO_HANDLES);
                    ...
                    break;
                case SWT.CHECK:
                    getApi().handle = GTK3.gtk_check_menu_item_new();
                    if (getApi().handle == 0)
                        error(SWT.ERROR_NO_HANDLES);
                    labelHandle = GTK3.gtk_accel_label_new(buffer);
                    if (labelHandle == 0)
                ...
                case SWT.CASCADE:
                    getApi().handle = GTK3.gtk_menu_item_new();
                    if (getApi().handle == 0)
                        error(SWT.ERROR_NO_HANDLES);
                    labelHandle = GTK3.gtk_accel_label_new(buffer);
                    if (labelHandle == 0)
            ...
            if ((getApi().style & SWT.SEPARATOR) == 0) {
                if (boxHandle == 0) {
                    labelHandle = GTK3.gtk_bin_get_child(getApi().handle);
                }
            ...
            GTK3.gtk_menu_shell_insert(parentHandle, getApi().handle, index);
            if (!enabled)
    ...
    long getAccelGroup() {
        Menu menu = parent;
        while (menu != null && ((SwtMenu) menu.getImpl()).cascade != null) {
            menu = ((SwtMenuItem) ((SwtMenu) menu.getImpl()).cascade.getImpl()).parent;
        }
        if (menu == null)
            return 0;
        Decorations shell = ((SwtMenu) menu.getImpl()).parent;
        if (shell == null)
            return 0;
    ...
    public boolean getEnabled() {
        ...
        if (GTK.GTK4) {
            ...
            return GTK.gtk_widget_get_sensitive(getApi().handle);
        }
    }
    ...
    public boolean getSelection() {
        ...
        if (GTK.GTK4) {
            ...
            return GTK3.gtk_check_menu_item_get_active(getApi().handle);
        }
    }
    ...
    long gtk_activate(long widget) {
        if ((getApi().style & SWT.CASCADE) != 0 && menu != null)
            return 0;
        if (!GTK.GTK4) {
            /*
    ...
    long gtk_select(long item) {
        ((SwtMenu) parent.getImpl()).selectedItem = this.getApi();
        sendEvent(SWT.Arm);
        return 0;
    }
    ...
    long gtk3_show_help(long widget, long helpType) {
        ...
        if (handled) {
            ...
            handled = ((SwtMenu) parent.getImpl()).sendHelpEvent(helpType);
        }
        if (handled) {
    ...
    void hookEvents() {
        super.hookEvents();
        if (GTK.GTK4) {
            if ((getApi().style & SWT.PUSH) != 0 || (getApi().style & SWT.RADIO) != 0) {
                OS.g_signal_connect(actionHandle, OS.activate, ((SwtDisplay) display.getImpl()).activateProc, getApi().handle);
            }
        } else {
            OS.g_signal_connect_closure(getApi().handle, OS.activate, ((SwtDisplay) display.getImpl()).getClosure(ACTIVATE), false);
            OS.g_signal_connect_closure(getApi().handle, OS.select, ((SwtDisplay) display.getImpl()).getClosure(SELECT), false);
            OS.g_signal_connect_closure_by_id(getApi().handle, ((SwtDisplay) display.getImpl()).signalIds[SHOW_HELP], 0, ((SwtDisplay) display.getImpl()).getClosure(SHOW_HELP), false);
        }
    }
    ...
    void releaseParent() {
        super.releaseParent();
        if (menu != null) {
            if (((SwtMenu) menu.getImpl()).selectedItem == this.getApi())
                ((SwtMenu) menu.getImpl()).selectedItem = null;
            menu.dispose();
            menu = null;
        }
    }
    ...
    void releaseWidget() {
        super.releaseWidget();
        if (GTK.GTK4) {
            if (((SwtMenu) parent.getImpl()).actionGroup != 0 && actionName != null)
                OS.g_action_map_remove_action(((SwtMenu) parent.getImpl()).actionGroup, Converter.javaStringToCString(actionName));
        } else {
    ...
    void destroyWidget() {
        if (GTK.GTK4) {
            if ((getApi().style & SWT.SEPARATOR) != 0) {
                ...
                ((SwtMenu) parent.getImpl()).sections.remove(section);
            } else {
                OS.g_menu_remove(section.getSectionHandle(), section.sectionItems.indexOf(this.getApi()));
                section.sectionItems.remove(this.getApi());
            }
            ((SwtMenu) parent.getImpl()).items.remove(this.getApi());
            parent = null;
            if (modelHandle != 0)
                OS.g_object_unref(modelHandle);
            OS.g_object_unref(getApi().handle);
            releaseHandle();
        } else {
            super.destroyWidget();
        }
    ...
    void removeAccelerators(long accelGroup) {
        removeAccelerator(accelGroup);
        if (menu != null)
            ((SwtMenu) menu.getImpl()).removeAccelerators(accelGroup);
    }
    ...
    public void setAccelerator(int accelerator) {
        ...
        if (GTK.GTK4) {
            if (shortcutHandle != 0) {
                GTK4.gtk_shortcut_controller_remove_shortcut(((SwtMenu) parent.getImpl()).shortcutController, shortcutHandle);
                shortcutHandle = 0;
            }
    ...
    void addShortcut(int accelerator) {
        ...
        if (keyval != 0) {
            shortcutHandle = GTK4.gtk_shortcut_new(GTK4.gtk_keyval_trigger_new(keyval, mask), GTK4.gtk_named_action_new(Converter.javaStringToCString(actionName)));
            if (shortcutHandle == 0)
                error(SWT.ERROR_NO_HANDLES);
            GTK4.gtk_shortcut_controller_add_shortcut(((SwtMenu) parent.getImpl()).shortcutController, shortcutHandle);
        }
    }
    ...
    public void setEnabled(boolean enabled) {
        ...
        if (GTK.GTK4) {
            ...
            if (GTK.gtk_widget_get_sensitive(getApi().handle) == enabled)
                return;
            long accelGroup = getAccelGroup();
            if (accelGroup != 0)
                removeAccelerator(accelGroup);
    ...
    private void _setImage(Image image) {
        if (image != null) {
            ImageList imageList = ((SwtMenu) parent.getImpl()).imageList;
            if (imageList == null)
                imageList = ((SwtMenu) parent.getImpl()).imageList = new ImageList();
            ...
            if (!GTK3.GTK_IS_MENU_ITEM(getApi().handle))
                return;
    ...
    public void setMenu(Menu menu) {
        ...
        if (GTK.GTK4) {
            ...
            if (menu != null) {
                ...
                OS.g_menu_item_set_submenu(getApi().handle, ((SwtMenu) menu.getImpl()).modelHandle);
            } else {
                ((SwtMenu) oldMenu.getImpl()).cascade = null;
                OS.g_menu_item_set_submenu(getApi().handle, 0);
            }
            OS.g_menu_remove(section.getSectionHandle(), section.getItemPosition(this.getApi()));
            OS.g_menu_insert_item(section.getSectionHandle(), section.getItemPosition(this.getApi()), getApi().handle);
        } else {
            long accelGroup = getAccelGroup();
            if (accelGroup != 0)
			...
			*/
                ...
                GTK3.gtk_menu_item_set_submenu(getApi().handle, 0);
            }
            if ((this.menu = menu) != null) {
                ((SwtMenu) menu.getImpl()).cascade = this.getApi();
                GTK3.gtk_menu_item_set_submenu(getApi().handle, menu.handle);
            }
            if (accelGroup != 0)
    ...
    void setOrientation(boolean create) {
        ...
        if ((parent.style & SWT.RIGHT_TO_LEFT) != 0 || !create) {
            ...
            if (menu != null)
                ((SwtMenu) menu.getImpl())._setOrientation(parent.style & (SWT.RIGHT_TO_LEFT | SWT.LEFT_TO_RIGHT));
        }
    }
    ...
    public void setSelection(boolean selected) {
        ...
        if (GTK.GTK4) {
            ...
            OS.g_signal_handlers_block_matched(getApi().handle, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, ACTIVATE);
            GTK3.gtk_check_menu_item_set_active(getApi().handle, selected);
            if ((getApi().style & SWT.RADIO) != 0)
                GTK3.gtk_check_menu_item_set_active(groupHandle, !selected);
            OS.g_signal_handlers_unblock_matched(getApi().handle, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, ACTIVATE);
        }
    }
    ...
    public void setText(String string) {
        ...
        if (GTK.GTK4) {
            OS.g_menu_item_set_label(getApi().handle, buffer);
            MaskKeysym maskKeysym = getMaskKeysym();
            if (maskKeysym != null) {
                OS.g_menu_item_set_attribute(getApi().handle, Converter.javaStringToCString("accel"), Converter.javaStringToCString("s"), GTK.gtk_accelerator_name(maskKeysym.keysym, maskKeysym.mask));
            }
            OS.g_menu_remove(section.getSectionHandle(), section.getItemPosition(this.getApi()));
            OS.g_menu_insert_item(section.getSectionHandle(), section.getItemPosition(this.getApi()), getApi().handle);
        } else {
            if (labelHandle != 0 && GTK.GTK_IS_LABEL(labelHandle)) {
                ...
                if (GTK.GTK_IS_ACCEL_LABEL(labelHandle)) {
                    MaskKeysym maskKeysym = getMaskKeysym();
                    if (maskKeysym != null) {
                        GTK3.gtk_accel_label_set_accel_widget(labelHandle, getApi().handle);
                        GTK3.gtk_accel_label_set_accel(labelHandle, maskKeysym.keysym, maskKeysym.mask);
                    }
                    // A workaround for Ubuntu Unity global menu
                    OS.g_signal_emit_by_name(getApi().handle, OS.accel_closures_changed);
                }
            }
        }
    }
    ...
    void updateAccelerator(long accelGroup, boolean add) {
        ...
        if (keysym != 0) {
            if (add) {
                GTK3.gtk_widget_add_accelerator(getApi().handle, OS.activate, accelGroup, keysym, mask, GTK.GTK_ACCEL_VISIBLE);
            } else {
                GTK3.gtk_widget_remove_accelerator(getApi().handle, accelGroup, keysym, mask);
            }
        }
    }
    ...
    private MaskKeysym getMaskKeysym() {
        ...
        switch(word) {
            case "Del", "Delete" ->
                maskKeysym.keysym = GDK.GDK_Delete;
            case "Enter" ->
                maskKeysym.keysym = GDK.GDK_ISO_Enter;
            case "Ins", "Insert" ->
                maskKeysym.keysym = GDK.GDK_Insert;
            case "Ret", "Return" ->
                maskKeysym.keysym = GDK.GDK_Return;
            case "Up" ->
                maskKeysym.keysym = GDK.GDK_Up;
            case "Down" ->
                maskKeysym.keysym = GDK.GDK_Down;
            case "Right" ->
                maskKeysym.keysym = GDK.GDK_Right;
            case "Left" ->
                maskKeysym.keysym = GDK.GDK_Left;
            case "Page Up" ->
                maskKeysym.keysym = GDK.GDK_Page_Up;
            case "Page Down" ->
                maskKeysym.keysym = GDK.GDK_Page_Down;
            case "Keypad +", "Numpad +" ->
                maskKeysym.keysym = GDK.GDK_KP_Add;
            case "Keypad -", "Numpad -" ->
                maskKeysym.keysym = GDK.GDK_KP_Subtract;
            case "Keypad *", "Numpad *" ->
                maskKeysym.keysym = GDK.GDK_KP_Multiply;
            case "Keypad /", "Numpad /" ->
                maskKeysym.keysym = GDK.GDK_KP_Divide;
            case "ESC", "Escape" ->
                maskKeysym.keysym = GDK.GDK_Escape;
            case "Backspace" ->
                maskKeysym.keysym = GDK.GDK_BackSpace;
            case "Home" ->
                maskKeysym.keysym = GDK.GDK_Home;
            case "End" ->
                maskKeysym.keysym = GDK.GDK_End;
            case "Tab" ->
                maskKeysym.keysym = GDK.GDK_Tab;
            case "Space" ->
                maskKeysym.keysym = GDK.GDK_space;
            case "F1" ->
                maskKeysym.keysym = GDK.GDK_F1;
            case "F2" ->
                maskKeysym.keysym = GDK.GDK_F2;
            case "F3" ->
                maskKeysym.keysym = GDK.GDK_F3;
            case "F4" ->
                maskKeysym.keysym = GDK.GDK_F4;
            case "F5" ->
                maskKeysym.keysym = GDK.GDK_F5;
            case "F6" ->
                maskKeysym.keysym = GDK.GDK_F6;
            case "F7" ->
                maskKeysym.keysym = GDK.GDK_F7;
            case "F8" ->
                maskKeysym.keysym = GDK.GDK_F8;
            case "F9" ->
                maskKeysym.keysym = GDK.GDK_F9;
            case "F10" ->
                maskKeysym.keysym = GDK.GDK_F10;
            case "F11" ->
                maskKeysym.keysym = GDK.GDK_F11;
            case "F12" ->
                maskKeysym.keysym = GDK.GDK_F12;
            case "F13" ->
                maskKeysym.keysym = GDK.GDK_F13;
            case "F14" ->
                maskKeysym.keysym = GDK.GDK_F14;
            case "F15" ->
                maskKeysym.keysym = GDK.GDK_F15;
            default ->
                {
                    if (word.length() == 1) {
    ...
    boolean updateAcceleratorText(boolean show) {
        ...
        if (maskKeysym.keysym != 0) {
            long accelGroup = getAccelGroup();
            if (show) {
                GTK3.gtk_widget_add_accelerator(getApi().handle, OS.activate, accelGroup, maskKeysym.keysym, maskKeysym.mask, GTK.GTK_ACCEL_VISIBLE);
            } else {
                GTK3.gtk_widget_remove_accelerator(getApi().handle, accelGroup, maskKeysym.keysym, maskKeysym.mask);
            }
        }
