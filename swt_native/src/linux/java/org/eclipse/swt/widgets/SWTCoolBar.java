...
public class SWTCoolBar extends SWTComposite implements ICoolBar {
    ...
    SWTCoolItem[][] items = new SWTCoolItem[0][0];

    SWTCoolItem[] originalItems = new SWTCoolItem[0];

    Cursor hoverCursor, dragCursor, cursor;

    SWTCoolItem dragging = null;
     ...
     * @see Widget#getStyle
     */
    public SWTCoolBar(SWTComposite parent, int style) {
        ...
        int[] events = new int[] { SWT.Dispose, SWT.MouseDown, SWT.MouseExit, SWT.MouseMove, SWT.MouseUp, SWT.MouseDoubleClick, SWT.Paint, SWT.Resize };
    ...
    public Point computeSize(int wHint, int hHint, boolean changed) {
        ...
        for (int row = 0; row < items.length; row++) {
            ...
            for (int i = 0; i < items[row].length; i++) {
                SWTCoolItem item = (SWTCoolItem) (items[row][i]);
    ...
    SWTCoolItem getGrabbedItem(int x, int y) {
        for (int row = 0; row < items.length; row++) {
            for (int i = 0; i < items[row].length; i++) {
                SWTCoolItem item = (SWTCoolItem) (items[row][i]);
     ...
     * </ul>
     */
    public ICoolItem getItem(int index) {
        checkWidget();
        if (index < 0)
     ...
     * </ul>
     */
    public ICoolItem[] getItems() {
        checkWidget();
        SWTCoolItem[] result = new SWTCoolItem[getItemCount()];
        int offset = 0;
        for (int row = 0; row < items.length; row++) {
    ...
    Point findItem(SWTCoolItem item) {
        for (int row = 0; row < items.length; row++) {
            for (int i = 0; i < items[row].length; i++) {
     ...
     * </ul>
     */
    public int indexOf(ICoolItem item_) {
        SWTCoolItem item = (SWTCoolItem) item_;
        checkWidget();
        if (item == null)
    ...
    boolean insertItemIntoRow(SWTCoolItem item, int rowIndex, int x_root) {
        ...
        SWTCoolItem[] newRow = new SWTCoolItem[oldLength + 1];
        ...
        if (index > 0) {
            SWTCoolItem left = (SWTCoolItem) (items[rowIndex][index - 1]);
        ...
        if (index < items[rowIndex].length - 1) {
            SWTCoolItem right = (SWTCoolItem) (items[rowIndex][index + 1]);
    ...
    void createItem(SWTCoolItem item, int index) {
        int itemCount = getItemCount(), row = 0;
        if (!(0 <= index && index <= itemCount))
            error(SWT.ERROR_INVALID_RANGE);
        if (items.length == 0) {
            items = new SWTCoolItem[1][1];
            items[0][0] = item;
        } else {
            int i = index;
            ...
            SWTCoolItem lastItem = (SWTCoolItem) (items[row][lastIndex]);
            if (lastItem.ideal) {
        ...
        item.requestedWidth = SWTCoolItem.MINIMUM_WIDTH;
        int length = originalItems.length;
        SWTCoolItem[] newOriginals = new SWTCoolItem[length + 1];
    ...
    void destroyItem(SWTCoolItem item) {
        if (inDispose)
            return;
        ...
        SWTCoolItem[] newOriginals = new SWTCoolItem[length];
        System.arraycopy(originalItems, 0, newOriginals, 0, index);
    ...
    void moveDown(SWTCoolItem item, int x_root) {
    ...
    void moveLeft(SWTCoolItem item, int pixels) {
        ...
        SWTCoolItem left = (SWTCoolItem) (items[row][index - 1]);
        Rectangle leftBounds = left.internalGetBounds();
    ...
    void moveRight(SWTCoolItem item, int pixels) {
        Point point = findItem(item);
        int row = point.y;
        int index = point.x;
        if (index == 0)
        ...
        if (index + 1 == items[row].length) {
            ...
            SWTCoolItem right = (SWTCoolItem) (items[row][index + 1]);
    ...
    void moveUp(SWTCoolItem item, int x_root) {
        Point point = findItem(item);
    ...
    void onMouseMove(Event event) {
        if (isLocked)
            return;
        fixEvent(event);
        SWTCoolItem grabbed = (SWTCoolItem) (getGrabbedItem(event.x, event.y));
        if (dragging != null) {
    ...
    void onMouseDoubleClick(Event event) {
        ...
        dragging = null;
        fixEvent(event);
        SWTCoolItem target = (SWTCoolItem) (getGrabbedItem(event.x, event.y));
        if (target == null) {
            _setCursor(null);
        } else {
            ...
            if (items[row].length > 1) {
                ...
                if (bounds.width == maxSize) {
                    ...
                    for (int i = 0; i < items[row].length; i++) {
                        if (i != index) {
                            SWTCoolItem item = (SWTCoolItem) (items[row][i]);
                            item.requestedWidth = Math.max(item.internalGetMinimumWidth(), SWTCoolItem.MINIMUM_WIDTH);
                        }
                    }
    ...
    void removeControl(SWTControl control) {
        ...
        SWTCoolItem[] items = (SWTCoolItem[]) (getItems());
        for (int i = 0; i < items.length; i++) {
            SWTCoolItem item = (SWTCoolItem) (items[i]);
     ...
     * appropriately.
     */
    boolean removeItemFromRow(SWTCoolItem item, int rowIndex, boolean disposed) {
        ...
        item.wrap = false;
        if (newLength > 0) {
            SWTCoolItem[] newRow = new SWTCoolItem[newLength];
            System.arraycopy(items[rowIndex], 0, newRow, 0, index);
        ...
        if (!disposed) {
            if (index == 0) {
                SWTCoolItem first = (SWTCoolItem) (items[rowIndex][0]);
                ...
                SWTCoolItem previous = (SWTCoolItem) (items[rowIndex][index - 1]);
    ...
    int layoutItems() {
        ...
        for (int row = 0; row < items.length; row++) {
            ...
            /* lay the items out */
            for (int i = 0; i < count; i++) {
                SWTCoolItem child = (SWTCoolItem) (items[row][i]);
    ...
    public int[] getItemOrder() {
        ...
        for (int i = 0; i < items.length; i++) {
            for (int j = 0; j < items[i].length; j++) {
                SWTCoolItem item = (SWTCoolItem) (items[i][j]);
                int index = 0;
                while (index < originalItems.length) {
                    if (originalItems[index] == item)
                        break;
    ...
    public void setBackground(Color color) {
        for (SWTCoolItem ci : originalItems) {
            SWTControl ctrl = (SWTControl) (ci.getControl());
            if (ctrl != null) {
    ...
    void setItemOrder(int[] itemOrder) {
        ...
        SWTCoolItem[] row = new SWTCoolItem[count];
        for (int i = 0; i < count; i++) {
            row[i] = originalItems[itemOrder[i]];
        }
        items = new SWTCoolItem[1][count];
        items[0] = row;
    }
    ...
    public void setOrientation(int orientation) {
        ...
        for (int row = 0; row < items.length; row++) {
            for (int column = 0; column < items[row].length; column++) {
                SWTCoolItem item = (SWTCoolItem) (items[row][column]);
    ...
    void wrapItems(int maxWidth) {
        ...
        if (newItems.length != rowCount) {
            SWTCoolItem[][] tmp = new SWTCoolItem[rowCount][];
