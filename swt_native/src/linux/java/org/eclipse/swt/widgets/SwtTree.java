...
public class SwtTree extends SwtComposite implements ITree {
    ...
    TreeItem _getItem(long iter) {
        ...
        items[id] = new TreeItem(this.getApi(), parentIter, SWT.NONE, indices[indices.length - 1], iter);
	...
	*/
    ...
    long cellDataProc(long tree_column, long cell, long tree_model, long iter, long data) {
        ...
        if (columnCount == 0) {
            modelIndex = SwtTree.FIRST_COLUMN;
            customDraw = firstCustomDraw;
        } else {
            TreeColumn column = (TreeColumn) ((SwtDisplay) display.getImpl()).getWidget(tree_column);
            if (column != null) {
                modelIndex = ((SwtTreeColumn) column.getImpl()).modelIndex;
                customDraw = ((SwtTreeColumn) column.getImpl()).customDraw;
            }
        }
        if (modelIndex == -1)
            return 0;
        ...
        if ((getApi().style & SWT.VIRTUAL) != 0) {
            ...
            if (((SwtTreeItem) item.getImpl()).updated) {
                updated = true;
                ((SwtTreeItem) item.getImpl()).updated = false;
            }
        }
        long[] ptr = new long[1];
        if (setData) {
    ...
    boolean checkData(TreeItem item) {
        ...
        if ((getApi().style & SWT.VIRTUAL) != 0) {
            ...
            OS.g_signal_handlers_block_matched(modelHandle, mask, signal_id, 0, 0, 0, getApi().handle);
            currentItem = item;
            ((SwtTreeItem) item.getImpl()).settingData = true;
            sendEvent(SWT.SetData, event);
            ((SwtTreeItem) item.getImpl()).settingData = false;
            currentItem = null;
            //widget could be disposed at this point
            if (isDisposed())
                return false;
            OS.g_signal_handlers_unblock_matched(modelHandle, mask, signal_id, 0, 0, 0, getApi().handle);
            if (item.isDisposed())
                return false;
        }
        return true;
	...
	*/
	 ...
	 * it is visible.
	 */
        long expander_column = GTK.gtk_tree_view_get_expander_column(getApi().handle);
        if (expander_column == 0 && !GTK.gtk_tree_view_column_get_visible(column)) {
            GTK.gtk_tree_view_column_set_visible(column, true);
            expander_column = GTK.gtk_tree_view_get_expander_column(getApi().handle);
        ...
        if (expander_column == column) {
            ...
            GTK.gtk_tree_view_get_cell_area(getApi().handle, path, column, rect);
            width += rect.x;
            /* expander */
            if (!GTK.gtk_tree_view_column_get_visible(column)) {
                if (GTK.GTK4) {
                    ...
                    width += requisition.width + SwtTreeItem.EXPANDER_EXTRA_PADDING;
                } else {
                    GTK3.gtk_widget_style_get(getApi().handle, OS.expander_size, w, 0);
                    width += w[0] + SwtTreeItem.EXPANDER_EXTRA_PADDING;
                }
            }
        }
        /*
	 ...
	 */
        if (!GTK.GTK4) {
            GTK3.gtk_widget_style_get(getApi().handle, OS.focus_line_width, w, 0);
            width += 2 * w[0];
        }
        ...
        while (temp != 0) {
            ...
            if (renderer != 0) {
                gtk_cell_renderer_get_preferred_size(renderer, getApi().handle, w, null);
                width += w[0];
            }
        ...
        if (recurse) {
            if (path == 0)
                path = GTK.gtk_tree_model_get_path(modelHandle, iter);
            boolean expanded = GTK.gtk_tree_view_row_expanded(getApi().handle, path);
            if (expanded) {
        ...
        if (path != 0)
            GTK.gtk_tree_path_free(path);
        if (GTK.gtk_tree_view_get_grid_lines(getApi().handle) > GTK.GTK_TREE_VIEW_GRID_LINES_NONE) {
            /*
		 * Grid line width is handled via CSS in GTK4.
		 */
            if (!GTK.GTK4) {
                GTK3.gtk_widget_style_get(getApi().handle, OS.grid_line_width, w, 0);
                width += 2 * w[0];
            }
        }
        return width;
        ...
        if (!GTK.GTK4) {
            if (firstCompute) {
                for (TreeColumn column : columns) {
                    if (column != null)
                        GTK.gtk_widget_set_visible(((SwtTreeColumn) column.getImpl()).buttonHandle, true);
                }
                firstCompute = false;
            }
        }
        GTK.gtk_widget_realize(getApi().handle);
        Point size = computeNativeSize(getApi().handle, wHint, hHint, changed);
        /*
	 ...
	 */
        if (hHint == SWT.DEFAULT && size.y == getHeaderHeight()) {
            ...
            for (TreeItem item : items) {
                if (item != null && ((SwtTreeItem) item.getImpl()).isExpanded) {
	 ...
	 */
        if (size.y == this.headerHeight && this.headerVisible && (getApi().style & SWT.NO_SCROLL) != 0) {
...
*/
        ...
        if (columnCount != 0) {
            ...
            for (int i = 0; i < columnCount; i++) {
                int columnIndex = ((SwtTreeColumn) columns[i].getImpl()).modelIndex;
            ...
            if (modelIndex == modelLength) {
                ...
                GTK.gtk_tree_view_set_model(getApi().handle, newModel);
                setModel(newModel);
            }
        }
        ...
        if (index == 0 && columnCount > 0) {
            ...
            createRenderers(checkColumn.handle, ((SwtTreeColumn) checkColumn.getImpl()).modelIndex, false, checkColumn.style);
        }
        ...
        GTK.gtk_tree_view_insert_column(getApi().handle, columnHandle, index);
        /*
	...
	*/
        ...
        if (column != null) {
            ...
            ((SwtTreeColumn) column.getImpl()).modelIndex = modelIndex;
        }
        if (!searchEnabled()) {
            GTK.gtk_tree_view_set_search_column(getApi().handle, -1);
        } else {
            /* Set the search column whenever the model changes */
            int firstColumn = columnCount == 0 ? FIRST_COLUMN : ((SwtTreeColumn) columns[0].getImpl()).modelIndex;
            GTK.gtk_tree_view_set_search_column(getApi().handle, firstColumn + CELL_TEXT);
        }
    }
    ...
    void createHandle(int index) {
        ...
        if (modelHandle == 0)
            error(SWT.ERROR_NO_HANDLES);
        getApi().handle = GTK.gtk_tree_view_new_with_model(modelHandle);
        if (getApi().handle == 0)
            error(SWT.ERROR_NO_HANDLES);
        ...
        if (GTK.GTK4) {
            ...
            GTK4.gtk_scrolled_window_set_child(scrolledHandle, getApi().handle);
        } else {
            GTK3.gtk_container_add(fixedHandle, scrolledHandle);
            GTK3.gtk_container_add(scrolledHandle, getApi().handle);
        }
        int mode = (getApi().style & SWT.MULTI) != 0 ? GTK.GTK_SELECTION_MULTIPLE : GTK.GTK_SELECTION_BROWSE;
        long selectionHandle = GTK.gtk_tree_view_get_selection(getApi().handle);
        GTK.gtk_tree_selection_set_mode(selectionHandle, mode);
        GTK.gtk_tree_view_set_headers_visible(getApi().handle, false);
        int hsp = (getApi().style & SWT.H_SCROLL) != 0 ? GTK.GTK_POLICY_AUTOMATIC : GTK.GTK_POLICY_NEVER;
        int vsp = (getApi().style & SWT.V_SCROLL) != 0 ? GTK.GTK_POLICY_AUTOMATIC : GTK.GTK_POLICY_NEVER;
	 ...
	 */
        if ((getApi().style & SWT.VIRTUAL) != 0) {
            OS.g_object_set(getApi().handle, OS.fixed_height_mode, true, 0);
        }
        if (!searchEnabled()) {
            GTK.gtk_tree_view_set_search_column(getApi().handle, -1);
        }
    ...
    void bindArrowKeyBindings() {
        ...
        GTK4.gtk_widget_class_add_binding_signal(GTK.GTK_WIDGET_GET_CLASS(getApi().handle), keyval[0], 0, Converter.javaStringToCString("expand-collapse-cursor-row"), Converter.javaStringToCString("(bbb)"), false, false, false);
        GTK.gtk_accelerator_parse(Converter.javaStringToCString("Right"), keyval, null);
        GTK4.gtk_widget_class_add_binding_signal(GTK.GTK_WIDGET_GET_CLASS(getApi().handle), keyval[0], 0, Converter.javaStringToCString("expand-collapse-cursor-row"), Converter.javaStringToCString("(bbb)"), false, true, false);
    }
    ...
    void createItem(TreeColumn column, int index) {
        ...
        if (columnCount == 0) {
            column.handle = GTK.gtk_tree_view_get_column(getApi().handle, 0);
            ...
            ((SwtTreeColumn) column.getImpl()).modelIndex = FIRST_COLUMN;
            createRenderers(column.handle, ((SwtTreeColumn) column.getImpl()).modelIndex, true, column.style);
            ((SwtTreeColumn) column.getImpl()).customDraw = firstCustomDraw;
        ...
        ((SwtTreeColumn) column.getImpl()).buttonHandle = GTK.gtk_tree_view_column_get_button(column.handle);
        GTK.gtk_widget_set_focus_on_click(((SwtTreeColumn) column.getImpl()).buttonHandle, false);
    ...
    void createRenderers(long columnHandle, int modelIndex, boolean check, int columnStyle) {
        ...
        if ((getApi().style & SWT.CHECK) != 0 && check) {
            ...
            if (isOwnerDrawn) {
                GTK.gtk_tree_view_column_set_cell_data_func(columnHandle, checkRenderer, ((SwtDisplay) display.getImpl()).cellDataProc, getApi().handle, 0);
                OS.g_object_set_qdata(checkRenderer, SwtDisplay.SWT_OBJECT_INDEX1, columnHandle);
            }
        }
        long pixbufRenderer;
        if (GTK.GTK4) {
            ...
            pixbufRenderer = isOwnerDrawn ? OS.g_object_new(((SwtDisplay) display.getImpl()).gtk_cell_renderer_pixbuf_get_type(), 0) : GTK.gtk_cell_renderer_pixbuf_new();
        }
        if (pixbufRenderer == 0) {
            error(SWT.ERROR_NO_HANDLES);
        } else {
			 ...
			 */
                if ((getApi().style & SWT.VIRTUAL) != 0 && pixbufSizeSet) {
        ...
        long textRenderer = isOwnerDrawn ? OS.g_object_new(((SwtDisplay) display.getImpl()).gtk_cell_renderer_text_get_type(), 0) : GTK.gtk_cell_renderer_text_new();
        if (textRenderer == 0)
            error(SWT.ERROR_NO_HANDLES);
        if (isOwnerDrawn) {
            OS.g_object_set_qdata(pixbufRenderer, SwtDisplay.SWT_OBJECT_INDEX1, columnHandle);
            OS.g_object_set_qdata(textRenderer, SwtDisplay.SWT_OBJECT_INDEX1, columnHandle);
        }
        /*
	...
	*/
        ...
        if (columnCount != 0) {
            for (int i = 0; i < columnCount; i++) {
                if (columns[i].handle == columnHandle) {
                    customDraw = ((SwtTreeColumn) columns[i].getImpl()).customDraw;
                    break;
                }
            }
        }
        if ((getApi().style & SWT.VIRTUAL) != 0 || customDraw || isOwnerDrawn) {
            GTK.gtk_tree_view_column_set_cell_data_func(columnHandle, textRenderer, ((SwtDisplay) display.getImpl()).cellDataProc, getApi().handle, 0);
            GTK.gtk_tree_view_column_set_cell_data_func(columnHandle, pixbufRenderer, ((SwtDisplay) display.getImpl()).cellDataProc, getApi().handle, 0);
        }
    }
    ...
    void deregister() {
        super.deregister();
        ((SwtDisplay) display.getImpl()).removeWidget(GTK.gtk_tree_view_get_selection(getApi().handle));
        if (checkRenderer != 0)
    ...
    void destroyItem(TreeColumn column) {
        ...
        if (columnCount == 1) {
            firstCustomDraw = ((SwtTreeColumn) column.getImpl()).customDraw;
        }
        System.arraycopy(columns, index + 1, columns, index, --columnCount - index);
        columns[columnCount] = null;
        GTK.gtk_tree_view_remove_column(getApi().handle, columnHandle);
        if (columnCount == 0) {
            long oldModel = modelHandle;
            long[] types = getColumnTypes(1);
            long newModel = GTK.gtk_tree_store_newv(types.length, types);
            if (newModel == 0)
                error(SWT.ERROR_NO_HANDLES);
            copyModel(oldModel, ((SwtTreeColumn) column.getImpl()).modelIndex, newModel, FIRST_COLUMN, (long) 0, (long) 0, FIRST_COLUMN + CELL_TYPES);
            GTK.gtk_tree_view_set_model(getApi().handle, newModel);
            setModel(newModel);
            createColumn(null, 0);
        } else {
            for (int i = 0; i < items.length; i++) {
                ...
                if (item != null) {
                    ...
                    int modelIndex = ((SwtTreeColumn) column.getImpl()).modelIndex;
            ...
            if (index == 0) {
                ...
                createRenderers(firstColumn.handle, ((SwtTreeColumn) firstColumn.getImpl()).modelIndex, true, firstColumn.style);
            }
        }
        if (!searchEnabled()) {
            GTK.gtk_tree_view_set_search_column(getApi().handle, -1);
    ...
    boolean dragDetect(int x, int y, boolean filter, boolean dragOnTimeout, boolean[] consume) {
        ...
        if (OS.isX11()) {
            if (filter) {
                ...
                if (GTK.gtk_tree_view_get_path_at_pos(getApi().handle, x, y, path, null, null, null)) {
                    if (path[0] != 0) {
                        long selection = GTK.gtk_tree_view_get_selection(getApi().handle);
    ...
    Rectangle getClientAreaInPixels() {
        ...
        if (GTK.GTK4) {
            ...
            GTK.gtk_tree_view_convert_bin_window_to_widget_coords(getApi().handle, 0, 0, headerWidth, headerHeight);
            rect = new Rectangle(headerWidth[0], headerHeight[0], width, height);
        } else {
            GTK.gtk_widget_realize(getApi().handle);
            long fixedWindow = gtk_widget_get_window(fixedHandle);
            long binWindow = GTK3.gtk_tree_view_get_bin_window(getApi().handle);
    ...
    int getClientWidth() {
        ...
        if (GTK.GTK4) {
            long surface = gtk_widget_get_surface(getApi().handle);
            gdk_surface_get_size(surface, w, h);
        } else {
            GTK.gtk_widget_realize(getApi().handle);
            gdk_window_get_size(GTK3.gtk_tree_view_get_bin_window(getApi().handle), w, h);
        }
        return w[0];
    }
    ...
    public int[] getColumnOrder() {
        ...
        if (columnCount == 0)
            return new int[0];
        long list = GTK.gtk_tree_view_get_columns(getApi().handle);
        if (list == 0)
            return new int[0];
    ...
    GdkRGBA getContextColorGdkRGBA() {
        if (foreground != null) {
            ...
            return ((SwtDisplay) display.getImpl()).COLOR_LIST_FOREGROUND_RGBA;
        }
    }

    TreeItem getFocusItem() {
        long[] path = new long[1];
        GTK.gtk_tree_view_get_cursor(getApi().handle, path, null);
        if (path[0] == 0)
            return null;
        TreeItem item = null;
        long iter = OS.g_malloc(GTK.GtkTreeIter_sizeof());
    ...
    public int getHeaderHeight() {
        ...
        if (columnCount > 0) {
            GtkRequisition requisition = new GtkRequisition();
            for (int i = 0; i < columnCount; i++) {
                long buttonHandle = ((SwtTreeColumn) columns[i].getImpl()).buttonHandle;
                if (buttonHandle != 0) {
            ...
            if (GTK.GTK4) {
                ...
                GTK.gtk_tree_view_convert_bin_window_to_widget_coords(getApi().handle, 0, 0, null, headerHeight);
    ...
    public boolean getHeaderVisible() {
        checkWidget();
        return GTK.gtk_tree_view_get_headers_visible(getApi().handle);
    }
	 ...
	 */
        ...
        if (!GTK.gtk_tree_view_get_path_at_pos(getApi().handle, x, y, path, columnHandle, null, null))
            return null;
        if (path[0] == 0)
            return null;
        TreeItem item = null;
        ...
        if (GTK.gtk_tree_model_get_iter(modelHandle, iter, path[0])) {
            ...
            if (GTK.gtk_tree_view_get_expander_column(getApi().handle) == columnHandle[0]) {
                GdkRectangle rect = new GdkRectangle();
                GTK.gtk_tree_view_get_cell_area(getApi().handle, path[0], columnHandle[0], rect);
                if ((getApi().style & SWT.MIRRORED) != 0) {
    ...
    public int getItemHeight() {
        ...
        if (itemCount == 0) {
            long column = GTK.gtk_tree_view_get_column(getApi().handle, 0);
            int[] h = new int[1];
            ignoreSize = true;
            if (GTK.GTK4) {
            ...
            if (textRenderer != 0)
                GTK.gtk_cell_renderer_get_preferred_height_for_width(textRenderer, getApi().handle, 0, h, null);
            height += h[0];
            ignoreSize = false;
        } else {
            long iter = OS.g_malloc(GTK.GtkTreeIter_sizeof());
            ...
            for (int i = 0; i < columnCount; i++) {
                long column = GTK.gtk_tree_view_get_column(getApi().handle, i);
                GTK.gtk_tree_view_column_cell_set_cell_data(column, modelHandle, iter, false, false);
                int[] h = new int[1];
                if (GTK.GTK4) {
                    GTK4.gtk_tree_view_column_cell_get_size(column, null, null, null, h);
    ...
    public boolean getLinesVisible() {
        checkWidget();
        return GTK.gtk_tree_view_get_grid_lines(getApi().handle) > GTK.GTK_TREE_VIEW_GRID_LINES_NONE;
    }
	 ...
	 */
        long vAdjustment;
        vAdjustment = GTK.gtk_scrollable_get_vadjustment(getApi().handle);
	 ...
	 */
        ...
        long[] path = new long[1];
        GTK.gtk_widget_realize(getApi().handle);
        if (!GTK.gtk_tree_view_get_path_at_pos(getApi().handle, 1, 1, path, null, null, null))
            return null;
        if (path[0] == 0)
            return null;
        item = null;
	 ...
	 *  by setSelection().
	 */
        long treeSelect = GTK.gtk_tree_view_get_selection(getApi().handle);
	 ...
	 */
        if ((getApi().state & DRAG_DETECT) != 0 && hooks(SWT.DragDetect) && OS.isWayland() && eventType == GDK.GDK_BUTTON_PRESS) {
            ...
            if (nextEvent == 0) {
                long[] path = new long[1];
                long selection = GTK.gtk_tree_view_get_selection(getApi().handle);
                if (GTK.gtk_tree_view_get_path_at_pos(getApi().handle, (int) eventX[0], (int) eventY[0], path, null, null, null) && path[0] != 0) {
                    ...
                    if (GTK.gtk_tree_selection_path_is_selected(selection, path[0])) {
                        if (((eventState[0] & (GDK.GDK_CONTROL_MASK | GDK.GDK_SHIFT_MASK)) == 0) || ((eventState[0] & GDK.GDK_CONTROL_MASK) != 0)) {
                            /**
	...
	*/
        ...
        if (button == 3 && eventType == GDK.GDK_BUTTON_PRESS) {
            long[] path = new long[1];
            if (GTK.gtk_tree_view_get_path_at_pos(getApi().handle, (int) eventX[0], (int) eventY[0], path, null, null, null)) {
                if (path[0] != 0) {
                    long selection = GTK.gtk_tree_view_get_selection(getApi().handle);
                    if (GTK.gtk_tree_selection_path_is_selected(selection, path[0]))
                        result = 1;
	...
	*/
        if ((getApi().style & SWT.SINGLE) != 0 && getSelectionCount() == 0) {
            long[] path = new long[1];
            if (GTK.gtk_tree_view_get_path_at_pos(getApi().handle, (int) eventX[0], (int) eventY[0], path, null, null, null)) {
                if (path[0] != 0) {
                    long selection = GTK.gtk_tree_view_get_selection(getApi().handle);
                    OS.g_signal_handlers_block_matched(selection, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, CHANGED);
                    GTK.gtk_tree_view_set_cursor(getApi().handle, path[0], 0, false);
                    OS.g_signal_handlers_unblock_matched(selection, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, CHANGED);
                    GTK.gtk_tree_path_free(path[0]);
                }
            }
	 ...
	 */
        if ((getApi().state & DRAG_DETECT) != 0 && hooks(SWT.DragDetect) && OS.isWayland()) {
            long[] path = new long[1];
            long selection = GTK.gtk_tree_view_get_selection(getApi().handle);
            // free up the selection function on release.
            GTK.gtk_tree_selection_set_select_function(selection, 0, 0, 0);
            if (GTK.gtk_tree_view_get_path_at_pos(getApi().handle, (int) eventX[0], (int) eventY[0], path, null, null, null) && path[0] != 0 && GTK.gtk_tree_selection_path_is_selected(selection, path[0])) {
    ...
    void drawInheritedBackground(long cairo) {
        if ((getApi().state & PARENT_BACKGROUND) != 0 || backgroundImage != null) {
            Control control = findBackgroundControl();
            if (control != null) {
                int[] width = new int[1], height = new int[1];
                long gdkResource;
                if (GTK.GTK4) {
                    gdkResource = gtk_widget_get_surface(getApi().handle);
                    gdk_surface_get_size(gdkResource, width, height);
                } else {
                    gdkResource = GTK3.gtk_tree_view_get_bin_window(getApi().handle);
                ...
                while (itemCount != 0 && expanded && height[0] > (rect.y + rect.height)) {
                    ...
                    long path = GTK.gtk_tree_model_get_path(modelHandle, iter);
                    GTK.gtk_tree_view_get_cell_area(getApi().handle, path, 0, rect);
                    expanded = GTK.gtk_tree_view_row_expanded(getApi().handle, path);
                    GTK.gtk_tree_path_free(path);
                    if (parent != 0)
                        OS.g_free(parent);
                    parent = iter;
	...
	*/
        ...
        if (childCount != 0 && ((SwtTreeItem) item.getImpl()).isExpanded) {
            OS.g_signal_handlers_block_matched(getApi().handle, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, TEST_EXPAND_ROW);
            GTK.gtk_tree_view_expand_row(getApi().handle, path, false);
            OS.g_signal_handlers_unblock_matched(getApi().handle, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, TEST_EXPAND_ROW);
        }
        return 0;
    }
	...
	* collapsed.
	*/
        boolean changed = modelChanged || !GTK.gtk_tree_view_row_expanded(getApi().handle, path);
        modelChanged = oldModelChanged;
        if (isDisposed() || item.isDisposed())
            return 1;
        ((SwtTreeItem) item.getImpl()).isExpanded = false;
        /*
	...
	*/
        if (changed) {
            OS.g_signal_handlers_block_matched(getApi().handle, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, TEST_COLLAPSE_ROW);
            GTK.gtk_tree_view_collapse_row(getApi().handle, path);
            OS.g_signal_handlers_unblock_matched(getApi().handle, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, TEST_COLLAPSE_ROW);
            return 1;
        }
        return 0;
    }
	...
	*/
        ...
        if (isDisposed() || item.isDisposed())
            return 1;
        ((SwtTreeItem) item.getImpl()).isExpanded = true;
        /*
	* Bug in GTK.  Expanding or collapsing a row which has no more
	* children causes the model state to become invalid, causing
	* GTK to give warnings and behave strangely.  Other changes to
	...
	*/
	 ...
	 */
        ...
        long columns = GTK.gtk_tree_view_get_columns(getApi().handle), list = columns;
    ...
    void hideFirstColumn() {
        long firstColumn = GTK.gtk_tree_view_get_column(getApi().handle, 0);
    ...
    void hookEvents() {
        super.hookEvents();
        long selection = GTK.gtk_tree_view_get_selection(getApi().handle);
        OS.g_signal_connect_closure(selection, OS.changed, ((SwtDisplay) display.getImpl()).getClosure(CHANGED), false);
        OS.g_signal_connect_closure(getApi().handle, OS.row_activated, ((SwtDisplay) display.getImpl()).getClosure(ROW_ACTIVATED), false);
        OS.g_signal_connect_closure(getApi().handle, OS.test_expand_row, ((SwtDisplay) display.getImpl()).getClosure(TEST_EXPAND_ROW), false);
        OS.g_signal_connect_closure(getApi().handle, OS.test_collapse_row, ((SwtDisplay) display.getImpl()).getClosure(TEST_COLLAPSE_ROW), false);
        OS.g_signal_connect_closure(getApi().handle, OS.expand_collapse_cursor_row, ((SwtDisplay) display.getImpl()).getClosure(EXPAND_COLLAPSE_CURSOR_ROW), false);
        OS.g_signal_connect_closure(modelHandle, OS.row_has_child_toggled, ((SwtDisplay) display.getImpl()).getClosure(ROW_HAS_CHILD_TOGGLED), false);
        if (checkRenderer != 0) {
            OS.g_signal_connect_closure(checkRenderer, OS.toggled, ((SwtDisplay) display.getImpl()).getClosure(TOGGLED), false);
        }
        OS.g_signal_connect_closure(getApi().handle, OS.start_interactive_search, ((SwtDisplay) display.getImpl()).getClosure(START_INTERACTIVE_SEARCH), false);
    }
    ...
    boolean mnemonicHit(char key) {
        for (int i = 0; i < columnCount; i++) {
            long labelHandle = ((SwtTreeColumn) columns[i].getImpl()).labelHandle;
    ...
    long paintWindow() {
        ...
        return GTK3.gtk_tree_view_get_bin_window(getApi().handle);
    }
	 ...
	 */
        ...
        if (headerVisible && noChildDrawing && wasScrolled) {
            for (TreeColumn column : columns) {
                if (column != null) {
                    GTK.gtk_widget_queue_draw(((SwtTreeColumn) column.getImpl()).buttonHandle);
                }
            }
            wasScrolled = false;
        }
    ...
    void recreateRenderers() {
        ...
        if (columnCount == 0) {
            createRenderers(GTK.gtk_tree_view_get_column(getApi().handle, 0), SwtTree.FIRST_COLUMN, true, 0);
        } else {
            for (int i = 0; i < columnCount; i++) {
                TreeColumn column = columns[i];
                createRenderers(column.handle, ((SwtTreeColumn) column.getImpl()).modelIndex, i == 0, column.style);
            }
        }
    }
    ...
    void register() {
        super.register();
        ((SwtDisplay) display.getImpl()).addWidget(GTK.gtk_tree_view_get_selection(getApi().handle), this.getApi());
        if (checkRenderer != 0)
            ((SwtDisplay) display.getImpl()).addWidget(checkRenderer, this.getApi());
    ...
    void remove(long parentIter, int start, int end) {
        ...
        try {
            for (int i = start; i <= end; i++) {
				 ...
				 */
                    if (((SwtTreeItem) item.getImpl()).settingData) {
    ...
    void sendMeasureEvent(long cell, long width, long height) {
        if (!ignoreSize && GTK.GTK_IS_CELL_RENDERER_TEXT(cell) && hooks(SWT.MeasureItem)) {
            long iter = OS.g_object_get_qdata(cell, SwtDisplay.SWT_OBJECT_INDEX2);
            TreeItem item = null;
            if (iter != 0)
                item = _getItem(iter);
            if (item != null && !item.isDisposed()) {
                int columnIndex = 0;
                if (columnCount > 0) {
                    long columnHandle = OS.g_object_get_qdata(cell, SwtDisplay.SWT_OBJECT_INDEX1);
                    for (int i = 0; i < columnCount; i++) {
                        if (columns[i].handle == columnHandle) {
                            columnIndex = i;
                            break;
                ...
                GTK.gtk_cell_renderer_get_preferred_height_for_width(cell, getApi().handle, contentWidth[0], contentHeight, null);
    ...
    void rendererRender(long cell, long cr, long snapshot, long widget, long background_area, long cell_area, long expose_area, long flags) {
		 ...
		 */
            ...
            long path = GTK.gtk_tree_model_get_path(modelHandle, iter);
            GTK.gtk_tree_view_get_background_area(getApi().handle, path, columnHandle, columnRect);
            GTK.gtk_tree_path_free(path);
            y_offset = columnRect.y - rendererRect.y;
            columnRect.y -= y_offset;
        }
        if (item != null) {
            if (GTK.GTK_IS_CELL_RENDERER_TOGGLE(cell) || (columnIndex != 0 || (getApi().style & SWT.CHECK) == 0)) {
                drawFlags = (int) flags;
                drawState = SWT.FOREGROUND;
                long[] ptr = new long[1];
                GTK.gtk_tree_model_get(modelHandle, item.handle, SwtTree.BACKGROUND_COLUMN, ptr, -1);
                if (ptr[0] == 0) {
                    int modelIndex = columnCount == 0 ? SwtTree.FIRST_COLUMN : ((SwtTreeColumn) columns[columnIndex].getImpl()).modelIndex;
                    GTK.gtk_tree_model_get(modelHandle, item.handle, modelIndex + SwtTree.CELL_BACKGROUND, ptr, -1);
                }
                if (ptr[0] != 0) {
                ...
                if (textRenderer != 0)
                    gtk_cell_renderer_get_preferred_size(textRenderer, getApi().handle, null, null);
        ...
        if (item != null) {
            if (GTK.GTK_IS_CELL_RENDERER_TEXT(cell)) {
                if (hooks(SWT.PaintItem)) {
                    ...
                    gtk_cell_renderer_get_preferred_size(cell, getApi().handle, contentWidth, null);
                    ...
                    if (GTK.gtk_tree_view_get_expander_column(getApi().handle) == columnHandle) {
                        /* indent */
                        GdkRectangle rect3 = new GdkRectangle();
                        GTK.gtk_widget_realize(getApi().handle);
                        long path = GTK.gtk_tree_model_get_path(modelHandle, iter);
                        GTK.gtk_tree_view_get_cell_area(getApi().handle, path, columnHandle, rect3);
                        GTK.gtk_tree_path_free(path);
                        contentX[0] += rect3.x;
                    }
                    GC gc = getGC(cr);
                    if ((drawState & SWT.SELECTED) != 0) {
                        ...
                        gc.setBackground(item.getBackground(columnIndex));
                        Color foreground;
                        foreground = drawForegroundRGBA != null ? SwtColor.gtk_new(display, drawForegroundRGBA) : item.getForeground(columnIndex);
                        gc.setForeground(foreground);
                    }
                    gc.setFont(item.getFont(columnIndex));
                    if ((getApi().style & SWT.MIRRORED) != 0) {
    ...
    private GC getGC(long cr) {
        ...
        gcData.cairo = cr;
        gc = SwtGC.gtk_new(this.getApi(), gcData);
        return gc;
    }
    ...
    void resetCustomDraw() {
        ...
        for (int i = 0; i < end; i++) {
            boolean customDraw = columnCount != 0 ? ((SwtTreeColumn) columns[i].getImpl()).customDraw : firstCustomDraw;
            if (customDraw) {
                long column = GTK.gtk_tree_view_get_column(getApi().handle, i);
                long textRenderer = getTextRenderer(column);
                GTK.gtk_tree_view_column_set_cell_data_func(column, textRenderer, 0, 0, 0);
                if (columnCount != 0)
                    ((SwtTreeColumn) columns[i].getImpl()).customDraw = false;
            }
        }
        firstCustomDraw = false;
    }
    ...
    public void setInsertMark(TreeItem item, boolean before) {
        checkWidget();
        if (item == null) {
            GTK.gtk_tree_view_set_drag_dest_row(getApi().handle, 0, GTK.GTK_TREE_VIEW_DROP_BEFORE);
            return;
        }
        if (item.isDisposed())
            error(SWT.ERROR_INVALID_ARGUMENT);
    ...
    void setItemCount(long parentIter, int count) {
        ...
        if (isVirtual) {
            ...
            for (int i = itemCount; i < count; i++) {
                new TreeItem(this.getApi(), parentIter, SWT.NONE, itemCount, 0);
            }
        }
        if (!isVirtual)
            setRedraw(true);
	 ...
	 */
        ...
        String css = "treeview {background-color: " + ((SwtDisplay) display.getImpl()).gtk_rgba_to_css_string(background) + ";}\n" + "treeview:selected {background-color: " + ((SwtDisplay) display.getImpl()).gtk_rgba_to_css_string(selectedBackground) + ";}";
	...
	*/
    ...
    public void setColumnOrder(int[] order) {
        ...
        for (int i = 0; i < order.length; i++) {
            ...
            GTK.gtk_tree_view_move_column_after(getApi().handle, column, baseColumn);
            baseColumn = column;
        }
    }
    ...
    void setFontDescription(long font) {
        ...
        for (int i = 0; i < columns.length; i++) {
            if (columns[i] != null) {
                ((SwtTreeColumn) columns[i].getImpl()).setFontDescription(font);
            }
        }
    }
    ...
    void setForegroundGdkRGBA(GdkRGBA rgba) {
        foreground = rgba;
        GdkRGBA toSet = rgba == null ? ((SwtDisplay) display.getImpl()).COLOR_LIST_FOREGROUND_RGBA : rgba;
        setForegroundGdkRGBA(getApi().handle, toSet);
    }
    ...
    void updateHeaderCSS() {
		 ...
		 */
            css.append("background: " + ((SwtDisplay) display.getImpl()).gtk_rgba_to_css_string(headerBackground.handle) + "; ");
        }
        if (headerForeground != null) {
            css.append("color: " + ((SwtDisplay) display.getImpl()).gtk_rgba_to_css_string(headerForeground.handle) + "; ");
        }
        css.append("}\n");
        if (columnCount == 0) {
            long buttonHandle = GTK.gtk_tree_view_column_get_button(GTK.gtk_tree_view_get_column(getApi().handle, 0));
            ...
            for (TreeColumn column : columns) {
                if (column != null) {
                    ((SwtTreeColumn) column.getImpl()).setHeaderCSS(css.toString());
                }
            }
        }
    }
    ...
    public void setHeaderVisible(boolean show) {
        checkWidget();
        GTK.gtk_tree_view_set_headers_visible(getApi().handle, show);
    ...
    void setOrientation(boolean create) {
        ...
        if (items != null) {
            for (int i = 0; i < items.length; i++) {
                if (items[i] != null)
                    ((SwtWidget) items[i].getImpl()).setOrientation(create);
            }
        }
        if (columns != null) {
            for (int i = 0; i < columns.length; i++) {
                if (columns[i] != null)
                    ((SwtTreeColumn) columns[i].getImpl()).setOrientation(create);
            }
        }
    }
	 ...
	 */
        if (GTK.GTK4) {
            ...
            GTK3.gtk_widget_set_parent_window(child.getImpl().topHandle(), parentGdkWindow);
            hasChildren = true;
    ...
    public void setSelection(TreeItem[] items) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            showItem(path, false);
            if (first) {
                GTK.gtk_tree_view_set_cursor(getApi().handle, path, 0, false);
            }
            GTK.gtk_tree_selection_select_iter(selection, item.handle);
            GTK.gtk_tree_path_free(path);
            first = false;
	 ...
	 */
        ...
        long path = GTK.gtk_tree_model_get_path(modelHandle, item.handle);
        showItem(path, false);
        GTK.gtk_tree_view_scroll_to_cell(getApi().handle, path, 0, true, 0, 0);
        GTK.gtk_tree_path_free(path);
    }
    ...
    public void showColumn(TreeColumn column) {
        ...
        if (((SwtTreeColumn) column.getImpl()).parent != this.getApi())
            return;
        GTK.gtk_tree_view_scroll_to_cell(getApi().handle, 0, column.handle, false, 0, 0);
    }
	...
	*/
    ...
    void showItem(long path, boolean scroll) {
        ...
        if (depth > 1) {
            ...
            for (int i = 0; i < indices.length; i++) {
                ...
                GTK.gtk_tree_view_expand_row(getApi().handle, tempPath, false);
            }
            GTK.gtk_tree_path_free(tempPath);
        }
        if (scroll) {
            GTK.gtk_tree_view_scroll_to_cell(getApi().handle, path, 0, false, 0.5f, 0.0f);
        }
    }
    ...
    long windowProc(long handle, long arg0, long user_data) {
        switch((int) user_data) {
			 ...
			 */
                    int itemCount = GTK.gtk_tree_model_iter_n_children(modelHandle, 0);
                    if (itemCount == 0 && (getApi().state & OBSCURED) == 0) {
                        if ((getApi().state & PARENT_BACKGROUND) != 0 || backgroundImage != null) {
                            Control control = findBackgroundControl();
                            if (control != null) {
	 ...
	 */
        if (widget == scrolledHandle && getHeaderVisible()) {
            // this actually returns height
            int hScrollBarHeight = hScrollBarWidth();
	 ...
	 */
        for (int i = 0; i < items.length; i++) {
            TreeItem item = items[i];
            if (item != null && ((SwtTreeItem) item.getImpl()).settingData) {
                throwCannotRemoveItem(i);
            }
        }
    }
