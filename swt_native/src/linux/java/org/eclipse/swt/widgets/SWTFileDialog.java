...
public class SWTFileDialog extends SWTDialog implements IFileDialog {
     ...
     * </ul>
     */
    public SWTFileDialog(SWTShell parent) {
     ...
     * @see SWT#MULTI
     */
    public SWTFileDialog(SWTShell parent, int style) {
    ...
    Optional<String> openNativeChooserDialog() {
        ...
        SWTDisplay display = (SWTDisplay) (parent != null ? parent.getDisplay() : SWTDisplay.getCurrent());
        if (GTK.GTK4) {
        ...
        if (GTK.GTK4) {
            if ((style & SWT.MULTI) != 0) {
                file = SyncDialogUtil.run(Display.getInstance(display), asyncCallback -> GTK4.gtk_file_dialog_open_multiple(handle, shellHandle, 0, asyncCallback, 0), asyncResult -> GTK4.gtk_file_dialog_open_multiple_finish(handle, asyncResult, null));
            } else {
                if ((style & SWT.SAVE) != 0) {
                    file = SyncDialogUtil.run(Display.getInstance(display), asyncCallback -> GTK4.gtk_file_dialog_save(handle, shellHandle, 0, asyncCallback, 0), asyncResult -> GTK4.gtk_file_dialog_save_finish(handle, asyncResult, null));
                } else {
                    file = SyncDialogUtil.run(Display.getInstance(display), asyncCallback -> GTK4.gtk_file_dialog_open(handle, shellHandle, 0, asyncCallback, 0), asyncResult -> GTK4.gtk_file_dialog_open_finish(handle, asyncResult, null));
                }
            }
    ...
    void presetChooserDialog() {
        ...
        if ((style & SWT.SAVE) != 0) {
            ...
            if (fileName.length() > 0) {
                ...
                if ((fileName.lastIndexOf(FILE_EXTENSION_SEPARATOR) == -1) && (filterExtensions.length != 0)) {
                    // Filename doesn't contain the extension and user has provided filter extensions
                    ...
                    if (!isGlobPattern(extension)) {
                        //if the extension is of type glob pattern we should not add the extension
