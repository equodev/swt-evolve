...
public class SwtComposite extends SwtScrollable implements IComposite {
    ...
    void createHandle(int index) {
        getApi().state |= HANDLE | CANVAS | CHECK_SUBWINDOW;
        boolean scrolled = (getApi().style & (SWT.H_SCROLL | SWT.V_SCROLL)) != 0;
        if (!scrolled)
            getApi().state |= THEME_BACKGROUND;
        createHandle(index, true, scrolled || (getApi().style & SWT.BORDER) != 0);
    }
    ...
    void createHandle(int index, boolean fixed, boolean scrolled) {
        ...
        if ((getApi().style & SWT.EMBEDDED) != 0) {
            if (OS.isWayland()) {
                ...
                if (GTK.GTK4) {
                    ...
                    if (socketHandle == 0)
                        error(SWT.ERROR_NO_HANDLES);
                    GTK3.gtk_container_add(getApi().handle, socketHandle);
                }
            }
        }
        if (!GTK.GTK4) {
            if ((getApi().style & SWT.NO_REDRAW_RESIZE) != 0 && (getApi().style & SWT.RIGHT_TO_LEFT) == 0) {
                GTK3.gtk_widget_set_redraw_on_allocate(getApi().handle, false);
            }
            /*
		...
		*/
            if ((getApi().style & SWT.DOUBLE_BUFFERED) == 0 && (getApi().style & SWT.NO_BACKGROUND) != 0) {
                GTK3.gtk_widget_set_double_buffered(getApi().handle, false);
            }
        }
    }
            ...
            for (Control child : _getChildren()) {
                if (fixClipMap.containsKey(child)) {
                    ...
                    for (long widget : childHandles) {
                        ((SwtControl) child.getImpl()).adjustChildClipping(widget);
                    }
                }
            }
        }
    ...
    void fixParentGdkResource() {
        ...
        for (Control child : _getChildren()) {
            if (child.getImpl() instanceof SwtControl) {
                ((SwtControl) child.getImpl()).fixParentGdkResource();
            }
        }
    }
            ...
            if (windowList != 0) {
                ...
                while (windows != 0) {
                    ...
                    if (window != redrawWindow) {
                        ...
                        if (userData[0] == 0 || OS.G_OBJECT_TYPE(userData[0]) != ((SwtDisplay) display.getImpl()).gtk_fixed_get_type()) {
	...
	* fix is to avoid calling the default handler.
	*/
        if ((getApi().state & CANVAS) != 0 && socketHandle == 0) {
    ...
    long gtk_focus_in_event(long widget, long event) {
        ...
        return (getApi().state & CANVAS) != 0 ? 1 : result;
    }
    ...
    long gtk_scroll_child(long widget, long scrollType, long horizontal) {
        ...
        if (GTK.GTK4) {
            ...
            if (horizontalBar != null)
                ((SwtScrollBar) horizontalBar.getImpl()).detail = (int) scrollType;
            if (verticalBar != null)
                ((SwtScrollBar) verticalBar.getImpl()).detail = (int) scrollType;
        }
        return 1;
    }
    ...
    void hookEvents() {
        ...
        if ((getApi().state & CANVAS) != 0) {
            if (!GTK.GTK4)
                GTK3.gtk_widget_add_events(getApi().handle, GDK.GDK_POINTER_MOTION_HINT_MASK);
            if (scrolledHandle != 0) {
                OS.g_signal_connect_closure(scrolledHandle, OS.scroll_child, ((SwtDisplay) display.getImpl()).getClosure(SCROLL_CHILD), false);
            }
        }
    }
    ...
    void moveChildren(int oldWidth) {
        for (Control child : _getChildren()) {
            ...
            if (!GTK.GTK4) {
                if (child.getImpl()._enableWindow() != 0) {
                    GDK.gdk_window_move(child.getImpl()._enableWindow(), x, y);
                }
            }
            ((SwtControl) child.getImpl()).moveHandle(x, y);
            /*
		...
		*/
            ...
            if (control != null && control.getImpl()._backgroundImage() != null) {
                if (child.isVisible())
                    ((SwtControl) child.getImpl()).redrawWidget(0, 0, 0, 0, true, true, true);
            }
        }
    }
    ...
    void printWidget(GC gc, long drawable, int depth, int x, int y) {
        ...
        for (int i = children.length - 1; i >= 0; --i) {
            ...
            if (child.getVisible()) {
                Point location = ((SwtControl) child.getImpl()).getLocationInPixels();
                ((SwtControl) child.getImpl()).printWidget(gc, drawable, depth, x + location.x, y + location.y);
            }
        }
        ...
        if (container == fixedHandle) {
            if (GTK.GTK4) {
                ...
                while (temp != 0) {
                    ...
                    if (child != 0) {
                        Widget widget = ((SwtDisplay) display.getImpl()).getWidget(child);
                        if (widget != this.getApi()) {
                            if (noChildDrawing) {
    ...
    void resizeHandle(int width, int height) {
        ...
        if (socketHandle != 0) {
            OS.swt_fixed_resize(getApi().handle, socketHandle, width, height);
        }
    }
