...
public class SwtCaret extends SwtWidget implements ICaret {
    ...
    boolean drawCaret() {
        ...
        if (embeddedInto != null && !embeddedInto.isDisposed() && isFocusCaret()) {
            ...
            drawInCellEditor(((SwtControl) embeddedInto.getImpl()).paintWindow());
        } else {
    ...
    private void drawInCellEditor(long window) {
        ...
        if (parent.getImpl() instanceof SwtCanvas) {
            ((SwtCanvas) parent.getImpl()).drawCaretInFocus(cairo);
        }
    ...
    public void setBounds(int x, int y, int width, int height) {
        ...
        this.width = width;
        this.height = height;
        ((SwtCanvas) parent.getImpl()).updateCaret();
        if (isFocus && isVisible)
            showCaret();
    }
    ...
    public void setVisible(boolean visible) {
        ...
        if (canvas.getImpl() instanceof SwtCanvas) {
            ((SwtCanvas) canvas.getImpl()).blink = true;
        }
        if (canvas.getImpl() instanceof DartCanvas) {
            ((DartCanvas) canvas.getImpl()).drawFlag = visible;
        }
        if (canvas.getImpl() instanceof SwtCanvas) {
            ((SwtCanvas) canvas.getImpl()).drawFlag = visible;
        }
        ((SwtDisplay) display.getImpl()).resetCaretTiming();
    ...
    public Composite _embeddedInto() {
        return embeddedInto;
    }
