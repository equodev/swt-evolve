...
public class SWTSynchronizer implements ISynchronizer {
    ...
    final ConcurrentLinkedQueue<SWTRunnableLock> messages = new ConcurrentLinkedQueue<>();
     ...
     */
    public SWTSynchronizer(SWTDisplay display) {
    ...
    void addLast(SWTRunnableLock lock) {
    ...
    protected void asyncExec(Runnable runnable) {
        ...
        addLast(new SWTRunnableLock(runnable));
    }
    ...
    boolean runAsyncMessages(boolean all) {
        boolean run = false;
        do {
            SWTRunnableLock lock = (SWTRunnableLock) (removeFirst());
            if (lock == null)
                return run;
            run = true;
            synchronized (lock) {
    ...
    protected void syncExec(Runnable runnable) {
        SWTRunnableLock lock = (SWTRunnableLock) (null);
        synchronized (Device.class) {
            ...
            if (!display.isValidThread()) {
                ...
                lock = new SWTRunnableLock(runnable);
                /*
