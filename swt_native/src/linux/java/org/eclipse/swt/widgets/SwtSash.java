...
public class SwtSash extends SwtControl implements ISash {
    ...
    void gtk4_motion_event(long controller, double x, double y, long event) {
        ...
        if (jEvent.doit) {
            ...
            lastY = rect.y;
            if ((parent.style & SWT.MIRRORED) != 0)
                lastX = ((SwtControl) parent.getImpl()).getClientWidth() - width - lastX;
        }
        ((SwtControl) parent.getImpl()).update(true, (getApi().style & SWT.SMOOTH) == 0);
        if ((getApi().style & SWT.SMOOTH) != 0) {
            /*
    ...
    long gtk_key_press_event(long widget, long eventPtr) {
        ...
        switch(key[0]) {
            ...
            case GDK.GDK_Down:
                ...
                long gdkResource = gtk_widget_get_window(getApi().handle);
    ...
    long gtk_motion_notify_event(long widget, long eventPtr) {
        ...
        if (isHint) {
            ...
            ((SwtDisplay) display.getImpl()).getWindowPointerPosition(gdkResource, pointer_x, pointer_y, mask);
            eventX = pointer_x[0];
            eventY = pointer_y[0];
            eventState = mask[0];
        } else {
