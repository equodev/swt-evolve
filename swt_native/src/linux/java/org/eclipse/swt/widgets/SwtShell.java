...
public class SwtShell extends SwtDecorations implements IShell {
    ...
    void adjustTrim(int widthHint, int heightHint) {
        if (((SwtDisplay) display.getImpl()).ignoreTrim)
            return;
	...
	*/
        if (trimWidth > MAXIMUM_TRIM || trimHeight > MAXIMUM_TRIM || isCustomResize()) {
            ((SwtDisplay) display.getImpl()).ignoreTrim = true;
            return;
        }
        boolean hasTitle = false, hasResize = false, hasBorder = false;
        if ((getApi().style & SWT.NO_TRIM) == 0) {
            hasTitle = (getApi().style & (SWT.MIN | SWT.MAX | SWT.TITLE | SWT.MENU)) != 0;
            hasResize = (getApi().style & SWT.RESIZE) != 0;
            hasBorder = (getApi().style & SWT.BORDER) != 0;
        }
        int trimStyle;
        if (hasTitle) {
            if (hasResize) {
                trimStyle = SwtDisplay.TRIM_TITLE_RESIZE;
            } else if (hasBorder) {
                trimStyle = SwtDisplay.TRIM_TITLE_BORDER;
            } else {
                trimStyle = SwtDisplay.TRIM_TITLE;
            }
        } else if (hasResize) {
            trimStyle = SwtDisplay.TRIM_RESIZE;
        } else if (hasBorder) {
            trimStyle = SwtDisplay.TRIM_BORDER;
        } else {
            trimStyle = SwtDisplay.TRIM_NONE;
        }
        Rectangle bounds = getBoundsInPixels();
        int widthAdjustment = ((SwtDisplay) display.getImpl()).trimWidths[trimStyle] - trimWidth;
        int heightAdjustment = ((SwtDisplay) display.getImpl()).trimHeights[trimStyle] - trimHeight;
        ...
        ((SwtDisplay) display.getImpl()).trimWidths[trimStyle] = trimWidth;
        ((SwtDisplay) display.getImpl()).trimHeights[trimStyle] = trimHeight;
    }
    ...
    void bringToTop(boolean force) {
        ...
        Shell activeShell = ((SwtDisplay) display.getImpl()).activeShell;
        if (activeShell == this.getApi())
            return;
        if (!force) {
            if (activeShell == null)
                return;
            if (!((SwtDisplay) display.getImpl()).activePending) {
                long focusHandle = GTK.gtk_window_get_focus(((SwtShell) activeShell.getImpl()).shellHandle);
	...
	*/
        ...
        if ((xFocus || (getApi().style & SWT.ON_TOP) != 0)) {
            ...
            if (GTK.GTK4) {
                GTK4.gdk_toplevel_focus(gdkResource, ((SwtDisplay) display.getImpl()).lastUserEventTime);
            } else {
                GDK.gdk_window_focus(gdkResource, ((SwtDisplay) display.getImpl()).lastUserEventTime);
            }
        }
        ((SwtDisplay) display.getImpl()).activeShell = this.getApi();
        ((SwtDisplay) display.getImpl()).activePending = true;
    }
    ...
    Rectangle computeTrimInPixels(int x, int y, int width, int height) {
        ...
        int border = 0;
        if ((getApi().style & (SWT.NO_TRIM | SWT.BORDER | SWT.SHELL_TRIM)) == 0 || isCustomResize()) {
    ...
    void createHandle(int index) {
        ...
        if (shellHandle == 0) {
            ...
            if (getApi().handle == 0) {
                ...
                shellHandle = GTK.gtk_plug_new(getApi().handle);
            }
            if (shellHandle == 0)
                error(SWT.ERROR_NO_HANDLES);
            if (isChildShell) {
                if (GTK.GTK4) {
                    GTK.gtk_window_set_transient_for(shellHandle, parent.getImpl().topHandle());
				 ...
				 */
                    if (OS.isWayland()) {
                        ...
                        if (topLevelParent != null) {
                            GTK.gtk_window_set_transient_for(shellHandle, topLevelParent.getImpl().topHandle());
                        } else {
                            GTK.gtk_window_set_transient_for(shellHandle, parent.getImpl().topHandle());
                        }
                        // this marks the logical parent
                        GTK3.gtk_window_set_attached_to(shellHandle, parent.getImpl().topHandle());
                        ...
                        if (parent != topLevelParent && isMappedToPopup()) {
                            if (parent.getImpl() instanceof DartComposite) {
                                ((DartComposite) parent.getImpl()).popupChild = this.getApi();
                            }
                            if (parent.getImpl() instanceof SwtComposite) {
                                ((SwtComposite) parent.getImpl()).popupChild = this.getApi();
                            }
                        }
                    } else {
                        GTK.gtk_window_set_transient_for(shellHandle, parent.getImpl().topHandle());
            ...
            } else if ((getApi().style & SWT.ON_TOP) != 0) {
                /*
            ...
            if ((getApi().style & (SWT.NO_TRIM | SWT.BORDER | SWT.SHELL_TRIM)) == 0) {
		 ...
		 */
            if (!OS.isX11() && (getApi().style & SWT.SHELL_TRIM) == 0) {
                GTK.gtk_window_set_decorated(shellHandle, false);
            }
            if (isCustomResize()) {
	...
	*/
                ...
                if (xFocusEvent.mode == OS.NotifyNormal || xFocusEvent.mode == OS.NotifyWhileGrabbed) {
                    switch(xFocusEvent.detail) {
                        ...
                        case OS.NotifyAncestor:
                            ((SwtDisplay) display.getImpl()).activeShell = this.getApi();
                            ((SwtDisplay) display.getImpl()).activePending = false;
                ...
                if (xFocusEvent.mode == OS.NotifyNormal || xFocusEvent.mode == OS.NotifyWhileGrabbed) {
                    switch(xFocusEvent.detail) {
                        ...
                        case OS.NotifyVirtual:
                            ...
                            if (((SwtDisplay) display.getImpl()).activeShell == this.getApi()) {
                                ((SwtDisplay) display.getImpl()).activeShell = null;
                                ((SwtDisplay) display.getImpl()).activePending = false;
                            }
                            if (isDisposed())
    ...
    public Control findBackgroundControl() {
        return (getApi().state & BACKGROUND) != 0 || backgroundImage != null ? this.getApi() : null;
    }
    ...
    void hookEvents() {
        super.hookEvents();
        OS.g_signal_connect(shellHandle, OS.dpi_changed, ((SwtDisplay) display.getImpl()).notifyProc, SwtWidget.DPI_CHANGED);
        if (GTK.GTK4) {
            ...
            OS.g_signal_connect(gdkSurface, OS.notify_state, ((SwtDisplay) display.getImpl()).notifyProc, shellHandle);
            OS.g_signal_connect(gdkSurface, OS.compute_size, ((SwtDisplay) display.getImpl()).computeSizeProc, shellHandle);
            OS.g_signal_connect(shellHandle, OS.notify_default_height, ((SwtDisplay) display.getImpl()).notifyProc, SwtWidget.NOTIFY_DEFAULT_HEIGHT);
            OS.g_signal_connect(shellHandle, OS.notify_default_width, ((SwtDisplay) display.getImpl()).notifyProc, SwtWidget.NOTIFY_DEFAULT_WIDTH);
            OS.g_signal_connect(shellHandle, OS.notify_maximized, ((SwtDisplay) display.getImpl()).notifyProc, SwtWidget.NOTIFY_MAXIMIZED);
        } else {
            OS.g_signal_connect_closure_by_id(shellHandle, ((SwtDisplay) display.getImpl()).signalIds[WINDOW_STATE_EVENT], 0, ((SwtDisplay) display.getImpl()).getClosure(WINDOW_STATE_EVENT), false);
            OS.g_signal_connect_closure_by_id(shellHandle, ((SwtDisplay) display.getImpl()).signalIds[CONFIGURE_EVENT], 0, ((SwtDisplay) display.getImpl()).getClosure(CONFIGURE_EVENT), false);
            OS.g_signal_connect_closure_by_id(shellHandle, ((SwtDisplay) display.getImpl()).signalIds[MAP_EVENT], 0, ((SwtDisplay) display.getImpl()).shellMapProcClosure, false);
            OS.g_signal_connect_closure_by_id(shellHandle, ((SwtDisplay) display.getImpl()).signalIds[SIZE_ALLOCATE], 0, ((SwtDisplay) display.getImpl()).getClosure(SIZE_ALLOCATE), false);
        }
        if (GTK.GTK4) {
            OS.g_signal_connect_closure(shellHandle, OS.close_request, ((SwtDisplay) display.getImpl()).getClosure(CLOSE_REQUEST), false);
            OS.g_signal_connect(shellHandle, OS.notify_is_active, ((SwtDisplay) display.getImpl()).windowActiveProc, FOCUS_IN);
            ...
            long enterMotionAddress = ((SwtDisplay) display.getImpl()).enterMotionCallback.getAddress();
            ...
            if (isCustomResize()) {
                ...
                long leaveAddress = ((SwtDisplay) display.getImpl()).leaveCallback.getAddress();
                OS.g_signal_connect(enterLeaveController, OS.leave, leaveAddress, LEAVE);
            }
        } else {
            OS.g_signal_connect_closure_by_id(shellHandle, ((SwtDisplay) display.getImpl()).signalIds[DELETE_EVENT], 0, ((SwtDisplay) display.getImpl()).getClosure(DELETE_EVENT), false);
            OS.g_signal_connect_closure_by_id(shellHandle, ((SwtDisplay) display.getImpl()).signalIds[KEY_PRESS_EVENT], 0, ((SwtDisplay) display.getImpl()).getClosure(KEY_PRESS_EVENT), false);
            OS.g_signal_connect_closure_by_id(shellHandle, ((SwtDisplay) display.getImpl()).signalIds[FOCUS_IN_EVENT], 0, ((SwtDisplay) display.getImpl()).getClosure(FOCUS_IN_EVENT), false);
            OS.g_signal_connect_closure_by_id(shellHandle, ((SwtDisplay) display.getImpl()).signalIds[FOCUS_OUT_EVENT], 0, ((SwtDisplay) display.getImpl()).getClosure(FOCUS_OUT_EVENT), false);
            OS.g_signal_connect_closure_by_id(shellHandle, ((SwtDisplay) display.getImpl()).signalIds[ENTER_NOTIFY_EVENT], 0, ((SwtDisplay) display.getImpl()).getClosure(ENTER_NOTIFY_EVENT), false);
        }
        OS.g_signal_connect_closure(shellHandle, OS.move_focus, ((SwtDisplay) display.getImpl()).getClosure(MOVE_FOCUS), false);
        if (isCustomResize()) {
            if (!GTK.GTK4) {
                ...
                OS.g_signal_connect_closure_by_id(shellHandle, ((SwtDisplay) display.getImpl()).signalIds[MOTION_NOTIFY_EVENT], 0, ((SwtDisplay) display.getImpl()).getClosure(MOTION_NOTIFY_EVENT), false);
                OS.g_signal_connect_closure_by_id(shellHandle, ((SwtDisplay) display.getImpl()).signalIds[LEAVE_NOTIFY_EVENT], 0, ((SwtDisplay) display.getImpl()).getClosure(LEAVE_NOTIFY_EVENT), false);
                OS.g_signal_connect_closure_by_id(shellHandle, ((SwtDisplay) display.getImpl()).signalIds[EXPOSE_EVENT], 0, ((SwtDisplay) display.getImpl()).getClosure(EXPOSE_EVENT), false);
                OS.g_signal_connect_closure_by_id(shellHandle, ((SwtDisplay) display.getImpl()).signalIds[BUTTON_PRESS_EVENT], 0, ((SwtDisplay) display.getImpl()).getClosure(BUTTON_PRESS_EVENT), false);
            }
        }
    }
    ...
    boolean isUndecorated() {
        return (getApi().style & (SWT.SHELL_TRIM | SWT.BORDER)) == SWT.NONE || (getApi().style & (SWT.NO_TRIM | SWT.ON_TOP)) != 0;
    }
    ...
    boolean isCustomResize() {
        return (getApi().style & SWT.NO_TRIM) == 0 && (getApi().style & (SWT.RESIZE | SWT.ON_TOP)) == (SWT.RESIZE | SWT.ON_TOP);
    }
    ...
    void register() {
        super.register();
        ((SwtDisplay) display.getImpl()).addWidget(shellHandle, this.getApi());
    }
    ...
    void fixActiveShell() {
        ...
        if (((SwtDisplay) display.getImpl()).activeShell == this.getApi() && (getApi().style & SWT.ON_TOP) != 0) {
            Shell shell = null;
    ...
    long gtk_button_press_event(long widget, long event) {
        if (widget == shellHandle) {
            if (isCustomResize()) {
                ...
                if (eventButton[0] == 1) {
                    ((SwtDisplay) display.getImpl()).resizeLocationX = eventRX[0];
                    ((SwtDisplay) display.getImpl()).resizeLocationY = eventRY[0];
                    int[] x = new int[1], y = new int[1];
                    GTK3.gtk_window_get_position(shellHandle, x, y);
                    ((SwtDisplay) display.getImpl()).resizeBoundsX = x[0];
                    ((SwtDisplay) display.getImpl()).resizeBoundsY = y[0];
                    GtkAllocation allocation = new GtkAllocation();
                    GTK.gtk_widget_get_allocation(shellHandle, allocation);
                    ((SwtDisplay) display.getImpl()).resizeBoundsWidth = allocation.width;
                    ((SwtDisplay) display.getImpl()).resizeBoundsHeight = allocation.height;
                }
            }
            /**
    ...
    long gtk_configure_event(long widget, long event) {
        ...
        GTK3.gtk_window_get_position(shellHandle, x, y);
        if (!isVisible()) {
            //We shouldn't handle move/resize events if shell is hidden.
            return 0;
        }
    ...
    long gtk_focus(long widget, long directionType) {
        switch((int) directionType) {
            ...
            case GTK.GTK_DIR_TAB_BACKWARD:
                ...
                if (control != null) {
                    if ((control.state & CANVAS) != 0 && (control.style & SWT.EMBEDDED) != 0 && control.getShell() == this.getApi()) {
    ...
    long gtk_focus_out_event(long widget, long event) {
        ...
        if (((SwtDisplay) display.getImpl()).activeShell == this.getApi() && !ignoreFocusOut) {
            ((SwtDisplay) display.getImpl()).activeShell = null;
            ((SwtDisplay) display.getImpl()).activePending = false;
        }
        return 0;
    ...
    long gtk_move_focus(long widget, long directionType) {
        Control control = display.getFocusControl();
        if (control != null) {
            long focusHandle = ((SwtControl) control.getImpl()).focusHandle();
    ...
    long gtk_motion_notify_event(long widget, long event) {
        if (widget == shellHandle) {
            if (isCustomResize()) {
                ...
                if ((state[0] & GDK.GDK_BUTTON1_MASK) != 0) {
                    int border = gtk_container_get_border_width_or_margin(shellHandle);
                    int dx = (int) (eventRX[0] - ((SwtDisplay) display.getImpl()).resizeLocationX);
                    int dy = (int) (eventRY[0] - ((SwtDisplay) display.getImpl()).resizeLocationY);
                    int x = ((SwtDisplay) display.getImpl()).resizeBoundsX;
                    int y = ((SwtDisplay) display.getImpl()).resizeBoundsY;
                    int width = ((SwtDisplay) display.getImpl()).resizeBoundsWidth;
                    int height = ((SwtDisplay) display.getImpl()).resizeBoundsHeight;
                    ...
                    switch(((SwtDisplay) display.getImpl()).resizeMode) {
    ...
    long gtk_key_press_event(long widget, long event) {
        if (widget == shellHandle) {
            ...
            if (menuBar != null && !menuBar.isDisposed()) {
                ...
                if (focusControl != null && (((SwtWidget) focusControl.getImpl()).hooks(SWT.KeyDown) || ((SwtWidget) focusControl.getImpl()).filters(SWT.KeyDown))) {
	 ...
	 */
        ...
        if ((!resized || oldWidth != width || oldHeight != height) && (OS.isWayland() ? ((getApi().style & SWT.RESIZE) != 0) : true)) {
            oldWidth = width;
            oldHeight = height;
            //this is called to resize child widgets when the shell is resized.
    ...
    long gtk_realize(long widget) {
        ...
        if ((getApi().style & SWT.SHELL_TRIM) != SWT.SHELL_TRIM) {
            updateDecorations(gdkResource);
        } else if ((getApi().style & SWT.NO_MOVE) != 0) {
            if (GTK.GTK4) {
                /*TODO: GTK4 no longer supports specifying hints for window management functions to be available on the window frame
			 * Usually window managers don't do anything with the hint anyways. */
            } else {
	...
	*/
		...
		*/
            if ((getApi().style & SWT.RESIZE) != 0 || (geometry.getMinHeight() != 0 || geometry.getMinWidth() != 0 || geometry.getMaxHeight() != 0 || geometry.getMaxWidth() != 0)) {
                if (GTK.GTK4) {
                    /*
    ...
    public void setEnabled(boolean enabled) {
        ...
        if (fixFocus)
            fixFocus(control);
        if (enabled && ((SwtDisplay) display.getImpl()).activeShell == this.getApi()) {
            if (!restoreFocus())
                traverseGroup(false);
        }
    }
 ...
 */
		...
		*/
		 ...
		 */
            ...
            if (!(!GTK.GTK4 && OS.isX11() && GTK.GTK_IS_PLUG(shellHandle))) {
                ((SwtDisplay) display.getImpl()).dispatchEvents = new int[] { GDK.GDK_EXPOSE, GDK.GDK_FOCUS_CHANGE, GDK.GDK_CONFIGURE, GDK.GDK_MAP, GDK.GDK_UNMAP, GDK.GDK_NO_EXPOSE, GDK.GDK_WINDOW_STATE };
                Display display = this.display;
                ((SwtDisplay) display.getImpl()).putGdkEvents();
                ...
                do {
                    ...
                    if (isDisposed())
                        break;
                    iconic = minimized || (shell != null && ((SwtDecorations) shell.getImpl()).minimized);
                } while (!mapped && !iconic);
                ((SwtDisplay) display.getImpl()).dispatchEvents = null;
                if (isDisposed())
                    return;
                if (!iconic) {
                    update(true, true);
	...
	*/
    ...
    int trimHeight() {
        ...
        if (hasTitle) {
            if (hasResize)
                return ((SwtDisplay) display.getImpl()).trimHeights[SwtDisplay.TRIM_TITLE_RESIZE];
            if (hasBorder)
                return ((SwtDisplay) display.getImpl()).trimHeights[SwtDisplay.TRIM_TITLE_BORDER];
            return ((SwtDisplay) display.getImpl()).trimHeights[SwtDisplay.TRIM_TITLE];
        }
        if (hasResize)
            return ((SwtDisplay) display.getImpl()).trimHeights[SwtDisplay.TRIM_RESIZE];
        if (hasBorder)
            return ((SwtDisplay) display.getImpl()).trimHeights[SwtDisplay.TRIM_BORDER];
        return ((SwtDisplay) display.getImpl()).trimHeights[SwtDisplay.TRIM_NONE];
    }
    ...
    int trimWidth() {
        ...
        if (hasTitle) {
            if (hasResize)
                return ((SwtDisplay) display.getImpl()).trimWidths[SwtDisplay.TRIM_TITLE_RESIZE];
            if (hasBorder)
                return ((SwtDisplay) display.getImpl()).trimWidths[SwtDisplay.TRIM_TITLE_BORDER];
            return ((SwtDisplay) display.getImpl()).trimWidths[SwtDisplay.TRIM_TITLE];
        }
        if (hasResize)
            return ((SwtDisplay) display.getImpl()).trimWidths[SwtDisplay.TRIM_RESIZE];
        if (hasBorder)
            return ((SwtDisplay) display.getImpl()).trimWidths[SwtDisplay.TRIM_BORDER];
        return ((SwtDisplay) display.getImpl()).trimWidths[SwtDisplay.TRIM_NONE];
    }
    ...
    void updateModal() {
        ...
        if (((SwtDisplay) display.getImpl()).getModalDialog() == null) {
            ...
            while (shell != null) {
                if ((shell.style & mask) == 0) {
                    group = ((SwtShell) shell.getShell().getImpl()).group;
                    break;
                }
                topModalShell = shell;
                shell = shell.getImpl()._parent();
            }
            /*
		...
		*/
            if (group == 0 && topModalShell != null)
                group = ((SwtShell) topModalShell.getShell().getImpl()).group;
        }
        if (group == 0) {
            /*
    ...
    void updateMinimized(boolean minimized) {
        ...
        for (int i = 0; i < shells.length; i++) {
            ...
            Shell shell = shells[i];
            while (shell != null && shell != this.getApi() && !((SwtShell) shell.getImpl()).isUndecorated()) {
                shell = (Shell) shell.getParent();
            }
            if (shell != null && shell != this.getApi())
                update = true;
            if (update) {
                if (minimized) {
    ...
    void deregister() {
        super.deregister();
        Widget disposed = ((SwtDisplay) display.getImpl()).removeWidget(shellHandle);
    ...
    boolean requiresUngrab() {
        return OS.isWayland() && (getApi().style & SWT.ON_TOP) != 0 && (getApi().style & SWT.NO_FOCUS) == 0;
    }
	...
	*/
    ...
    Rectangle getBoundsInPixels() {
        checkWidget();
        int[] x = new int[1], y = new int[1];
        if ((getApi().state & SwtWidget.DISPOSE_SENT) == 0) {
            if (GTK.GTK4) {
                // TODO: GTK4 GtkWindow no longer has the ability to get position
            } else {
                GTK3.gtk_window_get_position(shellHandle, x, y);
