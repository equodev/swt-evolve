...
public class SwtToolBar extends SwtComposite implements IToolBar {
    ...
    static Callback menuItemSelectedFunc = new Callback(SwtToolBar.class, "MenuItemSelectedProc", 2);
	...
	*/
        ...
        GTK.gtk_orientable_set_orientation(getApi().handle, orientation);
    }
	...
	*/
    ...
    void createHandle(int index) {
        ...
        if (GTK.GTK4) {
            ...
            GTK.gtk_widget_add_css_class(getApi().handle, Converter.javaStringToCString("toolbar"));
        } else {
            GTK3.gtk_widget_set_has_window(fixedHandle, true);
            getApi().handle = GTK3.gtk_toolbar_new();
        }
        if (getApi().handle == 0)
            error(SWT.ERROR_NO_HANDLES);
        if (GTK.GTK4) {
	...
	*/
        if (!GTK.GTK4)
            GTK3.gtk_toolbar_set_icon_size(getApi().handle, GTK.GTK_ICON_SIZE_SMALL_TOOLBAR);
    ...
    ToolItem[] _getItems() {
        if (GTK.GTK4) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                Widget widget = ((SwtDisplay) display.getImpl()).getWidget(data);
                if (widget != null)
    ...
    static long MenuItemSelectedProc(long widget, long /*int*/
    user_data) {
        Display display = SwtDisplay.getCurrent();
        ToolItem item = (ToolItem) ((SwtDisplay) display.getImpl()).getWidget(user_data);
        if (item != null) {
            return ((SwtToolBar) item.getParent().getImpl()).menuItemSelected(widget, item);
        }
        return 0;
    }
        ...
        switch(item.style) {
            ...
            case SWT.RADIO:
                if ((getApi().style & SWT.NO_RADIO_GROUP) == 0)
                    ((SwtToolItem) item.getImpl()).selectRadio();
                break;
            case SWT.CHECK:
    ...
    boolean mnemonicHit(char key) {
        ToolItem[] items = getItems();
        for (int i = 0; i < items.length; i++) {
            long labelHandle = ((SwtToolItem) items[i].getImpl()).labelHandle;
    ...
    void relayout() {
        ...
        for (int i = 0; i < items.length; i++) {
            ...
            if (item != null) {
                ...
                hasText |= ((SwtItem) item.getImpl()).text != null && ((SwtItem) item.getImpl()).text.length() > 0;
		 ...
		 * therefore we will need to implement these style ourselves. */
            ...
            GTK3.gtk_toolbar_set_style(getApi().handle, type);
        }
    }
    ...
    void setBackgroundGdkRGBA(long context, long handle, GdkRGBA rgba) {
        ...
        String css = "toolbar {background-color: " + ((SwtDisplay) display.getImpl()).gtk_rgba_to_css_string(rgba) + ";}";
        // Cache background color
        this.cssBackground = css;
        // Apply background color and any foreground color
        String finalCss = ((SwtDisplay) display.getImpl()).gtk_css_create_css_color_string(this.cssBackground, this.cssForeground, SWT.BACKGROUND);
        gtk_css_provider_load_from_css(context, finalCss);
    }

    @Override
    void setParentBackground() {
        setBackgroundGdkRGBA(getApi().handle, display.getSystemColor(SWT.COLOR_TRANSPARENT).handle);
    ...
    void setForegroundGdkRGBA(long handle, GdkRGBA rgba) {
        ...
        String finalCss = ((SwtDisplay) display.getImpl()).gtk_css_create_css_color_string(this.cssBackground, this.cssForeground, SWT.FOREGROUND);
        gtk_css_provider_load_from_css(context, finalCss);
    }
    ...
    void setFontDescription(long font) {
        ...
        ToolItem[] items = getItems();
        for (int i = 0; i < items.length; i++) {
            ((SwtToolItem) items[i].getImpl()).setFontDescription(font);
        }
        relayout();
    }
    ...
    void setOrientation(boolean create) {
        ...
        ToolItem[] items = _getItems();
        for (int i = 0; i < items.length; i++) {
            ((SwtToolItem) items[i].getImpl()).setOrientation(create);
        }
    }
    ...
    public void setToolTipText(String string) {
        ...
        ToolItem[] items = getItems();
        for (int i = 0; i < items.length; i++) {
            String newString = string != null ? null : ((SwtToolItem) items[i].getImpl()).toolTipText;
            setToolTipText(items[i].handle, newString);
        }
    }

    public ToolItem _currentFocusItem() {
    ...
    public boolean _hasChildFocus() {
        return hasChildFocus;
    }
