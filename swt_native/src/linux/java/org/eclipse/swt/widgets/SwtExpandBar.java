...
public class SwtExpandBar extends SwtComposite implements IExpandBar {
    ...
    Point computeSizeInPixels(int wHint, int hHint, boolean changed) {
        ...
        int border = gtk_container_get_border_width_or_margin(getApi().handle);
        size.x += 2 * border;
        size.y += 2 * border;
        return size;
    }
    ...
    void createHandle(int index) {
        ...
        getApi().handle = gtk_box_new(GTK.GTK_ORIENTATION_VERTICAL, false, 0);
        if (getApi().handle == 0)
            error(SWT.ERROR_NO_HANDLES);
        ...
        gtk_container_set_border_width(getApi().handle, 0);
    ...
    void createItem(ExpandItem item, int style, int index) {
        ...
        items[index] = item;
        itemCount++;
        ((SwtExpandItem) item.getImpl()).width = Math.max(0, getClientAreaInPixels().width - spacing * 2);
        layoutItems();
    }
    ...
    boolean forceFocus(long focusHandle) {
        if (lastFocus != null && ((SwtExpandItem) lastFocus.getImpl()).setFocus())
            return true;
        for (int i = 0; i < itemCount; i++) {
            ExpandItem item = items[i];
            if (((SwtExpandItem) item.getImpl()).setFocus())
                return true;
        }
    ...
    boolean hasFocus() {
        for (int i = 0; i < itemCount; i++) {
            ExpandItem item = items[i];
            if (((SwtExpandItem) item.getImpl()).hasFocus())
                return true;
        }
    ...
    void hookEvents() {
        ...
        if (!GTK.GTK4) {
            if (scrolledHandle != 0)
                OS.g_signal_connect_closure(scrolledHandle, OS.size_allocate, ((SwtDisplay) display.getImpl()).getClosure(SIZE_ALLOCATE), true);
        }
    }
    ...
    long gtk_key_press_event(long widget, long event) {
        ...
        int index = 0;
        while (index < itemCount) {
            if (((SwtExpandItem) items[index].getImpl()).hasFocus())
                break;
            index++;
        }
        int[] key = new int[1];
    ...
    void layoutItems() {
        for (int i = 0; i < itemCount; i++) {
            ExpandItem item = items[i];
            if (item != null)
                ((SwtExpandItem) item.getImpl()).resizeControl();
        }
    }
    ...
    void setFontDescription(long font) {
        ...
        for (int i = 0; i < itemCount; i++) {
            ((SwtExpandItem) items[i].getImpl()).setFontDescription(font);
        }
        layoutItems();
    }
    ...
    void setForegroundGdkRGBA(GdkRGBA rgba) {
        ...
        for (int i = 0; i < itemCount; i++) {
            ((SwtExpandItem) items[i].getImpl()).setForegroundRGBA(rgba);
        }
    }
    ...
    void setOrientation(boolean create) {
        ...
        if (items != null) {
            for (int i = 0; i < items.length; i++) {
                if (items[i] != null)
                    ((SwtExpandItem) items[i].getImpl()).setOrientation(create);
            }
        }
    }
    ...
    void setSpacingInPixels(int spacing) {
        ...
        GTK.gtk_box_set_spacing(getApi().handle, spacing);
        gtk_container_set_border_width(getApi().handle, spacing);
    }
