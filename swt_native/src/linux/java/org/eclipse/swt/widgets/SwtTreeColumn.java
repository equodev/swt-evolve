 ...
 */
public class SwtTreeColumn extends SwtItem implements ITreeColumn {
	...
	*/
    ...
    public void pack() {
        ...
        if ((parent.style & SWT.VIRTUAL) != 0) {
            for (int i = 0; i < ((SwtTree) parent.getImpl()).items.length; i++) {
                TreeItem item = ((SwtTree) parent.getImpl()).items[i];
                if (item != null && ((SwtTreeItem) item.getImpl()).cached) {
                    width = Math.max(width, ((SwtTree) parent.getImpl()).calculateWidth(getApi().handle, item.handle, true));
                }
            }
        } else {
            long iter = OS.g_malloc(GTK.GtkTreeIter_sizeof());
            if (GTK.gtk_tree_model_get_iter_first(((SwtTree) parent.getImpl()).modelHandle, iter)) {
                do {
                    width = Math.max(width, ((SwtTree) parent.getImpl()).calculateWidth(getApi().handle, iter, true));
                } while (GTK.gtk_tree_model_iter_next(((SwtTree) parent.getImpl()).modelHandle, iter));
            }
            OS.g_free(iter);
        }
        setWidth(width);
    ...
    public void setAlignment(int alignment) {
        ...
        ((SwtTree) parent.getImpl()).createRenderers(getApi().handle, modelIndex, index == 0, getApi().style);
    }
    ...
    public void setImage(Image image) {
        ...
        if (image != null) {
            ImageList headerImageList = ((SwtTree) parent.getImpl()).headerImageList;
            if (headerImageList == null) {
                headerImageList = ((SwtTree) parent.getImpl()).headerImageList = new ImageList();
            }
