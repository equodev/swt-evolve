...
public class SwtButton extends SwtControl implements IButton {
	...
	*/
        ...
        Point size;
        boolean wrap = labelHandle != 0 && (getApi().style & SWT.WRAP) != 0 && GTK.gtk_widget_get_visible(labelHandle);
        if (wrap) {
            int borderWidth = gtk_container_get_border_width_or_margin(getApi().handle);
            int trimWidth, trimHeight;
            if (!GTK.GTK4) {
                int[] focusWidth = new int[1];
                GTK3.gtk_widget_style_get(getApi().handle, OS.focus_line_width, focusWidth, 0);
                int[] focusPadding = new int[1];
                GTK3.gtk_widget_style_get(getApi().handle, OS.focus_padding, focusPadding, 0);
            ...
            if ((getApi().style & (SWT.CHECK | SWT.RADIO)) != 0) {
                if (GTK.GTK4) {
                    long icon = GTK4.gtk_widget_get_first_child(getApi().handle);
                    ...
                    GTK3.gtk_widget_style_get(getApi().handle, OS.indicator_size, indicatorSize, 0);
                    GTK3.gtk_widget_style_get(getApi().handle, OS.indicator_spacing, indicatorSpacing, 0);
                    indicatorHeight = indicatorSize[0] + 2 * indicatorSpacing[0];
                    trimWidth += indicatorHeight + indicatorSpacing[0];
                }
            } else {
                Point thickness = getThickness(getApi().handle);
                trimWidth += thickness.x * 2;
                trimHeight += thickness.y * 2;
                GtkBorder innerBorder = getBorder(OS.inner_border, getApi().handle, INNER_BORDER);
                trimWidth += innerBorder.left + innerBorder.right;
                trimHeight += innerBorder.top + innerBorder.bottom;
                boolean canDefault = GTK.GTK4 ? GTK4.gtk_widget_get_receives_default(getApi().handle) : GTK3.gtk_widget_get_can_default(getApi().handle);
                if (canDefault) {
                    GtkBorder defaultBorder = getBorder(OS.default_border, getApi().handle, DEFAULT_BORDER);
        ...
        if (wHint != SWT.DEFAULT || hHint != SWT.DEFAULT) {
            boolean canDefault = GTK.GTK4 ? GTK4.gtk_widget_get_receives_default(getApi().handle) : GTK3.gtk_widget_get_can_default(getApi().handle);
            if (canDefault) {
                GtkBorder border = getBorder(OS.default_border, getApi().handle, DEFAULT_BORDER);
                if (wHint != SWT.DEFAULT)
    ...
    void createHandle(int index) {
        ...
        switch(getApi().style & bits) {
            case SWT.ARROW:
                ...
                if (GTK.GTK4) {
                    getApi().handle = GTK4.gtk_button_new_from_icon_name(arrowType);
                    if (getApi().handle == 0)
                        error(SWT.ERROR_NO_HANDLES);
                    arrowHandle = GTK4.gtk_widget_get_first_child(getApi().handle);
                    if (arrowHandle == 0)
                        error(SWT.ERROR_NO_HANDLES);
                } else {
                    ...
                    GTK3.gtk_button_set_image(getApi().handle, arrowHandle);
                }
                break;
            case SWT.TOGGLE:
                ...
                if (getApi().handle == 0)
                    error(SWT.ERROR_NO_HANDLES);
                if (SwtDisplay.themeName != null) {
                    toggleButtonTheming = (GTK.GTK_VERSION >= OS.VERSION(3, 24, 11) && SwtDisplay.themeName.contains("Adwaita")) || SwtDisplay.themeName.contains("Yaru");
                }
                break;
            case SWT.CHECK:
                getApi().handle = GTK.gtk_check_button_new();
                if (getApi().handle == 0)
                    error(SWT.ERROR_NO_HANDLES);
                if (SwtDisplay.themeName != null) {
                    toggleButtonTheming = (GTK.GTK_VERSION >= OS.VERSION(3, 24, 11) && SwtDisplay.themeName.contains("Adwaita")) || SwtDisplay.themeName.contains("Yaru");
			...
			*/
                if (GTK.GTK4) {
                    ...
                    if (getApi().handle == 0)
                        error(SWT.ERROR_NO_HANDLES);
                    GTK4.gtk_check_button_set_group(getApi().handle, groupHandle);
                } else {
                    groupHandle = GTK3.gtk_radio_button_new(0);
                    if (groupHandle == 0)
                        error(SWT.ERROR_NO_HANDLES);
                    OS.g_object_ref_sink(groupHandle);
                    getApi().handle = GTK3.gtk_radio_button_new(GTK3.gtk_radio_button_get_group(groupHandle));
                    if (getApi().handle == 0)
                        error(SWT.ERROR_NO_HANDLES);
                }
                if (SwtDisplay.themeName != null) {
            ...
            default:
                ...
                if (getApi().handle == 0)
                    error(SWT.ERROR_NO_HANDLES);
                if (!GTK.GTK4)
                    GTK3.gtk_widget_set_can_default(getApi().handle, true);
                break;
        }
        ...
        _setAlignment(getApi().style & (SWT.LEFT | SWT.CENTER | SWT.RIGHT));
    }
    ...
    public boolean getSelection() {
        ...
        if ((getApi().style & (SWT.CHECK | SWT.RADIO | SWT.TOGGLE)) == 0)
            return false;
        if (GTK.GTK4 && (getApi().style & (SWT.CHECK | SWT.RADIO)) != 0) {
            return GTK4.gtk_check_button_get_active(getApi().handle);
        } else {
            return GTK.gtk_toggle_button_get_active(getApi().handle);
        }
    }
    ...
    long gtk_clicked(long widget) {
        ...
        if ((getApi().style & SWT.RADIO) != 0) {
            ...
            if ((getApi().style & SWT.CHECK) != 0) {
                if (GTK.GTK4) {
                    boolean inconsistent = grayed && GTK4.gtk_check_button_get_active(getApi().handle);
                    GTK4.gtk_check_button_set_inconsistent(getApi().handle, inconsistent);
                } else {
                    boolean inconsistent = grayed && GTK.gtk_toggle_button_get_active(getApi().handle);
                    GTK3.gtk_toggle_button_set_inconsistent(getApi().handle, inconsistent);
                }
            }
        }
    ...
    long gtk_focus_in_event(long widget, long event) {
        ...
        if (getApi().handle == 0)
            return 0;
        if ((getApi().style & SWT.PUSH) != 0 && GTK.gtk_widget_has_default(getApi().handle)) {
            Decorations menuShell = menuShell();
            ((SwtDecorations) menuShell.getImpl()).defaultButton = this.getApi();
        }
        return result;
    }
    ...
    long gtk_focus_out_event(long widget, long event) {
        ...
        if ((getApi().style & SWT.PUSH) != 0) {
            Decorations menuShell = menuShell();
            if (((SwtDecorations) menuShell.getImpl()).defaultButton == this.getApi()) {
                ((SwtDecorations) menuShell.getImpl()).defaultButton = null;
            }
        }
        return result;
    }
    ...
    void hookEvents() {
        super.hookEvents();
        if (GTK.GTK4 && ((getApi().style & (SWT.RADIO | SWT.CHECK)) != 0)) {
            OS.g_signal_connect_closure(getApi().handle, OS.toggled, ((SwtDisplay) display.getImpl()).getClosure(CLICKED), false);
        } else {
	...
	*/
    ...
    void setBackgroundGdkRGBA(long context, long handle, GdkRGBA rgba) {
        ...
        String color = ((SwtDisplay) display.getImpl()).gtk_rgba_to_css_string(rgba);
        css += color + ";}";
        /*
	 * Feature in GTK: some toggle style (check, radio, and toggle) buttons
	 * have inverted colors, meaning the background color when checked is
	...
	*/
        ...
        switch(getApi().style & bits) {
            case SWT.CHECK:
                ...
                if (toggleButtonTheming) {
                    GdkRGBA checkRGBA = rgba == null ? ((SwtDisplay) display.getImpl()).COLOR_TOGGLE_BUTTON_FOREGROUND_RGBA : rgba;
                    checkColor = ((SwtDisplay) display.getImpl()).gtk_rgba_to_css_string(checkRGBA);
                } else {
                    checkColor = ((SwtDisplay) display.getImpl()).gtk_rgba_to_css_string(toSet);
                }
            ...
            case SWT.RADIO:
                ...
                String radioColor;
                if (toggleButtonTheming) {
                    GdkRGBA radioRGBA = rgba == null ? ((SwtDisplay) display.getImpl()).COLOR_TOGGLE_BUTTON_FOREGROUND_RGBA : rgba;
                    radioColor = ((SwtDisplay) display.getImpl()).gtk_rgba_to_css_string(radioRGBA);
                } else {
                    radioColor = ((SwtDisplay) display.getImpl()).gtk_rgba_to_css_string(toSet);
                }
        ...
        if ((getApi().style & SWT.RIGHT_TO_LEFT) != 0 || !create) {
            ...
            if (arrowHandle != 0) {
                if ((getApi().style & (SWT.LEFT | SWT.RIGHT)) != 0) {
                    byte[] arrowType = (getApi().style & SWT.RIGHT_TO_LEFT) != 0 ? GTK.GTK_NAMED_ICON_GO_NEXT : GTK.GTK_NAMED_ICON_GO_PREVIOUS;
                    if (GTK.GTK4) {
                        GTK4.gtk_image_set_from_icon_name(arrowHandle, arrowType);
                    } else {
                        GTK3.gtk_image_set_from_icon_name(arrowHandle, arrowType, GTK.GTK_ICON_SIZE_MENU);
    ...
    public void setSelection(boolean selected) {
        ...
        if (GTK.GTK4 && (getApi().style & (SWT.CHECK | SWT.RADIO)) != 0) {
            GTK4.gtk_check_button_set_active(getApi().handle, selected);
        } else {
