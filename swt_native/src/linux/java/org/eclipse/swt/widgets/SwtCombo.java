...
public class SwtCombo extends SwtComposite implements ICombo {
    ...
    public void add(String string, int index) {
        ...
        if ((getApi().style & SWT.RIGHT_TO_LEFT) != 0 && popupHandle != 0) {
            GTK3.gtk_container_forall(popupHandle, ((SwtDisplay) display.getImpl()).setDirectionProc, GTK.GTK_TEXT_DIR_RTL);
        }
    }
    ...
    private void gtk_combo_box_insert(String string, int index) {
        ...
        if (getApi().handle != 0) {
            ...
            GTK.gtk_combo_box_text_insert(getApi().handle, index, null, buffer);
    ...
    private void gtk_combo_box_toggle_wrap(boolean wrap) {
        if (getApi().handle == 0 || GTK.GTK4)
            return;
        if (!wrap) {
            if (GTK3.gtk_combo_box_get_wrap_width(getApi().handle) == 1) {
                GTK3.gtk_combo_box_set_wrap_width(getApi().handle, 0);
            }
        } else {
            if (delayedEnableWrap) {
                ...
                display.asyncExec(() -> {
                    if (!isDisposed() && getApi().handle != 0) {
                        GTK3.gtk_combo_box_set_wrap_width(getApi().handle, 1);
	...
	*/
    ...
    void clearText() {
        ...
        if ((getApi().style & SWT.READ_ONLY) != 0) {
            int index = GTK.gtk_combo_box_get_active(getApi().handle);
            if (index != -1) {
                long modelHandle = GTK.gtk_combo_box_get_model(getApi().handle);
                long[] ptr = new long[1];
        ...
        GTK.gtk_combo_box_set_active(getApi().handle, -1);
	 ...
	 */
        if ((getApi().style & SWT.READ_ONLY) != 0 && !GTK.GTK4) {
    ...
    void createHandle(int index) {
        ...
        if ((getApi().style & SWT.READ_ONLY) != 0) {
            getApi().handle = GTK.gtk_combo_box_text_new();
            if (getApi().handle == 0)
                error(SWT.ERROR_NO_HANDLES);
            cellHandle = GTK.GTK4 ? GTK4.gtk_combo_box_get_child(getApi().handle) : GTK3.gtk_bin_get_child(getApi().handle);
            if (cellHandle == 0)
                error(SWT.ERROR_NO_HANDLES);
            gtk_combo_box_toggle_wrap(true);
        } else {
            getApi().handle = GTK.gtk_combo_box_text_new_with_entry();
            if (getApi().handle == 0)
                error(SWT.ERROR_NO_HANDLES);
            entryHandle = GTK.GTK4 ? GTK4.gtk_combo_box_get_child(getApi().handle) : GTK3.gtk_bin_get_child(getApi().handle);
            if (entryHandle == 0)
                error(SWT.ERROR_NO_HANDLES);
        ...
        if (textRenderer == 0)
            error(SWT.ERROR_NO_HANDLES);
        GTK.gtk_cell_layout_clear(getApi().handle);
        GTK.gtk_cell_layout_pack_start(getApi().handle, textRenderer, true);
        GTK.gtk_cell_layout_set_attributes(getApi().handle, textRenderer, OS.text, 0, 0);
        /*
	...
	*/
        if ((getApi().style & SWT.READ_ONLY) != 0 && buttonHandle != 0) {
	...
	*/
        long result = 0;
        if (GTK.GTK4) {
            for (long child = GTK4.gtk_widget_get_first_child(getApi().handle); child != 0; child = GTK4.gtk_widget_get_next_sibling(child)) {
            ...
            long childHandle = getApi().handle;
            /*
		 ...
		 */
            GTK3.gtk_container_forall(getApi().handle, ((SwtDisplay) display.getImpl()).allChildrenProc, 0);
            if (((SwtDisplay) display.getImpl()).allChildren != 0) {
                long list = ((SwtDisplay) display.getImpl()).allChildren;
                while (list != 0) {
                ...
                OS.g_list_free(((SwtDisplay) display.getImpl()).allChildren);
                ((SwtDisplay) display.getImpl()).allChildren = 0;
            }
            buttonBoxHandle = childHandle;
            GTK3.gtk_container_forall(childHandle, ((SwtDisplay) display.getImpl()).allChildrenProc, 0);
            if (((SwtDisplay) display.getImpl()).allChildren != 0) {
                long list = ((SwtDisplay) display.getImpl()).allChildren;
                while (list != 0) {
                    long widget = OS.g_list_data(list);
        ...
        if (cellBoxHandle != 0) {
            if (GTK.GTK4) {
                for (long child = GTK4.gtk_widget_get_first_child(cellBoxHandle); child != 0; child = GTK4.gtk_widget_get_next_sibling(child)) {
                    String name = ((SwtDisplay) display.getImpl()).gtk_widget_get_name(child);
                ...
                GTK3.gtk_container_forall(cellBoxHandle, ((SwtDisplay) display.getImpl()).allChildrenProc, 0);
                if (((SwtDisplay) display.getImpl()).allChildren != 0) {
                    long list = ((SwtDisplay) display.getImpl()).allChildren;
                    while (list != 0) {
                        long widget = OS.g_list_data(list);
					 ...
					 */
                        String name = ((SwtDisplay) display.getImpl()).gtk_widget_get_name(widget);
	...
	*/
        ...
        if (GTK.GTK4) {
            ...
            GTK3.gtk_container_forall(popupHandle, ((SwtDisplay) display.getImpl()).allChildrenProc, 0);
            if (((SwtDisplay) display.getImpl()).allChildren != 0) {
                long list = ((SwtDisplay) display.getImpl()).allChildren;
                while (list != 0) {
                    long widget = OS.g_list_data(list);
	...
	*/
    ...
    void hookEvents() {
        ...
        if (entryHandle != 0) {
            OS.g_signal_connect_closure(entryHandle, OS.changed, ((SwtDisplay) display.getImpl()).getClosure(CHANGED), true);
            OS.g_signal_connect_closure(entryHandle, OS.insert_text, ((SwtDisplay) display.getImpl()).getClosure(INSERT_TEXT), false);
            OS.g_signal_connect_closure(entryHandle, OS.delete_text, ((SwtDisplay) display.getImpl()).getClosure(DELETE_TEXT), false);
            OS.g_signal_connect_closure(entryHandle, OS.activate, ((SwtDisplay) display.getImpl()).getClosure(ACTIVATE), false);
            if (!GTK.GTK4)
                OS.g_signal_connect_closure(entryHandle, OS.populate_popup, ((SwtDisplay) display.getImpl()).getClosure(POPULATE_POPUP), false);
        }
        ...
        for (int i = 0; i < handles.length; i++) {
            ...
            if (eventHandle != 0) {
                if (GTK.GTK4) {
                    ...
                    OS.g_signal_connect(motionController, OS.motion, ((SwtDisplay) display.getImpl()).enterMotionProc, MOTION_INVERSE);
                    long gestureController = GTK4.gtk_gesture_click_new();
                    GTK4.gtk_widget_add_controller(eventHandle, gestureController);
                    OS.g_signal_connect(gestureController, OS.pressed, ((SwtDisplay) display.getImpl()).gesturePressReleaseProc, GESTURE_PRESSED);
                    OS.g_signal_connect(gestureController, OS.released, ((SwtDisplay) display.getImpl()).gesturePressReleaseProc, GESTURE_RELEASED);
                    ...
                    OS.g_signal_connect_closure_by_id(eventHandle, ((SwtDisplay) display.getImpl()).signalIds[MOTION_NOTIFY_EVENT], 0, ((SwtDisplay) display.getImpl()).getClosure(MOTION_NOTIFY_EVENT_INVERSE), true);
				...
				*/
                    OS.g_signal_connect_closure_by_id(eventHandle, ((SwtDisplay) display.getImpl()).signalIds[BUTTON_PRESS_EVENT], 0, ((SwtDisplay) display.getImpl()).getClosure(BUTTON_PRESS_EVENT_INVERSE), true);
                    OS.g_signal_connect_closure_by_id(eventHandle, ((SwtDisplay) display.getImpl()).signalIds[BUTTON_RELEASE_EVENT], 0, ((SwtDisplay) display.getImpl()).getClosure(BUTTON_RELEASE_EVENT_INVERSE), true);
                    ...
                    if (OS.G_OBJECT_TYPE(eventHandle) == GTK3.GTK_TYPE_MENU()) {
                        OS.g_signal_connect_closure(eventHandle, OS.selection_done, ((SwtDisplay) display.getImpl()).getClosure(SELECTION_DONE), true);
                    }
                }
            }
        }
    ...
    public void deselect(int index) {
        ...
        if (index < 0 || index >= items.length)
            return;
        if (GTK.gtk_combo_box_get_active(getApi().handle) == index) {
            clearText();
        }
    }
    ...
    long eventWindow() {
        if ((getApi().style & SWT.READ_ONLY) != 0) {
            ...
            return gtk_widget_get_window(getApi().handle);
        }
        /*
    ...
    public int getSelectionIndex() {
        checkWidget();
        return GTK.gtk_combo_box_get_active(getApi().handle);
    }
	...
	*/
    ...
    int getTextHeightInPixels() {
        ...
        gtk_widget_get_preferred_size(getApi().handle, requisition);
    ...
    public int getTextLimit() {
        ...
        return limit == 0 ? Combo.LIMIT : limit;
    }
	...
	*/
	 ...
	 */
        ...
        if (parentHandle != 0) {
            if (parent.getImpl()._fixClipHandle() == 0) {
                if (parent.getImpl() instanceof DartComposite) {
                    ((DartComposite) parent.getImpl()).fixClipHandle = parentHandle;
                }
                if (parent.getImpl() instanceof SwtComposite) {
                    ((SwtComposite) parent.getImpl()).fixClipHandle = parentHandle;
                }
            }
            if (firstDraw) {
                if ((getApi().style & SWT.READ_ONLY) != 0) {
                    long[] array = { fixedHandle, getApi().handle, buttonBoxHandle, buttonHandle, cellBoxHandle, cellHandle };
                    parent.getImpl()._fixClipMap().put(this.getApi(), array);
                } else {
                    long[] array = { fixedHandle, getApi().handle, entryHandle, buttonBoxHandle, buttonHandle };
                    parent.getImpl()._fixClipMap().put(this.getApi(), array);
                }
                firstDraw = false;
	...
	*/
            ...
            case GDK.GDK_BUTTON_PRESS:
                {
                    ...
                    if (eventButton[0] == 1) {
                        if (!sendMouseEvent(SWT.MouseDown, eventButton[0], ((SwtDisplay) display.getImpl()).clickCount, 0, false, eventTime, eventRX[0], eventRY[0], false, eventState[0])) {
                            return 1;
                        }
                        if ((getApi().style & SWT.READ_ONLY) == 0 && widget == buttonHandle) {
            ...
            case GDK.GDK_FOCUS_CHANGE:
                {
                    if ((getApi().style & SWT.READ_ONLY) == 0) {
                        ...
                        if (focusIn[0]) {
                            ...
                            GTK.gtk_widget_set_focus_on_click(getApi().handle, true);
                        }
                    }
                    break;
                }
        ...
        if ((getApi().style & SWT.READ_ONLY) == 0) {
			...
			*/
            ...
            if (newIndex != oldIndex) {
                GTK.gtk_combo_box_set_active(getApi().handle, newIndex);
                return 1;
            }
        }
        return result;
    ...
    @Override
    long paintSurface() {
        long childHandle = entryHandle != 0 ? entryHandle : getApi().handle;
    ...
    public void remove(int index) {
        ...
        if (GTK.gtk_combo_box_get_active(getApi().handle) == index)
            clearText();
        if (getApi().handle != 0)
            GTK.gtk_combo_box_text_remove(getApi().handle, index);
    }
    ...
    public void remove(int start, int end) {
        ...
        for (int i = end; i >= start; i--) {
            if (getApi().handle != 0)
                GTK.gtk_combo_box_text_remove(getApi().handle, i);
        }
        gtk_combo_box_toggle_wrap(true);
    }
    ...
    public void select(int index) {
        ...
        if (index < 0 || index >= items.length)
            return;
        int selected = GTK.gtk_combo_box_get_active(getApi().handle);
        OS.g_signal_handlers_block_matched(getApi().handle, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, CHANGED);
        GTK.gtk_combo_box_set_active(getApi().handle, index);
        OS.g_signal_handlers_unblock_matched(getApi().handle, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, CHANGED);
        if ((getApi().style & SWT.READ_ONLY) != 0 && selected != index) {
            /*
    ...
    private void gtk_combo_box_text_remove_all() {
        gtk_combo_box_toggle_wrap(false);
        if (getApi().handle != 0)
            GTK.gtk_combo_box_text_remove_all(getApi().handle);
        gtk_combo_box_toggle_wrap(true);
    }
    ...
    public void setListVisible(boolean visible) {
        checkWidget();
        if (visible) {
            GTK.gtk_combo_box_popup(getApi().handle);
        } else {
            GTK.gtk_combo_box_popdown(getApi().handle);
        }
    }
    ...
    void setOrientation(boolean create) {
        ...
        if ((getApi().style & SWT.RIGHT_TO_LEFT) != 0 || !create) {
            ...
            if (!create) {
                if (popupHandle != 0)
                    GTK3.gtk_container_forall(popupHandle, ((SwtDisplay) display.getImpl()).setDirectionProc, dir);
            }
        }
    }
	...
	*/
    ...
    void updateCss() {
		 ...
		 */
            final String clrSelectionBack = ((SwtDisplay) display.getImpl()).gtk_rgba_to_css_string(((SwtDisplay) display.getImpl()).COLOR_LIST_SELECTION_RGBA);
            final String clrSelectionFore = ((SwtDisplay) display.getImpl()).gtk_rgba_to_css_string(((SwtDisplay) display.getImpl()).COLOR_LIST_SELECTION_TEXT_RGBA);
        ...
        if (foreground != null) {
            final String colorString = ((SwtDisplay) display.getImpl()).gtk_rgba_to_css_string(foreground);
