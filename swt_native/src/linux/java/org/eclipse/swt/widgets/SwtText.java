...
public class SwtText extends SwtScrollable implements IText {
		 ...
		 */
            ...
            if (!GTK.GTK4) {
                if ((style & SWT.ICON_SEARCH) == 0) {
                    ...
                    GTK.gtk_entry_set_icon_from_icon_name(getApi().handle, GTK.GTK_ENTRY_ICON_PRIMARY, null);
                } else {
                    // Default GtkSearchEntry always shows inactive "find" icon
                    // make it active and sensitive to be consistent with other platforms
                    GTK.gtk_entry_set_icon_activatable(getApi().handle, GTK.GTK_ENTRY_ICON_PRIMARY, true);
                    GTK.gtk_entry_set_icon_sensitive(getApi().handle, GTK.GTK_ENTRY_ICON_PRIMARY, true);
                }
    ...
    void createHandle(int index) {
        ...
        if ((getApi().style & SWT.SINGLE) != 0) {
            if ((getApi().style & SWT.SEARCH) != 0) {
                getApi().handle = GTK.gtk_search_entry_new();
            } else {
                getApi().handle = GTK.gtk_entry_new();
            }
            if (getApi().handle == 0)
                error(SWT.ERROR_NO_HANDLES);
            if (GTK.GTK4) {
                OS.swt_fixed_add(fixedHandle, getApi().handle);
                textHandle = GTK4.gtk_editable_get_delegate(getApi().handle);
                if ((getApi().style & SWT.SEARCH) == 0) {
                    bufferHandle = GTK4.gtk_entry_get_buffer(getApi().handle);
                } else {
                ...
                GTK3.gtk_entry_set_width_chars(getApi().handle, 1);
            }
            GTK.gtk_editable_set_editable(getApi().handle, (getApi().style & SWT.READ_ONLY) == 0);
            if (GTK.GTK4) {
                GTK4.gtk_text_set_visibility(textHandle, (getApi().style & SWT.PASSWORD) == 0);
            } else {
                GTK.gtk_entry_set_visibility(getApi().handle, (getApi().style & SWT.PASSWORD) == 0);
            }
            /*
		 ...
		 */
            ...
            if (scrolledHandle == 0)
                error(SWT.ERROR_NO_HANDLES);
            getApi().handle = GTK.gtk_text_view_new();
            if (getApi().handle == 0)
                error(SWT.ERROR_NO_HANDLES);
            bufferHandle = GTK.gtk_text_view_get_buffer(getApi().handle);
            if (bufferHandle == 0)
                error(SWT.ERROR_NO_HANDLES);
            if (GTK.GTK4) {
            ...
            GTK.gtk_text_view_set_editable(getApi().handle, (getApi().style & SWT.READ_ONLY) == 0);
            if ((getApi().style & SWT.WRAP) != 0)
                GTK.gtk_text_view_set_wrap_mode(getApi().handle, GTK.GTK_WRAP_WORD_CHAR);
            int hsp = (getApi().style & SWT.H_SCROLL) != 0 ? GTK.GTK_POLICY_ALWAYS : GTK.GTK_POLICY_NEVER;
            int vsp = (getApi().style & SWT.V_SCROLL) != 0 ? GTK.GTK_POLICY_ALWAYS : GTK.GTK_POLICY_NEVER;
            ...
            GTK.gtk_text_view_set_justification(getApi().handle, justification);
        }
    ...
    public void append(String string) {
        ...
        clearSegments(true);
        if ((getApi().style & SWT.SINGLE) != 0) {
            GTK.gtk_editable_insert_text(getApi().handle, buffer, buffer.length, new int[] { -1 });
            GTK.gtk_editable_set_position(getApi().handle, -1);
        } else {
            ...
            GTK.gtk_text_view_scroll_to_mark(getApi().handle, mark, 0, true, 0, 0);
        }
        applySegments();
    }
	 ...
	 */
        ...
        if ((getApi().style & SWT.SINGLE) != 0) {
            OS.g_signal_handlers_block_matched(getApi().handle, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, CHANGED);
            OS.g_signal_handlers_block_matched(getApi().handle, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, INSERT_TEXT);
            int limit = GTK.gtk_entry_get_max_length(getApi().handle);
            if (limit != 0)
                GTK.gtk_entry_set_max_length(getApi().handle, translateOffset(limit));
            int[] pos = new int[1];
            for (int i = 0; i < nSegments; i++) {
                ...
                if (GTK.GTK4) {
                    ...
                    ptr = GTK3.gtk_entry_get_text(getApi().handle);
                }
                pos[0] = (int) OS.g_utf16_offset_to_utf8_offset(ptr, pos[0]);
                GTK.gtk_editable_insert_text(getApi().handle, buffer, buffer.length, pos);
            }
            OS.g_signal_handlers_unblock_matched(getApi().handle, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, CHANGED);
            OS.g_signal_handlers_unblock_matched(getApi().handle, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, INSERT_TEXT);
        } else {
    ...
    void clearSegments(boolean applyText) {
        ...
        if ((getApi().style & SWT.SINGLE) != 0) {
            OS.g_signal_handlers_block_matched(getApi().handle, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, CHANGED);
            if (applyText) {
                OS.g_signal_handlers_block_matched(getApi().handle, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, DELETE_TEXT);
                long ptr;
                if (GTK.GTK4) {
                    ptr = GTK.gtk_entry_buffer_get_text(bufferHandle);
                } else {
                ...
                for (int i = 0; i < nSegments; i++) {
                    ...
                    GTK.gtk_editable_delete_text(getApi().handle, start, end);
                }
                OS.g_signal_handlers_unblock_matched(getApi().handle, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, DELETE_TEXT);
            }
            int limit = GTK.gtk_entry_get_max_length(getApi().handle);
            if (limit != 0)
                GTK.gtk_entry_set_max_length(getApi().handle, untranslateOffset(limit));
            OS.g_signal_handlers_unblock_matched(getApi().handle, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, CHANGED);
        } else if (applyText) {
            OS.g_signal_handlers_block_matched(bufferHandle, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, CHANGED);
    ...
    public void clearSelection() {
        checkWidget();
        if ((getApi().style & SWT.SINGLE) != 0) {
            int position = GTK.gtk_editable_get_position(getApi().handle);
            GTK.gtk_editable_select_region(getApi().handle, position, position);
        } else {
            byte[] position = new byte[ITER_SIZEOF];
    ...
    Point computeSizeInPixels(int wHint, int hHint, boolean changed) {
        ...
        if ((getApi().style & SWT.SINGLE) != 0) {
            ...
            long layout = GTK.gtk_widget_create_pango_layout(getApi().handle, text);
            OS.g_free(text);
        ...
        int width = w[0];
        int height = h[0];
        if ((getApi().style & SWT.SINGLE) != 0 && message.length() > 0) {
            byte[] buffer = Converter.wcsToMbcs(message, true);
            long layout = GTK.gtk_widget_create_pango_layout(getApi().handle, buffer);
        ...
        if ((getApi().style & SWT.SEARCH) != 0) {
            ...
            long context = GTK.gtk_widget_get_style_context(getApi().handle);
            int state_flag = GTK.gtk_widget_get_state_flags(getApi().handle);
    ...
    Rectangle computeTrimInPixels(int x, int y, int width, int height) {
        ...
        if ((getApi().style & SWT.SINGLE) != 0) {
            ...
            if ((getApi().style & SWT.BORDER) != 0) {
                int state = GTK.gtk_widget_get_state_flags(getApi().handle);
            ...
            if (!GTK.GTK4 || ((getApi().style & SWT.SEARCH) == 0)) {
                ...
                GTK.gtk_entry_get_icon_area(getApi().handle, GTK.GTK_ENTRY_ICON_PRIMARY, icon_area);
                trim.x -= icon_area.width;
                trim.width += icon_area.width;
                GTK.gtk_entry_get_icon_area(getApi().handle, GTK.GTK_ENTRY_ICON_SECONDARY, icon_area);
                trim.width += icon_area.width;
            }
        } else {
            int borderWidth = gtk_container_get_border_width_or_margin(getApi().handle);
            xborder += borderWidth;
            yborder += borderWidth;
        }
        /*
	 ...
	 */
        if (!GTK.GTK4) {
            ...
            GTK3.gtk_widget_style_get(getApi().handle, OS.interior_focus, property, 0);
            if (property[0] == 0) {
    ...
    public void copy() {
        ...
        if ((getApi().style & SWT.SINGLE) != 0) {
            if (GTK.GTK4) {
                ...
                GTK3.gtk_editable_copy_clipboard(getApi().handle);
            }
        } else {
            long clipboard = GTK.GTK4 ? GTK4.gtk_widget_get_clipboard(getApi().handle) : GTK3.gtk_clipboard_get(GDK.GDK_NONE);
            clearSegments(true);
    ...
    public void cut() {
        ...
        if ((getApi().style & SWT.SINGLE) != 0) {
            if (GTK.GTK4) {
                ...
                GTK3.gtk_editable_cut_clipboard(getApi().handle);
            }
        } else {
            long clipboard = GTK.GTK4 ? GTK4.gtk_widget_get_clipboard(getApi().handle) : GTK3.gtk_clipboard_get(GDK.GDK_NONE);
            clearSegments(true);
            GTK.gtk_text_buffer_cut_clipboard(bufferHandle, clipboard, GTK.gtk_text_view_get_editable(getApi().handle));
            applySegments();
        }
    }
	...
	*/
    ...
    Point getCaretLocationInPixels() {
        ...
        if ((getApi().style & SWT.SINGLE) != 0) {
            ...
            GTK3.gtk_entry_get_layout_offsets(getApi().handle, offset_x, offset_y);
            ...
            GTK.gtk_text_view_get_iter_location(getApi().handle, position, rect);
            int[] x = new int[1];
            int[] y = new int[1];
            GTK.gtk_text_view_buffer_to_window_coords(getApi().handle, GTK.GTK_TEXT_WINDOW_TEXT, rect.x, rect.y, x, y);
    ...
    public int getCaretPosition() {
        ...
        int result;
        if ((getApi().style & SWT.SINGLE) != 0) {
            long ptr = GTK3.gtk_entry_get_text(getApi().handle);
            result = (int) OS.g_utf8_offset_to_utf16_offset(ptr, GTK.gtk_editable_get_position(getApi().handle));
        } else {
            byte[] position = new byte[ITER_SIZEOF];
            long mark = GTK.gtk_text_buffer_get_insert(bufferHandle);
            GTK.gtk_text_buffer_get_iter_at_mark(bufferHandle, position, mark);
    ...
    public int getCharCount() {
        ...
        if ((getApi().style & SWT.SINGLE) != 0) {
            if (GTK.GTK4) {
                result = GTK4.gtk_entry_get_text_length(getApi().handle);
            } else {
                long str = GTK3.gtk_entry_get_text(getApi().handle);
    ...
    public char getEchoChar() {
        checkWidget();
        if ((getApi().style & SWT.SINGLE) != 0) {
            if (!GTK.gtk_entry_get_visibility(getApi().handle)) {
                return GTK.gtk_entry_get_invisible_char(getApi().handle);
            }
        }
        return '\0';
    }
    ...
    public boolean getEditable() {
        checkWidget();
        if ((getApi().style & SWT.SINGLE) != 0) {
            return GTK.gtk_editable_get_editable(getApi().handle);
        }
        return GTK.gtk_text_view_get_editable(getApi().handle);
    }
    ...
    int getPosition(Point point) {
        ...
        if ((getApi().style & SWT.SINGLE) != 0) {
            ...
            GTK.gtk_text_view_get_iter_at_location(getApi().handle, p, point.x, point.y);
            byte[] zero = new byte[ITER_SIZEOF];
            GTK.gtk_text_buffer_get_iter_at_offset(bufferHandle, zero, 0);
    ...
    public Point getSelection() {
        ...
        if ((getApi().style & SWT.SINGLE) != 0) {
            int[] start = new int[1];
            int[] end = new int[1];
            GTK.gtk_editable_get_selection_bounds(getApi().handle, start, end);
            long str;
            if (GTK.GTK4) {
                str = GTK.gtk_entry_buffer_get_text(GTK4.gtk_entry_get_buffer(getApi().handle));
            } else {
                str = GTK3.gtk_entry_get_text(getApi().handle);
            }
	...
	*/
    ...
    public char[] getTextChars() {
        ...
        if ((getApi().style & SWT.SINGLE) != 0) {
            if (GTK.GTK4) {
                ...
                address = GTK3.gtk_entry_get_text(getApi().handle);
            }
        } else {
	 ...
	 */
        long vAdjustment = GTK.gtk_scrollable_get_vadjustment(getApi().handle);
        ...
        GdkRectangle rect = new GdkRectangle();
        GTK.gtk_text_view_get_visible_rect(getApi().handle, rect);
        GTK.gtk_text_view_get_line_at_y(getApi().handle, position, rect.y, null);
        return GTK.gtk_text_iter_get_line(position);
    }
    ...
    int getTopPixelInPixels() {
        ...
        GTK.gtk_text_view_get_line_at_y(getApi().handle, position, rect.y, lineTop);
        return lineTop[0];
    }
	...
	*/
        ...
        if ((getApi().style & SWT.SEARCH) != 0 && !GTK.GTK4) {
            if ((getApi().style & SWT.ICON_CANCEL) == 0) {
                // Default GtkSearchEntry shows "clear" icon when there is text, manually revert this
                // when "cancel" icon style is not set
                GTK.gtk_entry_set_icon_from_icon_name(getApi().handle, GTK.GTK_ENTRY_ICON_SECONDARY, null);
            }
        }
        return 0;
    }
    ...
    long gtk_commit(long imContext, long text) {
        if (text == 0)
            return 0;
        if ((getApi().style & SWT.SINGLE) != 0) {
            if (!GTK.gtk_editable_get_editable(getApi().handle))
                return 0;
        }
	...
	*/
        ...
        OS.g_signal_handlers_unblock_matched(imContext, mask, id, 0, 0, 0, getApi().handle);
        if (newChars == chars) {
        ...
        OS.g_signal_handlers_block_matched(imContext, mask, id, 0, 0, 0, getApi().handle);
        if ((getApi().style & SWT.SINGLE) != 0) {
            if (fixStart != -1 && fixEnd != -1) {
                GTK.gtk_editable_set_position(getApi().handle, fixStart);
                GTK.gtk_editable_select_region(getApi().handle, fixStart, fixEnd);
            }
        }
        fixStart = fixEnd = -1;
        return 0;
    ...
    long gtk_delete_text(long widget, long start_pos, long end_pos) {
        ...
        if (newText == null) {
            ...
            GTK.gtk_editable_get_selection_bounds(getApi().handle, newStart, newEnd);
            if (newStart[0] != newEnd[0]) {
                fixStart = newStart[0];
                fixEnd = newEnd[0];
            }
            OS.g_signal_stop_emission_by_name(getApi().handle, OS.delete_text);
        } else {
            if (newText.length() > 0) {
                int[] pos = new int[1];
                pos[0] = (int) end_pos;
                ...
                OS.g_signal_handlers_unblock_matched(getApi().handle, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, INSERT_TEXT);
                OS.g_signal_handlers_unblock_matched(getApi().handle, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, CHANGED);
                GTK.gtk_editable_set_position(getApi().handle, pos[0]);
                Arrays.fill(buffer, (byte) 0);
            }
        }
        return 0;
	...
	*/
    ...
    long gtk_insert_text(long widget, long new_text, long new_text_length, long position) {
        ...
        if (newText != oldText && getApi().handle != 0) {
            int[] newStart = new int[1], newEnd = new int[1];
            GTK.gtk_editable_get_selection_bounds(getApi().handle, newStart, newEnd);
            if (newText != null) {
                if (newStart[0] != newEnd[0]) {
                    OS.g_signal_handlers_block_matched(getApi().handle, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, DELETE_TEXT);
                    OS.g_signal_handlers_block_matched(getApi().handle, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, CHANGED);
                    GTK.gtk_editable_delete_selection(getApi().handle);
                    OS.g_signal_handlers_unblock_matched(getApi().handle, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, DELETE_TEXT);
                    OS.g_signal_handlers_unblock_matched(getApi().handle, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, CHANGED);
                }
                byte[] buffer3 = Converter.wcsToMbcs(newText, false);
                OS.g_signal_handlers_block_matched(getApi().handle, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, INSERT_TEXT);
                GTK.gtk_editable_insert_text(getApi().handle, buffer3, buffer3.length, pos);
                OS.g_signal_handlers_unblock_matched(getApi().handle, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, INSERT_TEXT);
            ...
            OS.g_signal_stop_emission_by_name(getApi().handle, OS.insert_text);
        }
        return 0;
    }
    ...
    long gtk_populate_popup(long widget, long menu) {
        if ((getApi().style & SWT.RIGHT_TO_LEFT) != 0) {
            ...
            GTK3.gtk_container_forall(menu, ((SwtDisplay) display.getImpl()).setDirectionProc, GTK.GTK_TEXT_DIR_RTL);
        }
        return 0;
    }
    ...
    void hookEvents() {
        super.hookEvents();
        if ((getApi().style & SWT.SINGLE) != 0) {
            OS.g_signal_connect_closure(getApi().handle, OS.changed, ((SwtDisplay) display.getImpl()).getClosure(CHANGED), true);
            OS.g_signal_connect_closure(getApi().handle, OS.insert_text, ((SwtDisplay) display.getImpl()).getClosure(INSERT_TEXT), false);
            OS.g_signal_connect_closure(getApi().handle, OS.delete_text, ((SwtDisplay) display.getImpl()).getClosure(DELETE_TEXT), false);
            OS.g_signal_connect_closure(getApi().handle, OS.activate, ((SwtDisplay) display.getImpl()).getClosure(ACTIVATE), false);
            if (!GTK.GTK4) {
                OS.g_signal_connect_closure(getApi().handle, OS.grab_focus, ((SwtDisplay) display.getImpl()).getClosure(GRAB_FOCUS), false);
            }
            if ((getApi().style & SWT.SEARCH) != 0 && !GTK.GTK4) {
                OS.g_signal_connect_closure(getApi().handle, OS.icon_release, ((SwtDisplay) display.getImpl()).getClosure(ICON_RELEASE), false);
            }
        } else {
            OS.g_signal_connect_closure(bufferHandle, OS.changed, ((SwtDisplay) display.getImpl()).getClosure(CHANGED), false);
            OS.g_signal_connect_closure(bufferHandle, OS.insert_text, ((SwtDisplay) display.getImpl()).getClosure(TEXT_BUFFER_INSERT_TEXT), false);
            OS.g_signal_connect_closure(bufferHandle, OS.delete_range, ((SwtDisplay) display.getImpl()).getClosure(DELETE_RANGE), false);
        }
        if (imContext != 0) {
            OS.g_signal_connect_closure(imContext, OS.commit, ((SwtDisplay) display.getImpl()).getClosure(COMMIT), false);
            int id = OS.g_signal_lookup(OS.commit, GTK.gtk_im_context_get_type());
            int mask = OS.G_SIGNAL_MATCH_DATA | OS.G_SIGNAL_MATCH_ID;
            OS.g_signal_handlers_block_matched(imContext, mask, id, 0, 0, 0, getApi().handle);
        }
        if (!GTK.GTK4) {
            OS.g_signal_connect_closure(getApi().handle, OS.populate_popup, ((SwtDisplay) display.getImpl()).getClosure(POPULATE_POPUP), false);
        }
        // In GTK4, these event signals belong to GtkText which is the only child of GtkEntry
        long eventHandle = 0;
        if (GTK.GTK4) {
            eventHandle = ((getApi().style & SWT.SINGLE) != 0) ? textHandle : getApi().handle;
        } else {
            eventHandle = getApi().handle;
        }
        OS.g_signal_connect_closure(eventHandle, OS.backspace, ((SwtDisplay) display.getImpl()).getClosure(BACKSPACE), false);
        OS.g_signal_connect_closure(eventHandle, OS.backspace, ((SwtDisplay) display.getImpl()).getClosure(BACKSPACE_INVERSE), true);
        OS.g_signal_connect_closure(eventHandle, OS.copy_clipboard, ((SwtDisplay) display.getImpl()).getClosure(COPY_CLIPBOARD), false);
        OS.g_signal_connect_closure(eventHandle, OS.copy_clipboard, ((SwtDisplay) display.getImpl()).getClosure(COPY_CLIPBOARD_INVERSE), true);
        OS.g_signal_connect_closure(eventHandle, OS.cut_clipboard, ((SwtDisplay) display.getImpl()).getClosure(CUT_CLIPBOARD), false);
        OS.g_signal_connect_closure(eventHandle, OS.cut_clipboard, ((SwtDisplay) display.getImpl()).getClosure(CUT_CLIPBOARD_INVERSE), true);
        OS.g_signal_connect_closure(eventHandle, OS.paste_clipboard, ((SwtDisplay) display.getImpl()).getClosure(PASTE_CLIPBOARD), false);
        OS.g_signal_connect_closure(eventHandle, OS.paste_clipboard, ((SwtDisplay) display.getImpl()).getClosure(PASTE_CLIPBOARD_INVERSE), true);
        OS.g_signal_connect_closure(eventHandle, OS.delete_from_cursor, ((SwtDisplay) display.getImpl()).getClosure(DELETE_FROM_CURSOR), false);
        OS.g_signal_connect_closure(eventHandle, OS.delete_from_cursor, ((SwtDisplay) display.getImpl()).getClosure(DELETE_FROM_CURSOR_INVERSE), true);
        OS.g_signal_connect_closure(eventHandle, OS.move_cursor, ((SwtDisplay) display.getImpl()).getClosure(MOVE_CURSOR), false);
        OS.g_signal_connect_closure(eventHandle, OS.move_cursor, ((SwtDisplay) display.getImpl()).getClosure(MOVE_CURSOR_INVERSE), true);
        OS.g_signal_connect_closure(eventHandle, OS.direction_changed, ((SwtDisplay) display.getImpl()).getClosure(DIRECTION_CHANGED), true);
    }
    ...
    public void insert(String string) {
        ...
        if ((getApi().style & SWT.SINGLE) != 0) {
            ...
            GTK.gtk_editable_get_selection_bounds(getApi().handle, start, end);
            GTK.gtk_editable_delete_selection(getApi().handle);
            GTK.gtk_editable_insert_text(getApi().handle, buffer, buffer.length, start);
            GTK.gtk_editable_set_position(getApi().handle, start[0]);
        } else {
            byte[] start = new byte[ITER_SIZEOF];
            byte[] end = new byte[ITER_SIZEOF];
    ...
    private void scrollIfNotVisible(byte[] iter, byte[] scrollTo, boolean insert) {
        ...
        GTK.gtk_text_view_get_line_yrange(getApi().handle, iter, insertionCoordinates, null);
        ...
        if (indexMark != 0) {
            ...
            GTK.gtk_text_view_get_line_yrange(getApi().handle, indexIter, topIndexCoordinates, null);
            ...
            if (distanceTopIndex >= numLinesVisible) {
                if (insert) {
                    ...
                    GTK.gtk_text_view_scroll_to_iter(getApi().handle, scrollTo, 0, true, 0, 0);
                }
            }
        } else {
    ...
    long paintWindow() {
        if ((getApi().style & SWT.SINGLE) != 0) {
            ...
            if (GTK.GTK4) {
                ...
                GTK.gtk_widget_realize(getApi().handle);
                return GTK3.gtk_text_view_get_window(getApi().handle, GTK.GTK_TEXT_WINDOW_TEXT);
            }
        }
    }
    ...
    public void paste() {
        ...
        if ((getApi().style & SWT.SINGLE) != 0) {
            if (GTK.GTK4) {
                ...
                GTK3.gtk_editable_paste_clipboard(getApi().handle);
            }
        } else {
            long clipboard = GTK.GTK4 ? GTK4.gtk_widget_get_clipboard(getApi().handle) : GTK3.gtk_clipboard_get(GDK.GDK_NONE);
            clearSegments(true);
            GTK.gtk_text_buffer_paste_clipboard(bufferHandle, clipboard, null, GTK.gtk_text_view_get_editable(getApi().handle));
            applySegments();
        }
    }
    ...
    public void selectAll() {
        checkWidget();
        if ((getApi().style & SWT.SINGLE) != 0) {
            GTK.gtk_editable_select_region(getApi().handle, 0, -1);
        } else {
            byte[] start = new byte[ITER_SIZEOF];
            byte[] end = new byte[ITER_SIZEOF];
    ...
    GdkRGBA getContextBackgroundGdkRGBA() {
        if (background != null && (getApi().state & BACKGROUND) != 0) {
            return background;
        }
	 ...
	 */
        ...
        String properties;
        if ((getApi().style & SWT.SINGLE) != 0) {
            properties = "entry {background: " + ((SwtDisplay) display.getImpl()).gtk_rgba_to_css_string(background) + ";}\n" + "entry:selected {background-color: " + ((SwtDisplay) display.getImpl()).gtk_rgba_to_css_string(selectedBackground) + ";}\n" + "entry selection {color: " + ((SwtDisplay) display.getImpl()).gtk_rgba_to_css_string(selectedForeground) + ";}";
        } else {
            properties = "textview text {background-color: " + ((SwtDisplay) display.getImpl()).gtk_rgba_to_css_string(background) + ";}\n" + "textview text:selected {background-color: " + ((SwtDisplay) display.getImpl()).gtk_rgba_to_css_string(selectedBackground) + ";}\n" + "textview text selection {color: " + ((SwtDisplay) display.getImpl()).gtk_rgba_to_css_string(selectedForeground) + ";}";
        }
        css = properties;
    ...
    void setForegroundGdkRGBA(GdkRGBA rgba) {
        foreground = rgba;
        GdkRGBA toSet = rgba == null ? ((SwtDisplay) display.getImpl()).COLOR_WIDGET_FOREGROUND_RGBA : rgba;
    ...
    public void setEchoChar(char echo) {
        checkWidget();
        if ((getApi().style & SWT.SINGLE) != 0) {
            GTK.gtk_entry_set_visibility(getApi().handle, echo == '\0');
            GTK.gtk_entry_set_invisible_char(getApi().handle, echo);
        }
    }
    ...
    public void setEditable(boolean editable) {
        ...
        if (!editable)
            getApi().style |= SWT.READ_ONLY;
        if ((getApi().style & SWT.SINGLE) != 0) {
            GTK.gtk_editable_set_editable(getApi().handle, editable);
        } else {
            GTK.gtk_text_view_set_editable(getApi().handle, editable);
        }
    }
    ...
    public void setMessage(String message) {
        ...
        if ((getApi().style & SWT.SINGLE) != 0) {
            ...
            if (GTK.GTK4) {
                ...
                GTK.gtk_entry_set_placeholder_text(getApi().handle, buffer);
            }
            return;
        }
        redraw(false);
    ...
    public void setSelection(int start) {
        ...
        if ((getApi().style & SWT.SINGLE) != 0) {
            ...
            GTK.gtk_editable_set_position(getApi().handle, start);
        } else {
            byte[] startIter = new byte[ITER_SIZEOF];
            byte[] endIter = new byte[ITER_SIZEOF];
            GTK.gtk_text_buffer_get_bounds(bufferHandle, startIter, endIter);
    ...
    public void setSelection(int start, int end) {
        ...
        if ((getApi().style & SWT.SINGLE) != 0) {
            ...
            GTK.gtk_editable_set_position(getApi().handle, start);
            GTK.gtk_editable_select_region(getApi().handle, start, end);
        } else {
            byte[] startIter = new byte[ITER_SIZEOF];
            byte[] endIter = new byte[ITER_SIZEOF];
            GTK.gtk_text_buffer_get_bounds(bufferHandle, startIter, endIter);
    ...
    void setTabStops(int tabs) {
        ...
        if ((getApi().style & SWT.SINGLE) != 0) {
            if (GTK.GTK4) {
                ...
                GTK.gtk_entry_set_tabs(getApi().handle, tabArray);
            }
        } else {
            GTK.gtk_text_view_set_tabs(getApi().handle, tabArray);
        }
	...
	*/
    ...
    void setText(char[] text) {
        ...
        if ((getApi().style & SWT.SINGLE) != 0) {
            ...
            if (GTK.GTK4) {
                ...
                GTK3.gtk_entry_set_text(getApi().handle, buffer);
            }
            OS.g_signal_handlers_unblock_matched(getApi().handle, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, CHANGED);
            OS.g_signal_handlers_unblock_matched(getApi().handle, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, DELETE_TEXT);
            OS.g_signal_handlers_unblock_matched(getApi().handle, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, INSERT_TEXT);
    ...
    public void setTextLimit(int limit) {
        ...
        if ((getApi().style & SWT.SINGLE) != 0) {
            GTK.gtk_entry_set_max_length(getApi().handle, segments != null ? Math.min(Text.LIMIT, translateOffset(limit)) : limit);
        }
    }
	 ...
	 */
        ...
        GTK.gtk_text_view_scroll_to_mark(getApi().handle, indexMark, 0, true, 0, 0);
        long vAdjustment = GTK.gtk_scrollable_get_vadjustment(getApi().handle);
