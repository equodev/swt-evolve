 ...
 */
public class SwtTrayItem extends SwtItem implements ITrayItem {
    ...
    void createWidget(int index) {
        ...
        ((SwtTray) parent.getImpl()).createItem(this.getApi(), index);
    }

    @Override
    void createHandle(int index) {
        getApi().state |= HANDLE;
        getApi().handle = GTK3.gtk_status_icon_new();
        if (getApi().handle == 0)
            error(SWT.ERROR_NO_HANDLES);
        imageHandle = GTK.gtk_image_new();
        GTK3.gtk_status_icon_set_visible(getApi().handle, true);
    }
	...
	*/
        ...
        ((SwtDisplay) display.getImpl()).currentTrayItem = null;
        return 0;
    }
    ...
    void hookEvents() {
        ...
        OS.g_signal_connect_closure(getApi().handle, OS.popup_menu, ((SwtDisplay) display.getImpl()).getClosure(STATUS_ICON_POPUP_MENU), false);
    }
    ...
    public boolean getVisible() {
        checkWidget();
        return GTK3.gtk_status_icon_get_visible(getApi().handle);
    }
    ...
    public void setImage(Image image) {
        ...
        if (image != null) {
            ...
            GTK3.gtk_status_icon_set_from_pixbuf(getApi().handle, pixbuf);
            GTK3.gtk_status_icon_set_visible(getApi().handle, true);
        } else {
            GTK.gtk_widget_set_size_request(getApi().handle, 1, 1);
            GTK3.gtk_status_icon_set_from_pixbuf(getApi().handle, 0);
            GTK3.gtk_status_icon_set_visible(getApi().handle, false);
        }
    }
    ...
    public void setToolTipText(String string) {
        ...
        GTK3.gtk_status_icon_set_tooltip_text(getApi().handle, buffer);
    }
    ...
    public void setVisible(boolean visible) {
        checkWidget();
        if (GTK3.gtk_status_icon_get_visible(getApi().handle) == visible)
            return;
        if (visible) {
            /*
		...
		*/
            ...
            if (isDisposed())
                return;
            GTK3.gtk_status_icon_set_visible(getApi().handle, visible);
        } else {
            GTK3.gtk_status_icon_set_visible(getApi().handle, visible);
            sendEvent(SWT.Hide);
        }
