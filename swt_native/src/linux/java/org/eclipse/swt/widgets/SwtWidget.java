	...
	*/
    ...
    long dpiChanged(long object, long arg0) {
        ...
        if (oldScaleFactor != newScaleFactor) {
            ((SwtDisplay) display.getImpl()).dpiChanged(newScaleFactor);
 ...
 */
    ...
    void hookEvents() {
        if (getApi().handle != 0) {
            OS.g_signal_connect(getApi().handle, OS.dpi_changed, ((SwtDisplay) display.getImpl()).notifyProc, SwtWidget.DPI_CHANGED);
        }
    }
 ...
 */
    ...
    char[] sendIMKeyEvent(int type, long event, char[] chars) {
        ...
        long ptr = 0;
        if (event == 0) {
            long controller = SwtControl.getControl(this.getApi().handle).getImpl()._keyController();
    ...
    boolean setKeyState(Event javaEvent, long event) {
        ...
        switch(eventKeyval[0]) {
            case GDK.GDK_BackSpace:
                javaEvent.character = SWT.BS;
                break;
            case GDK.GDK_Linefeed:
                javaEvent.character = SWT.LF;
                break;
            case GDK.GDK_KP_Enter:
            case GDK.GDK_Return:
                javaEvent.character = SWT.CR;
                break;
            case GDK.GDK_KP_Delete:
            case GDK.GDK_Delete:
                javaEvent.character = SWT.DEL;
                break;
            case GDK.GDK_Escape:
                javaEvent.character = SWT.ESC;
                break;
            case GDK.GDK_Tab:
            case GDK.GDK_ISO_Left_Tab:
                javaEvent.character = SWT.TAB;
                break;
            default:
                {
                    if (javaEvent.keyCode == 0) {
                        ...
                        Map<Integer, Integer> groupLatinKeysCount = ((SwtDisplay) display.getImpl()).getGroupKeysCount();
                        if (!groupLatinKeysCount.containsKey(group)) {
                            group = ((SwtDisplay) display.getImpl()).getLatinKeyGroup();
                        }
                        long keymap = 0;
    ...
    int gdk_pointer_grab(long gdkResource, int grab_ownership, boolean owner_events, int event_mask, long confine_to, long cursor, int time_) {
        ...
        if (gdkSeatGrabPrepareFunc == null) {
            //$NON-NLS-1$
            gdkSeatGrabPrepareFunc = new Callback(SwtWidget.class, "GdkSeatGrabPrepareFunc", 3);
        }
    ...
    long windowProc(long handle, long user_data) {
        switch((int) user_data) {
            ...
            case CLOSE_REQUEST:
                return gtk_close_request(handle);
            case CREATE_MENU_PROXY:
                return gtk_create_menu_proxy(handle);
            case DAY_SELECTED:
                return gtk_day_selected(handle);
            case DAY_SELECTED_DOUBLE_CLICK:
            ...
            case POPUP_MENU:
                return gtk3_popup_menu(handle);
            case PREEDIT_CHANGED:
                return gtk_preedit_changed(handle);
            case REALIZE:
                return gtk_realize(handle);
            case START_INTERACTIVE_SEARCH:
                return gtk_start_interactive_search(handle);
            case STYLE_UPDATED:
                return gtk_style_updated(handle);
            case SELECT:
                return gtk_select(handle);
            case SELECTION_DONE:
                return gtk_selection_done(handle);
            case SHOW:
    ...
    long windowProc(long handle, long arg0, long user_data) {
        switch((int) user_data) {
            ...
            case SHOW_HELP:
                return gtk3_show_help(handle, arg0);
            case SIZE_ALLOCATE:
    ...
    long windowProc(long handle, long arg0, long arg1, long user_data) {
        switch((int) user_data) {
            ...
            case ROW_HAS_CHILD_TOGGLED:
                return gtk_row_has_child_toggled(handle, arg0, arg1);
            default:
                return 0;
        }
    }
    ...
    long windowProc(long handle, long arg0, long arg1, long arg2, long arg3, long user_data) {
        switch((int) user_data) {
            case POPPED_UP:
                return gtk_menu_popped_up(handle, arg0, arg1, arg2, arg3);
            default:
                return 0;
        }
    }
