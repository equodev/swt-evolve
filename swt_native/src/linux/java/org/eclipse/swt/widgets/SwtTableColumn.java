...
public class SwtTableColumn extends SwtItem implements ITableColumn {
    ...
    public boolean getMoveable() {
        checkWidget();
        return GTK.gtk_tree_view_column_get_reorderable(getApi().handle);
    }
    ...
    public boolean getResizable() {
        checkWidget();
        return GTK.gtk_tree_view_column_get_resizable(getApi().handle);
    }
    ...
    int getWidthInPixels() {
        ...
        if (useFixedWidth)
            return GTK.gtk_tree_view_column_get_fixed_width(getApi().handle);
        return GTK.gtk_tree_view_column_get_width(getApi().handle);
    }
	...
	*/
    ...
    void hookEvents() {
        ...
        if (buttonHandle != 0) {
            if (GTK.GTK4) {
                ...
                OS.g_signal_connect_closure_by_id(buttonHandle, ((SwtDisplay) display.getImpl()).signalIds[SIZE_ALLOCATE], 0, ((SwtDisplay) display.getImpl()).getClosure(SIZE_ALLOCATE), false);
                OS.g_signal_connect_closure_by_id(buttonHandle, ((SwtDisplay) display.getImpl()).signalIds[EVENT_AFTER], 0, ((SwtDisplay) display.getImpl()).getClosure(EVENT_AFTER), false);
            }
        }
        if (labelHandle != 0)
    ...
    public void pack() {
        ...
        if ((parent.style & SWT.VIRTUAL) != 0) {
            ...
            for (int i = 0; i < ((SwtTable) parent.getImpl()).items.length; i++) {
                TableItem item = ((SwtTable) parent.getImpl()).items[i];
                ...
                if (item != null && ((SwtTableItem) item.getImpl()).cached && (isVisible || calcWidth)) {
                    width = Math.max(width, ((SwtTable) parent.getImpl()).calculateWidth(getApi().handle, item.handle));
                }
            }
        } else {
            long iter = OS.g_malloc(GTK.GtkTreeIter_sizeof());
            if (GTK.gtk_tree_model_get_iter_first(((SwtTable) parent.getImpl()).modelHandle, iter)) {
                do {
                    width = Math.max(width, ((SwtTable) parent.getImpl()).calculateWidth(getApi().handle, iter));
                } while (GTK.gtk_tree_model_iter_next(((SwtTable) parent.getImpl()).modelHandle, iter));
            }
            OS.g_free(iter);
        }
    ...
    public void setAlignment(int alignment) {
        ...
        ((SwtTable) parent.getImpl()).createRenderers(getApi().handle, modelIndex, index == 0, getApi().style);
    }
    ...
    public void setImage(Image image) {
        ...
        if (image != null) {
            ImageList headerImageList = ((SwtTable) parent.getImpl()).headerImageList;
            if (headerImageList == null) {
                headerImageList = ((SwtTable) parent.getImpl()).headerImageList = new ImageList();
            }
    ...
    public void setResizable(boolean resizable) {
        checkWidget();
        GTK.gtk_tree_view_column_set_resizable(getApi().handle, resizable);
    }
    ...
    public void setMoveable(boolean moveable) {
        checkWidget();
        GTK.gtk_tree_view_column_set_reorderable(getApi().handle, moveable);
    }
    ...
    void setOrientation(boolean create) {
        if ((parent.style & SWT.RIGHT_TO_LEFT) != 0 || !create) {
            if (buttonHandle != 0) {
                ...
                GTK3.gtk_container_forall(buttonHandle, ((SwtDisplay) display.getImpl()).setDirectionProc, dir);
            }
        }
    }
    ...
    void setWidthInPixels(int width) {
        ...
        if (width > 0) {
            useFixedWidth = true;
            GTK.gtk_tree_view_column_set_fixed_width(getApi().handle, width);
        }
        /*
	 ...
	 */
        ...
        GTK.gtk_tree_view_column_set_visible(getApi().handle, width != 0);
        lastWidth = width;
        /*
