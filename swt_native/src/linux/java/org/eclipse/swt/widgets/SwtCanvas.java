...
public class SwtCanvas extends SwtComposite implements ICanvas {
    ...
    long gtk_commit(long imcontext, long text) {
        if (ime != null) {
            long result = ((SwtIME) ime.getImpl()).gtk_commit(imcontext, text);
            if (result != 0)
                return result;
        }
    ...
    private void drawCaret(long widget, long cairo) {
        ...
        if (drawFlag) {
            ...
            if (caret.getImpl()._image() != null && !caret.getImpl()._image().isDisposed() && caret.getImpl()._image().mask == 0) {
                ...
                Cairo.cairo_translate(cairo, nX, caret.getImpl()._y());
                ...
                if (nWidth <= 0)
                    nWidth = SwtCaret.DEFAULT_WIDTH;
                int nX = caret.getImpl()._x();
                if ((getApi().style & SWT.MIRRORED) != 0)
                    nX = getClientWidth() - nWidth - nX;
                Cairo.cairo_rectangle(cairo, nX, caret.getImpl()._y(), nWidth, nHeight);
            }
    ...
    long gtk_preedit_changed(long imcontext) {
        if (ime != null) {
            long result = ((SwtIME) ime.getImpl()).gtk_preedit_changed(imcontext);
            if (result != 0)
                return result;
        }
    ...
    void updateCaret() {
        ...
        rect.x = caret.getImpl()._x();
        rect.y = caret.getImpl()._y();
