...
public class SWTComposite extends SWTScrollable implements IComposite {
     ...
     */
    Map<SWTControl, long[]> fixClipMap = new HashMap<>();
     ...
     * See bug 535978.
     */
    HashMap<SWTWidget, Boolean> childrenLowered = new HashMap<>();
     ...
     * @see Widget#getStyle
     */
    public SWTComposite(SWTComposite parent, int style) {
    ...
    IControl[] _getChildren() {
        ...
        if (GTK.GTK4) {
            ...
            for (long child = GTK4.gtk_widget_get_first_child(parentHandle); child != 0; child = GTK4.gtk_widget_get_next_sibling(child)) {
                SWTWidget childWidget = (SWTWidget) (display.getWidget(child));
                if (childWidget != null && childWidget instanceof SWTControl && childWidget != this) {
                    childrenList.add((SWTControl) childWidget);
                }
            }
            return childrenList.toArray(new SWTControl[childrenList.size()]);
        } else {
    ...
    Point computeSizeInPixels(int wHint, int hHint, boolean changed) {
        ...
        if (layout != null) {
            if (wHint == SWT.DEFAULT || hHint == SWT.DEFAULT) {
                ...
                size = DPIUtil.autoScaleUp(layout.computeSize(Composite.getInstance(this), DPIUtil.autoScaleDown(wHint), DPIUtil.autoScaleDown(hHint), changed));
        ...
        if ((style & SWT.EMBEDDED) != 0) {
            if (OS.isWayland()) {
                ...
                if (GTK.GTK4) {
                    ...
                    if (Device.DEBUG) {
                        new SWTError(SWT.ERROR_INVALID_ARGUMENT, "SWT.EMBEDDED is not supported for GTK >= 4.").printStackTrace();
                    }
                } else {
    ...
    SWTMenu[] findMenus(SWTControl control) {
        ...
        SWTMenu[] result = (SWTMenu[]) (super.findMenus(control));
        IControl[] children = _getChildren();
        for (int i = 0; i < children.length; i++) {
            if (children[i] instanceof SWTControl child) {
                SWTMenu[] menuList = (SWTMenu[]) (child.findMenus(control));
                if (menuList.length != 0) {
                    SWTMenu[] newResult = new SWTMenu[result.length + menuList.length];
                    System.arraycopy(result, 0, newResult, 0, result.length);
    ...
    void fixModal(long group, long modalGroup) {
        IControl[] controls = _getChildren();
        for (int i = 0; i < controls.length; i++) {
            if (controls[i] instanceof SWTControl child) {
                child.fixModal(group, modalGroup);
            }
        }
    }
    ...
    void fixTabList(SWTControl control) {
        if (tabList == null)
            return;
        int count = 0;
        for (int i = 0; i < tabList.length; i++) {
     ...
     * @see #setTabList
     */
    public IControl[] getTabList() {
        checkWidget();
        SWTControl[] tabList = (SWTControl[]) (_getTabList());
        if (tabList == null) {
            int count = 0;
            IControl[] list = _getChildren();
            for (int i = 0; i < list.length; i++) {
                if (list[i] instanceof SWTControl child) {
                    if (child.isTabGroup())
                        count++;
	...
	*/
    ...
    public void layout(IControl[] changed, int flags) {
        ...
        if (changed != null) {
            ...
            for (int i = 0; i < changed.length; i++) {
                IControl child = (IControl) (changed[i]);
                IComposite composite = child.getParent();
                if (child instanceof SWTControl swtChild) {
                    // Update layout when the list of children has changed.
                    // See bug 497812.
                    swtChild.markLayout(false, false);
                }
                while (child != this) {
                    if (composite instanceof SWTComposite swtComposite) {
                        if (swtComposite.layout != null) {
                            swtComposite.state |= LAYOUT_NEEDED;
                            if (!swtComposite.layout.flushCache(Control.getInstance(child))) {
                                swtComposite.state |= LAYOUT_CHANGED;
                            }
                        }
                    }
    ...
    void moveChildren(int oldWidth) {
        IControl[] children = _getChildren();
        for (int i = 0; i < children.length; i++) {
            if (!(children[i] instanceof SWTControl)) continue;
		...
		*/
            ...
            SWTControl control = (SWTControl) (child.findBackgroundControl());
        ...
        if (container == fixedHandle) {
            if (GTK.GTK4) {
                ...
                while (temp != 0) {
                    ...
                    if (child != 0) {
                        SWTWidget widget = (SWTWidget) (display.getWidget(child));
                        if (widget != this) {
                            if (noChildDrawing) {
	 ...
	 */
        ...
        if (fixedHandle != 0 && handle != 0 && getVisible() && //if SWT State is not HIDDEN, but widget is hidden on GTK side.
    ...
    @Override
    boolean setScrollBarVisible(SWTScrollBar bar, boolean visible) {
     ...
     * </ul>
     */
    public void setTabList(IControl[] tabList_) {
        SWTControl[] tabList = (SWTControl[]) tabList_;
        checkWidget();
        if (tabList != null) {
            for (int i = 0; i < tabList.length; i++) {
                SWTControl control = (SWTControl) (tabList[i]);
                if (control == null)
                    error(SWT.ERROR_INVALID_ARGUMENT);
                if (control.isDisposed())
                    error(SWT.ERROR_INVALID_ARGUMENT);
            ...
            SWTControl[] newList = new SWTControl[tabList.length];
    ...
    void updateLayout(boolean all) {
        SWTComposite parent = (SWTComposite) (findDeferredControl());
        if (parent != null) {
        ...
        if ((state & LAYOUT_NEEDED) != 0) {
            ...
            display.runSkin();
            layout.layout(Composite.getInstance(this), changed);
        }
        if (all) {
            state &= ~LAYOUT_CHILD;
            IControl[] children = _getChildren();
