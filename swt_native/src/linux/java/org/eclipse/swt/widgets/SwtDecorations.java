...
public class SwtDecorations extends SwtCanvas implements IDecorations {
    ...
    void destroyAccelGroup() {
        if (accelGroup == 0)
            return;
        if (menuBar != null)
            ((SwtMenu) menuBar.getImpl()).removeAccelerators(accelGroup);
        long shellHandle = topHandle();
    ...
    void setOrientation(boolean create) {
        ...
        if (!create) {
            if (menuBar != null)
                ((SwtMenu) menuBar.getImpl())._setOrientation(getApi().style & (SWT.RIGHT_TO_LEFT | SWT.LEFT_TO_RIGHT));
        }
    }
	...
	*/
        if (!button.isVisible() || !button.isEnabled())
            return true;
        long shellHandle = ((SwtShell) _getShell().getImpl()).topHandle();
        if (GTK.GTK4) {
