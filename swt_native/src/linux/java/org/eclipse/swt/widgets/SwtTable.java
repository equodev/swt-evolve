...
public class SwtTable extends SwtComposite implements ITable {
	...
	*/
    ...
    long cellDataProc(long tree_column, long cell, long tree_model, long iter, long data) {
        ...
        if (columnCount == 0) {
            modelIndex = SwtTable.FIRST_COLUMN;
            customDraw = firstCustomDraw;
        } else {
            TableColumn column = (TableColumn) ((SwtDisplay) display.getImpl()).getWidget(tree_column);
            if (column != null) {
                modelIndex = ((SwtTableColumn) column.getImpl()).modelIndex;
                customDraw = ((SwtTableColumn) column.getImpl()).customDraw;
            }
        }
        if (modelIndex == -1)
            return 0;
    ...
    boolean checkData(TableItem item) {
        ...
        if ((getApi().style & SWT.VIRTUAL) != 0) {
            ...
            currentItem = item;
            ((SwtTableItem) item.getImpl()).settingData = true;
            sendEvent(SWT.SetData, event);
            ((SwtTableItem) item.getImpl()).settingData = false;
	 ...
	 */
        if (firstCompute) {
            for (int x = 0; x < columns.length; x++) {
                ...
                if (column != null)
                    GTK.gtk_widget_set_visible(((SwtTableColumn) column.getImpl()).buttonHandle, true);
            }
            firstCompute = false;
        }
    ...
    void createColumn(TableColumn column, int index) {
        ...
        if (columnCount != 0) {
            ...
            for (int i = 0; i < columnCount; i++) {
                int columnIndex = ((SwtTableColumn) columns[i].getImpl()).modelIndex;
			 ...
			 */
                GTK.gtk_tree_view_set_model(getApi().handle, 0);
        ...
        if (index == 0 && columnCount > 0) {
            ...
            createRenderers(checkColumn.handle, ((SwtTableColumn) checkColumn.getImpl()).modelIndex, false, checkColumn.style);
        }
	...
	*/
        ...
        if (column != null) {
            ...
            ((SwtTableColumn) column.getImpl()).modelIndex = modelIndex;
        }
        if (!searchEnabled()) {
            GTK.gtk_tree_view_set_search_column(getApi().handle, -1);
        } else {
            /* Set the search column whenever the model changes */
            int firstColumn = columnCount == 0 ? FIRST_COLUMN : ((SwtTableColumn) columns[0].getImpl()).modelIndex;
    ...
    void createItem(TableColumn column, int index) {
        ...
        if (columnCount == 0) {
            ...
            ((SwtTableColumn) column.getImpl()).modelIndex = FIRST_COLUMN;
            createRenderers(column.handle, ((SwtTableColumn) column.getImpl()).modelIndex, true, column.style);
            ((SwtTableColumn) column.getImpl()).customDraw = firstCustomDraw;
        ...
        ((SwtTableColumn) column.getImpl()).buttonHandle = GTK.gtk_tree_view_column_get_button(column.handle);
        GTK.gtk_widget_set_focus_on_click(((SwtTableColumn) column.getImpl()).buttonHandle, false);
	...
	*/
    ...
    void createRenderers(long columnHandle, int modelIndex, boolean check, int columnStyle) {
        ...
        long textRenderer = ownerDraw ? OS.g_object_new(((SwtDisplay) display.getImpl()).gtk_cell_renderer_text_get_type(), 0) : GTK.gtk_cell_renderer_text_new();
        if (textRenderer == 0)
            error(SWT.ERROR_NO_HANDLES);
        if (ownerDraw) {
	...
	*/
        ...
        if (columnCount != 0) {
            for (int i = 0; i < columnCount; i++) {
                if (columns[i].handle == columnHandle) {
                    customDraw = ((SwtTableColumn) columns[i].getImpl()).customDraw;
                    break;
                }
            }
        }
    ...
    void destroyItem(TableColumn column) {
        ...
        if (columnCount == 1) {
            firstCustomDraw = ((SwtTableColumn) column.getImpl()).customDraw;
        }
		 ...
		 */
            GTK.gtk_tree_view_set_model(getApi().handle, 0);
            copyModel(oldModel, ((SwtTableColumn) column.getImpl()).modelIndex, newModel, FIRST_COLUMN, FIRST_COLUMN + CELL_TYPES);
            GTK.gtk_tree_view_set_model(getApi().handle, newModel);
            setModel(newModel);
            createColumn(null, 0);
        } else {
            for (int i = 0; i < itemCount; i++) {
                ...
                if (item != null) {
                    ...
                    int modelIndex = ((SwtTableColumn) column.getImpl()).modelIndex;
            ...
            if (index == 0) {
                TableColumn checkColumn = columns[0];
                createRenderers(checkColumn.handle, ((SwtTableColumn) checkColumn.getImpl()).modelIndex, true, checkColumn.style);
            }
        }
        if (!searchEnabled()) {
            GTK.gtk_tree_view_set_search_column(getApi().handle, -1);
	 ...
	 */
        ...
        if (!GTK.gtk_tree_view_get_path_at_pos(getApi().handle, point.x, y, path, null, null, null))
            return null;
        if (path[0] == 0)
            return null;
        long indices = GTK.gtk_tree_path_get_indices(path[0]);
	...
	*/
    ...
    boolean mnemonicHit(char key) {
        for (int i = 0; i < columnCount; i++) {
            long labelHandle = ((SwtTableColumn) columns[i].getImpl()).labelHandle;
	 ...
	 */
        ...
        if (headerVisible && noChildDrawing && wasScrolled) {
            for (TableColumn column : columns) {
                if (column != null) {
                    GTK.gtk_widget_queue_draw(((SwtTableColumn) column.getImpl()).buttonHandle);
                }
            }
            wasScrolled = false;
        }
    ...
    void recreateRenderers() {
        ...
        if (columnCount == 0) {
            createRenderers(GTK.gtk_tree_view_get_column(getApi().handle, 0), SwtTable.FIRST_COLUMN, true, 0);
        } else {
            for (int i = 0; i < columnCount; i++) {
                TableColumn column = columns[i];
                createRenderers(column.handle, ((SwtTableColumn) column.getImpl()).modelIndex, i == 0, column.style);
            }
        }
    }
	 ...
	 */
        long selectionHandle = GTK.gtk_tree_view_get_selection(getApi().handle);
        boolean changeMode = (getApi().style & SWT.MULTI) != 0;
        if (changeMode)
    ...
    void rendererRender(long cell, long cr, long snapshot, long widget, long background_area, long cell_area, long expose_area, long flags) {
        ...
        if (item != null) {
            if (GTK.GTK_IS_CELL_RENDERER_TOGGLE(cell) || (columnIndex != 0 || (getApi().style & SWT.CHECK) == 0)) {
                ...
                long[] ptr = new long[1];
                GTK.gtk_tree_model_get(modelHandle, item.handle, SwtTable.BACKGROUND_COLUMN, ptr, -1);
                if (ptr[0] == 0) {
                    int modelIndex = columnCount == 0 ? SwtTable.FIRST_COLUMN : ((SwtTableColumn) columns[columnIndex].getImpl()).modelIndex;
                    GTK.gtk_tree_model_get(modelHandle, item.handle, modelIndex + SwtTable.CELL_BACKGROUND, ptr, -1);
                }
                if (ptr[0] != 0) {
            ...
            if (GTK.GTK_IS_CELL_RENDERER_TEXT(cell)) {
                if (hooks(SWT.PaintItem)) {
                    ...
                    if ((drawState & SWT.SELECTED) != 0) {
                        ...
                        if (GTK.gtk_widget_has_focus(getApi().handle)) {
						 ...
						 */
                            background = SwtColor.gtk_new(display, ((SwtDisplay) display.getImpl()).COLOR_LIST_SELECTION_INACTIVE_RGBA);
                            foreground = SwtColor.gtk_new(display, ((SwtDisplay) display.getImpl()).COLOR_LIST_SELECTION_TEXT_INACTIVE_RGBA);
                        }
                        gc.setBackground(background);
                        gc.setForeground(foreground);
                    } else {
                        gc.setBackground(item.getBackground(columnIndex));
                        Color foreground = drawForegroundRGBA != null ? SwtColor.gtk_new(display, drawForegroundRGBA) : item.getForeground(columnIndex);
                        gc.setForeground(foreground);
                    }
                    gc.setFont(item.getFont(columnIndex));
                    if ((getApi().style & SWT.MIRRORED) != 0)
    ...
    void resetCustomDraw() {
        ...
        for (int i = 0; i < end; i++) {
            boolean customDraw = columnCount != 0 ? ((SwtTableColumn) columns[i].getImpl()).customDraw : firstCustomDraw;
            if (customDraw) {
                long column = GTK.gtk_tree_view_get_column(getApi().handle, i);
                long textRenderer = getTextRenderer(column);
                GTK.gtk_tree_view_column_set_cell_data_func(column, textRenderer, 0, 0, 0);
                if (columnCount != 0)
                    ((SwtTableColumn) columns[i].getImpl()).customDraw = false;
            }
        }
        firstCustomDraw = false;
    }
	...
	*/
    ...
    void setFontDescription(long font) {
        ...
        for (int i = 0; i < columns.length; i++) {
            if (columns[i] != null) {
                ((SwtTableColumn) columns[i].getImpl()).setFontDescription(font);
            }
        }
    }
    ...
    void updateHeaderCSS() {
        ...
        if (columnCount == 0) {
            ...
            for (TableColumn column : columns) {
                if (column != null) {
                    ((SwtTableColumn) column.getImpl()).setHeaderCSS(css.toString());
                }
            }
        }
    }
    ...
    void setOrientation(boolean create) {
        ...
        for (int i = 0; i < columnCount; i++) {
            if (columns[i] != null)
                ((SwtTableColumn) columns[i].getImpl()).setOrientation(create);
        }
    }
	 ...
	 */
        ...
        GTK.gtk_tree_view_scroll_to_cell(getApi().handle, path, 0, true, 0f, 0f);
        GTK.gtk_tree_path_free(path);
    }
	...
	*/
    ...
    public void showItem(TableItem item) {
        ...
        if (item.isDisposed())
            error(SWT.ERROR_INVALID_ARGUMENT);
        if (((SwtTableItem) item.getImpl()).parent != this.getApi())
            return;
    ...
    void showItem(long iter) {
        long path = GTK.gtk_tree_model_get_path(modelHandle, iter);
        GTK.gtk_tree_view_scroll_to_cell(getApi().handle, path, 0, false, 0, 0);
        GTK.gtk_tree_path_free(path);
    }
    ...
    long windowProc(long handle, long arg0, long user_data) {
        switch((int) user_data) {
			 ...
			 * event.
			 */
                    if (itemCount == 0 && (getApi().state & OBSCURED) == 0 && !GTK.GTK4) {
                        if ((getApi().state & PARENT_BACKGROUND) != 0 || backgroundImage != null) {
                            Control control = findBackgroundControl();
                            if (control != null) {
	 ...
	 */
        for (int i = start; i < end; i++) {
            TableItem item = items[i];
            if (item != null && ((SwtTableItem) item.getImpl()).settingData) {
