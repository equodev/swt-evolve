...
public class SWTTracker extends SWTWidget implements ITracker {
     ...
     * @see Widget#getStyle
     */
    public SWTTracker(SWTComposite parent, int style) {
     ...
     * @see SWT#RESIZE
     */
    public SWTTracker(SWTDisplay display, int style) {
        if (display == null)
	...
	*/
        ...
        SWTTracker oldTracker = display.tracker;
    ...
    void update() {
        if (parent != null) {
            if (parent.isDisposed())
                return;
            ((SWTShell) (parent.getShell())).update();
        } else {
            display.update();
        }
    }
