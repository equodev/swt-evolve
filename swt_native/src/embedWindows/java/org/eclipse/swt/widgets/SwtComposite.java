...
public class SwtComposite extends SwtScrollable implements IComposite {
    ...
    public Control[] _getChildren() {
        ...
        int index = 0;
        hwndChild = OS.GetWindow(getApi().handle, OS.GW_CHILD);
        while (hwndChild != 0) {
            Control control = ((SwtDisplay) display.getImpl()).getControl(hwndChild);
    ...
    void copyArea(GC gc, int x, int y, int width, int height) {
        ...
        OS.MapWindowPoints(getApi().handle, hwndParent, lpPoint, 1);
        RECT rect = new RECT();
        ...
        //NECESSARY?
        OS.RedrawWindow(getApi().handle, null, 0, OS.RDW_UPDATENOW | OS.RDW_ALLCHILDREN);
        //0x00000001);
        OS.PrintWindow(getApi().handle, hDC, 0);
        if ((bits & OS.WS_VISIBLE) == 0) {
    ...
    void createHandle() {
        super.createHandle();
        getApi().state |= CANVAS;
        if ((getApi().style & (SWT.H_SCROLL | SWT.V_SCROLL)) == 0 || findThemeControl() == parent) {
	...
	*/
    ...
    boolean redrawChildren() {
        if (!super.redrawChildren())
            return false;
        for (Control element : _getChildren()) {
            if (element.getImpl() instanceof SwtControl) {
                SwtControl swtControl = (SwtControl) element.getImpl();
                swtControl.redrawChildren();
            } else if (element.getImpl() instanceof DartControl) {
    ...
    void resizeEmbeddedHandle(long embeddedHandle, int width, int height) {
        ...
        if (threadId != OS.GetCurrentThreadId()) {
            if (processID[0] == OS.GetCurrentProcessId()) {
                if (((SwtDisplay) display.getImpl()).msgHook == 0) {
                    ((SwtDisplay) display.getImpl()).getMsgCallback = new Callback(display, "getMsgProc", 3);
                    ((SwtDisplay) display.getImpl()).getMsgProc = ((SwtDisplay) display.getImpl()).getMsgCallback.getAddress();
                    ((SwtDisplay) display.getImpl()).msgHook = OS.SetWindowsHookEx(OS.WH_GETMESSAGE, ((SwtDisplay) display.getImpl()).getMsgProc, OS.GetLibraryHandle(), threadId);
	 ...
	 */
        ...
        while (i-- > 0) {
            if (children[i] != null && !children[i].isDisposed()) {
                ((SwtControl) children[i].getImpl()).updateTextDirection(textDirection);
            }
        }
        /*
        ...
        if ((hdr.uFlags & OS.TTF_IDISHWND) == 0) {
			...
			*/
                if (string.length() > TOOLTIP_LIMIT / 4) {
                    string = ((SwtDisplay) display.getImpl()).wrapText(string, getApi().handle, ((SwtToolTip) toolTip.getImpl()).getWidth());
                }
            }
            return string;
        }
        ((SwtShell) shell.getImpl()).setToolTipTitle(hdr.hwndFrom, null, 0);
        OS.SendMessage(hdr.hwndFrom, OS.TTM_SETMAXTIPWIDTH, 0, 0x7FFF);
        Control control = ((SwtDisplay) display.getImpl()).getControl(hdr.idFrom);
        return control != null ? control.getImpl()._toolTipText() : null;
    }
    ...
    void updateBackgroundColor() {
        ...
        for (Control element : _getChildren()) {
            if ((element.state & PARENT_BACKGROUND) != 0) {
                ((SwtControl) element.getImpl()).updateBackgroundColor();
            }
        }
    }
    ...
    void updateBackgroundImage() {
        ...
        for (Control element : _getChildren()) {
            if ((element.state & PARENT_BACKGROUND) != 0) {
                ((SwtControl) element.getImpl()).updateBackgroundImage();
            }
        }
    }
    ...
    void updateFont(Font oldFont, Font newFont) {
        ...
        for (Control control : _getChildren()) {
            if (!control.isDisposed()) {
                ((SwtControl) control.getImpl()).updateFont(oldFont, newFont);
            }
        }
    }
    ...
    LRESULT WM_ERASEBKGND(long wParam, long lParam) {
        ...
        if ((getApi().state & CANVAS) != 0) {
            ...
            if ((getApi().style & (SWT.NO_BACKGROUND | SWT.TRANSPARENT)) != 0) {
                return LRESULT.ZERO;
            }
        }
        return result;
	...
	*/
        if (OS.IsAppThemed()) {
            if ((getApi().state & CANVAS) != 0) {
                long code = callWindowProc(getApi().handle, OS.WM_NCHITTEST, wParam, lParam);
                if (code == OS.HTCLIENT) {
                    RECT rect = new RECT();
                    ...
                    OS.MapWindowPoints(0, getApi().handle, pt, 1);
    ...
    LRESULT WM_PAINT(long wParam, long lParam) {
        if ((getApi().state & DISPOSE_SENT) != 0)
            return LRESULT.ZERO;
        if ((getApi().state & CANVAS) == 0 || (getApi().state & FOREIGN_HANDLE) != 0) {
        ...
        if (hooks(SWT.Paint) || filters(SWT.Paint)) {
            ...
            if ((getApi().style & SWT.DOUBLE_BUFFERED) != 0) {
                if ((getApi().style & (SWT.NO_MERGE_PAINTS | SWT.RIGHT_TO_LEFT | SWT.TRANSPARENT)) == 0) {
                    bufferedPaint = true;
                }
            }
            if (bufferedPaint) {
                long hDC = OS.BeginPaint(getApi().handle, ps);
                ...
                GC gc = SwtGC.win32_new(this.getApi(), data);
                /* Get the system region for the paint HDC */
                long sysRgn = 0;
                if ((getApi().style & (SWT.DOUBLE_BUFFERED | SWT.TRANSPARENT)) != 0 || (getApi().style & SWT.NO_MERGE_PAINTS) != 0) {
                ...
                if (width != 0 && height != 0) {
                    GC paintGC = null;
                    Image image = null;
                    if ((getApi().style & (SWT.DOUBLE_BUFFERED | SWT.TRANSPARENT)) != 0) {
    ...
    LRESULT WM_SIZE(long wParam, long lParam) {
		...
		*/
        ...
        if (OS.IsWindowVisible(getApi().handle)) {
            if ((getApi().state & CANVAS) != 0) {
                if ((getApi().style & SWT.NO_REDRAW_RESIZE) == 0) {
                    if (hooks(SWT.Paint)) {
                        OS.InvalidateRect(getApi().handle, null, true);
                    }
                }
        ...
        if ((getApi().state & CANVAS) != 0 && (getApi().style & SWT.EMBEDDED) != 0) {
            resizeEmbeddedHandle(OS.GetWindow(getApi().handle, OS.GW_CHILD), OS.LOWORD(lParam), OS.HIWORD(lParam));
        }
        return result;
    }
	...
	*/
        ...
        switch(cmd) {
            ...
            case OS.SC_VSCROLL:
                ...
                long code = callWindowProc(getApi().handle, OS.WM_SYSCOMMAND, wParam, lParam);
    ...
    LRESULT WM_UPDATEUISTATE(long wParam, long lParam) {
        ...
        if (result != null)
            return result;
        if ((getApi().state & CANVAS) != 0 && hooks(SWT.Paint)) {
            OS.InvalidateRect(getApi().handle, null, true);
        }
        return result;
    }
    ...
    LRESULT wmNCPaint(long hwnd, long wParam, long lParam) {
        ...
        if ((getApi().state & CANVAS) != 0 || (hwnd == borderHandle && getApi().handle != borderHandle)) {
            if (OS.IsAppThemed()) {
                ...
                if ((bits1 & OS.WS_EX_CLIENTEDGE) != 0) {
                    ...
                    OS.DrawThemeBackground(((SwtDisplay) display.getImpl()).hEditTheme(getApi().nativeZoom), hDC, OS.EP_EDITTEXT, OS.ETS_NORMAL, rect, null);
            ...
            case OS.TTN_GETDISPINFO:
				...
				*/
                        ...
                        if ((lpnmtdi.uFlags & OS.TTF_IDISHWND) != 0) {
                            widget = ((SwtDisplay) display.getImpl()).getControl(hwnd);
                        } else {
                            if (hdr.hwndFrom == ((SwtShell) shell.getImpl()).toolTipHandle || hdr.hwndFrom == ((SwtShell) shell.getImpl()).balloonTipHandle) {
                                widget = ((SwtShell) shell.getImpl()).findToolTip((int) hdr.idFrom);
                            }
                        }
                        if (widget != null) {
