...
public abstract class SwtWidget implements IWidget {
    ...
    void maybeEnableDarkSystemTheme(long handle) {
        if (((SwtDisplay) display.getImpl()).useDarkModeExplorerTheme) {
            ...
            OS.SetWindowTheme(handle, SwtDisplay.EXPLORER, null);
        }
    }
 ...
 */
    ...
    boolean sendMouseEvent(int type, int button, long hwnd, long lParam) {
        return sendMouseEvent(type, button, ((SwtDisplay) display.getImpl()).getClickCount(type, button, hwnd, lParam), 0, false, hwnd, lParam);
    }
    ...
    boolean sendMouseWheelEvent(int type, long hwnd, long wParam, long lParam) {
        ...
        MouseWheelData wheelData = new MouseWheelData(vertical, null, wParam, ((SwtDisplay) display.getImpl()).scrollRemainderEvt);
	...
	*/
        switch(((SwtDisplay) display.getImpl()).lastAscii) {
            case SWT.DEL:
                if (((SwtDisplay) display.getImpl()).lastKey == SWT.BS)
                    ((SwtDisplay) display.getImpl()).lastAscii = SWT.BS;
                break;
            case SWT.LF:
                if (((SwtDisplay) display.getImpl()).lastKey == SWT.CR)
                    ((SwtDisplay) display.getImpl()).lastAscii = SWT.CR;
                break;
        }
        /*
	...
	*/
        if (((SwtDisplay) display.getImpl()).lastKey == SWT.CR && ((SwtDisplay) display.getImpl()).lastAscii == SWT.CR) {
            if ((lParam & 0x1000000) != 0)
                ((SwtDisplay) display.getImpl()).lastKey = SWT.KEYPAD_CR;
        }
        setLocationMask(event, type, wParam, lParam);
        if (((SwtDisplay) display.getImpl()).lastVirtual) {
            /*
		...
		*/
            if (((SwtDisplay) display.getImpl()).lastKey == OS.VK_DELETE)
                ((SwtDisplay) display.getImpl()).lastAscii = 0x7F;
            /*
		...
		* to zero.
		*/
            if (((SwtDisplay) display.getImpl()).lastKey == OS.VK_CANCEL)
                ((SwtDisplay) display.getImpl()).lastAscii = 0x0;
            event.keyCode = SwtDisplay.translateKey(((SwtDisplay) display.getImpl()).lastKey);
        } else {
            event.keyCode = ((SwtDisplay) display.getImpl()).lastKey;
        }
        event.character = (char) ((SwtDisplay) display.getImpl()).lastAscii;
    ...
    int setLocationMask(Event event, int type, long wParam, long lParam) {
        ...
        if (((SwtDisplay) display.getImpl()).lastVirtual) {
            switch(((SwtDisplay) display.getImpl()).lastKey) {
                case OS.VK_SHIFT:
            ...
            if (((SwtDisplay) display.getImpl()).numpadKey(((SwtDisplay) display.getImpl()).lastKey) != 0) {
                location = SWT.KEYPAD;
            }
        } else {
            if (((SwtDisplay) display.getImpl()).lastKey == SWT.KEYPAD_CR) {
                location = SWT.KEYPAD;
            }
        }
    ...
    boolean showMenu(int x, int y, int detail) {
        ...
        if (menu != null && !menu.isDisposed()) {
            // In Pixels
            Point locInPixels = Win32DPIUtils.pointToPixelAsLocation(event.getLocation(), getAutoscalingZoom());
    ...
    LRESULT wmCaptureChanged(long hwnd, long wParam, long lParam) {
        ((SwtDisplay) display.getImpl()).captureChanged = true;
        return null;
    }
	...
	*/
    ...
    LRESULT wmKeyDown(long hwnd, long wParam, long lParam) {
        ...
        ((SwtDisplay) display.getImpl()).lastAscii = ((SwtDisplay) display.getImpl()).lastKey = 0;
        ((SwtDisplay) display.getImpl()).lastVirtual = ((SwtDisplay) display.getImpl()).lastDead = false;
	...
	*/
        ...
        if (OS.PeekMessage(msg, hwnd, OS.WM_DEADCHAR, OS.WM_DEADCHAR, flags)) {
            ((SwtDisplay) display.getImpl()).lastDead = true;
            ((SwtDisplay) display.getImpl()).lastVirtual = mapKey == 0;
            ((SwtDisplay) display.getImpl()).lastKey = ((SwtDisplay) display.getImpl()).lastVirtual ? (int) wParam : mapKey;
            return null;
        }
        /*
	...
	*/
        ((SwtDisplay) display.getImpl()).lastVirtual = mapKey == 0 || ((SwtDisplay) display.getImpl()).numpadKey((int) wParam) != 0;
        if (((SwtDisplay) display.getImpl()).lastVirtual) {
            ((SwtDisplay) display.getImpl()).lastKey = (int) wParam;
            /*
		* Feature in Windows.  The virtual key VK_DELETE is not
		* treated as both a virtual key and an ASCII key by Windows.
		* Therefore, we will not receive a WM_CHAR for this key.
		...
		* down.
		*/
            if (OS.VK_NUMPAD0 <= ((SwtDisplay) display.getImpl()).lastKey && ((SwtDisplay) display.getImpl()).lastKey <= OS.VK_DIVIDE) {
                ((SwtDisplay) display.getImpl()).lastAscii = ((SwtDisplay) display.getImpl()).numpadKey(((SwtDisplay) display.getImpl()).lastKey);
            }
        } else {
            /*
		...
		*/
            ((SwtDisplay) display.getImpl()).lastKey = (int) OS.CharLower(OS.LOWORD(mapKey));
            /*
		...
		*/
            if (wParam == OS.VK_CANCEL)
                ((SwtDisplay) display.getImpl()).lastVirtual = true;
            if (OS.GetKeyState(OS.VK_CONTROL) < 0) {
                /*
			 ...
			 */
                if (OS.GetKeyState(OS.VK_SHIFT) < 0) {
                    ((SwtDisplay) display.getImpl()).lastAscii = ((SwtDisplay) display.getImpl()).shiftedKey((int) wParam);
                    if (((SwtDisplay) display.getImpl()).lastAscii == 0)
                        ((SwtDisplay) display.getImpl()).lastAscii = mapKey;
                } else {
                    ((SwtDisplay) display.getImpl()).lastAscii = (int) OS.CharLower(OS.LOWORD(mapKey));
                }
                ((SwtDisplay) display.getImpl()).lastAscii = ((SwtDisplay) display.getImpl()).controlKey(((SwtDisplay) display.getImpl()).lastAscii);
            }
        }
        if (isCharPending) {
            return null;
	...
	*/
        if (!hooks(SWT.KeyUp) && !((SwtDisplay) display.getImpl()).filters(SWT.KeyUp)) {
            ((SwtDisplay) display.getImpl()).lastKey = ((SwtDisplay) display.getImpl()).lastAscii = 0;
            ((SwtDisplay) display.getImpl()).lastVirtual = ((SwtDisplay) display.getImpl()).lastDead = false;
            return null;
        }
        ...
        if (((SwtDisplay) display.getImpl()).lastDead)
            return null;
        /*
	...
	*/
		...
		*/
            if (wParam == OS.VK_CANCEL)
                ((SwtDisplay) display.getImpl()).lastVirtual = true;
            if (((SwtDisplay) display.getImpl()).lastKey == 0) {
                ((SwtDisplay) display.getImpl()).lastAscii = 0;
	 ...
	 * directions or when window focus changes.
	 */
        ((SwtDisplay) display.getImpl()).scrollRemainderEvt.x = 0;
        ((SwtDisplay) display.getImpl()).scrollRemainderEvt.y = 0;
        ((SwtDisplay) display.getImpl()).scrollRemainderBar.x = 0;
        ((SwtDisplay) display.getImpl()).scrollRemainderBar.y = 0;
	...
	*/
    ...
    LRESULT wmLButtonDown(long hwnd, long wParam, long lParam) {
        ...
        int count = ((SwtDisplay) display.getImpl()).getClickCount(SWT.MouseDown, 1, hwnd, lParam);
        if (count == 1 && (getApi().state & DRAG_DETECT) != 0 && hooks(SWT.DragDetect)) {
            /*
	...
	*/
    ...
    LRESULT wmMouseMove(long hwnd, long wParam, long lParam) {
        ...
        Display display = this.display;
        int pos = OS.GetMessagePos();
        if (pos != ((SwtDisplay) display.getImpl()).lastMouse || ((SwtDisplay) display.getImpl()).captureChanged) {
            boolean trackMouse = (getApi().state & TRACK_MOUSE) != 0;
            boolean mouseEnter = hooks(SWT.MouseEnter) || ((SwtDisplay) display.getImpl()).filters(SWT.MouseEnter);
            boolean mouseExit = hooks(SWT.MouseExit) || ((SwtDisplay) display.getImpl()).filters(SWT.MouseExit);
            boolean mouseHover = hooks(SWT.MouseHover) || ((SwtDisplay) display.getImpl()).filters(SWT.MouseHover);
            if (trackMouse || mouseEnter || mouseExit || mouseHover) {
                ...
                if (mouseEnter && (getApi().state & MOUSE_OVER) == 0) {
                    /*
            ...
            if (pos != ((SwtDisplay) display.getImpl()).lastMouse) {
                ((SwtDisplay) display.getImpl()).lastMouse = pos;
        ...
        if ((lParam & OS.PRF_NONCLIENT) != 0) {
            if (OS.IsAppThemed()) {
                ...
                if ((bits & OS.WS_EX_CLIENTEDGE) != 0) {
                    ...
                    OS.DrawThemeBackground(((SwtDisplay) display.getImpl()).hEditTheme(getApi().nativeZoom), wParam, OS.EP_EDITTEXT, OS.ETS_NORMAL, rect, null);
                    return new LRESULT(code);
                }
            }
        }
	...
	*/
    ...
    LRESULT wmSysChar(long hwnd, long wParam, long lParam) {
        ...
        if (!hooks(SWT.KeyDown) && !((SwtDisplay) display.getImpl()).filters(SWT.KeyDown)) {
            return null;
        }
        /* Call the window proc to determine whether it is a system key or mnemonic */
        boolean oldKeyHit = ((SwtDisplay) display.getImpl()).mnemonicKeyHit;
        ((SwtDisplay) display.getImpl()).mnemonicKeyHit = true;
        long result = callWindowProc(hwnd, OS.WM_SYSCHAR, wParam, lParam);
        boolean consumed = false;
        if (!((SwtDisplay) display.getImpl()).mnemonicKeyHit) {
            consumed = !sendKeyEvent(SWT.KeyDown, OS.WM_SYSCHAR, wParam, lParam);
            // widget could be disposed at this point
        }
        consumed |= ((SwtDisplay) display.getImpl()).mnemonicKeyHit;
        ((SwtDisplay) display.getImpl()).mnemonicKeyHit = oldKeyHit;
	...
	*/
    ...
    LRESULT wmXButtonDown(long hwnd, long wParam, long lParam) {
        ...
        Display display = this.display;
        ((SwtDisplay) display.getImpl()).captureChanged = false;
        ((SwtDisplay) display.getImpl()).xMouse = true;
        int button = OS.HIWORD(wParam) == OS.XBUTTON1 ? 4 : 5;
	...
	*/
    ...
    int getSystemMetrics(int nIndex) {
        return OS.GetSystemMetricsForDpi(nIndex, DPIUtil.mapZoomToDPI(getApi().nativeZoom));
    }
