...
public abstract class SwtControl extends SwtWidget implements Drawable, IControl {
    ...
    void checkMirrored() {
        if ((getApi().style & SWT.RIGHT_TO_LEFT) != 0) {
            int bits = OS.GetWindowLong(getApi().handle, OS.GWL_EXSTYLE);
    ...
    void checkGesture() {
		 ...
		 */
            ...
            config.dwWant = 1;
            config.dwBlock = 0;
            OS.SetGestureConfig(getApi().handle, 0, 1, config, GESTURECONFIG.sizeof);
        }
    }
    ...
    long defaultFont() {
        return SWTFontProvider.getSystemFontHandle(display, getApi().nativeZoom);
    }
    ...
    boolean dragDetect(int button, int count, int stateMask, int x, int y) {
        if (button != 1 || count != 1)
            return false;
        boolean dragging = dragDetect(getApi().handle, x, y, false, null, null);
		...
		*/
            if (button == 1 && OS.GetKeyState(OS.VK_ESCAPE) >= 0) {
                ...
                OS.SendMessage(getApi().handle, OS.WM_LBUTTONUP, wParam, lParam);
            }
            return false;
        }
    ...
    void drawImageBackground(long hDC, long hwnd, long hBitmap, RECT rect, int tx, int ty) {
        ...
        OS.MapWindowPoints(hwnd, getApi().handle, rect2, 2);
    ...
    void enableWidget(boolean enabled) {
        OS.EnableWindow(getApi().handle, enabled);
    }
        ...
        if (control != null) {
            ...
            if (image != null) {
                if (control.getImpl() instanceof SwtControl) {
                    ((SwtControl) control.getImpl()).drawImageBackground(hDC, getApi().handle, SwtImage.win32_getHandle(image, getAutoscalingZoom()), rect, tx, ty);
                }
            }
        }
    }
    ...
    void fillThemeBackground(long hDC, Control control, RECT rect) {
        ...
        if (control != null && control.getImpl() instanceof SwtControl) {
            ((SwtControl) control.getImpl()).drawThemeBackground(hDC, getApi().handle, rect);
        }
    }
	...
	*/
    ...
    public Color getBackground() {
        ...
        if (backgroundAlpha == 0) {
            Color color = SwtColor.win32_new(display, background, 0);
            return color;
        } else {
            Control control = findBackgroundControl();
            if (control == null)
    ...
    public boolean getEnabled() {
        checkWidget();
        return OS.IsWindowEnabled(getApi().handle);
    }
    ...
    public Font getFont() {
        ...
        if (hFont == 0)
            hFont = defaultFont();
        return SWTFontProvider.getFont(display, hFont, getApi().nativeZoom);
    }
    ...
    public Monitor getMonitor() {
        checkWidget();
        long hmonitor = OS.MonitorFromWindow(getApi().handle, OS.MONITOR_DEFAULTTONEAREST);
        return ((SwtDisplay) display.getImpl()).getMonitor(hmonitor);
    }
    ...
    public int getTextDirection() {
        ...
        int bits = OS.GetWindowLong(getApi().handle, OS.GWL_EXSTYLE) & flags;
    ...
    public boolean getTouchEnabled() {
        checkWidget();
        return OS.IsTouchWindow(getApi().handle, null);
    }
    ...
    boolean hasCursor() {
        RECT rect = new RECT();
        if (!OS.GetClientRect(getApi().handle, rect))
            return false;
        OS.MapWindowPoints(getApi().handle, 0, rect, 2);
        POINT pt = new POINT();
	...
	*/
        ...
        while (hwndFocus != 0) {
            if (hwndFocus == getApi().handle)
                return true;
            if (((SwtDisplay) display.getImpl()).getControl(hwndFocus) != null) {
                return false;
            }
	...
	*/
    ...
    public boolean isTabItem() {
        ...
        if ((bits & OS.WS_TABSTOP) != 0)
            return false;
        long code = OS.SendMessage(getApi().handle, OS.WM_GETDLGCODE, 0, 0);
    ...
    GC new_GC(GCData data) {
        return SwtGC.win32_new(this.getApi(), data);
    }
    ...
    public boolean print(GC gc) {
        ...
        float scaleFactor = 1f * gc.getGCData().nativeZoom / getApi().nativeZoom;
        if (gdipGraphics != 0) {
            long clipRgn = 0;
	...
	*/
		...
		*/
            ...
            if (fixPrintWindow) {
                ...
                Shell shell = getShell();
                Control savedFocus = ((SwtDecorations) shell.getImpl()).savedFocus;
                OS.SetParent(hwnd, 0);
                ((SwtDecorations) shell.getImpl()).setSavedFocus(savedFocus);
                if ((bits1 & OS.WS_VISIBLE) != 0) {
	...
	*/
	 ...
	 */
        ...
        OS.RedrawWindow(getApi().handle, rect, 0, flags);
    }
    ...
    void showWidget(boolean visible) {
        ...
        if (getApi().handle != topHandle)
            OS.ShowWindow(getApi().handle, visible ? OS.SW_SHOW : OS.SW_HIDE);
    }
	...
	*/
    ...
    boolean sendGestureEvent(GESTUREINFO gi) {
         ...
         */
        if (gi.hwndTarget != getApi().handle)
            return true;
        ...
        switch(gi.dwID) {
            case OS.GID_ZOOM:
                ...
                if ((gi.dwFlags & OS.GF_BEGIN) != 0) {
                    ...
                    ((SwtDisplay) display.getImpl()).magStartDistance = ((SwtDisplay) display.getImpl()).lastDistance = fingerDistance;
			...
			*/
                if (fingerDistance == ((SwtDisplay) display.getImpl()).lastDistance && event.detail == SWT.GESTURE_MAGNIFY)
                    return true;
                if (fingerDistance != 0)
                    event.magnification = fingerDistance / ((SwtDisplay) display.getImpl()).magStartDistance;
                ((SwtDisplay) display.getImpl()).lastDistance = fingerDistance;
                break;
            case OS.GID_PAN:
                type = SWT.Gesture;
                ...
                if ((gi.dwFlags & OS.GF_BEGIN) != 0) {
                    event.detail = SWT.GESTURE_BEGIN;
                    ((SwtDisplay) display.getImpl()).lastX = point.x;
                    ((SwtDisplay) display.getImpl()).lastY = point.y;
                } else if ((gi.dwFlags & OS.GF_END) != 0) {
                    event.detail = SWT.GESTURE_END;
                }
                if (((SwtDisplay) display.getImpl()).lastX == point.x && ((SwtDisplay) display.getImpl()).lastY == point.y && event.detail == SWT.GESTURE_PAN)
                    return true;
                event.xDirection = point.x - ((SwtDisplay) display.getImpl()).lastX;
                event.yDirection = point.y - ((SwtDisplay) display.getImpl()).lastY;
                ((SwtDisplay) display.getImpl()).lastX = point.x;
                ((SwtDisplay) display.getImpl()).lastY = point.y;
                break;
            case OS.GID_ROTATE:
                ...
                if ((gi.dwFlags & OS.GF_BEGIN) != 0) {
                    event.detail = SWT.GESTURE_BEGIN;
                    ((SwtDisplay) display.getImpl()).rotationAngle = rotationInRadians;
                } else if ((gi.dwFlags & OS.GF_END) != 0) {
                    event.detail = SWT.GESTURE_END;
                }
                /*
			...
			*/
                if (((SwtDisplay) display.getImpl()).rotationAngle == rotationInRadians && event.detail == SWT.GESTURE_ROTATE)
                    return true;
    ...
    void sendTouchEvent(TOUCHINPUT[] touchInput) {
        ...
        for (int i = 0; i < touchInput.length; i++) {
            ...
            TouchSource inputSource = ((SwtDisplay) display.getImpl()).findTouchSource(ti.hSource, monitor);
            int state = 0;
    ...
    void setBackground() {
        ...
        if (control.getImpl()._backgroundImage() != null) {
            ...
            setBackgroundImage(SwtImage.win32_getHandle(control.getImpl()._backgroundImage(), getAutoscalingZoom()));
        } else {
    ...
    void setBoundsInPixels(int x, int y, int width, int height, int flags, boolean defer) {
        if (findImageControl() != null) {
            ...
            if (OS.GetWindow(getApi().handle, OS.GW_CHILD) == 0) {
                if (OS.IsAppThemed()) {
    ...
    private void fitInParentBounds(Rectangle boundsInPixels, int zoom) {
        ...
        Rectangle parentBoundsInPixels = ((SwtControl) parent.getImpl()).getBoundsInPixels();
        ...
        boolean childFittingHorizontallyWithOffByOne = parentBoundsInPixels.width >= boundsInPixels.x + boundsInPixels.width - Win32DPIUtils.pointToPixel(1.0f, zoom);
        ...
        boolean childFittingVerticallyWithOffByOne = parentBoundsInPixels.height >= boundsInPixels.y + boundsInPixels.height - Win32DPIUtils.pointToPixel(1.0f, zoom);
    ...
    public boolean setAutoscalingMode(AutoscalingMode autoscalingMode) {
        ...
        if (getApi().nativeZoom != newZoom) {
            getApi().nativeZoom = newZoom;
    ...
    public void setCursor(Cursor cursor) {
        ...
        if (hwndCursor == 0) {
            ...
            while (hwnd != 0 && hwnd != getApi().handle) {
    ...
    void setDefaultFont() {
        long hFont = SWTFontProvider.getSystemFontHandle(display, getApi().nativeZoom);
	...
	*/
    ...
    public void setFont(Font font) {
        checkWidget();
        Font newFont = GraphicsUtilsSwt.copyFontToSwt(font);
        if (newFont != null) {
            if (newFont.isDisposed())
                error(SWT.ERROR_INVALID_ARGUMENT);
            newFont = SwtFont.win32_new(newFont, getApi().nativeZoom);
        }
        long hFont = 0;
        if (newFont != null) {
            if (newFont.isDisposed())
                error(SWT.ERROR_INVALID_ARGUMENT);
            hFont = SwtFont.win32_getHandle(newFont);
        }
        this.font = newFont;
        if (hFont == 0)
            hFont = defaultFont();
        OS.SendMessage(getApi().handle, OS.WM_SETFONT, hFont, 1);
    }
    ...
    private boolean embedsWin32Control() {
        ...
        if (this.getApi() instanceof Browser browser) {
            // The Edge browser embeds webView2
            return "edge".equals(browser.getBrowserType());
        }
        if (this.getApi() instanceof OleClientSite) {
    ...
    public void setRegion(Region region) {
        ...
        if (region != null) {
            ...
            OS.CombineRgn(hRegion, SwtRegion.win32_getHandle(region, getAutoscalingZoom()), hRegion, OS.RGN_OR);
        }
        OS.SetWindowRgn(getApi().handle, hRegion, true);
        this.region = region;
    }
    ...
    void setToolTipText(Shell shell, String string) {
        ((SwtShell) shell.getImpl()).setToolTipText(getApi().handle, string);
    }
    ...
    public void setTouchEnabled(boolean enabled) {
        checkWidget();
        if (enabled) {
            OS.RegisterTouchWindow(getApi().handle, 0);
        } else {
            OS.UnregisterTouchWindow(getApi().handle);
        }
    }
	...
	*/
    ...
    void subclass() {
        ...
        long newProc = ((SwtDisplay) display.getImpl()).windowProc;
        if (oldProc == newProc)
            return;
        OS.SetWindowLongPtr(getApi().handle, OS.GWLP_WNDPROC, newProc);
    }
    ...
    Point toDisplayInPixels(int x, int y) {
        ...
        pt.x = x;
        pt.y = y;
        OS.ClientToScreen(getApi().handle, pt);
        return new Point(pt.x, pt.y);
    }
    ...
    boolean translateMnemonic(MSG msg) {
        ...
        if (shell.isVisible() && shell.isEnabled()) {
            ((SwtDisplay) display.getImpl()).lastAscii = (int) msg.wParam;
            ((SwtDisplay) display.getImpl()).lastDead = false;
            Event event = new Event();
    ...
    boolean translateTraversal(MSG msg) {
        ...
        event.doit = doit;
        event.detail = detail;
        ((SwtDisplay) display.getImpl()).lastKey = lastKey;
        ((SwtDisplay) display.getImpl()).lastAscii = lastAscii;
        ((SwtDisplay) display.getImpl()).lastVirtual = lastVirtual;
        ((SwtDisplay) display.getImpl()).lastDead = false;
        if (!setKeyState(event, SWT.Traverse, msg.wParam, msg.lParam))
            return false;
        Shell shell = getShell();
	...
	*/
    ...
    void unsubclass() {
        ...
        long oldProc = ((SwtDisplay) display.getImpl()).windowProc;
        if (oldProc == newProc)
            return;
        OS.SetWindowLongPtr(getApi().handle, OS.GWLP_WNDPROC, newProc);
    }
    ...
    void updateBackgroundImage() {
        ...
        setBackgroundImage(image != null ? SwtImage.win32_getHandle(image, getAutoscalingZoom()) : 0);
    }
    ...
    void updateOrientation() {
        ...
        OS.SetWindowLong(getApi().handle, OS.GWL_EXSTYLE, bits);
        OS.InvalidateRect(getApi().handle, null, true);
    }
	...
	*/
    ...
    long windowProc(long hwnd, int msg, long wParam, long lParam) {
        ...
        switch(msg) {
            ...
            case OS.WM_ERASEBKGND:
                ...
                break;
            case OS.WM_GESTURE:
                result = WM_GESTURE(wParam, lParam);
                break;
            ...
            case OS.WM_MOUSEWHEEL:
                ...
                break;
            case OS.WM_MOUSEHWHEEL:
                result = WM_MOUSEHWHEEL(wParam, lParam);
                break;
            case OS.WM_MOVE:
            ...
            case OS.WM_PAINT:
                ...
                break;
            case OS.WM_ENTERMENULOOP:
                result = WM_ENTERMENULOOP(wParam, lParam);
                break;
            case OS.WM_EXITMENULOOP:
                result = WM_EXITMENULOOP(wParam, lParam);
                break;
            case OS.WM_ENTERSIZEMOVE:
                result = WM_ENTERSIZEMOVE(wParam, lParam);
                break;
            case OS.WM_EXITSIZEMOVE:
                result = WM_EXITSIZEMOVE(wParam, lParam);
                break;
            ...
            case OS.WM_SYSKEYUP:
                ...
                break;
            case OS.WM_TABLET_FLICK:
                result = WM_TABLET_FLICK(wParam, lParam);
                break;
            case OS.WM_TIMER:
                result = WM_TIMER(wParam, lParam);
                break;
            case OS.WM_TOUCH:
                result = WM_TOUCH(wParam, lParam);
                break;
            case OS.WM_UNDO:
                result = WM_UNDO(wParam, lParam);
                break;
            case OS.WM_UNINITMENUPOPUP:
                result = WM_UNINITMENUPOPUP(wParam, lParam);
                break;
            ...
            case OS.WM_XBUTTONUP:
                ...
                break;
            case OS.WM_DPICHANGED:
                result = WM_DPICHANGED(wParam, lParam);
                break;
            case OS.WM_DISPLAYCHANGE:
                result = WM_DISPLAYCHANGE(wParam, lParam);
                break;
        }
        if (result != null)
            return result.value;
    ...
    LRESULT WM_CAPTURECHANGED(long wParam, long lParam) {
        return wmCaptureChanged(getApi().handle, wParam, lParam);
    }

    LRESULT WM_CHANGEUISTATE(long wParam, long lParam) {
        if ((getApi().state & IGNORE_WM_CHANGEUISTATE) != 0)
            return LRESULT.ZERO;
        return null;
    }

    LRESULT WM_CHAR(long wParam, long lParam) {
        return wmChar(getApi().handle, wParam, lParam);
    }
	...
	*/
    ...
    LRESULT WM_CONTEXTMENU(long wParam, long lParam) {
        return wmContextMenu(getApi().handle, wParam, lParam);
    }
    ...
    LRESULT WM_ERASEBKGND(long wParam, long lParam) {
        if ((getApi().state & DRAW_BACKGROUND) != 0) {
            if (findImageControl() != null)
                return LRESULT.ONE;
        }
        if ((getApi().state & THEME_BACKGROUND) != 0) {
    ...
    LRESULT WM_HELP(long wParam, long lParam) {
        ...
        if (lphi.iContextType == OS.HELPINFO_MENUITEM) {
            MenuItem item = ((SwtDisplay) display.getImpl()).getMenuItem(lphi.iCtrlId);
            if (item != null && item.isEnabled()) {
                Widget widget = null;
                if (((SwtWidget) item.getImpl()).hooks(SWT.Help)) {
                    widget = item;
                } else {
                    Menu menu = ((SwtMenuItem) item.getImpl()).parent;
                    if (((SwtWidget) menu.getImpl()).hooks(SWT.Help))
                        widget = menu;
                }
                if (widget != null) {
                    long hwndShell = shell.handle;
                    OS.SendMessage(hwndShell, OS.WM_CANCELMODE, 0, 0);
                    ((SwtWidget) widget.getImpl()).postEvent(SWT.Help);
                    return LRESULT.ONE;
                }
            }
            return null;
    ...
    LRESULT WM_IME_CHAR(long wParam, long lParam) {
        return wmIMEChar(getApi().handle, wParam, lParam);
    }
    ...
    LRESULT WM_UNINITMENUPOPUP(long wParam, long lParam) {
        Menu hiddenMenu = ((SwtDecorations) menuShell().getImpl()).findMenu(wParam);
        if (hiddenMenu != null) {
            Shell shell = getShell();
            hiddenMenu.getImpl().sendEvent(SWT.Hide);
            if (hiddenMenu == ((SwtShell) shell.getImpl()).activeMenu)
                ((SwtShell) shell.getImpl()).activeMenu = null;
        }
        return null;
    }
    ...
    LRESULT WM_INITMENUPOPUP(long wParam, long lParam) {
        ...
        if (((SwtDisplay) display.getImpl()).accelKeyHit)
            return null;
        /*
	...
	*/
        Shell shell = getShell();
        Menu oldMenu = ((SwtShell) shell.getImpl()).activeMenu, newMenu = null;
        if (OS.HIWORD(lParam) == 0) {
            newMenu = ((SwtDecorations) menuShell().getImpl()).findMenu(wParam);
            if (newMenu != null)
                ((SwtMenu) newMenu.getImpl()).update();
        }
        Menu menu = newMenu;
        ...
        if (menu == null) {
            menu = ((SwtShell) shell.getImpl()).activeMenu;
            while (menu != null) {
                /*
			* It is possible (but unlikely), that application
			...
			* to follow.
			*/
                menu.getImpl().sendEvent(SWT.Hide);
                if (menu.isDisposed())
                    break;
                menu = menu.getParentMenu();
	...
	*/
        if (newMenu != null && newMenu.isDisposed())
            newMenu = null;
        ((SwtShell) shell.getImpl()).activeMenu = newMenu;
        /* Send the show event */
        if (newMenu != null && newMenu != oldMenu) {
            newMenu.getImpl().sendEvent(SWT.Show);
            // widget could be disposed at this point
        }
        return null;
    }
    ...
    LRESULT WM_INPUTLANGCHANGE(long wParam, long lParam) {
        ((SwtDecorations) menuShell().getImpl()).destroyAccelerators();
        return null;
    }

    LRESULT WM_KEYDOWN(long wParam, long lParam) {
        return wmKeyDown(getApi().handle, wParam, lParam);
    }

    LRESULT WM_KEYUP(long wParam, long lParam) {
        return wmKeyUp(getApi().handle, wParam, lParam);
    }
	 ...
	 */
        if (wParam == 0)
            ((SwtDecorations) menuShell().getImpl()).setSavedFocus(this.getApi());
        return wmKillFocus(getApi().handle, wParam, lParam);
    }

    LRESULT WM_LBUTTONDBLCLK(long wParam, long lParam) {
        return wmLButtonDblClk(getApi().handle, wParam, lParam);
    }

    LRESULT WM_LBUTTONDOWN(long wParam, long lParam) {
        return wmLButtonDown(getApi().handle, wParam, lParam);
    }

    LRESULT WM_LBUTTONUP(long wParam, long lParam) {
        return wmLButtonUp(getApi().handle, wParam, lParam);
    }

    LRESULT WM_MBUTTONDBLCLK(long wParam, long lParam) {
        return wmMButtonDblClk(getApi().handle, wParam, lParam);
    }

    LRESULT WM_MBUTTONDOWN(long wParam, long lParam) {
        return wmMButtonDown(getApi().handle, wParam, lParam);
    }

    LRESULT WM_MBUTTONUP(long wParam, long lParam) {
        return wmMButtonUp(getApi().handle, wParam, lParam);
    }
	...
	*/
        ...
        if (type == 0 || type == OS.MF_SYSMENU) {
            ((SwtDisplay) display.getImpl()).mnemonicKeyHit = false;
    ...
    LRESULT WM_MENUSELECT(long wParam, long lParam) {
        ...
        OS.KillTimer(this.getApi().handle, SwtMenu.ID_TOOLTIP_TIMER);
        if (activeMenu != null)
            ((SwtMenu) activeMenu.getImpl()).hideCurrentToolTip();
        if (code == 0xFFFF && lParam == 0) {
			...
			* a mnemonic.
			*/
                ((SwtDisplay) display.getImpl()).mnemonicKeyHit = true;
                /*
			* It is possible (but unlikely), that application
			* code could have disposed the widget in the hide
			* event.  If this happens, stop searching up the
		...
		*/
        ...
        if ((code & OS.MF_HILITE) != 0) {
            ...
            if ((code & OS.MF_POPUP) != 0) {
                ...
                Menu newMenu = ((SwtDecorations) menuShell.getImpl()).findMenu(lParam);
                if (newMenu != null) {
                    int id = OS.LOWORD(wParam);
                    item = ((SwtDisplay) display.getImpl()).getMenuItem(id);
                }
    ...
    LRESULT WM_MOUSEHOVER(long wParam, long lParam) {
        return wmMouseHover(getApi().handle, wParam, lParam);
    }

    LRESULT WM_MOUSELEAVE(long wParam, long lParam) {
        ((SwtShell) getShell().getImpl()).fixToolTip();
        return wmMouseLeave(getApi().handle, wParam, lParam);
    }

    LRESULT WM_MOUSEMOVE(long wParam, long lParam) {
        return wmMouseMove(getApi().handle, wParam, lParam);
    }

    LRESULT WM_MOUSEWHEEL(long wParam, long lParam) {
        return wmMouseWheel(getApi().handle, wParam, lParam);
    }

    LRESULT WM_MOUSEHWHEEL(long wParam, long lParam) {
        return wmMouseHWheel(getApi().handle, wParam, lParam);
    }

    LRESULT WM_MOVE(long wParam, long lParam) {
        getApi().state |= MOVE_OCCURRED;
        if (findImageControl() != null) {
        ...
        if ((getApi().state & MOVE_DEFERRED) == 0)
            sendEvent(SWT.Move);
        // widget could be disposed at this point
        return null;
    }
    ...
    LRESULT WM_NCPAINT(long wParam, long lParam) {
        return wmNCPaint(getApi().handle, wParam, lParam);
    }
    ...
    LRESULT WM_PAINT(long wParam, long lParam) {
        if ((getApi().state & DISPOSE_SENT) != 0)
            return LRESULT.ZERO;
        return wmPaint(getApi().handle, wParam, lParam);
    }
    ...
    LRESULT WM_PRINT(long wParam, long lParam) {
        return wmPrint(getApi().handle, wParam, lParam);
    }
    ...
    LRESULT WM_RBUTTONDBLCLK(long wParam, long lParam) {
        return wmRButtonDblClk(getApi().handle, wParam, lParam);
    }

    LRESULT WM_RBUTTONDOWN(long wParam, long lParam) {
        return wmRButtonDown(getApi().handle, wParam, lParam);
    }

    LRESULT WM_RBUTTONUP(long wParam, long lParam) {
        return wmRButtonUp(getApi().handle, wParam, lParam);
    }

    LRESULT WM_SETCURSOR(long wParam, long lParam) {
        int hitTest = (short) OS.LOWORD(lParam);
        if (hitTest == OS.HTCLIENT) {
            Control control = ((SwtDisplay) display.getImpl()).getControl(wParam);
            if (control == null)
                return null;
    ...
    LRESULT WM_SETFOCUS(long wParam, long lParam) {
        return wmSetFocus(getApi().handle, wParam, lParam);
    }
    ...
    LRESULT WM_SIZE(long wParam, long lParam) {
        getApi().state |= RESIZE_OCCURRED;
        if ((getApi().state & RESIZE_DEFERRED) == 0)
            sendEvent(SWT.Resize);
        // widget could be disposed at this point
        return null;
    }
    ...
    LRESULT WM_SYSCHAR(long wParam, long lParam) {
        return wmSysChar(getApi().handle, wParam, lParam);
    }
	...
	*/
        ...
        switch(cmd) {
			...
			*/
                if (lParam == 0) {
                    ...
                    if (menu == null) {
                        ...
                        if (control != null) {
                            if (((SwtWidget) control.getImpl()).hooks(SWT.KeyDown) || ((SwtWidget) control.getImpl()).hooks(SWT.KeyUp)) {
                                ((SwtDisplay) display.getImpl()).mnemonicKeyHit = false;
                                return LRESULT.ZERO;
                            }
                        }
			...
			*/
                Decorations shell = menuShell();
                if (!shell.isEnabled() || !shell.getImpl().isActive()) {
                    return LRESULT.ZERO;
                }
                break;
            case OS.SC_MINIMIZE:
                /* Save the focus widget when the shell is minimized */
                ((SwtDecorations) menuShell().getImpl()).saveFocus();
                break;
        }
        return null;
    }
    ...
    LRESULT WM_SYSKEYDOWN(long wParam, long lParam) {
        return wmSysKeyDown(getApi().handle, wParam, lParam);
    }

    LRESULT WM_SYSKEYUP(long wParam, long lParam) {
        return wmSysKeyUp(getApi().handle, wParam, lParam);
    }
    ...
    LRESULT WM_WINDOWPOSCHANGED(long wParam, long lParam) {
        try {
            ((SwtDisplay) display.getImpl()).resizeCount++;
            long code = callWindowProc(getApi().handle, OS.WM_WINDOWPOSCHANGED, wParam, lParam);
	...
	*/
    ...
    LRESULT WM_XBUTTONDBLCLK(long wParam, long lParam) {
        return wmXButtonDblClk(getApi().handle, wParam, lParam);
    }

    LRESULT WM_XBUTTONDOWN(long wParam, long lParam) {
        return wmXButtonDown(getApi().handle, wParam, lParam);
    }

    LRESULT WM_XBUTTONUP(long wParam, long lParam) {
        return wmXButtonUp(getApi().handle, wParam, lParam);
    }
    ...
    LRESULT wmColorChild(long wParam, long lParam) {
        ...
        if (control.getImpl()._backgroundImage() != null) {
            ...
            long hBitmap = SwtImage.win32_getHandle(control.getImpl()._backgroundImage(), getAutoscalingZoom());
            OS.MapWindowPoints(getApi().handle, hwnd, rect, 2);
            POINT lpPoint = new POINT();
            ...
            if ((getApi().state & DRAW_BACKGROUND) != 0) {
                ...
                OS.MapWindowPoints(hwnd, getApi().handle, rect, 2);
    ...
    private static class DPIChangeProcessingCallback {
        ...
        // Executed as callback method referenced by signature description
    ...
    int getSystemMetrics(int nIndex) {
        Shell shell = getShell();
        int zoom = shell != null ? shell.getZoom() : getApi().nativeZoom;
