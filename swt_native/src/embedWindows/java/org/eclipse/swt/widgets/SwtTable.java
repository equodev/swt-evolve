...
public class SwtTable extends SwtComposite implements ITable {
    ...
    boolean _checkGrow(int count) {
        ...
        if (keys == null) {
			...
			* memory usage.
			*/
                boolean small = getDrawing() && OS.IsWindowVisible(getApi().handle);
    ...
    TableItem _getItem(int index, boolean create, int count) {
        ...
        if (keys == null) {
            ...
            if (keyIndex < 0) {
                if (count == -1) {
                    count = (int) OS.SendMessage(getApi().handle, OS.LVM_GETITEMCOUNT, 0, 0);
                }
                ...
                if (items[keyIndex] != null)
                    return items[keyIndex];
            }
            return items[keyIndex] = new TableItem(this.getApi(), SWT.NONE, -1, false);
        }
    }
		...
		*/
            ...
            case OS.WM_WINDOWPOSCHANGED:
				...
				*/
                    ...
                    OS.SendMessage(getApi().handle, OS.LVM_SETBKCOLOR, 0, 0xFFFFFF);
                }
            //FALL THROUGH
            /* Mouse messages */
            ...
            case OS.WM_TIMER:
                {
                    if (findImageControl() != null) {
                        topIndex = (int) OS.SendMessage(getApi().handle, OS.LVM_GETTOPINDEX, 0, 0);
                    }
                }
        }
	...
	*/
        ...
        if (msg == OS.WM_PAINT) {
            int bits0 = OS.GetWindowLong(getApi().handle, OS.GWL_STYLE);
            if ((bits0 & OS.LVS_NOCOLUMNHEADER) == 0) {
                long hwndParent = OS.GetParent(getApi().handle), hwndOwner = 0;
                while (hwndParent != 0) {
                ...
                case OS.WM_WINDOWPOSCHANGING:
                    {
                        ...
                        if ((getApi().style & SWT.H_SCROLL) == 0 && (bits & OS.WS_HSCROLL) != 0) {
                            fixScroll = true;
                            bits &= ~OS.WS_HSCROLL;
                        }
                        if ((getApi().style & SWT.V_SCROLL) == 0 && (bits & OS.WS_VSCROLL) != 0) {
                            fixScroll = true;
                            bits &= ~OS.WS_VSCROLL;
                        }
                        if (fixScroll)
        ...
        if (checkSelection) {
            if (wasSelected || forceSelect) {
                ...
                int index = (int) OS.SendMessage(getApi().handle, OS.LVM_GETNEXTITEM, -1, OS.LVNI_FOCUSED);
                if (index != -1)
        ...
        switch(msg) {
            ...
            case OS.WM_WINDOWPOSCHANGED:
                if (redraw) {
                    OS.SendMessage(getApi().handle, OS.LVM_SETBKCOLOR, 0, OS.CLR_NONE);
                    OS.DefWindowProc(getApi().handle, OS.WM_SETREDRAW, 1, 0);
                    OS.InvalidateRect(getApi().handle, null, true);
                    long hwndHeader = OS.SendMessage(getApi().handle, OS.LVM_GETHEADER, 0, 0);
                    if (hwndHeader != 0)
            ...
            case OS.WM_TIMER:
                {
                    if (findImageControl() != null) {
                        if (topIndex != OS.SendMessage(getApi().handle, OS.LVM_GETTOPINDEX, 0, 0)) {
                            OS.InvalidateRect(getApi().handle, null, true);
                        }
                    }
                    break;
	...
	*/
        if (!ignoreCustomDraw && !ignoreDrawFocus && nmcd.left != nmcd.right) {
            if (OS.IsWindowVisible(getApi().handle) && OS.IsWindowEnabled(getApi().handle)) {
                if (!explorerTheme && (getApi().style & SWT.FULL_SELECTION) != 0) {
                    if ((int) OS.SendMessage(getApi().handle, OS.LVM_GETBKCOLOR, 0, 0) == OS.CLR_NONE) {
                        int dwExStyle = (int) OS.SendMessage(getApi().handle, OS.LVM_GETEXTENDEDLISTVIEWSTYLE, 0, 0);
                            ...
                            if (OS.SendMessage(getApi().handle, OS.LVM_GETNEXTITEM, -1, OS.LVNI_FOCUSED) == nmcd.dwItemSpec) {
                                if (getApi().handle == OS.GetFocus()) {
                                    ...
                                    if ((uiState & OS.UISF_HIDEFOCUS) == 0) {
                                        ...
                                        OS.SendMessage(getApi().handle, OS.LVM_GETITEMRECT, nmcd.dwItemSpec, rect);
                                        long hwndHeader = OS.SendMessage(getApi().handle, OS.LVM_GETHEADER, 0, 0);
                                        int index = (int) OS.SendMessage(hwndHeader, OS.HDM_ORDERTOINDEX, 0, 0);
                                        RECT itemRect = new RECT();
                                        if (index == 0) {
                                            itemRect.left = OS.LVIR_LABEL;
                                            OS.SendMessage(getApi().handle, OS.LVM_GETITEMRECT, index, itemRect);
                                        } else {
                                            itemRect.top = index;
                                            itemRect.left = OS.LVIR_ICON;
                                            OS.SendMessage(getApi().handle, OS.LVM_GETSUBITEMRECT, nmcd.dwItemSpec, itemRect);
                                        }
	...
	* custom draw.
	*/
        if (--customCount == 0 && OS.IsWindowVisible(getApi().handle)) {
            if (!explorerTheme && (getApi().style & SWT.FULL_SELECTION) != 0) {
                if ((int) OS.SendMessage(getApi().handle, OS.LVM_GETBKCOLOR, 0, 0) == OS.CLR_NONE) {
                    int dwExStyle = (int) OS.SendMessage(getApi().handle, OS.LVM_GETEXTENDEDLISTVIEWSTYLE, 0, 0);
                    if ((dwExStyle & OS.LVS_EX_FULLROWSELECT) == 0) {
					...
					*/
                        long hwndToolTip = OS.SendMessage(getApi().handle, OS.LVM_SETTOOLTIPS, 0, 0);
                        long rgn = OS.CreateRectRgn(0, 0, 0, 0);
                        int result = OS.GetUpdateRgn(getApi().handle, rgn, true);
                        OS.SendMessage(getApi().handle, OS.LVM_SETEXTENDEDLISTVIEWSTYLE, bits, bits);
					...
					*/
                        hwndToolTip = OS.SendMessage(getApi().handle, OS.LVM_SETTOOLTIPS, hwndToolTip, hwndToolTip);
                    }
                }
            }
        }
	...
	*/
            ...
            if (!explorerTheme && (getApi().style & SWT.FULL_SELECTION) != 0) {
                if ((int) OS.SendMessage(getApi().handle, OS.LVM_GETBKCOLOR, 0, 0) == OS.CLR_NONE) {
					...
					*/
                        ...
                        long rgn = OS.CreateRectRgn(0, 0, 0, 0);
                        int result = OS.GetUpdateRgn(getApi().handle, rgn, true);
                        OS.SendMessage(getApi().handle, OS.LVM_SETEXTENDEDLISTVIEWSTYLE, bits, 0);
                        OS.ValidateRect(getApi().handle, null);
                        if (result != OS.NULLREGION)
                            OS.InvalidateRgn(getApi().handle, rgn, true);
                        OS.DeleteObject(rgn);
		...
		*/
            ...
            if (explorerTheme && columnCount == 0) {
                long hDC = nmcd.hdc;
                if (OS.IsWindowEnabled(getApi().handle) || findImageControl() != null || hasCustomBackground()) {
                ...
                if (control != null && control.getImpl()._backgroundImage() != null) {
                    ...
                    final boolean enabled = OS.IsWindowEnabled(getApi().handle);
                        ...
                        if (OS.IsAppThemed()) {
                            if (sortColumn != null && sortDirection != SWT.NONE) {
                                ...
                                if (index != -1) {
                                    ...
                                    OS.GetClientRect(getApi().handle, columnRect);
                                    long hwndHeader = OS.SendMessage(getApi().handle, OS.LVM_GETHEADER, 0, 0);
            ...
            if ((int) OS.SendMessage(getApi().handle, OS.LVM_GETBKCOLOR, 0, 0) != OS.CLR_NONE) {
                if ((sortDirection & (SWT.UP | SWT.DOWN)) != 0) {
                    if (sortColumn != null && !sortColumn.isDisposed()) {
                        int oldColumn = (int) OS.SendMessage(getApi().handle, OS.LVM_GETSELECTEDCOLUMN, 0, 0);
                        if (oldColumn == -1) {
                            int newColumn = indexOf(sortColumn);
                            long rgn = OS.CreateRectRgn(0, 0, 0, 0);
                            int result = OS.GetUpdateRgn(getApi().handle, rgn, true);
                            OS.SendMessage(getApi().handle, OS.LVM_SETSELECTEDCOLUMN, newColumn, 0);
                            OS.ValidateRect(getApi().handle, null);
                            if (result != OS.NULLREGION)
                                OS.InvalidateRgn(getApi().handle, rgn, true);
                            OS.DeleteObject(rgn);
	...
	*/
        ...
        if (item == null || item.isDisposed())
            return null;
        long hFont = ((SwtTableItem) item.getImpl()).fontHandle(nmcd.iSubItem);
        if (hFont != -1)
        ...
        int clrText = ((SwtTableItem) item.getImpl()).cellForeground != null ? ((SwtTableItem) item.getImpl()).cellForeground[nmcd.iSubItem] : -1;
        if (clrText == -1)
            clrText = ((SwtTableItem) item.getImpl()).foreground;
        int clrTextBk = ((SwtTableItem) item.getImpl()).cellBackground != null ? ((SwtTableItem) item.getImpl()).cellBackground[nmcd.iSubItem] : -1;
        if (clrTextBk == -1)
            clrTextBk = ((SwtTableItem) item.getImpl()).background;
	...
	*/
        final boolean enabled = OS.IsWindowEnabled(getApi().handle);
        if (OS.IsWindowVisible(getApi().handle) && enabled) {
            if (!explorerTheme && !ignoreDrawSelection && (getApi().style & SWT.FULL_SELECTION) != 0) {
                int bits = (int) OS.SendMessage(getApi().handle, OS.LVM_GETEXTENDEDLISTVIEWSTYLE, 0, 0);
				...
				*/
                    ...
                    long result = OS.SendMessage(getApi().handle, OS.LVM_GETITEM, 0, lvItem);
                    if ((result != 0 && (lvItem.state & OS.LVIS_SELECTED) != 0)) {
                        int clrSelection = -1;
                        if (nmcd.iSubItem == 0) {
                            if (OS.GetFocus() == getApi().handle || display.getHighContrast()) {
                                clrSelection = OS.GetSysColor(OS.COLOR_HIGHLIGHT);
                            } else {
                                if ((getApi().style & SWT.HIDE_SELECTION) == 0) {
		...
		*/
            ...
            if (hFont == -1 && clrText == -1 && clrTextBk == -1) {
                if (((SwtTableItem) item.getImpl()).cellForeground == null && ((SwtTableItem) item.getImpl()).cellBackground == null && ((SwtTableItem) item.getImpl()).cellFont == null) {
	...
	*/
        if ((enabled && clrTextBk != -1) || (!enabled && hasCustomBackground())) {
            ...
            if (oldColumn != -1 && oldColumn == nmcd.iSubItem) {
                long rgn = OS.CreateRectRgn(0, 0, 0, 0);
                int result = OS.GetUpdateRgn(getApi().handle, rgn, true);
                OS.SendMessage(getApi().handle, OS.LVM_SETSELECTEDCOLUMN, -1, 0);
                OS.ValidateRect(getApi().handle, null);
                if (result != OS.NULLREGION)
                    OS.InvalidateRgn(getApi().handle, rgn, true);
                OS.DeleteObject(rgn);
        ...
        if (!((SwtTableItem) item.getImpl()).cached) {
            ...
            if (redraw) {
                if (!setScrollWidth(item, false)) {
                    ((SwtTableItem) item.getImpl()).redraw();
                }
            }
        }
        return true;
    ...
    public void clear(int index) {
        ...
        if (item != null) {
            if (item != currentItem)
                ((SwtTableItem) item.getImpl()).clear();
            /*
		...
		* cached bounds.
		*/
            if ((getApi().style & SWT.VIRTUAL) == 0 && ((SwtTableItem) item.getImpl()).cached) {
                LVITEM lvItem = new LVITEM();
                lvItem.mask = OS.LVIF_TEXT | OS.LVIF_INDENT;
                lvItem.pszText = OS.LPSTR_TEXTCALLBACK;
                lvItem.iItem = index;
                OS.SendMessage(getApi().handle, OS.LVM_SETITEM, 0, lvItem);
                ((SwtTableItem) item.getImpl()).cached = false;
            }
            if (currentItem == null && getDrawing() && OS.IsWindowVisible(getApi().handle)) {
                OS.SendMessage(getApi().handle, OS.LVM_REDRAWITEMS, index, index);
            }
    ...
    public void clear(int start, int end) {
        ...
        if (start == 0 && end == count - 1) {
            ...
            if (cleared) {
                if (currentItem == null && getDrawing() && OS.IsWindowVisible(getApi().handle)) {
                    OS.SendMessage(getApi().handle, OS.LVM_REDRAWITEMS, start, end);
                }
    ...
    public void clearAll() {
        ...
        if (cleared) {
            if (currentItem == null && getDrawing() && OS.IsWindowVisible(getApi().handle)) {
                OS.SendMessage(getApi().handle, OS.LVM_REDRAWITEMS, 0, count - 1);
            }
            setScrollWidth(null, false);
        }
    }
    ...
    Point computeSizeInPixels(Point hintInPoints, int zoom, boolean changed) {
        ...
        if (hintInPoints.x != SWT.DEFAULT) {
            ...
            int count = (int) OS.SendMessage(hwndHeader, OS.HDM_GETITEMCOUNT, 0, 0);
            for (int i = 0; i < count; i++) {
                width += OS.SendMessage(getApi().handle, OS.LVM_GETCOLUMNWIDTH, i, 0);
            }
            bits |= width & 0xFFFF;
        }
        long result = OS.SendMessage(getApi().handle, OS.LVM_APPROXIMATEVIEWRECT, -1, OS.MAKELPARAM(bits, 0xFFFF));
        int width = OS.LOWORD(result);
        long empty = OS.SendMessage(getApi().handle, OS.LVM_APPROXIMATEVIEWRECT, 0, 0);
        long oneItem = OS.SendMessage(getApi().handle, OS.LVM_APPROXIMATEVIEWRECT, 1, 0);
        int itemHeight = OS.HIWORD(oneItem) - OS.HIWORD(empty);
        height += (int) OS.SendMessage(getApi().handle, OS.LVM_GETITEMCOUNT, 0, 0) * itemHeight;
        if (width == 0)
            width = DEFAULT_WIDTH;
        if (height == 0)
    ...
    void createHandle() {
        ...
        hwndHeader = OS.SendMessage(getApi().handle, OS.LVM_GETHEADER, 0, 0);
        ...
        if ((getApi().style & SWT.CHECK) != 0) {
            ...
            OS.SendMessage(getApi().handle, OS.LVM_SETCALLBACKMASK, OS.LVIS_STATEIMAGEMASK, 0);
        }
        /*
	...
	*/
        ...
        OS.SendMessage(getApi().handle, OS.LVM_INSERTCOLUMN, 0, lvColumn);
        ...
        OS.SendMessage(getApi().handle, OS.LVM_SETEXTENDEDLISTVIEWSTYLE, bits1, bits1);
        /*
	...
	*/
    ...
    void createItem(TableColumn column, int index) {
        ...
        int oldColumn = (int) OS.SendMessage(getApi().handle, OS.LVM_GETSELECTEDCOLUMN, 0, 0);
        if (oldColumn >= index) {
            OS.SendMessage(getApi().handle, OS.LVM_SETSELECTEDCOLUMN, oldColumn + 1, 0);
        }
        ...
        int itemCount = (int) OS.SendMessage(getApi().handle, OS.LVM_GETITEMCOUNT, 0, 0);
        for (int i = 0; i < itemCount; i++) {
            TableItem item = _getItem(i, false);
            if (item != null) {
                String[] strings = ((SwtTableItem) item.getImpl()).strings;
                if (strings != null) {
                    String[] temp = new String[columnCount + 1];
                    System.arraycopy(strings, 0, temp, 0, index);
                    System.arraycopy(strings, index, temp, index + 1, columnCount - index);
                    ((SwtTableItem) item.getImpl()).strings = temp;
                }
                Image[] images = ((SwtTableItem) item.getImpl()).images;
                if (images != null) {
                    Image[] temp = new Image[columnCount + 1];
                    System.arraycopy(images, 0, temp, 0, index);
                    System.arraycopy(images, index, temp, index + 1, columnCount - index);
                    ((SwtTableItem) item.getImpl()).images = temp;
                }
                if (index == 0) {
                    if (columnCount != 0) {
                        if (strings == null) {
                            ((SwtTableItem) item.getImpl()).strings = new String[columnCount + 1];
                            ((SwtTableItem) item.getImpl()).strings[1] = ((SwtItem) item.getImpl()).text;
                        }
                        //$NON-NLS-1$
                        ((SwtItem) item.getImpl()).text = "";
                        if (images == null) {
                            ((SwtTableItem) item.getImpl()).images = new Image[columnCount + 1];
                            ((SwtTableItem) item.getImpl()).images[1] = ((SwtItem) item.getImpl()).image;
                        }
                        ((SwtItem) item.getImpl()).image = null;
                    }
                }
                if (((SwtTableItem) item.getImpl()).cellBackground != null) {
                    int[] cellBackground = ((SwtTableItem) item.getImpl()).cellBackground;
                    int[] temp = new int[columnCount + 1];
                    System.arraycopy(cellBackground, 0, temp, 0, index);
                    System.arraycopy(cellBackground, index, temp, index + 1, columnCount - index);
                    temp[index] = -1;
                    ((SwtTableItem) item.getImpl()).cellBackground = temp;
                }
                if (((SwtTableItem) item.getImpl()).cellForeground != null) {
                    int[] cellForeground = ((SwtTableItem) item.getImpl()).cellForeground;
                    int[] temp = new int[columnCount + 1];
                    System.arraycopy(cellForeground, 0, temp, 0, index);
                    System.arraycopy(cellForeground, index, temp, index + 1, columnCount - index);
                    temp[index] = -1;
                    ((SwtTableItem) item.getImpl()).cellForeground = temp;
                }
                if (((SwtTableItem) item.getImpl()).cellFont != null) {
                    Font[] cellFont = ((SwtTableItem) item.getImpl()).cellFont;
                    Font[] temp = new Font[columnCount + 1];
                    System.arraycopy(cellFont, 0, temp, 0, index);
                    System.arraycopy(cellFont, index, temp, index + 1, columnCount - index);
                    ((SwtTableItem) item.getImpl()).cellFont = temp;
                }
            }
        }
        /*
	...
	*/
        ...
        if (index == 0) {
            if (columnCount > 1) {
                ...
                OS.SendMessage(getApi().handle, OS.LVM_INSERTCOLUMN, 1, lvColumn);
                OS.SendMessage(getApi().handle, OS.LVM_GETCOLUMN, 1, lvColumn);
                ...
                OS.SendMessage(getApi().handle, OS.LVM_GETCOLUMN, 0, lvColumn);
                OS.SendMessage(getApi().handle, OS.LVM_SETCOLUMN, 1, lvColumn);
                lvColumn.fmt = OS.LVCFMT_IMAGE;
                lvColumn.cx = width;
                lvColumn.iImage = OS.I_IMAGENONE;
                lvColumn.pszText = lvColumn.cchTextMax = 0;
                OS.SendMessage(getApi().handle, OS.LVM_SETCOLUMN, 0, lvColumn);
                ...
                OS.SendMessage(getApi().handle, OS.LVM_SETCOLUMNWIDTH, 0, 0);
            }
            /*
		* Bug in Windows.  Despite the fact that every item in the
		* table always has LPSTR_TEXTCALLBACK, Windows caches the
		...
		*/
            ...
            lvColumn.fmt = fmt;
            OS.SendMessage(getApi().handle, OS.LVM_INSERTCOLUMN, index, lvColumn);
        }
        ...
        if (headerToolTipHandle != 0) {
            ...
            if (OS.SendMessage(hwndHeader, OS.HDM_GETITEMRECT, index, rect) != 0) {
                ...
                lpti.uId = ((SwtTableColumn) column.getImpl()).id = ((SwtDisplay) display.getImpl()).nextToolTipId++;
                lpti.left = rect.left;
                lpti.top = rect.top;
	...
	*/
        ...
        int result = (int) OS.SendMessage(getApi().handle, OS.LVM_INSERTITEM, 0, lvItem);
    ...
    public void deselect(int[] indices) {
		...
		*/
            if (index >= 0) {
                ignoreSelect = true;
                OS.SendMessage(getApi().handle, OS.LVM_SETITEMSTATE, index, lvItem);
                ignoreSelect = false;
            }
        }
    }
	...
	*/
    ...
    public void deselect(int start, int end) {
		...
		*/
            ...
            for (int i = start; i <= end; i++) {
                ignoreSelect = true;
                OS.SendMessage(getApi().handle, OS.LVM_SETITEMSTATE, i, lvItem);
                ignoreSelect = false;
            }
        }
    }
    ...
    public void deselectAll() {
        ...
        lvItem.stateMask = OS.LVIS_SELECTED;
        ignoreSelect = true;
        OS.SendMessage(getApi().handle, OS.LVM_SETITEMSTATE, -1, lvItem);
        ignoreSelect = false;
    }
    ...
    void destroyItem(TableColumn column) {
        ...
        OS.SendMessage(getApi().handle, OS.LVM_GETCOLUMNORDERARRAY, columnCount, oldOrder);
		...
		*/
        ...
        if (columnCount > 1) {
            if (OS.SendMessage(getApi().handle, OS.LVM_DELETECOLUMN, index, 0) == 0) {
        ...
        for (int i = 0; i < itemCount; i++) {
            ...
            if (item != null) {
                if (columnCount == 0) {
                    ((SwtTableItem) item.getImpl()).strings = null;
                    ((SwtTableItem) item.getImpl()).images = null;
                    ((SwtTableItem) item.getImpl()).cellBackground = null;
                    ((SwtTableItem) item.getImpl()).cellForeground = null;
                    ((SwtTableItem) item.getImpl()).cellFont = null;
                } else {
                    if (((SwtTableItem) item.getImpl()).strings != null) {
                        String[] strings = ((SwtTableItem) item.getImpl()).strings;
                        if (index == 0) {
                            //$NON-NLS-1$
                    ...
                    if (((SwtTableItem) item.getImpl()).images != null) {
                        Image[] images = ((SwtTableItem) item.getImpl()).images;
                        if (index == 0)
		...
		*/
            ...
            if (j != newOrder.length) {
                OS.SendMessage(getApi().handle, OS.LVM_SETCOLUMNORDERARRAY, newOrder.length, newOrder);
                /*
			...
			*/
            ...
            for (int i = orderIndex; i < newOrder.length; i++) {
                ...
                ((SwtTableColumn) newColumns[i - orderIndex].getImpl()).updateToolTip(newOrder[i]);
            }
        ...
        if (headerToolTipHandle != 0) {
            TOOLINFO lpti = new TOOLINFO();
            lpti.cbSize = TOOLINFO.sizeof;
            lpti.uId = ((SwtTableColumn) column.getImpl()).id;
            lpti.hwnd = OS.SendMessage(getApi().handle, OS.LVM_GETHEADER, 0, 0);
    ...
    void destroyItem(TableItem item) {
        ...
        setDeferResize(true);
        ignoreSelect = ignoreShrink = true;
        long code = OS.SendMessage(getApi().handle, OS.LVM_DELETEITEM, index, 0);
        ignoreSelect = ignoreShrink = false;
        if (code == 0)
            error(SWT.ERROR_ITEM_NOT_REMOVED);
	...
	*/
        if ((getApi().style & SWT.CHECK) == 0)
            return;
        long hImageList = OS.SendMessage(getApi().handle, OS.LVM_GETIMAGELIST, OS.LVSIL_SMALL, 0);
        if (hImageList == 0)
            return;
        int[] cx = new int[1], cy = new int[1];
        OS.ImageList_GetIconSize(hImageList, cx, cy);
        long hStateList = OS.SendMessage(getApi().handle, OS.LVM_GETIMAGELIST, OS.LVSIL_STATE, 0);
        if (hStateList == 0)
            return;
    ...
    public int[] getColumnOrder() {
        ...
        OS.SendMessage(getApi().handle, OS.LVM_GETCOLUMNORDERARRAY, columnCount, order);
        return order;
    }
    ...
    int getFocusIndex() {
        //	checkWidget ();
        return (int) OS.SendMessage(getApi().handle, OS.LVM_GETNEXTITEM, -1, OS.LVNI_FOCUSED);
    }
    ...
    TableItem getItemInPixels(Point point) {
        ...
        if ((getApi().style & SWT.FULL_SELECTION) == 0) {
			...
			*/
                if (OS.SendMessage(getApi().handle, OS.LVM_SUBITEMHITTEST, 0, pinfo) < 0) {
                    ...
                    ignoreCustomDraw = true;
                    long code = OS.SendMessage(getApi().handle, OS.LVM_GETITEMRECT, 0, rect);
					...
					*  The fix is to consider any value that is negative a failure.
					*/
                        OS.SendMessage(getApi().handle, OS.LVM_SUBITEMHITTEST, 0, pinfo);
                        if (pinfo.iItem < 0)
                            pinfo.iItem = -1;
                    }
                }
        ...
        OS.SendMessage(getApi().handle, OS.LVM_HITTEST, 0, pinfo);
        if (pinfo.iItem != -1) {
            /*
            ...
            if (pinfo.iItem == 0) {
                ...
                if ((bits & OS.LVS_NOCOLUMNHEADER) == 0) {
                    if (hwndHeader != 0) {
                        ...
                        pt.x = pinfo.x;
                        pt.y = pinfo.y;
                        OS.MapWindowPoints(getApi().handle, 0, pt, 1);
    ...
    public int getItemCount() {
        checkWidget();
        return (int) OS.SendMessage(getApi().handle, OS.LVM_GETITEMCOUNT, 0, 0);
    }
    ...
    public TableItem[] getSelection() {
        checkWidget();
        int i = -1, j = 0, count = (int) OS.SendMessage(getApi().handle, OS.LVM_GETSELECTEDCOUNT, 0, 0);
        TableItem[] result = new TableItem[count];
        while ((i = (int) OS.SendMessage(getApi().handle, OS.LVM_GETNEXTITEM, i, OS.LVNI_SELECTED)) != -1) {
            result[j++] = _getItem(i);
        }
        return result;
    }
    ...
    public int getSelectionCount() {
        checkWidget();
        return (int) OS.SendMessage(getApi().handle, OS.LVM_GETSELECTEDCOUNT, 0, 0);
    }
    ...
    public int getSelectionIndex() {
        checkWidget();
        int focusIndex = (int) OS.SendMessage(getApi().handle, OS.LVM_GETNEXTITEM, -1, OS.LVNI_FOCUSED);
        int selectedIndex = (int) OS.SendMessage(getApi().handle, OS.LVM_GETNEXTITEM, -1, OS.LVNI_SELECTED);
    ...
    public int[] getSelectionIndices() {
        checkWidget();
        int count = (int) OS.SendMessage(getApi().handle, OS.LVM_GETSELECTEDCOUNT, 0, 0);
        int[] result = new int[count];
        int lastIndex = -1;
        for (int i = 0; i < count; i++) {
            lastIndex = (int) OS.SendMessage(getApi().handle, OS.LVM_GETNEXTITEM, lastIndex, OS.LVNI_SELECTED);
            if (lastIndex == -1) {
                break;
            }
            result[i] = lastIndex;
	...
	*/
        return Math.max(0, (int) OS.SendMessage(getApi().handle, OS.LVM_GETTOPINDEX, 0, 0));
    }

    boolean hasChildren() {
        long hwndChild = OS.GetWindow(getApi().handle, OS.GW_CHILD);
        while (hwndChild != 0) {
    ...
    boolean hitTestSelection(int index, int x, int y) {
        ...
        if (0 <= index && index < count) {
            ...
            long hFont = ((SwtTableItem) item.getImpl()).fontHandle(0);
            if (hFont != -1)
    ...
    int imageIndex(Image image, int column) {
		...
		*/
            ...
            OS.SendMessage(getApi().handle, OS.LVM_SETIMAGELIST, OS.LVSIL_SMALL, hImageList);
    ...
    void releaseWidget() {
        ...
        currentItem = null;
        if (imageList != null) {
            OS.SendMessage(getApi().handle, OS.LVM_SETIMAGELIST, OS.LVSIL_SMALL, 0);
        ...
        long hStateList = OS.SendMessage(getApi().handle, OS.LVM_GETIMAGELIST, OS.LVSIL_STATE, 0);
        OS.SendMessage(getApi().handle, OS.LVM_SETIMAGELIST, OS.LVSIL_STATE, 0);
        if (hStateList != 0)
        ...
        if (start == 0 && end == count - 1) {
            ...
            while (index <= end) {
                ...
                if (item != null && !item.isDisposed())
                    item.getImpl().release(false);
                ignoreSelect = ignoreShrink = true;
                long code = OS.SendMessage(getApi().handle, OS.LVM_DELETEITEM, start, 0);
                ignoreSelect = ignoreShrink = false;
                if (code == 0)
                    break;
                index++;
    ...
    public void removeAll() {
        ...
        setDeferResize(true);
        ignoreSelect = ignoreShrink = true;
        long code = OS.SendMessage(getApi().handle, OS.LVM_DELETEALLITEMS, 0, 0);
        ignoreSelect = ignoreShrink = false;
        if (code == 0)
            error(SWT.ERROR_ITEM_NOT_REMOVED);
        setTableEmpty();
    ...
    public void select(int[] indices) {
		...
		*/
            if (indices[i] >= 0) {
                ignoreSelect = true;
                OS.SendMessage(getApi().handle, OS.LVM_SETITEMSTATE, indices[i], lvItem);
                ignoreSelect = false;
            }
        }
    }
	...
	*/
    ...
    Event sendMeasureItemEvent(TableItem item, int row, int column, long hDC) {
        ...
        int nSavedDC = OS.SaveDC(hDC);
        GC gc = createNewGC(hDC, data);
        RECT itemRect = ((SwtTableItem) item.getImpl()).getBounds(row, column, true, true, false, false, hDC);
        Event event = new Event();
        event.item = item;
        event.gc = gc;
        event.index = column;
        ...
        if (OS.IsWindowEnabled(getApi().handle)) {
            ...
            long result = OS.SendMessage(getApi().handle, OS.LVM_GETITEM, 0, lvItem);
            boolean selected = (result != 0 && (lvItem.state & OS.LVIS_SELECTED) != 0);
            if (selected && (column == 0 || (getApi().style & SWT.FULL_SELECTION) != 0)) {
                if (OS.GetFocus() == getApi().handle || display.getHighContrast()) {
                    drawSelected = true;
                } else {
                    drawSelected = (getApi().style & SWT.HIDE_SELECTION) == 0;
    ...
    LRESULT sendMouseDownEvent(int type, int button, int msg, long wParam, long lParam) {
        ...
        if (!sendMouseEvent(type, button, getApi().handle, lParam)) {
	...
	*/
        if ((getApi().style & SWT.SINGLE) != 0 || hooks(SWT.MouseDown) || hooks(SWT.MouseUp)) {
            if (pinfo.iItem == -1) {
                if (!((SwtDisplay) display.getImpl()).captureChanged && !isDisposed()) {
                    if (OS.GetCapture() != getApi().handle)
                        OS.SetCapture(getApi().handle);
	...
	*/
        ...
        if (count == 1 && pinfo.iItem != -1) {
            ...
            OS.SendMessage(getApi().handle, OS.LVM_GETITEM, 0, lvItem);
	...
	*/
        boolean dragDetect = (getApi().state & DRAG_DETECT) != 0 && hooks(SWT.DragDetect);
        if (!dragDetect) {
            int flags = OS.LVHT_ONITEMICON | OS.LVHT_ONITEMLABEL;
	...
	*/
        if (fullRowSelect) {
            ...
            OS.DefWindowProc(getApi().handle, OS.WM_SETREDRAW, 0, 0);
            OS.SendMessage(getApi().handle, OS.LVM_SETEXTENDEDLISTVIEWSTYLE, OS.LVS_EX_FULLROWSELECT, OS.LVS_EX_FULLROWSELECT);
        }
        dragStarted = false;
        ((SwtDisplay) display.getImpl()).dragCancelled = false;
        if (!dragDetect)
            ((SwtDisplay) display.getImpl()).runDragDrop = false;
        long code = callWindowProc(getApi().handle, msg, wParam, lParam, forceSelect);
        if (!dragDetect)
            ((SwtDisplay) display.getImpl()).runDragDrop = true;
        if (fullRowSelect) {
            fullRowSelect = false;
            OS.DefWindowProc(getApi().handle, OS.WM_SETREDRAW, 1, 0);
            OS.SendMessage(getApi().handle, OS.LVM_SETEXTENDEDLISTVIEWSTYLE, OS.LVS_EX_FULLROWSELECT, 0);
        }
        if (dragStarted || ((SwtDisplay) display.getImpl()).dragCancelled) {
            if (!((SwtDisplay) display.getImpl()).captureChanged && !isDisposed()) {
                if (OS.GetCapture() != getApi().handle)
                    OS.SetCapture(getApi().handle);
            }
            ...
            if (!fakeMouseUp && (getApi().style & SWT.MULTI) != 0) {
                fakeMouseUp = (pinfo.flags & OS.LVHT_ONITEMSTATEICON) == 0;
            }
            if (fakeMouseUp) {
                sendMouseEvent(SWT.MouseUp, button, getApi().handle, lParam);
            }
        }
        return new LRESULT(code);
    }
	...
	*/
    ...
    void setBackgroundPixel(int newPixel) {
        int oldPixel = (int) OS.SendMessage(getApi().handle, OS.LVM_GETBKCOLOR, 0, 0);
	...
	*/
        ...
        if (transparent) {
			...
			*/
                OS.SendMessage(getApi().handle, OS.LVM_SETBKCOLOR, 0, OS.CLR_NONE);
                OS.SendMessage(getApi().handle, OS.LVM_SETTEXTBKCOLOR, 0, OS.CLR_NONE);
                OS.InvalidateRect(getApi().handle, null, true);
                /* Clear LVS_EX_FULLROWSELECT */
                if (!explorerTheme && (getApi().style & SWT.FULL_SELECTION) != 0) {
                    int bits = OS.LVS_EX_FULLROWSELECT;
            ...
            if (oldPixel == OS.CLR_NONE) {
                ...
                if ((sortDirection & (SWT.UP | SWT.DOWN)) != 0) {
                    if (sortColumn != null && !sortColumn.isDisposed()) {
                        ...
                        if (column != -1) {
                            OS.SendMessage(getApi().handle, OS.LVM_SETSELECTEDCOLUMN, column, 0);
                            /*
						* Bug in Windows.  When LVM_SETSELECTEDCOLUMN is set, Windows
						* does not redraw either the new or the previous selected column.
						* The fix is to force a redraw.
	...
	*/
    ...
    public void setColumnOrder(int[] order) {
        ...
        if (reorder) {
            ...
            OS.SendMessage(getApi().handle, OS.LVM_SETCOLUMNORDERARRAY, order.length, order);
            /*
		* Bug in Windows.  When LVM_SETCOLUMNORDERARRAY is used to change
		* the column order, the header redraws correctly but the table does
		* not.  The fix is to force a redraw.
            ...
            for (int i = 0; i < columnCount; i++) {
                ...
                if (!column.isDisposed()) {
                    ...
                    if (newRect.left != oldRects[i].left) {
                        ((SwtTableColumn) column.getImpl()).updateToolTip(i);
    ...
    void setDeferResize(boolean defer) {
        if (defer) {
			...
			*/
                if (hooks(SWT.MeasureItem) || hooks(SWT.EraseItem) || hooks(SWT.PaintItem)) {
                    if (drawCount++ == 0 && OS.IsWindowVisible(getApi().handle)) {
                        OS.DefWindowProc(getApi().handle, OS.WM_SETREDRAW, 0, 0);
                        OS.SendMessage(getApi().handle, OS.LVM_SETBKCOLOR, 0, 0xFFFFFF);
                    }
                }
	...
	*/
        ...
        long hOldStateList = OS.SendMessage(getApi().handle, OS.LVM_GETIMAGELIST, OS.LVSIL_STATE, 0);
        OS.SendMessage(getApi().handle, OS.LVM_SETIMAGELIST, OS.LVSIL_STATE, hStateList);
        if (hOldStateList != 0)
	...
	*/
        ...
        OS.SendMessage(getApi().handle, OS.LVM_SETITEMSTATE, index, lvItem);
        ignoreSelect = false;
        OS.SendMessage(getApi().handle, OS.LVM_SETSELECTIONMARK, 0, index);
    }
	...
	*/
        ...
        if (itemHeight != -1) {
            int bits = OS.GetWindowLong(getApi().handle, OS.GWL_STYLE);
            OS.SetWindowLong(getApi().handle, OS.GWL_STYLE, bits | OS.LVS_OWNERDRAWFIXED);
        }
        super.setFont(font);
        if (itemHeight != -1) {
            int bits = OS.GetWindowLong(getApi().handle, OS.GWL_STYLE);
	...
	*/
        ...
        OS.SendMessage(getApi().handle, OS.LVM_SETTEXTCOLOR, 0, pixel);
        /*
	...
	*/
    ...
    public void setHeaderVisible(boolean show) {
        checkWidget();
        int newBits = OS.GetWindowLong(getApi().handle, OS.GWL_STYLE);
	...
	*/
    ...
    public void setItemCount(int count) {
        ...
        while (index < itemCount) {
            ...
            if (!isVirtual) {
                ignoreSelect = ignoreShrink = true;
                long code = OS.SendMessage(getApi().handle, OS.LVM_DELETEITEM, count, 0);
                ignoreSelect = ignoreShrink = false;
                if (code == 0)
                    break;
            }
        ...
        if (isVirtual) {
            ...
            OS.SendMessage(getApi().handle, OS.LVM_SETITEMCOUNT, count, flags);
            /*
	...
	*/
		...
		*/
            ...
            ignoreResize = true;
            OS.SetWindowPos(getApi().handle, 0, 0, 0, width, height + 1, flags);
            OS.SetWindowPos(getApi().handle, 0, 0, 0, width, height, flags);
            ignoreResize = false;
            OS.SetWindowLong(getApi().handle, OS.GWL_STYLE, bits);
        }
        if (fixScroll && topIndex != 0) {
    ...
    public void setLinesVisible(boolean show) {
        ...
        OS.SendMessage(getApi().handle, OS.LVM_SETEXTENDEDLISTVIEWSTYLE, OS.LVS_EX_GRIDLINES, newBits);
        OS.InvalidateRect(hwndHeader, null, true);
    }
    ...
    void setScrollWidth(int width) {
        if (width != (int) OS.SendMessage(getApi().handle, OS.LVM_GETCOLUMNWIDTH, 0, 0)) {
            /*
		...
		*/
            ...
            if (redraw)
                OS.DefWindowProc(getApi().handle, OS.WM_SETREDRAW, 0, 0);
            OS.SendMessage(getApi().handle, OS.LVM_SETCOLUMNWIDTH, 0, width);
            if (redraw) {
                OS.DefWindowProc(getApi().handle, OS.WM_SETREDRAW, 1, 0);
                int flags = OS.RDW_ERASE | OS.RDW_FRAME | OS.RDW_INVALIDATE | OS.RDW_ALLCHILDREN;
                OS.RedrawWindow(getApi().handle, null, 0, flags);
    ...
    boolean setScrollWidth(TableItem item, boolean force) {
        ...
        if (!force && (!getDrawing() || !OS.IsWindowVisible(getApi().handle))) {
            fixScrollWidth = true;
            return false;
        }
	...
	*/
        if (columnCount == 0) {
            ...
            while (index < itemCount) {
                ...
                if (item != null) {
                    ...
                    imageIndent = Math.max(imageIndent, ((SwtTableItem) item.getImpl()).imageIndent);
                    hFont = ((SwtTableItem) item.getImpl()).fontHandle(0);
                } else {
                    TableItem tableItem = _getItem(index, false);
                    if (tableItem != null) {
                        string = ((SwtItem) tableItem.getImpl()).text;
                        imageIndent = Math.max(imageIndent, ((SwtTableItem) tableItem.getImpl()).imageIndent);
                        hFont = ((SwtTableItem) tableItem.getImpl()).fontHandle(0);
                    }
                }
                if (string != null && string.length() != 0) {
                    if (hFont != -1) {
                        ...
                        newWidth = Math.max(newWidth, (int) OS.SendMessage(getApi().handle, OS.LVM_GETSTRINGWIDTH, 0, buffer));
                    }
                }
                if (item != null)
                    break;
    ...
    public void setSortColumn(TableColumn column) {
        ...
        if (column != null && column.isDisposed())
            error(SWT.ERROR_INVALID_ARGUMENT);
        if (sortColumn != null && !sortColumn.isDisposed()) {
            ((SwtTableColumn) sortColumn.getImpl()).setSortDirection(SWT.NONE);
        }
        sortColumn = column;
        if (sortColumn != null && sortDirection != SWT.NONE) {
            ((SwtTableColumn) sortColumn.getImpl()).setSortDirection(sortDirection);
        }
    }
    ...
    public void setSortDirection(int direction) {
        ...
        if (sortColumn != null && !sortColumn.isDisposed()) {
            ((SwtTableColumn) sortColumn.getImpl()).setSortDirection(direction);
        }
    }
    ...
    void setSubImagesVisible(boolean visible) {
        ...
        OS.SendMessage(getApi().handle, OS.LVM_SETEXTENDEDLISTVIEWSTYLE, OS.LVS_EX_SUBITEMIMAGES, bits);
    }
    ...
    public void setTopIndex(int index) {
        checkWidget();
        int topIndex = (int) OS.SendMessage(getApi().handle, OS.LVM_GETTOPINDEX, 0, 0);
        if (index == topIndex)
            return;
        if (!painted && hooks(SWT.MeasureItem))
	...
	*/
        if (OS.SendMessage(getApi().handle, OS.LVM_GETCOUNTPERPAGE, 0, 0) <= 0) {
            /*
		...
		* to the exact item.
		*/
            OS.SendMessage(getApi().handle, OS.LVM_ENSUREVISIBLE, index, 1);
            if (index != OS.SendMessage(getApi().handle, OS.LVM_GETTOPINDEX, 0, 0)) {
                OS.SendMessage(getApi().handle, OS.LVM_ENSUREVISIBLE, index, 1);
            }
            return;
        }
        ...
        rect.left = OS.LVIR_BOUNDS;
        ignoreCustomDraw = true;
        OS.SendMessage(getApi().handle, OS.LVM_GETITEMRECT, 0, rect);
        ignoreCustomDraw = false;
        int dy = (index - topIndex) * (rect.bottom - rect.top);
        OS.SendMessage(getApi().handle, OS.LVM_SCROLL, 0, dy);
    }
	...
	*/
        ...
        if (index == 0) {
            itemRect.top = 1;
            ignoreCustomDraw = true;
            OS.SendMessage(getApi().handle, OS.LVM_GETSUBITEMRECT, -1, itemRect);
            ignoreCustomDraw = false;
	...
	*/
        ...
        if (itemRect.left < rect.left) {
            ...
            OS.SendMessage(getApi().handle, OS.LVM_SCROLL, dx, 0);
        } else {
	...
	*/
        long counterPage = OS.SendMessage(getApi().handle, OS.LVM_GETCOUNTPERPAGE, 0, 0);
        if (counterPage <= 0) {
            /*
		* Bug in Windows.  For some reason, LVM_ENSUREVISIBLE can
		* scroll one item more or one item less when there is not
		...
		*/
		 ...
		 */
            long topIndex = OS.SendMessage(getApi().handle, OS.LVM_GETTOPINDEX, 0, 0);
            if (topIndex > index || index >= topIndex + counterPage) {
                OS.SendMessage(getApi().handle, OS.LVM_ENSUREVISIBLE, index, 0);
            }
        }
    }
    ...
    public void showSelection() {
        checkWidget();
        int index = (int) OS.SendMessage(getApi().handle, OS.LVM_GETNEXTITEM, -1, OS.LVNI_SELECTED);
        if (index != -1) {
            /*
		 ...
		 */
            if (display.getActiveShell() == getShell() && (getApi().style & SWT.NO_SCROLL) == 0 && (verticalBar == null || !verticalBar.isVisible())) {
                showItem(0);
            } else {
                showItem(index);
            }
    ...
    String toolTipText(NMTTDISPINFO hdr) {
        ...
        if (headerToolTipHandle == hdr.hwndFrom) {
            for (int i = 0; i < columnCount; i++) {
                TableColumn column = columns[i];
                if (((SwtTableColumn) column.getImpl()).id == hdr.idFrom)
                    return ((SwtTableColumn) column.getImpl()).toolTipText;
            }
        }
	...
	*/
        ...
        if (fixSubclass) {
            oldTableProc = OS.SetWindowLongPtr(getApi().handle, OS.GWLP_WNDPROC, TableProc);
            oldHeaderProc = OS.SetWindowLongPtr(hwndHeader, OS.GWLP_WNDPROC, HeaderProc);
        }
        super.update(all);
        if (fixSubclass) {
            OS.SetWindowLongPtr(getApi().handle, OS.GWLP_WNDPROC, oldTableProc);
    ...
    void updateMenuLocation(Event event) {
        ...
        if (focusIndex != -1) {
            ...
            Rectangle bounds = ((SwtTableItem) focusItem.getImpl()).getBoundsInPixels(0);
            if (((SwtItem) focusItem.getImpl()).text != null && ((SwtItem) focusItem.getImpl()).text.length() != 0) {
                bounds = ((SwtTableItem) focusItem.getImpl()).getBoundsInPixels();
            }
    ...
    void updateMoveable() {
        int index = 0;
        while (index < columnCount) {
            if (((SwtTableColumn) columns[index].getImpl()).moveable)
                break;
            index++;
        }
        int newBits = index < columnCount ? OS.LVS_EX_HEADERDRAGDROP : 0;
        OS.SendMessage(getApi().handle, OS.LVM_SETEXTENDEDLISTVIEWSTYLE, OS.LVS_EX_HEADERDRAGDROP, newBits);
    }
        ...
        if (super.updateTextDirection(textDirection)) {
            if (textDirection == AUTO_TEXT_DIRECTION || (getApi().state & HAS_AUTO_DIRECTION) != 0) {
                for (TableItem item : items) {
                    if (item != null) {
                        ((SwtItem) item.getImpl()).updateTextDirection(textDirection == AUTO_TEXT_DIRECTION ? AUTO_TEXT_DIRECTION : getApi().style & SWT.FLIP_TEXT_DIRECTION);
                    }
                }
            }
            OS.InvalidateRect(getApi().handle, null, true);
	...
	*/
    ...
    long windowProc(long hwnd, int msg, long wParam, long lParam) {
                ...
                case OS.WM_SETCURSOR:
                    {
                        if (wParam == hwnd) {
                            ...
                            if (hitTest == OS.HTCLIENT) {
                                ...
                                if (0 <= index && index < columnCount && !((SwtTableColumn) columns[index].getImpl()).resizable) {
		...
		*/
            int topIndex = (int) OS.SendMessage(getApi().handle, OS.LVM_GETTOPINDEX, 0, 0);
            int selection = (int) OS.SendMessage(getApi().handle, OS.LVM_GETNEXTITEM, topIndex - 1, OS.LVNI_SELECTED);
            if (selection == -1)
                return 0;
    ...
    LRESULT WM_CHAR(long wParam, long lParam) {
        ...
        switch((int) wParam) {
            case ' ':
                if ((getApi().style & SWT.CHECK) != 0) {
                    int index = -1;
                    while ((index = (int) OS.SendMessage(getApi().handle, OS.LVM_GETNEXTITEM, index, OS.LVNI_SELECTED)) != -1) {
                        TableItem item = _getItem(index);
                        ((SwtTableItem) item.getImpl()).setChecked(!item.getChecked(), true);
                        OS.NotifyWinEvent(OS.EVENT_OBJECT_FOCUS, getApi().handle, OS.OBJID_CLIENT, index + 1);
                    }
                }
                /*
	...
	*/
        if (!((SwtDisplay) display.getImpl()).runDragDrop)
            return LRESULT.ZERO;
	...
	* the table.
	*/
        if (imageList != null || (getApi().style & SWT.CHECK) != 0) {
	...
	*/
        ...
        pinfo.x = OS.GET_X_LPARAM(lParam);
        pinfo.y = OS.GET_Y_LPARAM(lParam);
        int index = (int) OS.SendMessage(getApi().handle, OS.LVM_HITTEST, 0, pinfo);
        Display display = this.display;
        ((SwtDisplay) display.getImpl()).captureChanged = false;
        ...
        if (pinfo.iItem != -1)
            callWindowProc(getApi().handle, OS.WM_LBUTTONDBLCLK, wParam, lParam);
        if (!((SwtDisplay) display.getImpl()).captureChanged && !isDisposed()) {
            if (OS.GetCapture() != getApi().handle)
                OS.SetCapture(getApi().handle);
        }
	...
	*/
        ...
        Display display = this.display;
        ((SwtDisplay) display.getImpl()).captureChanged = false;
        sendMouseEvent(SWT.MouseDown, 3, getApi().handle, lParam);
        if (sendMouseEvent(SWT.MouseDoubleClick, 3, getApi().handle, lParam)) {
            if (pinfo.iItem != -1)
                callWindowProc(getApi().handle, OS.WM_RBUTTONDBLCLK, wParam, lParam);
        }
        if (!((SwtDisplay) display.getImpl()).captureChanged && !isDisposed()) {
            if (OS.GetCapture() != getApi().handle)
                OS.SetCapture(getApi().handle);
	...
	*/
        ...
        if (index == -1) {
            ...
            lvItem.stateMask = OS.LVIS_FOCUSED;
            ignoreSelect = true;
            OS.SendMessage(getApi().handle, OS.LVM_SETITEMSTATE, 0, lvItem);
            ignoreSelect = false;
        }
        return result;
    }
	...
	*/
        OS.DefWindowProc(getApi().handle, OS.WM_SETREDRAW, wParam, lParam);
        long code = callWindowProc(getApi().handle, OS.WM_SETREDRAW, wParam, lParam);
        if (wParam == 0) {
            if ((int) OS.SendMessage(getApi().handle, OS.LVM_GETBKCOLOR, 0, 0) == OS.CLR_NONE) {
                OS.SendMessage(getApi().handle, OS.LVM_SETBKCOLOR, 0, 0xFFFFFF);
            }
	...
	*/
        ...
        if (OS.LOWORD(wParam) != OS.SB_ENDSCROLL) {
            if (columnCount > H_SCROLL_LIMIT) {
                int rowCount = (int) OS.SendMessage(getApi().handle, OS.LVM_GETCOUNTPERPAGE, 0, 0);
                if (rowCount > V_SCROLL_LIMIT)
                    fixScroll = getDrawing() && OS.IsWindowVisible(getApi().handle);
            }
        }
        if (fixScroll)
            OS.DefWindowProc(getApi().handle, OS.WM_SETREDRAW, 0, 0);
		...
		*/
            ...
            for (int i = 0; i < columnCount; i++) {
                ...
                if (OS.SendMessage(getApi().handle, OS.LVM_GETSUBITEMRECT, 0, headerRect) != 0) {
        ...
        switch(hdr.code) {
			...
			*/
				...
				*/
                        ...
                        rect.left = OS.LVIR_BOUNDS;
                        ignoreCustomDraw = true;
                        long code = OS.SendMessage(getApi().handle, OS.LVM_GETITEMRECT, plvfi.iItem, rect);
                        ignoreCustomDraw = false;
                        if (code != 0)
                            OS.InvalidateRect(getApi().handle, rect, true);
                        break;
			...
			*/
					...
					*/
                            ...
                            if (length > 1 && (getApi().state & HAS_AUTO_DIRECTION) != 0) {
                            ...
                            char[] buffer = ((SwtDisplay) display.getImpl()).tableBuffer;
                            if (buffer == null || plvfi.cchTextMax > buffer.length) {
                                buffer = ((SwtDisplay) display.getImpl()).tableBuffer = new char[plvfi.cchTextMax];
                            }
                    ...
                    if ((plvfi.mask & OS.LVIF_STATE) != 0) {
                        if (plvfi.iSubItem == 0) {
                            int state = 1;
                            if (((SwtTableItem) item.getImpl()).checked)
                                state++;
                            if (((SwtTableItem) item.getImpl()).grayed)
                                state += 2;
                            if (!OS.IsWindowEnabled(getApi().handle))
                                state += 4;
                    ...
                    if ((plvfi.mask & OS.LVIF_INDENT) != 0) {
                        if (plvfi.iSubItem == 0) {
                            plvfi.iIndent = ((SwtTableItem) item.getImpl()).imageIndent;
                            move = true;
                        }
                    }
                    if (move)
                    ...
                    if (hooks(SWT.EraseItem) || hooks(SWT.PaintItem)) {
                        ...
                        if (count != 0) {
                            ...
                            if (pnmlv.iItem != -1) {
                                ...
                                itemRect.left = OS.LVIR_BOUNDS;
                                ignoreCustomDraw = true;
                                OS.SendMessage(getApi().handle, OS.LVM_GETITEMRECT, pnmlv.iItem, itemRect);
                                ignoreCustomDraw = false;
                                RECT headerRect = new RECT();
                                int index = (int) OS.SendMessage(hwndHeader, OS.HDM_ORDERTOINDEX, count - 1, 0);
                                OS.SendMessage(hwndHeader, OS.HDM_GETITEMRECT, index, headerRect);
            ...
            case OS.NM_CUSTOMDRAW:
                {
					 ...
					 */
						...
						OS.DeleteObject (pen);
						*/
                                    int lineColor = (((SwtDisplay) display.getImpl()).tableHeaderLinePixel != -1) ? ((SwtDisplay) display.getImpl()).tableHeaderLinePixel : OS.GetSysColor(OS.COLOR_3DFACE);
                                    ...
                                    if (((SwtItem) columns[i].getImpl()).image != null) {
                                        ...
                                        data.device = display;
                                        GC gc = createNewGC(nmcd.hdc, data);
                                        int y = Math.max(0, (nmcd.bottom - Win32DPIUtils.pointToPixel(((SwtItem) columns[i].getImpl()).image.getBounds(), getAutoscalingZoom()).height) / 2);
                                        int zoom = getAutoscalingZoom();
                                        gc.drawImage(((SwtItem) columns[i].getImpl()).image, DPIUtil.pixelToPoint(x, zoom), DPIUtil.pixelToPoint(y, zoom));
                                        x += Win32DPIUtils.pointToPixel(((SwtItem) columns[i].getImpl()).image.getBounds(), getAutoscalingZoom()).width + 12;
                                        gc.dispose();
                                    }
                    ...
                    if (!ignoreColumnResize) {
                        ...
                        if (phdn.pitem != 0) {
                            ...
                            if ((pitem.mask & OS.HDI_WIDTH) != 0) {
                                TableColumn column = columns[phdn.iItem];
                                if (column != null) {
                                    ((SwtTableColumn) column.getImpl()).updateToolTip(phdn.iItem);
							...
							*/
                                    ...
                                    for (int i = 0; i < columnCount; i++) {
                                        ...
                                        if (moved && !nextColumn.isDisposed()) {
                                            ((SwtTableColumn) nextColumn.getImpl()).updateToolTip(order[i]);
	...
	*/
    ...
    void destroyImageList() {
        ...
        long hImageList = OS.ImageList_Create(1, 1, 0, 0, 0);
        long oldStateImageList = OS.SendMessage(getApi().handle, OS.LVM_SETIMAGELIST, OS.LVSIL_STATE, hImageList);
        long oldhImageList = OS.SendMessage(getApi().handle, OS.LVM_SETIMAGELIST, OS.LVSIL_SMALL, hImageList);
