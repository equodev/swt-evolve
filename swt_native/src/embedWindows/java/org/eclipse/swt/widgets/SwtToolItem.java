...
public class SwtToolItem extends SwtItem implements IToolItem {
	...
	*/
    ...
    @Override
    void destroyWidget() {
        ((SwtToolBar) parent.getImpl()).destroyItem(this.getApi());
        releaseHandle();
    }
    ...
    public Color getBackground() {
        checkWidget();
        return SwtColor.win32_new(display, ((SwtToolBar) parent.getImpl()).getBackgroundPixel(this.getApi()));
    }
    ...
    public Color getForeground() {
        checkWidget();
        return SwtColor.win32_new(display, ((SwtToolBar) parent.getImpl()).getForegroundPixel(this.getApi()));
    }
	...
	*/
        ...
        if (!enabled && ((SwtToolBar) parent.getImpl()).lastFocusId == id) {
            ((SwtToolBar) parent.getImpl()).lastFocusId = -1;
        }
    }
        ...
        if (string.length() != 0) {
            ...
            if ((getApi().style & SWT.FLIP_TEXT_DIRECTION) != 0) {
                ...
                if ((bits & OS.WS_EX_LAYOUTRTL) != 0) {
                    buffer = new TCHAR(((SwtControl) parent.getImpl()).getCodePage(), LRE + string, true);
                } else {
                    buffer = new TCHAR(((SwtControl) parent.getImpl()).getCodePage(), RLE + string, true);
                }
            } else {
                buffer = new TCHAR(((SwtControl) parent.getImpl()).getCodePage(), string, true);
            }
	...
	*/
        ((SwtToolBar) parent.getImpl()).setDropDownItems(false);
        long hwnd = parent.handle;
        long hFont = OS.SendMessage(hwnd, OS.WM_GETFONT, 0, 0);
        OS.SendMessage(hwnd, OS.WM_SETFONT, hFont, 0);
        ((SwtToolBar) parent.getImpl()).setDropDownItems(true);
        ((SwtToolBar) parent.getImpl()).layoutItems();
    }
    ...
    void updateImages(boolean enabled) {
        ...
        if (info.iImage == OS.I_IMAGENONE) {
            ...
            if (imageList == null) {
                imageList = ((SwtDisplay) display.getImpl()).getImageListToolBar(listStyle, boundsInPoints.width, boundsInPoints.height, getAutoscalingZoom());
            }
            if (disabledImageList == null) {
                disabledImageList = ((SwtDisplay) display.getImpl()).getImageListToolBarDisabled(listStyle, boundsInPoints.width, boundsInPoints.height, getAutoscalingZoom());
            }
            if (hotImageList == null) {
                hotImageList = ((SwtDisplay) display.getImpl()).getImageListToolBarHot(listStyle, boundsInPoints.width, boundsInPoints.height, getAutoscalingZoom());
            }
