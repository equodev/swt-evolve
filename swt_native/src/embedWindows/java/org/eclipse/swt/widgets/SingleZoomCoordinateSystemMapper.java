...
class SingleZoomCoordinateSystemMapper implements CoordinateSystemMapper {
    ...
    private int getZoomLevelForMapping(Control from, Control to) {
        ...
        if (to != null) {
            return to.getImpl().getAutoscalingZoom();
        }
        return from.getImpl().getAutoscalingZoom();
    }
    ...
    public Rectangle map(Control from, Control to, Rectangle rectangle) {
        ...
        return Win32DPIUtils.pixelToPoint(((SwtDisplay) display.getImpl()).mapInPixels(from, to, rectangle), zoom);
    }
