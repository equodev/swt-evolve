...
public class SwtTaskItem extends SwtItem implements ITaskItem {
    ...
    @Override
    void destroyWidget() {
        if (null != ((SwtTaskBar) parent.getImpl()).mTaskbarList3) {
    ...
    public void setMenu(Menu menu) {
        ...
        if (shell != null)
            return;
        this.menu = menu;
        ((SwtTaskBar) parent.getImpl()).setMenu(menu);
    }
    ...
    void updateImage() {
        ...
        switch(overlayImage.type) {
            case SWT.BITMAP:
                image2 = SwtDisplay.createIcon(overlayImage, getAutoscalingZoom());
                hIcon = SwtImage.win32_getHandle(image2, getAutoscalingZoom());
                break;
            case SWT.ICON:
                hIcon = SwtImage.win32_getHandle(overlayImage, getAutoscalingZoom());
                break;
        }
        ((SwtTaskBar) parent.getImpl()).mTaskbarList3.SetOverlayIcon(shell.handle, hIcon, 0);
        if (image2 != null)
            image2.dispose();
    }
    ...
    void updateProgress() {
        ...
        if (progressState == SWT.DEFAULT)
            return;
        ((SwtTaskBar) parent.getImpl()).mTaskbarList3.SetProgressValue(shell.handle, progress, PROGRESS_MAX);
    }
    ...
    void updateProgressState() {
        ...
        ((SwtTaskBar) parent.getImpl()).mTaskbarList3.SetProgressValue(shell.handle, progress, PROGRESS_MAX);
        ((SwtTaskBar) parent.getImpl()).mTaskbarList3.SetProgressState(shell.handle, tbpFlags);
    }
