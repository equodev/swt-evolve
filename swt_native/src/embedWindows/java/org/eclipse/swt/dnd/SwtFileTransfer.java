 ...
 * @see Transfer
 */
public class SwtFileTransfer extends SwtByteArrayTransfer implements IFileTransfer {
    ...
    private long generateCidaFromFilepaths(String[] fileNames) {
        ...
        try {
            ...
            // offsets are from cida begin so the first is equal to cida size
            cida.aoffset = cidaSize;
            int[] pidlOffsets = new int[n];
            int[] pidlSizes = new int[n];
            // initialize with 2 for the empty (but double null terminated) parent pidl
            int pidlSizeSum = 2;
            for (int i = 0; i < n; i++) {
