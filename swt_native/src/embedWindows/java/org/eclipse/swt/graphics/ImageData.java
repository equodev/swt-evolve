 ...
 */
    ...
    static void fillGradientRectangle(GC gc, Device device, int x, int y, int width, int height, boolean vertical, RGB fromRGB, RGB toRGB, int redBits, int greenBits, int blueBits, int zoom) {
        ...
        ImageDataAtSizeProvider imageDataAtSizeProvider = (imageWidth, imageHeight) -> {
            return createGradientBand(imageWidth, imageHeight, vertical, fromRGB, toRGB, redBits, greenBits, blueBits);
        };
        Image image = new Image(device, imageDataAtSizeProvider);
        ImageData band = image.getImageData(zoom);
        if ((band.width == 1) || (band.height == 1)) {
            gc.drawImage(image, DPIUtil.pixelToPoint(x, zoom), DPIUtil.pixelToPoint(y, zoom), DPIUtil.pixelToPoint(width, zoom), DPIUtil.pixelToPoint(height, zoom));
        } else {
            if (vertical) {
                for (int dx = 0; dx < width; dx += band.width) {
                    ...
                    gc.drawImage(image, DPIUtil.pixelToPoint(dx + x, zoom), DPIUtil.pixelToPoint(y, zoom), DPIUtil.pixelToPoint(blitWidth, zoom), DPIUtil.pixelToPoint(band.height, zoom));
                }
            } else {
                for (int dy = 0; dy < height; dy += band.height) {
                    ...
                    gc.drawImage(image, DPIUtil.pixelToPoint(x, zoom), DPIUtil.pixelToPoint(dy + y, zoom), DPIUtil.pixelToPoint(band.width, zoom), DPIUtil.pixelToPoint(blitHeight, zoom));
                }
            }
        }
        image.dispose();
