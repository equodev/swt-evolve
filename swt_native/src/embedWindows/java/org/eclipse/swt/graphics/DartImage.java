...
public final class DartImage extends DartResource implements Drawable, IImage {
            ...
            case SWT.IMAGE_COPY:
                {
                    switch(getApi().type) {
                        case SWT.BITMAP:
                            for (InternalImageHandle imageHandle : ((DartImage) srcImage.getImpl()).imageHandleManager.getAllImageHandles()) {
    ...
    void destroyHandlesExcept(Set<Integer> zoomLevels) {
        imageHandleManager.destroyHandles(zoom -> !zoomLevels.contains(zoom));
    }
    ...
    Rectangle getBounds(int zoom) {
        if (isDisposed())
            SWT.error(SWT.ERROR_GRAPHIC_DISPOSED);
        return new Rectangle(0, 0, this.imageData.width, this.imageData.height);
    }
