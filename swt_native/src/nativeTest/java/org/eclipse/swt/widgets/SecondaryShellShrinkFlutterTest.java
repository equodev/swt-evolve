 ...
 */
@Tag("flutter-it")
class SecondaryShellShrinkFlutterTest {
    ...
    void secondaryDialog_neverCallingSetSize_doesNotShrinkMainShell() {
        ...
        // A secondary Shell owned by main, never sized/packed,
        // just opened.
    ...
    void parentlessSecondaryShell_explicitBounds_doesNotShrinkMainShellOrReachNativeWindow() {
        // A parentless secondary Shell (owned by the Display
        ...
        Shell recorder = new Shell(display, SWT.SHELL_TRIM); // no parent
        recorder.open();
        recorder.setBounds(569, 288, 590, 425); // the dialog sizes itself explicitly
        ...
        assertThat(recorder.getBounds())
                .as("the recorder keeps its own explicit geometry")
    ...
    void redundantSetLocation_onChromiumMainWindow_isNotForwarded() {
        // A dialog centering flow that calls setLocation(x, y) on the
