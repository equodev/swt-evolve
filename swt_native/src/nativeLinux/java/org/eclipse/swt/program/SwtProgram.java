...
public final class SwtProgram implements IProgram {
    ...
    public ImageData getImageData(int zoom) {
        ...
        if (gicon != 0) {
			 ...
			 */
                // default display needs to exist
                ...
                long gicon_info = GTK3.gtk_icon_theme_lookup_by_gicon(icon_theme, gicon, 16, /*size*/
                0);
                if (gicon_info != 0) {
    ...
    static Program gio_getProgram(long application) {
        ...
        if (applicationName != 0) {
            ...
            if (length > 0) {
                ...
                ((SwtProgram) program.getImpl()).name = new String(Converter.mbcsToWcs(buffer));
            }
        }
        ...
        if (icon != 0) {
            ...
            if (icon_name != 0) {
                ...
                if (length > 0) {
                    ...
                    ((SwtProgram) program.getImpl()).iconPath = new String(Converter.mbcsToWcs(buffer));
                }
                OS.g_free(icon_name);
            }
        }
        return ((SwtProgram) program.getImpl()).command != null ? program : null;
    }
    ...
    static boolean isExecutable(String fileName) {
        ...
        if (file != 0) {
            ...
            if (fileInfo != 0) {
                ...
                if (contentType != 0) {
                    //$NON-NLS-1$
                    byte[] exeType = Converter.wcsToMbcs("application/x-executable", true);
    ...
    boolean gio_execute(String fileName) {
        ...
        long application = OS.g_app_info_create_from_commandline(commandBuffer, nameBuffer, gioExpectUri ? OS.G_APP_INFO_CREATE_SUPPORTS_URIS : OS.G_APP_INFO_CREATE_NONE, 0);
        if (application != 0) {
            byte[] fileNameBuffer = Converter.wcsToMbcs(fileName, true);
            long file = 0;
    ...
    public static boolean launch(String fileName, String workingDir) {
        ...
        if (index != -1) {
            ...
            Program program = SwtProgram.findProgram(extension);
        ...
        if (lowercaseName.startsWith(PREFIX_HTTP) || lowercaseName.startsWith(PREFIX_HTTPS)) {
            //$NON-NLS-1$
            Program program = SwtProgram.findProgram(".html");
            if (program == null) {
                //$NON-NLS-1$
                program = SwtProgram.findProgram(".htm");
            }
            if (program != null && program.execute(fileName))
                return true;
        }
    ...
    public boolean equals(Object other) {
        ...
        return name.equals(((SwtProgram) program.getImpl()).name) && command.equals(((SwtProgram) program.getImpl()).command) && gioExpectUri == ((SwtProgram) program.getImpl()).gioExpectUri;
    }
