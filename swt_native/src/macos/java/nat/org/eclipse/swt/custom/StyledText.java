 ...
 */
public class StyledText extends Canvas implements IStyledText {
    ...
    static class Printing implements Runnable {
            ...
            for (int i = 0; i < lineCount; i++) {
                ...
                if (color != null) {
                    if (printOptions.printLineBackground) {
                        ...
                        printerRenderer.setLineBackground(i, 1, printerColor.getApi());
                    } else {
        ...
        void init() {
            ...
            printerRenderer.setFont(printerFont.getApi(), tabLength);
        ...
        void print() {
            ...
            for (int i = startLine; i <= endLine && page <= endPage; i++) {
                if (paintY == clientArea.y) {
                    printer.startPage();
                    printDecoration(page, true, printLayout.getApi());
                }
                TextLayout layout = printerRenderer.getTextLayout(i, orientation, width, lineSpacing);
                Color lineBackground = printerRenderer.getLineBackground(i, background.getApi());
                ...
                if (paragraphBottom <= pageBottom) {
                    ...
                    printLine(paintX, paintY, gc.getApi(), foreground.getApi(), lineBackground.getApi(), layout.getApi(), printLayout.getApi(), i);
                    ...
                    if (lineCount == 0) {
                        ...
                        printDecoration(page, false, printLayout.getApi());
                        printer.endPage();
                        page++;
                        if (page <= endPage) {
                            printer.startPage();
                ...
                printerRenderer.disposeTextLayout(layout.getApi());
            }
        ...
        void printDecoration(int page, boolean header, TextLayout layout) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                if (segmentIndex == -1) {
                    ...
                    printDecorationSegment(segment, i, page, header, layout.getApi());
                    break;
                } else {
    ...
    public Point computeSize(int wHint, int hHint, boolean changed) {
        ...
        if (wHint == SWT.DEFAULT || hHint == SWT.DEFAULT) {
            ...
            for (int lineIndex = 0; lineIndex < lineCount; lineIndex++) {
                ...
                renderer.disposeTextLayout(layout.getApi());
 ...
 */
    ...
    void handlePaint(Event event) {
        ...
        if (endY > 0) {
            ...
            y += renderer.drawLines(startLine, endLine, x, y, endY, gc.getApi(), background.getApi(), foreground.getApi());
            if (y < endY) {
                gc.setBackground(background);
            ...
            public void getTextBounds(AccessibleTextEvent e) {
                ...
                for (int lineIndex = startLine; lineIndex <= endLine; lineIndex++) {
                    ...
                    if (lineIndex == endLine) {
                        ...
                        st.renderer.disposeTextLayout(layout.getApi());
                    }
 ...
 */
     ...
     * </ul>
     */
    public void setLineBackground(int startLine, int lineCount, IColor ibackground) {
        Color background = (Color) ibackground;
        checkWidget();
        if (isListening(ST.LineGetBackground))
            return;
        if (startLine < 0 || startLine + lineCount > content.getLineCount()) {
        ...
        if (background != null) {
            renderer.setLineBackground(startLine, lineCount, background.getApi());
        } else {
     ...
     * @since 3.2
     */
    public void setLineBullet(int startLine, int lineCount, IBullet ibullet) {
        Bullet bullet = (Bullet) ibullet;
        checkWidget();
        if (isListening(ST.LineGetStyle))
            return;
        if (startLine < 0 || startLine + lineCount > content.getLineCount()) {
        ...
        renderer.setLineBullet(startLine, lineCount, bullet.getApi());
        resetCache(startLine, lineCount);
     ...
     * @since 3.5
     */
    public void setMarginColor(IColor icolor) {
        Color color = (Color) icolor;
        checkWidget();
     ...
     * @since 2.1
     */
    public void setSelectionBackground(IColor icolor) {
        Color color = (Color) icolor;
        checkWidget();
        if (color != null) {
            if (color.isDisposed())
     ...
     * @since 2.1
     */
    public void setSelectionForeground(IColor icolor) {
        Color color = (Color) icolor;
        checkWidget();
        if (color != null) {
            if (color.isDisposed())
