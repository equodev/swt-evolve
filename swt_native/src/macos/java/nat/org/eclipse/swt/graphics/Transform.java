 ...
 * @since 3.1
 */
public class Transform extends Resource implements ITransform {
     ...
     * </ul>
     */
    public void multiply(ITransform imatrix) {
        Transform matrix = (Transform) imatrix;
        if (isDisposed())
            SWT.error(SWT.ERROR_GRAPHIC_DISPOSED);
        if (matrix == null)
            SWT.error(SWT.ERROR_NULL_ARGUMENT);
    ...
    public static Transform safeDelegate(org.eclipse.swt.graphics.Transform api) {
        return (api != null) ? (Transform) api.getDelegate() : null;
    }
}
