 ...
 */
public final class Region extends Resource implements IRegion {
     ...
     * </ul>
     */
    public void add(IRegion iregion) {
     ...
     * @since 3.0
     */
    public void intersect(IRegion iregion) {
        Region region = (Region) iregion;
        if (isDisposed())
            SWT.error(SWT.ERROR_GRAPHIC_DISPOSED);
        if (region == null)
     ...
     * @since 3.0
     */
    public void subtract(IRegion iregion) {
        Region region = (Region) iregion;
        if (isDisposed())
            SWT.error(SWT.ERROR_GRAPHIC_DISPOSED);
        if (region == null)
    ...
    public static Region safeDelegate(org.eclipse.swt.graphics.Region api) {
        return (api != null) ? (Region) api.getDelegate() : null;
    }
}
