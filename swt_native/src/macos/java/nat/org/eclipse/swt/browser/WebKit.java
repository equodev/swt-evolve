 ...
 * *****************************************************************************
 */
package nat.org.eclipse.swt.browser;
...
import nat.org.eclipse.swt.layout.*;
...
class WebKit extends WebBrowser {
    ...
    /* Safari version on OSX 10.5 initial release */
    //$NON-NLS-1$
    ...
    public void create(Composite parent, int style) {
        if (delegateClass == 0) {
            ...
            //$NON-NLS-1$
            Callback4 = new Callback(webKitClass, "browserProc", 4);
            long proc4 = Callback4.getAddress();
            //$NON-NLS-1$
            Callback5 = new Callback(webKitClass, "browserProc", 5);
            long proc5 = Callback5.getAddress();
            //$NON-NLS-1$
            Callback6 = new Callback(webKitClass, "browserProc", 6);
            long proc6 = Callback6.getAddress();
            //$NON-NLS-1$
            Callback7 = new Callback(webKitClass, "browserProc", 7);
            ...
            //$NON-NLS-1$
            OS.class_addMethod(delegateClass, OS.sel_webView_frame_exceededDatabaseQuotaForSecurityOrigin_database_, proc6, "@:@@@@");
            //$NON-NLS-1$
            ...
            //$NON-NLS-1$
            OS.class_addMethod(delegateClass, OS.sel_callJava, proc6, "@:@@@@");
            //$NON-NLS-1$
	...
	*/
        ...
        /* needed for tabbing into the Browser */
    ...
    static void webView_frame_exceededDatabaseQuotaForSecurityOrigin_database(long /* int */
    sender, long /* int */
    frame, long /* int */
    origin, long /* int */
    database) {
    ...
    boolean close(boolean showPrompters) {
        ...
        // $NON-NLS-1$ // $NON-NLS-2$
        Boolean result = (Boolean) evaluate("return " + functionName + "(window);");
        if (result == null)
            return false;
    ...
    public boolean execute(String script) {
        ...
        //$NON-NLS-1$
        byte[] bytes = (script + '\0').getBytes(StandardCharsets.UTF_8);
        long scriptString = OS.JSStringCreateWithUTF8CString(bytes);
        //$NON-NLS-1$
        bytes = (getUrl() + '\0').getBytes(StandardCharsets.UTF_8);
	...
	*/
    ...
    void webView_didFailProvisionalLoadWithError_forFrame(long sender, long error, long frame) {
		...
		*/
        ...
        if (info != null) {
            //$NON-NLS-1$
            id id = info.valueForKey(NSString.stringWith("NSErrorFailingURLKey"));
            if (id != null)
        ...
        if (description != null) {
            ...
            //$NON-NLS-1$
            message += Compatibility.getMessage("SWT_Page_Load_Failed", new Object[] { descriptionString });
	...
	*/
    ...
    boolean webView_runBeforeUnloadConfirmPanelWithMessage_initiatedByFrame(long sender, long messageID, long frame) {
        ...
        //$NON-NLS-1$
        StringBuilder text = new StringBuilder(Compatibility.getMessage("SWT_OnBeforeUnload_Message1"));
        //$NON-NLS-1$
        text.append("\n\n");
	...
	*/
    ...
    Object convertToJava(long value) {
        ...
        //$NON-NLS-1$
        long clazz = OS.objc_lookUpClass("NSString");
