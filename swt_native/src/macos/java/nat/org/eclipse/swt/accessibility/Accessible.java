...
import org.eclipse.swt.accessibility.IAccessible;
...
public class Accessible implements IAccessible {
     ...
     * @since 3.6
     */
    public void addRelation(int type, IAccessible itarget) {
        Accessible target = (Accessible) itarget;
        checkWidget();
        if (target == null)
            SWT.error(SWT.ERROR_NULL_ARGUMENT);
    ...
    id getHeaderAttribute(int childID) {
        ...
        if (Accessible.safeDelegate(tableEvent.accessible) != null)
            returnValue = Accessible.safeDelegate(tableEvent.accessible).delegate;
        return returnValue;
    }
        ...
        if (event.selected != null) {
            ...
            for (int i = 0; i < selected.length; i++) {
                ...
                if (Accessible.safeDelegate(event.accessible) != null)
                    array.addObject(Accessible.safeDelegate(event.accessible).delegate);
            }
            returnValue = array;
        }
        return returnValue == null ? NSArray.array() : returnValue;
    ...
    id getRowsAttribute(int childID) {
        ...
        if (event.accessibles == null)
            event.accessibles = Convert.array(new Accessible[0], org.eclipse.swt.accessibility.IAccessible::getApi, org.eclipse.swt.accessibility.Accessible[]::new);
    ...
    public id internal_accessibilityFocusedUIElement(int childID) {
        ...
        if (Accessible.safeDelegate(event.accessible) != null) {
            return new id(OS.NSAccessibilityUnignoredAncestor(Accessible.safeDelegate(event.accessible).control.view.id));
        }
    ...
    public id internal_accessibilityHitTest(NSPoint point, int childID) {
        ...
        // The listener did not respond, so let Cocoa figure it out.
        if (event.childID == ACC.CHILDID_MULTIPLE && Accessible.safeDelegate(event.accessible) == null)
            return null;
        if (Accessible.safeDelegate(event.accessible) != null) {
            return new id(OS.NSAccessibilityUnignoredAncestor(Accessible.safeDelegate(event.accessible).delegate.id));
        }
        if (event.childID == ACC.CHILDID_SELF || event.childID == ACC.CHILDID_NONE) {
            return new id(OS.NSAccessibilityUnignoredAncestor(control.view.id));
        }
    ...
    id getAttributedStringForRangeParameterizedAttribute(id parameter, int childID) {
        ...
        while (event.offset < range.location + range.length) {
            ...
            if (TextStyle.safeDelegate(event.textStyle) != null) {
                TextStyle ts = TextStyle.safeDelegate(event.textStyle);
                if (ts.font != null) {
                ...
                if (ts.underline) {
                    ...
                    NSNumber styleObj = switch(style) {
                        case SWT.UNDERLINE_SINGLE ->
                            NSNumber.numberWithInt(OS.kAXUnderlineStyleSingle);
                        case SWT.UNDERLINE_DOUBLE ->
                            NSNumber.numberWithInt(OS.kAXUnderlineStyleDouble);
                        ...
                        default ->
                            NSNumber.numberWithInt(OS.kAXUnderlineStyleNone);
                    };
        ...
        if (docAttributes.indent != Integer.MAX_VALUE) {
            ...
            int osAlignment = switch(docAttributes.alignment) {
                case SWT.CENTER ->
                    OS.NSTextAlignmentCenter;
                case SWT.RIGHT ->
                    OS.NSTextAlignmentRight;
                case SWT.LEFT ->
                    OS.NSTextAlignmentLeft;
                default ->
                    OS.NSTextAlignmentLeft;
            };
    ...
    id getValueAttribute(int childID) {
        ...
        if (accessibleValueListenersSize() > 0) {
            ...
            switch(role) {
                ...
                case ACC.ROLE_SLIDER:
                case // the value associated with the fill level of the progress bar
    ...
    id getFocusedAttribute(int childID) {
        ...
        /* The application can optionally answer an accessible. */
        if (Accessible.safeDelegate(event.accessible) != null) {
            boolean hasFocus = (Accessible.safeDelegate(event.accessible).index == childID) && (Accessible.safeDelegate(event.accessible).control == this.control);
    ...
    id getCellForColumnAndRowParameter(id parameter, int childID) {
        ...
        if (parameterObject.count() == 2) {
            ...
            for (int i = 0; i < accessibleTableListenersSize(); i++) {
                ...
                returnValue = Accessible.safeDelegate(event.accessible).delegate;
            }
        }
        return returnValue;
    }
     ...
     * @since 3.6
     */
    public void removeRelation(int type, IAccessible itarget) {
        Accessible target = (Accessible) itarget;
        checkWidget();
        if (target == null)
            SWT.error(SWT.ERROR_NULL_ARGUMENT);
    ...
    String roleToOs(int role) {
        ...
        switch(role) {
            ...
            case ACC.ROLE_SCROLLBAR:
                ...
                break;
            case ACC.ROLE_DIALOG:
                nsReturnValue = concatStringsAsRole(OS.NSAccessibilityWindowRole, OS.NSAccessibilityDialogSubrole);
                break;
            case ACC.ROLE_LABEL:
            ...
            case ACC.ROLE_TREE:
                ...
                break;
            case ACC.ROLE_TREEITEM:
                nsReturnValue = concatStringsAsRole(OS.NSAccessibilityOutlineRole, OS.NSAccessibilityOutlineRowSubrole);
                break;
            case ACC.ROLE_TABFOLDER:
    ...
    public static Accessible safeDelegate(org.eclipse.swt.accessibility.Accessible api) {
        return (api != null) ? (Accessible) api.getDelegate() : null;
    }
}
