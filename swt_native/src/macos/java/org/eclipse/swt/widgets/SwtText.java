...
public class SwtText extends SwtScrollable implements IText {
	...
	*/
    ...
    public SwtText(Composite parent, int style) {
		...
		*/
            ...
            if ((style & SWT.ICON_CANCEL) != 0) {
                ...
                cancelCell.setTarget(getApi().view);
            ...
            if ((style & SWT.ICON_SEARCH) != 0) {
                ...
                searchCell.setTarget(getApi().view);
    ...
    public void copy() {
        ...
        if ((style & SWT.SINGLE) != 0) {
            ...
            NSText text = (NSText) getApi().view;
    ...
    public void cut() {
        ...
        if (cut) {
            if ((style & SWT.SINGLE) != 0) {
                ...
                ((NSTextView) getApi().view).cut(null);
            }
        }
	 ...
	 */
        ...
        // Default height of search field on Cocoa
        ...
        NSSearchField searchField = ((NSSearchField) getApi().view);
        NSCell _cell = (NSCell) searchField.cell();
        SWTSearchFieldCell cell = new SWTSearchFieldCell(_cell.id);
        if (cell.searchButtonCell() != null) {
            cell.searchButtonCell().drawInteriorWithFrame(cell.searchButtonRectForBounds(cellFrame), getApi().view);
        }
        if (cell.cancelButtonCell() != null && ((NSSearchField) getApi().view).stringValue().length() > 0) {
            cell.cancelButtonCell().drawInteriorWithFrame(cell.cancelButtonRectForBounds(cellFrame), getApi().view);
        }
    }
        ...
        if (paste) {
            if ((style & SWT.SINGLE) != 0) {
                ...
                if (textLimit != Text.LIMIT) {
                    ...
                    ((NSTextView) getApi().view).paste(null);
                }
            }
        }
        sendEvent(SWT.Modify);
    ...
    void sendSearchSelection() {
        if (targetSearch != null) {
            ((NSSearchField) getApi().view).sendAction(actionSearch, targetSearch);
        }
    ...
    void sendCancelSelection() {
        if (targetCancel != null) {
            ((NSSearchField) getApi().view).sendAction(actionCancel, targetCancel);
        }
        Event event = new Event();
    ...
    void setBackgroundColor(NSColor nsColor) {
		 ...
		 */
            ...
            ((NSTextView) getApi().view).setBackgroundColor(nsColor);
        }
    }
	...
	*/
    ...
    public void setSelection(int start, int end) {
        ...
        if ((style & SWT.SINGLE) != 0) {
            ...
            int length = (int) ((NSTextView) getApi().view).textStorage().length();
            int selStart = Math.min(Math.max(Math.min(start, end), 0), length);
            int selEnd = Math.min(Math.max(Math.max(start, end), 0), length);
            NSRange range = new NSRange();
	...
	*/
    ...
    void updateThemeColors() {
        ...
        if (foreground == null) {
            if (getEnabled()) {
                ((NSTextView) getApi().view).setTextColor(NSColor.textColor());
            } else {
                ((NSTextView) getApi().view).setTextColor(NSColor.disabledControlTextColor());
            }
        }
