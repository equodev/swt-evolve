...
public class SwtButton extends SwtControl implements IButton {
    ...
    NSRect drawTitleWithFrameInView(long id, long sel, long title, NSRect titleRect, long view) {
        ...
        if (wrap) {
            ...
            if (isEnabled) {
                foreground2 = foreground == null ? ((SwtDisplay) display.getImpl()).getNSColorRGB(NSColor.textColor()) : foreground;
	...
	*/
    ...
    void sendSelection() {
        ...
        if ((getApi().style & SWT.CHECK) != 0) {
            ...
            if (!grayed && ((NSButton) getApi().view).state() == OS.NSMixedState) {
                ((NSButton) getApi().view).setState(OS.NSOnState);
            }
        }
        sendSelectionEvent(SWT.Selection);
    }
    ...
    public void setGrayed(boolean grayed) {
        ...
        this.grayed = grayed;
        ((NSButton) getApi().view).setAllowsMixedState(grayed);
        if (checked) {
            if (grayed) {
                ((NSButton) getApi().view).setState(OS.NSMixedState);
            } else {
                ((NSButton) getApi().view).setState(OS.NSOnState);
            }
        }
    ...
    public void setImage(Image image) {
		 ...
		 */
            ((NSButton) getApi().view).setImage(image != null ? image.handle : null);
        ...
        if (image != null) {
            if ((getApi().style & (SWT.PUSH | SWT.TOGGLE)) != 0 && (getApi().style & (SWT.FLAT | SWT.WRAP)) == 0) {
                ...
                } else if (((SwtDisplay) display.getImpl()).smallFonts) {
    ...
    public void setSelection(boolean selected) {
        ...
        if (grayed) {
            ...
            ((NSButton) getApi().view).setState(selected ? OS.NSOnState : OS.NSOffState);
        }
    }
