...
public abstract class SwtWidget implements IWidget {
	...
	*/
        ...
        ((SwtDisplay) display.getImpl()).isPainting.addObject(view);
		...
		*/
        ...
        ((SwtDisplay) display.getImpl()).isPainting.removeObjectIdenticalTo(view);
    }
	...
	*/
    ...
    private int calculateKeycode(Event event, NSEvent nsEvent) {
        ...
        // of UCKeyTranslate().
        final int[] modifierTests = new int[] { // Many non-latin layouts have a latin plane on Cmd key in order
        ...
        OS.cmdKey >> 8, // Fallback just in case. It's unknown if it's actually helpful
        // in any layouts.
        0 };
	...
	*/
        OS.objc_msgSend(id, OS.sel_setClipsToBounds_, true);
        if (flag && ((SwtDisplay) display.getImpl()).isPainting.containsObject(view)) {
            NSMutableArray needsDisplay = ((SwtDisplay) display.getImpl()).needsDisplay;
            if (needsDisplay == null) {
                needsDisplay = (NSMutableArray) new NSMutableArray().alloc();
                ((SwtDisplay) display.getImpl()).needsDisplay = needsDisplay = needsDisplay.initWithCapacity(12);
            }
	...
	*/
        OS.objc_msgSend(id, OS.sel_setClipsToBounds_, true);
        if (((SwtDisplay) display.getImpl()).isPainting.containsObject(view)) {
            NSMutableArray needsDisplayInRect = ((SwtDisplay) display.getImpl()).needsDisplayInRect;
            if (needsDisplayInRect == null) {
                needsDisplayInRect = (NSMutableArray) new NSMutableArray().alloc();
                ((SwtDisplay) display.getImpl()).needsDisplayInRect = needsDisplayInRect = needsDisplayInRect.initWithCapacity(12);
            }
