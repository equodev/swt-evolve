...
public class SwtTableItem extends SwtItem implements ITableItem {
    ...
    int calculateWidth(int index, GC gc, boolean rowSelected) {
        ...
        if (image != null)
            size.width += ((SwtTable) parent.getImpl()).imageBounds.width + SwtTable.IMAGE_GAP;
        ...
        if (sendMeasure && ((SwtWidget) parent.getImpl()).hooks(SWT.MeasureItem)) {
            ...
            event.width = width;
            event.height = height;
            if (rowSelected && ((((SwtWidget) parent.getImpl()).style & SWT.HIDE_SELECTION) == 0 || ((SwtControl) parent.getImpl()).hasFocus()))
    ...
    public void setImage(int index, Image image) {
        ...
        if (((SwtTable) parent.getImpl()).imageBounds == null && image != null) {
            ((SwtTable) parent.getImpl()).setItemHeight(image, null, false);
        }
        if (index == 0) {
