...
public class SwtTable extends SwtComposite implements ITable {
    ...
    int calculateWidth(TableItem[] items, int index, GC gc) {
        ...
        for (int i = 0; i < itemCount; i++) {
            ...
            if (item != null && ((SwtTableItem) item.getImpl()).cached) {
                width = Math.max(width, ((SwtTableItem) item.getImpl()).calculateWidth(index, gc, isSelected(index)));
            }
        }
        return width;
    }
    ...
    NSSize cellSize(long id, long sel) {
        ...
        if (hooks(SWT.MeasureItem)) {
            ...
            for (int i = 0; i < columnCount; i++) {
                if (((SwtTableColumn) columns[i].getImpl()).nsColumn.id == tableColumn) {
                    columnIndex = i;
                    break;
                }
            }
	...
	*/
    ...
    void clearCachedWidth(TableItem[] items) {
        ...
        for (int i = 0; i < items.length; i++) {
            if (items[i] != null)
                ((SwtTableItem) items[i].getImpl()).width = -1;
        }
    }
    ...
    void createColumn(TableItem item, int index) {
        ...
        Color[] cellBackground = ((SwtTableItem) item.getImpl()).cellBackground;
        ...
        Color[] cellForeground = ((SwtTableItem) item.getImpl()).cellForeground;
	...
	*/
    ...
    void createItem(TableColumn column, int index) {
        ...
        ((SwtTableColumn) column.getImpl()).nsColumn = nsColumn;
        nsColumn.setWidth(0);
    ...
    NSFont defaultNSFont() {
        return ((SwtDisplay) display.getImpl()).tableViewFont;
    }
    ...
    void destroyItem(TableColumn column) {
        ...
        int oldIndex = indexOf(((SwtTableColumn) column.getImpl()).nsColumn);
        System.arraycopy(columns, index + 1, columns, index, --columnCount - index);
        columns[columnCount] = null;
        if (columnCount == 0) {
            //TODO - reset attributes
            firstColumn = ((SwtTableColumn) column.getImpl()).nsColumn;
            firstColumn.retain();
            /*
		* Feature in Cocoa.  If a column's width is too small to show any content
		* then tableView_objectValueForTableColumn_row is never invoked to
		...
		*/
            ...
            ((NSTableView) getApi().view).removeTableColumn(((SwtTableColumn) column.getImpl()).nsColumn);
        }
        NSArray array = ((NSTableView) getApi().view).tableColumns();
    ...
    void drawInteriorWithFrame_inView(long id, long sel, NSRect rect, long view) {
        ...
        NSTableView widget = (NSTableView) this.getApi().view;
        long[] outValue = new long[1];
        OS.object_getInstanceVariable(id, SwtDisplay.SWT_ROW, outValue);
        long rowIndex = outValue[0];
        TableItem item = _getItem((int) rowIndex);
        ...
        Color background = ((SwtTableItem) item.getImpl()).cellBackground != null ? ((SwtTableItem) item.getImpl()).cellBackground[columnIndex] : null;
        if (background == null)
            background = ((SwtTableItem) item.getImpl()).background;
		 ...
		 */
            ((SwtTableItem) item.getImpl()).width = -1;
            Event event = new Event();
            event.item = item;
            event.gc = gc;
            event.index = columnIndex;
        ...
        if (headerView != null) {
            ...
            if (index != -1) {
                ...
                for (int i = 0; i < columnCount; i++) {
                    ...
                    if (((SwtTableColumn) column.getImpl()).nsColumn.id == nsColumn.id) {
                        return column;
                    }
                }
            }
    ...
    TableColumn getColumn(id id) {
        for (int i = 0; i < columnCount; i++) {
            if (((SwtTableColumn) columns[i].getImpl()).nsColumn.id == id.id) {
                return columns[i];
            }
        }
        return null;
    ...
    public boolean getHeaderVisible() {
        checkWidget();
        return ((NSTableView) getApi().view).headerView() != null;
    }
    ...
    public int getItemHeight() {
        checkWidget();
        return (int) ((NSTableView) getApi().view).rowHeight() + CELL_GAP;
    }
    ...
    public boolean getLinesVisible() {
        checkWidget();
        return ((NSTableView) getApi().view).usesAlternatingRowBackgroundColors();
    }
    ...
    public int getSelectionCount() {
        checkWidget();
        return (int) ((NSTableView) getApi().view).numberOfSelectedRows();
    }
	...
	*/
    ...
    public boolean isSelected(int index) {
        ...
        if (!(0 <= index && index < itemCount))
            return false;
        return ((NSTableView) getApi().view).isRowSelected(index);
    }
    ...
    long menuForEvent(long id, long sel, long theEvent) {
		 ...
		 */
            ...
            NSTableView table = (NSTableView) getApi().view;
 ...
 */
    ...
    long nextState(long id, long sel) {
        ...
        if (((SwtTableItem) item.getImpl()).grayed) {
            return ((SwtTableItem) item.getImpl()).checked ? OS.NSOffState : OS.NSMixedState;
        }
        return ((SwtTableItem) item.getImpl()).checked ? OS.NSOffState : OS.NSOnState;
    }
    ...
    public void setHeaderVisible(boolean show) {
        checkWidget();
        ((NSTableView) getApi().view).setHeaderView(show ? headerView : null);
        scrollView.tile();
    }
    ...
    void setItemHeight(int itemHeight) {
        ...
        if (itemHeight == -1) {
            ...
            ((NSTableView) getApi().view).setRowHeight(itemHeight);
        }
    }
    ...
    public void setLinesVisible(boolean show) {
        checkWidget();
        ((NSTableView) getApi().view).setUsesAlternatingRowBackgroundColors(show);
        ((NSTableView) getApi().view).setGridStyleMask(show ? OS.NSTableViewSolidVerticalGridLineMask : OS.NSTableViewGridNone);
    }
    ...
    boolean setScrollWidth(TableItem[] items, boolean set) {
        ...
        for (int i = 0; i < items.length; i++) {
            TableItem item = items[i];
            if (item != null) {
                newWidth = Math.max(newWidth, ((SwtTableItem) item.getImpl()).calculateWidth(0, gc, isSelected(indexOf(item))));
            }
        }
        gc.dispose();
        if (!set) {
    ...
    void setSort(TableColumn column, int direction) {
        ...
        if (column != null) {
            nsColumn = ((SwtTableColumn) column.getImpl()).nsColumn;
        ...
        if (sortColumn != null && sortColumn != column) {
            widget.setIndicatorImage(null, ((SwtTableColumn) sortColumn.getImpl()).nsColumn);
        }
        ...
        if (!isDisposed() && !item.isDisposed()) {
            ...
            if (contentWidth != event.width) {
                if (columnCount == 0 && columnIndex == 0) {
                    ((SwtTableItem) item.getImpl()).width = event.width;
                    if (setScrollWidth(item)) {
    ...
    long tableView_objectValueForTableColumn_row(long id, long sel, long aTableView, long aTableColumn, long rowIndex) {
        ...
        if (checkColumn != null && aTableColumn == checkColumn.id) {
            NSNumber value;
            if (((SwtTableItem) item.getImpl()).checked && ((SwtTableItem) item.getImpl()).grayed) {
                value = NSNumber.numberWithInt(OS.NSMixedState);
            } else {
                value = NSNumber.numberWithInt(((SwtTableItem) item.getImpl()).checked ? OS.NSOnState : OS.NSOffState);
            }
            return value.id;
        }
        for (int i = 0; i < columnCount; i++) {
    ...
    boolean tableView_shouldReorderColumn_toColumn(long id, long sel, long aTableView, long currentColIndex, long newColIndex) {
        ...
        for (int i = 0; i < columnCount; i++) {
            if (((SwtTableColumn) columns[i].getImpl()).nsColumn.id == nsColumn.id) {
                return ((SwtTableColumn) columns[i].getImpl()).movable;
            }
        }
        return true;
    }
    ...
    void tableView_willDisplayCell_forTableColumn_row(long id, long sel, long aTableView, long cell, long tableColumn, long rowIndex) {
        ...
        OS.object_setInstanceVariable(cell, SwtDisplay.SWT_ROW, rowIndex);
        OS.object_setInstanceVariable(cell, SwtDisplay.SWT_COLUMN, tableColumn);
        Image image = index == 0 ? ((SwtItem) item.getImpl()).image : (((SwtTableItem) item.getImpl()).images == null ? null : ((SwtTableItem) item.getImpl()).images[index]);
        ...
        if (textCell.isEnabled()) {
            if (textCell.isHighlighted()) {
                ...
                Color foreground = ((SwtTableItem) item.getImpl()).cellForeground != null ? ((SwtTableItem) item.getImpl()).cellForeground[index] : null;
                if (foreground == null)
                    foreground = ((SwtTableItem) item.getImpl()).foreground;
                if (foreground == null)
        ...
        Font font = ((SwtTableItem) item.getImpl()).cellFont != null ? ((SwtTableItem) item.getImpl()).cellFont[index] : null;
        if (font == null)
            font = ((SwtTableItem) item.getImpl()).font;
        if (font == null)
            font = this.font;
        if (font == null)
            font = defaultFont();
