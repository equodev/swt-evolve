...
public abstract class SwtControl extends SwtWidget implements Drawable, IControl {
    ...
    long accessibleHandle() {
        long returnValue = getApi().view.id;
        if (getApi().view instanceof NSControl) {
            if (((NSControl) getApi().view).cell() != null) {
                returnValue = ((NSControl) getApi().view).cell().id;
            }
        }
        return returnValue;
    }
    ...
    void doCommandBySelector(long id, long sel, long selector) {
        if (hasKeyboardFocus(id)) {
			 ...
			 */
                ...
                if (((SwtShell) s.getImpl()).keyInputHappened == false || (modifiers & OS.NSCommandKeyMask) != 0) {
    ...
    boolean dragDetect(int x, int y, boolean filter, boolean[] consume) {
        ...
        while (eventType != OS.NSLeftMouseUp) {
            NSEvent event = application.nextEventMatchingMask((OS.NSLeftMouseUpMask | OS.NSLeftMouseDraggedMask), timeout, OS.NSEventTrackingRunLoopMode, true);
            ...
            if (eventType == OS.NSLeftMouseDragged) {
                ...
                NSPoint viewLoc = getApi().view.convertPoint_fromView_(windowLoc, null);
                if (!getApi().view.isFlipped()) {
                    viewLoc.y = getApi().view.bounds().height - viewLoc.y;
                }
    ...
    void enableWidget(boolean enabled) {
        if (getApi().view instanceof NSControl) {
            ((NSControl) getApi().view).setEnabled(enabled);
        }
    ...
    void fillBackground(NSView view, NSGraphicsContext context, NSRect rect, int imgHeight, NSView gcView, int tx, int ty) {
        ...
        if (background == null) {
            if (isTransparent())
                return;
            background = ((SwtControl) control.getImpl()).defaultBackground().handle;
            alpha = getThemeAlpha();
        } else {
            alpha = background[3];
        }
    ...
    boolean gestureEvent(long id, long eventPtr, int detail) {
        ...
        if (phase == OS.NSEventPhaseBegan) {
            ...
            ((SwtDisplay) display.getImpl()).rotation = 0.0;
            ((SwtDisplay) display.getImpl()).magnification = 1.0;
            ((SwtDisplay) display.getImpl()).gestureActive = true;
        } else if (phase == OS.NSEventPhaseCancelled || phase == OS.NSEventPhaseEnded) {
            detail = SWT.GESTURE_END;
            ((SwtDisplay) display.getImpl()).gestureActive = false;
        }
        event.detail = detail;
        switch(detail) {
            ...
            case SWT.GESTURE_ROTATE:
                {
                    ((SwtDisplay) display.getImpl()).rotation += nsEvent.rotation();
                    event.rotation = ((SwtDisplay) display.getImpl()).rotation;
                    break;
                }
            case SWT.GESTURE_MAGNIFY:
                ((SwtDisplay) display.getImpl()).magnification *= (1.0 + nsEvent.magnification());
    ...
    long getVisibleRegion() {
        if (visibleRgn == 0) {
            ...
            calculateVisibleRegion(getApi().view, visibleRgn, true);
        }
    ...
    public long internal_new_GC(GCData data) {
		 ...
		 */
            ...
            if (data != null) {
                ...
                ((SwtDisplay) display.getImpl()).addContext(data);
            }
        }
        if (data != null) {
    ...
    boolean isActive() {
        ...
        if (dialog == null)
            return true;
        NSPanel panel = ((SwtDisplay) display.getImpl()).getModalPanel();
        if (panel == null)
            return false;
        NSWindow parentWindow = getApi().view.window().parentWindow();
 ...
 */
    ...
    boolean isObscured() {
        ...
        short[] rect = new short[4];
        NSRect bounds = getApi().view.visibleRect();
        OS.SetRect(rect, (short) bounds.x, (short) bounds.y, (short) (bounds.x + bounds.width), (short) (bounds.y + bounds.height));
	...
	*/
    ...
    void keyDown(long id, long sel, long theEvent) {
        if (hasKeyboardFocus(id)) {
            ...
            if (!textInput) {
                ...
                if (!((SwtShell) s.getImpl()).keyInputHappened) {
                    NSEvent nsEvent = new NSEvent(theEvent);
                    boolean[] consume = new boolean[1];
                    if (translateTraversal(nsEvent.keyCode(), nsEvent, consume))
                        return;
    ...
    boolean hasKeyboardFocus(long inId) {
        return getApi().view.window().firstResponder().id == inId;
    }
    ...
    long menuForEvent(long id, long sel, long theEvent) {
        if (((SwtDisplay) display.getImpl()).lastHandledMenuForEventId == theEvent)
            return 0;
        ((SwtDisplay) display.getImpl()).lastHandledMenuForEventId = theEvent;
        if (!isEnabled())
            return 0;
        NSPoint pt = NSEvent.mouseLocation();
        pt.y = (int) (((SwtDisplay) display.getImpl()).getPrimaryFrame().height - pt.y);
        int x = (int) pt.x;
        int y = (int) pt.y;
        Event event = new Event();
        event.x = x;
    ...
    boolean mouseEvent(long id, long sel, long theEvent, int type) {
        ...
        switch(nsType) {
            case OS.NSLeftMouseDown:
                if (nsEvent.clickCount() == 1 && (nsEvent.modifierFlags() & OS.NSControlKeyMask) == 0 && (state & DRAG_DETECT) != 0 && hooks(SWT.DragDetect)) {
                    ...
                    if (!getApi().view.isFlipped()) {
                        location.y = getApi().view.bounds().height - location.y;
                    }
            ...
            case OS.NSOtherMouseUp:
                if (((SwtDisplay) display.getImpl()).clickCount == 2) {
                    sendMouseEvent(nsEvent, SWT.MouseDoubleClick, false);
                }
                runEnterExit = true;
                runEnterExitControl = ((SwtDisplay) display.getImpl()).findControl(true);
                break;
        }
        ...
        if (runEnterExit)
            ((SwtDisplay) display.getImpl()).checkEnterExit(runEnterExitControl, nsEvent, false);
    ...
    public void redraw(int x, int y, int width, int height, boolean all) {
        ...
        rect.width = width;
        rect.height = height;
        getApi().view.setNeedsDisplayInRect(rect);
    }
 ...
 */
	...
	*/
    ...
    boolean sendMouseEvent(NSEvent nsEvent, int type, boolean send) {
        ...
        if (event.button != 0)
            event.count = ((SwtDisplay) display.getImpl()).clickCount;
        NSPoint windowPoint;
        NSView view = eventView();
    ...
    Touch touchStateFromNSTouch(NSTouch touch) {
        TouchSource source = ((SwtDisplay) display.getImpl()).findTouchSource(touch);
        ...
        ((SwtDisplay) display.getImpl()).touchCounter++;
        boolean primary = false;
        NSPoint normalizedPos = touch.normalizedPosition();
        double normalizedX = normalizedPos.x;
        double normalizedY = 1 - normalizedPos.y;
        if (((SwtDisplay) display.getImpl()).currentTouches().count() == 1)
            ((SwtDisplay) display.getImpl()).primaryIdentifier = identity;
        if (((SwtDisplay) display.getImpl()).primaryIdentifier == identity)
            primary = true;
	...
	*/
    ...
    void setFont(NSFont font) {
        if (getApi().view instanceof NSControl) {
            ((NSControl) getApi().view).setFont(font);
        }
    }
	...
	*/
    ...
    boolean touchEvent(long id, long sel, long eventPtr) {
        ...
        if (!touchEnabled)
            return true;
        NSEvent nsEvent = new NSEvent(eventPtr);
        NSMutableArray currentTouches = ((SwtDisplay) display.getImpl()).currentTouches();
        Event event = new Event();
        NSPoint location = getApi().view.convertPoint_fromView_(nsEvent.locationInWindow(), null);
        if (!getApi().view.isFlipped()) {
            location.y = getApi().view.bounds().height - location.y;
        ...
        if (currentTouches.count() == 0)
            ((SwtDisplay) display.getImpl()).touchCounter = 0;
    ...
    boolean translateTraversal(int key, NSEvent theEvent, boolean[] consume) {
        ...
        switch(key) {
            ...
            case 124:
                /* Right arrow */
                {
                    boolean next = key == 125 || /* Down arrow */
                    key == 124;
	...
	*/
    ...
    boolean update(boolean all) {
        ...
        if (NSGraphicsContext.currentContext() == null) {
            if (!getApi().view.lockFocusIfCanDraw()) {
                return false;
            }
        ...
        //	checkWidget();
        NSArray isPainting = ((SwtDisplay) display.getImpl()).isPainting;
        if (isPainting.containsObject(getApi().view))
            return false;
        for (int i = 0, length = (int) isPainting.count(); i < length; i++) {
            NSView view = new NSView(isPainting.objectAtIndex(i));
            if (view.isDescendantOf(this.getApi().view))
                return false;
        }
        if (isResizing())
            return false;
        Shell shell = getShell();
        NSWindow window = ((SwtShell) shell.getImpl()).deferFlushing && ((SwtShell) shell.getImpl()).scrolling ? getApi().view.window() : null;
        try {
            if (window != null) {
                window.retain();
