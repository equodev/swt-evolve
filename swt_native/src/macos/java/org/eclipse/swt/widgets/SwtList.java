...
public class SwtList extends SwtScrollable implements IList {
    ...
    public int getFocusIndex() {
        checkWidget();
        return (int) ((NSTableView) getApi().view).selectedRow();
    }
    ...
    public int getItemHeight() {
        checkWidget();
        return (int) ((NSTableView) getApi().view).rowHeight();
    }
    ...
    public int getTopIndex() {
        ...
        point.x = rect.x;
        point.y = rect.y;
        int result = (int) ((NSTableView) getApi().view).rowAtPoint(point);
        if (result == -1)
            result = 0;
        return result;
    }
 ...
 */
    ...
    void sendDoubleSelection() {
        if (((NSTableView) getApi().view).clickedRow() != -1) {
    ...
    boolean sendMouseEvent(NSEvent nsEvent, int type, boolean send) {
        ...
        if (handleMouseDown) {
            if (type == SWT.MouseDown) {
                ...
                if (rowsChanged) {
                    rowsChanged = false;
                    ((NSTableView) getApi().view).noteNumberOfRowsChanged();
                }
            }
        }
    ...
    void setFont(NSFont font) {
        ...
        ((NSTableView) getApi().view).setRowHeight(height);
        setScrollWidth();
    }
    ...
    public void setItems(String... items) {
        ...
        ((NSTableView) getApi().view).reloadData();
        setScrollWidth();
    }
    ...
    void showIndex(int index) {
        if (0 <= index && index < itemCount) {
            ((NSTableView) getApi().view).scrollRowToVisible(index);
        }
    }
    ...
    long tableView_objectValueForTableColumn_row(long id, long sel, long aTableView, long aTableColumn, long rowIndex) {
        double[] fg = ((NSTableView) getApi().view).isRowSelected(rowIndex) ? null : foreground;
