...
public class SwtTracker extends SwtWidget implements ITracker {
	...
	*/
        if (clientCursor == null) {
            ...
            ((SwtDisplay) display.getImpl()).lockCursor = false;
            newCursor.handle.set();
            ((SwtDisplay) display.getImpl()).lockCursor = true;
    ...
    void drawRectangles(NSWindow window, Rectangle[] rects, boolean erase) {
        ...
        double screenHeight = ((SwtDisplay) display.getImpl()).getPrimaryFrame().height;
        for (int i = 0; i < rects.length; i++) {
    ...
    void mouse(NSEvent nsEvent) {
        ...
        location.y = ((SwtDisplay) display.getImpl()).getPrimaryFrame().height - location.y;
	...
	*/
        ...
        Control oldTrackingControl = ((SwtDisplay) display.getImpl()).trackingControl;
        ((SwtDisplay) display.getImpl()).trackingControl = null;
        /* Tracker behaves like a Dialog with its own OS event loop. */
        while (tracking && !cancelled) {
            ((SwtDisplay) display.getImpl()).addPool();
            try {
                ...
                ((SwtDisplay) display.getImpl()).removePool();
            }
        }
        /*
	...
	*/
        ...
        if (oldTrackingControl != null && !oldTrackingControl.isDisposed()) {
            ((SwtDisplay) display.getImpl()).trackingControl = oldTrackingControl;
        }
        ((SwtDisplay) display.getImpl()).setCursor(((SwtDisplay) display.getImpl()).findControl(true));
        if (!isDisposed()) {
            for (int i = 0; i < windows.length; i++) {
