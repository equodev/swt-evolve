...
public class SwtTreeItem extends SwtItem implements ITreeItem {
    ...
    int calculateWidth(int index, GC gc) {
        ...
        if (image != null)
            size.width += ((SwtTree) parent.getImpl()).imageBounds.width + SwtTree.IMAGE_GAP;
    ...
    void sendExpand(boolean expand, boolean recurse) {
        ...
        if (recurse) {
            for (int i = 0; i < itemCount; i++) {
                if (items[i] != null)
                    ((SwtTreeItem) items[i].getImpl()).sendExpand(expand, recurse);
            }
        }
    }
    ...
    public void setImage(int index, Image image) {
        ...
        if (((SwtTree) parent.getImpl()).imageBounds == null && image != null) {
            ((SwtTree) parent.getImpl()).setItemHeight(image, null, false);
        }
        if (index == 0) {
