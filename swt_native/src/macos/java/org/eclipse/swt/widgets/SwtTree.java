...
public class SwtTree extends SwtComposite implements ITree {
    ...
    TreeItem _getItem(TreeItem parentItem, int index, boolean create) {
        ...
        TreeItem[] items;
        if (parentItem != null) {
            count = ((SwtTreeItem) parentItem.getImpl()).itemCount;
            items = ((SwtTreeItem) parentItem.getImpl()).items;
        } else {
    ...
    int calculateWidth(TreeItem[] items, int index, GC gc, boolean recurse) {
        ...
        for (int i = 0; i < items.length; i++) {
            ...
            if (item != null) {
                int itemWidth = ((SwtTreeItem) item.getImpl()).calculateWidth(index, gc);
                width = Math.max(width, itemWidth);
                if (recurse && item.getExpanded()) {
                    width = Math.max(width, calculateWidth(((SwtTreeItem) item.getImpl()).items, index, gc, recurse));
                }
            }
        }
        return width;
    ...
    NSSize cellSize(long id, long sel) {
        ...
        if (hooks(SWT.MeasureItem)) {
            ...
            OS.object_getInstanceVariable(id, SwtDisplay.SWT_ROW, outValue);
            TreeItem item = (TreeItem) ((SwtDisplay) display.getImpl()).getWidget(outValue[0]);
            OS.object_getInstanceVariable(id, SwtDisplay.SWT_COLUMN, outValue);
	...
	*/
    ...
    void checkItems() {
        ...
        ((NSOutlineView) getApi().view).reloadData();
    ...
    void clearAll(TreeItem parentItem, boolean all) {
        ...
        if (count == 0)
            return;
        TreeItem[] children = parentItem == null ? items : ((SwtTreeItem) parentItem.getImpl()).items;
    ...
    void clearCachedWidth(TreeItem[] items) {
        ...
        for (int i = 0; i < items.length; i++) {
            ...
            if (item == null)
                break;
            ((SwtTreeItem) item.getImpl()).width = -1;
            clearCachedWidth(((SwtTreeItem) item.getImpl()).items);
        }
    }

    @Override
    void collapseItem_collapseChildren(long id, long sel, long itemID, boolean children) {
        TreeItem item = (TreeItem) ((SwtDisplay) display.getImpl()).getWidget(itemID);
        if (item == null)
            return;
        if (!ignoreExpand)
    ...
    public Point computeSize(int wHint, int hHint, boolean changed) {
        ...
        if (hHint == SWT.DEFAULT) {
            height = (int) ((NSOutlineView) getApi().view).numberOfRows() * getItemHeight() + getHeaderHeight();
        } else {
            height = hHint;
        }
        if (width <= 0)
    ...
    void createColumn(TreeItem item, int index) {
        if (((SwtTreeItem) item.getImpl()).items != null) {
            for (int i = 0; i < ((SwtTreeItem) item.getImpl()).items.length; i++) {
                if (((SwtTreeItem) item.getImpl()).items[i] != null)
                    createColumn(((SwtTreeItem) item.getImpl()).items[i], index);
            }
        }
        ...
        Color[] cellBackground = ((SwtTreeItem) item.getImpl()).cellBackground;
        ...
        Color[] cellForeground = ((SwtTreeItem) item.getImpl()).cellForeground;
	...
	*/
    ...
    void createItem(TreeColumn column, int index) {
        ...
        ((SwtDisplay) display.getImpl()).addWidget(headerCell, column);
        ((SwtTreeColumn) column.getImpl()).nsColumn = nsColumn;
        nsColumn.setWidth(0);
    ...
    void createItem(TreeItem item, TreeItem parentItem, int index) {
        ...
        if (count == items.length) {
            ...
            items = newItems;
            if (parentItem != null) {
                ((SwtTreeItem) parentItem.getImpl()).items = items;
            } else {
                this.items = items;
            }
        }
        ...
        if (parentItem != null && ((SwtTreeItem) parentItem.getImpl()).itemCount == 1 && ((SwtTreeItem) parentItem.getImpl()).expanded) {
    ...
    NSFont defaultNSFont() {
        return ((SwtDisplay) display.getImpl()).outlineViewFont;
    }
    ...
    public void deselectAll() {
        checkWidget();
        NSTableView widget = (NSOutlineView) getApi().view;
        ignoreSelect = true;
    ...
    void deregister() {
        ...
        if (buttonCell != null)
            ((SwtDisplay) display.getImpl()).removeWidget(buttonCell);
    }
    ...
    void destroyItem(TreeColumn column) {
        ...
        int oldIndex = indexOf(((SwtTreeColumn) column.getImpl()).nsColumn);
        System.arraycopy(columns, index + 1, columns, index, --columnCount - index);
        columns[columnCount] = null;
        if (columnCount == 0) {
            //TODO - reset attributes
            firstColumn = ((SwtTreeColumn) column.getImpl()).nsColumn;
            firstColumn.retain();
            /*
		* Feature in Cocoa.  If a column's width is too small to show any content
		* then outlineView_objectValueForTableColumn_byItem is never invoked to
		...
		*/
            ...
            if (index == 0) {
                ((NSOutlineView) getApi().view).setOutlineTableColumn(((SwtTreeColumn) columns[0].getImpl()).nsColumn);
            }
            ((NSOutlineView) getApi().view).removeTableColumn(((SwtTreeColumn) column.getImpl()).nsColumn);
        }
        NSArray array = ((NSOutlineView) getApi().view).tableColumns();
    ...
    void drawInteriorWithFrame_inView(long id, long sel, NSRect rect, long view) {
        ...
        NSOutlineView widget = (NSOutlineView) this.getApi().view;
        long[] outValue = new long[1];
        OS.object_getInstanceVariable(id, SwtDisplay.SWT_ROW, outValue);
        long rowIndex = widget.rowForItem(new id(outValue[0]));
        if (rowIndex == -1) {
            // the row item doesn't exist or has been disposed
            return;
        }
        TreeItem item = (TreeItem) ((SwtDisplay) display.getImpl()).getWidget(outValue[0]);
        if (item == null)
        ...
        Color background = ((SwtTreeItem) item.getImpl()).cellBackground != null ? ((SwtTreeItem) item.getImpl()).cellBackground[columnIndex] : null;
        if (background == null)
            background = ((SwtTreeItem) item.getImpl()).background;
    ...
    void expandItem_expandChildren(long id, long sel, long itemID, boolean children) {
        ...
        if (!children) {
            ignoreExpand = true;
            TreeItem[] items = ((SwtTreeItem) item.getImpl()).items;
            for (int i = 0; i < ((SwtTreeItem) item.getImpl()).itemCount; i++) {
                if (items[i] != null)
                    ((SwtTreeItem) items[i].getImpl()).updateExpanded();
            }
            ignoreExpand = false;
        }
        setScrollWidth(false, ((SwtTreeItem) item.getImpl()).items, true);
    }
        ...
        if (headerView != null) {
            ...
            if (index != -1) {
                ...
                for (int i = 0; i < columnCount; i++) {
                    ...
                    if (((SwtTreeColumn) column.getImpl()).nsColumn.id == nsColumn.id) {
                        return column;
                    }
                }
            }
    ...
    TreeColumn getColumn(id id) {
        for (int i = 0; i < columnCount; i++) {
            if (((SwtTreeColumn) columns[i].getImpl()).nsColumn.id == id.id) {
                return columns[i];
            }
        }
        return null;
    ...
    public int getHeaderHeight() {
        checkWidget();
        NSTableHeaderView headerView = ((NSOutlineView) getApi().view).headerView();
        if (headerView == null)
            return 0;
    ...
    public boolean getHeaderVisible() {
        checkWidget();
        return ((NSOutlineView) getApi().view).headerView() != null;
    }
    ...
    public TreeItem getItem(Point point) {
        ...
        Widget item = ((SwtDisplay) display.getImpl()).getWidget(id.id);
    ...
    int getItemCount(TreeItem item) {
        return item == null ? itemCount : ((SwtTreeItem) item.getImpl()).itemCount;
    }
    ...
    public int getItemHeight() {
        checkWidget();
        return (int) ((NSOutlineView) getApi().view).rowHeight() + CELL_GAP;
    }
    ...
    public boolean getLinesVisible() {
        checkWidget();
        return ((NSOutlineView) getApi().view).usesAlternatingRowBackgroundColors();
    }
    ...
    public int getSelectionCount() {
        checkWidget();
        return (int) ((NSOutlineView) getApi().view).numberOfSelectedRows();
    }
	 ...
	 */
        ...
        return (TreeItem) ((SwtDisplay) display.getImpl()).getWidget(item.id);
    }
	...
	*/
    ...
    long image(long id, long sel) {
        ...
        OS.object_getInstanceVariable(id, SwtDisplay.SWT_IMAGE, image);
        return image[0];
    }
    ...
    int indexOf(NSTableColumn column) {
        return (int) ((NSTableView) getApi().view).tableColumns().indexOfObjectIdenticalTo(column);
    }
    ...
    public int indexOf(TreeItem item) {
        ...
        if (item.isDisposed())
            error(SWT.ERROR_INVALID_ARGUMENT);
        if (((SwtTreeItem) item.getImpl()).parentItem != null)
            return -1;
        for (int i = 0; i < itemCount; i++) {
            if (item == items[i])
                return i;
    ...
    long menuForEvent(long id, long sel, long theEvent) {
        if (((SwtDisplay) display.getImpl()).lastHandledMenuForEventId == theEvent)
            return 0;
        if (id != headerView.id) {
            /*
		 ...
		 */
            ...
            NSOutlineView tree = (NSOutlineView) getApi().view;
            // get the current selections for the outline view.
            NSIndexSet selectedRowIndexes = tree.selectedRowIndexes();
            // select the row that was clicked before showing the menu for the event
            NSPoint mousePoint = getApi().view.convertPoint_fromView_(event.locationInWindow(), null);
 ...
 */
    ...
    long nextState(long id, long sel) {
        ...
        TreeItem item = (TreeItem) ((SwtDisplay) display.getImpl()).getWidget(outlineView.itemAtRow(index).id);
        if (((SwtTreeItem) item.getImpl()).grayed) {
            return ((SwtTreeItem) item.getImpl()).checked ? OS.NSOffState : OS.NSMixedState;
        }
        return ((SwtTreeItem) item.getImpl()).checked ? OS.NSOffState : OS.NSOnState;
    }

    @Override
    long outlineView_child_ofItem(long id, long sel, long outlineView, long index, long itemID) {
        TreeItem parent = (TreeItem) ((SwtDisplay) display.getImpl()).getWidget(itemID);
    ...
    long outlineView_objectValueForTableColumn_byItem(long id, long sel, long outlineView, long tableColumn, long itemID) {
        ...
        if (checkColumn != null && tableColumn == checkColumn.id) {
            NSNumber value;
            if (((SwtTreeItem) item.getImpl()).checked && ((SwtTreeItem) item.getImpl()).grayed) {
                value = NSNumber.numberWithInt(OS.NSMixedState);
            } else {
                value = NSNumber.numberWithInt(((SwtTreeItem) item.getImpl()).checked ? OS.NSOnState : OS.NSOffState);
            }
            return value.id;
        }
        for (int i = 0; i < columnCount; i++) {
    ...
    boolean outlineView_shouldReorderColumn_toColumn(long id, long sel, long aTableView, long currentColIndex, long newColIndex) {
        ...
        for (int i = 0; i < columnCount; i++) {
            if (((SwtTreeColumn) columns[i].getImpl()).nsColumn.id == nsColumn.id) {
                return ((SwtTreeColumn) columns[i].getImpl()).movable;
            }
        }
        return true;
    }
    ...
    void outlineView_willDisplayCell_forTableColumn_item(long id, long sel, long outlineView, long cell, long tableColumn, long itemID) {
        ...
        OS.object_setInstanceVariable(cell, SwtDisplay.SWT_ROW, itemID);
        OS.object_setInstanceVariable(cell, SwtDisplay.SWT_COLUMN, tableColumn);
        Image image = index == 0 ? ((SwtItem) item.getImpl()).image : (((SwtTreeItem) item.getImpl()).images == null ? null : ((SwtTreeItem) item.getImpl()).images[index]);
        ...
        if (textCell.isEnabled()) {
            if (textCell.isHighlighted()) {
                ...
                Color foreground = ((SwtTreeItem) item.getImpl()).cellForeground != null ? ((SwtTreeItem) item.getImpl()).cellForeground[index] : null;
                if (foreground == null)
                    foreground = ((SwtTreeItem) item.getImpl()).foreground;
                if (foreground == null)
        ...
        Font font = ((SwtTreeItem) item.getImpl()).cellFont != null ? ((SwtTreeItem) item.getImpl()).cellFont[index] : null;
        if (font == null)
            font = ((SwtTreeItem) item.getImpl()).font;
        if (font == null)
            font = this.font;
        if (font == null)
            font = defaultFont();
    ...
    void sendSelection() {
        ...
        if (row == -1)
            ...
            TreeItem item = (TreeItem) ((SwtDisplay) display.getImpl()).getWidget(_id.id);
            Event event = new Event();
            event.item = item;
            event.index = row;
    ...
    void outlineView_setObjectValue_forTableColumn_byItem(long id, long sel, long outlineView, long object, long tableColumn, long itemID) {
        if (checkColumn != null && tableColumn == checkColumn.id) {
            TreeItem item = (TreeItem) ((SwtDisplay) display.getImpl()).getWidget(itemID);
            ((SwtTreeItem) item.getImpl()).checked = !((SwtTreeItem) item.getImpl()).checked;
            Event event = new Event();
    ...
    void setImage(long id, long sel, long arg0) {
        OS.object_setInstanceVariable(id, SwtDisplay.SWT_IMAGE, arg0);
    }
    ...
    void sendDoubleSelection() {
        ...
        if (rowIndex != -1) {
            ...
            if (itemAtRow != null) {
                TreeItem item = (TreeItem) ((SwtDisplay) display.getImpl()).getWidget(itemAtRow.id);
                Event event = new Event();
                event.item = item;
        ...
        if (!isDisposed() && !item.isDisposed()) {
            ...
            if (contentWidth != event.width) {
                if (columnCount == 0 && columnIndex == 0) {
                    ((SwtTreeItem) item.getImpl()).width = event.width;
                    ((SwtTreeItem) item.getImpl()).width += widget.indentationPerLevel() * (1 + widget.levelForItem(item.handle));
                    if (setScrollWidth(item)) {
        ...
        if (type == SWT.DragDetect) {
            ...
            if (selectedRowIndex != -1) {
                if (dragDetected) {
                    ...
                    if (itemID != null) {
                        Widget item = ((SwtDisplay) display.getImpl()).getWidget(itemID.id);
                        if (item != null && item instanceof TreeItem) {
                            event.item = ((SwtDisplay) display.getImpl()).getWidget(itemID.id);
                            sendSelectionEvent(SWT.Selection, event, false);
                        }
                    }
                    selectedRowIndex = -1;
    ...
    void setBackgroundColor(NSColor nsColor) {
        ((NSTableView) getApi().view).setBackgroundColor(nsColor);
    }
    ...
    void setFont(NSFont font) {
        ...
        getApi().view.setNeedsDisplay(true);
        clearCachedWidth(items);
        setScrollWidth();
    }
    ...
    public void setHeaderVisible(boolean show) {
        checkWidget();
        ((NSOutlineView) getApi().view).setHeaderView(show ? headerView : null);
        scrollView.tile();
    }
    ...
    void setItemCount(TreeItem parentItem, int count) {
		...
		*/
            ...
            if ((style & SWT.VIRTUAL) == 0) {
                ...
                widget.reloadItem(parentItem != null ? parentItem.handle : null, expanded);
                selectItems(selectedItems, true);
                if (parentItem != null && itemCount == 0 && ((SwtTreeItem) parentItem.getImpl()).expanded) {
                    ignoreExpand = true;
                    widget.expandItem(parentItem.handle);
                    ignoreExpand = false;
                }
    ...
    void setItemHeight(int itemHeight) {
        ...
        if (itemHeight == -1) {
            ...
            ((NSOutlineView) getApi().view).setRowHeight(itemHeight);
        }
    }
    ...
    void setItemHeight(Image image, NSFont font, boolean set) {
        ...
        if (((SwtDisplay) display.getImpl()).useNativeItemHeight) {
    ...
    public void setLinesVisible(boolean show) {
        checkWidget();
        ((NSOutlineView) getApi().view).setUsesAlternatingRowBackgroundColors(show);
        ((NSOutlineView) getApi().view).setGridStyleMask(show ? OS.NSTableViewSolidVerticalGridLineMask : OS.NSTableViewGridNone);
    }
    ...
    boolean setScrollWidth(boolean set, TreeItem[] items, boolean recurse) {
        ...
        if (horizontalBar != null && ((SwtScrollBar) horizontalBar.getImpl()).view != null)
            redrawWidget(((SwtScrollBar) horizontalBar.getImpl()).view, false);
        return true;
    }
    ...
    void setSort(TreeColumn column, int direction) {
        ...
        if (column != null) {
            nsColumn = ((SwtTreeColumn) column.getImpl()).nsColumn;
        ...
        if (sortColumn != null && sortColumn != column) {
            widget.setIndicatorImage(null, ((SwtTreeColumn) sortColumn.getImpl()).nsColumn);
        }
