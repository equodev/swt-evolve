	...
	*/
    ...
    public Point computeSize(int wHint, int hHint, boolean changed) {
        ...
        if (wHint == SWT.DEFAULT && items.length > 0) {
            NSSize minSize = ((NSTabView) getApi().view).minimumSize();
    ...
    public Rectangle computeTrim(int x, int y, int width, int height) {
        checkWidget();
        NSTabView widget = (NSTabView) getApi().view;
    ...
    NSFont defaultNSFont() {
        return ((SwtDisplay) display.getImpl()).tabViewFont;
    }
    ...
    void destroyItem(TabItem item) {
        ...
        itemCount = count;
        ((NSTabView) getApi().view).removeTabViewItem(((SwtTabItem) item.getImpl()).nsItem);
    }
    ...
    Widget findTooltip(NSPoint pt) {
        ...
        NSTabViewItem nsItem = ((NSTabView) getApi().view).tabViewItemAtPoint(pt);
        if (nsItem != null) {
            for (int i = 0; i < itemCount; i++) {
                TabItem item = items[i];
                if (((SwtTabItem) item.getImpl()).nsItem.id == nsItem.id)
                    return item;
            }
        }
    ...
    public Rectangle getClientArea() {
        checkWidget();
        NSRect rect = ((NSTabView) getApi().view).contentRect();
    ...
    public TabItem getItem(Point point) {
        ...
        nsPoint.x = point.x;
        nsPoint.y = point.y;
        NSTabView tabView = (NSTabView) getApi().view;
        NSTabViewItem tabViewItem = tabView.tabViewItemAtPoint(nsPoint);
        for (int i = 0; i < itemCount; i++) {
            NSTabViewItem item = ((SwtTabItem) items[i].getImpl()).nsItem;
    ...
    public int getSelectionIndex() {
        checkWidget();
        NSTabViewItem selected = ((NSTabView) getApi().view).selectedTabViewItem();
        if (selected == null)
            return -1;
        for (int i = 0; i < itemCount; i++) {
            if (((SwtTabItem) items[i].getImpl()).nsItem.id == selected.id)
                return i;
        }
        return -1;
    }
    ...
    void setFont(NSFont font) {
        ((NSTabView) getApi().view).setFont(font);
        int index = getSelectionIndex();
        for (int i = 0; i < itemCount; i++) {
    ...
    void setSelection(int index, boolean notify, boolean force) {
        ...
        ignoreSelect = true;
        ((NSTabView) getApi().view).selectTabViewItemAtIndex(index);
        ignoreSelect = false;
    ...
    @Override
    void setSmallSize() {
        ((NSTabView) getApi().view).setControlSize(OS.NSSmallControlSize);
    }
        ...
        for (int i = 0; i < itemCount; i++) {
            ...
            if (((SwtTabItem) item.getImpl()).nsItem.id == tabViewItem) {
                ...
                if (currentIndex != -1) {
                    ...
                    if (selected != null) {
                        ((SwtTabItem) selected.getImpl()).updateText(false);
                        Control control = ((SwtTabItem) selected.getImpl()).control;
                        if (control != null && !control.isDisposed()) {
                            control.setVisible(false);
                        }
                    }
                ...
                ((SwtTabItem) item.getImpl()).updateText(true);
                break;
            }
        }
    }
