	...
	*/
    ...
    void createHandle() {
        ...
        if ((this.style & SWT.DROP_DOWN) != 0) {
            ...
            buttonWidget.setTarget(getApi().view);
            buttonWidget.setAction(OS.sel_sendVerticalSelection);
            getApi().view.addSubview(buttonWidget);
    ...
    void createPopupShell(int year, int month, int day) {
        ...
        ((SwtShell) popupShell.getImpl()).isPopup = true;
        ((SwtShell) popupShell.getImpl()).window.setHasShadow(true);
    ...
    NSFont defaultNSFont() {
        return ((SwtDisplay) display.getImpl()).datePickerFont;
    }
    ...
    NSCalendarDate getCalendarDate() {
        NSDate date = ((NSDatePicker) getApi().view).dateValue();
    ...
    void resized() {
        ...
        NSRect rect = getApi().view.bounds();
	 ...
	 */
        if (nsColor != null && nsColor.alphaComponent() == 0)
            return;
        ((NSDatePicker) getApi().view).setBackgroundColor(nsColor);
    }

    @Override
    void setBackgroundImage(NSImage image) {
        ((NSDatePicker) getApi().view).setDrawsBackground(image == null);
    }
    ...
    public void setDate(int year, int month, int day) {
        ...
        if (newDate.yearOfCommonEra() == year && newDate.monthOfYear() == month + 1 && newDate.dayOfMonth() == day) {
            ((NSDatePicker) getApi().view).setDateValue(newDate);
        }
    }
    ...
    void setForeground(double[] color) {
        ...
        ((NSDatePicker) getApi().view).setTextColor(nsColor);
    }
    ...
    void setSmallSize() {
        if (buttonView != null)
            buttonView.cell().setControlSize(OS.NSMiniControlSize);
    }
