...
public class SwtMenu extends SwtWidget implements IMenu {
    ...
    void createHandle() {
        ...
        if (nsMenu == null) {
            ...
            long dynNSMenu_class = ((SwtDisplay) display.getImpl()).createMenuSubclass(cls, "SWTSystemMenu", true);
    ...
    void createItem(MenuItem item, int index) {
        ...
        if (nsItem == null) {
            ...
            ((SwtMenuItem) item.getImpl()).nsItem = nsItem;
        } else {
            long cls = OS.object_getClass(nsItem.id);
            long dynNSMenuItem_class = ((SwtDisplay) display.getImpl()).createMenuItemSubclass(cls, "SWTSystemMenuItem", true);
            if (cls != dynNSMenuItem_class) {
                OS.object_setClass(nsItem.id, dynNSMenuItem_class);
            }
            nsItem.retain();
            ((SwtMenuItem) item.getImpl()).nsItemAction = nsItem.action();
            ((SwtMenuItem) item.getImpl()).nsItemTarget = nsItem.target();
            nsItem.setTarget(nsItem);
            nsItem.setAction(OS.sel_sendSelection);
            ...
            if (keyEquivString != null) {
                ...
                if ((keyMask & OS.NSCommandKeyMask) != 0)
                    keyEquiv |= SWT.COMMAND;
                if ((keyMask & OS.NSAlternateKeyMask) != 0)
                    keyEquiv |= SWT.ALT;
                ((SwtMenuItem) item.getImpl()).accelerator = (int) keyEquiv;
            }
            add = false;
        }
    ...
    void menuWillOpen(long id, long sel, long menu) {
        ...
        for (int i = 0; i < itemCount; i++) {
            ...
            if (accelIndex != -1) {
                String accelText = ((SwtItem) item.getImpl()).text.substring(accelIndex);
    ...
    public void setEnabled(boolean enabled) {
        ...
        for (int i = 0; i < items.length; i++) {
			...
			*/
                ((SwtMenuItem) item.getImpl()).nsItem.setEnabled(enabled && item.getEnabled());
            }
        }
    }
