...
public class SwtShell extends SwtDecorations implements IShell {
    ...
    void attachObserversToWindow(NSWindow newWindow) {
        ...
        if (sendEventImpl == SwtDisplay.windowCallback3.getAddress())
            return;
    ...
    void becomeKeyWindow(long id, long sel) {
        ...
        if (modal != null && ((SwtShell) modal.getImpl()).window != null) {
            ((SwtShell) modal.getImpl()).window.makeKeyAndOrderFront(null);
            return;
        }
    ...
    public Point computeSize(int wHint, int hHint, boolean changed) {
        ...
        if (toolBar != null) {
            if (wHint == SWT.DEFAULT && ((SwtToolBar) toolBar.getImpl()).itemCount > 0) {
    ...
    void createHandle() {
        ...
        if (window == null) {
            NSWindow hostWindow = getApi().view.window();
        ...
        if (fieldEditorWindow == null)
            fieldEditorWindow = getApi().view.window();
    ...
    void deregister() {
        ...
        if (windowDelegate != null)
            ((SwtDisplay) display.getImpl()).removeWidget(windowDelegate);
    }
    ...
    void destroyWidget() {
        ...
        if (window != null) {
            if (!display.isDisposed() && display.getShells().length == 0) {
                ((SwtDisplay) display.getImpl()).setMenuBar(null);
            }
            window.release();
        }
    }
	...
	*/
    ...
    public Rectangle getBounds() {
        ...
        if (window != null) {
            NSRect frame = window.frame();
            double y = ((SwtDisplay) display.getImpl()).getPrimaryFrame().height - (int) (frame.y + frame.height);
            ...
            NSRect primaryFrame = ((SwtDisplay) display.getImpl()).getPrimaryFrame();
            if (!getApi().view.isFlipped()) {
                pt.y = getApi().view.bounds().height - pt.y;
            }
            pt = getApi().view.convertPoint_toView_(pt, null);
            pt = getApi().view.window().convertBaseToScreen(pt);
    ...
    public Point getSize() {
        checkWidget();
        NSRect frame = (window != null ? window.frame() : getApi().view.frame());
	...
	*/
    ...
    void mouseMoved(long id, long sel, long theEvent) {
         ...
         */
        if (id == getApi().view.id && window == null) {
            getApi().view.window().mouseMoved(new NSEvent(theEvent));
        }
    }
    ...
    void removeObserversFromWindow() {
        ...
        if (hostWindow != null) {
            ...
            if (embedCount.intValue() <= 0) {
                ...
                ((SwtDisplay) display.getImpl()).removeWidget(hostWindow);
                hostWindow.release();
                hostWindow = null;
                hostWindowClass = 0;
            }
    ...
    void sendToolTipEvent(boolean enter) {
        if (!isVisible())
            return;
        NSWindow eventWindow = getApi().view.window();
        if (tooltipTag == 0) {
	...
	*/
    ...
    void setBounds(int x, int y, int width, int height, boolean move, boolean resize) {
        if (window == null) {
            ...
            if (resize) {
                ...
                getApi().view.setFrameSize(frameSize);
                return;
            }
        }
        if (_getFullScreen())
        ...
        if (sheet && move && !resize)
            return;
        int screenHeight = (int) ((SwtDisplay) display.getImpl()).getPrimaryFrame().height;
        NSRect frame = window.frame();
        if (!move) {
            x = (int) frame.x;
        ...
        if (sheet) {
            y = screenHeight - (int) (frame.y + frame.height);
            NSRect parentRect = ((SwtShell) parent.getShell().getImpl()).window.frame();
            frame.width = width;
            frame.height = height;
    ...
    public void setFullScreen(boolean fullScreen) {
        ...
        if (_getFullScreen() == fullScreen)
            return;
        if ((window.collectionBehavior() & OS.NSWindowCollectionBehaviorFullScreenPrimary) != 0) {
            window.toggleFullScreen(SwtDisplay.applicationDelegate);
            return;
        }
        this.fullScreen = fullScreen;
        if (fullScreen) {
            currentFrame = window.frame();
            //only hides resize indicator
    ...
    void setZOrder() {
        ...
        if (window == null)
            return;
        window.setContentView(scrollView != null ? scrollView : getApi().view);
        if (fixResize()) {
    ...
    void setZOrder(Control control, boolean above) {
        ...
        if (control == null) {
            ...
            NSWindow otherWindow = ((SwtShell) control.getShell().getImpl()).window;
        ...
        if (window != null) {
            if (visible) {
                if (parent != null && parent.getVisible()) {
                    NSWindow parentWindow = ((SwtDisplay) display.getImpl()).modalPanel != null ? ((SwtDisplay) display.getImpl()).modalPanel : parentWindow();
    ...
    void updateSystemUIMode() {
        ...
        if (!getMonitor().equals(display.getPrimaryMonitor()))
            return;
        int mode = ((SwtDisplay) display.getImpl()).systemUIMode, options = ((SwtDisplay) display.getImpl()).systemUIOptions;
        if (fullScreen) {
    ...
    void windowDidBecomeKey(long id, long sel, long notification) {
        ...
        if ((window.collectionBehavior() & OS.NSWindowCollectionBehaviorFullScreenPrimary) == 0) {
            ...
            if (!((SwtShell) parentShell.getImpl())._getFullScreen() || menuBar != null) {
                updateSystemUIMode();
            } else {
                ((SwtShell) parentShell.getImpl()).updateSystemUIMode();
            }
        }
    }
    ...
    void windowSendEvent(long id, long sel, long event) {
        ...
        switch(type) {
            ...
            case OS.NSOtherMouseDown:
                ((SwtDisplay) display.getImpl()).clickCount = (int) (((SwtDisplay) display.getImpl()).clickCountButton == nsEvent.buttonNumber() ? nsEvent.clickCount() : 1);
                ((SwtDisplay) display.getImpl()).clickCountButton = (int) nsEvent.buttonNumber();
                break;
            ...
            case OS.NSMouseMoved:
                ...
                if (((SwtDisplay) display.getImpl()).tooltipControl != control || ((SwtDisplay) display.getImpl()).tooltipTarget != target) {
                    ...
                    if (oldShell != null)
                        ((SwtShell) oldShell.getImpl()).sendToolTipEvent(false);
                    if (shell != null)
                        ((SwtShell) shell.getImpl()).sendToolTipEvent(true);
                }
                ((SwtDisplay) display.getImpl()).tooltipControl = control;
                ((SwtDisplay) display.getImpl()).tooltipTarget = target;
                break;
            case OS.NSKeyDown:
                /*
			...
			*/
                if (((SwtDisplay) display.getImpl()).escAsAcceleratorPresent && nsEvent.keyCode() == 53) /* ESC */
                {
                    ...
                    if (chars != null && chars.length() == 1) {
                        ...
                        switch(firstChar) {
                            ...
                            case OS.NSPageUpFunctionKey:
                                getApi().view.window().firstResponder().keyDown(nsEvent);
                                return;
                        }
                    }
                }
    ...
    private void updateEscMenuItem() {
        ...
        } else if (((SwtDisplay) display.getImpl()).appMenuBar != null && !((SwtDisplay) display.getImpl()).appMenuBar.isDisposed()) {
            searchForEscMenuItem(((SwtDisplay) display.getImpl()).appMenuBar);
        }
    }
