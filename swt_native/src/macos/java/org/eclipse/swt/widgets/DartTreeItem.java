...
public class DartTreeItem extends DartItem implements ITreeItem {
    ...
    public DartTreeItem(TreeItem parentItem, int style, TreeItem api) {
        this(((DartTreeItem) checkNull(parentItem).getImpl()).parent, parentItem, style, -1, true, api);
    }
    ...
    public DartTreeItem(TreeItem parentItem, int style, int index, TreeItem api) {
        this(((DartTreeItem) checkNull(parentItem).getImpl()).parent, parentItem, style, checkIndex(index), true, api);
    }

    DartTreeItem(Tree parent, TreeItem parentItem, int style, int index, boolean create, TreeItem api) {
        super(parent, style, api);
        this.parent = parent;
        this.parentItem = parentItem;
        if (create) {
            ((DartTree) parent.getImpl()).createItem(this.getApi(), parentItem, index);
        } else {
            createJNIRef();
            register();
    ...
    public void clear(int index, boolean all) {
        ...
        ((DartTree) parent.getImpl()).clear(this.getApi(), index, all);
    }
    ...
    public void clearAll(boolean all) {
        checkWidget();
        ((DartTree) parent.getImpl()).clearAll(this.getApi(), all);
    }
    ...
    public TreeItem getItem(int index) {
        ...
        if (index >= itemCount)
            error(SWT.ERROR_INVALID_RANGE);
        return ((DartTree) parent.getImpl())._getItem(this.getApi(), index, true);
    }
    ...
    public TreeItem[] getItems() {
        ...
        for (int i = 0; i < itemCount; i++) {
            result[i] = ((DartTree) parent.getImpl())._getItem(this.getApi(), i, true);
        }
        return result;
    }
    ...
    public int indexOf(TreeItem item) {
        ...
        if (((DartTreeItem) item.getImpl()).parentItem != this.getApi())
            return -1;
        for (int i = 0; i < itemCount; i++) {
            if (item == items[i])
                return i;
    ...
    void redraw(int columnIndex) {
        if (((DartTree) parent.getImpl()).ignoreRedraw || !isDrawing())
            return;
        if (columnIndex == -1 || ((DartWidget) parent.getImpl()).hooks(SWT.MeasureItem) || ((DartWidget) parent.getImpl()).hooks(SWT.EraseItem) || ((DartWidget) parent.getImpl()).hooks(SWT.PaintItem)) {
            ...
            if (((DartTree) parent.getImpl()).columnCount == 0) {
                ...
                if (0 <= columnIndex && columnIndex < ((DartTree) parent.getImpl()).columnCount) {
                } else {
                    return;
                }
            }
    ...
    public void release(boolean destroy) {
        ...
        if (selectedItems != null)
            ((DartTree) parent.getImpl()).selectItems(selectedItems, true);
    }
    ...
    public void removeAll() {
        checkWidget();
        ((DartTree) parent.getImpl()).setItemCount(this.getApi(), 0);
    }
    ...
    void sendExpand(boolean expand, boolean recurse) {
        ...
        if (recurse) {
            for (int i = 0; i < itemCount; i++) {
                if (items[i] != null)
                    ((DartTreeItem) items[i].getImpl()).sendExpand(expand, recurse);
            }
        }
    }
    ...
    public void setImage(int index, Image image) {
        ...
        if (((DartTree) parent.getImpl()).imageBounds == null && image != null) {
        }
        if (index == 0) {
        ...
        cached = true;
        if (index == 0)
            ((DartTree) parent.getImpl()).setScrollWidth(this.getApi());
        if (0 <= index && index < count)
            redraw(index);
    }
    ...
    public void setItemCount(int count) {
        ...
        ((DartTree) parent.getImpl()).setItemCount(this.getApi(), count);
    }
