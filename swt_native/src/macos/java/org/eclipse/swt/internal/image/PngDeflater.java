...
public class PngDeflater {
    ...
    void compress() {
        ...
        // BFINAL = 0x01 (final block)
        writeBits(0x01, 1);
        // BTYPE = 0x01 (compression with fixed Huffman codes)
        writeBits(0x01, 2);
    ...
    void store() {
        ...
        int length = inLength;
        int blockLength;
        // BFINAL = 0x00 or 0x01 (if final block), BTYPE = 0x00 (no compression)
        int BFINAL = 0x00;
        while (length > 0) {
            if (length < 65535) {
                blockLength = length;
    ...
    public byte[] deflate(byte[] input) {
        ...
        // write zlib header
        // window size = 0x70 (32768), compression method = 0x08
        bytes.write((byte) 0x78);
        // compression level = 0x80 (default), check bits = 0x1C
        bytes.write((byte) 0x9C);
        // compute checksum
