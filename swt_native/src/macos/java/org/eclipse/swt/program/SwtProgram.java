...
public final class SwtProgram implements IProgram {
    ...
    static Program getProgram(NSBundle bundle) {
        ...
        ((SwtProgram) program.getImpl()).name = name.getString();
        if (fullPath != null)
            ((SwtProgram) program.getImpl()).fullPath = fullPath.getString();
        ((SwtProgram) program.getImpl()).identifier = identifier != null ? identifier.getString() : "";
        return program;
    }
