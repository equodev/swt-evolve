...
public class SwtGLCanvas extends SwtCanvas implements IGLCanvas {
	 ...
	 */
        ...
        NSOpenGLContext ctx = data.shareContext != null ? ((SwtGLCanvas) data.shareContext.getImpl()).context : null;
        ...
        NSNotificationCenter.defaultCenter().addObserver(getApi().view, OS.sel_updateOpenGLContext_, OS.NSViewGlobalFrameDidChangeNotification, getApi().view);
        Listener listener = event -> {
            switch(event.type) {
                case SWT.Dispose:
                    setData(GLCONTEXT_KEY, null);
                    NSNotificationCenter.defaultCenter().removeObserver(getApi().view);
                    if (context != null) {
