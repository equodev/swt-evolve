...
public class SwtCTabFolderRenderer implements ICTabFolderRenderer {
    ...
    void antialias(int[] shape, Color innerColor, Color outerColor, GC gc) {
        ...
        if (getApi().parent.getImpl()._simple())
            return;
        ...
        if (outerColor != null) {
            int index = 0;
            boolean left = true;
            int oldY = getApi().parent.getImpl()._onBottom() ? 0 : getApi().parent.getSize().y;
            int[] outer = new int[shape.length];
            for (int i = 0; i < shape.length / 2; i++) {
                if (left && (index + 3 < shape.length)) {
                    left = getApi().parent.getImpl()._onBottom() ? oldY <= shape[index + 3] : oldY >= shape[index + 3];
                    oldY = shape[index + 1];
                }
    ...
    public Point computeSize(int part, int state, GC gc, int wHint, int hHint) {
        ...
        switch(part) {
            case CTabFolderRenderer.PART_HEADER:
                if (getApi().parent.getImpl()._fixedTabHeight() != SWT.DEFAULT) {
                    // +1 for line drawn across top of tab
                    height = getApi().parent.getImpl()._fixedTabHeight() == 0 ? 0 : getApi().parent.getImpl()._fixedTabHeight() + 1;
                } else {
                    CTabItem[] items = getApi().parent.getImpl()._items();
                    if (items.length == 0) {
                        //$NON-NLS-1$
            ...
            default:
                if (0 <= part && part < getApi().parent.getItemCount()) {
                    updateCurves();
                    CTabItem item = getApi().parent.getImpl()._items()[part];
                    if (item.isDisposed())
                        return new Point(0, 0);
                    Image image = item.getImage();
                    if (image != null && !image.isDisposed()) {
                        Rectangle bounds = image.getBounds();
                        if (((state & SWT.SELECTED) != 0 && getApi().parent.getImpl()._showSelectedImage()) || ((state & SWT.SELECTED) == 0 && getApi().parent.getImpl()._showUnselectedImage())) {
                            width += bounds.width;
                        }
                    ...
                    if ((state & CTabFolderRenderer.MINIMUM_SIZE) != 0) {
                        int minChars = getApi().parent.getImpl()._minChars();
    ...
    private boolean shouldDrawCloseIcon(CTabItem item) {
        ...
        boolean showClose = folder.getImpl()._showClose() || ((SwtCTabItem) item.getImpl()).showClose;
        boolean isSelectedOrShowCloseForUnselected = (item.state & SWT.SELECTED) != 0 || folder.getImpl()._showUnselectedClose();
    ...
    private boolean shouldApplyLargeTextPadding(CTabFolder tabFolder) {
        return !tabFolder.getImpl()._showSelectedImage() && !tabFolder.getImpl()._showUnselectedImage();
    }
    ...
    public Rectangle computeTrim(int part, int state, int x, int y, int width, int height) {
        int borderLeft = getApi().parent.getImpl()._borderVisible() ? 1 : 0;
        int borderRight = borderLeft;
        int borderTop = getApi().parent.getImpl()._onBottom() ? borderLeft : 0;
        int borderBottom = getApi().parent.getImpl()._onBottom() ? 0 : borderLeft;
        int tabHeight = getApi().parent.getImpl()._tabHeight();
        switch(part) {
            case CTabFolderRenderer.PART_BODY:
                ...
                if (getApi().parent.getImpl()._fixedTabHeight() == 0 && (style & SWT.FLAT) != 0 && (style & SWT.BORDER) == 0) {
                    highlight_header = 0;
                }
                ...
                if (getApi().parent.getImpl()._minimized()) {
                    y = getApi().parent.getImpl()._onBottom() ? y - borderTop : y - highlight_header - tabHeight - borderTop;
                    height = borderTop + borderBottom + tabHeight + highlight_header;
                } else {
                    y = getApi().parent.getImpl()._onBottom() ? y - marginHeight - highlight_margin - borderTop : y - marginHeight - highlight_header - tabHeight - borderTop;
            ...
            default:
                if (0 <= part && part < getApi().parent.getItemCount()) {
                    ...
                    if (!getApi().parent.getImpl()._simple() && !getApi().parent.getImpl()._single() && (state & SWT.SELECTED) != 0) {
    ...
    void createAntialiasColors() {
        ...
        RGB innerRGB = getApi().parent.getImpl()._selectionBackground().getRGB();
        if (getApi().parent.getImpl()._selectionBgImage() != null || (getApi().parent.getImpl()._selectionGradientColors() != null && getApi().parent.getImpl()._selectionGradientColors().length > 1)) {
            innerRGB = null;
        }
        RGB outerRGB = getApi().parent.getBackground().getRGB();
        if (getApi().parent.getImpl()._gradientColors() != null && getApi().parent.getImpl()._gradientColors().length > 1) {
            outerRGB = null;
        }
        if (outerRGB != null) {
            RGB from = lineRGB;
    ...
    void createSelectionHighlightGradientColors(Color start) {
        ...
        int fadeGradientSize = getApi().parent.getImpl()._tabHeight();
        RGB from = start.getRGB();
        RGB to = getApi().parent.getImpl()._selectionBackground().getRGB();
    ...
    void drawBackground(GC gc, Rectangle bounds, int state) {
        ...
        Color defaultBackground = selected && ((SwtCTabFolder) getApi().parent.getImpl()).shouldHighlight() ? getApi().parent.getImpl()._selectionBackground() : getApi().parent.getBackground();
        Image image = selected ? getApi().parent.getImpl()._selectionBgImage() : null;
        Color[] colors = selected & ((SwtCTabFolder) getApi().parent.getImpl()).shouldHighlight() ? getApi().parent.getImpl()._selectionGradientColors() : getApi().parent.getImpl()._gradientColors();
        int[] percents = selected ? getApi().parent.getImpl()._selectionGradientPercents() : getApi().parent.getImpl()._gradientPercents();
        boolean vertical = selected ? getApi().parent.getImpl()._selectionGradientVertical() : getApi().parent.getImpl()._gradientVertical();
    ...
    void drawBackground(GC gc, int[] shape, boolean selected) {
        ...
        int width = size.x;
        int height = getApi().parent.getImpl()._tabHeight() + ((getApi().parent.getStyle() & SWT.FLAT) != 0 ? 1 : 3);
        int x = 0;
        int borderLeft = getApi().parent.getImpl()._borderVisible() ? 1 : 0;
        int borderTop = getApi().parent.getImpl()._onBottom() ? borderLeft : 0;
        int borderBottom = getApi().parent.getImpl()._onBottom() ? 0 : borderLeft;
        ...
        int y = getApi().parent.getImpl()._onBottom() ? size.y - borderBottom - height : borderTop;
    ...
    void drawBackground(GC gc, int[] shape, int x, int y, int width, int height, Color defaultBackground, Image image, Color[] colors, int[] percents, boolean vertical) {
        ...
        if (image != null) {
            ...
            if (colors.length == 1) {
                ...
                if (vertical) {
                    if (getApi().parent.getImpl()._onBottom()) {
                        int pos = 0;
    ...
    void drawBody(GC gc, Rectangle bounds, int state) {
        ...
        int selectedIndex = getApi().parent.getImpl()._selectedIndex();
        int tabHeight = getApi().parent.getImpl()._tabHeight();
        int borderLeft = getApi().parent.getImpl()._borderVisible() ? 1 : 0;
        int borderRight = borderLeft;
        int borderTop = getApi().parent.getImpl()._onBottom() ? borderLeft : 0;
        ...
        if (!getApi().parent.getImpl()._minimized()) {
            ...
            if (highlight_margin > 0) {
                ...
                if (selectedIndex != -1 && getApi().parent.getImpl()._selectionGradientColors() != null && getApi().parent.getImpl()._selectionGradientColors().length > 1 && !getApi().parent.getImpl()._selectionGradientVertical()) {
                    drawBackground(gc, shape, true);
                } else if (selectedIndex == -1 && getApi().parent.getImpl()._gradientColors() != null && getApi().parent.getImpl()._gradientColors().length > 1 && !getApi().parent.getImpl()._gradientVertical()) {
                    drawBackground(gc, shape, false);
                } else {
                    gc.setBackground(selectedIndex != -1 && ((SwtCTabFolder) getApi().parent.getImpl()).shouldHighlight() ? getApi().parent.getImpl()._selectionBackground() : getApi().parent.getBackground());
        ...
        if (borderLeft > 0) {
            ...
            int x1 = borderLeft - 1;
            int x2 = size.x - borderRight;
            int y1 = getApi().parent.getImpl()._onBottom() ? borderTop - 1 : borderTop + tabHeight;
            int y2 = getApi().parent.getImpl()._onBottom() ? size.y - tabHeight - borderBottom - 1 : size.y - borderBottom;
            // left
    ...
    void drawClose(GC gc, Rectangle closeRect, int closeImageState) {
        ...
        y += getApi().parent.getImpl()._onBottom() ? -1 : 1;
    ...
    void drawChevron(GC gc, Rectangle chevronRect, int chevronImageState) {
        ...
        switch(chevronImageState & (SWT.HOT | SWT.SELECTED)) {
            case SWT.NONE:
                {
                    Color chevronBorder = getApi().parent.getImpl()._single() ? getApi().parent.getSelectionForeground() : getApi().parent.getForeground();
    ...
    void drawHighlight(GC gc, Rectangle bounds, int state, int rightEdge) {
        ...
        if (getApi().parent.getImpl()._simple() || getApi().parent.getImpl()._onBottom())
            return;
        ...
        int d = getApi().parent.getImpl()._tabHeight() - topCurveHighlightEnd.length / 2;
        int lastX = 0;
        int lastY = 0;
    ...
    void drawLeftUnselectedBorder(GC gc, Rectangle bounds, int state) {
        ...
        int[] shape = null;
        if (getApi().parent.getImpl()._onBottom()) {
            int[] left = getApi().parent.getImpl()._simple() ? SIMPLE_UNSELECTED_INNER_CORNER : BOTTOM_LEFT_CORNER;
            ...
            int[] left = getApi().parent.getImpl()._simple() ? SIMPLE_UNSELECTED_INNER_CORNER : TOP_LEFT_CORNER;
            shape = new int[left.length + 2];
            int index = 0;
            shape[index++] = x;
    ...
    void drawRightUnselectedBorder(GC gc, Rectangle bounds, int state) {
        ...
        if (getApi().parent.getImpl()._onBottom()) {
            int[] right = getApi().parent.getImpl()._simple() ? SIMPLE_UNSELECTED_INNER_CORNER : BOTTOM_RIGHT_CORNER;
            ...
            int[] right = getApi().parent.getImpl()._simple() ? SIMPLE_UNSELECTED_INNER_CORNER : TOP_RIGHT_CORNER;
            shape = new int[right.length + 2];
            int index = 0;
            for (int i = 0; i < right.length / 2; i++) {
                shape[index++] = startX + right[2 * i];
    ...
    void drawSelected(int itemIndex, GC gc, Rectangle bounds, int state) {
        CTabItem item = getApi().parent.getImpl()._items()[itemIndex];
        int x = bounds.x;
        int y = bounds.y;
        int height = bounds.height;
        int width = bounds.width;
        if (!getApi().parent.getImpl()._simple() && !getApi().parent.getImpl()._single())
            width -= (curveWidth - curveIndent);
        int borderLeft = getApi().parent.getImpl()._borderVisible() ? 1 : 0;
        int borderRight = borderLeft;
        int borderTop = getApi().parent.getImpl()._onBottom() ? borderLeft : 0;
        ...
        if ((state & SWT.BACKGROUND) != 0) {
            ...
            int xx = borderLeft;
            int yy = getApi().parent.getImpl()._onBottom() ? size.y - borderBottom - getApi().parent.getImpl()._tabHeight() - highlight_header : borderTop + getApi().parent.getImpl()._tabHeight() + 1;
            int ww = size.x - borderLeft - borderRight;
            int hh = highlight_header - 1;
            int[] shape = new int[] { xx, yy, xx + ww, yy, xx + ww, yy + hh, xx, yy + hh };
            if (getApi().parent.getImpl()._selectionGradientColors() != null && !getApi().parent.getImpl()._selectionGradientVertical()) {
                drawBackground(gc, shape, ((SwtCTabFolder) getApi().parent.getImpl()).shouldHighlight());
            } else {
                gc.setBackground(((SwtCTabFolder) getApi().parent.getImpl()).shouldHighlight() ? getApi().parent.getImpl()._selectionBackground() : getApi().parent.getBackground());
                gc.fillRectangle(xx, yy, ww, hh);
            }
            if (getApi().parent.getImpl()._single()) {
                ...
                if (!((SwtCTabItem) item.getImpl()).showing) {
                    ...
                    int y1 = (getApi().parent.getImpl()._onBottom()) ? y - 1 : y + height;
                    int x2 = size.x - borderRight;
                    gc.setForeground(getApi().parent.getDisplay().getSystemColor(BORDER1_COLOR));
                    gc.drawLine(x1, y1, x2, y1);
                    return;
                ...
                shape = null;
                if (getApi().parent.getImpl()._onBottom()) {
                    int[] left = getApi().parent.getImpl()._simple() ? SIMPLE_BOTTOM_LEFT_CORNER : BOTTOM_LEFT_CORNER;
                    int[] right = getApi().parent.getImpl()._simple() ? SIMPLE_BOTTOM_RIGHT_CORNER : curve;
                    if (borderLeft == 0 && itemIndex == getApi().parent.getImpl()._firstIndex()) {
                    ...
                    for (int i = 0; i < right.length / 2; i++) {
                        shape[index++] = getApi().parent.getImpl()._simple() ? rightEdge - 1 + right[2 * i] : rightEdge - curveIndent + right[2 * i];
                        shape[index++] = getApi().parent.getImpl()._simple() ? y + height + right[2 * i + 1] - 1 : y + right[2 * i + 1] - 2;
                    }
                    shape[index++] = getApi().parent.getImpl()._simple() ? rightEdge - 1 : rightEdge + curveWidth - curveIndent;
                    shape[index++] = y - 1;
                    shape[index++] = getApi().parent.getImpl()._simple() ? rightEdge - 1 : rightEdge + curveWidth - curveIndent;
                    shape[index++] = y - 1;
                } else {
                    int[] left = getApi().parent.getImpl()._simple() ? SIMPLE_TOP_LEFT_CORNER : TOP_LEFT_CORNER;
                    int[] right = getApi().parent.getImpl()._simple() ? SIMPLE_TOP_RIGHT_CORNER : curve;
                    if (borderLeft == 0 && itemIndex == getApi().parent.getImpl()._firstIndex()) {
                        left = new int[] { x, y };
                    }
                    shape = new int[left.length + right.length + 8];
                ...
                if (tabInPaint) {
                    ...
                    if (getApi().parent.getImpl()._selectionGradientColors() != null && !getApi().parent.getImpl()._selectionGradientVertical()) {
                        ...
                        Color defaultBackground = ((SwtCTabFolder) getApi().parent.getImpl()).shouldHighlight() ? getApi().parent.getImpl()._selectionBackground() : getApi().parent.getBackground();
                        Image image = getApi().parent.getImpl()._selectionBgImage();
                        Color[] colors = getApi().parent.getImpl()._selectionGradientColors();
                        int[] percents = getApi().parent.getImpl()._selectionGradientPercents();
                        boolean vertical = getApi().parent.getImpl()._selectionGradientVertical();
                        xx = x;
                        yy = getApi().parent.getImpl()._onBottom() ? y - 1 : y + 1;
                        ww = width;
                        hh = height;
                        if (!getApi().parent.getImpl()._single() && !getApi().parent.getImpl()._simple())
                ...
                if (getApi().parent.getImpl()._selectionHighlightBarThickness() > 0 && getApi().parent.getImpl()._simple()) {
                    ...
                    getApi().parent.getImpl()._onBottom() ? y + height - 1 - getApi().parent.getImpl()._selectionHighlightBarThickness() : y + 1, width - 2, getApi().parent.getImpl()._selectionHighlightBarThickness());
                    gc.setBackground(previousColor);
                }
                // draw outline
                shape[0] = Math.max(0, borderLeft - 1);
                if (borderLeft == 0 && itemIndex == getApi().parent.getImpl()._firstIndex()) {
                    shape[1] = getApi().parent.getImpl()._onBottom() ? y + height - 1 : y;
        ...
        if ((state & SWT.FOREGROUND) != 0) {
            ...
            int xDraw = x - trim.x;
            if (getApi().parent.getImpl()._single() && shouldDrawCloseIcon(item))
                xDraw += ((SwtCTabItem) item.getImpl()).closeRect.width;
            Image image = item.getImage();
            if (image != null && !image.isDisposed() && getApi().parent.getImpl()._showSelectedImage()) {
                Rectangle imageBounds = image.getBounds();
                // only draw image if it won't overlap with close button
                int maxImageWidth = rightEdge - xDraw - (trim.width + trim.x);
                if (!getApi().parent.getImpl()._single() && ((SwtCTabItem) item.getImpl()).closeRect.width > 0)
                    maxImageWidth -= ((SwtCTabItem) item.getImpl()).closeRect.width + INTERNAL_SPACING;
                if (imageBounds.width < maxImageWidth) {
                    int imageX = xDraw;
                    int imageY = y + (height - imageBounds.height) / 2;
                    imageY += getApi().parent.getImpl()._onBottom() ? -1 : 1;
            ...
            if (textWidth > 0) {
                ...
                textY += getApi().parent.getImpl()._onBottom() ? -1 : 1;
    ...
    void drawTabArea(GC gc, Rectangle bounds, int state) {
        ...
        if (tabHeight == 0) {
            ...
            int x1 = borderLeft - 1;
            int x2 = size.x - borderRight;
            int y1 = getApi().parent.getImpl()._onBottom() ? size.y - borderBottom - highlight_header - 1 : borderTop + highlight_header;
            int y2 = getApi().parent.getImpl()._onBottom() ? size.y - borderBottom : borderTop;
            if (borderLeft > 0 && getApi().parent.getImpl()._onBottom())
                y2 -= 1;
        ...
        int y = getApi().parent.getImpl()._onBottom() ? size.y - borderBottom - tabHeight : borderTop;
        int width = size.x - borderLeft - borderRight + 1;
        int height = tabHeight - 1;
        boolean simple = getApi().parent.getImpl()._simple();
        // Draw Tab Header
        if (getApi().parent.getImpl()._onBottom()) {
            int[] left, right;
        ...
        // Fill in background
        boolean single = getApi().parent.getImpl()._single();
        ...
        if (selectedIndex == -1) {
            ...
            int x1 = borderLeft;
            int y1 = (getApi().parent.getImpl()._onBottom()) ? size.y - borderBottom - tabHeight - 1 : borderTop + tabHeight;
            int x2 = size.x - borderRight;
            gc.setForeground(borderColor);
            gc.drawLine(x1, y1, x2, y1);
        }
    ...
    void drawUnselected(int index, GC gc, Rectangle bounds, int state) {
        CTabItem item = getApi().parent.getImpl()._items()[index];
        int x = bounds.x;
        int y = bounds.y;
        int height = bounds.height;
        int width = bounds.width;
        ...
        if (!clipping.intersects(bounds))
            return;
        if ((state & SWT.BACKGROUND) != 0) {
            if (index > 0 && index < getApi().parent.getImpl()._selectedIndex())
                drawLeftUnselectedBorder(gc, bounds, state);
            // If it is the last one then draw a line
            if (index > getApi().parent.getImpl()._selectedIndex())
        ...
        if ((state & SWT.FOREGROUND) != 0) {
            ...
            int xDraw = x - trim.x;
            Image image = item.getImage();
            if (image != null && !image.isDisposed() && getApi().parent.getImpl()._showUnselectedImage()) {
                Rectangle imageBounds = image.getBounds();
                // only draw image if it won't overlap with close button
    ...
    boolean isSelectionHighlightColorsCacheHit(Color start) {
        ...
        if (selectionHighlightGradientColorsCache.length != getApi().parent.getImpl()._tabHeight())
            return false;
        //Compare existing highlight end to what it would be (selectionBackground)
        if (!highlightEnd.equals(getApi().parent.getImpl()._selectionBackground()))
            return false;
        return true;
    }
    ...
    void setSelectionHighlightGradientColor(Color start) {
        ...
        if (getApi().parent.getImpl()._selectionGradientColors().length < 2)
            return;
	 ...
	 */
    boolean useEllipses() {
        return getApi().parent.getImpl()._simple();
    }
