 ...
 */
class SwtStyledTextRenderer implements IStyledTextRenderer {
    ...
    SwtStyledTextRenderer(Device device, StyledText styledText, StyledTextRenderer api) {
        setApi(api);
        this.device = device;
    ...
    void copyInto(StyledTextRenderer renderer) {
        if (ranges != null) {
            int[] newRanges = ((SwtStyledTextRenderer) renderer.getImpl()).ranges = new int[styleCount << 1];
            System.arraycopy(ranges, 0, newRanges, 0, newRanges.length);
        }
        if (styles != null) {
            StyleRange[] newStyles = ((SwtStyledTextRenderer) renderer.getImpl()).styles = new StyleRange[styleCount];
            for (int i = 0; i < newStyles.length; i++) {
                newStyles[i] = (StyleRange) styles[i].clone();
            }
            ((SwtStyledTextRenderer) renderer.getImpl()).styleCount = styleCount;
        }
        if (lines != null) {
            LineInfo[] newLines = ((SwtStyledTextRenderer) renderer.getImpl()).lines = new LineInfo[lineCount];
            for (int i = 0; i < newLines.length; i++) {
                newLines[i] = new LineInfo(lines[i]);
            }
            ((SwtStyledTextRenderer) renderer.getImpl()).lineCount = lineCount;
        }
    }
