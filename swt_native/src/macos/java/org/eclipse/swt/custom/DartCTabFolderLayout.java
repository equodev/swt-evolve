...
class DartCTabFolderLayout extends DartLayout implements ICTabFolderLayout {
    ...
    public Point computeSize(Composite composite, int wHint, int hHint, boolean flushCache) {
        ...
        if (wHint == SWT.DEFAULT) {
            ...
            Rectangle[] rects = ((DartCTabFolder) folder.getImpl()).computeControlBounds(size, positions);
        ...
        if (leftControl)
            width += DartCTabFolder.SPACING * 2;
        if (rightControl)
            width += DartCTabFolder.SPACING * 2;
        tabW += width;
        gc.dispose();
        ...
        if (minWidth == 0)
            minWidth = DartCTabFolder.DEFAULT_WIDTH;
        if (minHeight == 0)
            minHeight = DartCTabFolder.DEFAULT_HEIGHT;
        if (wHint != SWT.DEFAULT)
            minWidth = wHint;
