...
public final class SwtImage extends SwtResource implements Drawable, IImage {
    ...
    public SwtImage(Device device, Image srcImage, int flag, Image api) {
        ...
        try {
            ...
            copyAlphaInfo(((SwtImage) srcImage.getImpl()).alphaInfo_100, alphaInfo_100);
            if (((SwtImage) srcImage.getImpl()).alphaInfo_200 != null) {
                alphaInfo_200 = new AlphaInfo();
                copyAlphaInfo(((SwtImage) srcImage.getImpl()).alphaInfo_200, alphaInfo_200);
            }
            /* Create the image */
            getApi().handle = (NSImage) new NSImage().alloc();
            ...
            createRepFromSourceAndApplyFlag(((SwtImage) srcImage.getImpl()).getOrCreateRepresentation(100), srcWidth, srcHeight, flag);
    ...
    ImageData _getImageData(NSBitmapImageRep imageRep, AlphaInfo info) {
        ...
        if (imageRep.hasAlpha() && info.transparentPixel == -1 && info.alpha == -1 && info.alphaData == null) {
            ...
            if (info.transparentPixel == -1 && getApi().type == SWT.ICON) {
    ...
    public static Image cocoa_new(Device device, int type, NSImage nsImage) {
        ...
        ((SwtImage) image.getImpl()).alphaInfo_100 = new AlphaInfo();
        return image;
    }
    ...
    private NSBitmapImageRep createRepresentation(ImageData imageData, AlphaInfo alphaInfo) {
        ...
        if (palette.isDirect) {
            ImageData.blit(imageData.data, imageData.depth, imageData.bytesPerLine, imageData.getByteOrder(), imageData.width, imageData.height, palette.redMask, palette.greenMask, palette.blueMask, buffer, 32, bpr, ImageData.MSB_FIRST, imageData.width, imageData.height, 0xFF0000, 0xFF00, 0xFF, false, false);
        } else {
            ...
            ImageData.blit(imageData.width, imageData.height, imageData.data, imageData.depth, imageData.bytesPerLine, imageData.getByteOrder(), srcReds, srcGreens, srcBlues, buffer, 32, bpr, ImageData.MSB_FIRST, 0xFF0000, 0xFF00, 0xFF);
        }
        ...
        if (transparency == SWT.TRANSPARENCY_MASK || imageData.transparentPixel != -1) {
            this.getApi().type = imageData.transparentPixel != -1 ? SWT.BITMAP : SWT.ICON;
    ...
    NSBitmapImageRep getOrCreateRepresentation(int scaleFactor) {
        ...
        long repCount = getApi().handle.representations().count();
        NSSize targetSize = getTargetSize(scaleFactor);
        NSBitmapImageRep newRep = createImageRep(targetSize);
        for (int i = 0; i < repCount; i++) {
            getApi().handle.removeRepresentation(new NSImageRep(getApi().handle.representations().objectAtIndex(0)));
        }
        getApi().handle.addRepresentation(newRep);
        return newRep;
    }
    ...
    NSBitmapImageRep getRepresentation(int scaleFactor) {
        NSArray reps = getApi().handle.representations();
        NSSize targetSize = getTargetSize(scaleFactor);
    ...
    private NSSize getTargetSize(int scaleFactor) {
        NSSize size = getApi().handle.size();
    ...
    private class CachedImageAtSize {
        ...
        private Optional<Image> loadImageAtSize(int destWidth, int destHeight) {
            ...
            if (styleFlag != SWT.IMAGE_COPY) {
                NSBitmapImageRep representation = ((SwtImage) image.getImpl()).getOrCreateRepresentation(100);
