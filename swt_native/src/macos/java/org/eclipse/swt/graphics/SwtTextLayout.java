...
public final class SwtTextLayout extends SwtResource implements ITextLayout {
	...
	*/
        ...
        if (numberOfLines == 0) {
            Font font = this.font != null ? this.font : ((SwtDevice) device.getImpl()).systemFont;
    ...
    public FontMetrics getLineMetrics(int lineIndex) {
        ...
        try {
            ...
            if (!(0 <= lineIndex && lineIndex < lineCount))
                SWT.error(SWT.ERROR_INVALID_RANGE);
            if (fixedLineMetrics != null)
                return ((SwtFontMetrics) fixedLineMetrics.getImpl()).makeCopy();
            int length = text.length();
            if (length == 0) {
                Font font = this.font != null ? this.font : ((SwtDevice) device.getImpl()).systemFont;
                ...
                return SwtFontMetrics.cocoa_new(ascent, descent, 0.0, 0, ascent + descent);
            }
            ...
            return SwtFontMetrics.cocoa_new(rect.height - baseline, baseline, 0.0, 0, rect.height);
        } finally {
            if (pool != null)
                pool.release();
        }
