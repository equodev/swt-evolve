...
public final class SwtCursor extends SwtResource implements ICursor {
        ...
        try {
            switch(style) {
                case SWT.CURSOR_HAND:
                    getApi().handle = NSCursor.pointingHandCursor();
                    break;
                case SWT.CURSOR_ARROW:
                    getApi().handle = NSCursor.arrowCursor();
                    break;
                case SWT.CURSOR_WAIT:
                    {
                        getApi().handle = busyButClickableCursor();
                ...
                case SWT.CURSOR_CROSS:
                    getApi().handle = NSCursor.crosshairCursor();
                    break;
                ...
                case SWT.CURSOR_SIZENS:
                    getApi().handle = NSCursor.resizeUpDownCursor();
                    break;
                case SWT.CURSOR_SIZENWSE:
                    getApi().handle = NSCursor.crosshairCursor();
                    break;
                case SWT.CURSOR_SIZEWE:
                    getApi().handle = NSCursor.resizeLeftRightCursor();
                    break;
                case SWT.CURSOR_SIZEN:
                    getApi().handle = NSCursor.resizeUpCursor();
                    break;
                case SWT.CURSOR_SIZES:
                    getApi().handle = NSCursor.resizeDownCursor();
                    break;
                case SWT.CURSOR_SIZEE:
                    getApi().handle = NSCursor.resizeRightCursor();
                    break;
                case SWT.CURSOR_SIZEW:
                    getApi().handle = NSCursor.resizeLeftCursor();
                    break;
                ...
                case SWT.CURSOR_IBEAM:
                    ...
                    if (OS.VERSION < OS.VERSION(10, 14, 0)) {
                        ...
                        getApi().handle = NSCursor.IBeamCursor();
                    }
                    break;
                case SWT.CURSOR_NO:
                    getApi().handle = NSCursor.operationNotAllowedCursor();
                    break;
                default:
                    SWT.error(SWT.ERROR_INVALID_ARGUMENT);
            }
            if (getApi().handle == null && shouldCreateCursor) {
                NSImage nsImage = (NSImage) new NSImage().alloc();
                NSBitmapImageRep nsImageRep = (NSBitmapImageRep) new NSBitmapImageRep().alloc();
                getApi().handle = (NSCursor) new NSCursor().alloc();
                int width = 16, height = 16;
                NSSize size = new NSSize();
                size.width = width;
                size.height = height;
                nsImage = nsImage.initWithSize(size);
                nsImageRep = nsImageRep.initWithBitmapDataPlanes(0, width, height, 8, 4, true, false, OS.NSDeviceRGBColorSpace, OS.NSAlphaFirstBitmapFormat | OS.NSAlphaNonpremultipliedBitmapFormat, width * 4, 32);
                ...
                if (style == SWT.CURSOR_WAIT) {
                    ...
                    // values from NSCursor.IBeamCursor().hotSpot();
                    point.x = 4;
                    // values from NSCursor.IBeamCursor().hotSpot();
                    point.y = 8;
                }
                nsImage.addRepresentation(nsImageRep);
                getApi().handle = getApi().handle.initWithImage(nsImage, point);
                nsImageRep.release();
                nsImage.release();
            } else {
                getApi().handle.retain();
            }
            getApi().handle.setOnMouseEntered(true);
            init();
        } finally {
            if (pool != null)
                pool.release();
    ...
    void createNSCursor(int hotspotX, int hotspotY, byte[] buffer, int width, int height, boolean hasAlpha) {
        ...
        size.height = height;
        nsImage = nsImage.initWithSize(size);
        nsImageRep = nsImageRep.initWithBitmapDataPlanes(0, width, height, 8, hasAlpha ? 4 : 3, hasAlpha, false, OS.NSDeviceRGBColorSpace, OS.NSAlphaFirstBitmapFormat | OS.NSAlphaNonpremultipliedBitmapFormat, width * 4, 32);
