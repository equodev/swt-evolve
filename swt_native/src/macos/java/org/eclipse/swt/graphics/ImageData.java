...
public final class ImageData implements Cloneable {
    ...
    public ImageData getTransparencyMask() {
        ...
        return switch(transparencyType) {
            ...
            case SWT.TRANSPARENCY_MASK ->
                new ImageData(width, height, 1, bwPalette(), maskPad, maskData);
    ...
    public ImageData scaledTo(int width, int height) {
        ...
        if (palette.isDirect)
            blit(this.data, this.depth, this.bytesPerLine, this.getByteOrder(), this.width, this.height, 0, 0, 0, dest.data, dest.depth, dest.bytesPerLine, dest.getByteOrder(), dest.width, dest.height, 0, 0, 0, flipX, flipY);
        else
            blit(this.data, this.depth, this.bytesPerLine, this.getByteOrder(), this.width, this.height, dest.data, dest.depth, dest.bytesPerLine, dest.getByteOrder(), dest.width, dest.height, flipX, flipY);
        ...
        if (maskData != null) {
            ...
            blit(this.maskData, 1, srcBpl, MSB_FIRST, this.width, this.height, dest.maskData, 1, destBpl, MSB_FIRST, dest.width, dest.height, flipX, flipY);
        } else if (alpha != -1) {
            dest.alpha = this.alpha;
        } else if (alphaData != null) {
            dest.alphaData = new byte[dest.width * dest.height];
            blit(this.alphaData, 8, this.width, MSB_FIRST, this.width, this.height, dest.alphaData, 8, dest.width, MSB_FIRST, dest.width, dest.height, flipX, flipY);
        }
        return dest;
    }
    ...
    TYPE_INDEX_16_LSB = 11, TYPE_INDEX_8 = 6, TYPE_INDEX_4 = 7, TYPE_INDEX_2 = 8, TYPE_INDEX_1_MSB = 9, TYPE_INDEX_1_LSB = 10;
    ...
    static void blit(byte[] srcData, int srcDepth, int srcStride, int srcOrder, int srcWidth, int srcHeight, int srcRedMask, int srcGreenMask, int srcBlueMask, byte[] destData, int destDepth, int destStride, int destOrder, int destWidth, int destHeight, int destRedMask, int destGreenMask, int destBlueMask, boolean flipX, boolean flipY) {
        ...
        for (int dy = destHeight, sfy = sfyi; dy > 0; --dy, sp = spr += (sfy >>> 16) * srcStride, sfy = (sfy & 0xffff) + sfyi, dp = dpr += dpryi) {
            for (int dx = destWidth, sfx = sfxi; dx > 0; --dx, dp += dprxi, sfx = (sfx & 0xffff) + sfxi) {
                 ...
                 * WRITE NEXT PIXEL **
                 */
                final int data = (r >>> destRedPreShift << destRedShift) | (g >>> destGreenPreShift << destGreenShift) | (b >>> destBluePreShift << destBlueShift);
                switch(dtype) {
                    case TYPE_GENERIC_8:
                        {
    ...
    static void blit(byte[] srcData, int srcDepth, int srcStride, int srcOrder, int srcWidth, int srcHeight, byte[] destData, int destDepth, int destStride, int destOrder, int destWidth, int destHeight, boolean flipX, boolean flipY) {
        ...
        if (stype == dtype) {
             ...
             */
            for (int dy = destHeight, sfy = sfyi; dy > 0; --dy, sp = spr += (sfy >>> 16) * srcPixelsPerStride, sfy = (sfy & 0xffff) + sfyi, dp = dpr += dpryi) {
                for (int dx = destWidth, sfx = sfxi; dx > 0; --dx, dp += dprxi, sp += (sfx >>> 16), sfx = (sfx & 0xffff) + sfxi) {
                    int index;
                    /**
                     * READ NEXT PIXEL **
                     */
 ...
 */
    ...
    static void fillGradientRectangle(GC gc, Device device, int x, int y, int width, int height, boolean vertical, RGB fromRGB, RGB toRGB, int redBits, int greenBits, int blueBits, int zoom) {
        ...
        ImageDataProvider imageDataProvider = imageZoom -> {
            ...
            return createGradientBand(scaledWidth, scaledHeight, vertical, fromRGB, toRGB, redBits, greenBits, blueBits);
        };
        Image image = new Image(device, imageDataProvider);
        ImageData band = image.getImageData(zoom);
        if ((band.width == 1) || (band.height == 1)) {
            gc.drawImage(image, 0, 0, DPIUtil.pixelToPoint(band.width, zoom), DPIUtil.pixelToPoint(band.height, zoom), DPIUtil.pixelToPoint(x, zoom), DPIUtil.pixelToPoint(y, zoom), DPIUtil.pixelToPoint(width, zoom), DPIUtil.pixelToPoint(height, zoom));
        } else {
            if (vertical) {
                for (int dx = 0; dx < width; dx += band.width) {
                    ...
                    gc.drawImage(image, 0, 0, DPIUtil.pixelToPoint(blitWidth, zoom), DPIUtil.pixelToPoint(band.height, zoom), DPIUtil.pixelToPoint(dx + x, zoom), DPIUtil.pixelToPoint(y, zoom), DPIUtil.pixelToPoint(blitWidth, zoom), DPIUtil.pixelToPoint(band.height, zoom));
                }
            } else {
                for (int dy = 0; dy < height; dy += band.height) {
                    ...
                    gc.drawImage(image, 0, 0, DPIUtil.pixelToPoint(band.width, zoom), DPIUtil.pixelToPoint(blitHeight, zoom), DPIUtil.pixelToPoint(x, zoom), DPIUtil.pixelToPoint(dy + y, zoom), DPIUtil.pixelToPoint(band.width, zoom), DPIUtil.pixelToPoint(blitHeight, zoom));
                }
            }
        }
        image.dispose();
