...
public class SwtPath extends SwtResource implements IPath {
    ...
    public SwtPath(Device device, Path api) {
        ...
        try {
            getApi().handle = NSBezierPath.bezierPath();
            if (getApi().handle == null)
                SWT.error(SWT.ERROR_NO_HANDLES);
            getApi().handle.retain();
            getApi().handle.moveToPoint(new NSPoint());
            init();
        } finally {
            if (pool != null)
                pool.release();
    ...
    public SwtPath(Device device, Path path, float flatness, Path api) {
        ...
        try {
            ...
            if (flatness == 0) {
                getApi().handle = new NSBezierPath(path.handle.copy().id);
            } else {
                double defaultFlatness = NSBezierPath.defaultFlatness();
                NSBezierPath.setDefaultFlatness(flatness);
                getApi().handle = path.handle.bezierPathByFlatteningPath();
                getApi().handle.retain();
        ...
        for (int i = 0; i < count; i++) {
            ...
            switch(element) {
                case OS.NSMoveToBezierPathElement:
                    ...
                    if (closed) {
                        getApi().handle.moveToPoint(pt1);
                    } else {
                        getApi().handle.lineToPoint(pt1);
                    }
                    break;
                ...
                case OS.NSCurveToBezierPathElement:
                    ...
                    getApi().handle.curveToPoint(pt3, pt1, pt2);
                    closed = false;
                    break;
    ...
    public void addPath(Path path) {
        ...
        if (!NSThread.isMainThread())
            pool = (NSAutoreleasePool) new NSAutoreleasePool().alloc().init();
        try {
            getApi().handle.appendBezierPath(path.handle);
    ...
    public void addRectangle(float x, float y, float width, float height) {
        ...
        if (!NSThread.isMainThread())
            pool = (NSAutoreleasePool) new NSAutoreleasePool().alloc().init();
        try {
            getApi().handle.appendBezierPathWithRect(rect);
            closed = true;
        } finally {
            if (pool != null)
                pool.release();
    ...
    public void addString(String string, float x, float y, Font font) {
		...
		*/
            ...
            if (range.length != 0) {
                ...
                path.transformUsingAffineTransform(transform);
                getApi().handle.appendBezierPath(path);
            }
    ...
    public boolean contains(float x, float y, GC gc, boolean outline) {
        ...
        try {
            ...
            if (outline) {
                ...
                point.x = x;
                point.y = y;
                return getApi().handle.containsPoint(point);
            }
        } finally {
            if (pool != null)
                pool.release();
    ...
    public void cubicTo(float cx1, float cy1, float cx2, float cy2, float x, float y) {
        ...
        try {
            ...
            ct2.x = cx2;
            ct2.y = cy2;
            getApi().handle.curveToPoint(pt, ct1, ct2);
            closed = false;
        } finally {
            if (pool != null)
                pool.release();
    ...
    public void getBounds(float[] bounds) {
        ...
        if (!NSThread.isMainThread())
            pool = (NSAutoreleasePool) new NSAutoreleasePool().alloc().init();
        try {
            NSRect rect = getApi().handle.controlPointBounds();
    ...
    public void getCurrentPoint(float[] point) {
        ...
        if (!NSThread.isMainThread())
            pool = (NSAutoreleasePool) new NSAutoreleasePool().alloc().init();
        try {
            NSPoint pt = getApi().handle.currentPoint();
            point[0] = (float) pt.x;
            point[1] = (float) pt.y;
        } finally {
            if (pool != null)
    ...
    public PathData getPathData() {
        ...
        if (!NSThread.isMainThread())
            pool = (NSAutoreleasePool) new NSAutoreleasePool().alloc().init();
        try {
            int count = (int) getApi().handle.elementCount();
            ...
            NSPoint pt = new NSPoint();
            for (int i = 0; i < count; i++) {
                int element = (int) getApi().handle.elementAtIndex(i, points);
                switch(element) {
                    case OS.NSMoveToBezierPathElement:
    ...
    public void quadTo(float cx, float cy, float x, float y) {
        ...
        if (!NSThread.isMainThread())
            pool = (NSAutoreleasePool) new NSAutoreleasePool().alloc().init();
        try {
            NSPoint current = getApi().handle.isEmpty() ? new NSPoint() : getApi().handle.currentPoint();
            NSPoint ct1 = new NSPoint();
