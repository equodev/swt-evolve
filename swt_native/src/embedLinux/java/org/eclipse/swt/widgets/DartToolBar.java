...
public class DartToolBar extends DartComposite implements IToolBar {
	...
	*/
    ...
    user_data) {
        ...
        if (item != null) {
            return ((DartToolBar) item.getParent().getImpl()).menuItemSelected(widget, item);
        }
        return 0;
    }
        ...
        switch(item.style) {
            ...
            case SWT.RADIO:
                if ((getApi().style & SWT.NO_RADIO_GROUP) == 0)
                    ((DartToolItem) item.getImpl()).selectRadio();
                break;
            case SWT.CHECK:
                boolean currentSelection = item.getSelection();
                item.setSelection(!currentSelection);
        }
        ((DartWidget) item.getImpl()).sendSelectionEvent(SWT.Selection, event, false);
        return 0;
    }
    ...
    boolean mnemonicHit(char key) {
        ToolItem[] items = getItems();
        for (int i = 0; i < items.length; i++) {
            long labelHandle = ((DartToolItem) items[i].getImpl()).labelHandle;
    ...
    void relayout() {
        ...
        for (int i = 0; i < items.length; i++) {
            ...
            if (item != null) {
                ...
                hasText |= ((DartItem) item.getImpl()).text != null && ((DartItem) item.getImpl()).text.length() > 0;
                hasImage |= ((DartItem) item.getImpl()).image != null;
            }
        }
    }
    ...
    void setFontDescription(long font) {
        ...
        ToolItem[] items = getItems();
        for (int i = 0; i < items.length; i++) {
            ((DartToolItem) items[i].getImpl()).setFontDescription(font);
        }
        relayout();
    }
    ...
    void setOrientation(boolean create) {
        ...
        ToolItem[] items = _getItems();
        for (int i = 0; i < items.length; i++) {
            ((DartToolItem) items[i].getImpl()).setOrientation(create);
        }
    }
    ...
    public void setToolTipText(String string) {
        ...
        ToolItem[] items = getItems();
        for (int i = 0; i < items.length; i++) {
            String newString = string != null ? null : ((DartToolItem) items[i].getImpl()).toolTipText;
            setToolTipText(items[i].handle, newString);
        }
    }

    ToolItem[] items = new ToolItem[0];
