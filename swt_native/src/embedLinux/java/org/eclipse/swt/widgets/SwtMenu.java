 ...
 */
public class SwtMenu extends SwtWidget implements IMenu {
    ...
    class Section {
        ...
        public long getSectionHandle() {
            return sectionHandle != 0 ? sectionHandle : ((SwtMenuItem) separator.getImpl()).modelHandle;
        }
			...
			*/
                if ((parent.getImpl()._getShell().style & SWT.ON_TOP) != 0) {
					 ...
					 */
                        ...
                        GTK3.gtk_menu_shell_set_take_focus(getApi().handle, false);
                    }
                }
                long eventPtr = 0;
                if (ableToSetLocation()) {
					 ...
					 */
                        long currentParent = GTK.gtk_widget_get_parent(getApi().handle);
                        if (currentParent == parent.handle) {
                            double[] relX = new double[1], relY = new double[1];
                            if (GTK4.gtk_widget_translate_coordinates(((SwtShell) parent.getShell().getImpl()).topHandle(), parent.handle, x, y, relX, relY)) {
                        ...
                        GTK.gtk_popover_set_pointing_to(getApi().handle, popoverPosition);
                        GTK.gtk_popover_popup(getApi().handle);
                    } else {
						 ...
						 */
                            ...
                            long gdkResource = GTK3.gtk_widget_get_window(((SwtShell) getShell().getImpl()).topHandle());
                        ...
                        GTK3.gtk_menu_popup_at_rect(getApi().handle, event.window, rect, GDK.GDK_GRAVITY_NORTH_WEST, GDK.GDK_GRAVITY_NORTH_WEST, eventPtr);
					 ...
					 */
                        ...
                        GTK3.gtk_menu_popup_at_pointer(getApi().handle, eventPtr);
            ...
            if (GTK.GTK4) {
                GTK.gtk_popover_popdown(getApi().handle);
            } else {
                GTK3.gtk_menu_popdown(getApi().handle);
            }
        }
    }
    ...
    void addAccelerators(long accelGroup) {
        ...
        for (int i = 0; i < items.length; i++) {
            ...
            ((SwtMenuItem) item.getImpl()).addAccelerators(accelGroup);
        }
    }
    ...
    void createHandle(int index) {
        ...
        if (GTK.GTK4) {
            ...
            switch(getApi().style & bits) {
                case SWT.BAR:
                    getApi().handle = GTK4.gtk_popover_menu_bar_new_from_model(modelHandle);
                    if (getApi().handle == 0)
                        error(SWT.ERROR_NO_HANDLES);
                    GTK4.gtk_box_prepend(((SwtDecorations) parent.getImpl()).vboxHandle, getApi().handle);
                    break;
                case SWT.DROP_DOWN:
                    getApi().handle = modelHandle;
                    break;
                case SWT.POP_UP:
                default:
                    getApi().handle = GTK4.gtk_popover_menu_new_from_model_full(modelHandle, GTK4.GTK_POPOVER_MENU_NESTED);
                    GTK.gtk_widget_set_parent(getApi().handle, parent.handle);
                    GTK.gtk_popover_set_position(getApi().handle, GTK.GTK_POS_BOTTOM);
                    GTK4.gtk_popover_set_has_arrow(getApi().handle, false);
                    GTK.gtk_widget_set_halign(getApi().handle, GTK.GTK_ALIGN_START);
                    if (getApi().handle == 0)
                        error(SWT.ERROR_NO_HANDLES);
            }
            ...
            if (actionGroup == 0)
                error(SWT.ERROR_NO_HANDLES);
            long shellHandle = ((SwtShell) parent.getShell().getImpl()).topHandle();
            ...
            if ((getApi().style & SWT.BAR) != 0) {
                getApi().handle = GTK3.gtk_menu_bar_new();
                if (getApi().handle == 0)
                    error(SWT.ERROR_NO_HANDLES);
                long vboxHandle = ((SwtDecorations) parent.getImpl()).vboxHandle;
                GTK3.gtk_container_add(vboxHandle, getApi().handle);
                gtk_box_set_child_packing(vboxHandle, getApi().handle, false, true, 0, GTK.GTK_PACK_START);
            } else {
                getApi().handle = GTK3.gtk_menu_new();
                if (getApi().handle == 0)
                    error(SWT.ERROR_NO_HANDLES);
                menuHandle = getApi().handle;
    ...
    public void fixMenus(Decorations newParent) {
        ...
        MenuItem[] items = getItems();
        for (int i = 0; i < items.length; i++) {
            ((SwtMenuItem) items[i].getImpl()).fixMenus(newParent);
        }
    ...
    public MenuItem getItem(int index) {
        ...
        if (GTK.GTK4) {
            ...
            if (data == 0)
                error(SWT.ERROR_CANNOT_GET_ITEM);
            return (MenuItem) ((SwtDisplay) display.getImpl()).getWidget(data);
        }
    }
    ...
    public MenuItem[] getItems() {
        ...
        if (GTK.GTK4) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                MenuItem item = (MenuItem) ((SwtDisplay) display.getImpl()).getWidget(data);
                if (item != null)
                    items[index++] = item;
    ...
    String getNameText() {
        ...
        if (length > 0) {
            for (int i = 0; i < length - 1; i++) {
                result = result + ((SwtMenuItem) items[i].getImpl()).getNameText() + ", ";
            }
            result = result + ((SwtMenuItem) items[length - 1].getImpl()).getNameText();
        }
        return result;
    }
    ...
    public boolean getVisible() {
        ...
        return GTK.gtk_widget_get_mapped(getApi().handle);
    }
    ...
    long gtk_map(long widget) {
        if (GTK.GTK4) {
            ...
            } else if ((getApi().style & SWT.POP_UP) != 0) {
                /*
			 ...
			 */
                connectCascadeSubMenuSignals(this.getApi(), getApi().handle);
            }
        }
    ...
    private void connectDropDownMenuSignals() {
        if (items == null)
            return;
        long barItem = GTK4.gtk_widget_get_first_child(getApi().handle);
        for (MenuItem menuItem : items) {
            if (barItem == 0)
                break;
            if ((menuItem.style & SWT.SEPARATOR) != 0)
                continue;
            if (((SwtMenuItem) menuItem.getImpl()).menu != null && ((SwtMenuItem) menuItem.getImpl()).menu.getImpl()._popoverHandle() == 0) {
                long popover = findGtkPopoverMenuChild(barItem);
                if (popover != 0) {
                    OS.g_object_ref(popover);
                    if (((SwtMenuItem) menuItem.getImpl()).menu.getImpl() instanceof DartMenu) {
                        ((DartMenu) ((SwtMenuItem) menuItem.getImpl()).menu.getImpl()).popoverHandle = popover;
                    }
                    if (((SwtMenuItem) menuItem.getImpl()).menu.getImpl() instanceof SwtMenu) {
                        ((SwtMenu) ((SwtMenuItem) menuItem.getImpl()).menu.getImpl()).popoverHandle = popover;
                    }
                    ((SwtDisplay) display.getImpl()).addWidget(popover, ((SwtMenuItem) menuItem.getImpl()).menu);
                    OS.g_signal_connect_closure_by_id(popover, ((SwtDisplay) display.getImpl()).signalIds[SHOW], 0, ((SwtDisplay) display.getImpl()).getClosure(SHOW), false);
                    OS.g_signal_connect_closure_by_id(popover, ((SwtDisplay) display.getImpl()).signalIds[HIDE], 0, ((SwtDisplay) display.getImpl()).getClosure(HIDE), false);
                    /*
				 * Also connect SHOW/HIDE signals for nested CASCADE submenus.
				 */
                    connectCascadeSubMenuSignals(((SwtMenuItem) menuItem.getImpl()).menu);
                }
            }
    ...
    private void connectCascadeSubMenuSignals(Menu menu, long parentPopoverHandle) {
        ...
        for (MenuItem item : menu.getImpl().getItems()) {
            if ((item.style & SWT.CASCADE) != 0 && ((SwtMenuItem) item.getImpl()).menu != null) {
                /*
			 ...
			 */
                if (((SwtMenuItem) item.getImpl()).menu.getImpl()._popoverHandle() != 0)
                    continue;
                long nestedPopover = findNestedPopoverForModel(parentPopoverHandle, ((SwtMenuItem) item.getImpl()).menu.getImpl()._modelHandle());
                if (nestedPopover != 0) {
                    OS.g_object_ref(nestedPopover);
                    if (((SwtMenuItem) item.getImpl()).menu.getImpl() instanceof DartMenu) {
                        ((DartMenu) ((SwtMenuItem) item.getImpl()).menu.getImpl()).popoverHandle = nestedPopover;
                    }
                    if (((SwtMenuItem) item.getImpl()).menu.getImpl() instanceof SwtMenu) {
                        ((SwtMenu) ((SwtMenuItem) item.getImpl()).menu.getImpl()).popoverHandle = nestedPopover;
                    }
                    ((SwtDisplay) display.getImpl()).addWidget(nestedPopover, ((SwtMenuItem) item.getImpl()).menu);
                    OS.g_signal_connect_closure_by_id(nestedPopover, ((SwtDisplay) display.getImpl()).signalIds[SHOW], 0, ((SwtDisplay) display.getImpl()).getClosure(SHOW), false);
                    OS.g_signal_connect_closure_by_id(nestedPopover, ((SwtDisplay) display.getImpl()).signalIds[HIDE], 0, ((SwtDisplay) display.getImpl()).getClosure(HIDE), false);
                    // Recurse to handle further nested CASCADE submenus
                    connectCascadeSubMenuSignals(((SwtMenuItem) item.getImpl()).menu);
                }
            }
        }
    }
    ...
    long gtk_hide(long widget) {
        if ((getApi().style & SWT.POP_UP) != 0) {
            if (((SwtDisplay) display.getImpl()).activeShell != null) {
                ((SwtDisplay) display.getImpl()).activeShell = getShell();
                if (((SwtShell) ((SwtDisplay) display.getImpl()).activeShell.getImpl()).ignoreFocusOut)
                    ((SwtShell) ((SwtDisplay) display.getImpl()).activeShell.getImpl()).ignoreFocusIn = true;
                ((SwtShell) ((SwtDisplay) display.getImpl()).activeShell.getImpl()).ignoreFocusOut = false;
            }
        }
        sendEvent(SWT.Hide);
        if (OS.ubuntu_menu_proxy_get() != 0) {
            ...
            for (int i = 0; i < items.length; i++) {
                MenuItem item = items[i];
                if (((SwtMenuItem) item.getImpl()).updateAcceleratorText(false))
                    continue;
            }
        }
        return 0;
    ...
    long gtk_show(long widget) {
        if ((getApi().style & SWT.POP_UP) != 0) {
            if (((SwtDisplay) display.getImpl()).activeShell != null) {
                ((SwtDisplay) display.getImpl()).activeShell = getShell();
                ((SwtShell) ((SwtDisplay) display.getImpl()).activeShell.getImpl()).ignoreFocusOut = true;
            }
            return 0;
        }
        sendEvent(SWT.Show);
        if (OS.ubuntu_menu_proxy_get() != 0) {
            ...
            for (int i = 0; i < items.length; i++) {
                MenuItem item = items[i];
                if (((SwtMenuItem) item.getImpl()).updateAcceleratorText(true))
                    continue;
            }
        }
        return 0;
    ...
    long gtk3_show_help(long widget, long helpType) {
        if (sendHelpEvent(helpType)) {
            GTK3.gtk_menu_shell_deactivate(getApi().handle);
            return 1;
        }
        return 0;
    }
    ...
    void hookEvents() {
        ...
        if (GTK.GTK4) {
            ...
            if ((getApi().style & SWT.DROP_DOWN) == 0) {
                OS.g_signal_connect_closure_by_id(getApi().handle, ((SwtDisplay) display.getImpl()).signalIds[SHOW], 0, ((SwtDisplay) display.getImpl()).getClosure(SHOW), false);
                OS.g_signal_connect_closure_by_id(getApi().handle, ((SwtDisplay) display.getImpl()).signalIds[HIDE], 0, ((SwtDisplay) display.getImpl()).getClosure(HIDE), false);
                if ((getApi().style & (SWT.BAR | SWT.POP_UP)) != 0) {
                    /*
				 ...
				 */
                    OS.g_signal_connect_closure_by_id(getApi().handle, ((SwtDisplay) display.getImpl()).signalIds[MAP], 0, ((SwtDisplay) display.getImpl()).getClosure(MAP), false);
                }
            }
        } else {
            ...
            if (OS.SWT_MENU_LOCATION_DEBUGGING) {
                OS.g_signal_connect_closure_by_id(getApi().handle, ((SwtDisplay) display.getImpl()).signalIds[POPPED_UP], 0, ((SwtDisplay) display.getImpl()).getClosure(POPPED_UP), false);
            }
            OS.g_signal_connect_closure_by_id(getApi().handle, ((SwtDisplay) display.getImpl()).signalIds[SHOW], 0, ((SwtDisplay) display.getImpl()).getClosure(SHOW), false);
            OS.g_signal_connect_closure_by_id(getApi().handle, ((SwtDisplay) display.getImpl()).signalIds[HIDE], 0, ((SwtDisplay) display.getImpl()).getClosure(HIDE), false);
        }
    ...
    void deregister() {
        super.deregister();
        if (GTK.GTK4 && (getApi().style & SWT.DROP_DOWN) != 0 && popoverHandle != 0) {
            ((SwtDisplay) display.getImpl()).removeWidget(popoverHandle);
    ...
    void removeAccelerators(long accelGroup) {
        ...
        for (int i = 0; i < items.length; i++) {
            MenuItem item = items[i];
            ((SwtMenuItem) item.getImpl()).removeAccelerators(accelGroup);
        }
    }
    ...
    boolean sendHelpEvent(long helpType) {
        if (selectedItem != null && !selectedItem.isDisposed()) {
            if (((SwtWidget) selectedItem.getImpl()).hooks(SWT.Help)) {
                ((SwtWidget) selectedItem.getImpl()).postEvent(SWT.Help);
                return true;
            }
        }
        if (hooks(SWT.Help)) {
        ...
        return ((SwtControl) parent.getImpl()).sendHelpEvent(helpType);
    }
    ...
    void setOrientation(boolean create) {
        if ((getApi().style & SWT.RIGHT_TO_LEFT) != 0 || !create) {
            ...
            MenuItem[] items = getItems();
            for (int i = 0; i < items.length; i++) {
                ((SwtMenuItem) items[i].getImpl()).setOrientation(create);
            }
        }
    }
    ...
    void verifyMenuPosition(int itemCount) {
        if (OS.isX11()) {
			 ...
			 */
                GTK3.gtk_widget_get_preferred_height(getApi().handle, null, naturalHeight);
                if (naturalHeight[0] > 0) {
					 ...
					 * design of resizing and moving in GTK4 */
                        ...
                        long topLevelWidget = GTK3.gtk_widget_get_toplevel(getApi().handle);
