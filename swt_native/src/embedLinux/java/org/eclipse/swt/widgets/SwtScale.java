...
public class SwtScale extends SwtControl implements IScale {
    ...
    void createHandle(int index) {
        ...
        if (hAdjustment == 0)
            error(SWT.ERROR_NO_HANDLES);
        if ((getApi().style & SWT.HORIZONTAL) != 0) {
            getApi().handle = GTK.gtk_scale_new(GTK.GTK_ORIENTATION_HORIZONTAL, hAdjustment);
        } else {
            getApi().handle = GTK.gtk_scale_new(GTK.GTK_ORIENTATION_VERTICAL, hAdjustment);
        }
        if (getApi().handle == 0)
            error(SWT.ERROR_NO_HANDLES);
        if (GTK.GTK4) {
        ...
        GTK.gtk_scale_set_digits(getApi().handle, 0);
        GTK.gtk_scale_set_draw_value(getApi().handle, false);
    }
    ...
    public void setIncrement(int increment) {
        ...
        GTK.gtk_range_set_increments(getApi().handle, increment, getPageIncrement());
    ...
    public void setMaximum(int value) {
        ...
        if (value <= minimum)
            return;
        OS.g_signal_handlers_block_matched(getApi().handle, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, VALUE_CHANGED);
        GTK.gtk_range_set_range(getApi().handle, minimum, value);
        OS.g_signal_handlers_unblock_matched(getApi().handle, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, VALUE_CHANGED);
    }
    ...
    public void setMinimum(int value) {
        ...
        if (value >= maximum)
            return;
        OS.g_signal_handlers_block_matched(getApi().handle, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, VALUE_CHANGED);
        GTK.gtk_range_set_range(getApi().handle, value, maximum);
        OS.g_signal_handlers_unblock_matched(getApi().handle, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, VALUE_CHANGED);
    }
    ...
    public void setPageIncrement(int pageIncrement) {
        ...
        if (pageIncrement < 1)
            return;
        OS.g_signal_handlers_block_matched(getApi().handle, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, VALUE_CHANGED);
        GTK.gtk_range_set_increments(getApi().handle, getIncrement(), pageIncrement);
        OS.g_signal_handlers_unblock_matched(getApi().handle, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, VALUE_CHANGED);
    }
    ...
    public void setSelection(int value) {
        checkWidget();
        OS.g_signal_handlers_block_matched(getApi().handle, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, VALUE_CHANGED);
        GTK.gtk_range_set_value(getApi().handle, value);
        OS.g_signal_handlers_unblock_matched(getApi().handle, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, VALUE_CHANGED);
    }
	 ...
	 */
        if (widget == getApi().handle) {
            ...
            GTK.gtk_widget_get_preferred_size(getApi().handle, minimumSize, naturalSize);
