...
public class SwtClipboard implements IClipboard {
    ...
    public void clearContents(int clipboards) {
        ...
        if (GTK.GTK4) {
            ...
            proxy.clear(this.getApi(), clipboards, false);
        } else {
            ClipboardProxy proxy = ClipboardProxy._getInstance(display);
            proxy.clear(this.getApi(), clipboards);
        }
    }
    ...
    public Object getContents(Transfer transfer, int clipboards) {
        if (GTK.GTK4) {
            ...
            try {
                ...
                ClipboardProxyGTK4 proxy = ClipboardProxyGTK4._getInstance(display);
                proxy.clear(this.getApi(), clipboards, true);
                return null;
            }
        }
    ...
    private CompletableFuture<Object> gtk4_getContentsAsync(Transfer transfer, int clipboards) {
        ...
        if (transfer == null)
            DND.error(SWT.ERROR_NULL_ARGUMENT);
        ClipboardProxyGTK4 proxy = ClipboardProxyGTK4._getInstance(display);
        return proxy.getData(this.getApi(), transfer, clipboards);
    }
    ...
    public void setContents(Object[] data, Transfer[] dataTypes, int clipboards) {
        ...
        if (GTK.GTK4) {
            ClipboardProxyGTK4 proxy = ClipboardProxyGTK4._getInstance(display);
            if (!proxy.setData(this.getApi(), data, dataTypes, clipboards)) {
	 ...
	 */
        return Arrays.stream(gtk4_getAvailableTypeNames(clipboards)).map(SwtTransfer::registerType).map((type) -> {
    ...
    private String[] gtk4_getAvailableTypeNames(int clipboards) {
        long gtkClipboard = ((clipboards & DND.CLIPBOARD) != 0) ? SwtClipboard.GTKCLIPBOARD : SwtClipboard.GTKPRIMARYCLIPBOARD;
    ...
    private long gtk3_clipboard_wait_for_contents(long clipboard, long target) {
        ...
        if (selection_data == 0 && duration > 5000) {
            // Bug 241957: In case of timeout take clipboard ownership to unblock future calls
            ClipboardProxy._getInstance(display).setData(this.getApi(), new String[] { " " }, new Transfer[] { SwtTextTransfer.getInstance() }, clipboard == GTKCLIPBOARD ? DND.CLIPBOARD : DND.SELECTION_CLIPBOARD);
        }
