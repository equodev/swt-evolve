...
public class SwtPath extends SwtResource implements IPath {
    ...
    public SwtPath(Device device, Path path, float flatness, Path api) {
        ...
        ((SwtPath) path.getImpl()).operations.forEach(this::storeAndApplyOperationOnAllHandles);
        if (flatness != 0) {
    ...
    private static class PathHandle {
        ...
        private boolean containsInPixels(float x, float y, GC gc, boolean outline) {
            ...
            if (outline) {
                return Gdip.GraphicsPath_IsOutlineVisible(handle, x, y, gc.getImpl()._data().gdipPen, gc.getImpl()._data().gdipGraphics);
            } else {
                return Gdip.GraphicsPath_IsVisible(handle, x, y, gc.getImpl()._data().gdipGraphics);
            }
        }
    ...
    private class AddPathOperation implements Operation {
        ...
        public AddPathOperation(Path path) {
            this.pathOperations = ((SwtPath) path.getImpl()).operations;
        }
    ...
    private class AddStringOperation implements Operation {
        ...
        private void addStringInPixels(PathHandle pathHandle, float x, float y) {
            ...
            long gdipFont = SwtGC.createGdipFont(hDC, SWTFontProvider.getFontHandle(device, this.fontData, zoom), 0, ((SwtDevice) device.getImpl()).fontCollection, family, null);
