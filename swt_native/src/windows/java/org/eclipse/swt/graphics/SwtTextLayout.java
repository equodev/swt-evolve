            ...
            if (wrapWidth != -1 && lineWidth + run.width > wrapWidthInPixels && !run.tab && !run.lineBreak) {
				...
				*/
                    if (start == 0 && i != lineStart && !run.tab) {
                        if (logAttr.fSoftBreak && !logAttr.fWhiteSpace) {
                            OS.MoveMemory(properties, ((SwtDevice) device.getImpl()).scripts[run.analysis.eScript], SCRIPT_PROPERTIES.sizeof);
                            int langID = properties.langid;
                            StyleItem pRun = allRuns[i - 1];
                            OS.MoveMemory(properties, ((SwtDevice) device.getImpl()).scripts[pRun.analysis.eScript], SCRIPT_PROPERTIES.sizeof);
        ...
        while (start < end) {
			 ...
			 */
                if (nextItemIndex < itemCount && merge) {
                    if (!item.lineBreak) {
                        OS.MoveMemory(sp, ((SwtDevice) device.getImpl()).scripts[item.analysis.eScript], SCRIPT_PROPERTIES.sizeof);
                        if (!sp.fComplex || item.tab) {
                            for (int i = 0; i < MERGE_MAX; i++) {
                                ...
                                if (c == '\t' != item.tab)
                                    break;
                                OS.MoveMemory(sp, ((SwtDevice) device.getImpl()).scripts[scriptItem.a.eScript], SCRIPT_PROPERTIES.sizeof);
 ...
 */
	...
	*/
    ...
    boolean shape(long hdc, StyleItem run, char[] chars, int[] glyphCount, int maxGlyphs, SCRIPT_PROPERTIES sp) {
        ...
        int scriptShaprHr = OS.ScriptShape(hdc, run.psc, chars, chars.length, maxGlyphs, run.analysis, run.glyphs, run.clusters, run.visAttrs, glyphCount);
 ...
 */
    void shape(GC gc, final long hdc, final StyleItem run) {
        ...
        OS.MoveMemory(sp, ((SwtDevice) device.getImpl()).scripts[script], SCRIPT_PROPERTIES.sizeof);
