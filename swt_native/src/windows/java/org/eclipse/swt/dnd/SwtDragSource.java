...
public class SwtDragSource extends SwtWidget implements IDragSource {
    ...
    private class COMIDropSource extends COMObject {
        ...
        public long method3(long[] args) {
            return QueryContinueDrag((int) args[0], (int) args[1]);
        }

        @Override
        public long method4(long[] args) {
            return GiveFeedback((int) args[0]);
        }
        ...
        public long Release() {
            refCount--;
            if (refCount == 0) {
                if (SwtDragSource.this.iDropSource == this) {
                    SwtDragSource.this.iDropSource = null;
                }
                this.dispose();
    ...
    private class COMIDataObject extends COMObject {
        ...
        public long method5(long[] args) {
            return QueryGetData(transferAgents, args[0]);
        }

        // method6 GetCanonicalFormatEtc - not implemented
        @Override
        public long method7(long[] args) {
            return SetData(args[0], args[1], (int) args[2]);
        }

        @Override
        public long method8(long[] args) {
            return EnumFormatEtc(transferAgents, (int) args[0], args[1]);
        }
        ...
        public long Release() {
            refCount--;
            if (refCount == 0) {
                if (SwtDragSource.this.iDataObject == this) {
                    SwtDragSource.this.iDataObject = null;
                }
                this.dispose();
                if (COM.FreeUnusedLibraries) {
                    COM.CoFreeUnusedLibraries();
        ...
        private boolean isActive() {
            return SwtDragSource.this.iDataObject == this;
        }
        ...
        private int GetData(long pFormatetc, long pmedium) {
			...
			*/
            ...
            if (isActive()) {
                ...
                event.widget = SwtDragSource.this.getApi();
