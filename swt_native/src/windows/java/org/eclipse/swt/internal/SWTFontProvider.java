...
public class SWTFontProvider {
    ...
    public static long getSystemFontHandle(Device device, int zoom) {
        return SwtFont.win32_getHandle(getSystemFont(device, zoom));
    }
    ...
    public static long getFontHandle(Device device, FontData fontData, int zoom) {
        return SwtFont.win32_getHandle(getFont(device, fontData, zoom));
    }
    ...
    public static long getFontHandle(Font font, int zoom) {
        ...
        return SwtFont.win32_getHandle(getFont(font.getDevice(), font.getFontData()[0], zoom));
    }
    ...
    private static SWTFontRegistry newFontRegistry(Device device) {
        if ("legacy".equalsIgnoreCase(System.getProperty(SWT_FONT_REGISTRY)) && device instanceof Display display && !display.isRescalingAtRuntime()) {
