 ...
 */
public class DartCCombo extends DartComposite implements ICCombo {
     ...
     * @see Widget#getStyle()
     */
    public DartCCombo(Composite parent, int style, CCombo api) {
        super(parent, style = checkStyle(style), api);
        _shell = super.getShell();
        int checkedStyle = checkStyle(style);
        editable = (checkedStyle & SWT.READ_ONLY) == 0;
        selection = new Point(0, 0);
        // The parent constructor may have modified the style, so we restore it
        getValue().setStyle(checkedStyle);
        listener = event -> {
            if (isDisposed())
                return;
            ...
            if (DartCCombo.this.getApi() == event.widget) {
                comboEvent(event);
                return;
            }
        ...
        // Initialize list without creating popup shell (for tests)
        int listStyle = SWT.SINGLE | SWT.V_SCROLL | SWT.H_SCROLL;
        if ((checkedStyle & SWT.FLAT) != 0)
            listStyle |= SWT.FLAT;
        if ((checkedStyle & SWT.RIGHT_TO_LEFT) != 0)
        ...
        if ((style & SWT.SIMPLE) == 0) {
            ...
            if (itemHeight != 0) {
                ...
                if (monitor != null) {
                    int maxHeight = monitor.getClientArea().height / 3;
    ...
    static int checkStyle(int style) {
        int mask = SWT.BORDER | SWT.READ_ONLY | SWT.FLAT | SWT.SIMPLE | SWT.LEFT_TO_RIGHT | SWT.RIGHT_TO_LEFT | SWT.LEAD | SWT.CENTER | SWT.TRAIL;
    ...
    void createText(int comboStyle) {
        ...
        int limit = 0;
        boolean enabled = true, focus = false;
        // Use instance field
        boolean textEditable = this.editable;
        Font font = null;
    ...
    public void deselect(int index) {
        checkWidget();
        if (0 <= index && index < items.length && index == getSelectionIndex() && _text.equals(items[index])) {
            //$NON-NLS-1$
            text.setText("");
            _text = "";
            list.deselect(index);
 ...
 */
    ...
    public void remove(int start, int end) {
        ...
        String[] newItems = new String[items.length - count];
    ...
    public void select(int index) {
        ...
        if (0 <= index && index < items.length) {
            // Always update _text, regardless of current selection
            text.setText(items[index]);
            _text = items[index];
    ...
    public void setText(String string) {
        ...
        if (string == null)
            SWT.error(SWT.ERROR_NULL_ARGUMENT);
        // Use indexOf() which searches in items array, not list widget
        int index = indexOf(string);
        if (index == -1) {
            list.deselectAll();
            text.setText(string);
    ...
    public CCombo getApi() {
        if (api == null)
            api = CCombo.createApi(this);
        return (CCombo) api;
    }

    public VCCombo getValue() {
        if (value == null)
            value = new VCCombo(this);
        return (VCCombo) value;
    }
}
