...
class Relation {
    ...
    static final String[] relationTypeString = { //$NON-NLS-1$
    ...
    static final String[] localizedRelationTypeString = { //$NON-NLS-1$
    ...
    SWT.getMessage("SWT_Subwindow_Of") };
    ...
    void createIAccessibleRelation() {
        objIAccessibleRelation = new COMObject(new int[] { 2, 0, 0, 1, 1, 1, 2, 3 }) {
            ...
            public long method3(long[] args) {
                return get_relationType(args[0]);
            }

            @Override
            public long method4(long[] args) {
                return get_localizedRelationType(args[0]);
            }

            @Override
            public long method5(long[] args) {
                return get_nTargets(args[0]);
            }

            @Override
            public long method6(long[] args) {
                return get_target((int) args[0], args[1]);
            }

            @Override
            public long method7(long[] args) {
                return get_targets((int) args[0], args[1], args[2]);
            }
        };
    }
    ...
    int get_target(int targetIndex, long ppTarget) {
        ...
        ((SwtAccessible) target.getImpl()).AddRef();
        OS.MoveMemory(ppTarget, new long[] { ((SwtAccessible) target.getImpl()).getAddress() }, C.PTR_SIZEOF);
        return COM.S_OK;
    }
    ...
    int get_targets(int maxTargets, long ppTargets, long pNTargets) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            ((SwtAccessible) target.getImpl()).AddRef();
            OS.MoveMemory(ppTargets + i * C.PTR_SIZEOF, new long[] { ((SwtAccessible) target.getImpl()).getAddress() }, C.PTR_SIZEOF);
        }
