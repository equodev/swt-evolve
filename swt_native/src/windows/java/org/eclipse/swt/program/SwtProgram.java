...
public final class SwtProgram implements IProgram {
    ...
    public static Program findProgram(String extension) {
        ...
        if (command != null) {
            ...
            if (iconName == null)
                iconName = "";
            program = new Program();
            ((SwtProgram) program.getImpl()).name = name;
            ((SwtProgram) program.getImpl()).command = command;
            ((SwtProgram) program.getImpl()).iconName = iconName;
            ((SwtProgram) program.getImpl()).extension = extension;
        }
        return program;
    }
    ...
    public ImageData getImageData(int zoom) {
        ...
        int nativeZoomFactor = 100 * SwtDisplay.getCurrent().getPrimaryMonitor().getZoom() / DPIUtil.getDeviceZoom();
        ...
        if (extension != null) {
            ...
            if (shfi.hIcon != 0) {
                Image image = SwtImage.win32_new(null, SWT.ICON, shfi.hIcon);
        ...
        if (phiconSmall[0] == 0)
            return null;
        Image image = SwtImage.win32_new(null, SWT.ICON, phiconSmall[0]);
        ImageData imageData = image.getImageData(imageZoomFactor);
        image.dispose();
        return imageData;
    }
    ...
    public boolean equals(Object other) {
        ...
        if (other instanceof Program) {
            ...
            return name.equals(((SwtProgram) program.getImpl()).name) && command.equals(((SwtProgram) program.getImpl()).command) && iconName.equals(((SwtProgram) program.getImpl()).iconName);
        }
        return false;
    }
