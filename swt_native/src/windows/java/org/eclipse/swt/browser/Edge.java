...
class Edge extends WebBrowser {
    ...
    //$NON-NLS-1$
    private static final String EVENT_MOUSEDOWN = "mousedown";

    //$NON-NLS-1$
    private static final String EVENT_MOUSEUP = "mouseup";

    //$NON-NLS-1$
    private static final String EVENT_MOUSEMOVE = "mousemove";

    //$NON-NLS-1$
    private static final String EVENT_MOUSEOVER = "mouseover";

    //$NON-NLS-1$
    private static final String EVENT_MOUSEOUT = "mouseout";

    //$NON-NLS-1$
    private static final String EVENT_DRAGSTART = "dragstart";

    //$NON-NLS-1$
    private static final String EVENT_DRAGEND = "dragend";

    //$NON-NLS-1$
    private static final String EVENT_DOUBLECLICK = "dblclick";

    //$NON-NLS-1$
    private static final String EVENT_MOUSEWHEEL = "wheel";

    private static final Set<String> MOUSE_RELATED_DOM_EVENTS = //
    Set.//
    of(//
    EVENT_MOUSEDOWN, //
    EVENT_MOUSEUP, //
    ...
    private static record MouseRelatedDomEvent(String eventType, boolean altKey, boolean ctrlKey, boolean shiftKey, long clientX, long clientY, long button, boolean fromElementSet, boolean toElementSet, long deltaY) {
    }
    ...
    static ICoreWebView2CookieManager getCookieManager() {
        WebViewEnvironment environmentWrapper = webViewEnvironments.get(SwtDisplay.getCurrent());
    ...
    WebViewEnvironment createEnvironment() {
        ...
        long[] ppv = new long[1];
        int hr = callAndWait(ppv, completion -> COM.CreateCoreWebView2EnvironmentWithOptions(pBrowserDir, pDataDir, options.getAddress(), completion.getAddress()));
        options.Release();
            ...
            switch(result) {
                ...
                default:
                    ...
                    if (previousAttempts < MAXIMUM_CREATION_RETRIES) {
                        ...
                        SWT.error(SWT.ERROR_UNSPECIFIED, null, String.format(" Aborting Edge initialization after %d retries with result %d", MAXIMUM_CREATION_RETRIES, result));
                    }
                    break;
            }
            return COM.S_OK;
    ...
    void setupBrowser(int hr, long pv) {
        ...
        // VARIANT
        long[] hostObj = { COM.VT_DISPATCH, hostDisp.getAddress(), 0 };
    ...
    static class HandleCoreWebView2SwtHost implements ICoreWebView2SwtHost {
        ...
        public long CallJava(int index, long bstrToken, long bstrArgsJson) {
            ...
            if (function != null && token.equals(((SwtBrowserFunction) function.getImpl()).token)) {
                inCallback++;
                try {
    ...
    int handleDOMContentLoaded(long pView, long pArgs) {
        ...
        if (startEvent != null && startEvent.top) {
            if (lastCustomText != null && getUrl().equals(ABOUT_BLANK)) {
                ...
                webViewProvider.getWebView(true).ExecuteScript(stringToWstr("document.open(); document.write('" + escapeForSingleQuotedJSString(lastCustomText) + "'); document.close();"), postExecute);
                postExecute.Release();
        ...
        executeInternal("""
            ...
            """.//
        replace("%%events%%", //
        MOUSE_RELATED_DOM_EVENTS.stream().map(x -> "'" + x + "'").collect(Collectors.joining(", "))).replace("%%webmessagekind%%", WEBMESSAGE_KIND_MOUSE_RELATED_DOM_EVENT));
        return COM.S_OK;
    }
    ...
    int handleContextMenuRequested(long pView, long pArgs) {
        ...
        Point pt = new //
        Point(//
        DPIUtil.pointToPixel(win32Point.x, DPIUtil.getNativeDeviceZoom()), DPIUtil.pointToPixel(win32Point.y, DPIUtil.getNativeDeviceZoom()));
        ...
        pt = new //
        Point(//
        DPIUtil.pixelToPoint(pt.x, DPIUtil.getZoomForAutoscaleProperty(browser.getShell().nativeZoom)), DPIUtil.pixelToPoint(pt.y, DPIUtil.getZoomForAutoscaleProperty(browser.getShell().nativeZoom)));
    ...
    int handleWebMessageReceived(long pView, long pArgs) {
        ...
        try {
            ...
            if (WEBMESSAGE_KIND_MOUSE_RELATED_DOM_EVENT.equals(data[0])) {
                MouseRelatedDomEvent mouseRelatedDomEvent = new //
	 ...
	 */
        ...
        Event newEvent = new Event();
        newEvent.widget = browser;
        newEvent.x = (int) domEvent.clientX();
        newEvent.y = (int) domEvent.clientY();
        if (domEvent.ctrlKey())
            mask |= SWT.CTRL;
        if (domEvent.altKey())
            mask |= SWT.ALT;
        ...
        if (eventType.equals(EVENT_MOUSEDOWN)) {
            ...
            switch(newEvent.button) {
                case 1:
                    ...
                    break;
                case 2:
                    newEvent.stateMask |= SWT.BUTTON2;
                    break;
                case 3:
                    newEvent.stateMask |= SWT.BUTTON3;
                    break;
                case 4:
                    newEvent.stateMask |= SWT.BUTTON4;
                    break;
                case 5:
                    newEvent.stateMask |= SWT.BUTTON5;
                    break;
            }
		...
		*/
            ...
            lastMouseMoveX = newEvent.x;
            lastMouseMoveY = newEvent.y;
