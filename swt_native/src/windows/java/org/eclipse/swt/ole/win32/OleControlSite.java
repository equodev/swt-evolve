...
public class OleControlSite extends OleClientSite {
    ...
    protected void createCOMInterfaces() {
        ...
        iDispatch = new COMObject(new int[] { 2, 0, 0, 1, 3, 5, 8 }) {
            ...
            public long method6(long[] args) {
                return Invoke((int) args[0], args[1], (int) args[2], (int) args[3], args[4], args[5], args[6], args[7]);
            }
        };
    }
        ...
        if (objIUnknown != null) {
            ...
            if (varBackColor != null) {
                ...
                if (COM.OleTranslateColor(varBackColor.getInt(), 0, colorRef) == COM.S_OK)
                    return SwtColor.win32_new(getDisplay(), colorRef[0]);
            }
        }
    ...
    protected long getLicenseInfo(GUID clsid) {
        long[] ppvObject = new long[1];
        if (COM.CoGetClassObject(clsid, COM.CLSCTX_INPROC_HANDLER | COM.CLSCTX_INPROC_SERVER, 0, COM.IIDIClassFactory, ppvObject) != COM.S_OK) {
            return 0;
        }
        long result = 0;
