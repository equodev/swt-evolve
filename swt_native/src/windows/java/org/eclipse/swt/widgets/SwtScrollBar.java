...
public class SwtScrollBar extends SwtWidget implements IScrollBar {
	...
	*/
    ...
    void destroyWidget() {
        ...
        ((SwtScrollable) parent.getImpl()).destroyScrollBar(getApi().style);
        releaseHandle();
    }
    ...
    public boolean getVisible() {
        ...
        int idObject = (getApi().style & SWT.VERTICAL) != 0 ? OS.OBJID_VSCROLL : OS.OBJID_HSCROLL;
        OS.GetScrollBarInfo(hwndScrollBar(), idObject, psbi);
        return (psbi.rgstate[0] & OS.STATE_SYSTEM_INVISIBLE) == 0;
    }

    long hwndScrollBar() {
        return ((SwtScrollable) parent.getImpl()).scrolledHandle();
    }
    ...
    int scrollBarType() {
        return (getApi().style & SWT.VERTICAL) != 0 ? OS.SB_VERT : OS.SB_HORZ;
    }
	...
	*/
        ...
        if (!visible || (getApi().state & DISABLED) != 0)
            fRedraw = false;
	...
	*/
        if (!visible) {
            if (OS.IsAppThemed()) {
                SCROLLBARINFO psbi = new SCROLLBARINFO();
                psbi.cbSize = SCROLLBARINFO.sizeof;
                int idObject = (getApi().style & SWT.VERTICAL) != 0 ? OS.OBJID_HSCROLL : OS.OBJID_VSCROLL;
