...
public class SwtMenu extends SwtWidget implements IMenu {
    ...
    SwtMenu(Decorations parent, int style, long handle, Menu api) {
    ...
    public void _setVisible(boolean visible) {
		...
		*/
            boolean success = OS.TrackPopupMenu(getApi().handle, flags, nX, nY, 0, hwndParent, null);
            // widget could be disposed at this point
            display.sendPostExternalEventDispatchEvent();
            if (!success && OS.GetMenuItemCount(getApi().handle) == 0) {
	...
	*/
    ...
    void createHandle() {
        ...
        if ((getApi().style & SWT.BAR) != 0) {
            getApi().handle = OS.CreateMenu();
        } else {
            getApi().handle = OS.CreatePopupMenu();
        }
        if (getApi().handle == 0)
    ...
    void createItem(MenuItem item, int index) {
        int count = OS.GetMenuItemCount(getApi().handle);
	...
	*/
        ...
        if (!success) {
            ((SwtDisplay) display.getImpl()).removeMenuItem(item);
		 ...
		 */
            ...
            OS.SetMenuItemInfo(getApi().handle, index, true, info);
        }
        redraw();
    }
    ...
    void destroyItem(MenuItem item) {
        if (!OS.DeleteMenu(getApi().handle, ((SwtMenuItem) item.getImpl()).id, OS.MF_BYCOMMAND)) {
    ...
    void destroyWidget() {
        ...
        long hMenu = getApi().handle;
        releaseHandle();
        if (cascade != null) {
            ((SwtMenuItem) cascade.getImpl()).setMenu(null, true);
        } else {
            if (hMenu != 0)
    ...
    void fixMenus(Decorations newParent) {
        ...
        for (MenuItem item : getItems()) {
            ((SwtMenuItem) item.getImpl()).fixMenus(newParent);
        }
    ...
    Rectangle getBounds() {
        ...
        if ((getApi().style & SWT.BAR) != 0) {
            ...
            if (count != 0) {
                RECT rect1 = new RECT();
                if (OS.GetMenuItemRect(0, getApi().handle, 0, rect1)) {
                    RECT rect2 = new RECT();
                    if (OS.GetMenuItemRect(0, getApi().handle, count - 1, rect2)) {
    ...
    public MenuItem getDefaultItem() {
        checkWidget();
        int id = OS.GetMenuDefaultItem(getApi().handle, OS.MF_BYCOMMAND, OS.GMDI_USEDISABLED);
        if (id == -1)
            return null;
        MENUITEMINFO info = new MENUITEMINFO();
        info.cbSize = MENUITEMINFO.sizeof;
        info.fMask = OS.MIIM_ID;
        if (OS.GetMenuItemInfo(getApi().handle, id, false, info)) {
            return ((SwtDisplay) display.getImpl()).getMenuItem(info.wID);
        }
        return null;
    }
    ...
    public MenuItem getItem(int index) {
        ...
        if (!OS.GetMenuItemInfo(getApi().handle, index, true, info)) {
            error(SWT.ERROR_INVALID_RANGE);
        }
    ...
    public int getItemCount() {
        checkWidget();
        return OS.GetMenuItemCount(getApi().handle);
    }
    ...
    public MenuItem[] getItems() {
        checkWidget();
        int index = 0, count = 0;
        int length = OS.GetMenuItemCount(getApi().handle);
        if (length < 0) {
            int error = OS.GetLastError();
            //$NON-NLS-1$ $NON-NLS-2$
            SWT.error(SWT.ERROR_CANNOT_GET_COUNT, null, " [GetLastError=0x" + Integer.toHexString(error) + "]");
        }
        MenuItem[] items = new MenuItem[length];
        MENUITEMINFO info = new MENUITEMINFO();
        info.cbSize = MENUITEMINFO.sizeof;
        info.fMask = OS.MIIM_DATA;
        while (OS.GetMenuItemInfo(getApi().handle, index, true, info)) {
            if (count == items.length) {
                MenuItem[] newItems = new MenuItem[count + 4];
                System.arraycopy(items, 0, newItems, 0, count);
                items = newItems;
            }
            MenuItem item = ((SwtDisplay) display.getImpl()).getMenuItem((int) info.dwItemData);
            if (item != null)
                items[count++] = item;
            index++;
        }
    ...
    String getNameText() {
        ...
        if (length > 0) {
            for (int i = 0; i <= length - 1; i++) {
                result += (items[i] == null ? "null" : ((SwtMenuItem) items[i].getImpl()).getNameText()) + (i < (length - 1) ? ", " : "");
            }
        }
        return result;
    }
    ...
    public Menu getParentMenu() {
        checkWidget();
        if (cascade != null)
            return ((SwtMenuItem) cascade.getImpl()).parent;
        return null;
    }
    ...
    void hideCurrentToolTip() {
        if (this.selectedMenuItem != null) {
            ((SwtMenuItem) selectedMenuItem.getImpl()).hideToolTip();
        }
    }
    ...
    public int indexOf(MenuItem item) {
        ...
        if (((SwtMenuItem) item.getImpl()).parent != this.getApi())
            return -1;
        int index = 0;
        MENUITEMINFO info = new MENUITEMINFO();
        info.cbSize = MENUITEMINFO.sizeof;
        info.fMask = OS.MIIM_DATA;
        while (OS.GetMenuItemInfo(getApi().handle, index, true, info)) {
            if (info.dwItemData == ((SwtMenuItem) item.getImpl()).id)
                return index;
            index++;
        }
        return -1;
    ...
    public void setDefaultItem(MenuItem item) {
        ...
        if (item != null) {
            ...
            if (((SwtMenuItem) item.getImpl()).parent != this.getApi())
                return;
            newID = ((SwtMenuItem) item.getImpl()).id;
        }
        int oldID = OS.GetMenuDefaultItem(getApi().handle, OS.MF_BYCOMMAND, OS.GMDI_USEDISABLED);
        if (newID == oldID)
            return;
        OS.SetMenuDefaultItem(getApi().handle, newID, OS.MF_BYCOMMAND);
        redraw();
    }
    ...
    void _setOrientation(int orientation) {
        ...
        for (MenuItem itm : getItems()) {
            ((SwtMenuItem) itm.getImpl()).setOrientation(orientation);
        }
    }
    ...
    void update() {
        ...
        OS.GetMenuInfo(getApi().handle, lpcmi);
        if (hasImage && !hasCheck) {
            lpcmi.dwStyle |= OS.MNS_CHECKORBMP;
        } else {
            lpcmi.dwStyle &= ~OS.MNS_CHECKORBMP;
        }
        OS.SetMenuInfo(getApi().handle, lpcmi);
    }
    ...
    LRESULT wmTimer(long wParam, long lParam) {
        if (wParam == ID_TOOLTIP_TIMER) {
            POINT pt = new POINT();
            OS.GetCursorPos(pt);
            if (selectedMenuItem != null && ((SwtMenuItem) selectedMenuItem.getImpl()).parent != null) {
                RECT rect = new RECT();
                boolean success = OS.GetMenuItemRect(0, ((SwtMenuItem) selectedMenuItem.getImpl()).parent.handle, indexOf(selectedMenuItem), rect);
                if (!success)
                    return null;
                if (OS.PtInRect(rect, pt)) {
                    // Mouse cursor is within the bounds of menu item
                    ((SwtMenuItem) selectedMenuItem.getImpl()).showTooltip(pt.x, pt.y + getSystemMetrics(OS.SM_CYCURSOR) / 2 + 5);
                } else {
                    /*
				 * Mouse cursor is outside the bounds of the menu item:
				 * Keyboard or mnemonic was used to select menu item
				 */
                    ((SwtMenuItem) selectedMenuItem.getImpl()).showTooltip((rect.right + rect.left) / 2, rect.bottom + 5);
                }
            }
        }
        return null;
