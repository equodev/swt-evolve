...
public class SwtTreeColumn extends SwtItem implements ITreeColumn {
    ...
    public SwtTreeColumn(Tree parent, int style, TreeColumn api) {
        ...
        resizable = true;
        this.parent = parent;
        ((SwtTree) parent.getImpl()).createItem(this.getApi(), parent.getColumnCount());
    }
    ...
    public void pack() {
        ...
        if (index == -1)
            return;
        int columnWidth = 0;
        long hwnd = parent.handle, hwndHeader = ((SwtTree) parent.getImpl()).hwndHeader;
        ...
        while (tvItem.hItem != 0) {
            ...
            if (item != null) {
                ...
                if (((SwtWidget) parent.getImpl()).hooks(SWT.MeasureItem)) {
                    ...
                    Event event = ((SwtTree) parent.getImpl()).sendMeasureItemEvent(item, index, hDC, detail);
    ...
    void setImage(Image image, boolean sort, boolean right) {
        ...
        if (image != null) {
            if (sort) {
                ...
                hdItem.fmt &= ~OS.HDF_BITMAP;
                hdItem.fmt |= OS.HDF_IMAGE;
                hdItem.iImage = ((SwtTree) parent.getImpl()).imageIndexHeader(image);
            }
            if (right)
                hdItem.fmt |= OS.HDF_BITMAP_ON_RIGHT;
        } else {
	...
	*/
    ...
    public void setToolTipText(String string) {
        checkWidget();
        toolTipText = string;
        long hwndHeaderToolTip = ((SwtTree) parent.getImpl()).headerToolTipHandle;
        if (hwndHeaderToolTip == 0) {
            ((SwtTree) parent.getImpl()).createHeaderToolTips();
            ((SwtTree) parent.getImpl()).updateHeaderToolTips();
        }
    }
    ...
    void setWidthInPixels(int width) {
        ...
        rect.left = headerRect.left;
        OS.InvalidateRect(hwnd, rect, true);
        ((SwtTree) parent.getImpl()).setScrollWidth();
    }
