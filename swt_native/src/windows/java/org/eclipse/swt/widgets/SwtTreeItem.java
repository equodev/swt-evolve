...
public class SwtTreeItem extends SwtItem implements ITreeItem {
    ...
    public SwtTreeItem(Tree parent, int style, TreeItem api) {
        this(parent, style, OS.TVI_ROOT, OS.TVI_LAST, 0, api);
    }
    ...
    public SwtTreeItem(Tree parent, int style, int index, TreeItem api) {
        this(parent, style, OS.TVI_ROOT, findPrevious(parent, index), 0, api);
    }
    ...
    public SwtTreeItem(TreeItem parentItem, int style, TreeItem api) {
        this(((SwtTreeItem) checkNull(parentItem).getImpl()).parent, style, parentItem.handle, OS.TVI_LAST, 0, api);
    }
    ...
    public SwtTreeItem(TreeItem parentItem, int style, int index, TreeItem api) {
        this(((SwtTreeItem) checkNull(parentItem).getImpl()).parent, style, parentItem.handle, findPrevious(parentItem, index), 0, api);
    }

    SwtTreeItem(Tree parent, int style, long hParent, long hInsertAfter, long hItem, TreeItem api) {
        super(parent, style, api);
        this.parent = parent;
        ((SwtTree) parent.getImpl()).createItem(this.getApi(), hParent, hInsertAfter, hItem);
    }
    ...
    static long findPrevious(Tree parent, int index) {
        ...
        long hItem = ((SwtTree) parent.getImpl()).findItem(hFirstItem, index - 1);
        if (hItem == 0)
            SWT.error(SWT.ERROR_INVALID_RANGE);
        return hItem;
    }
    ...
    static long findPrevious(TreeItem parentItem, int index) {
        ...
        if (index == 0)
            return OS.TVI_FIRST;
        Tree parent = ((SwtTreeItem) parentItem.getImpl()).parent;
    ...
    public void clear(int index, boolean all) {
        checkWidget();
        long hwnd = parent.handle;
        long hItem = OS.SendMessage(hwnd, OS.TVM_GETNEXTITEM, OS.TVGN_CHILD, getApi().handle);
        if (hItem == 0)
            error(SWT.ERROR_INVALID_RANGE);
        hItem = ((SwtTree) parent.getImpl()).findItem(hItem, index);
        if (hItem == 0)
            error(SWT.ERROR_INVALID_RANGE);
        TVITEM tvItem = new TVITEM();
        tvItem.mask = OS.TVIF_HANDLE | OS.TVIF_PARAM;
        ((SwtTree) parent.getImpl()).clear(hItem, tvItem);
        if (all) {
            hItem = OS.SendMessage(hwnd, OS.TVM_GETNEXTITEM, OS.TVGN_CHILD, hItem);
            ((SwtTree) parent.getImpl()).clearAll(hItem, tvItem, all);
        }
    }
    ...
    void destroyWidget() {
        TVITEM tvItem = new TVITEM();
        tvItem.mask = OS.TVIF_HANDLE | OS.TVIF_PARAM;
        ((SwtTree) parent.getImpl()).releaseItem(getApi().handle, tvItem, false);
        ((SwtTree) parent.getImpl()).destroyItem(this.getApi(), getApi().handle);
        releaseHandle();
    }
    ...
    public Color getBackground() {
        checkWidget();
        if (!((SwtTree) parent.getImpl()).checkData(this.getApi(), true))
    ...
    RECT getBounds(int index, boolean getText, boolean getImage, boolean fullText, boolean fullImage, boolean clip, long hDC) {
        if (!getText && !getImage)
            return new RECT();
        long hwnd = parent.handle;
        if ((parent.style & SWT.VIRTUAL) == 0 && !cached && !((SwtTree) parent.getImpl()).painted) {
            TVITEM tvItem = new TVITEM();
            tvItem.mask = OS.TVIF_HANDLE | OS.TVIF_TEXT;
            tvItem.hItem = getApi().handle;
            tvItem.pszText = OS.LPSTR_TEXTCALLBACK;
            ((SwtTree) parent.getImpl()).ignoreCustomDraw = true;
            OS.SendMessage(hwnd, OS.TVM_SETITEM, 0, tvItem);
            ((SwtTree) parent.getImpl()).ignoreCustomDraw = false;
        }
        boolean firstColumn = index == 0;
        int columnCount = 0;
        long hwndHeader = ((SwtTree) parent.getImpl()).hwndHeader;
        if (hwndHeader != 0) {
            columnCount = ((SwtTree) parent.getImpl()).columnCount;
            firstColumn = index == ((SwtTree) parent.getImpl()).getFirstColumnIndex();
        }
        RECT rect = new RECT();
        if (firstColumn) {
            boolean full = columnCount == 0 && getText && getImage && fullText && fullImage;
            if (!OS.TreeView_GetItemRect(hwnd, getApi().handle, rect, !full)) {
                return new RECT();
            }
            if (getImage && !fullImage) {
                if (OS.SendMessage(hwnd, OS.TVM_GETIMAGELIST, OS.TVSIL_NORMAL, 0) != 0) {
                    Point size = ((SwtTree) parent.getImpl()).getImageSize();
                    rect.left -= size.x + SwtTree.INSET;
                    if (!getText)
            ...
            if (fullText || fullImage || clip) {
                if (hwndHeader != 0) {
                    RECT headerRect;
                    if (columnCount != 0) {
                        headerRect = ((SwtTree) parent.getImpl()).getColumnRect(index);
                        if (headerRect == null) {
                            return new RECT();
                        }
                    } else {
                        headerRect = new RECT();
                        headerRect.right = ((SwtTree) parent.getImpl()).scrollWidth;
            ...
            if (!(0 <= index && index < columnCount))
                return new RECT();
            RECT headerRect = ((SwtTree) parent.getImpl()).getColumnRect(index);
            if (headerRect == null) {
                return new RECT();
            }
            if (!OS.TreeView_GetItemRect(hwnd, getApi().handle, rect, false)) {
                return new RECT();
            }
            rect.left = headerRect.left;
            if (fullText && getImage && clip) {
                ...
                if (getText) {
                    if (fullText && clip) {
                        rect.left = rect.right + SwtTree.INSET;
                        rect.right = headerRect.right;
                    } else {
                        ...
                        if (string != null) {
                            ...
                            if (getImage) {
                                rect.right += textRect.right - textRect.left + SwtTree.INSET * 3;
                            } else {
                                rect.left = rect.right + SwtTree.INSET;
                                rect.right = rect.left + (textRect.right - textRect.left) + SwtTree.INSET;
                            }
                        }
                    }
                }
	...
	*/
        int state = (int) OS.SendMessage(hwnd, OS.TVM_GETITEMSTATE, getApi().handle, OS.TVIS_EXPANDED);
    ...
    public TreeItem getItem(int index) {
        ...
        if (!((SwtTree) parent.getImpl()).checkData(this.getApi(), true))
            error(SWT.ERROR_WIDGET_DISPOSED);
        long hwnd = parent.handle;
        long hFirstItem = OS.SendMessage(hwnd, OS.TVM_GETNEXTITEM, OS.TVGN_CHILD, getApi().handle);
        if (hFirstItem == 0)
            error(SWT.ERROR_INVALID_RANGE);
        long hItem = ((SwtTree) parent.getImpl()).findItem(hFirstItem, index);
        if (hItem == 0)
            error(SWT.ERROR_INVALID_RANGE);
        return ((SwtTree) parent.getImpl())._getItem(hItem);
    }
    ...
    public int getItemCount() {
        ...
        if (hItem == 0)
            return 0;
        return ((SwtTree) parent.getImpl()).getItemCount(hItem);
    }
    ...
    public TreeItem[] getItems() {
        ...
        return ((SwtTree) parent.getImpl()).getItems(hItem);
    }
    ...
    public TreeItem getParentItem() {
        checkWidget();
        long hwnd = parent.handle;
        long hItem = OS.SendMessage(hwnd, OS.TVM_GETNEXTITEM, OS.TVGN_PARENT, getApi().handle);
        return hItem != 0 ? ((SwtTree) parent.getImpl())._getItem(hItem) : null;
    }
    ...
    Rectangle getTextBoundsInPixels(int index) {
        ...
        if (index == 0)
            rect.left += SwtTree.INSET - 1;
        rect.left = Math.min(rect.left, rect.right);
        // Add 1 px margin to avoid truncation of text seen with "Segoe UI" font
        rect.right = rect.right - SwtTree.INSET + 1;
    ...
    public int indexOf(TreeItem item) {
        ...
        long hwnd = parent.handle;
        long hItem = OS.SendMessage(hwnd, OS.TVM_GETNEXTITEM, OS.TVGN_CHILD, getApi().handle);
        return hItem == 0 ? -1 : ((SwtTree) parent.getImpl()).findIndex(hItem, item.handle);
    }

    void redraw() {
        if (((SwtTree) parent.getImpl()).currentItem == this.getApi() || !((SwtControl) parent.getImpl()).getDrawing())
            return;
        long hwnd = parent.handle;
	...
	*/
        ...
        if (!full) {
            full = ((SwtTree) parent.getImpl()).columnCount != 0;
            if (!full) {
                if (((SwtWidget) parent.getImpl()).hooks(SWT.EraseItem) || ((SwtWidget) parent.getImpl()).hooks(SWT.PaintItem)) {
                    full = true;
                }
            }
        }
    ...
    void releaseChildren(boolean destroy) {
        if (destroy) {
            TVITEM tvItem = new TVITEM();
            tvItem.mask = OS.TVIF_HANDLE | OS.TVIF_PARAM;
            ((SwtTree) parent.getImpl()).releaseItems(getApi().handle, tvItem);
        }
    ...
    public void removeAll() {
        ...
        TVITEM tvItem = new TVITEM();
        tvItem.mask = OS.TVIF_HANDLE | OS.TVIF_PARAM;
        tvItem.hItem = OS.SendMessage(hwnd, OS.TVM_GETNEXTITEM, OS.TVGN_CHILD, getApi().handle);
        /**
         * Performance optimization, switch off redraw for high amount of elements
         */
        boolean disableRedraw = ((SwtTree) parent.getImpl()).cachedItemCount > 30;
        if (disableRedraw) {
        ...
        try {
            while (tvItem.hItem != 0) {
                ...
                TreeItem item = tvItem.lParam != -1 ? ((SwtTree) parent.getImpl()).items[(int) tvItem.lParam] : null;
                if (item != null && !item.isDisposed()) {
                    item.dispose();
                } else {
                    ((SwtTree) parent.getImpl()).releaseItem(tvItem.hItem, tvItem, false);
                    ((SwtTree) parent.getImpl()).destroyItem(null, tvItem.hItem);
                }
                tvItem.hItem = OS.SendMessage(hwnd, OS.TVM_GETNEXTITEM, OS.TVGN_CHILD, getApi().handle);
            }
        } finally {
    ...
    public void setBackground(Color color) {
        ...
        int pixel = -1;
        if (color != null) {
            ((SwtTree) parent.getImpl()).customDraw = true;
            pixel = color.handle;
        }
        if (background == pixel)
            return;
	...
	*/
        if ((parent.style & SWT.VIRTUAL) != 0) {
            if (((SwtTree) parent.getImpl()).currentItem == this.getApi() && OS.IsWindowVisible(hwnd)) {
                RECT rect = new RECT();
                if (OS.TreeView_GetItemRect(hwnd, getApi().handle, rect, false)) {
                    OS.InvalidateRect(hwnd, rect, true);
                }
    ...
    public void setExpanded(boolean expanded) {
        ...
        long hwnd = parent.handle;
        if (OS.SendMessage(hwnd, OS.TVM_GETNEXTITEM, OS.TVGN_CHILD, getApi().handle) == 0) {
            return;
        }
        /*
	* Bug in Windows.  Despite the fact that TVM_GETITEMSTATE claims
	...
	*/
        ...
        if (noScroll && hTopItem != 0) {
            ...
            if (((SwtControl) parent.getImpl()).getDrawing() && OS.IsWindowVisible(hwnd)) {
                boolean noAnimate = true;
                count = (int) OS.SendMessage(hwnd, OS.TVM_GETVISIBLECOUNT, 0, 0);
                rects = new RECT[count + 1];
                long hItem = hTopItem;
                int index = 0;
                while (hItem != 0 && (noAnimate || hItem != getApi().handle) && index < count) {
                    RECT rect = new RECT();
                ...
                if (noAnimate || hItem != getApi().handle) {
                    redraw = true;
                    count = index;
                    hBottomItem = hItem;
                    oldRect = new RECT();
	...
	*/
        ...
        OS.SendMessage(hwnd, OS.TVM_EXPAND, expanded ? OS.TVE_EXPAND : OS.TVE_COLLAPSE, getApi().handle);
        ...
        if (noScroll && hTopItem != 0) {
            ...
            if (redraw) {
                ...
                if (fixScroll) {
                    ((SwtTree) parent.getImpl()).updateScrollBar();
        ...
        if (hNewItem != hOldItem) {
            ...
            if (hNewItem != 0) {
                event.item = ((SwtTree) parent.getImpl())._getItem(hNewItem);
                ((SwtTree) parent.getImpl()).hAnchor = hNewItem;
            }
            ((SwtWidget) parent.getImpl()).sendSelectionEvent(SWT.Selection, event, true);
        }
    }
	...
	*/
    ...
    public void setImage(int index, Image image) {
        ...
        ((SwtTree) parent.getImpl()).imageIndex(image, index);
        if (index == 0) {
            if ((parent.style & SWT.VIRTUAL) == 0 && !cached && !((SwtTree) parent.getImpl()).painted) {
                return;
            }
    ...
    public void setItemCount(int count) {
        ...
        ((SwtTree) parent.getImpl()).setItemCount(count, getApi().handle);
    }
    ...
    void sort() {
        ...
        if ((parent.style & SWT.VIRTUAL) != 0)
            return;
        ((SwtTree) parent.getImpl()).sort(getApi().handle, false);
    }
