...
public class SwtToolBar extends SwtComposite implements IToolBar {
	...
	*/
		...
		*/
            ...
            OS.SetWindowLong(getApi().handle, OS.GWL_STYLE, bits | OS.CCS_VERT);
        } else {
    ...
    Point computeSizeInPixels(int wHint, int hHint, boolean changed) {
        ...
        if ((getApi().style & SWT.VERTICAL) != 0) {
            ...
            for (int i = 0; i < count; i++) {
                OS.SendMessage(getApi().handle, OS.TB_GETITEMRECT, i, rect);
                ...
                if ((lpButton.fsStyle & OS.BTNS_SEP) != 0) {
                    ...
                    OS.SendMessage(getApi().handle, OS.TB_GETBUTTONINFO, lpButton.idCommand, info);
            ...
            OS.GetWindowRect(getApi().handle, oldRect);
            ...
            OS.SetWindowPos(getApi().handle, 0, 0, 0, newWidth, newHeight, flags);
            if (count != 0) {
                RECT rect = new RECT();
                OS.SendMessage(getApi().handle, OS.TB_GETITEMRECT, count - 1, rect);
                width = Math.max(width, rect.right);
                height = Math.max(height, rect.bottom);
            }
            OS.SetWindowPos(getApi().handle, 0, 0, 0, oldWidth, oldHeight, flags);
            if (redraw)
	...
	*/
    ...
    Widget computeTabGroup() {
        ...
        if (tabItemList == null) {
            int i = 0;
            while (i < items.length && ((SwtToolItem) items[i].getImpl()).control == null) i++;
            if (i == items.length)
                return super.computeTabGroup();
        }
        int index = (int) OS.SendMessage(getApi().handle, OS.TB_GETHOTITEM, 0, 0);
        if (index == -1)
            index = lastHotId;
        while (index >= 0) {
	...
	*/
        ...
        OS.SendMessage(getApi().handle, OS.TB_SETBITMAPSIZE, 0, 0);
        OS.SendMessage(getApi().handle, OS.TB_SETBUTTONSIZE, 0, 0);
        /* Set the extended style bits */
        int bits = OS.TBSTYLE_EX_DRAWDDARROWS | OS.TBSTYLE_EX_MIXEDBUTTONS | OS.TBSTYLE_EX_DOUBLEBUFFER;
        OS.SendMessage(getApi().handle, OS.TB_SETEXTENDEDSTYLE, 0, bits);
    }
	...
	*/
            ...
            if (OS.SendMessage(getApi().handle, OS.TB_INSERTBUTTON, index, lpButton) == 0) {
                error(SWT.ERROR_ITEM_NOT_ADDED);
            }
        }
        items[((SwtToolItem) item.getImpl()).id = id] = item;
        if ((getApi().style & SWT.VERTICAL) != 0)
            setRowCount(count + 1);
        layoutItems();
    }
    ...
    void destroyItem(ToolItem item) {
        ...
        int index = (int) OS.SendMessage(getApi().handle, OS.TB_GETBUTTONINFO, ((SwtToolItem) item.getImpl()).id, info);
        /*
	...
	*/
        ...
        OS.SendMessage(getApi().handle, OS.TB_DELETEBUTTON, index, 0);
        if (((SwtToolItem) item.getImpl()).id == lastFocusId)
            lastFocusId = -1;
        if (((SwtToolItem) item.getImpl()).id == lastArrowId)
            lastArrowId = -1;
        if (((SwtToolItem) item.getImpl()).id == lastHotId)
            lastHotId = -1;
        items[((SwtToolItem) item.getImpl()).id] = null;
        ((SwtToolItem) item.getImpl()).id = -1;
        int count = (int) OS.SendMessage(getApi().handle, OS.TB_BUTTONCOUNT, 0, 0);
        if (count == 0) {
            if (imageList != null) {
                OS.SendMessage(getApi().handle, OS.TB_SETIMAGELIST, 0, 0);
                ((SwtDisplay) display.getImpl()).releaseToolImageList(imageList);
            }
            if (hotImageList != null) {
                OS.SendMessage(getApi().handle, OS.TB_SETHOTIMAGELIST, 0, 0);
                ((SwtDisplay) display.getImpl()).releaseToolHotImageList(hotImageList);
            }
            if (disabledImageList != null) {
                OS.SendMessage(getApi().handle, OS.TB_SETDISABLEDIMAGELIST, 0, 0);
	...
	*/
        for (ToolItem item : items) {
            if (item != null) {
                if ((item.style & SWT.SEPARATOR) == 0) {
                    ((SwtToolItem) item.getImpl()).updateImages(enabled && item.getEnabled());
                }
            }
        }
    }
    ...
    public ToolItem getItem(int index) {
        ...
        if (!(0 <= index && index < count))
            error(SWT.ERROR_INVALID_RANGE);
        TBBUTTON lpButton = new TBBUTTON();
        long result = OS.SendMessage(getApi().handle, OS.TB_GETBUTTON, index, lpButton);
        if (result == 0)
    ...
    ToolItem getItemInPixels(Point point) {
        for (ToolItem item : getItems()) {
            Rectangle rect = ((SwtToolItem) item.getImpl()).getBoundsInPixels();
    ...
    public int getItemCount() {
        checkWidget();
        return (int) OS.SendMessage(getApi().handle, OS.TB_BUTTONCOUNT, 0, 0);
    }
    ...
    public int getRowCount() {
        ...
        return (int) OS.SendMessage(getApi().handle, OS.TB_GETROWS, 0, 0);
    }
    ...
    public int indexOf(ToolItem item) {
        ...
        return (int) OS.SendMessage(getApi().handle, OS.TB_COMMANDTOINDEX, ((SwtToolItem) item.getImpl()).id, 0);
    }
	...
	*/
        if (OS.IsAppThemed()) {
            if ((getApi().style & SWT.RIGHT) != 0 && (getApi().style & SWT.VERTICAL) == 0) {
                boolean hasText = false, hasImage = false;
                for (ToolItem item : items) {
                    if (item != null) {
                        if (!hasText)
                            hasText = ((SwtItem) item.getImpl()).text.length() != 0;
                        if (!hasImage)
                            hasImage = ((SwtItem) item.getImpl()).image != null;
                        if (hasText && hasImage)
                            break;
                    }
                }
	 ...
	 */
        if (OS.IsAppThemed()) {
            if ((getApi().style & SWT.RIGHT) == 0 && (getApi().style & SWT.HORIZONTAL) != 0) {
                boolean hasText = false, hasImage = false, hasTextAndImageInSingleItem = false;
                for (ToolItem item : items) {
                    if (item != null) {
                        boolean itemHasText = false, itemHasImage = false;
                        itemHasText = ((SwtItem) item.getImpl()).text.length() != 0;
                        itemHasImage = ((SwtItem) item.getImpl()).image != null;
                        if (!hasText) {
                            hasText = itemHasText;
                        }
                        if (!hasImage) {
	...
	*/
        if ((getApi().style & SWT.VERTICAL) != 0) {
            int itemCount = (int) OS.SendMessage(getApi().handle, OS.TB_BUTTONCOUNT, 0, 0);
            if (itemCount > 1) {
                TBBUTTONINFO info = new TBBUTTONINFO();
                info.cbSize = TBBUTTONINFO.sizeof;
                info.dwMask = OS.TBIF_SIZE;
                long size = OS.SendMessage(getApi().handle, OS.TB_GETBUTTONSIZE, 0, 0);
                ...
                if (index < items.length) {
                    long padding = OS.SendMessage(getApi().handle, OS.TB_GETPADDING, 0, 0);
                    info.cx += OS.LOWORD(padding + extraPadding) * 2;
                }
                for (ToolItem item : items) {
                    if (item != null && (item.style & SWT.SEPARATOR) == 0) {
                        OS.SendMessage(getApi().handle, OS.TB_SETBUTTONINFO, ((SwtToolItem) item.getImpl()).id, info);
                    }
                }
            }
        }
	...
	*/
        if ((getApi().style & (SWT.WRAP | SWT.VERTICAL)) != 0) {
            ...
            if ((bits & OS.TBSTYLE_LIST) != 0) {
                ...
                info.dwMask = OS.TBIF_SIZE;
                for (ToolItem item : items) {
                    if (item != null && ((SwtToolItem) item.getImpl()).cx > 0) {
                        info.cx = ((SwtToolItem) item.getImpl()).cx;
                        OS.SendMessage(getApi().handle, OS.TB_SETBUTTONINFO, ((SwtToolItem) item.getImpl()).id, info);
                    }
                }
            }
        ...
        for (ToolItem item : items) {
            if (item != null)
                ((SwtToolItem) item.getImpl()).resizeControl();
        }
    }
    ...
    boolean mnemonicHit(char ch) {
        int[] id = new int[1];
        if (OS.SendMessage(getApi().handle, OS.TB_MAPACCELERATOR, ch, id) == 0) {
            return false;
        }
        if ((getApi().style & SWT.FLAT) != 0 && !setTabGroupFocus())
            return false;
        int index = (int) OS.SendMessage(getApi().handle, OS.TB_COMMANDTOINDEX, id[0], 0);
        if (index == -1)
            return false;
        OS.SendMessage(getApi().handle, OS.TB_SETHOTITEM, index, 0);
        ((SwtToolItem) items[id[0]].getImpl()).click(false);
        return true;
    }
	...
	*/
        ...
        if (index == -1)
            return false;
        return findMnemonic(((SwtItem) items[id[0]].getImpl()).text) != '\0';
    }
    ...
    public void removeControl(Control control) {
        ...
        for (ToolItem item : items) {
            if (item != null && ((SwtToolItem) item.getImpl()).control == control) {
	...
	*/
            ...
            if (hasImage && !hasText) {
                for (ToolItem item : items) {
                    if (item != null && (item.style & SWT.DROP_DOWN) != 0) {
                        ...
                        OS.SendMessage(getApi().handle, OS.TB_GETBUTTONINFO, ((SwtToolItem) item.getImpl()).id, info);
                        if (set) {
    ...
    void setDisabledImageList(ImageList imageList) {
        ...
        setDropDownItems(false);
        OS.SendMessage(getApi().handle, OS.TB_SETDISABLEDIMAGELIST, 0, hImageList);
        setDropDownItems(true);
    }
	...
	*/
    ...
    void setHotImageList(ImageList imageList) {
        ...
        setDropDownItems(false);
        OS.SendMessage(getApi().handle, OS.TB_SETHOTIMAGELIST, 0, hImageList);
        setDropDownItems(true);
    }
    ...
    void setImageList(ImageList imageList) {
        ...
        setDropDownItems(false);
        OS.SendMessage(getApi().handle, OS.TB_SETIMAGELIST, 0, hImageList);
        setDropDownItems(true);
    }
    ...
    public boolean setParent(Composite parent) {
        ...
        OS.SendMessage(getApi().handle, OS.TB_SETPARENT, hwndParent, 0);
        /*
	...
	*/
        ...
        long hwndToolTip = OS.SendMessage(getApi().handle, OS.TB_GETTOOLTIPS, 0, 0);
    ...
    void setRowCount(int count) {
		...
		*/
            count += 2;
            OS.SendMessage(getApi().handle, OS.TB_SETROWS, OS.MAKEWPARAM(count, 1), 0);
            int flags = OS.SWP_NOACTIVATE | OS.SWP_NOMOVE | OS.SWP_NOZORDER;
            OS.SetWindowPos(getApi().handle, 0, 0, 0, rect.right - rect.left, rect.bottom - rect.top, flags);
            ignoreResize = false;
        }
    }
    ...
    void setTabItemList(ToolItem[] tabList) {
        ...
        if (tabList != null) {
            for (ToolItem item : tabList) {
                ...
                if (item.isDisposed())
                    error(SWT.ERROR_INVALID_ARGUMENT);
                if (((SwtToolItem) item.getImpl()).parent != this.getApi())
    ...
    boolean updateTextDirection(int textDirection) {
        if (super.updateTextDirection(textDirection)) {
            ...
            int i = items.length;
            while (i-- > 0) {
                ((SwtToolItem) items[i].getImpl()).updateTextDirection(getApi().style & SWT.FLIP_TEXT_DIRECTION);
            }
            return true;
        }
        return false;
	...
	*/
		...
		*/
            ...
            if ((getApi().style & flags) != 0 && (getApi().style & flags) != flags) {
            ...
            if (0 <= index && index < items.length) {
				...
				*/
                    if (lastArrowId != -1)
                        return "";
                    return ((SwtToolItem) item.getImpl()).toolTipText;
                }
            }
        }
	...
	*/
        for (ToolItem item : items) {
            if (item != null) {
                int fsState = (int) OS.SendMessage(getApi().handle, OS.TB_GETSTATE, ((SwtToolItem) item.getImpl()).id, 0);
                if ((fsState & OS.TBSTATE_PRESSED) != 0) {
                    fsState &= ~OS.TBSTATE_PRESSED;
                    OS.SendMessage(getApi().handle, OS.TB_SETSTATE, ((SwtToolItem) item.getImpl()).id, fsState);
                }
            }
        }
        return result;
    ...
    LRESULT WM_CHAR(long wParam, long lParam) {
        ...
        switch((int) wParam) {
            case ' ':
                ...
                if (index != -1) {
                    TBBUTTON lpButton = new TBBUTTON();
                    long code = OS.SendMessage(getApi().handle, OS.TB_GETBUTTON, index, lpButton);
                    if (code != 0) {
                        ((SwtToolItem) items[lpButton.idCommand].getImpl()).click(false);
                        return LRESULT.ZERO;
                    }
                }
        }
	...
	*/
    ...
    LRESULT WM_MOUSEMOVE(long wParam, long lParam) {
        if (OS.GetMessagePos() != ((SwtDisplay) display.getImpl()).lastMouse)
            lastArrowId = -1;
	...
	*/
    ...
    LRESULT WM_SETFOCUS(long wParam, long lParam) {
        ...
        if (lastFocusId != -1 && getApi().handle == OS.GetFocus()) {
            int index = (int) OS.SendMessage(getApi().handle, OS.TB_COMMANDTOINDEX, lastFocusId, 0);
            OS.SendMessage(getApi().handle, OS.TB_SETHOTITEM, index, 0);
        }
        return result;
    }
    ...
    LRESULT WM_SIZE(long wParam, long lParam) {
        if (ignoreResize) {
            long code = callWindowProc(getApi().handle, OS.WM_SIZE, wParam, lParam);
            if (code == 0)
                return LRESULT.ZERO;
	...
	*/
        if ((getApi().style & SWT.BORDER) != 0 && (getApi().style & SWT.WRAP) != 0) {
            RECT windowRect = new RECT();
            OS.GetWindowRect(getApi().handle, windowRect);
            ...
            int bits = (int) OS.SendMessage(getApi().handle, OS.TB_GETEXTENDEDSTYLE, 0, 0);
            if (index == count) {
	...
	*/
        ...
        if (!OS.IsWindowVisible(getApi().handle))
            return result;
        if (OS.SendMessage(getApi().handle, OS.TB_GETROWS, 0, 0) == 1) {
            return result;
        }
        ...
        RECT oldRect = new RECT();
        OS.GetClientRect(getApi().handle, oldRect);
        RECT newRect = new RECT();
        OS.SetRect(newRect, 0, 0, lpwp.cx, lpwp.cy);
        OS.SendMessage(getApi().handle, OS.WM_NCCALCSIZE, 0, newRect);
        int oldWidth = oldRect.right - oldRect.left;
        ...
        if (newWidth > oldWidth) {
            ...
            OS.InvalidateRect(getApi().handle, rect, false);
        }
        return result;
    }
    ...
    int getForegroundPixel(ToolItem item) {
        if (item != null && ((SwtToolItem) item.getImpl()).foreground != -1) {
            return ((SwtToolItem) item.getImpl()).foreground;
        }
        return getForegroundPixel();
    }

    int getBackgroundPixel(ToolItem item) {
        if (item != null && ((SwtToolItem) item.getImpl()).background != -1) {
            return ((SwtToolItem) item.getImpl()).background;
        }
                    ...
                    case OS.HICF_ARROWKEYS:
                        {
                            ...
                            OS.GetClientRect(getApi().handle, client);
                            int index = (int) OS.SendMessage(getApi().handle, OS.TB_COMMANDTOINDEX, lpnmhi.idNew, 0);
                            RECT rect = new RECT();
                            OS.SendMessage(getApi().handle, OS.TB_GETITEMRECT, index, rect);
