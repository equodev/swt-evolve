...
public class SwtCoolBar extends SwtComposite implements ICoolBar {
	...
	*/
    ...
    Point computeSizeInPixels(int wHint, int hHint, boolean changed) {
        ...
        if (count != 0) {
            ...
            RECT rect = new RECT();
            OS.SendMessage(getApi().handle, OS.RB_GETRECT, count - 1, rect);
            ...
            for (int i = 0; i < count; i++) {
                OS.SendMessage(getApi().handle, OS.RB_GETBANDINFO, i, rbBand);
	...
	*/
        if (index == 0 && count > 0) {
            ((SwtCoolItem) getItem(0).getImpl()).setWrap(false);
        }
        /* Insert the item */
        if (OS.SendMessage(getApi().handle, OS.RB_INSERTBAND, index, rbBand) == 0) {
        ...
        items[((SwtCoolItem) item.getImpl()).id = id] = item;
    ...
    void destroyItem(CoolItem item) {
        int index = (int) OS.SendMessage(getApi().handle, OS.RB_IDTOINDEX, ((SwtCoolItem) item.getImpl()).id, 0);
        int count = (int) OS.SendMessage(getApi().handle, OS.RB_GETBANDCOUNT, 0, 0);
        if (count != 0) {
	...
	*/
        ...
        items[((SwtCoolItem) item.getImpl()).id] = null;
        ((SwtCoolItem) item.getImpl()).id = -1;
        if (ignoreResize) {
            ((SwtCoolItem) nextItem.getImpl()).setWrap(true);
            ignoreResize = false;
        }
    ...
    void drawThemeBackground(long hDC, long hwnd, RECT rect) {
        ...
        RECT rect2 = new RECT();
        OS.GetClientRect(getApi().handle, rect2);
        OS.MapWindowPoints(getApi().handle, hwnd, rect2, 2);
        POINT lpPoint = new POINT();
        OS.SetWindowOrgEx(hDC, -rect2.left, -rect2.top, lpPoint);
        OS.SendMessage(getApi().handle, OS.WM_PRINT, hDC, OS.PRF_CLIENT | OS.PRF_ERASEBKGND);
    ...
    int getMargin(int index) {
        ...
        OS.SendMessage(getApi().handle, OS.RB_GETBANDMARGINS, 0, margins);
        margin += margins.cxLeftWidth + margins.cxRightWidth;
        RECT rect = new RECT();
        OS.SendMessage(getApi().handle, OS.RB_GETBANDBORDERS, index, rect);
        if ((getApi().style & SWT.FLAT) != 0) {
            /*
    ...
    public CoolItem getItem(int index) {
        ...
        OS.SendMessage(getApi().handle, OS.RB_GETBANDINFO, index, rbBand);
    ...
    public int getItemCount() {
        checkWidget();
        return (int) OS.SendMessage(getApi().handle, OS.RB_GETBANDCOUNT, 0, 0);
    }
    ...
    Point[] getItemSizesInPixels() {
        ...
        int separator = (getApi().style & SWT.FLAT) == 0 ? SEPARATOR_WIDTH : 0;
        MARGINS margins = new MARGINS();
        for (int i = 0; i < count; i++) {
            RECT rect = new RECT();
            OS.SendMessage(getApi().handle, OS.RB_GETRECT, i, rect);
            OS.SendMessage(getApi().handle, OS.RB_GETBANDINFO, i, rbBand);
            OS.SendMessage(getApi().handle, OS.RB_GETBANDMARGINS, 0, margins);
    ...
    boolean isLastItemOfRow(int index) {
        ...
        rbBand.cbSize = REBARBANDINFO.sizeof;
        rbBand.fMask = OS.RBBIM_STYLE;
        OS.SendMessage(getApi().handle, OS.RB_GETBANDINFO, index + 1, rbBand);
    ...
    public int[] getWrapIndices() {
        ...
        for (int i = 0; i < items.length; i++) {
            if (((SwtCoolItem) items[i].getImpl()).getWrap())
                indices[count++] = i;
        }
    ...
    public int indexOf(CoolItem item) {
        ...
        return (int) OS.SendMessage(getApi().handle, OS.RB_IDTOINDEX, ((SwtCoolItem) item.getImpl()).id, 0);
    }
	...
	*/
        ...
        if (0 <= index && index < count) {
            ...
            rbBand.fMask = OS.RBBIM_SIZE;
            OS.SendMessage(getApi().handle, OS.RB_SETBANDINFO, index, rbBand);
        }
    }
    ...
    public void removeControl(Control control) {
        ...
        for (CoolItem item : items) {
            if (item != null && ((SwtCoolItem) item.getImpl()).control == control) {
    ...
    void setBackgroundPixel(int pixel) {
        ...
        OS.SendMessage(getApi().handle, OS.RB_SETBKCOLOR, 0, pixel);
        setItemColors((int) OS.SendMessage(getApi().handle, OS.RB_GETTEXTCOLOR, 0, 0), pixel);
        /*
	...
	*/
    ...
    void setForegroundPixel(int pixel) {
        ...
        OS.SendMessage(getApi().handle, OS.RB_SETTEXTCOLOR, 0, pixel);
        setItemColors(pixel, (int) OS.SendMessage(getApi().handle, OS.RB_GETBKCOLOR, 0, 0));
    }
    ...
    void setItemColors(int foreColor, int backColor) {
        ...
        for (int i = 0; i < count; i++) {
            OS.SendMessage(getApi().handle, OS.RB_SETBANDINFO, i, rbBand);
        }
    }
 ...
 */
    void setItemOrder(int[] itemOrder) {
        if (itemOrder == null)
            error(SWT.ERROR_NULL_ARGUMENT);
        int itemCount = (int) OS.SendMessage(getApi().handle, OS.RB_GETBANDCOUNT, 0, 0);
        ...
        for (int i = 0; i < itemOrder.length; i++) {
            int id = ((SwtCoolItem) originalItems[itemOrder[i]].getImpl()).id;
            int index = (int) OS.SendMessage(getApi().handle, OS.RB_IDTOINDEX, id, 0);
            if (index != i) {
                ...
                /* Move the item */
                OS.SendMessage(getApi().handle, OS.RB_MOVEBAND, index, i);
 ...
 */
    void setItemSizes(Point[] sizes) {
        ...
        for (int i = 0; i < count; i++) {
            OS.SendMessage(getApi().handle, OS.RB_GETBANDINFO, i, rbBand);
            ((SwtCoolItem) items[rbBand.wID].getImpl()).setSizeInPixels(sizes[i].x, sizes[i].y);
        }
    }
    ...
    public void setWrapIndices(int[] indices) {
        ...
        for (int i = 0; i < items.length; i++) {
            ...
            if (((SwtCoolItem) item.getImpl()).getWrap()) {
                ...
                ((SwtCoolItem) item.getImpl()).setWrap(false);
            }
        }
        ...
        for (int index : indices) {
            if (0 <= index && index < items.length) {
                ...
                ((SwtCoolItem) item.getImpl()).setWrap(true);
                resizeToMaximumWidth(index - 1);
            }
        }
        setRedraw(true);
            ...
            case OS.RBN_BEGINDRAG:
                {
                    ...
                    int button = ((SwtDisplay) display.getImpl()).lastButton != 0 ? ((SwtDisplay) display.getImpl()).lastButton : 1;
			...
			*/
                    ...
                    if (lprbcs.uBand != -1) {
                        ...
                        if (control != null) {
                            ...
                            ((SwtControl) control.getImpl()).setBoundsInPixels(lprbcs.rcChild_left, lprbcs.rcChild_top, width, height);
                        }
                    }
                    break;
                }
            case OS.RBN_HEIGHTCHANGE:
                {
                    if (!ignoreResize) {
                        ...
                        int border = getBorderWidthInPixels();
                        int barHeight = (int) OS.SendMessage(getApi().handle, OS.RB_GETBARHEIGHT, 0, 0);
                        if ((getApi().style & SWT.VERTICAL) != 0) {
            ...
            case OS.NM_CUSTOMDRAW:
                {
                    if (findBackgroundControl() != null || (getApi().style & SWT.FLAT) != 0) {
    ...
    private static void handleDPIChange(Widget widget, int newZoom, float scalingFactor) {
        ...
        for (int index = 0; index < sizes.length; index++) {
            ...
            if (((coolBar.style & SWT.VERTICAL) != 0)) {
                ...
                scaledSizes[index] = new Point(Math.round((sizes[index].x) * scalingFactor), Math.max(Math.round((sizes[index].y) * scalingFactor), 0));
                ((SwtCoolItem) item.getImpl()).setMinimumSizeInPixels(Math.round(minSizes[index].x * scalingFactor), controlHeight);
