...
public class SwtButton extends SwtControl implements IButton {
    ...
    void _setImage(Image image) {
        ...
        if (image != null) {
            ...
            OS.SendMessage(getApi().handle, OS.BCM_SETIMAGELIST, 0, 0);
        }
        /*
	...
	*/
        if ((getApi().style & SWT.RIGHT_TO_LEFT) != 0) {
            if (!OS.IsAppThemed()) {
                text = OS.IsWindowEnabled(getApi().handle) ? text : text + " ";
            }
        }
	...
	*/
        ignoreMouse = true;
        OS.SendMessage(getApi().handle, OS.BM_CLICK, 0, 0);
        ignoreMouse = false;
    }
	...
	*/
		...
		*/
            if ((getApi().style & SWT.RADIO) != 0) {
                getApi().state |= DRAW_BACKGROUND;
            }
            useDarkModeExplorerTheme = ((SwtDisplay) display.getImpl()).useDarkModeExplorerTheme;
    ...
    private boolean customForegroundDrawing() {
        return foreground != -1 && !text.isEmpty() && OS.IsWindowEnabled(getApi().handle);
    }
	...
	*/
    ...
    private boolean isChecked() {
        long flags = OS.SendMessage(getApi().handle, OS.BM_GETCHECK, 0, 0);
    ...
    int resolveTextDirection() {
        return (getApi().style & SWT.ARROW) != 0 ? SWT.NONE : BidiUtil.resolveTextDirection(text);
    }
    ...
    void setDefault(boolean value) {
        ...
        if (value) {
            ...
            OS.SendMessage(hwndShell, OS.DM_SETDEFID, getApi().handle, 0);
        } else {
            bits &= ~OS.BS_DEFPUSHBUTTON;
            OS.SendMessage(hwndShell, OS.DM_SETDEFID, 0, 0);
        }
        OS.SendMessage(getApi().handle, OS.BM_SETSTYLE, bits, 1);
    }
	...
	*/
    ...
    void setMessage(String message) {
        ...
        if ((getApi().style & SWT.COMMAND) != 0) {
            ...
            OS.SendMessage(getApi().handle, OS.BCM_SETNOTE, 0, chars);
        }
    }
    ...
    void updateSelection(int flags) {
        if (flags != OS.SendMessage(getApi().handle, OS.BM_GETCHECK, 0, 0)) {
            /*
		...
		*/
            ...
            OS.SendMessage(getApi().handle, OS.BM_SETCHECK, flags, 0);
            if (bits != OS.GetWindowLong(getApi().handle, OS.GWL_STYLE)) {
                OS.SetWindowLong(getApi().handle, OS.GWL_STYLE, bits);
            }
        }
	...
	*/
    ...
    LRESULT WM_KILLFOCUS(long wParam, long lParam) {
        ...
        if ((getApi().style & SWT.PUSH) != 0 && getDefault()) {
            ((SwtDecorations) menuShell().getImpl()).setDefaultButton(null, false);
        }
        return result;
    }
	...
	*/
        ...
        if ((getApi().style & SWT.PUSH) != 0) {
            ((SwtDecorations) menuShell().getImpl()).setDefaultButton(this.getApi(), false);
        }
        return result;
    }
	...
	* buttons.
	*/
        if ((getApi().style & (SWT.RADIO | SWT.CHECK | SWT.TOGGLE)) != 0) {
            ...
            if (redraw) {
                ...
                long code = OS.DefWindowProc(getApi().handle, OS.WM_UPDATEUISTATE, wParam, lParam);
	...
	*/
    ...
    LRESULT wmCommandChild(long wParam, long lParam) {
        ...
        switch(code) {
            ...
            case OS.BN_DOUBLECLICKED:
                if ((getApi().style & (SWT.CHECK | SWT.TOGGLE)) != 0) {
                    ...
                    case OS.CDDS_PREPAINT:
                        {
                            ...
                            if (customBackgroundDrawing()) {
                                ...
                                if ((getApi().style & SWT.TOGGLE) != 0 && isChecked()) {
                                    pixel = getDifferentColor(background);
                                }
                            ...
                            if (customForegroundDrawing()) {
                                ...
                                if ((getApi().style & SWT.WRAP) != 0) {
                                    ...
                                    // TODO: this always draws the prefix
                                ...
                                } else if ((getApi().style & SWT.LEFT) != 0) {
                                    flags |= OS.DT_LEFT;
                                } else if ((getApi().style & SWT.RIGHT) != 0) {
                                    flags |= OS.DT_RIGHT;
    ...
    LRESULT wmDrawChild(long wParam, long lParam) {
        ...
        if (OS.IsAppThemed()) {
            ...
            int iStateId = getThemeStateId(getApi().style, pressed, enabled);
            OS.DrawThemeBackground(((SwtDisplay) display.getImpl()).hScrollBarThemeAuto(getApi().nativeZoom), struct.hDC, OS.SBP_ARROWBTN, iStateId, rect, null);
        } else {
            int uState = OS.DFCS_SCROLLLEFT;
            switch(getApi().style & (SWT.UP | SWT.DOWN | SWT.LEFT | SWT.RIGHT)) {
                case SWT.UP:
            ...
            if ((getApi().style & SWT.FLAT) == SWT.FLAT)
                uState |= OS.DFCS_FLAT;
