 ...
 */
public class SwtColorDialog extends SwtDialog implements IColorDialog {

    // from the MS spec for CHOOSECOLOR.lpCustColors
	...
	*/
        ...
        if (dialogOrientation != parentOrientation) {
            ...
            hwndOwner = OS.CreateWindowEx(exStyle, SwtShell.DialogClass, null, 0, OS.CW_USEDEFAULT, 0, OS.CW_USEDEFAULT, 0, hwndParent, 0, OS.GetModuleHandle(null), null);
        ...
        Display display = parent.getImpl()._display();
        if (((SwtDisplay) display.getImpl()).lpCustColors == 0) {
            long hHeap = OS.GetProcessHeap();
            ((SwtDisplay) display.getImpl()).lpCustColors = OS.HeapAlloc(hHeap, OS.HEAP_ZERO_MEMORY, CUSTOM_COLOR_COUNT * 4);
            for (int i = 0; i < CUSTOM_COLOR_COUNT; i++) {
                colors[i] = 0x00FFFFFF;
            }
            OS.MoveMemory(((SwtDisplay) display.getImpl()).lpCustColors, colors, CUSTOM_COLOR_COUNT * 4);
        }
        ...
        lpcc.lpCustColors = ((SwtDisplay) display.getImpl()).lpCustColors;
        if (rgb != null) {
        ...
        if ((style & (SWT.APPLICATION_MODAL | SWT.SYSTEM_MODAL)) != 0) {
            oldModal = ((SwtDisplay) display.getImpl()).getModalDialog();
        ...
        OS.MoveMemory(colors, ((SwtDisplay) display.getImpl()).lpCustColors, colors.length * 4);
    ...
    public ColorDialog getApi() {
        if (api == null)
            api = ColorDialog.createApi(this);
        return (ColorDialog) api;
    }
}
