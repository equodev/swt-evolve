...
public class SwtTrayItem extends SwtItem implements ITrayItem {
	 ...
	 */
        iconData.uID = id = (newIcon ? ((SwtDisplay) display.getImpl()).nextTrayId++ : id);
        iconData.hWnd = ((SwtDisplay) display.getImpl()).hwndMessage;
        iconData.uFlags = OS.NIF_MESSAGE;
        iconData.uCallbackMessage = SwtDisplay.SWT_TRAYICONMSG;
        /*
	...
	*/
        switch((int) lParam) {
            ...
            case OS.NIN_BALLOONSHOW:
                if (toolTip != null && !((SwtToolTip) toolTip.getImpl()).visible) {
                    ((SwtToolTip) toolTip.getImpl()).visible = true;
                    if (((SwtWidget) toolTip.getImpl()).hooks(SWT.Show)) {
                        OS.SetForegroundWindow(hwnd);
                        toolTip.getImpl().sendEvent(SWT.Show);
                        // widget could be disposed at this point
                        if (isDisposed())
                            return 0;
                    }
            ...
            case OS.NIN_BALLOONUSERCLICK:
                if (toolTip != null) {
                    if (((SwtToolTip) toolTip.getImpl()).visible) {
                        ((SwtToolTip) toolTip.getImpl()).visible = false;
                        if (((SwtWidget) toolTip.getImpl()).hooks(SWT.Hide)) {
                            OS.SetForegroundWindow(hwnd);
                            toolTip.getImpl().sendEvent(SWT.Hide);
                            // widget could be disposed at this point
                            if (isDisposed())
                                return 0;
                        }
                    ...
                    if (lParam == OS.NIN_BALLOONUSERCLICK) {
                        if (((SwtWidget) toolTip.getImpl()).hooks(SWT.Selection)) {
                            OS.SetForegroundWindow(hwnd);
                            ((SwtWidget) toolTip.getImpl()).sendSelectionEvent(SWT.Selection);
                            // widget could be disposed at this point
                            if (isDisposed())
                                return 0;
                        }
    ...
    void releaseWidget() {
        super.releaseWidget();
        if (toolTip != null)
            ((SwtToolTip) toolTip.getImpl()).item = null;
        toolTip = null;
        if (image2 != null)
            image2.dispose();
        image2 = null;
