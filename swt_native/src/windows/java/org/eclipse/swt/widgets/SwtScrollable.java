 ...
 */
public abstract class SwtScrollable extends SwtControl implements IScrollable {
    ...
    Rectangle getClientAreaInPixels() {
        ...
        if (scrolledHandle != getApi().handle) {
            ...
            OS.MapWindowPoints(getApi().handle, scrolledHandle, rect, 2);
            x = -rect.left;
            y = -rect.top;
        }
    ...
    LRESULT WM_HSCROLL(long wParam, long lParam) {
        ...
        if (horizontalBar != null && lParam == 0) {
            return wmScroll(horizontalBar, (getApi().state & CANVAS) != 0, getApi().handle, OS.WM_HSCROLL, wParam, lParam);
        }
        return result;
    }
    ...
    LRESULT WM_MOUSEWHEEL(long wParam, long lParam) {
        return wmScrollWheel((getApi().state & CANVAS) != 0, wParam, lParam, false);
    }

    @Override
    LRESULT WM_MOUSEHWHEEL(long wParam, long lParam) {
        return wmScrollWheel((getApi().state & CANVAS) != 0, -1 * wParam, lParam, true);
    }
    ...
    LRESULT WM_VSCROLL(long wParam, long lParam) {
        ...
        if (verticalBar != null && lParam == 0) {
            return wmScroll(verticalBar, (getApi().state & CANVAS) != 0, getApi().handle, OS.WM_VSCROLL, wParam, lParam);
        }
        return result;
    }
	...
	*/
        if (update) {
            ...
            MouseWheelData wheelData = new MouseWheelData(vertical, bar, wParam, ((SwtDisplay) display.getImpl()).scrollRemainderBar);
            if (wheelData.count == 0)
                return null;
            SCROLLINFO info = new SCROLLINFO();
            info.cbSize = SCROLLINFO.sizeof;
            info.fMask = OS.SIF_POS;
            OS.GetScrollInfo(getApi().handle, ((SwtScrollBar) bar.getImpl()).scrollBarType(), info);
            info.nPos -= wheelData.count;
            OS.SetScrollInfo(getApi().handle, ((SwtScrollBar) bar.getImpl()).scrollBarType(), info, true);
            int msg = vertical ? OS.WM_VSCROLL : OS.WM_HSCROLL;
            OS.SendMessage(getApi().handle, msg, OS.SB_THUMBPOSITION, 0);
            return LRESULT.ZERO;
        }
        /*
	...
	*/
        ...
        if (verticalBar != null) {
            ...
            if (position != vPosition) {
                ...
                ((SwtWidget) verticalBar.getImpl()).sendSelectionEvent(SWT.Selection, event, true);
            }
        }
        if (horizontalBar != null) {
            ...
            if (position != hPosition) {
                ...
                ((SwtWidget) horizontalBar.getImpl()).sendSelectionEvent(SWT.Selection, event, true);
            }
        }
        return new LRESULT(code);
    }
