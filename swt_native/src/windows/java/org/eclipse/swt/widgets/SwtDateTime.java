...
public class SwtDateTime extends SwtComposite implements IDateTime {
    ...
    public SwtDateTime(Composite parent, int style, DateTime api) {
        ...
        if ((this.getApi().style & SWT.SHORT) != 0) {
            String buffer = ((this.getApi().style & SWT.DATE) != 0) ? getCustomShortDateFormat() : getCustomShortTimeFormat();
            TCHAR lpszFormat = new TCHAR(0, buffer, true);
            OS.SendMessage(getApi().handle, OS.DTM_SETFORMAT, 0, lpszFormat);
        }
    }
	...
	*/
        ...
        if (wHint == SWT.DEFAULT || hHint == SWT.DEFAULT) {
            if ((getApi().style & SWT.CALENDAR) != 0) {
                ...
                if ((getApi().style & SWT.CALENDAR_WEEKNUMBERS) != 0) {
                    ...
                    OS.SendMessage(getApi().handle, OS.DTM_SETMCSTYLE, 0, bits | OS.MCS_WEEKNUMBERS);
                }
                SIZE size = new SIZE();
                OS.SendMessage(getApi().handle, OS.DTM_GETIDEALSIZE, 0, size);
                width = size.cx;
                height = size.cy;
    ...
    public int getDay() {
        ...
        int msg = (getApi().style & SWT.CALENDAR) != 0 ? OS.MCM_GETCURSEL : OS.DTM_GETSYSTEMTIME;
        OS.SendMessage(getApi().handle, msg, 0, systime);
        return systime.wDay;
    }
    ...
    public void setDate(int year, int month, int day) {
        ...
        int msg = (getApi().style & SWT.CALENDAR) != 0 ? OS.MCM_GETCURSEL : OS.DTM_GETSYSTEMTIME;
        OS.SendMessage(getApi().handle, msg, 0, systime);
        msg = (getApi().style & SWT.CALENDAR) != 0 ? OS.MCM_SETCURSEL : OS.DTM_SETSYSTEMTIME;
    ...
    public void setHours(int hours) {
        ...
        OS.SendMessage(getApi().handle, msg, 0, systime);
        if ((getApi().style & SWT.CALENDAR) != 0 && hours >= 0 && hours <= 23)
    ...
    public void setMinutes(int minutes) {
        ...
        OS.SendMessage(getApi().handle, msg, 0, systime);
        if ((getApi().style & SWT.CALENDAR) != 0 && minutes >= 0 && minutes <= 59)
    ...
    public void setSeconds(int seconds) {
        ...
        OS.SendMessage(getApi().handle, msg, 0, systime);
        if ((getApi().style & SWT.CALENDAR) != 0 && seconds >= 0 && seconds <= 59)
	...
	*/
        ...
        if ((getApi().style & SWT.DATE) != 0) {
            bits |= ((getApi().style & SWT.MEDIUM) != 0 ? OS.DTS_SHORTDATECENTURYFORMAT : OS.DTS_LONGDATEFORMAT);
    ...
    @Override
    TCHAR windowClass() {
        return (getApi().style & SWT.CALENDAR) != 0 ? CalendarClass : DateTimeClass;
    }

    @Override
    long windowProc() {
        return (getApi().style & SWT.CALENDAR) != 0 ? CalendarProc : DateTimeProc;
    }
        ...
        switch(hdr.code) {
			...
			*/
            ...
            case OS.MCN_SELCHANGE:
                {
                    if (ignoreSelection)
                        break;
                    SYSTEMTIME systime = new SYSTEMTIME();
                    OS.SendMessage(getApi().handle, OS.MCM_GETCURSEL, 0, systime);
            ...
            case OS.DTN_DATETIMECHANGE:
                {
                    SYSTEMTIME systime = new SYSTEMTIME();
                    OS.SendMessage(getApi().handle, OS.DTM_GETSYSTEMTIME, 0, systime);
	...
	*/
    ...
    LRESULT WM_LBUTTONDBLCLK(long wParam, long lParam) {
        ...
        if ((getApi().style & SWT.CALENDAR) != 0) {
            ...
            pMCHitTest.pt = pt;
            long code = OS.SendMessage(getApi().handle, OS.MCM_HITTEST, 0, pMCHitTest);
	...
	*/
        ...
        long code = callWindowProc(getApi().handle, OS.WM_TIMER, wParam, lParam);
