...
public class SwtTaskBar extends SwtWidget implements ITaskBar {
    ...
    static {
        //$NON-NLS-1$
        OS.PSPropertyKeyFromString("{F29F85E0-4FF9-1068-AB91-08002B27B3D9} 2\0".toCharArray(), PKEY_Title);
        //$NON-NLS-1$
        OS.PSPropertyKeyFromString("{9F4C2855-9F79-4B39-A8D0-E1D42DE1D5F3}, 6\0".toCharArray(), PKEY_AppUserModel_IsDestListSeparator);
    ...
    IShellLink createShellLink(MenuItem item) {
        ...
        if ((style & SWT.SEPARATOR) != 0) {
            ...
            if (text == null)
                text = SwtDisplay.LAUNCHER_PREFIX + SwtDisplay.TASKBAR_EVENT + ((SwtMenuItem) item.getImpl()).id;
            length = text.length();
            buffer = new char[length + 1];
            text.getChars(0, length, buffer, 0);
    ...
    String getIconsDir() {
        if (iconsDir != null)
            return iconsDir;
        File dir = new File(((SwtDisplay) display.getImpl()).appLocalDir + "\\ico_dir");
        if (dir.exists()) {
            // remove old icons
    ...
    public TaskItem getItem(Shell shell) {
        ...
        for (TaskItem item : items) {
            if (item != null && ((SwtTaskItem) item.getImpl()).shell == shell) {
                return item;
            }
        }
        TaskItem item = new TaskItem(this.getApi(), SWT.NONE);
        if (shell != null)
            ((SwtTaskItem) item.getImpl()).setShell(shell);
        return item;
    }
    ...
    void setMenu(Menu menu) {
        ...
        String appName = SwtDisplay.APP_NAME;
        char[] buffer = { 'S', 'W', 'T', '\0' };
