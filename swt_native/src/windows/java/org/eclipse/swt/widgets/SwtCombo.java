...
public class SwtCombo extends SwtComposite implements ICombo {
    ...
    public void add(String string) {
        ...
        int result = (int) OS.SendMessage(getApi().handle, OS.CB_ADDSTRING, 0, buffer);
        if (result == OS.CB_ERR)
    ...
    public void add(String string, int index) {
        ...
        TCHAR buffer = new TCHAR(getCodePage(), string, true);
        int result = (int) OS.SendMessage(getApi().handle, OS.CB_INSERTSTRING, index, buffer);
    ...
    public void addSegmentListener(SegmentListener listener) {
        ...
        if (!noSelection) {
            selection = (int) OS.SendMessage(getApi().handle, OS.CB_GETCURSEL, 0, 0);
        }
        clearSegments(true);
        applyEditSegments();
        applyListSegments();
        if (selection != OS.CB_ERR) {
            OS.SendMessage(getApi().handle, OS.CB_SETCURSEL, selection, 0);
        }
    }
    ...
    void applyEditSegments() {
        ...
        if (!hooks(SWT.Segments) && !filters(SWT.Segments) && (getApi().state & HAS_AUTO_DIRECTION) == 0)
            return;
        long hwndText = OS.GetDlgItem(getApi().handle, CBID_EDIT);
        ...
        OS.SendMessage(hwndText, OS.EM_SETLIMITTEXT, limit + Math.min(nSegments, Combo.LIMIT - limit), 0);
        length += nSegments;
	 ...
	 */
		 ...
		 */
            int auto = getApi().state & HAS_AUTO_DIRECTION;
    ...
    void applyListSegments() {
        ...
        while (index-- > 0) {
            buffer = null;
            if (add) {
                int length = (int) OS.SendMessage(getApi().handle, OS.CB_GETLBTEXTLEN, index, 0);
                if (length == OS.CB_ERR)
                    error(SWT.ERROR);
                buffer = new TCHAR(cp, length + 1);
                if (OS.SendMessage(getApi().handle, OS.CB_GETLBTEXT, index, buffer) == OS.CB_ERR)
                    return;
                items[index] = string = buffer.toString(0, length);
            } else {
                string = items[index];
            }
            if (OS.SendMessage(getApi().handle, OS.CB_DELETESTRING, index, 0) == OS.CB_ERR)
                return;
            if (buffer == null)
                buffer = new TCHAR(cp, string, true);
            if (OS.SendMessage(getApi().handle, OS.CB_INSERTSTRING, index, buffer) == OS.CB_ERR)
                return;
        }
        if (selection != OS.CB_ERR) {
            OS.SendMessage(getApi().handle, OS.CB_SETCURSEL, selection, 0);
    ...
    long callWindowProc(long hwnd, int msg, long wParam, long lParam) {
        ...
        long hwndList = OS.GetDlgItem(getApi().handle, CBID_LIST);
        if (hwnd == hwndList) {
    ...
    public void clearSelection() {
        checkWidget();
        OS.SendMessage(getApi().handle, OS.CB_SETEDITSEL, 0, -1);
    }
    ...
    Point computeSizeInPixels(int wHint, int hHint, boolean changed) {
        ...
        if (wHint == SWT.DEFAULT) {
            ...
            if ((getApi().style & SWT.H_SCROLL) != 0) {
                ...
                for (int i = 0; i < count; i++) {
                    length = (int) OS.SendMessage(getApi().handle, OS.CB_GETLBTEXTLEN, i, 0);
                    if (length != OS.CB_ERR) {
                        if (length + 1 > buffer.length)
                            buffer = new char[length + 1];
                        int result = (int) OS.SendMessage(getApi().handle, OS.CB_GETLBTEXT, i, buffer);
                        if (result != OS.CB_ERR) {
                            OS.DrawText(hDC, buffer, length, rect, flags);
                            width = Math.max(width, rect.right - rect.left);
                        }
        ...
        if (((getApi().style & SWT.SIMPLE) == 0) && OS.GetComboBoxInfo(getApi().handle, pcbi)) {
            ...
            int textHeight = (int) OS.SendMessage(getApi().handle, OS.CB_GETITEMHEIGHT, -1, 0);
        ...
        if ((getApi().style & SWT.SIMPLE) != 0 && (getApi().style & SWT.H_SCROLL) != 0) {
	...
	*/
        ...
        if (((SwtDisplay) display.getImpl()).comboUseDarkTheme) {
            OS.AllowDarkModeForWindow(getApi().handle, true);
            OS.SetWindowTheme(getApi().handle, "CFD\0".toCharArray(), null);
        }
	...
	*/
        if ((getApi().style & SWT.SIMPLE) != 0) {
            ...
            OS.SetWindowPos(getApi().handle, 0, 0, 0, 0x3FFF, 0x3FFF, flags);
            OS.SetWindowPos(getApi().handle, 0, 0, 0, 0, 0, flags);
        }
    }
    ...
    void deregister() {
        ...
        long hwndList = OS.GetDlgItem(getApi().handle, CBID_LIST);
        if (hwndList != 0)
            ((SwtDisplay) display.getImpl()).removeControl(hwndList);
    }
    ...
    public void deselect(int index) {
        checkWidget();
        int selection = (int) OS.SendMessage(getApi().handle, OS.CB_GETCURSEL, 0, 0);
        if (index != selection)
            return;
        OS.SendMessage(getApi().handle, OS.CB_SETCURSEL, -1, 0);
    ...
    boolean dragDetect(long hwnd, int x, int y, boolean filter, boolean[] detect, boolean[] consume) {
        if (filter && (getApi().style & SWT.READ_ONLY) == 0) {
            long hwndText = OS.GetDlgItem(getApi().handle, CBID_EDIT);
            if (hwndText != 0) {
                int[] start = new int[1], end = new int[1];
                OS.SendMessage(getApi().handle, OS.CB_GETEDITSEL, start, end);
                if (start[0] != end[0]) {
	...
	*/
        ...
        OS.MapWindowPoints(hwndText, getApi().handle, point, 1);
	...
	*/
    ...
    public String getItem(int index) {
        ...
        if (length != OS.CB_ERR) {
            if (hooks(SWT.Segments) || filters(SWT.Segments) || (getApi().state & HAS_AUTO_DIRECTION) != 0)
                return items[index];
            char[] buffer = new char[length + 1];
            int result = (int) OS.SendMessage(getApi().handle, OS.CB_GETLBTEXT, index, buffer);
            if (result != OS.CB_ERR)
    ...
    int getItemHeightInPixels() {
        int result = (int) OS.SendMessage(getApi().handle, OS.CB_GETITEMHEIGHT, 0, 0);
        if (result == OS.CB_ERR)
    ...
    public boolean getListVisible() {
        checkWidget();
        if ((getApi().style & SWT.DROP_DOWN) != 0) {
            return OS.SendMessage(getApi().handle, OS.CB_GETDROPPEDSTATE, 0, 0) != 0;
        }
        return true;
    }
    ...
    public void setListVisible(boolean visible) {
        checkWidget();
        OS.SendMessage(getApi().handle, OS.CB_SHOWDROPDOWN, visible ? 1 : 0, 0);
    }
    ...
    Event getSegments(String string) {
        ...
        if ((getApi().state & HAS_AUTO_DIRECTION) != 0) {
			 ...
			 */
                direction = (getApi().style & SWT.RIGHT_TO_LEFT) != 0 ? SWT.RIGHT_TO_LEFT : SWT.LEFT_TO_RIGHT;
            }
    ...
    public Point getSelection() {
        ...
        if ((getApi().style & SWT.DROP_DOWN) != 0 && (getApi().style & SWT.READ_ONLY) != 0) {
            return new Point(0, OS.GetWindowTextLength(getApi().handle));
        }
        int[] start = new int[1], end = new int[1];
        OS.SendMessage(getApi().handle, OS.CB_GETEDITSEL, start, end);
    ...
    public int getSelectionIndex() {
        ...
        if (noSelection)
            return -1;
        return (int) OS.SendMessage(getApi().handle, OS.CB_GETCURSEL, 0, 0);
    }
    ...
    int getTextHeightInPixels() {
        ...
        int result = (int) OS.SendMessage(getApi().handle, OS.CB_GETITEMHEIGHT, -1, 0);
        if (result == OS.CB_ERR)
            error(SWT.ERROR_CANNOT_GET_ITEM_HEIGHT);
        return (getApi().style & SWT.DROP_DOWN) != 0 ? result + 6 : result + 10;
    }
    ...
    public int getTextLimit() {
        ...
        if (hwndText == 0)
            return Combo.LIMIT;
        int limit = (int) OS.SendMessage(hwndText, OS.EM_GETLIMITTEXT, 0, 0) & 0x7FFFFFFF;
        if (segments != null && limit < Combo.LIMIT)
	...
	*/
        ...
        TCHAR buffer = new TCHAR(getCodePage(), string, true);
        do {
            index = (int) OS.SendMessage(getApi().handle, OS.CB_FINDSTRINGEXACT, last = index, buffer);
    ...
    void stateFlagsAdd(int flags) {
        final long tagCBoxPtr = OS.GetWindowLongPtr(getApi().handle, 0);
        /*
 ...
 */
    ...
    void register() {
        ...
        long hwndList = OS.GetDlgItem(getApi().handle, CBID_LIST);
        if (hwndList != 0)
            ((SwtDisplay) display.getImpl()).addControl(hwndList, this.getApi());
    }
    ...
    void remove(int index, boolean notify) {
        ...
        int length = OS.GetWindowTextLength(getApi().handle);
        int code = (int) OS.SendMessage(getApi().handle, OS.CB_DELETESTRING, index, 0);
        if (code == OS.CB_ERR) {
            int count = (int) OS.SendMessage(getApi().handle, OS.CB_GETCOUNT, 0, 0);
            if (0 <= index && index < count)
                error(SWT.ERROR_ITEM_NOT_REMOVED);
		 ...
		 */
            OS.SendMessage(getApi().handle, OS.CB_RESETCONTENT, 0, 0);
        }
        if ((getApi().style & SWT.H_SCROLL) != 0)
            setScrollWidth(buffer, true);
        if (notify && length != OS.GetWindowTextLength(getApi().handle)) {
            sendEvent(SWT.Modify);
            if (isDisposed())
                return;
        }
    ...
    public void remove(int start, int end) {
        ...
        int textLength = OS.GetWindowTextLength(getApi().handle);
        RECT rect = null;
        long hDC = 0, oldFont = 0, newFont = 0;
        int newWidth = 0;
        if ((getApi().style & SWT.H_SCROLL) != 0) {
            rect = new RECT();
            hDC = OS.GetDC(getApi().handle);
            newFont = OS.SendMessage(getApi().handle, OS.WM_GETFONT, 0, 0);
            if (newFont != 0)
                oldFont = OS.SelectObject(hDC, newFont);
        }
        ...
        for (int i = start; i <= end; i++) {
            char[] buffer = null;
            if ((getApi().style & SWT.H_SCROLL) != 0) {
                int length = (int) OS.SendMessage(getApi().handle, OS.CB_GETLBTEXTLEN, start, 0);
                if (length == OS.CB_ERR)
                    break;
                buffer = new char[length + 1];
                int result = (int) OS.SendMessage(getApi().handle, OS.CB_GETLBTEXT, start, buffer);
                if (result == OS.CB_ERR)
                    break;
            }
            int result = (int) OS.SendMessage(getApi().handle, OS.CB_DELETESTRING, start, 0);
            if (result == OS.CB_ERR) {
                error(SWT.ERROR_ITEM_NOT_REMOVED);
            } else if (result == 0) {
                /*
        ...
        if (textLength != OS.GetWindowTextLength(getApi().handle)) {
            sendEvent(SWT.Modify);
            if (isDisposed())
                return;
        }
	...
	*/
        if (OS.GetKeyState(OS.VK_LBUTTON) < 0) {
            if (OS.GetDlgItem(getApi().handle, CBID_EDIT) == OS.GetCapture())
                return true;
        }
    ...
    public void select(int index) {
        ...
        if (0 <= index && index < count) {
            int selection = (int) OS.SendMessage(getApi().handle, OS.CB_GETCURSEL, 0, 0);
            int code = (int) OS.SendMessage(getApi().handle, OS.CB_SETCURSEL, index, 0);
	...
	*/
		...
		*/
            ...
            if (rect.right - rect.left != 0) {
                if (OS.SendMessage(getApi().handle, OS.CB_GETDROPPEDCONTROLRECT, 0, rect) != 0) {
	...
	*/
    ...
    public void setItems(String... items) {
        ...
        for (String item : items) {
            ...
            int code = (int) OS.SendMessage(getApi().handle, OS.CB_ADDSTRING, 0, buffer);
            if (code == OS.CB_ERR)
                error(SWT.ERROR_ITEM_NOT_ADDED);
    ...
    void setScrollWidth() {
        ...
        int flags = OS.DT_CALCRECT | OS.DT_SINGLELINE | OS.DT_NOPREFIX;
        for (int i = 0; i < count; i++) {
            int length = (int) OS.SendMessage(getApi().handle, OS.CB_GETLBTEXTLEN, i, 0);
            if (length != OS.CB_ERR) {
                char[] buffer = new char[length + 1];
                int result = (int) OS.SendMessage(getApi().handle, OS.CB_GETLBTEXT, i, buffer);
                if (result != OS.CB_ERR) {
    ...
    void setScrollWidth(int scrollWidth) {
        ...
        if ((getApi().style & SWT.SIMPLE) != 0) {
            OS.SendMessage(getApi().handle, OS.CB_SETHORIZONTALEXTENT, scrollWidth, 0);
            return;
        }
	...
	*/
        ...
        if ((getApi().style & SWT.READ_ONLY) == 0)
            lockText = true;
        if (scroll) {
            OS.SendMessage(getApi().handle, OS.CB_SETDROPPEDWIDTH, 0, 0);
            OS.SendMessage(getApi().handle, OS.CB_SETHORIZONTALEXTENT, scrollWidth, 0);
        } else {
            scrollWidth += getSystemMetrics(OS.SM_CYHSCROLL);
            OS.SendMessage(getApi().handle, OS.CB_SETDROPPEDWIDTH, scrollWidth, 0);
            OS.SendMessage(getApi().handle, OS.CB_SETHORIZONTALEXTENT, 0, 0);
        }
        if ((getApi().style & SWT.READ_ONLY) == 0)
            lockText = oldLockText;
    }
    ...
    public void setSelection(Point selection) {
        ...
        OS.SendMessage(getApi().handle, OS.CB_SETEDITSEL, 0, bits);
    }
    ...
    public void setText(String string) {
        ...
        TCHAR buffer = new TCHAR(getCodePage(), string, true);
        if (OS.SetWindowText(getApi().handle, buffer)) {
            applyEditSegments();
            sendEvent(SWT.Modify);
            // widget could be disposed at this point
        }
    ...
    public void setTextLimit(int limit) {
        ...
        if (segments != null && limit > 0) {
            OS.SendMessage(getApi().handle, OS.CB_LIMITTEXT, limit + Math.min(segments.length, Combo.LIMIT - limit), 0);
        } else {
            OS.SendMessage(getApi().handle, OS.CB_LIMITTEXT, limit, 0);
        }
    }
    ...
    void setToolTipText(Shell shell, String string) {
        ...
        long hwndList = OS.GetDlgItem(getApi().handle, CBID_LIST);
        if (hwndText != 0)
            ((SwtShell) shell.getImpl()).setToolTipText(hwndText, string);
        if (hwndList != 0)
            ((SwtShell) shell.getImpl()).setToolTipText(hwndList, string);
	...
	*/
        switch((int) (msg.wParam)) {
            ...
            case OS.VK_ESCAPE:
                if ((getApi().style & SWT.DROP_DOWN) != 0) {
                    if (OS.SendMessage(getApi().handle, OS.CB_GETDROPPEDSTATE, 0, 0) != 0) {
                        return false;
                    }
                }
        }
    ...
    boolean traverseEscape() {
        if ((getApi().style & SWT.DROP_DOWN) != 0) {
            if (OS.SendMessage(getApi().handle, OS.CB_GETDROPPEDSTATE, 0, 0) != 0) {
                OS.SendMessage(getApi().handle, OS.CB_SHOWDROPDOWN, 0, 0);
                return true;
            }
        }
	...
	*/
        if ((getApi().style & SWT.DROP_DOWN) != 0) {
            RECT rect = new RECT();
            OS.SendMessage(getApi().handle, OS.CB_GETDROPPEDCONTROLRECT, 0, rect);
            int visibleCount = getItemCount() == 0 ? VISIBLE_COUNT : this.visibleCount;
            ...
            if (height != (rect.bottom - rect.top)) {
                ...
                OS.SetWindowPos(getApi().handle, 0, 0, 0, rect.right - rect.left, height, flags);
            }
        }
    }
    ...
    void updateDropDownTheme() {
        COMBOBOXINFO pcbi = new COMBOBOXINFO();
        pcbi.cbSize = COMBOBOXINFO.sizeof;
        if (!OS.GetComboBoxInfo(getApi().handle, pcbi))
            return;
        if (pcbi.hwndList == 0)
            return;
    ...
    void updateOrientation() {
		...
		*/
            ...
            OS.SetWindowPos(getApi().handle, 0, 0, 0, widthCombo - 1, heightCombo - 1, uFlags);
            OS.SetWindowPos(hwndText, 0, 0, 0, width, height, uFlags);
            OS.SetWindowPos(getApi().handle, 0, 0, 0, widthCombo, heightCombo, uFlags);
	...
	*/
    ...
    long windowProc(long hwnd, int msg, long wParam, long lParam) {
        ...
        if (hwnd != getApi().handle) {
            ...
            if ((hwndText != 0 && hwnd == hwndText) || (hwndList != 0 && hwnd == hwndList)) {
                ...
                switch(msg) {
                    /* Keyboard messages */
                    case OS.WM_CHAR:
                        processSegments = (hooks(SWT.Segments) || filters(SWT.Segments) || ((getApi().state & HAS_AUTO_DIRECTION) != 0)) && !ignoreCharacter && OS.GetKeyState(OS.VK_CONTROL) >= 0 && OS.GetKeyState(OS.VK_MENU) >= 0;
                    ...
                    case OS.WM_IME_CHAR:
                        ...
                        break;
                    case OS.WM_KEYDOWN:
                        processSegments = wParam == OS.VK_DELETE && (hooks(SWT.Segments) || filters(SWT.Segments) || ((getApi().state & HAS_AUTO_DIRECTION) != 0));
                    ...
                    case OS.WM_CUT:
                    case OS.WM_PASTE:
                        processSegments = hooks(SWT.Segments) || filters(SWT.Segments) || ((getApi().state & HAS_AUTO_DIRECTION) != 0);
                    case OS.WM_SETTEXT:
                        if (hwnd == hwndText) {
        ...
        switch(msg) {
                            ...
                            if (newText != null && !newText.equals(oldText)) {
                                ...
                                if (!newText.equals(oldText)) {
                                    ...
                                    if (index != -1 && index != wParam) {
                                        return callWindowProc(getApi().handle, OS.CB_SETCURSEL, index, lParam);
                                    }
                                }
                            }
                        }
            ...
            case OS.CB_FINDSTRINGEXACT:
                if (lParam != 0 && (hooks(SWT.Segments) || filters(SWT.Segments) || ((getApi().state & HAS_AUTO_DIRECTION) != 0))) {
                    long code = OS.CB_ERR;
            ...
            case OS.CB_DELETESTRING:
                {
                    if (hooks(SWT.Segments) || filters(SWT.Segments) || ((getApi().state & HAS_AUTO_DIRECTION) != 0)) {
                        ...
                        if (code != OS.CB_ERR && code != OS.CB_ERRSPACE) {
                            ...
                            if (!noSelection) {
                                index = (int) OS.SendMessage(getApi().handle, OS.CB_GETCURSEL, 0, 0);
                                if (index == wParam) {
                                    clearSegments(false);
                                    applyEditSegments();
                                }
	 ...
	 */
        final boolean isReadonly = ((getApi().style & SWT.READ_ONLY) != 0);
	...
	*/
        int oldSelection = (int) OS.SendMessage(getApi().handle, OS.CB_GETCURSEL, 0, 0);
        LRESULT result = super.WM_LBUTTONDOWN(wParam, lParam);
        if (result == LRESULT.ZERO)
            return result;
        if ((getApi().style & SWT.READ_ONLY) == 0) {
            int newSelection = (int) OS.SendMessage(getApi().handle, OS.CB_GETCURSEL, 0, 0);
	...
	*/
        if ((getApi().style & SWT.SIMPLE) != 0) {
            ...
            if (OS.IsWindowVisible(getApi().handle)) {
                ...
                OS.RedrawWindow(getApi().handle, null, 0, uFlags);
            }
            return result;
        }
        /*
	...
	*/
    ...
    LRESULT wmCommandChild(long wParam, long lParam) {
        ...
        switch(code) {
			...
			*/
                int index = (int) OS.SendMessage(getApi().handle, OS.CB_GETCURSEL, 0, 0);
                if (index != OS.CB_ERR) {
                    OS.SendMessage(getApi().handle, OS.CB_SETCURSEL, index, 0);
                }
                /*
			* It is possible (but unlikely), that application
            ...
            case OS.VK_DOWN:
                if (segments != null) {
                    ...
                    while (true) {
                        ...
                        OS.SendMessage(getApi().handle, OS.CB_GETEDITSEL, newStart, newEnd);
