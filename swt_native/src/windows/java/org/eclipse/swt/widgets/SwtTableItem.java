...
public class SwtTableItem extends SwtItem implements ITableItem {
    ...
    public Color getBackground() {
        checkWidget();
        if (!((SwtTable) parent.getImpl()).checkData(this.getApi(), true))
    ...
    RECT getBounds(int row, int column, boolean getText, boolean getImage, boolean fullText, boolean fullImage, long hDC) {
        ...
        if (((SwtTable) parent.getImpl()).fixScrollWidth)
            ((SwtTable) parent.getImpl()).setScrollWidth(null, true);
        RECT rect = new RECT();
        long hwnd = parent.handle;
        int bits = (int) OS.SendMessage(hwnd, OS.LVM_GETEXTENDEDLISTVIEWSTYLE, 0, 0);
        if (column == 0 && (bits & OS.LVS_EX_FULLROWSELECT) == 0) {
            if (((SwtTable) parent.getImpl()).explorerTheme) {
                rect.left = OS.LVIR_ICON;
                ((SwtTable) parent.getImpl()).ignoreCustomDraw = true;
                long code = OS.SendMessage(hwnd, OS.LVM_GETITEMRECT, row, rect);
                ((SwtTable) parent.getImpl()).ignoreCustomDraw = false;
                if (code == 0)
                    return new RECT();
                if (getText) {
                    int width = 0;
                    ...
                    rect.right += width + SwtTable.INSET * 2;
                }
            } else {
                if (getText) {
			...
			*/
                ...
                if (hasImage) {
                    if (column != 0 && getText && !getImage) {
                        ...
                        if (OS.SendMessage(hwnd, OS.LVM_GETSUBITEMRECT, row, iconRect) != 0) {
                            rect.left = iconRect.right + SwtTable.INSET / 2;
                        }
                    }
                } else {
                    if (getImage && !getText)
                ...
                if (getText) {
                    ...
                    if (string != null) {
                        ...
                        rect.right += textRect.right - textRect.left + SwtTable.INSET * 3 + 2;
                    }
                }
            }
        }
	...
	*/
    ...
    Rectangle getTextBoundsInPixels(int index) {
        ...
        rect.left += 2;
        if (index != 0)
            rect.left += SwtTable.INSET;
        rect.left = Math.min(rect.left, rect.right);
        rect.right = rect.right - SwtTable.INSET;
    ...
    void redraw() {
        if (((SwtTable) parent.getImpl()).currentItem == this.getApi() || !((SwtControl) parent.getImpl()).getDrawing())
            return;
        long hwnd = parent.handle;
    ...
    public void setBackground(Color color) {
        ...
        int pixel = -1;
        if (color != null) {
            ((SwtTable) parent.getImpl()).setCustomDraw(true);
            pixel = color.handle;
        }
        if (background == pixel)
            return;
    ...
    void setChecked(boolean checked, boolean notify) {
        ...
        if (notify) {
            ...
            ((SwtWidget) parent.getImpl()).sendSelectionEvent(SWT.Selection, event, false);
        }
        redraw();
    }
	...
	*/
        ...
        ((SwtTable) parent.getImpl()).setScrollWidth(this.getApi(), false);
        redraw();
    }
    ...
    public void setImage(int index, Image image) {
        ...
        ((SwtTable) parent.getImpl()).imageIndex(image, index);
        if (index == 0)
            ((SwtTable) parent.getImpl()).setScrollWidth(this.getApi(), false);
    ...
    private static void handleDPIChange(Widget widget, int newZoom, float scalingFactor) {
        ...
        if (cellFonts != null) {
            Shell shell = ((SwtTableItem) tableItem.getImpl()).parent.getShell();
