...
public class SwtTracker extends SwtWidget implements ITracker {
	...
	*/
        if (parent == null) {
            ...
            hwndTransparent = OS.CreateWindowEx(OS.WS_EX_LAYERED | OS.WS_EX_NOACTIVATE | OS.WS_EX_TOOLWINDOW, ((SwtDisplay) display.getImpl()).windowClass, null, OS.WS_POPUP, bounds.x, bounds.y, bounds.width, bounds.height, 0, 0, OS.GetModuleHandle(null), null);
            OS.SetLayeredWindowAttributes(hwndTransparent, 0, (byte) 0x01, OS.LWA_ALPHA);
            hwndOpaque = OS.CreateWindowEx(OS.WS_EX_LAYERED | OS.WS_EX_NOACTIVATE | OS.WS_EX_TOOLWINDOW, ((SwtDisplay) display.getImpl()).windowClass, null, OS.WS_POPUP, bounds.x, bounds.y, bounds.width, bounds.height, hwndTransparent, 0, OS.GetModuleHandle(null), null);
		...
		*/
            if (!mouseDown) {
                Rectangle bounds = ((SwtDisplay) display.getImpl()).getBoundsInPixels();
                hwndTransparent = OS.CreateWindowEx(OS.WS_EX_TRANSPARENT, ((SwtDisplay) display.getImpl()).windowClass, null, OS.WS_POPUP, bounds.x, bounds.y, bounds.width, bounds.height, 0, 0, OS.GetModuleHandle(null), null);
                //$NON-NLS-1$
                newProc = new Callback(this, "transparentProc", 4);
                long newProcAddress = newProc.getAddress();
                oldTransparentProc = OS.GetWindowLongPtr(hwndTransparent, OS.GWLP_WNDPROC);
