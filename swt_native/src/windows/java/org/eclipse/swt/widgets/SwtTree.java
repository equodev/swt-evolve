...
public class SwtTree extends SwtComposite implements ITree {
                        ...
                        if (explorerTheme) {
                            if (OS.IsWindowEnabled(getApi().handle) && !hooks(SWT.EraseItem)) {
                                ...
                                if (image != null) {
                                    ...
                                    // To Points
                                    if (size == null)
                                        size = DPIUtil.scaleDown(getImageSize(), zoom);
                ...
                if (cellRect[0].left > rect.right) {
                    ...
                    if (OS.PtInRect(cellRect[0], pt)) {
                        if (isCustomToolTip()) {
                            ...
                            itemRect[0] = ((SwtTreeItem) item[0].getImpl()).getBounds(order[index[0]], true, false, false, false, false, hDC);
                        }
	...
	*/
    ...
    private void calculateAndApplyIndentSize() {
        int indent = DPIUtil.scaleUp(DEFAULT_INDENT, getApi().nativeZoom);
                        ...
                        case OS.CDDS_POSTPAINT:
						   ...
						   The method getSlightlyDifferentColor gives us a color, that is near enough to the windows algorithm. */
                                    ...
                                    if (((SwtItem) columns[i].getImpl()).image != null) {
                                        ...
                                        GC gc = createNewGC(nmcd.hdc, data);
                                        int zoom = getZoom();
                                        Rectangle imageBounds = DPIUtil.scaleBounds(((SwtItem) columns[i].getImpl()).image.getBounds(), zoom, 100);
