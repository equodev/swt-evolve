...
public class SwtTree extends SwtComposite implements ITree {
    ...
    static {
        ...
        DPIZoomChangeRegistry.registerHandler(SwtTree::handleDPIChange, Tree.class);
    }
	...
	*/
    ...
    void _addListener(int eventType, Listener listener) {
        ...
        switch(eventType) {
            ...
            case SWT.PaintItem:
                {
                    ...
                    if (isCustomToolTip())
                        createItemToolTips();
                    OS.SendMessage(getApi().handle, OS.TVM_SETSCROLLTIME, 0, 0);
                    int bits = OS.GetWindowLong(getApi().handle, OS.GWL_STYLE);
			...
			*/
                    ...
                    if (bits != OS.GetWindowLong(getApi().handle, OS.GWL_STYLE)) {
                        OS.SetWindowLong(getApi().handle, OS.GWL_STYLE, bits);
				...
				*/
                        int count = (int) OS.SendMessage(getApi().handle, OS.TVM_GETCOUNT, 0, 0);
                        if (count != 0 && (bits & OS.TVS_NOHSCROLL) != 0) {
                            OS.ShowScrollBar(getApi().handle, OS.SB_HORZ, false);
                        }
                    }
                    break;
                }
    ...
    TreeItem _getItem(long hItem) {
        ...
        tvItem.hItem = hItem;
        if (OS.SendMessage(getApi().handle, OS.TVM_GETITEM, 0, tvItem) != 0) {
    ...
    void _setBackgroundPixel(int newPixel) {
        int oldPixel = (int) OS.SendMessage(getApi().handle, OS.TVM_GETBKCOLOR, 0, 0);
		...
		*/
            if (oldPixel != -1) {
                OS.SendMessage(getApi().handle, OS.TVM_SETBKCOLOR, 0, -1);
            }
            /* Set the background color */
            OS.SendMessage(getApi().handle, OS.TVM_SETBKCOLOR, 0, newPixel);
            /*
		...
		*/
            if (explorerTheme && ENABLE_TVS_EX_FADEINOUTEXPANDOS) {
                int bits2 = (int) OS.SendMessage(getApi().handle, OS.TVM_GETEXTENDEDSTYLE, 0, 0);
                if (newPixel == -1 && findImageControl() == null) {
                    bits2 |= OS.TVS_EX_FADEINOUTEXPANDOS;
                } else {
                    bits2 &= ~OS.TVS_EX_FADEINOUTEXPANDOS;
                }
                OS.SendMessage(getApi().handle, OS.TVM_SETEXTENDEDSTYLE, 0, bits2);
            }
	...
	*/
        ...
        if (hwndHeader != 0) {
            OS.MapWindowPoints(hwndParent, getApi().handle, clientRect, 2);
            if (columnCount != 0) {
        ...
        for (int i = 0; i < Math.max(1, columnCount); i++) {
            ...
            if (x + width > clientRect.left) {
                ...
                if (i == 0) {
                    ...
                    if (findImageControl() != null) {
                        if (explorerTheme) {
                            if (OS.IsWindowEnabled(getApi().handle) && !hooks(SWT.EraseItem)) {
                                Image image = null;
                                if (index == 0) {
                                    image = ((SwtItem) item.getImpl()).image;
                                } else {
                                    Image[] images = ((SwtTreeItem) item.getImpl()).images;
                                    if (images != null)
                                        image = images[index];
                                }
                                if (image != null) {
                                    ...
                                    // To Points
                                    if (size == null)
                                        size = Win32DPIUtils.pixelToPoint(getImageSize(), zoom);
                ...
                if (OS.IsWindowEnabled(getApi().handle)) {
                    ...
                    if (selected) {
                        if (i != 0 && (getApi().style & SWT.FULL_SELECTION) == 0) {
                            OS.SetTextColor(hDC, getForegroundPixel());
                    ...
                    if (drawForeground) {
                        clrText = ((SwtTreeItem) item.getImpl()).cellForeground != null ? ((SwtTreeItem) item.getImpl()).cellForeground[index] : -1;
                        if (clrText == -1)
                            clrText = ((SwtTreeItem) item.getImpl()).foreground;
                    }
                    if (drawBackground) {
                        clrTextBk = ((SwtTreeItem) item.getImpl()).cellBackground != null ? ((SwtTreeItem) item.getImpl()).cellBackground[index] : -1;
                        if (clrTextBk == -1)
                            clrTextBk = ((SwtTreeItem) item.getImpl()).background;
                ...
                if (drawItem) {
					...
					*/
                        if (rect.left < rect.right) {
                            String string = null;
                            if (index == 0) {
                                string = ((SwtItem) item.getImpl()).text;
                            } else {
                                String[] strings = ((SwtTreeItem) item.getImpl()).strings;
                                if (strings != null)
                            ...
                            if (string != null) {
                                ...
                                if ((string != null) && (string.length() > SwtItem.TEXT_LIMIT)) {
                                    string = string.substring(0, SwtItem.TEXT_LIMIT - SwtItem.ELLIPSIS.length()) + SwtItem.ELLIPSIS;
                                }
        ...
        if (!ignoreDrawFocus && focusRect != null) {
            ...
            if (!explorerTheme) {
                if (getApi().handle == OS.GetFocus()) {
                    int uiState = (int) OS.SendMessage(getApi().handle, OS.WM_QUERYUISTATE, 0, 0);
                    if ((uiState & OS.UISF_HIDEFOCUS) == 0) {
                        long hItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_CARET, 0);
	...
	*/
        ...
        if (OS.IsWindowVisible(getApi().handle) && nmcd.left < nmcd.right && nmcd.top < nmcd.bottom) {
            if (hFont != -1)
                OS.SelectObject(hDC, hFont);
            if (linesVisible) {
                RECT rect = new RECT();
		...
		*/
        ...
        if (clrText == -1 && clrTextBk == -1 && hFont == -1) {
            ...
            if (hFont != -1)
                OS.SelectObject(hDC, hFont);
            if (OS.IsWindowEnabled(getApi().handle) && OS.IsWindowVisible(getApi().handle)) {
                /*
                    ...
                    if ((bits & OS.TVS_FULLROWSELECT) == 0) {
                        if (columnCount != 0 && hwndHeader != 0) {
                            ...
                            if (!OS.IsAppThemed()) {
                                ...
                                if (OS.TreeView_GetItemRect(getApi().handle, item.handle, itemRect, true)) {
    ...
    LRESULT CDDS_POSTPAINT(NMTVCUSTOMDRAW nmcd, long wParam, long lParam) {
        ...
        if (OS.IsWindowVisible(getApi().handle)) {
                ...
                if (sortColumn != null && sortDirection != SWT.NONE) {
                    if (findImageControl() == null) {
						...
						*/
                            ...
                            if (hItem != 0) {
                                RECT rect = new RECT();
                                if (OS.TreeView_GetItemRect(getApi().handle, hItem, rect, false)) {
                                    top = rect.bottom;
                                }
                            }
                            RECT rect = new RECT();
			...
			*/
                ...
                if (height == 0) {
                    height = (int) OS.SendMessage(getApi().handle, OS.TVM_GETITEMHEIGHT, 0, 0);
    ...
    LRESULT CDDS_PREPAINT(NMTVCUSTOMDRAW nmcd, long wParam, long lParam) {
        if (explorerTheme) {
            if ((OS.IsWindowEnabled(getApi().handle) && hooks(SWT.EraseItem)) || hasCustomBackground() || findImageControl() != null) {
                RECT rect = new RECT();
                OS.SetRect(rect, nmcd.left, nmcd.top, nmcd.right, nmcd.bottom);
    ...
    long callWindowProc(long hwnd, int msg, long wParam, long lParam) {
        ...
        switch(msg) {
			...
			*/
                    ...
                    long hItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_CARET, 0);
                    if (hItem == 0) {
                        hItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_FIRSTVISIBLE, 0);
                        if (hItem != 0) {
                            TVITEM tvItem = new TVITEM();
                            tvItem.mask = OS.TVIF_HANDLE | OS.TVIF_STATE;
                            tvItem.hItem = hItem;
                            OS.SendMessage(getApi().handle, OS.TVM_GETITEM, 0, tvItem);
                            hSelect = hItem;
                            ignoreDeselect = ignoreSelect = lockSelection = true;
                            OS.SendMessage(getApi().handle, OS.TVM_SELECTITEM, OS.TVGN_CARET, hItem);
                            ignoreDeselect = ignoreSelect = lockSelection = false;
                            hSelect = 0;
                            if ((tvItem.state & OS.TVIS_SELECTED) == 0) {
                                OS.SendMessage(getApi().handle, OS.TVM_SETITEM, 0, tvItem);
                            }
                        }
                    }
                    break;
        ...
        switch(msg) {
            ...
            /* Resize messages */
            case OS.WM_SIZE:
                redraw = findImageControl() != null && getDrawing() && OS.IsWindowVisible(getApi().handle);
                if (redraw)
                    OS.DefWindowProc(getApi().handle, OS.WM_SETREDRAW, 0, 0);
            //FALL THROUGH
            /* Mouse messages */
        ...
        switch(msg) {
            ...
            case OS.WM_SIZE:
                if (redraw) {
                    OS.DefWindowProc(getApi().handle, OS.WM_SETREDRAW, 1, 0);
                    OS.InvalidateRect(getApi().handle, null, true);
                    if (hwndHeader != 0)
            ...
            case OS.WM_TIMER:
                {
                    if (findImageControl() != null) {
                        if (hItem != OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_FIRSTVISIBLE, 0)) {
                            OS.InvalidateRect(getApi().handle, null, true);
                        }
                    }
                    updateScrollBar();
    ...
    void checkBuffered() {
        ...
        if (OS.IsAppThemed()) {
            int exStyle = (int) OS.SendMessage(getApi().handle, OS.TVM_GETEXTENDEDSTYLE, 0, 0);
    ...
    boolean checkData(TreeItem item, int index, boolean redraw) {
		...
		* the entire tree.
		*/
            long hTopItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_FIRSTVISIBLE, 0);
            ...
            if (isDisposed() || item.isDisposed())
                return false;
            if (redraw)
                ((SwtTreeItem) item.getImpl()).redraw();
            if (hTopItem != OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_FIRSTVISIBLE, 0)) {
                OS.InvalidateRect(getApi().handle, null, true);
            }
        }
        return true;
	...
	*/
        if (getDrawing())
            return false;
        long hRoot = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_ROOT, 0);
        long hParent = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_PARENT, hItem);
        while (hParent != hRoot && hParent != 0) {
            hParent = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_PARENT, hParent);
        }
        return hParent == 0;
    }
    ...
    public void clear(int index, boolean all) {
        checkWidget();
        long hItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_ROOT, 0);
        if (hItem == 0)
        ...
        clear(hItem, tvItem);
        if (all) {
            hItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_CHILD, hItem);
    ...
    void clear(long hItem, TVITEM tvItem) {
        ...
        if (item != null) {
            if ((getApi().style & SWT.VIRTUAL) != 0 && !((SwtTreeItem) item.getImpl()).cached)
                return;
            ((SwtTreeItem) item.getImpl()).clear();
            ((SwtTreeItem) item.getImpl()).redraw();
        }
    }
    ...
    void clearAll(long hItem, TVITEM tvItem, boolean all) {
        while (hItem != 0) {
            clear(hItem, tvItem);
            if (all) {
                long hFirstItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_CHILD, hItem);
                clearAll(hFirstItem, tvItem, all);
            }
            hItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_NEXT, hItem);
        }
    }
    ...
    Point computeSizeInPixels(int wHint, int hHint, boolean changed) {
        ...
        long hItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_ROOT, 0);
        while (hItem != 0) {
            if ((getApi().style & SWT.VIRTUAL) == 0 && !painted) {
                TVITEM tvItem = new TVITEM();
            ...
            if (OS.TreeView_GetItemRect(getApi().handle, hItem, rect, true)) {
                width = Math.max(width, rect.right);
                height += rect.bottom - rect.top;
            }
            hItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_NEXTVISIBLE, hItem);
        }
        if (width == 0)
            width = DEFAULT_WIDTH;
        if (height == 0)
	...
	*/
        ...
        OS.SendMessage(getApi().handle, OS.WM_SETFONT, hFont, 0);
        /*
	 ...
	 */
        ...
        createdAsRTL = (getApi().style & SWT.RIGHT_TO_LEFT) != 0;
    }
    ...
    void createHeaderToolTips() {
        ...
        headerToolTipHandle = OS.CreateWindowEx(bits, new TCHAR(0, OS.TOOLTIPS_CLASS, true), null, OS.TTS_NOPREFIX, OS.CW_USEDEFAULT, 0, OS.CW_USEDEFAULT, 0, getApi().handle, 0, OS.GetModuleHandle(null), null);
        if (headerToolTipHandle == 0)
        ...
        for (TreeItem item : items) {
            if (item != null) {
                ...
                if (strings != null) {
                    ...
                    ((SwtTreeItem) item.getImpl()).strings = temp;
                }
                Image[] images = ((SwtTreeItem) item.getImpl()).images;
                if (images != null) {
                    ...
                    ((SwtTreeItem) item.getImpl()).images = temp;
                }
                if (index == 0) {
                    if (columnCount != 0) {
                        if (strings == null) {
                            ((SwtTreeItem) item.getImpl()).strings = new String[columnCount + 1];
                            ((SwtTreeItem) item.getImpl()).strings[1] = ((SwtItem) item.getImpl()).text;
                        }
                        ((SwtItem) item.getImpl()).text = "";
                        if (images == null) {
                            ((SwtTreeItem) item.getImpl()).images = new Image[columnCount + 1];
                            ((SwtTreeItem) item.getImpl()).images[1] = ((SwtItem) item.getImpl()).image;
                        }
                        ((SwtItem) item.getImpl()).image = null;
                    }
                }
                if (((SwtTreeItem) item.getImpl()).cellBackground != null) {
                    int[] cellBackground = ((SwtTreeItem) item.getImpl()).cellBackground;
                    int[] temp = new int[columnCount + 1];
                    System.arraycopy(cellBackground, 0, temp, 0, index);
                    System.arraycopy(cellBackground, index, temp, index + 1, columnCount - index);
                    temp[index] = -1;
                    ((SwtTreeItem) item.getImpl()).cellBackground = temp;
                }
                if (((SwtTreeItem) item.getImpl()).cellForeground != null) {
                    int[] cellForeground = ((SwtTreeItem) item.getImpl()).cellForeground;
                    int[] temp = new int[columnCount + 1];
                    System.arraycopy(cellForeground, 0, temp, 0, index);
                    System.arraycopy(cellForeground, index, temp, index + 1, columnCount - index);
                    temp[index] = -1;
                    ((SwtTreeItem) item.getImpl()).cellForeground = temp;
                }
                if (((SwtTreeItem) item.getImpl()).cellFont != null) {
                    Font[] cellFont = ((SwtTreeItem) item.getImpl()).cellFont;
                    Font[] temp = new Font[columnCount + 1];
                    System.arraycopy(cellFont, 0, temp, 0, index);
                    System.arraycopy(cellFont, index, temp, index + 1, columnCount - index);
                    ((SwtTreeItem) item.getImpl()).cellFont = temp;
                }
            }
        }
	...
	*/
		...
		*/
        ...
        if (columnCount == 1 && OS.SendMessage(getApi().handle, OS.TVM_GETCOUNT, 0, 0) != 0) {
            OS.InvalidateRect(getApi().handle, null, true);
        }
        ...
        if (headerToolTipHandle != 0) {
            ...
            if (OS.SendMessage(hwndHeader, OS.HDM_GETITEMRECT, index, rect) != 0) {
                ...
                lpti.uId = ((SwtTreeColumn) column.getImpl()).id = ((SwtDisplay) display.getImpl()).nextToolTipId++;
                lpti.left = rect.left;
                lpti.top = rect.top;
    ...
    void createItem(TreeItem item, long hParent, long hInsertAfter, long hItem) {
        ...
        long hNewItem = 0;
        long hFirstItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_CHILD, hParent);
        ...
        if (hItem == 0) {
            ...
            ignoreCustomDraw = true;
            hNewItem = OS.SendMessage(getApi().handle, OS.TVM_INSERTITEM, 0, tvInsert);
            ignoreCustomDraw = false;
            if (hNewItem == 0)
		...
		*/
            if (fixParent && (hParent != OS.TVI_ROOT)) {
                if (getDrawing() && OS.IsWindowVisible(getApi().handle)) {
                    RECT rect = new RECT();
                    if (OS.TreeView_GetItemRect(getApi().handle, hParent, rect, false)) {
		...
		*/
            if ((getApi().style & SWT.VIRTUAL) != 0) {
                if (currentItem != null) {
                    RECT rect = new RECT();
                    if (OS.TreeView_GetItemRect(getApi().handle, hNewItem, rect, false)) {
                        RECT damageRect = new RECT();
                        boolean damaged = OS.GetUpdateRect(getApi().handle, damageRect, true);
                        if (damaged && damageRect.top < rect.bottom) {
                            long rgn = OS.CreateRectRgn(0, 0, 0, 0);
                            int result = OS.GetUpdateRgn(getApi().handle, rgn, true);
                            if (result != OS.NULLREGION) {
                                OS.OffsetRgn(rgn, 0, rect.bottom - rect.top);
                                OS.InvalidateRgn(getApi().handle, rgn, true);
                            }
                            OS.DeleteObject(rgn);
                        }
                    }
    ...
    void createItemToolTips() {
        ...
        OS.SetWindowLong(getApi().handle, OS.GWL_STYLE, bits1);
        int bits2 = 0;
        if ((getApi().style & SWT.RIGHT_TO_LEFT) != 0)
	...
	*/
        ...
        itemToolTipHandle = OS.CreateWindowEx(bits2, new TCHAR(0, OS.TOOLTIPS_CLASS, true), null, OS.TTS_NOPREFIX | OS.TTS_NOANIMATE | OS.TTS_NOFADE, OS.CW_USEDEFAULT, 0, OS.CW_USEDEFAULT, 0, getApi().handle, 0, OS.GetModuleHandle(null), null);
        if (itemToolTipHandle == 0)
            error(SWT.ERROR_NO_HANDLES);
	...
	*/
    ...
    void createParent() {
        ...
        final int oldStyle = OS.GetWindowLong(getApi().handle, OS.GWL_STYLE);
        int newStyle = super.widgetStyle();
        newStyle &= ~OS.WS_VISIBLE;
        /* Show control once everything is configured */
        ...
        if ((oldStyle & OS.WS_BORDER) != 0) {
            ...
            OS.SetWindowLong(getApi().handle, OS.GWL_STYLE, noBorderStyle);
        }
        /* Create hwndParent */
        ...
        if ((getApi().style & SWT.RIGHT_TO_LEFT) != 0)
            bits |= OS.WS_EX_LAYOUTRTL;
        hwndHeader = OS.CreateWindowEx(bits, HeaderClass, null, OS.HDS_BUTTONS | OS.HDS_FULLDRAG | OS.HDS_DRAGDROP | OS.HDS_HIDDEN | OS.WS_CHILD | OS.WS_CLIPSIBLINGS, 0, 0, 0, 0, hwndParent, 0, OS.GetModuleHandle(null), null);
        if (hwndHeader == 0)
            error(SWT.ERROR_NO_HANDLES);
        /* Old code, not sure if needed */
        ...
        long hFont = OS.SendMessage(getApi().handle, OS.WM_GETFONT, 0, 0);
        if (hFont != 0)
            OS.SendMessage(hwndHeader, OS.WM_SETFONT, hFont, 0);
        /* Copy Tree's tab-order to hwndParent */
        long hwndInsertAfter = OS.GetWindow(getApi().handle, OS.GW_HWNDPREV);
    ...
    void deregister() {
        super.deregister();
        if (hwndParent != 0)
            ((SwtDisplay) display.getImpl()).removeControl(hwndParent);
        if (hwndHeader != 0)
    ...
    public void deselectAll() {
        ...
        if ((getApi().style & SWT.SINGLE) != 0) {
            ...
            long oldProc = OS.GetWindowLongPtr(getApi().handle, OS.GWLP_WNDPROC);
            OS.SetWindowLongPtr(getApi().handle, OS.GWLP_WNDPROC, TreeProc);
            if ((getApi().style & SWT.VIRTUAL) != 0) {
                long hItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_ROOT, 0);
            ...
            OS.SetWindowLongPtr(getApi().handle, OS.GWLP_WNDPROC, oldProc);
        }
    }
    ...
    void destroyItem(TreeColumn column) {
        ...
        for (TreeItem item : items) {
            if (item != null) {
                if (columnCount == 0) {
                    ((SwtTreeItem) item.getImpl()).strings = null;
                    ((SwtTreeItem) item.getImpl()).images = null;
                    ((SwtTreeItem) item.getImpl()).cellBackground = null;
                    ((SwtTreeItem) item.getImpl()).cellForeground = null;
                    ((SwtTreeItem) item.getImpl()).cellFont = null;
                } else {
                    if (((SwtTreeItem) item.getImpl()).strings != null) {
                        String[] strings = ((SwtTreeItem) item.getImpl()).strings;
                        if (index == 0) {
                            ((SwtItem) item.getImpl()).text = strings[1] != null ? strings[1] : "";
                        }
                    ...
                    if (((SwtTreeItem) item.getImpl()).images != null) {
                        Image[] images = ((SwtTreeItem) item.getImpl()).images;
                        if (index == 0)
                            ((SwtItem) item.getImpl()).image = images[1];
	...
	*/
        ...
        if (columnCount != 0) {
            ...
            for (int i = orderIndex; i < newOrder.length; i++) {
                ...
                ((SwtTreeColumn) newColumns[i - orderIndex].getImpl()).updateToolTip(newOrder[i]);
            }
        ...
        if (headerToolTipHandle != 0) {
            TOOLINFO lpti = new TOOLINFO();
            lpti.cbSize = TOOLINFO.sizeof;
            lpti.uId = ((SwtTreeColumn) column.getImpl()).id;
            lpti.hwnd = hwndHeader;
	...
	*/
        ...
        if ((getApi().style & SWT.DOUBLE_BUFFERED) == 0) {
            if (getDrawing() && OS.IsWindowVisible(getApi().handle)) {
                RECT rect = new RECT();
                fixRedraw = !OS.TreeView_GetItemRect(getApi().handle, hItem, rect, false);
            }
        }
        if (fixRedraw) {
            hParent = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_PARENT, hItem);
	...
	*/
        long hwndToolTip = OS.SendMessage(getApi().handle, OS.TVM_GETTOOLTIPS, 0, 0);
        if (hwndToolTip != 0)
            OS.SendMessage(hwndToolTip, OS.TTM_POP, 0, 0);
        shrink = ignoreShrink = true;
        OS.SendMessage(getApi().handle, OS.TVM_DELETEITEM, 0, hItem);
        ignoreShrink = false;
        /*
	 ...
	 */
        ...
        if (fixRedraw) {
            OS.DefWindowProc(getApi().handle, OS.WM_SETREDRAW, 1, 0);
            OS.ValidateRect(getApi().handle, null);
            /*
		...
		*/
            if (OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_CHILD, hParent) == 0) {
                RECT rect = new RECT();
                if (OS.TreeView_GetItemRect(getApi().handle, hParent, rect, false)) {
                    OS.InvalidateRect(getApi().handle, rect, true);
                }
        ...
        if (count == 0) {
            if (imageList != null) {
                OS.SendMessage(getApi().handle, OS.TVM_SETIMAGELIST, 0, 0);
	...
	*/
    ...
    boolean findCell(int x, int y, TreeItem[] item, int[] index, RECT[] cellRect, RECT[] itemRect) {
        ...
        lpht.x = x;
        lpht.y = y;
        OS.SendMessage(getApi().handle, OS.TVM_HITTEST, 0, lpht);
        if (lpht.hItem != 0) {
            ...
            if (hwndParent != 0) {
                ...
                OS.MapWindowPoints(hwndParent, getApi().handle, rect, 2);
            } else {
                OS.GetClientRect(getApi().handle, rect);
            }
            ...
            while (index[0] < count && !quit) {
                long hFont = ((SwtTreeItem) item[0].getImpl()).fontHandle(order[index[0]]);
                if (hFont != -1)
                    hFont = OS.SelectObject(hDC, hFont);
                cellRect[0] = ((SwtTreeItem) item[0].getImpl()).getBounds(order[index[0]], true, false, true, false, true, hDC);
                if (cellRect[0].left > rect.right) {
                    ...
                    if (OS.PtInRect(cellRect[0], pt)) {
                        if (isCustomToolTip()) {
                            int state = (int) OS.SendMessage(getApi().handle, OS.TVM_GETITEMSTATE, lpht.hItem, OS.TVIS_SELECTED);
                            ...
                            itemRect[0] = ((SwtTreeItem) item[0].getImpl()).getBounds(order[index[0]], true, true, false, false, false, hDC);
                        }
    ...
    int findIndex(long hFirstItem, long hItem) {
        ...
        if (hFirstItem == cachedFirstItem) {
            ...
            if (cachedIndexItem == hItem)
                return cachedIndex;
            long hPrevItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_PREVIOUS, cachedIndexItem);
            if (hPrevItem == hItem) {
                cachedIndexItem = hPrevItem;
                return --cachedIndex;
            }
            long hNextItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_NEXT, cachedIndexItem);
            if (hNextItem == hItem) {
            ...
            while (hPrevItem != 0 && hPrevItem != hItem) {
                hPrevItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_PREVIOUS, hPrevItem);
                --previousIndex;
            }
            if (hPrevItem == hItem) {
                cachedIndexItem = hPrevItem;
            ...
            while (hNextItem != 0 && hNextItem != hItem) {
                hNextItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_NEXT, hNextItem);
                nextIndex++;
            }
            if (hNextItem == hItem) {
                cachedIndexItem = hNextItem;
    ...
    long findItem(long hFirstItem, int index) {
        ...
        if (hFirstItem == cachedFirstItem) {
            ...
            if (cachedIndex - 1 == index) {
                --cachedIndex;
                return cachedIndexItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_PREVIOUS, cachedIndexItem);
            }
            if (cachedIndex + 1 == index) {
                cachedIndex++;
                return cachedIndexItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_NEXT, cachedIndexItem);
            }
    ...
    Point getImageSize() {
        if (imageList != null)
            return Win32DPIUtils.pointToPixel(imageList.getImageSize(), getZoom());
        return new Point(0, getItemHeightInPixels());
    }

    long getBottomItem() {
        long hItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_FIRSTVISIBLE, 0);
        if (hItem == 0)
            return 0;
        int index = 0, count = (int) OS.SendMessage(getApi().handle, OS.TVM_GETVISIBLECOUNT, 0, 0);
        while (index <= count) {
            long hNextItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_NEXTVISIBLE, hItem);
            if (hNextItem == 0)
                return hItem;
            hItem = hNextItem;
            index++;
    ...
    public TreeItem getItem(int index) {
        ...
        if (index < 0)
            error(SWT.ERROR_INVALID_RANGE);
        long hFirstItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_ROOT, 0);
        if (hFirstItem == 0)
            error(SWT.ERROR_INVALID_RANGE);
	...
	*/
    ...
    int getItemCount(long hItem) {
        ...
        while (hFirstItem != 0) {
            hFirstItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_NEXT, hFirstItem);
            count++;
        }
        if (hItem == cachedFirstItem)
    ...
    int getItemHeightInPixels() {
        return (int) OS.SendMessage(getApi().handle, OS.TVM_GETITEMHEIGHT, 0, 0);
    }
	...
	*/
        while (tvItem.hItem != 0) {
            ...
            if (item != null)
                result[index++] = item;
            tvItem.hItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_NEXT, tvItem.hItem);
        }
        if (index != count) {
    ...
    int getSelection(long hItem, TVITEM tvItem, TreeItem[] selection, int index, int count, boolean bigSelection, boolean all) {
        while (hItem != 0) {
            ...
            if (bigSelection) {
                ...
                int state = (int) OS.SendMessage(getApi().handle, OS.TVM_GETITEMSTATE, hItem, OS.TVIS_SELECTED | OS.TVIS_EXPANDED);
                if ((state & OS.TVIS_SELECTED) != 0) {
    ...
    public TreeItem[] getSelection() {
        ...
        int count = 0;
        TreeItem[] guess = new TreeItem[(getApi().style & SWT.VIRTUAL) != 0 ? 8 : 1];
        long oldProc = OS.GetWindowLongPtr(getApi().handle, OS.GWLP_WNDPROC);
        OS.SetWindowLongPtr(getApi().handle, OS.GWLP_WNDPROC, TreeProc);
        if ((getApi().style & SWT.VIRTUAL) != 0) {
            TVITEM tvItem = new TVITEM();
        ...
        tvItem.mask = OS.TVIF_HANDLE | OS.TVIF_PARAM | OS.TVIF_STATE;
        long hItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_ROOT, 0);
        int itemCount = (int) OS.SendMessage(getApi().handle, OS.TVM_GETCOUNT, 0, 0);
    ...
    int getSortColumnPixel() {
        int pixel = OS.IsWindowEnabled(getApi().handle) || hasCustomBackground() ? getBackgroundPixel() : OS.GetSysColor(OS.COLOR_3DFACE);
    ...
    boolean hitTestSelection(long hItem, int x, int y) {
        ...
        if (newFont != 0)
            oldFont = OS.SelectObject(hDC, newFont);
        long hFont = ((SwtTreeItem) item.getImpl()).fontHandle(order[index[0]]);
        if (hFont != -1)
            hFont = OS.SelectObject(hDC, hFont);
        int state = (int) OS.SendMessage(getApi().handle, OS.TVM_GETITEMSTATE, hItem, OS.TVIS_SELECTED);
        int detail = (state & OS.TVIS_SELECTED) != 0 ? SWT.SELECTED : 0;
    ...
    void register() {
        super.register();
        if (hwndParent != 0)
            ((SwtDisplay) display.getImpl()).addControl(hwndParent, this.getApi());
        if (hwndHeader != 0)
	...
	*/
        customDraw = false;
        if (imageList != null) {
            OS.SendMessage(getApi().handle, OS.TVM_SETIMAGELIST, OS.TVSIL_NORMAL, 0);
            ((SwtDisplay) display.getImpl()).releaseImageList(imageList);
        }
        if (headerImageList != null) {
            if (hwndHeader != 0) {
        ...
        long hStateList = OS.SendMessage(getApi().handle, OS.TVM_GETIMAGELIST, OS.TVSIL_STATE, 0);
        OS.SendMessage(getApi().handle, OS.TVM_SETIMAGELIST, OS.TVSIL_STATE, 0);
        if (hStateList != 0)
    ...
    public void removeAll() {
        ...
        if (redraw)
            OS.DefWindowProc(getApi().handle, OS.WM_SETREDRAW, 0, 0);
        shrink = ignoreShrink = true;
        long result = OS.SendMessage(getApi().handle, OS.TVM_DELETEITEM, 0, OS.TVI_ROOT);
        ignoreShrink = false;
        if (redraw) {
            OS.DefWindowProc(getApi().handle, OS.WM_SETREDRAW, 1, 0);
            OS.InvalidateRect(getApi().handle, null, true);
    ...
    void reskinChildren(int flags) {
        if (items != null) {
            for (TreeItem item : items) {
                if (item != null)
                    ((SwtWidget) item.getImpl()).reskinChildren(flags);
            }
        }
        if (columns != null) {
            for (TreeColumn column : columns) {
    ...
    public void setInsertMark(TreeItem item, boolean before) {
        ...
        hInsert = hItem;
        insertAfter = !before;
        OS.SendMessage(getApi().handle, OS.TVM_SETINSERTMARK, insertAfter ? 1 : 0, hInsert);
    }
    ...
    void setItemCount(int count, long hParent) {
        ...
        long itemFirstChild = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_CHILD, hParent);
        ...
        long itemDeleteFrom = 0;
        {
            ...
            while (itemNext != 0 && itemCount < count) {
                itemPrev = itemNext;
                itemNext = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_NEXT, itemNext);
                itemCount++;
            }
        ...
        boolean redraw = false;
        if (OS.SendMessage(getApi().handle, OS.TVM_GETCOUNT, 0, 0) == 0) {
            redraw = getDrawing() && OS.IsWindowVisible(getApi().handle);
            if (redraw)
                OS.DefWindowProc(getApi().handle, OS.WM_SETREDRAW, 0, 0);
        }
        ...
        if (!redraw && (getApi().style & SWT.VIRTUAL) != 0) {
			 ...
			 */
                int state = (int) OS.SendMessage(getApi().handle, OS.TVM_GETITEMSTATE, hParent, OS.TVIS_EXPANDED);
                expanded = (state & OS.TVIS_EXPANDED) != 0;
            }
        }
        if (itemDeleteFrom != 0) {
            while (itemDeleteFrom != 0) {
                ...
                OS.SendMessage(getApi().handle, OS.TVM_GETITEM, 0, tvItem);
                itemDeleteFrom = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_NEXT, itemDeleteFrom);
    ...
    void setItemHeight(int itemHeight) {
        ...
        if (itemHeight < -1)
            error(SWT.ERROR_INVALID_ARGUMENT);
        OS.SendMessage(getApi().handle, OS.TVM_SETITEMHEIGHT, itemHeight, 0);
    }
    ...
    long scrolledHandle() {
        ...
        return columnCount == 0 && scrollWidth == 0 ? getApi().handle : hwndParent;
    }
    ...
    void select(long hItem, TVITEM tvItem) {
        while (hItem != 0) {
            tvItem.hItem = hItem;
            OS.SendMessage(getApi().handle, OS.TVM_SETITEM, 0, tvItem);
            int state = (int) OS.SendMessage(getApi().handle, OS.TVM_GETITEMSTATE, hItem, OS.TVIS_EXPANDED);
    ...
    public void select(TreeItem item) {
		...
		*/
            ...
            if ((bits & (OS.TVS_NOHSCROLL | OS.TVS_NOSCROLL)) == 0) {
                ...
                OS.GetScrollInfo(getApi().handle, OS.SB_HORZ, hInfo);
            }
            SCROLLINFO vInfo = new SCROLLINFO();
            vInfo.cbSize = SCROLLINFO.sizeof;
            vInfo.fMask = OS.SIF_ALL;
            OS.GetScrollInfo(getApi().handle, OS.SB_VERT, vInfo);
            boolean redraw = getDrawing() && OS.IsWindowVisible(getApi().handle);
            if (redraw) {
                OS.UpdateWindow(getApi().handle);
                OS.DefWindowProc(getApi().handle, OS.WM_SETREDRAW, 0, 0);
            ...
            if (hInfo != null) {
                ...
                OS.SendMessage(getApi().handle, OS.WM_HSCROLL, hThumb, 0);
            }
            /*
		...
		*/
            OS.SetScrollInfo(getApi().handle, OS.SB_VERT, vInfo, true);
            long vThumb = OS.MAKELPARAM(OS.SB_THUMBPOSITION, vInfo.nPos);
            OS.SendMessage(getApi().handle, OS.WM_VSCROLL, vThumb, 0);
            if (redraw) {
                OS.DefWindowProc(getApi().handle, OS.WM_SETREDRAW, 1, 0);
                OS.InvalidateRect(getApi().handle, null, true);
                if ((getApi().style & SWT.DOUBLE_BUFFERED) == 0) {
                    int oldStyle = getApi().style;
                    getApi().style |= SWT.DOUBLE_BUFFERED;
                    OS.UpdateWindow(getApi().handle);
                    getApi().style = oldStyle;
                }
            }
            return;
        }
    ...
    void setBackgroundImage(long hBitmap) {
		...
		* brush.
		*/
            if (OS.SendMessage(getApi().handle, OS.TVM_GETBKCOLOR, 0, 0) == -1) {
                OS.SendMessage(getApi().handle, OS.TVM_SETBKCOLOR, 0, -1);
            }
	...
	*/
        ...
        long hCursor = cursor == null ? OS.LoadCursor(0, OS.IDC_ARROW) : SwtCursor.win32_getHandle(cursor, DPIUtil.getZoomForAutoscaleProperty(getNativeZoom()));
            ...
            for (int i = 0; i < columnCount; i++) {
                ...
                if (!column.isDisposed()) {
                    ...
                    if (newRect.left != oldRects[i].left) {
                        ((SwtTreeColumn) column.getImpl()).updateToolTip(i);
    ...
    void setCheckboxImageList() {
        ...
        if (!OS.IsAppThemed())
            flags |= OS.ILC_MASK;
        int height = (int) OS.SendMessage(getApi().handle, OS.TVM_GETITEMHEIGHT, 0, 0), width = height;
	...
	*/
		 ...
		 */
            SIZE size = new SIZE();
            OS.GetThemePartSize(((SwtDisplay) display.getImpl()).hButtonTheme(getApi().nativeZoom), memDC, OS.BP_CHECKBOX, 0, null, OS.TS_TRUE, size);
        ...
        if (OS.IsAppThemed()) {
            long hTheme = ((SwtDisplay) display.getImpl()).hButtonTheme(getApi().nativeZoom);
        ...
        long hOldStateList = OS.SendMessage(getApi().handle, OS.TVM_GETIMAGELIST, OS.TVSIL_STATE, 0);
        OS.SendMessage(getApi().handle, OS.TVM_SETIMAGELIST, OS.TVSIL_STATE, hStateList);
        if (hOldStateList != 0)
	...
	*/
        ...
        OS.SendMessage(getApi().handle, OS.TVM_SETTEXTCOLOR, 0, pixel);
    }
	...
	*/
        ...
        if (willEnableDraw) {
            ...
            if (count == 0) {
                ...
                hItem = OS.SendMessage(getApi().handle, OS.TVM_INSERTITEM, 0, tvInsert);
            }
            OS.DefWindowProc(getApi().handle, OS.WM_SETREDRAW, 1, 0);
            updateScrollBar();
        }
    ...
    void setScrollWidth(int width) {
        ...
        ignoreResize = true;
        OS.SetWindowPos(getApi().handle, 0, pos.x - left, pos.y + pos.cy, w + left, h, OS.SWP_NOACTIVATE | OS.SWP_NOZORDER);
    ...
    public void setSelection(TreeItem[] items) {
        ...
        if (item != null) {
            if (item.isDisposed())
                error(SWT.ERROR_INVALID_ARGUMENT);
            long hOldItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_CARET, 0);
		...
		*/
            ...
            if (fixScroll) {
                OS.SendMessage(getApi().handle, OS.WM_SETREDRAW, 1, 0);
                OS.DefWindowProc(getApi().handle, OS.WM_SETREDRAW, 0, 0);
            }
            ignoreSelect = true;
            OS.SendMessage(getApi().handle, OS.TVM_SELECTITEM, OS.TVGN_CARET, hNewItem);
            ignoreSelect = false;
            if (OS.SendMessage(getApi().handle, OS.TVM_GETVISIBLECOUNT, 0, 0) == 0) {
                OS.SendMessage(getApi().handle, OS.TVM_SELECTITEM, OS.TVGN_FIRSTVISIBLE, hNewItem);
                long hParent = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_PARENT, hNewItem);
                if (hParent == 0)
                    OS.SendMessage(getApi().handle, OS.WM_HSCROLL, OS.SB_TOP, 0);
            }
            if (fixScroll) {
                OS.DefWindowProc(getApi().handle, OS.WM_SETREDRAW, 1, 0);
                OS.SendMessage(getApi().handle, OS.WM_SETREDRAW, 0, 0);
            }
            /*
    ...
    public void setSortColumn(TreeColumn column) {
        ...
        if (sortColumn != null && !sortColumn.isDisposed()) {
            ((SwtTreeColumn) sortColumn.getImpl()).setSortDirection(SWT.NONE);
        }
        sortColumn = column;
        if (sortColumn != null && sortDirection != SWT.NONE) {
            ((SwtTreeColumn) sortColumn.getImpl()).setSortDirection(sortDirection);
        }
    }
    ...
    public void setSortDirection(int direction) {
        ...
        if (sortColumn != null && !sortColumn.isDisposed()) {
            ((SwtTreeColumn) sortColumn.getImpl()).setSortDirection(direction);
        }
    }
    ...
    public void setTopItem(TreeItem item) {
        ...
        OS.SendMessage(getApi().handle, OS.TVM_SELECTITEM, OS.TVGN_FIRSTVISIBLE, hItem);
        if (hParent != 0) {
            if (hInfo != null) {
                long hThumb = OS.MAKELPARAM(OS.SB_THUMBPOSITION, hInfo.nPos);
                OS.SendMessage(getApi().handle, OS.WM_HSCROLL, hThumb, 0);
    ...
    private void calculateAndApplyIndentSize() {
        int indent = Win32DPIUtils.pointToPixel(DEFAULT_INDENT, getApi().nativeZoom);
        OS.SendMessage(getApi().handle, OS.TVM_SETINDENT, indent, 0);
    }
	...
	*/
        if (OS.SendMessage(getApi().handle, OS.TVM_GETVISIBLECOUNT, 0, 0) == 0) {
            ...
            boolean scroll = true;
            RECT itemRect = new RECT();
            if (OS.TreeView_GetItemRect(getApi().handle, hItem, itemRect, true)) {
                forceResize();
                RECT rect = new RECT();
                OS.GetClientRect(getApi().handle, rect);
                POINT pt = new POINT();
            ...
            if (scroll) {
                ...
                OS.SendMessage(getApi().handle, OS.TVM_ENSUREVISIBLE, 0, hItem);
                if (fixScroll) {
                    OS.DefWindowProc(getApi().handle, OS.WM_SETREDRAW, 1, 0);
                    OS.SendMessage(getApi().handle, OS.WM_SETREDRAW, 0, 0);
                }
    ...
    public void showColumn(TreeColumn column) {
		 ...
		 *  This is the same behaviour like in linux and on mac.*/
            ...
            if (scrollBecauseLeft || (headerRect.right - headerRect.left > rect.right - rect.left)) {
                ...
                info.cbSize = SCROLLINFO.sizeof;
                info.fMask = OS.SIF_POS;
                info.nPos = Math.max(0, headerRect.left - SwtTree.INSET / 2);
                OS.SetScrollInfo(hwndParent, OS.SB_HORZ, info, true);
                setScrollWidth();
                ...
                info.nPos = Math.max(0, wideHeader + headerRect.left - wideRect - SwtTree.INSET / 2);
                info.nPos = Math.min(rect.right - SwtTree.INSET / 2, info.nPos);
                OS.SetScrollInfo(hwndParent, OS.SB_HORZ, info, true);
                setScrollWidth();
            }
        }
    ...
    public void showSelection() {
        ...
        long hItem = 0;
        if ((getApi().style & SWT.SINGLE) != 0) {
            hItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_CARET, 0);
            if (hItem == 0)
                return;
            int state = (int) OS.SendMessage(getApi().handle, OS.TVM_GETITEMSTATE, hItem, OS.TVIS_SELECTED);
            if ((state & OS.TVIS_SELECTED) == 0)
            ...
            if ((getApi().style & SWT.VIRTUAL) != 0) {
                ...
                while (index < items.length) {
                    ...
                    if (item != null) {
                        int state = (int) OS.SendMessage(getApi().handle, OS.TVM_GETITEMSTATE, item.handle, OS.TVIS_SELECTED);
                        if ((state & OS.TVIS_SELECTED) != 0) {
                            hItem = item.handle;
                            break;
                        }
    ...
    void sort(long hParent, boolean all) {
        ...
        if (sortDirection == SWT.UP || sortDirection == SWT.NONE) {
            OS.SendMessage(getApi().handle, OS.TVM_SORTCHILDREN, all ? 1 : 0, hParent);
        } else {
            ...
            OS.SendMessage(getApi().handle, OS.TVM_SORTCHILDRENCB, all ? 1 : 0, psort);
    ...
    void subclass() {
        super.subclass();
        if (hwndHeader != 0) {
            OS.SetWindowLongPtr(hwndHeader, OS.GWLP_WNDPROC, ((SwtDisplay) display.getImpl()).windowProc);
        }
    }
    ...
    String toolTipText(NMTTDISPINFO hdr) {
        ...
        if (headerToolTipHandle == hdr.hwndFrom) {
            for (int i = 0; i < columnCount; i++) {
                ...
                if (((SwtTreeColumn) column.getImpl()).id == hdr.idFrom)
                    return ((SwtTreeColumn) column.getImpl()).toolTipText;
            }
        ...
        if (itemToolTipHandle == hdr.hwndFrom) {
            ...
            if (findCell(pt.x, pt.y, item, index, cellRect, itemRect)) {
                String text = null;
                if (index[0] == 0) {
                    text = ((SwtItem) item[0].getImpl()).text;
                } else {
                    String[] strings = ((SwtTreeItem) item[0].getImpl()).strings;
                    if (strings != null)
	...
	*/
    ...
    void updateMenuLocation(Event event) {
        ...
        if (focusItem != null) {
            Rectangle bounds = ((SwtTreeItem) focusItem.getImpl()).getBoundsInPixels(0);
            if (((SwtItem) focusItem.getImpl()).text != null && ((SwtItem) focusItem.getImpl()).text.length() != 0) {
                bounds = ((SwtTreeItem) focusItem.getImpl()).getBoundsInPixels();
            }
            ...
            if (headerImageList != null) {
                ...
                if (columns != null) {
                    for (int i = 0; i < columns.length; i++) {
                        TreeColumn column = columns[i];
                        if (column != null) {
                            Image image = ((SwtItem) column.getImpl()).image;
                            if (image != null) {
                                HDITEM hdItem = new HDITEM();
        ...
        if (hwndParent != 0) {
            if (columnCount != 0 || scrollWidth != 0) {
                ...
                if (itemCount == 0) {
                    ...
                    OS.GetScrollInfo(getApi().handle, OS.SB_VERT, info);
                    if (info.nPage == 0) {
                        SCROLLBARINFO psbi = new SCROLLBARINFO();
                        psbi.cbSize = SCROLLBARINFO.sizeof;
                        OS.GetScrollBarInfo(getApi().handle, OS.OBJID_VSCROLL, psbi);
    ...
    long windowProc(long hwnd, int msg, long wParam, long lParam) {
                ...
                case OS.WM_NOTIFY:
                    {
                        ...
                        switch(hdr.code) {
                            ...
                            case OS.TTN_GETDISPINFO:
                                return OS.SendMessage(getApi().handle, msg, wParam, lParam);
                        }
                        break;
                    }
                case OS.WM_SETCURSOR:
                    {
                        if (wParam == hwnd) {
                            ...
                            if (hitTest == OS.HTCLIENT) {
                                ...
                                if (0 <= index && index < columnCount && !((SwtTreeColumn) columns[index].getImpl()).resizable) {
        ...
        if (hwndParent != 0 && hwnd == hwndParent) {
            switch(msg) {
				...
				*/
                        ...
                        OS.SetScrollInfo(getApi().handle, OS.SB_VERT, info, true);
                        long code = OS.SendMessage(getApi().handle, OS.WM_VSCROLL, wParam, lParam);
                        OS.GetScrollInfo(getApi().handle, OS.SB_VERT, info);
                        OS.SetScrollInfo(hwndParent, OS.SB_VERT, info, true);
                        return code;
                    }
        ...
        if (msg == SwtDisplay.DI_GETDRAGIMAGE) {
            /*
		...
		* selected.
		*/
            if ((getApi().style & SWT.MULTI) != 0 || hooks(SWT.EraseItem) || hooks(SWT.PaintItem)) {
                long hItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_FIRSTVISIBLE, 0);
                ...
                OS.MapWindowPoints(0, getApi().handle, mousePos, 1);
                RECT clientRect = new RECT();
                ...
                for (int i = 1; i < count; i++) {
                    ...
                    if (rect.bottom > clientRect.bottom)
                        break;
                    RECT itemRect = ((SwtTreeItem) items[i].getImpl()).getBounds(0, true, true, false);
                    if ((getApi().style & SWT.FULL_SELECTION) != 0) {
            ...
            case ' ':
                {
                    ...
                    if (hItem != 0) {
                        ...
                        if ((getApi().style & SWT.CHECK) != 0) {
                            ...
                            OS.SendMessage(getApi().handle, OS.TVM_SETITEM, 0, tvItem);
                            long id = OS.SendMessage(getApi().handle, OS.TVM_MAPHTREEITEMTOACCID, hItem, 0);
                            OS.NotifyWinEvent(OS.EVENT_OBJECT_FOCUS, getApi().handle, OS.OBJID_CLIENT, (int) id);
                        }
                        tvItem.stateMask = OS.TVIS_SELECTED;
                        OS.SendMessage(getApi().handle, OS.TVM_GETITEM, 0, tvItem);
                        if ((getApi().style & SWT.MULTI) != 0 && OS.GetKeyState(OS.VK_CONTROL) < 0) {
                            if ((tvItem.state & OS.TVIS_SELECTED) != 0) {
                                tvItem.state &= ~OS.TVIS_SELECTED;
                            } else {
                                tvItem.state |= OS.TVIS_SELECTED;
	...
	* package.
	*/
        if ((getApi().style & SWT.CHECK) != 0 || hwndParent != 0) {
            if (accessible == null)
    ...
    LRESULT WM_HSCROLL(long wParam, long lParam) {
        ...
        if ((getApi().style & SWT.DOUBLE_BUFFERED) != 0) {
            fixScroll = (getApi().style & SWT.VIRTUAL) != 0 || hooks(SWT.EraseItem) || hooks(SWT.PaintItem);
        }
        if (fixScroll) {
            getApi().style &= ~SWT.DOUBLE_BUFFERED;
            if (explorerTheme) {
                OS.SendMessage(getApi().handle, OS.TVM_SETEXTENDEDSTYLE, OS.TVS_EX_DOUBLEBUFFER, 0);
            }
        }
        ...
        if (fixScroll) {
            getApi().style |= SWT.DOUBLE_BUFFERED;
            if (explorerTheme) {
                OS.SendMessage(getApi().handle, OS.TVM_SETEXTENDEDSTYLE, OS.TVS_EX_DOUBLEBUFFER, OS.TVS_EX_DOUBLEBUFFER);
            }
        }
        if (result != null)
            return result;
    ...
    LRESULT WM_KEYDOWN(long wParam, long lParam) {
        ...
        switch((int) wParam) {
			...
			* create the control.
			*/
                boolean isRTL = (getApi().style & SWT.RIGHT_TO_LEFT) != 0;
                if (isRTL != createdAsRTL) {
                    long code = callWindowProc(getApi().handle, OS.WM_KEYDOWN, wParam == OS.VK_RIGHT ? OS.VK_LEFT : OS.VK_RIGHT, lParam);
            ...
            case OS.VK_END:
                {
                    ...
                    if (OS.GetKeyState(OS.VK_SHIFT) < 0) {
                        ...
                        if (hItem != 0) {
                            ...
                            long hNewItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_CARET, 0);
                            TVITEM tvItem = new TVITEM();
                            tvItem.mask = OS.TVIF_HANDLE | OS.TVIF_STATE;
                            tvItem.stateMask = OS.TVIS_SELECTED;
                            long hDeselectItem = hItem;
                            RECT rect1 = new RECT();
                            if (!OS.TreeView_GetItemRect(getApi().handle, hAnchor, rect1, false)) {
                                hAnchor = hItem;
                                OS.TreeView_GetItemRect(getApi().handle, hAnchor, rect1, false);
                            }
                            RECT rect2 = new RECT();
                            OS.TreeView_GetItemRect(getApi().handle, hDeselectItem, rect2, false);
                            ...
                            while (hDeselectItem != hAnchor) {
                                ...
                                OS.SendMessage(getApi().handle, OS.TVM_SETITEM, 0, tvItem);
                                hDeselectItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, flags, hDeselectItem);
                            }
                            long hSelectItem = hAnchor;
                            OS.TreeView_GetItemRect(getApi().handle, hNewItem, rect1, false);
                            OS.TreeView_GetItemRect(getApi().handle, hSelectItem, rect2, false);
                            tvItem.state = OS.TVIS_SELECTED;
                            flags = rect1.top < rect2.top ? OS.TVGN_PREVIOUSVISIBLE : OS.TVGN_NEXTVISIBLE;
                            while (hSelectItem != hNewItem) {
                                tvItem.hItem = hSelectItem;
                                OS.SendMessage(getApi().handle, OS.TVM_SETITEM, 0, tvItem);
                                hSelectItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, flags, hSelectItem);
                            }
                            tvItem.hItem = hNewItem;
                            OS.SendMessage(getApi().handle, OS.TVM_SETITEM, 0, tvItem);
                            tvItem.mask = OS.TVIF_HANDLE | OS.TVIF_PARAM;
                    ...
                    if (OS.GetKeyState(OS.VK_CONTROL) < 0) {
                        ...
                        if (hItem != 0) {
                            ...
                            switch((int) wParam) {
                                case OS.VK_UP:
                                    hNewItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_PREVIOUSVISIBLE, hItem);
                                    break;
                                case OS.VK_DOWN:
                                    hNewItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_NEXTVISIBLE, hItem);
                                    break;
                                case OS.VK_HOME:
                                    hNewItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_ROOT, 0);
                                    break;
                                case OS.VK_PRIOR:
                                    hNewItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_FIRSTVISIBLE, 0);
                                    if (hNewItem == hItem) {
                                        OS.SendMessage(getApi().handle, OS.WM_VSCROLL, OS.SB_PAGEUP, 0);
                                        hNewItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_FIRSTVISIBLE, 0);
                                    }
                                    break;
                                case OS.VK_NEXT:
                                    ...
                                    hNewItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_FIRSTVISIBLE, 0);
                                    do {
                                        long hVisible = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_NEXTVISIBLE, hNewItem);
                                        if (hVisible == 0)
                                            break;
                                        if (!OS.TreeView_GetItemRect(getApi().handle, hVisible, rect, false))
                                            break;
                                        if (rect.bottom > clientRect.bottom)
                                            break;
                                        if ((hNewItem = hVisible) == hItem) {
                                            OS.SendMessage(getApi().handle, OS.WM_VSCROLL, OS.SB_PAGEDOWN, 0);
                                        }
                                    } while (hNewItem != 0);
                                    break;
                                case OS.VK_END:
                                    hNewItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_LASTVISIBLE, 0);
                                    break;
                            }
                            if (hNewItem != 0) {
                                OS.SendMessage(getApi().handle, OS.TVM_ENSUREVISIBLE, 0, hNewItem);
                                tvItem.hItem = hNewItem;
                                OS.SendMessage(getApi().handle, OS.TVM_GETITEM, 0, tvItem);
                                boolean newSelected = (tvItem.state & OS.TVIS_SELECTED) != 0;
                                boolean redraw = !newSelected && getDrawing() && OS.IsWindowVisible(getApi().handle);
                                if (redraw) {
                                    OS.UpdateWindow(getApi().handle);
                                    OS.DefWindowProc(getApi().handle, OS.WM_SETREDRAW, 0, 0);
                                }
                                ...
                                if (redraw) {
                                    ...
                                    OS.TreeView_GetItemRect(getApi().handle, hItem, rect1, false);
                                    OS.TreeView_GetItemRect(getApi().handle, hNewItem, rect2, false);
                                    OS.DefWindowProc(getApi().handle, OS.WM_SETREDRAW, 1, 0);
                                    OS.InvalidateRect(getApi().handle, rect1, true);
                                    OS.InvalidateRect(getApi().handle, rect2, true);
                                    OS.UpdateWindow(getApi().handle);
                                }
                                return LRESULT.ZERO;
                            }
                    ...
                    long code = callWindowProc(getApi().handle, OS.WM_KEYDOWN, wParam, lParam);
                    hAnchor = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_CARET, 0);
                    return new LRESULT(code);
                }
        }
        return result;
	...
	*/
        boolean redraw = (getApi().style & SWT.MULTI) != 0;
        ...
        if (lpht.hItem != 0) {
            if ((getApi().style & SWT.CHECK) != 0) {
                if ((lpht.flags & OS.TVHT_ONITEMSTATEICON) != 0) {
                    ...
                    ((SwtDisplay) display.getImpl()).captureChanged = false;
                    sendMouseEvent(SWT.MouseDown, 1, getApi().handle, lParam);
                    if (!sendMouseEvent(SWT.MouseDoubleClick, 1, getApi().handle, lParam)) {
                        if (!((SwtDisplay) display.getImpl()).captureChanged && !isDisposed()) {
                            if (OS.GetCapture() != getApi().handle)
                                OS.SetCapture(getApi().handle);
                        }
                        return LRESULT.ZERO;
                    }
                    if (!((SwtDisplay) display.getImpl()).captureChanged && !isDisposed()) {
                    ...
                    tvItem.state = state << 12;
                    OS.SendMessage(getApi().handle, OS.TVM_SETITEM, 0, tvItem);
                    long id = OS.SendMessage(getApi().handle, OS.TVM_MAPHTREEITEMTOACCID, tvItem.hItem, 0);
                    OS.NotifyWinEvent(OS.EVENT_OBJECT_FOCUS, getApi().handle, OS.OBJID_CLIENT, (int) id);
                    Event event = new Event();
	...
	*/
        ...
        if (lpht.hItem == 0 || (lpht.flags & OS.TVHT_ONITEMBUTTON) != 0) {
            Display display = this.display;
            ((SwtDisplay) display.getImpl()).captureChanged = false;
            if (!sendMouseEvent(SWT.MouseDown, 1, getApi().handle, lParam)) {
                if (!((SwtDisplay) display.getImpl()).captureChanged && !isDisposed()) {
                    if (OS.GetCapture() != getApi().handle)
                        OS.SetCapture(getApi().handle);
                }
            ...
            long hOldSelection = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_CARET, 0);
            if (lpht.hItem != 0 && (getApi().style & SWT.MULTI) != 0) {
                if (hOldSelection != 0) {
                    TVITEM tvItem = new TVITEM();
                    tvItem.mask = OS.TVIF_HANDLE | OS.TVIF_STATE;
                    tvItem.hItem = lpht.hItem;
                    ...
                    if ((tvItem.state & OS.TVIS_EXPANDED) != 0) {
                        fixSelection = true;
                        tvItem.stateMask = OS.TVIS_SELECTED;
                        long hNext = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_NEXTVISIBLE, lpht.hItem);
                        while (hNext != 0) {
                            if (hNext == hAnchor)
                                hAnchor = 0;
                            tvItem.hItem = hNext;
                            ...
                            tvItem.state = 0;
                            OS.SendMessage(getApi().handle, OS.TVM_SETITEM, 0, tvItem);
                            long hItem = hNext = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_NEXTVISIBLE, hNext);
                            while (hItem != 0 && hItem != lpht.hItem) {
                                hItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_PARENT, hItem);
                            }
                            if (hItem == 0)
                                break;
                        }
            ...
            long code = callWindowProc(getApi().handle, OS.WM_LBUTTONDOWN, wParam, lParam);
            /* Bug 225404 */
            ...
            long hNewSelection = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_CARET, 0);
	...
	*/
        ...
        if (fakeSelection) {
            if (hOldItem == 0 || (hNewItem == hOldItem && lpht.hItem != hOldItem)) {
                OS.SendMessage(getApi().handle, OS.TVM_SELECTITEM, OS.TVGN_CARET, lpht.hItem);
                hNewItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_CARET, 0);
            }
            if (!dragStarted && (getApi().state & DRAG_DETECT) != 0 && hooks(SWT.DragDetect)) {
                dragStarted = dragDetect(getApi().handle, lpht.x, lpht.y, false, null, null);
            }
        }
        ignoreDeselect = ignoreSelect = false;
        hSelect = 0;
	...
	*/
        ...
        if ((getApi().style & SWT.MULTI) != 0) {
            ...
            if (hittestSelected || (wParam & OS.MK_CONTROL) != 0) {
                ...
                RECT rect1 = new RECT(), rect2 = new RECT();
                OS.TreeView_GetItemRect(getApi().handle, hOldItem, rect1, false);
                OS.TreeView_GetItemRect(getApi().handle, hNewItem, rect2, false);
                OS.DefWindowProc(getApi().handle, OS.WM_SETREDRAW, 1, 0);
                OS.InvalidateRect(getApi().handle, rect1, true);
                OS.InvalidateRect(getApi().handle, rect2, true);
            ...
            if ((wParam & OS.MK_CONTROL) == 0) {
                if (!hittestSelected || !dragStarted) {
                    ...
                    if ((wParam & OS.MK_SHIFT) != 0) {
                        ...
                        if (OS.TreeView_GetItemRect(getApi().handle, hAnchor, rect1, false)) {
                            RECT rect2 = new RECT();
                            if (OS.TreeView_GetItemRect(getApi().handle, hNewItem, rect2, false)) {
                                ...
                                while (hItem != hNewItem) {
                                    tvItem.hItem = hItem;
                                    OS.SendMessage(getApi().handle, OS.TVM_SETITEM, 0, tvItem);
                                    hItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, flags, hItem);
                                }
                            }
                        }
                    }
	...
	*/
        if (dragStarted) {
            ...
            if ((bits & OS.TVS_DISABLEDRAGDROP) == 0) {
                sendMouseEvent(SWT.MouseUp, 1, getApi().handle, lParam);
            }
        }
        dragStarted = false;
        return new LRESULT(code);
	...
	*/
        Display display = this.display;
        ((SwtDisplay) display.getImpl()).captureChanged = false;
        if (!sendMouseEvent(SWT.MouseDown, 3, getApi().handle, lParam)) {
            if (!((SwtDisplay) display.getImpl()).captureChanged && !isDisposed()) {
                if (OS.GetCapture() != getApi().handle)
                    OS.SetCapture(getApi().handle);
            }
	...
	*/
        ...
        OS.SendMessage(getApi().handle, OS.TVM_HITTEST, 0, lpht);
        if (lpht.hItem != 0) {
            boolean fakeSelection = (getApi().style & SWT.FULL_SELECTION) != 0;
            if (!fakeSelection) {
                if (hooks(SWT.MeasureItem)) {
            ...
            if (fakeSelection) {
                if ((wParam & (OS.MK_CONTROL | OS.MK_SHIFT)) == 0) {
                    ...
                    if ((tvItem.state & OS.TVIS_SELECTED) == 0) {
                        ignoreSelect = true;
                        OS.SendMessage(getApi().handle, OS.TVM_SELECTITEM, OS.TVGN_CARET, 0);
                        ignoreSelect = false;
                        OS.SendMessage(getApi().handle, OS.TVM_SELECTITEM, OS.TVGN_CARET, lpht.hItem);
                    }
                }
    ...
    LRESULT WM_PAINT(long wParam, long lParam) {
        ...
        if ((getApi().style & SWT.DOUBLE_BUFFERED) != 0 || findImageControl() != null) {
            ...
            if (doubleBuffer) {
                ...
                if (hooksPaint) {
                    GCData data = new GCData();
                    data.ps = ps;
                    data.hwnd = getApi().handle;
                    gc = SwtGC.win32_new(this.getApi(), data);
                    paintDC = gc.handle;
                } else {
                    paintDC = OS.BeginPaint(getApi().handle, ps);
                }
                ...
                if (width != 0 && height != 0) {
                    ...
                    drawBackground(hDC, rect);
                    callWindowProc(getApi().handle, OS.WM_PAINT, hDC, 0);
	...
	*/
        long code = OS.DefWindowProc(getApi().handle, OS.WM_SETREDRAW, wParam, lParam);
	...
	*/
        if (explorerTheme && (getApi().style & SWT.FULL_SELECTION) != 0) {
            OS.InvalidateRect(getApi().handle, null, false);
        }
        if (ignoreResize)
            return null;
	...
	*/
	 ...
	 */
        long bits = OS.SendMessage(getApi().handle, OS.TVM_GETEXTENDEDSTYLE, 0, 0);
        if ((bits & OS.TVS_EX_FADEINOUTEXPANDOS) != 0) {
            if (!OS.IsWindowVisible(getApi().handle)) {
                ...
                if (lastTimerCount >= TIMER_MAX_COUNT) {
                    OS.CallWindowProc(TreeProc, getApi().handle, OS.WM_MOUSEMOVE, 0, 0);
                    lastTimerID = -1;
                    lastTimerCount = 0;
                }
            } else {
	...
	*/
    ...
    LRESULT wmNotifyChild(NMHDR hdr, long wParam, long lParam) {
        switch(hdr.code) {
				...
				*/
                        if (!ignoreShrink) {
                            if (items != null && lptvdi.lParam != -1) {
                                if (items[(int) lptvdi.lParam] != null && ((SwtTreeItem) items[(int) lptvdi.lParam].getImpl()).cached) {
                                    checkVisible = false;
                                }
                            }
                        }
                        if (checkVisible) {
                            if (!getDrawing() || !OS.IsWindowVisible(getApi().handle))
                                break;
                            RECT itemRect = new RECT();
                            if (!OS.TreeView_GetItemRect(getApi().handle, lptvdi.hItem, itemRect, false)) {
                                break;
                            }
                            RECT rect = new RECT();
                            OS.GetClientRect(getApi().handle, rect);
			...
			*/
                    ...
                    if ((lptvdi.mask & (OS.TVIF_IMAGE | OS.TVIF_SELECTEDIMAGE)) != 0) {
                        ...
                        if (explorerTheme && OS.IsWindowEnabled(getApi().handle)) {
                            if (findImageControl() != null) {
                                lptvdi.iImage = lptvdi.iSelectedImage = OS.I_IMAGENONE;
                            }
                        }
		...
		*/
            ...
            case OS.TVN_ITEMEXPANDING:
                {
                    ...
                    if (hooks(SWT.EraseItem) || hooks(SWT.PaintItem))
                        getApi().style &= ~SWT.DOUBLE_BUFFERED;
                    if (findImageControl() != null && getDrawing() && OS.IsWindowVisible(getApi().handle)) {
                        OS.DefWindowProc(getApi().handle, OS.WM_SETREDRAW, 0, 0);
                    }
                    /*
			...
			*/
                    if (hInsert != 0) {
                        OS.SendMessage(getApi().handle, OS.TVM_SETINSERTMARK, 0, 0);
                    }
                    if (!ignoreExpand) {
                        NMTREEVIEW treeView = new NMTREEVIEW();
                        OS.MoveMemory(treeView, lParam, NMTREEVIEW.sizeof);
				...
				* code in this method.
				*/
                        long hFirstItem = OS.SendMessage(getApi().handle, OS.TVM_GETNEXTITEM, OS.TVGN_CHILD, tvItem.hItem);
			...
			*/
                    if (imageList != null) {
                        ...
                        if (tvItem.hItem != 0) {
                            ...
                            if ((bits & OS.TVS_FULLROWSELECT) == 0) {
                                RECT rect = new RECT();
                                if (OS.TreeView_GetItemRect(getApi().handle, tvItem.hItem, rect, false)) {
                                    OS.InvalidateRect(getApi().handle, rect, true);
                                }
                            }
                        }
            ...
            case OS.TVN_BEGINRDRAG:
                {
                    ...
                    if (tvItem.hItem != 0 && (tvItem.state & OS.TVIS_SELECTED) == 0) {
                        ...
                        ignoreSelect = ignoreDeselect = true;
                        OS.SendMessage(getApi().handle, OS.TVM_SELECTITEM, OS.TVGN_CARET, tvItem.hItem);
                        ignoreSelect = ignoreDeselect = false;
                        hSelect = 0;
                    }
                    break;
	...
	*/
            ...
            case OS.NM_CUSTOMDRAW:
                {
					 ...
					 */
                                ...
                                // if we got here, we will paint everything ourself
                        ...
                        case OS.CDDS_POSTPAINT:
						   ...
						   The method getSlightlyDifferentColor gives us a color, that is near enough to the windows algorithm. */
                                    ...
                                    if (((SwtItem) columns[i].getImpl()).image != null) {
                                        GCData data = new GCData();
                                        data.device = display;
                                        GC gc = createNewGC(nmcd.hdc, data);
                                        int zoom = getZoom();
                                    ...
                                    if (((SwtItem) columns[i].getImpl()).text != null) {
                                        ...
                                        if ((columns[i].style & SWT.RIGHT) != 0)
                                            flags |= OS.DT_RIGHT;
                                        char[] buffer = ((SwtItem) columns[i].getImpl()).text.toCharArray();
            ...
            case OS.HDN_ENDDRAG:
                {
                    ...
                    if (phdn.iItem != -1 && phdn.pitem != 0) {
                        ...
                        if ((pitem.mask & OS.HDI_ORDER) != 0 && pitem.iOrder != -1) {
                            int[] oldOrder = getColumnOrder();
                            // dnd may have changed item order
                            cachedItemOrder = null;
                            ...
                            for (int i = start; i <= end; i++) {
                                ...
                                if (!column.isDisposed()) {
                                    ((SwtWidget) column.getImpl()).postEvent(SWT.Move);
                                }
                            }
                        }
                    }
                    ...
                    if (phdn.pitem != 0) {
                        ...
                        if ((newItem.mask & OS.HDI_WIDTH) != 0) {
                            ...
                            if (explorerTheme || (findImageControl() != null || hooks(SWT.MeasureItem) || hooks(SWT.EraseItem) || hooks(SWT.PaintItem))) {
                                ...
                                OS.ScrollWindowEx(getApi().handle, deltaX, 0, rect, null, 0, null, flags);
                            }
                    ...
                    if (phdn.pitem != 0) {
                        ...
                        if ((pitem.mask & OS.HDI_WIDTH) != 0) {
                            ...
                            if (!ignoreColumnResize) {
                                TreeColumn column = columns[phdn.iItem];
                                if (column != null) {
                                    ((SwtTreeColumn) column.getImpl()).updateToolTip(phdn.iItem);
                                    ...
                                    for (int i = 0; i < columnCount; i++) {
                                        ...
                                        if (moved && !nextColumn.isDisposed()) {
                                            ((SwtTreeColumn) nextColumn.getImpl()).updateToolTip(order[i]);
                                            nextColumn.getImpl().sendEvent(SWT.Move);
                                        }
            ...
            case OS.HDN_ITEMCLICK:
                {
                    ...
                    TreeColumn column = columns[phdn.iItem];
                    if (column != null) {
                        ((SwtWidget) column.getImpl()).sendSelectionEvent(SWT.Selection);
                    }
                    break;
                }
            case OS.HDN_ITEMDBLCLICK:
                {
                    ...
                    TreeColumn column = columns[phdn.iItem];
                    if (column != null) {
                        ((SwtWidget) column.getImpl()).sendSelectionEvent(SWT.DefaultSelection);
                    }
                    break;
                }
        }
            ...
            case OS.CDDS_POSTPAINT:
                {
                    if (OS.SendMessage(itemToolTipHandle, OS.TTM_GETCURRENTTOOL, 0, 0) != 0) {
                        ...
                        if (OS.SendMessage(itemToolTipHandle, OS.TTM_GETCURRENTTOOL, 0, lpti) != 0) {
                            ...
                            if (findCell(pt.x, pt.y, item, index, cellRect, itemRect)) {
                                long hDC = OS.GetDC(getApi().handle);
                                long hFont = ((SwtTreeItem) item[0].getImpl()).fontHandle(index[0]);
                                if (hFont == -1)
                                    hFont = OS.SendMessage(getApi().handle, OS.WM_GETFONT, 0, 0);
                                long oldFont = OS.SelectObject(hDC, hFont);
                                boolean drawForeground = true;
                                cellRect[0] = ((SwtTreeItem) item[0].getImpl()).getBounds(index[0], true, true, false, false, false, hDC);
                                if (hooks(SWT.EraseItem)) {
                                ...
                                if (drawForeground) {
                                    ...
                                    if (image != null || index[0] == 0) {
                                        ...
                                        RECT imageRect = ((SwtTreeItem) item[0].getImpl()).getBounds(index[0], false, true, false, false, false, hDC);
                                        if (imageList == null)
                                ...
                                if (hooks(SWT.PaintItem)) {
                                    itemRect[0] = ((SwtTreeItem) item[0].getImpl()).getBounds(index[0], true, true, false, false, false, hDC);
    ...
    private static void handleDPIChange(Widget widget, int newZoom, float scalingFactor) {
        ...
        // Reset ImageList
        if (((SwtTree) tree.getImpl()).headerImageList != null) {
            ((SwtDisplay) display.getImpl()).releaseImageList(((SwtTree) tree.getImpl()).headerImageList);
            ((SwtTree) tree.getImpl()).headerImageList = null;
        }
        if (((SwtTree) tree.getImpl()).imageList != null) {
            ((SwtDisplay) display.getImpl()).releaseImageList(((SwtTree) tree.getImpl()).imageList);
            ...
            ((SwtTree) tree.getImpl()).imageList = null;
        }
        ...
        ((SwtTree) tree.getImpl()).setItemHeight(-1);
        ...
        ((SwtTree) tree.getImpl()).calculateAndApplyIndentSize();
        ((SwtTree) tree.getImpl()).updateOrientation();
