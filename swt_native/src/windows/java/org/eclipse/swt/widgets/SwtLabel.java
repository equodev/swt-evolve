...
public class SwtLabel extends SwtControl implements ILabel {
	...
	*/
    ...
    int resolveTextDirection() {
        return (getApi().style & SWT.SEPARATOR) != 0 ? SWT.NONE : BidiUtil.resolveTextDirection(text);
    }
	...
	*/
    ...
    void wmDrawChildSeparator(DRAWITEMSTRUCT struct) {
        if ((getApi().style & SWT.SHADOW_NONE) != 0)
            return;
        RECT rect = new RECT();
        int lineWidth = getSystemMetrics(OS.SM_CXBORDER);
        int flags = (getApi().style & SWT.SHADOW_IN) != 0 ? OS.EDGE_SUNKEN : OS.EDGE_ETCHED;
        if ((getApi().style & SWT.HORIZONTAL) != 0) {
    ...
    void wmDrawChildText(DRAWITEMSTRUCT struct) {
        ...
        long uiState = OS.SendMessage(getApi().handle, OS.WM_QUERYUISTATE, 0, 0);
        if ((uiState & OS.UISF_HIDEACCEL) != 0)
            flags |= OS.DT_HIDEPREFIX;
        if (!getEnabled()) {
            int foregroundPixel = OS.GetSysColor(OS.COLOR_GRAYTEXT);
            if (((SwtDisplay) display.getImpl()).disabledLabelForegroundPixel != -1)
                foregroundPixel = ((SwtDisplay) display.getImpl()).disabledLabelForegroundPixel;
