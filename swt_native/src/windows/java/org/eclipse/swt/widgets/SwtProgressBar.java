 ...
 */
public class SwtProgressBar extends SwtControl implements IProgressBar {
    ...
    void createHandle() {
        super.createHandle();
        if (((SwtDisplay) display.getImpl()).progressbarUseColors) {
            char[] noTheme = new char[] { 0 };
            OS.SetWindowTheme(getApi().handle, noTheme, noTheme);
        }
        startTimer();
    }
    ...
    public int getMaximum() {
        checkWidget();
        return (int) OS.SendMessage(getApi().handle, OS.PBM_GETRANGE, 0, 0);
    }
    ...
    public int getMinimum() {
        checkWidget();
        return (int) OS.SendMessage(getApi().handle, OS.PBM_GETRANGE, 1, 0);
    }
    ...
    public int getSelection() {
        checkWidget();
        return (int) OS.SendMessage(getApi().handle, OS.PBM_GETPOS, 0, 0);
    }
    ...
    public int getState() {
        checkWidget();
        int state = (int) OS.SendMessage(getApi().handle, OS.PBM_GETSTATE, 0, 0);
        switch(state) {
            case OS.PBST_NORMAL:
                return SWT.NORMAL;
            case OS.PBST_ERROR:
    ...
    void startTimer() {
        if ((getApi().style & SWT.INDETERMINATE) != 0) {
            ...
            if ((bits & OS.PBS_MARQUEE) == 0) {
                OS.SetTimer(getApi().handle, TIMER_ID, DELAY, 0);
            } else {
                OS.SendMessage(getApi().handle, OS.PBM_SETMARQUEE, 1, DELAY);
            }
        }
    }
    ...
    void stopTimer() {
        if ((getApi().style & SWT.INDETERMINATE) != 0) {
            int bits = OS.GetWindowLong(getApi().handle, OS.GWL_STYLE);
            if ((bits & OS.PBS_MARQUEE) == 0) {
                OS.KillTimer(getApi().handle, TIMER_ID);
            } else {
                OS.SendMessage(getApi().handle, OS.PBM_SETMARQUEE, 0, 0);
            }
        }
    }
    ...
    void setBackgroundPixel(int pixel) {
        ...
        OS.SendMessage(getApi().handle, OS.PBM_SETBKCOLOR, 0, pixel);
    }
    ...
    void setForegroundPixel(int pixel) {
        if (pixel == -1)
            pixel = OS.CLR_DEFAULT;
        OS.SendMessage(getApi().handle, OS.PBM_SETBARCOLOR, 0, pixel);
    }
    ...
    public void setMaximum(int value) {
        checkWidget();
        int minimum = (int) OS.SendMessage(getApi().handle, OS.PBM_GETRANGE, 1, 0);
        if (0 <= minimum && minimum < value) {
            OS.SendMessage(getApi().handle, OS.PBM_SETRANGE32, minimum, value);
        }
    }
    ...
    public void setMinimum(int value) {
        checkWidget();
        int maximum = (int) OS.SendMessage(getApi().handle, OS.PBM_GETRANGE, 0, 0);
        if (0 <= value && value < maximum) {
            OS.SendMessage(getApi().handle, OS.PBM_SETRANGE32, value, maximum);
        }
    }
    ...
    public void setSelection(int value) {
        checkWidget();
        OS.SendMessage(getApi().handle, OS.PBM_SETPOS, value, 0);
        /*
	...
	* a second time.
	*/
        long state = OS.SendMessage(getApi().handle, OS.PBM_GETSTATE, 0, 0);
    ...
    public void setState(int state) {
        ...
        switch(state) {
            case SWT.NORMAL:
                OS.SendMessage(getApi().handle, OS.PBM_SETSTATE, OS.PBST_NORMAL, 0);
                break;
            case SWT.ERROR:
                OS.SendMessage(getApi().handle, OS.PBM_SETSTATE, OS.PBST_ERROR, 0);
                break;
            case SWT.PAUSED:
                OS.SendMessage(getApi().handle, OS.PBM_SETSTATE, OS.PBST_PAUSED, 0);
                break;
        }
    }
	...
	*/
    ...
    LRESULT WM_TIMER(long wParam, long lParam) {
        ...
        if ((getApi().style & SWT.INDETERMINATE) != 0) {
            ...
            if ((bits & OS.PBS_MARQUEE) == 0) {
                if (wParam == TIMER_ID) {
                    OS.SendMessage(getApi().handle, OS.PBM_STEPIT, 0, 0);
                }
            }
        }
        return result;
