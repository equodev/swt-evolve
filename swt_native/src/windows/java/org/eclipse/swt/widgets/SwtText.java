...
public class SwtText extends SwtScrollable implements IText {
            ...
            case OS.WM_PAINT:
                {
                    ...
                    if ((getApi().style & SWT.SINGLE) != 0 && (getApi().style & SWT.READ_ONLY) != 0 && message.length() > 0) {
                        drawMessage = hwnd != OS.GetFocus() && OS.GetWindowTextLength(getApi().handle) == 0;
                    }
					...
					*/
                            if (drawMessage) {
                                ...
                                long margins = OS.SendMessage(getApi().handle, OS.EM_GETMARGINS, 0, 0);
                                ...
                                boolean rtl = (getApi().style & SWT.RIGHT_TO_LEFT) != 0;
                                if (rtl)
                                    uFormat |= OS.DT_RTLREADING;
                                int alignment = getApi().style & (SWT.LEFT | SWT.CENTER | SWT.RIGHT);
                                switch(alignment) {
                                    case SWT.LEFT:
                ...
                case OS.WM_MOUSEMOVE:
                    {
                        ...
                        long hwndIcon = OS.ChildWindowFromPointEx(getApi().handle, pt, OS.CWP_SKIPINVISIBLE);
                        if (hwndIcon == getApi().handle)
                            hwndIcon = 0;
                ...
                case OS.WM_LBUTTONUP:
                    {
                        if (hwndActiveIcon != 0) {
                            Event e = new Event();
                            if (hwndActiveIcon == OS.GetDlgItem(getApi().handle, SWT.ICON_SEARCH)) {
    ...
    void createHandle() {
		 ...
		 */
            ...
            super.createHandle();
            getApi().style |= SWT.BORDER;
            editStyle = OS.GetWindowLongPtr(getApi().handle, OS.GWL_STYLE);
            editStyle |= OS.WS_BORDER;
            OS.SetWindowLongPtr(getApi().handle, OS.GWL_STYLE, editStyle);
            OS.SetWindowPos(getApi().handle, 0, 0, 0, 0, 0, OS.SWP_NOMOVE | OS.SWP_NOSIZE | OS.SWP_NOZORDER | OS.SWP_FRAMECHANGED);
        }
        OS.SendMessage(getApi().handle, OS.EM_LIMITTEXT, 0, 0);
        ...
        if ((getApi().style & SWT.SEARCH) != 0) {
            if ((getApi().style & SWT.ICON_SEARCH) != 0) {
                long hwndSearch = OS.CreateWindowEx(0, SwtLabel.LabelClass, null, OS.WS_CHILD | OS.WS_VISIBLE | OS.WS_CLIPSIBLINGS | OS.SS_OWNERDRAW, 0, 0, 0, 0, getApi().handle, SWT.ICON_SEARCH, OS.GetModuleHandle(null), null);
                if (hwndSearch == 0)
                    error(SWT.ERROR_NO_HANDLES);
            }
            if ((getApi().style & SWT.ICON_CANCEL) != 0) {
                getApi().state |= TRACK_MOUSE;
                long hwndCancel = OS.CreateWindowEx(0, SwtLabel.LabelClass, null, OS.WS_CHILD | OS.WS_CLIPSIBLINGS | OS.SS_OWNERDRAW, 0, 0, 0, 0, getApi().handle, SWT.ICON_CANCEL, OS.GetModuleHandle(null), null);
                if (hwndCancel == 0)
                    error(SWT.ERROR_NO_HANDLES);
            }
        }
    ...
    public void append(String string) {
        ...
        OS.SendMessage(getApi().handle, OS.EM_SETSEL, length, length);
        clearSegments(true);
	...
	*/
        ...
        OS.SendMessage(getApi().handle, OS.EM_REPLACESEL, 0, buffer);
        ignoreCharacter = false;
        OS.SendMessage(getApi().handle, OS.EM_SCROLLCARET, 0, 0);
	 ...
	 */
        ...
        int limit = (int) OS.SendMessage(getApi().handle, OS.EM_GETLIMITTEXT, 0, 0) & 0x7fffffff;
        OS.SendMessage(getApi().handle, OS.EM_SETLIMITTEXT, limit + Math.min(nSegments, Text.LIMIT - limit), 0);
        length += nSegments;
        ...
        OS.SendMessage(getApi().handle, OS.EM_GETSEL, start, end);
	 ...
	 */
        newChars[length] = 0;
        OS.SendMessage(getApi().handle, OS.EM_SETSEL, 0, -1);
        long undo = OS.SendMessage(getApi().handle, OS.EM_CANUNDO, 0, 0);
        OS.SendMessage(getApi().handle, OS.EM_REPLACESEL, undo, newChars);
        /* Restore selection */
        start[0] = translateOffset(start[0]);
        end[0] = translateOffset(end[0]);
        OS.SendMessage(getApi().handle, OS.EM_SETSEL, start[0], end[0]);
    ...
    void clearSegments(boolean applyText) {
        ...
        int limit = (int) OS.SendMessage(getApi().handle, OS.EM_GETLIMITTEXT, 0, 0) & 0x7fffffff;
        if (limit < Text.LIMIT) {
            OS.SendMessage(getApi().handle, OS.EM_SETLIMITTEXT, Math.max(1, limit - nSegments), 0);
        }
        if (!applyText) {
            segments = null;
            return;
	 ...
	 */
        OS.SendMessage(getApi().handle, OS.EM_SETSEL, 0, -1);
        long undo = OS.SendMessage(getApi().handle, OS.EM_CANUNDO, 0, 0);
        OS.SendMessage(getApi().handle, OS.EM_REPLACESEL, undo, buffer);
        OS.SendMessage(getApi().handle, OS.EM_SETSEL, start[0], end[0]);
        ignoreCharacter = oldIgnoreCharacter;
        ignoreModify = oldIgnoreModify;
        ignoreVerify = oldIgnoreVerify;
    ...
    public void clearSelection() {
        checkWidget();
        OS.SendMessage(getApi().handle, OS.EM_SETSEL, -1, 0);
    }
    ...
    Point computeSizeInPixels(int wHint, int hHint, boolean changed) {
        ...
        if (wHint == SWT.DEFAULT || hHint == SWT.DEFAULT) {
            ...
            int count = (getApi().style & SWT.SINGLE) != 0 ? 1 : (int) OS.SendMessage(getApi().handle, OS.EM_GETLINECOUNT, 0, 0);
	...
	*/
    ...
    public void copy() {
        checkWidget();
        OS.SendMessage(getApi().handle, OS.WM_COPY, 0, 0);
    }
    ...
    public void cut() {
        ...
        if ((getApi().style & SWT.READ_ONLY) != 0)
            return;
        OS.SendMessage(getApi().handle, OS.WM_CUT, 0, 0);
    }

    @Override
    int defaultBackground() {
        int bits = OS.GetWindowLong(getApi().handle, OS.GWL_STYLE);
        if ((bits & OS.ES_READONLY) != 0 || !OS.IsWindowEnabled(getApi().handle)) {
    ...
    boolean dragDetect(long hwnd, int x, int y, boolean filter, boolean[] detect, boolean[] consume) {
        if (filter) {
            ...
            if (start[0] != end[0]) {
                ...
                int position = OS.LOWORD(OS.SendMessage(getApi().handle, OS.EM_CHARFROMPOS, 0, lParam));
	...
	*/
        if ((getApi().style & SWT.MIRRORED) != 0)
            return;
        int bits1 = OS.GetWindowLong(getApi().handle, OS.GWL_EXSTYLE);
        int bits2 = OS.GetWindowLong(getApi().handle, OS.GWL_STYLE);
        ...
        OS.SetWindowLong(getApi().handle, OS.GWL_EXSTYLE, bits1);
        OS.SetWindowLong(getApi().handle, OS.GWL_STYLE, bits2);
    }
    ...
    public int getCaretLineNumber() {
        checkWidget();
        return (int) OS.SendMessage(getApi().handle, OS.EM_LINEFROMCHAR, -1, 0);
    }
	...
	*/
        ...
        long caretPos = OS.SendMessage(getApi().handle, OS.EM_POSFROMCHAR, position, 0);
        if (caretPos == -1) {
            caretPos = 0;
            if (position >= OS.GetWindowTextLength(getApi().handle)) {
                int[] start = new int[1], end = new int[1];
                OS.SendMessage(getApi().handle, OS.EM_GETSEL, start, end);
                OS.SendMessage(getApi().handle, OS.EM_SETSEL, position, position);
                /*
			* Feature in Windows.  When an edit control with ES_MULTILINE
			* style that does not have the WS_VSCROLL style is full (i.e.
			* there is no space at the end to draw any more characters),
			...
			*/
                ...
                OS.SendMessage(getApi().handle, OS.EM_REPLACESEL, 0, new char[] { ' ', '\0' });
                caretPos = OS.SendMessage(getApi().handle, OS.EM_POSFROMCHAR, position, 0);
                OS.SendMessage(getApi().handle, OS.EM_SETSEL, position, position + 1);
                OS.SendMessage(getApi().handle, OS.EM_REPLACESEL, 0, new char[] { '\0' });
                ignoreCharacter = ignoreModify = false;
                OS.SendMessage(getApi().handle, OS.EM_SETSEL, start[0], start[0]);
                OS.SendMessage(getApi().handle, OS.EM_SETSEL, start[0], end[0]);
            }
        }
	...
	*/
        int caret = start[0];
        if (start[0] != end[0]) {
            int startLine = (int) OS.SendMessage(getApi().handle, OS.EM_LINEFROMCHAR, start[0], 0);
            int endLine = (int) OS.SendMessage(getApi().handle, OS.EM_LINEFROMCHAR, end[0], 0);
            if (startLine == endLine) {
                int idThread = OS.GetWindowThreadProcessId(getApi().handle, null);
                GUITHREADINFO lpgui = new GUITHREADINFO();
                lpgui.cbSize = GUITHREADINFO.sizeof;
                if (OS.GetGUIThreadInfo(idThread, lpgui)) {
                    if (lpgui.hwndCaret == getApi().handle || lpgui.hwndCaret == 0) {
                        POINT ptCurrentPos = new POINT();
                        if (OS.GetCaretPos(ptCurrentPos)) {
                            long endPos = OS.SendMessage(getApi().handle, OS.EM_POSFROMCHAR, end[0], 0);
                            if (endPos == -1) {
                                long startPos = OS.SendMessage(getApi().handle, OS.EM_POSFROMCHAR, start[0], 0);
                ...
                int caretPos = (int) OS.SendMessage(getApi().handle, OS.EM_LINEINDEX, -1, 0);
                int caretLine = (int) OS.SendMessage(getApi().handle, OS.EM_LINEFROMCHAR, caretPos, 0);
    ...
    public char getEchoChar() {
        checkWidget();
        return (char) OS.SendMessage(getApi().handle, OS.EM_GETPASSWORDCHAR, 0, 0);
    }
    ...
    public int getLineCount() {
        checkWidget();
        return (int) OS.SendMessage(getApi().handle, OS.EM_GETLINECOUNT, 0, 0);
    }
	...
	*/
    ...
    public int getTextLimit() {
        checkWidget();
        int limit = (int) OS.SendMessage(getApi().handle, OS.EM_GETLIMITTEXT, 0, 0) & 0x7FFFFFFF;
        if (segments != null && limit < Text.LIMIT)
    ...
    public int getTopIndex() {
        ...
        if ((getApi().style & SWT.SINGLE) != 0)
            return 0;
        return (int) OS.SendMessage(getApi().handle, OS.EM_GETFIRSTVISIBLELINE, 0, 0);
    }
	...
	*/
        ...
        long code = OS.SendMessage(getApi().handle, OS.EM_GETSCROLLPOS, 0, buffer);
        if (code == 1)
            return buffer[1];
	...
	*/
    ...
    public void paste() {
        ...
        if ((getApi().style & SWT.READ_ONLY) != 0)
            return;
        OS.SendMessage(getApi().handle, OS.WM_PASTE, 0, 0);
    }
    ...
    int resolveTextDirection() {
        ...
        if (length > 0) {
			...
			*/
                textDirection = (getApi().style & SWT.RIGHT_TO_LEFT) != 0 ? SWT.RIGHT_TO_LEFT : SWT.LEFT_TO_RIGHT;
            }
        }
	...
	*/
        if (OS.GetKeyState(OS.VK_LBUTTON) < 0) {
            if (getApi().handle == OS.GetCapture())
                return true;
        }
        ...
        switch(key) {
            case 0x08:
                ...
                if (start[0] == end[0]) {
                    if (start[0] == 0)
                        return true;
                    int lineStart = (int) OS.SendMessage(getApi().handle, OS.EM_LINEINDEX, -1, 0);
            ...
            case 0x7F:
                ...
                if (start[0] == end[0]) {
                    ...
                    if (start[0] == length)
                        return true;
                    int line = (int) OS.SendMessage(getApi().handle, OS.EM_LINEFROMCHAR, end[0], 0);
                    int lineStart = (int) OS.SendMessage(getApi().handle, OS.EM_LINEINDEX, line + 1, 0);
        ...
        if (newText == oldText)
            return true;
        newText = SwtDisplay.withCrLf(newText);
        ...
        if ((flags & OS.SWP_NOSIZE) == 0 && width != 0) {
            ...
            if (rect.right - rect.left <= marginWidth) {
                ...
                if (start[0] != 0 || end[0] != 0) {
                    OS.SetWindowPos(getApi().handle, 0, x, y, width, height, flags);
                    OS.SendMessage(getApi().handle, OS.EM_SETSEL, 0, 0);
                    OS.SendMessage(getApi().handle, OS.EM_SETSEL, start[0], end[0]);
                    return;
                }
            }
	...
	*/
        if ((flags & OS.SWP_NOSIZE) == 0) {
            ...
            if ((bits & OS.ES_MULTILINE) != 0) {
                ...
                if ((rect.bottom - rect.top) < tm.tmHeight) {
                    ...
                    OS.SendMessage(getApi().handle, OS.EM_SETRECT, 0, rect);
                }
            }
        }
    }
    ...
    public void setEchoChar(char echo) {
        ...
        OS.SendMessage(getApi().handle, OS.EM_SETPASSWORDCHAR, echo, 0);
	...
	*/
    ...
    public void setEditable(boolean editable) {
        ...
        if (!editable)
            getApi().style |= SWT.READ_ONLY;
        OS.SendMessage(getApi().handle, OS.EM_SETREADONLY, editable ? 0 : 1, 0);
    }
    ...
    void setMargins() {
        if ((getApi().style & SWT.SEARCH) != 0) {
            ...
            if (flags != 0) {
                ...
                OS.SendMessage(getApi().handle, OS.EM_SETMARGINS, flags, OS.MAKELPARAM(iconWidth, iconWidth));
            }
        }
    }
    ...
    public void setMessage(String message) {
        ...
        if ((bits & OS.ES_MULTILINE) == 0) {
            ...
            OS.SendMessage(getApi().handle, OS.EM_SETCUEBANNER, 0, chars);
        }
    }
    ...
    public void setSelection(int start) {
        ...
        OS.SendMessage(getApi().handle, OS.EM_SETSEL, start, start);
        OS.SendMessage(getApi().handle, OS.EM_SCROLLCARET, 0, 0);
    }
    ...
    public void setSelection(int start, int end) {
        ...
        OS.SendMessage(getApi().handle, OS.EM_SETSEL, start, end);
        OS.SendMessage(getApi().handle, OS.EM_SCROLLCARET, 0, 0);
    }
	...
	*/
        ...
        OS.SendMessage(getApi().handle, OS.EM_SETTABSTOPS, 1, new int[] { width });
    }
	...
	*/
    ...
    public void setTextLimit(int limit) {
        ...
        if (segments != null && limit > 0) {
            OS.SendMessage(getApi().handle, OS.EM_SETLIMITTEXT, limit + Math.min(segments.length, Text.LIMIT - limit), 0);
        } else {
            OS.SendMessage(getApi().handle, OS.EM_SETLIMITTEXT, limit, 0);
        }
    }
    ...
    public void setTopIndex(int index) {
        ...
        if ((getApi().style & SWT.SINGLE) != 0)
            return;
        int count = (int) OS.SendMessage(getApi().handle, OS.EM_GETLINECOUNT, 0, 0);
        index = Math.min(Math.max(index, 0), count - 1);
        int topIndex = (int) OS.SendMessage(getApi().handle, OS.EM_GETFIRSTVISIBLELINE, 0, 0);
        OS.SendMessage(getApi().handle, OS.EM_LINESCROLL, 0, index - topIndex);
    }
	...
	*/
    ...
    long windowProc(long hwnd, int msg, long wParam, long lParam) {
        boolean processSegments = hooks(SWT.Segments) || filters(SWT.Segments), redraw = false, updateDirection = (getApi().state & HAS_AUTO_DIRECTION) != 0;
        long code;
        ...
        if (msg == SwtDisplay.SWT_RESTORECARET) {
        ...
        switch((int) wParam) {
            case SWT.DEL:
                if (OS.GetKeyState(OS.VK_CONTROL) < 0) {
                    if ((getApi().style & SWT.READ_ONLY) != 0 || (getApi().style & SWT.PASSWORD) != 0)
                        return LRESULT.ZERO;
                    Point selection = getSelection();
                    int x = selection.x;
                    int y = selection.y;
                    if (x == y) {
                        ...
                        if (m.find()) {
                            x = m.start();
                            y = m.end();
                            OS.SendMessage(getApi().handle, OS.EM_SETSEL, x, y);
                        }
                    }
                    if (x < y) {
                        /*
					...
					*/
                        OS.SendMessage(getApi().handle, OS.EM_REPLACESEL, 1, 0);
                    }
                    return LRESULT.ZERO;
                }
        }
	...
	*/
    ...
    LRESULT WM_DRAWITEM(long wParam, long lParam) {
        ...
        POINT pt = new POINT();
        OS.MapWindowPoints(struct.hwndItem, getApi().handle, pt, 1);
        ...
        if (struct.CtlID == SWT.ICON_CANCEL && struct.hwndItem == hwndActiveIcon && OS.IsAppThemed()) {
            ...
            OS.DrawThemeBackground(((SwtDisplay) display.getImpl()).hButtonThemeAuto(getApi().nativeZoom), struct.hDC, OS.BP_PUSHBUTTON, state, rect, null);
        }
        int width = rect.right - rect.left;
        int height = rect.bottom - rect.top;
        int size = Math.min(width, height);
        long hIcon = (struct.CtlID == SWT.ICON_SEARCH) ? ((SwtDisplay) display.getImpl()).getTextSearchIcon(size) : ((SwtDisplay) display.getImpl()).getTextCancelIcon(size);
	...
	*/
    ...
    LRESULT WM_IME_CHAR(long wParam, long lParam) {
        ...
        ((SwtDisplay) display.getImpl()).lastKey = 0;
        ((SwtDisplay) display.getImpl()).lastAscii = (int) wParam;
        ((SwtDisplay) display.getImpl()).lastVirtual = ((SwtDisplay) display.getImpl()).lastDead = false;
	...
	*/
        ignoreCharacter = true;
        long result = callWindowProc(getApi().handle, OS.WM_IME_CHAR, wParam, lParam);
        MSG msg = new MSG();
        int flags = OS.PM_REMOVE | OS.PM_NOYIELD | OS.PM_QS_INPUT | OS.PM_QS_POSTMESSAGE;
        while (OS.PeekMessage(msg, getApi().handle, OS.WM_CHAR, OS.WM_CHAR, flags)) {
        ...
        // widget could be disposed at this point
        ((SwtDisplay) display.getImpl()).lastKey = ((SwtDisplay) display.getImpl()).lastAscii = 0;
	...
	*/
        ...
        if (start[0] == end[0]) {
            int length = OS.GetWindowTextLength(getApi().handle);
            if (length == start[0]) {
                int code = (int) OS.SendMessage(getApi().handle, OS.EM_LINELENGTH, length, 0);
                if (code == 0)
                    return LRESULT.ZERO;
            }
        }
    ...
    LRESULT WM_SIZE(long wParam, long lParam) {
        ...
        if ((getApi().style & SWT.SEARCH) != 0) {
            ...
            boolean rtl = (getApi().style & SWT.RIGHT_TO_LEFT) != 0;
            long hwndLeading = OS.GetDlgItem(getApi().handle, rtl ? SWT.ICON_CANCEL : SWT.ICON_SEARCH);
            long hwndTrailing = OS.GetDlgItem(getApi().handle, rtl ? SWT.ICON_SEARCH : SWT.ICON_CANCEL);
    ...
    LRESULT wmClipboard(int msg, long wParam, long lParam) {
        ...
        switch(msg) {
            ...
            case OS.EM_UNDO:
            case OS.WM_UNDO:
                if (OS.SendMessage(getApi().handle, OS.EM_CANUNDO, 0, 0) != 0) {
                    ignoreModify = ignoreCharacter = true;
                    callWindowProc(getApi().handle, msg, wParam, lParam);
                    int length = OS.GetWindowTextLength(getApi().handle);
                    int[] newStart = new int[1], newEnd = new int[1];
                    OS.SendMessage(getApi().handle, OS.EM_GETSEL, newStart, newEnd);
        ...
        if (msg == OS.WM_UNDO) {
            ...
            callWindowProc(getApi().handle, OS.WM_UNDO, wParam, lParam);
    ...
    LRESULT wmCommandChild(long wParam, long lParam) {
        ...
        switch(code) {
            case OS.EN_CHANGE:
                ...
                if ((getApi().style & SWT.SEARCH) != 0) {
                    boolean showCancel = OS.GetWindowTextLength(getApi().handle) != 0;
                    long hwndCancel = OS.GetDlgItem(getApi().handle, SWT.ICON_CANCEL);
                    if (hwndCancel != 0)
        ...
        if (segments != null) {
            switch((int) wParam) {
                ...
                case OS.VK_DOWN:
                    ...
                    OS.SendMessage(getApi().handle, OS.EM_GETSEL, start, end);
                    while (true) {
                        code = callWindowProc(getApi().handle, OS.WM_KEYDOWN, wParam, lParam);
                        OS.SendMessage(getApi().handle, OS.EM_GETSEL, newStart, newEnd);
