...
public class SwtTabFolder extends SwtComposite implements ITabFolder {
	...
	*/
    ...
    Point computeSizeInPixels(int wHint, int hHint, boolean changed) {
        ...
        int count = (int) OS.SendMessage(getApi().handle, OS.TCM_GETITEMCOUNT, 0, 0);
        if (count != 0) {
            OS.SendMessage(getApi().handle, OS.TCM_GETITEMRECT, count - 1, itemRect);
            width = Math.max(width, itemRect.right - insetRect.right);
        }
        RECT rect = new RECT();
        OS.SetRect(rect, 0, 0, width, size.y);
        OS.SendMessage(getApi().handle, OS.TCM_ADJUSTRECT, 1, rect);
    ...
    void createItem(TabItem item, int index) {
        ...
        if (OS.SendMessage(getApi().handle, OS.TCM_INSERTITEM, index, tcItem) == -1) {
	...
	* a large value.
	*/
        long hwndToolTip = OS.SendMessage(getApi().handle, OS.TCM_GETTOOLTIPS, 0, 0);
    ...
    void destroyItem(TabItem item) {
        ...
        if (index == count)
            return;
        int selectionIndex = (int) OS.SendMessage(getApi().handle, OS.TCM_GETCURSEL, 0, 0);
        if (OS.SendMessage(getApi().handle, OS.TCM_DELETEITEM, index, 0) == 0) {
        ...
        if (count == 0) {
            if (imageList != null) {
                OS.SendMessage(getApi().handle, OS.TCM_SETIMAGELIST, 0, 0);
    ...
    void drawThemeBackground(long hDC, long hwnd, RECT rect) {
        ...
        if (OS.IntersectRect(new RECT(), rect2, rect)) {
            OS.DrawThemeBackground(((SwtDisplay) display.getImpl()).hTabTheme(getApi().nativeZoom), hDC, OS.TABP_BODY, 0, rect2, null);
        }
    }
    ...
    Rectangle getClientAreaInPixels() {
        ...
        OS.SendMessage(getApi().handle, OS.TCM_ADJUSTRECT, 0, rect);
    ...
    public TabItem getItem(Point point) {
        ...
        pinfo.x = point.x;
        pinfo.y = point.y;
        int index = (int) OS.SendMessage(getApi().handle, OS.TCM_HITTEST, 0, pinfo);
        if (index == -1)
            return null;
        return items[index];
    }
    ...
    public int getItemCount() {
        checkWidget();
        return (int) OS.SendMessage(getApi().handle, OS.TCM_GETITEMCOUNT, 0, 0);
    }
    ...
    public TabItem[] getSelection() {
        checkWidget();
        int index = (int) OS.SendMessage(getApi().handle, OS.TCM_GETCURSEL, 0, 0);
        if (index == -1)
    ...
    public int getSelectionIndex() {
        checkWidget();
        return (int) OS.SendMessage(getApi().handle, OS.TCM_GETCURSEL, 0, 0);
    }
    ...
    void setSelection(int index, boolean notify) {
        int oldIndex = (int) OS.SendMessage(getApi().handle, OS.TCM_GETCURSEL, 0, 0);
        if (oldIndex == index)
            return;
        if (oldIndex != -1) {
        ...
        OS.SendMessage(getApi().handle, OS.TCM_SETCURSEL, index, 0);
        int newIndex = (int) OS.SendMessage(getApi().handle, OS.TCM_GETCURSEL, 0, 0);
        if (newIndex != -1) {
    ...
    boolean updateTextDirection(int textDirection) {
        if (super.updateTextDirection(textDirection)) {
            ...
            for (int i = 0, n = items.length; i < n && items[i] != null; i++) {
                ((SwtTabItem) items[i].getImpl()).updateTextDirection(textDirection);
            }
            return true;
        }
        return false;
    ...
    String toolTipText(NMTTDISPINFO hdr) {
		...
		*/
            ...
            if (0 <= index && index < items.length) {
                TabItem item = items[index];
                if (item != null)
                    return ((SwtTabItem) item.getImpl()).toolTipText;
            }
        }
        ...
        if (imageList != null) {
            ...
            for (int i = 0; i < items.length; i++) {
                ...
                if (image != null) {
                    ...
                    OS.SendMessage(getApi().handle, OS.TCM_SETITEM, i, tcItem);
                }
            }
        }
    }
	...
	*/
        long hittest = OS.DefWindowProc(getApi().handle, OS.WM_NCHITTEST, wParam, lParam);
	...
	*/
    ...
    LRESULT WM_WINDOWPOSCHANGING(long wParam, long lParam) {
        ...
        OS.SendMessage(getApi().handle, OS.WM_NCCALCSIZE, 0, rect);
        int newWidth = rect.right - rect.left;
        int newHeight = rect.bottom - rect.top;
        OS.GetClientRect(getApi().handle, rect);
        ...
        RECT inset = new RECT();
        OS.SendMessage(getApi().handle, OS.TCM_ADJUSTRECT, 0, inset);
