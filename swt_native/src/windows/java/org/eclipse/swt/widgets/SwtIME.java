        ...
        if (hIMC != 0) {
            ...
            if ((lParam & OS.GCS_RESULTSTR) != 0) {
                ...
                if (length > 0) {
                    ...
                    //$NON-NLS-1$
                    event.text = text = buffer != null ? new String(buffer) : "";
                    ...
                    if (event.doit) {
                        ...
                        for (int i = 0; i < length; i++) {
                            ...
                            ((SwtDisplay) display.getImpl()).lastAscii = c;
                            event = new Event();
                            event.character = c;
                            parent.getImpl().sendEvent(SWT.KeyDown, event);
                        }
                    }
                }
                                    ...
                                    if (attr != null) {
                                        switch(attr.crText.type) {
                                            case OS.TF_CT_COLORREF:
                                                style.foreground = SwtColor.win32_new(display, attr.crText.cr);
                                                break;
                                            case OS.TF_CT_SYSCOLOR:
                                                int colorRef = OS.GetSysColor(attr.crText.cr);
                                                style.foreground = SwtColor.win32_new(display, colorRef);
                                                break;
                                        }
                                        switch(attr.crBk.type) {
                                            case OS.TF_CT_COLORREF:
                                                style.background = SwtColor.win32_new(display, attr.crBk.cr);
                                                break;
                                            case OS.TF_CT_SYSCOLOR:
                                                int colorRef = OS.GetSysColor(attr.crBk.cr);
                                                style.background = SwtColor.win32_new(display, colorRef);
                                                break;
                                        }
                                        switch(attr.crLine.type) {
                                            case OS.TF_CT_COLORREF:
                                                style.underlineColor = SwtColor.win32_new(display, attr.crLine.cr);
                                                break;
                                            case OS.TF_CT_SYSCOLOR:
                                                int colorRef = OS.GetSysColor(attr.crLine.cr);
                                                style.underlineColor = SwtColor.win32_new(display, colorRef);
                                                break;
                                        }
