...
public class SwtDisplay extends SwtDevice implements Executor, IDisplay {
    ...
    //$NON-NLS-1$
    static final boolean disableCustomThemeTweaks = Boolean.valueOf(System.getProperty("org.eclipse.swt.internal.win32.disableCustomThemeTweaks"));
	...
	*/
    ...
    { OS.VK_MENU, SWT.ALT }, { OS.VK_SHIFT, SWT.SHIFT }, { OS.VK_CONTROL, SWT.CONTROL }, //		{OS.VK_????,	SWT.COMMAND},
    /* NOT CURRENTLY USED */
    ...
    { OS.VK_F1, SWT.F1 }, { OS.VK_F2, SWT.F2 }, { OS.VK_F3, SWT.F3 }, { OS.VK_F4, SWT.F4 }, { OS.VK_F5, SWT.F5 }, { OS.VK_F6, SWT.F6 }, { OS.VK_F7, SWT.F7 }, { OS.VK_F8, SWT.F8 }, { OS.VK_F9, SWT.F9 }, { OS.VK_F10, SWT.F10 }, { OS.VK_F11, SWT.F11 }, { OS.VK_F12, SWT.F12 }, { OS.VK_F13, SWT.F13 }, { OS.VK_F14, SWT.F14 }, { OS.VK_F15, SWT.F15 }, { OS.VK_F16, SWT.F16 }, { OS.VK_F17, SWT.F17 }, { OS.VK_F18, SWT.F18 }, { OS.VK_F19, SWT.F19 }, { OS.VK_F20, SWT.F20 }, /* Numeric Keypad Keys */
    ...
    /* Other keys */
    { OS.VK_CAPITAL, SWT.CAPS_LOCK }, { OS.VK_NUMLOCK, SWT.NUM_LOCK }, { OS.VK_SCROLL, SWT.SCROLL_LOCK }, { OS.VK_PAUSE, SWT.PAUSE }, { OS.VK_CANCEL, SWT.BREAK }, { OS.VK_SNAPSHOT, SWT.PRINT_SCREEN } //		{OS.VK_????,		SWT.HELP},
    };
...
*/
    ...
    void addMenuItem(MenuItem item) {
        ...
        for (int i = 0; i < items.length; i++) {
            if (items[i] == null) {
                ((SwtMenuItem) item.getImpl()).id = i + ID_START;
                items[i] = item;
                return;
            }
        }
        ((SwtMenuItem) item.getImpl()).id = items.length + ID_START;
        ...
        synchronized (SwtDisplay.class) {
            for (Display display : Displays) {
                if (display != null) {
                    ...
                    if (((SwtDisplay) display.getImpl()).thread == thread)
                        SWT.error(SWT.ERROR_THREAD_INVALID_ACCESS, null, " Another Display is already associated by this thread: " + thread);
                }
            }
        }
    }
    ...
    void clearModal(Shell shell) {
        ...
        if (index == 0 && modalShells[0] == null)
            modalShells = null;
        for (Shell activeShell : getShells()) ((SwtShell) activeShell.getImpl()).updateModal();
    }
    ...
    static long create32bitDIB(Image image, int zoom) {
        long handle = SwtImage.win32_getHandle(image, zoom);
    ...
    long getTextSearchIcon(int size) {
        if (!sizeToSearchIconHandle.containsKey(size)) {
            int searchIconResource = textUseDarkthemeIcons ? SwtText.IDI_SEARCH_DARKTHEME : SwtText.IDI_SEARCH;
    ...
    long getTextCancelIcon(int size) {
        if (!sizeToCancelIconHandle.containsKey(size)) {
            int searchIconResource = textUseDarkthemeIcons ? SwtText.IDI_CANCEL_DARKTHEME : SwtText.IDI_CANCEL;
            long iconHandle = OS.LoadImage(OS.GetLibraryHandle(), searchIconResource, OS.IMAGE_ICON, size, size, 0);
            if (iconHandle == 0)
                error(SWT.ERROR_NO_HANDLES);
    ...
    public Widget findWidget(long handle, long id) {
        checkDevice();
        Control control = getControl(handle);
        return control != null ? ((SwtWidget) control.getImpl()).findItem(id) : null;
    }
    ...
    public Color getSystemColor(int id) {
        ...
        switch(id) {
            ...
            case SWT.COLOR_WIDGET_DISABLED_FOREGROUND:
                pixel = OS.GetSysColor(OS.COLOR_GRAYTEXT);
                break;
        ...
        return SwtColor.win32_new(this.getApi(), pixel);
    }
    ...
    Font getSystemFont(int zoom) {
        checkDevice();
        Font systemFont = SWTFontProvider.getSystemFont(this.getApi(), zoom);
    ...
    public Tray getSystemTray() {
        checkDevice();
        if (tray == null)
            tray = new Tray(this.getApi(), SWT.NONE);
        return tray;
    }
    ...
    public void init() {
        ...
        appLocalDir = System.getenv("LOCALAPPDATA") + "\\" + SwtDisplay.APP_NAME.replaceAll("[\\\\/:*?\"<>|]", "_");
    ...
    long messageProc(long hwnd, long msg, long wParam, long lParam) {
        switch((int) msg) {
                                                    ...
                                                    if (!accentKey) {
                                                        for (short accent : ACCENTS) {
                                                            ...
                                                            if (value != -1 && (value & 0xFF) == keyMsg.wParam) {
                                                                int state = value >> 8;
                                                                if ((OS.GetKeyState(OS.VK_SHIFT) < 0) == ((state & 0x1) != 0) && (OS.GetKeyState(OS.VK_CONTROL) < 0) == ((state & 0x2) != 0) && (OS.GetKeyState(OS.VK_MENU) < 0) == ((state & 0x4) != 0)) {
                                                                    if ((state & 0x7) != 0)
                                                                        accentKey = true;
                                                                    break;
                                                                }
            ...
            case SWT_TRAYICONMSG:
                {
                    if (tray != null) {
                        for (TrayItem item : ((SwtTray) tray.getImpl()).items) {
                            if (item != null && ((SwtTrayItem) item.getImpl()).id == wParam) {
                                return ((SwtTrayItem) item.getImpl()).messageProc(hwnd, (int) msg, wParam, lParam);
                            }
                        }
                    }
                    return 0;
                    ...
                    if (wParam != 0) {
                        if (!isXMouseActive()) {
                            ...
                            if (hwndActive != 0 && OS.IsWindowEnabled(hwndActive))
                                break;
                            Shell modal = modalDialog != null ? ((SwtDialog) modalDialog.getImpl()).parent : getModalShell();
                            if (modal != null && !modal.isDisposed()) {
                                long hwndModal = modal.handle;
                                if (OS.IsWindowEnabled(hwndModal)) {
                                    ((SwtDecorations) modal.getImpl()).bringToTop();
                                    if (modal.isDisposed())
                                        break;
                                }
            ...
            default:
                {
                    if ((int) msg == TASKBARCREATED) {
                        if (tray != null) {
                            for (TrayItem item : ((SwtTray) tray.getImpl()).items) {
                                if (item != null)
                                    ((SwtTrayItem) item.getImpl()).recreate();
                            }
                        }
                    }
                    if ((int) msg == SWT_OPENDOC) {
                        ...
                        if (filename != null) {
                            if (filename.startsWith(TASKBAR_EVENT)) {
                                ...
                                if (item != null) {
                                    ((SwtWidget) item.getImpl()).sendSelectionEvent(SWT.Selection);
                                }
                            } else {
                                Event event = new Event();
                                event.text = filename;
	...
	*/
    ...
    public boolean readAndDispatch() {
        ...
        runDeferredLayouts();
        runPopups();
        if (OS.PeekMessage(getApi().msg, 0, 0, 0, OS.PM_REMOVE)) {
            if (!filterMessage(getApi().msg)) {
                OS.TranslateMessage(getApi().msg);
                OS.DispatchMessage(getApi().msg);
            }
            runDeferredEvents();
            return true;
        }
    ...
    public Control removeControl(long handle) {
    ...
    void removeMenuItem(MenuItem item) {
        if (items == null)
            return;
        items[((SwtMenuItem) item.getImpl()).id - ID_START] = null;
    }
	...
	*/
    ...
    void runSettings() {
        ...
        for (Shell shell : getShells()) {
            if (!shell.isDisposed()) {
                if (!sameFont) {
                    ((SwtComposite) shell.getImpl()).updateFont(oldFont, newFont);
                }
    ...
    public void setData(String key, Object value) {
        ...
        switch(key) {
            ...
            case USE_DARKMODE_EXPLORER_THEME_KEY:
                useDarkModeExplorerTheme = _toBoolean(value) && !disableCustomThemeTweaks && OS.IsDarkModeAvailable();
            ...
            case COMBO_USE_DARK_THEME:
                comboUseDarkTheme = _toBoolean(value) && !disableCustomThemeTweaks && OS.IsDarkModeAvailable();
                break;
    ...
    void setModalDialog(Dialog modalDailog) {
        ...
        for (Shell shell : getShells()) {
            ((SwtShell) shell.getImpl()).updateModal();
        }
    }
    ...
    void setModalShell(Shell shell) {
        ...
        for (Shell activeShell : getShells()) {
            ((SwtShell) activeShell.getImpl()).updateModal();
        }
    }
	...
	*/
    ...
    long windowProc(long hwnd, long msg, long wParam, long lParam) {
        ...
        if (control != null && control.getImpl() instanceof SwtControl) {
            return ((SwtControl) control.getImpl()).windowProc(hwnd, (int) msg, wParam, lParam);
        }
        return OS.DefWindowProc(hwnd, (int) msg, wParam, lParam);
    }
