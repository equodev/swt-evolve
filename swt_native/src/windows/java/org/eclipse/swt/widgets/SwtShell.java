...
public class SwtShell extends SwtDecorations implements IShell {
    ...
    final static int[] SYSTEM_COLORS = { OS.COLOR_BTNFACE, OS.COLOR_WINDOW, OS.COLOR_BTNTEXT, OS.COLOR_WINDOWTEXT, OS.COLOR_HIGHLIGHT, OS.COLOR_SCROLLBAR };
    ...
    void center() {
        ...
        Rectangle parentRect = ((SwtDisplay) display.getImpl()).mapInPixels(parent, null, ((SwtScrollable) parent.getImpl()).getClientAreaInPixels());
    ...
    void createBalloonTipHandle() {
        balloonTipHandle = OS.CreateWindowEx(0, new TCHAR(0, OS.TOOLTIPS_CLASS, true), null, OS.TTS_ALWAYSTIP | OS.TTS_NOPREFIX | OS.TTS_BALLOON, OS.CW_USEDEFAULT, 0, OS.CW_USEDEFAULT, 0, getApi().handle, 0, OS.GetModuleHandle(null), null);
        if (balloonTipHandle == 0)
	...
	*/
        ...
        ((SwtDisplay) display.getImpl()).addControl(balloonTipHandle, this.getApi());
    ...
    @Override
    void createHandle() {
        boolean embedded = getApi().handle != 0 && (getApi().state & FOREIGN_HANDLE) == 0;
        /*
	...
	*/
        ...
        if (getApi().handle == 0 || embedded) {
            ...
            windowProc = OS.GetWindowLongPtr(getApi().handle, OS.GWL_WNDPROC);
        }
        /*
	* The following code is intentionally commented.
	*/
        ...
        if (!embedded) {
            ...
            if ((getApi().style & SWT.NO_TRIM) == 0) {
                if ((getApi().style & (SWT.BORDER | SWT.RESIZE)) == 0)
                    bits |= OS.WS_BORDER;
            }
            /*
    ...
    void createToolTipHandle() {
        toolTipHandle = createToolTipHandle(getApi().handle);
    }

    long createToolTipHandle(long parent) {
        long toolTipHandle = OS.CreateWindowEx(0, new TCHAR(0, OS.TOOLTIPS_CLASS, true), null, OS.TTS_ALWAYSTIP | OS.TTS_NOPREFIX, OS.CW_USEDEFAULT, 0, OS.CW_USEDEFAULT, 0, parent, 0, OS.GetModuleHandle(null), null);
        if (toolTipHandle == 0)
            error(SWT.ERROR_NO_HANDLES);
        if (ToolTipProc == 0) {
	...
	*/
        ...
        ((SwtDisplay) display.getImpl()).addControl(toolTipHandle, this.getApi());
        OS.SetWindowLongPtr(toolTipHandle, OS.GWLP_WNDPROC, ((SwtDisplay) display.getImpl()).windowProc);
    ...
    void deregister() {
        super.deregister();
        if (toolTipHandle != 0)
            ((SwtDisplay) display.getImpl()).removeControl(toolTipHandle);
        if (balloonTipHandle != 0)
            ((SwtDisplay) display.getImpl()).removeControl(balloonTipHandle);
        if (menuItemToolTipHandle != 0)
            ((SwtDisplay) display.getImpl()).removeControl(menuItemToolTipHandle);
    }
    ...
    public Control findBackgroundControl() {
        return background != -1 || backgroundImage != null ? this.getApi() : null;
    }
	...
	*/
        long hwndParent = OS.GetParent(getApi().handle);
        if (hwndParent != 0 && getApi().handle == OS.GetActiveWindow()) {
	...
	*/
    ...
    public void forceActive() {
        ...
        if (!isVisible())
            return;
        OS.SetForegroundWindow(getApi().handle);
    }
    ...
    public int getAlpha() {
        ...
        if (OS.GetLayeredWindowAttributes(getApi().handle, null, pbAlpha, null)) {
    ...
    public int getImeInputMode() {
        ...
        if (!OS.IsDBLocale)
            return 0;
        long hIMC = OS.ImmGetContext(getApi().handle);
        ...
        OS.ImmReleaseContext(getApi().handle, hIMC);
        if (!open)
            return SWT.NONE;
        int result = 0;
    ...
    Point getMaximumSizeInPixels() {
        ...
        if ((getApi().style & SWT.NO_TRIM) == 0 && (getApi().style & trim) != 0) {
    ...
    long hwndMDIClient() {
        if (hwndMDIClient == 0) {
            ...
            hwndMDIClient = OS.CreateWindowEx(0, new TCHAR(0, "MDICLIENT", true), null, widgetStyle, 0, 0, 0, 0, getApi().handle, 0, OS.GetModuleHandle(null), new CREATESTRUCT());
	...
	*/
    ...
    void register() {
        ...
        if (balloonTipHandle != 0)
            ((SwtDisplay) display.getImpl()).addControl(balloonTipHandle, this.getApi());
        if (menuItemToolTipHandle != 0)
            ((SwtDisplay) display.getImpl()).addControl(menuItemToolTipHandle, this.getApi());
    }
	...
	*/
    ...
    public void setAlpha(int alpha) {
        ...
        if (alpha == 0xFF) {
            OS.SetWindowLong(getApi().handle, OS.GWL_EXSTYLE, bits & ~OS.WS_EX_LAYERED);
            int flags = OS.RDW_ERASE | OS.RDW_INVALIDATE | OS.RDW_FRAME | OS.RDW_ALLCHILDREN;
            OS.RedrawWindow(getApi().handle, null, 0, flags);
        } else {
            OS.SetWindowLong(getApi().handle, OS.GWL_EXSTYLE, bits | OS.WS_EX_LAYERED);
            OS.SetLayeredWindowAttributes(getApi().handle, 0, (byte) alpha, OS.LWA_ALPHA);
        }
    }
	...
	*/
    ...
    public void setEnabled(boolean enabled) {
        ...
        if (enabled && getApi().handle == OS.GetActiveWindow()) {
            if (!restoreFocus())
                traverseGroup(true);
        }
    }
    ...
    public void setFullScreen(boolean fullScreen) {
        ...
        int styleFlags = OS.GetWindowLong(getApi().handle, OS.GWL_STYLE);
        ...
        OS.SetWindowLong(getApi().handle, OS.GWL_STYLE, styleFlags);
        if (wasMaximized) {
            OS.ShowWindow(getApi().handle, OS.SW_HIDE);
            stateFlags = OS.SW_SHOWMAXIMIZED;
        }
        if (visible)
            OS.ShowWindow(getApi().handle, stateFlags);
    ...
    void setToolTipText(long hwnd, String text) {
        ...
        boolean isCurrentlyVisible = (OS.SendMessage(hwndToolTip, OS.TTM_GETCURRENTTOOL, 0, lpti) != 0) && (lpti.uId == hwnd);
	...
	*/
        if (showWithParent && !visible) {
            OS.ShowOwnedPopups(getApi().handle, false);
        }
        if (!visible)
            fixActiveShell();
        ...
        if (showWithParent != visible) {
            ...
            if (visible) {
                OS.ShowOwnedPopups(getApi().handle, true);
            }
        }
    ...
    void updateModal() {
        if (SwtDisplay.TrimEnabled) {
            ...
            OS.EnableWindow(getApi().handle, isActive());
        }
    }
	...
	*/
    ...
    TCHAR windowClass() {
        if ((getApi().style & SWT.TOOL) != 0) {
            int trim = SWT.TITLE | SWT.CLOSE | SWT.MIN | SWT.MAX | SWT.BORDER | SWT.RESIZE;
            if ((getApi().style & trim) == 0)
                return ((SwtDisplay) display.getImpl()).windowShadowClass;
        }
    ...
    Rectangle getClientRectInWindow() {
        ...
        OS.ClientToScreen(getApi().handle, clientWindowLT);
    ...
    void overpaintMenuBorder() {
        if ((menuBar == null) || (((SwtDisplay) display.getImpl()).menuBarBorderPen == 0))
            return;
        Rectangle clientArea = getClientRectInWindow();
        long dc = OS.GetWindowDC(getApi().handle);
        long oldPen = OS.SelectObject(dc, ((SwtDisplay) display.getImpl()).menuBarBorderPen);
    ...
    long windowProc(long hwnd, int msg, long wParam, long lParam) {
        if (getApi().handle == 0)
            return 0;
        if ((getApi().style & SWT.NO_MOVE) != 0 && msg == OS.WM_NCLBUTTONDOWN && wParam == OS.HTCAPTION)
            return 0;
        if (hwnd == toolTipHandle || hwnd == balloonTipHandle || hwnd == menuItemToolTipHandle) {
            switch(msg) {
                case OS.WM_TIMER:
                ...
                case OS.WM_LBUTTONDOWN:
                    {
                        ...
                        if (tip != null) {
                            tip.setVisible(false);
                            ((SwtWidget) tip.getImpl()).sendSelectionEvent(SWT.Selection);
                        }
                        break;
                    }
            }
        ...
        if (hwnd == getApi().handle) {
            if (msg == SwtDisplay.TASKBARBUTTONCREATED) {
                if (((SwtDisplay) display.getImpl()).taskBar != null) {
                    for (TaskItem item : ((SwtTaskBar) ((SwtDisplay) display.getImpl()).taskBar.getImpl()).items) {
                        if (item != null && ((SwtTaskItem) item.getImpl()).shell == this.getApi()) {
                            ((SwtTaskItem) item.getImpl()).onTaskbarButtonCreated();
                            break;
                        }
                    }
                }
	...
	*/
    ...
    LRESULT WM_ACTIVATE(long wParam, long lParam) {
        ...
        if (OS.LOWORD(wParam) == 0) {
            if (lParam == 0 || (lParam != toolTipHandle && lParam != balloonTipHandle && lParam != menuItemToolTipHandle)) {
	...
	*/
    ...
    LRESULT WM_ENTERIDLE(long wParam, long lParam) {
        ...
        if (((SwtDisplay) display.getImpl()).runAsyncMessages(false))
    ...
    LRESULT WM_GETMINMAXINFO(long wParam, long lParam) {
        ...
        if (result != null)
            return result;
        if (minWidth != SWT.DEFAULT || minHeight != SWT.DEFAULT || maxWidth != SWT.DEFAULT || maxHeight != SWT.DEFAULT) {
            ...
            default:
                {
                    ...
                    if (control != null) {
                        ...
                        if (decorations.getShell() == this.getApi() && decorations != this.getApi()) {
                            ((SwtDisplay) display.getImpl()).ignoreRestoreFocus = true;
                            ((SwtDisplay) display.getImpl()).lastHittest = hittest;
                            ((SwtDisplay) display.getImpl()).lastHittestControl = null;
                            if (hittest == OS.HTMENU || hittest == OS.HTSYSMENU) {
                                ((SwtDisplay) display.getImpl()).lastHittestControl = control;
                                return null;
                            }
	...
	*/
        ...
        if (hwnd == 0)
            return null;
        Control control = ((SwtDisplay) display.getImpl()).findControl(hwnd);
        /*
	...
	*/
        ...
        long code = callWindowProc(getApi().handle, OS.WM_MOUSEACTIVATE, wParam, lParam);
    ...
    LRESULT WM_NCHITTEST(long wParam, long lParam) {
        ...
        if (!isEnabled() || !isActive()) {
            ...
            long hittest = callWindowProc(getApi().handle, OS.WM_NCHITTEST, wParam, lParam);
	...
	*/
        ...
        long code = callWindowProc(getApi().handle, OS.WM_NCLBUTTONDOWN, wParam, lParam);
        ((SwtDisplay) display.getImpl()).lockActiveWindow = false;
        Control focusControl = ((SwtDisplay) display.getImpl()).lastHittestControl;
        if (focusControl != null && !focusControl.isDisposed()) {
            focusControl.setFocus();
        }
        ((SwtDisplay) display.getImpl()).lastHittestControl = null;
        ((SwtDisplay) display.getImpl()).ignoreRestoreFocus = false;
        return new LRESULT(code);
    }
	...
	*/
        ...
        if (msg == OS.WM_LBUTTONDOWN) {
            if (!SwtDisplay.TrimEnabled) {
                Shell modalShell = ((SwtDisplay) display.getImpl()).getModalShell();
            ...
            if (!OS.IsWindowEnabled(getApi().handle)) {
                long hwndPopup = OS.GetLastActivePopup(getApi().handle);
                if (hwndPopup != 0 && hwndPopup != getApi().handle) {
                    if (((SwtDisplay) display.getImpl()).getControl(hwndPopup) == null) {
	...
	*/
        ...
        if (hitTest == OS.HTERROR) {
            if (!getEnabled()) {
                ...
                if (control == this.getApi() && cursor != null) {
                    POINT pt = new POINT();
                    int pos = OS.GetMessagePos();
                    OS.POINTSTOPOINT(pt, pos);
	...
	*/
        if (lParam == OS.SW_PARENTOPENING) {
            ...
            while (control != null) {
                ...
                if (!((SwtShell) shell.getImpl()).showWithParent)
                    return LRESULT.ZERO;
