...
public class SwtTableColumn extends SwtItem implements ITableColumn {
    ...
    public SwtTableColumn(Table parent, int style, TableColumn api) {
        ...
        resizable = true;
        this.parent = parent;
        ((SwtTable) parent.getImpl()).createItem(this.getApi(), parent.getColumnCount());
    }
    ...
    private int calcAutoWidth(int index, boolean withHeader) {
        ...
        boolean isTableDrawing = ((SwtControl) parent.getImpl()).getDrawing();
    ...
    public void pack() {
        ...
        ((SwtTable) parent.getImpl()).ignoreColumnResize = true;
        int columnWidth = 0;
        if (((SwtWidget) parent.getImpl()).hooks(SWT.MeasureItem)) {
            ...
            for (int i = 0; i < count; i++) {
                TableItem item = ((SwtTable) parent.getImpl())._getItem(i, false);
                if (item != null) {
                    long hFont = ((SwtTableItem) item.getImpl()).fontHandle(index);
                    if (hFont != -1)
                        hFont = OS.SelectObject(hDC, hFont);
                    Event event = ((SwtTable) parent.getImpl()).sendMeasureItemEvent(item, i, index, hDC);
                    if (hFont != -1)
                        hFont = OS.SelectObject(hDC, hFont);
			...
			*/
                if (((SwtTable) parent.getImpl()).imageList == null)
                    columnWidth += 2;
                /*
			* Bug in Windows.  When LVM_SETCOLUMNWIDTH is used with LVSCW_AUTOSIZE
        ...
        ((SwtTable) parent.getImpl()).ignoreColumnResize = false;
        if (oldWidth != columnWidth) {
            ...
            for (int columnindex : parent.getColumnOrder()) {
                ...
                if (moved && !column.isDisposed()) {
                    ((SwtTableColumn) column.getImpl()).updateToolTip(columnindex);
                    column.getImpl().sendEvent(SWT.Move);
                }
                if (column == this.getApi())
                    moved = true;
            }
        }
    }
    ...
    public void setAlignment(int alignment) {
        ...
        int fmt = 0;
        if ((getApi().style & SWT.LEFT) == SWT.LEFT)
            fmt = OS.LVCFMT_LEFT;
        if ((getApi().style & SWT.CENTER) == SWT.CENTER)
            fmt = OS.LVCFMT_CENTER;
        if ((getApi().style & SWT.RIGHT) == SWT.RIGHT)
            fmt = OS.LVCFMT_RIGHT;
        lvColumn.fmt |= fmt;
	...
	*/
    ...
    void setImage(Image image, boolean sort, boolean right) {
        ...
        if (image != null) {
            ...
            lvColumn.iImage = ((SwtTable) parent.getImpl()).imageIndexHeader(image);
            if (right)
    ...
    public void setMoveable(boolean moveable) {
        ...
        ((SwtTable) parent.getImpl()).updateMoveable();
    }
        ...
        switch(direction) {
            ...
            case SWT.NONE:
                ...
                if (image != null) {
                    ...
                    hdItem.iImage = ((SwtTable) parent.getImpl()).imageIndexHeader(image);
                } else {
                    hdItem.fmt &= ~OS.HDF_IMAGE;
                    hdItem.mask &= ~OS.HDI_IMAGE;
                }
	...
	*/
    ...
    public void setToolTipText(String string) {
        checkWidget();
        toolTipText = string;
        long hwndHeaderToolTip = ((SwtTable) parent.getImpl()).headerToolTipHandle;
        if (hwndHeaderToolTip == 0) {
            ((SwtTable) parent.getImpl()).createHeaderToolTips();
            ((SwtTable) parent.getImpl()).updateHeaderToolTips();
        }
    }
