...
public class SwtExpandBar extends SwtComposite implements IExpandBar {
    ...
    Point computeSizeInPixels(int wHint, int hHint, boolean changed) {
        ...
        if (wHint == SWT.DEFAULT || hHint == SWT.DEFAULT) {
            if (itemCount > 0) {
                ...
                long hTheme = 0;
                if (isAppThemed()) {
                    hTheme = ((SwtDisplay) display.getImpl()).hExplorerBarTheme(getApi().nativeZoom);
                }
                ...
                for (int i = 0; i < itemCount; i++) {
                    ...
                    height += ((SwtExpandItem) item.getImpl()).getHeaderHeightInPixels();
                    if (((SwtExpandItem) item.getImpl()).expanded)
                        height += ((SwtExpandItem) item.getImpl()).height;
                    height += spacing;
                    width = Math.max(width, ((SwtExpandItem) item.getImpl()).getPreferredWidth(hTheme, hDC));
                }
                if (hCurrentFont != 0) {
    ...
    void createItem(ExpandItem item, int style, int index) {
        ...
        ((SwtExpandItem) item.getImpl()).width = Math.max(0, rect.right - rect.left - spacing * 2);
    ...
    void destroyItem(ExpandItem item) {
        ...
        if (item == focusItem) {
            ...
            if (focusIndex < itemCount) {
                ...
                ((SwtExpandItem) focusItem.getImpl()).redraw(true);
            } else {
                focusItem = null;
            }
        }
    ...
    void drawThemeBackground(long hDC, long hwnd, RECT rect) {
        ...
        OS.DrawThemeBackground(((SwtDisplay) display.getImpl()).hExplorerBarTheme(getApi().nativeZoom), hDC, OS.EBP_NORMALGROUPBACKGROUND, 0, rect2, null);
    }
    ...
    int getBandHeight() {
        ...
        OS.ReleaseDC(getApi().handle, hDC);
        return Math.max(SwtExpandItem.CHEVRON_SIZE, lptm.tmHeight + 4);
    }
    ...
    void layoutItems(int index, boolean setScrollbar) {
        if (index < itemCount) {
            ...
            for (int i = 0; i < index; i++) {
                ...
                y += ((SwtExpandItem) item.getImpl()).getHeaderHeightInPixels() + spacing;
            }
            for (int i = index; i < itemCount; i++) {
                ExpandItem item = items[i];
                ((SwtExpandItem) item.getImpl()).setBoundsInPixels(spacing, y, 0, 0, true, false);
                if (((SwtExpandItem) item.getImpl()).expanded)
                    y += ((SwtExpandItem) item.getImpl()).height;
                y += ((SwtExpandItem) item.getImpl()).getHeaderHeightInPixels() + spacing;
            }
    ...
    void setSpacingInPixels(int spacing) {
        ...
        for (int i = 0; i < itemCount; i++) {
            ...
            if (((SwtExpandItem) item.getImpl()).width != width)
                ((SwtExpandItem) item.getImpl()).setBoundsInPixels(0, 0, width, ((SwtExpandItem) item.getImpl()).height, false, true);
        }
        layoutItems(0, true);
    ...
    void showFocus(boolean up) {
        ...
        int updateY = 0;
        if (up) {
            if (((SwtExpandItem) focusItem.getImpl()).y < 0) {
                updateY = Math.min(yCurrentScroll, -((SwtExpandItem) focusItem.getImpl()).y);
            }
        } else {
            int itemHeight = ((SwtExpandItem) focusItem.getImpl()).y + getBandHeight();
            if (((SwtExpandItem) focusItem.getImpl()).expanded) {
                if (height >= getBandHeight() + ((SwtExpandItem) focusItem.getImpl()).height) {
                    itemHeight += ((SwtExpandItem) focusItem.getImpl()).height;
                }
            }
        ...
        if (updateY != 0) {
            ...
            OS.ScrollWindowEx(getApi().handle, 0, updateY, null, null, 0, null, OS.SW_SCROLLCHILDREN | OS.SW_INVALIDATE);
            for (int i = 0; i < itemCount; i++) {
                ((SwtExpandItem) items[i].getImpl()).y += updateY;
            }
        }
    }
    ...
    @Override
    TCHAR windowClass() {
        return ((SwtDisplay) display.getImpl()).windowClass;
    }

    @Override
    long windowProc() {
        return ((SwtDisplay) display.getImpl()).windowProc;
    }
    ...
    LRESULT WM_KEYDOWN(long wParam, long lParam) {
        ...
        switch((int) wParam) {
            ...
            case OS.VK_RETURN:
                ...
                sendEvent(((SwtExpandItem) focusItem.getImpl()).expanded ? SWT.Collapse : SWT.Expand, event);
    ...
    LRESULT WM_LBUTTONDOWN(long wParam, long lParam) {
        ...
        for (int i = 0; i < itemCount; i++) {
            ExpandItem item = items[i];
            boolean hover = ((SwtExpandItem) item.getImpl()).isHover(x, y);
    ...
    LRESULT WM_LBUTTONUP(long wParam, long lParam) {
        ...
        int x = OS.GET_X_LPARAM(lParam);
        int y = OS.GET_Y_LPARAM(lParam);
        boolean hover = ((SwtExpandItem) focusItem.getImpl()).isHover(x, y);
        if (hover) {
            Event event = new Event();
            event.item = focusItem;
            sendEvent(((SwtExpandItem) focusItem.getImpl()).expanded ? SWT.Collapse : SWT.Expand, event);
    ...
    LRESULT WM_MOUSELEAVE(long wParam, long lParam) {
        ...
        for (int i = 0; i < itemCount; i++) {
            ExpandItem item = items[i];
            if (((SwtExpandItem) item.getImpl()).hover) {
                ((SwtExpandItem) item.getImpl()).hover = false;
                ((SwtExpandItem) item.getImpl()).redraw(false);
                break;
            }
        }
        return result;
    ...
    LRESULT WM_MOUSEMOVE(long wParam, long lParam) {
        ...
        for (int i = 0; i < itemCount; i++) {
            ExpandItem item = items[i];
            boolean hover = ((SwtExpandItem) item.getImpl()).isHover(x, y);
            if (((SwtExpandItem) item.getImpl()).hover != hover) {
                ((SwtExpandItem) item.getImpl()).hover = hover;
                ((SwtExpandItem) item.getImpl()).redraw(false);
            }
        }
        return result;
