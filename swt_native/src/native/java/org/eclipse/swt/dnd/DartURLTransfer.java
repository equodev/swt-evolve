 ...
 * @since 3.4
 */
public class DartURLTransfer extends DartByteArrayTransfer implements IURLTransfer {

    static URLTransfer _instance = new URLTransfer();

    // XXX: SWT uses the deprecated format name here,
    // the new name is "public.url" aka NSPasteboardTypeURL
    static final String URL = "Apple URL pasteboard type";

    static final String URL1 = "public.url";

    static final int URL_ID = registerType(URL);

    static final int URL_ID1 = registerType(URL1);

    DartURLTransfer(URLTransfer api) {
        super(api);
    }
    ...
    public URLTransfer getApi() {
        if (api == null)
            api = URLTransfer.createApi(this);
        return (URLTransfer) api;
    }

    public VURLTransfer getValue() {
        if (value == null)
            value = new VURLTransfer(this);
        return (VURLTransfer) value;
    }
}
