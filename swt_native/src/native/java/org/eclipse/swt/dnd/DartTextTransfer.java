 ...
 * @see Transfer
 */
public class DartTextTransfer extends DartByteArrayTransfer implements ITextTransfer {

    static TextTransfer _instance = new TextTransfer();

    static final String ID_NAME = "public.utf8-plain-text";

    static final int ID = registerType(ID_NAME);

    DartTextTransfer(TextTransfer api) {
        super(api);
    }
    ...
    public TextTransfer getApi() {
        if (api == null)
            api = TextTransfer.createApi(this);
        return (TextTransfer) api;
    }

    public VTextTransfer getValue() {
        if (value == null)
            value = new VTextTransfer(this);
        return (VTextTransfer) value;
    }
}
