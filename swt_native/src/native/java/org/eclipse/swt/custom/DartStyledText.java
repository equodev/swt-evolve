...
public class DartStyledText extends DartCanvas implements IStyledText {
    ...
    // workaround p -> p.x lose typing and cannot be chained in comparing...
    ...
    // x and y are start and end caret offsets of selection (x <= y)
    Point[] selection = { new Point(0, 0) };

    // position of selection anchor for the selection ranges. 0 based offset from beginning of text
    int[] selectionAnchors = { 0 };
    ...
    // process line breaks inside logical lines (inserted by BidiSegmentEvent)
    ...
    static class Printing implements Runnable {
        ...
        Printing(StyledText styledText, Printer printer, StyledTextPrintOptions printOptions) {
            ...
            if (scope == PrinterData.PAGE_RANGE) {
                ...
                selection = Arrays.copyOf(((DartStyledText) styledText.getImpl()).selection, ((DartStyledText) styledText.getImpl()).selection.length);
            }
        ...
        void cacheLineData(StyledText styledText) {
            StyledTextRenderer renderer = ((DartStyledText) styledText.getImpl()).renderer;
            ((DartStyledTextRenderer) renderer.getImpl()).copyInto(printerRenderer);
            fontData = styledText.getFont().getFontData()[0];
            tabLength = ((DartStyledText) styledText.getImpl()).tabLength;
            ...
            lineSpacing = ((DartStyledText) styledText.getImpl()).lineSpacing * scaleFactorY;
        ...
        void init() {
            ...
            if (scope == PrinterData.PAGE_RANGE) {
                //WRONG
                int pageSize = clientArea.height / lineHeight;
    ...
    void clearSelection(boolean sendEvent) {
        ...
        if (selectionEnd - selectionStart > 0) {
            ...
            //if (redrawEnd - redrawStart > 0) {    internalRedrawRange(redrawStart, redrawEnd - redrawStart);}
            ;
            if (sendEvent) {
                sendSelectionEvent();
            }
    ...
    boolean copySelection(int type) {
        ...
        try {
            if (blockSelection && blockXLocation != -1) {
                ...
                if (text.length() > 0) {
                    //TODO RTF support
                    TextTransfer plainTextTransfer = DartTextTransfer.getInstance();
                ...
                if (text.length() > 0) {
                    //TODO RTF support
                    clipboard.setContents(new Object[] { text.toString() }, new Transfer[] { DartTextTransfer.getInstance() }, type);
                    return true;
                }
            } else {
    ...
    void doBackspace() {
        ...
        if (Arrays.stream(selection).anyMatch(p -> p.x != p.y)) {
            // at least one range to delete
            for (int i = selection.length - 1; i >= 0; i--) {
                // from bottom to top to avoid moving ranges
            ...
            for (int i = caretOffsets.length - 1; i >= 0; i--) {
                // reverse loop, process from bottom to top
    ...
    void doMouseSelection() {
        if (caretOffsets[0] <= selection[0].x || (caretOffsets[0] > selection[0].x && caretOffsets[0] < selection[0].y && selectionAnchors[0] == selection[0].x)) {
            doSelection(ST.COLUMN_PREVIOUS);
        } else {
            doSelection(ST.COLUMN_NEXT);
        }
    ...
    void doPageEnd() {
        ...
        if (isSingleLine()) {
            doLineEnd();
        } else if (caretOffsets.length == 1) {
            // pageEnd doesn't make sense with multi-carets
            int bottomOffset;
            if (isWordWrap()) {
    ...
    void doSelectionPageUp(int pixels) {
        if (caretOffsets.length > 1) {
            // operation doesn't make sense for multi-carets
            return;
        }
        int oldColumnX = columnX = getPointAtOffset(caretOffsets[0]).x;
    ...
    public int getLineHeight(int offset) {
        ...
        //return height;
        ;
        return StyledTextHelper.getLineHeightAtOffset((DartStyledTextRenderer) renderer.getImpl(), content, offset);
    }
        ...
        if (clickCount == 1) {
            ...
            if (doubleClickEnabled) {
                ...
                if (wordSelect) {
                    ...
                    if (event.x > offsetPoint.x && // Not beyond EOL
                    offset < Math.min(max, lineOffset + lineLength) && !Character.isWhitespace(line.charAt(offset - lineOffset))) {
                        // Not on whitespace
                        offset++;
                    }
    ...
    void handlePaint(Event event) {
        ...
        if (carets != null) {
            for (int i = 1; i < carets.length; i++) {
                //skip 1st caret that's already drawn
    ...
    void initializeAccessible() {
        ...
        accTextExtendedAdapter = new AccessibleTextExtendedAdapter() {
            ...
            public void getSelection(AccessibleTextEvent e) {
                StyledText st = DartStyledText.this.getApi();
                if (((DartStyledText) st.getImpl()).blockSelection && ((DartStyledText) st.getImpl()).blockXLocation != -1) {
            ...
            public void removeSelection(AccessibleTextEvent e) {
                StyledText st = DartStyledText.this.getApi();
                if (e.index == 0) {
                    if (((DartStyledText) st.getImpl()).blockSelection) {
            ...
            public void getTextBounds(AccessibleTextEvent e) {
                ...
                for (int lineIndex = startLine; lineIndex <= endLine; lineIndex++) {
                    ...
                    rect.height = ((DartStyledTextRenderer) ((DartStyledText) st.getImpl()).renderer.getImpl()).getLineHeight(lineIndex);
                    if (lineIndex == startLine) {
                        rect.x = ((DartStyledText) st.getImpl()).getPointAtOffset(start).x;
                    } else {
                        rect.x = ((DartStyledText) st.getImpl()).leftMargin - ((DartStyledText) st.getImpl()).horizontalScrollOffset;
                    }
                    if (lineIndex == endLine) {
                        rect.width = ((DartStyledText) st.getImpl()).getPointAtOffset(end).x - rect.x;
                    } else {
                        TextLayout layout = ((DartStyledTextRenderer) ((DartStyledText) st.getImpl()).renderer.getImpl()).getTextLayout(lineIndex);
                        rect.width = layout.getBounds().width - rect.x;
                        ((DartStyledTextRenderer) ((DartStyledText) st.getImpl()).renderer.getImpl()).disposeTextLayout(layout);
                    }
            ...
            int[] getRanges(int left, int top, int right, int bottom) {
                ...
                for (int lineIndex = lineStart; lineIndex <= lineEnd; lineIndex++) {
                    String line = ((DartStyledText) st.getImpl()).content.getLine(lineIndex);
                    int lineOffset = ((DartStyledText) st.getImpl()).content.getOffsetAtLine(lineIndex);
                    int lineEndOffset = lineOffset + line.length();
                    int linePixel = st.getLinePixel(lineIndex);
                    int start = ((DartStyledText) st.getImpl()).getOffsetAtPoint(left, linePixel, null, true);
                    if (start == -1) {
                        start = left < ((DartStyledText) st.getImpl()).leftMargin ? lineOffset : lineEndOffset;
                    }
                    int[] trailing = new int[1];
                    int end = ((DartStyledText) st.getImpl()).getOffsetAtPoint(right, linePixel, trailing, true);
                    if (end == -1) {
                        end = right < ((DartStyledText) st.getImpl()).leftMargin ? lineOffset : lineEndOffset;
                    } else {
                        end += trailing[0];
                    }
                    if (start > end) {
            ...
            public void getText(AccessibleTextEvent e) {
                ...
                e.end = end;
                e.count = count;
                e.result = ((DartStyledText) st.getImpl()).content.getTextRange(start, end - start);
            }
                    ...
                    case ACC.SCROLL_TYPE_TOP_EDGE:
                        {
                            ...
                            if (e.type != ACC.SCROLL_TYPE_TOP_EDGE) {
                                horizontalPixel = horizontalPixel + rect.x - ((DartStyledText) st.getImpl()).leftMargin;
                            }
                            if (e.type != ACC.SCROLL_TYPE_LEFT_EDGE) {
                                topPixel = topPixel + rect.y - ((DartStyledText) st.getImpl()).topMargin;
                            }
                            break;
                        }
                    ...
                    case ACC.SCROLL_TYPE_RIGHT_EDGE:
                        {
                            ...
                            if (e.type != ACC.SCROLL_TYPE_BOTTOM_EDGE) {
                                horizontalPixel = horizontalPixel - ((DartStyledText) st.getImpl()).clientAreaWidth + rect.x + rect.width + ((DartStyledText) st.getImpl()).rightMargin;
                            }
                            if (e.type != ACC.SCROLL_TYPE_RIGHT_EDGE) {
                                topPixel = topPixel - ((DartStyledText) st.getImpl()).clientAreaHeight + rect.y + rect.height + ((DartStyledText) st.getImpl()).bottomMargin;
                            }
                            break;
                        }
        ...
        accAttributeAdapter = new AccessibleAttributeAdapter() {
            ...
            public void getTextAttributes(AccessibleTextAttributeEvent e) {
                StyledText st = DartStyledText.this.getApi();
                int contentLength = st.getCharCount();
                if (!isListening(ST.LineGetStyle) && ((DartStyledTextRenderer) ((DartStyledText) st.getImpl()).renderer.getImpl()).styleCount == 0) {
                    e.start = 0;
                    e.end = contentLength;
 ...
 */
    ...
    public void setBlockSelectionBounds(int x, int y, int width, int height) {
        ...
        this.blockSelectionBounds = newValue;
    ...
    void setCaretLocations(final Point[] locations, int direction) {
        ...
        if (firstCaret != null) {
            ...
            for (int i = Math.min(caretOffsets.length, locations.length) - 1; i >= 0; i--) {
                // reverse order, seee bug 579028#c7
                final Caret caret = carets[i];
                final int caretOffset = caretOffsets[i];
                final Point location = locations[i];
                final StyleRange styleAtOffset = content.getCharCount() > 0 ? (caretOffset < content.getCharCount() ? getStyleRangeAtOffset(caretOffset) : // caret after last char: use last char style
                getStyleRangeAtOffset(content.getCharCount() - 1)) : null;
    ...
    public void setClipboardContent(int start, int length, int clipboardType) throws SWTError {
        ...
        if (clipboardType == DND.SELECTION_CLIPBOARD) {
            ...
            try {
                RTFTransfer rtfTransfer = DartRTFTransfer.getInstance();
                RTFWriter rtfWriter = new RTFWriter(this.getApi(), start, length);
                String rtfText = getPlatformDelimitedText(rtfWriter);
                HTMLTransfer htmlTransfer = DartHTMLTransfer.getInstance();
                ...
                OutOfMemoryError customOome = new OutOfMemoryError("Out of Memory: Copied only plain text (" + plainText.lines().count() + " lines).");
    ...
    public void setLineIndent(int startLine, int lineCount, int indent) {
        ...
        //redrawLines(startLine, lineCount, oldBottom != newBottom);
        ;
        //if (Arrays.stream(caretOffsets).map(content::getLineAtOffset).anyMatch(caretLine -> startLine <= caretLine && caretLine < startLine + lineCount)) {    setCaretLocations();}
        ;
    }
    ...
    public void setMouseNavigatorEnabled(boolean enabled) {
        ...
        this.mouseNavigatorEnabled = newValue;
        if (enabled) {
    ...
    void setSelection(int[] regions, boolean sendEvent, boolean doBlock) {
        ...
        if (regions.length == 2 && selection.length == 1) {
            ...
            // different?
            if (selection[0].x != start || selection[0].y != end || (length > 0 && selectionAnchor != selection[0].x) || (length < 0 && selectionAnchor != selection[0].y)) {
                if (blockSelection && doBlock) {
                    ...
                    //if (redrawY - redrawX > 0) {    internalRedrawRange(redrawX, redrawY - redrawX);}
                    ;
    ...
    void setStyleRanges(int start, int length, int[] ranges, StyleRange[] styles, boolean reset) {
        ...
        //if (!isFixedLineHeight() && !reset) {    int lineEnd = content.getLineAtOffset(Math.max(end, rangeEnd));    int partialTopIndex = getPartialTopIndex();    int partialBottomIndex = getPartialBottomIndex();    if (partialTopIndex <= lineEnd && lineEnd <= partialBottomIndex) {        expectedBottom = getLinePixel(lineEnd + 1);    }}
        ;
        if (reset) {
    ...
    private SortedSet<Integer> computeModifiedLines(int[] referenceRanges, StyleRange[] referenceStyles, int[] newRanges, StyleRange[] newStyles) {
        ...
        while (currentOffset < content.getCharCount() && (referenceRangeIndex < referenceStyles.length || newRangeIndex < newRanges.length)) {
            // next new range start/end after current offset
            ...
            if (isInRange(referenceRanges, referenceRangeIndex, currentOffset)) {
                // has styling
                ...
                // no range, default styling
                // beginning of next range
    ...
    public void setText(String text) {
        ...
        if (renderer != null) {
            StyledTextHelper.syncRendererLineCount((DartStyledTextRenderer) renderer.getImpl());
        }
    }
        ...
        if (caret != null) {
            ...
            if (blockSelection && blockXLocation != -1) {
                ...
                Arrays.stream(carets).forEach(c -> {
                    ...
                    boolean visible = topMargin <= location.y + size.y && location.y <= clientAreaHeight - bottomMargin && leftMargin <= location.x + size.x && location.x <= clientAreaWidth - rightMargin;
    ...
    void updateSelection(int startOffset, int replacedLength, int newLength) {
        ...
        Arrays.stream(selection).filter(sel -> sel.y > startOffset).filter(sel -> sel.x < startOffset).forEach(sel -> internalRedrawRange(sel.x, startOffset - sel.x));
        Arrays.stream(selection).filter(sel -> sel.y > startOffset).filter(sel -> sel.y > startOffset + replacedLength && sel.x < startOffset + replacedLength).forEach(sel -> {
    ...
    StyleRange[] _styleRanges = new StyleRange[0];
    ...
    public StyleRange[] __styleRanges() {
        return _styleRanges;
    }
