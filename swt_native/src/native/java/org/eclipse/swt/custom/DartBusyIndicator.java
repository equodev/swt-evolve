...
public class DartBusyIndicator implements IBusyIndicator {
    ...
    public static void showWhile(Future<?> future) {
        ...
        if (!future.isDone()) {
            Display display = DartDisplay.getCurrent();
    ...
    public static <V, E extends Exception> CompletableFuture<V> compute(SwtCallable<V, E> action, Executor executor) {
        ...
        Display display = DartDisplay.findDisplay(Thread.currentThread());
        if (display == null) {
            try {
    ...
    protected VBusyIndicator value;

    public VBusyIndicator getValue() {
        if (value == null)
            value = new VBusyIndicator(this);
        return (VBusyIndicator) value;
    }

    public DartBusyIndicator(BusyIndicator api) {
        setApi(api);
    }
}
