 ...
 */
public class DartTray extends DartWidget implements ITray {

    int itemCount;

    TrayItem[] items = new TrayItem[4];

    DartTray(Display display, int style, Tray api) {
        super(api);
    ...
    void releaseParent() {
        super.releaseParent();
        if (((DartDisplay) display.getImpl()).tray == this.getApi())
            ((DartDisplay) display.getImpl()).tray = null;
    }
    ...
    public Tray getApi() {
        if (api == null)
            api = Tray.createApi(this);
        return (Tray) api;
    }

    public VTray getValue() {
        if (value == null)
            value = new VTray(this);
        return (VTray) value;
    }
}
