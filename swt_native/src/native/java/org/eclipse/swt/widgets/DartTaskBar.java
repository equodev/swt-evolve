 ...
 */
public class DartTaskBar extends DartWidget implements ITaskBar {

    int itemCount;

    TaskItem[] items = new TaskItem[4];

    DartTaskBar(Display display, int style, TaskBar api) {
        super(api);
        if (display == null)
    ...
    public TaskItem getItem(Shell shell) {
        ...
        for (int i = 0; i < itemCount; i++) {
            if (items[i] != null && ((DartTaskItem) items[i].getImpl()).shell == shell) {
                return items[i];
            }
        }
        TaskItem item = null;
    ...
    void releaseParent() {
        super.releaseParent();
        if (((DartDisplay) display.getImpl()).taskBar == this.getApi())
            ((DartDisplay) display.getImpl()).taskBar = null;
    }
    ...
    public VTaskBar getValue() {
        if (value == null)
            value = new VTaskBar(this);
        return (VTaskBar) value;
    }
}
