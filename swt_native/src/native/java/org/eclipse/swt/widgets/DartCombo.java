...
public class DartCombo extends DartComposite implements ICombo {
	...
	*/
    ...
    public int getTextHeight() {
        checkWidget();
        return Sizes.computeSize(this, SWT.DEFAULT, SWT.DEFAULT, false).y;
    }
    ...
    @Override
    void releaseWidget() {
        if (((DartDisplay) display.getImpl()).currentCombo == this.getApi()) {
            ((DartDisplay) display.getImpl()).currentCombo = null;
        }
        super.releaseWidget();
        if ((getApi().style & SWT.READ_ONLY) == 0) {
        }
