    ...
    public void layout(Control[] changed, int flags) {
        ...
        if (changed != null) {
            for (int i = 0; i < changed.length; i++) {
                ...
                Composite composite = ((DartControl) control.getImpl()).parent;
                while (composite != null) {
                    ancestor = composite == this.getApi();
                    if (ancestor)
                        break;
                    composite = ((DartControl) composite.getImpl()).parent;
                }
                if (!ancestor)
            ...
            for (int i = 0; i < changed.length; i++) {
                ...
                Composite composite = ((DartControl) child.getImpl()).parent;
                // Update layout when the list of children has changed.
                // See bug 497812.
                child.getImpl().markLayout(false, false);
                while (child != this.getApi()) {
                    if (((DartComposite) composite.getImpl()).layout != null) {
                        composite.state |= LAYOUT_NEEDED;
                        if (!((DartComposite) composite.getImpl()).layout.flushCache(child)) {
                    ...
                    composite = ((DartControl) child.getImpl()).parent;
                }
            }
            if ((flags & SWT.DEFER) != 0) {
                setLayoutDeferred(true);
                ((DartDisplay) display.getImpl()).addLayoutDeferred(this.getApi());
            }
        ...
        if (all) {
            for (Control child : _getChildren()) {
                if (child != null && !child.isDisposed() && child.isVisible())
                    child.redraw(x, y, width, height, true);
            }
        }
    }
    ...
    public void setTabList(Control[] tabList) {
        ...
        if (tabList != null) {
            for (int i = 0; i < tabList.length; i++) {
                ...
                if (control.isDisposed())
                    error(SWT.ERROR_INVALID_ARGUMENT);
                if (((DartControl) control.getImpl()).parent != this.getApi())
                    error(SWT.ERROR_INVALID_PARENT);
            }
