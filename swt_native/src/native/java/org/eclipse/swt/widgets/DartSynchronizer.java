 ...
 */
public class DartSynchronizer implements ISynchronizer {
     ...
     */
    public DartSynchronizer(Display display, Synchronizer api) {
        setApi(api);
    ...
    boolean runAsyncMessages(boolean all) {
        ...
        do {
            ...
            synchronized (lock) {
                ...
                ((DartDisplay) display.getImpl()).sendPreEvent(SWT.None);
                try {
                    lock.run(display);
                } catch (Throwable t) {
                    lock.throwable = t;
                    ...
                    if (display != null && !display.isDisposed()) {
                        ((DartDisplay) display.getImpl()).sendPostEvent(SWT.None);
                    }
                    syncThread = null;
                    lock.notifyAll();
                }
    ...
    public ConcurrentLinkedQueue<RunnableLock> _messages() {
        return messages;
    }

    public Thread _syncThread() {
        return syncThread;
    }
