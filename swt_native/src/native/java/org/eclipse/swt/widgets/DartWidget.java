...
public abstract class DartWidget implements IWidget {
    ...
    void reskinWidget() {
        if ((getApi().state & SKIN_NEEDED) != SKIN_NEEDED) {
            ...
            ((DartDisplay) display.getImpl()).addSkinnableWidget(this.getApi());
        }
    }
	...
	*/
    ...
    boolean filters(int eventType) {
        return ((DartDisplay) display.getImpl()).filters(eventType);
    }
    ...
    boolean isValidSubclass() {
        return DartDisplay.isValidClass(getClass());
    }

    boolean isValidThread() {
        return ((DartDisplay) getDisplay().getImpl()).isValidThread();
    }
    ...
    void releaseWidget() {
        deregister();
        if (((DartDisplay) display.getImpl()).tooltipTarget == this.getApi())
            ((DartDisplay) display.getImpl()).tooltipTarget = null;
        eventTable = null;
        data = null;
    ...
    public void sendEvent(Event event) {
        ((DartDisplay) display.getImpl()).sendEvent(eventTable, event);
    }
    ...
    public void sendEvent(int eventType, Event event, boolean send) {
        if (eventTable == null && !((DartDisplay) display.getImpl()).filters(eventType)) {
            return;
        }
        if (event == null)
            event = new Event();
        ...
        if (event.time == 0) {
            event.time = ((DartDisplay) display.getImpl()).getLastEventTime();
        }
        if (send) {
            sendEvent(event);
        } else {
            ((DartDisplay) display.getImpl()).postEvent(event);
        }
    }
