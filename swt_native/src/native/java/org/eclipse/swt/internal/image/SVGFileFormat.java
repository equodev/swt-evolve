...
public class SVGFileFormat extends FileFormat {
    ...
    static {
        ...
        try {
            rasterizer = ServiceLoader.load(SVGRasterizer.class, SVGFileFormat.class.getClassLoader()).findFirst().orElse(null);
    ...
    List<ElementAtZoom<ImageData>> loadFromByteStream(int fileZoom, int targetZoom) {
        if (RASTERIZER == null) {
            return java.util.Collections.singletonList(new ElementAtZoom<ImageData>(SVGFileFormatHelper.fallbackImageData(inputStream, true), targetZoom));
        }
        if (targetZoom <= 0) {
    ...
    ImageData loadFromByteStreamBySize(int width, int height) {
        if (RASTERIZER == null) {
            return SVGFileFormatHelper.fallbackImageData(inputStream, width, height, true);
        }
        if (width <= 0 || height <= 0) {
