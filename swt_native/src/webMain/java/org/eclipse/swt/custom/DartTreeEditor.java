...
import org.eclipse.swt.custom.VTreeEditor.*;
import dev.equo.swt.*;
 ...
 */
public class DartTreeEditor extends DartControlEditor implements ITreeEditor {
     ...
     */
    public DartTreeEditor(Tree tree, TreeEditor api) {
        super(tree, api);
        this.tree = tree;
        ...
        treeListener = new TreeListener() {
            ...
            final Runnable runnable = () -> {
                if (editor == null || editor.isDisposed())
                    return;
                if (DartTreeEditor.this.tree.isDisposed())
                    return;
                layout();
    ...
    Rectangle computeBounds() {
        ...
        Rectangle editorRect = new Rectangle(cell.x, cell.y, getApi().minimumWidth, getApi().minimumHeight);
        if (getApi().grabHorizontal) {
            ...
            editorRect.width = Math.max(cell.width, getApi().minimumWidth);
        }
        if (getApi().grabVertical) {
            editorRect.height = Math.max(cell.height, getApi().minimumHeight);
        }
        if (getApi().horizontalAlignment == SWT.RIGHT) {
            editorRect.x += cell.width - editorRect.width;
        } else if (getApi().horizontalAlignment == SWT.LEFT) {
        ...
        if (getApi().verticalAlignment == SWT.BOTTOM) {
            editorRect.y += cell.height - editorRect.height;
        } else if (getApi().verticalAlignment == SWT.TOP) {
    ...
    public void dispose() {
        ...
        if (tree != null && !tree.isDisposed() && tree.getImpl() instanceof DartTree) {
            ((DartTree) tree.getImpl())._removeEditor(getApi());
        }
        columnListener = null;
        treeListener = null;
        tree = null;
    ...
    public TreeEditor getApi() {
        ...
        return (TreeEditor) api;
    }

    public VTreeEditor getValue() {
        if (value == null)
            value = new VTreeEditor(this);
        return (VTreeEditor) value;
    }
}
