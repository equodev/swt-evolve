...
class DartCTabFolderLayout extends DartLayout implements ICTabFolderLayout {
    ...
    public void layout(Composite composite, boolean flushCache) {
        ...
        // resize content
        if (((DartCTabFolder) folder.getImpl()).selectedIndex != -1) {
            Control control = ((DartCTabItem) ((DartCTabFolder) folder.getImpl()).items[((DartCTabFolder) folder.getImpl()).selectedIndex].getImpl()).control;
