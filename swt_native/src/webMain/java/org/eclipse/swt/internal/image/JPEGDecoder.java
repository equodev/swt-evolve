...
public class JPEGDecoder {
    ...
    static int[] jpeg_natural_order = { 0, 1, 8, 16, 9, 2, 3, 10, 17, 24, 32, 25, 18, 11, 4, 5, 12, 19, 26, 33, 40, 48, 41, 34, 27, 20, 13, 6, 7, 14, 21, 28, 35, 42, 49, 56, 57, 50, 43, 36, 29, 22, 15, 23, 30, 37, 44, 51, 58, 59, 52, 45, 38, 31, 39, 46, 53, 60, 61, 54, 47, 55, 62, 63, 63, 63, 63, 63, 63, 63, 63, 63, /* extra entries for safety in decoder */
    ...
    static final class d_derived_tbl {
        ...
        byte[] look_sym = new byte[1 << HUFF_LOOKAHEAD];
        /* symbol, or unused */
    }
    ...
    static final class huff_entropy_decoder extends jpeg_entropy_decoder {
        ...
        boolean process_restart(jpeg_decompress_struct cinfo) {
            ...
            for (ci = 0; ci < cinfo.comps_in_scan; ci++) entropy.saved.last_dc_val[ci] = 0;
        ...
        boolean decode_mcu_AC_first(jpeg_decompress_struct cinfo, short[][] MCU_data) {
            ...
            if (!entropy.insufficient_data) {
                ...
                if (EOBRUN > 0)
                    ...
                    EOBRUN--;
                else /* ...process it now (we do nothing) */
                {
                    //					BITREAD_LOAD_STATE(cinfo,entropy.bitstate);
                    br_state.cinfo = cinfo;
                    br_state.buffer = cinfo.buffer;
    ...
    static void jinit_d_coef_controller(jpeg_decompress_struct cinfo, boolean need_full_buffer) {
        ...
        if (need_full_buffer) {
            ...
            for (ci = 0; ci < cinfo.num_components; ci++) {
                ...
                //access_rows *= 3;
                //#endif
                coef.whole_image[ci] = new short[(int) jround_up(compptr.height_in_blocks, compptr.v_samp_factor)][(int) jround_up(compptr.width_in_blocks, compptr.h_samp_factor)][DCTSIZE2];
            }
    ...
    static void prepare_range_limit_table(jpeg_decompress_struct cinfo) /* Allocate and fill in the sample_range_limit table */
    {
        ...
        for (i = 0; i <= MAXJSAMPLE; i++) table[i + offset] = (byte) i;
        offset += CENTERJSAMPLE;
        /* Point to where post-IDCT table starts */
        /* End of simple table, rest of first half of post-IDCT table */
        for (i = CENTERJSAMPLE; i < 2 * (MAXJSAMPLE + 1); i++) table[i + offset] = (byte) MAXJSAMPLE;
        ...
        switch(cinfo.out_color_space) {
            case JCS_GRAYSCALE:
                ...
                if (cinfo.jpeg_color_space == JCS_GRAYSCALE || cinfo.jpeg_color_space == JCS_YCbCr) {
                    ...
                    for (ci = 1; ci < cinfo.num_components; ci++) cinfo.comp_info[ci].component_needed = false;
                } else
                    error();
 ...
 */
    ...
    static void jinit_phuff_decoder(jpeg_decompress_struct cinfo) {
        ...
        for (ci = 0; ci < cinfo.num_components; ci++) for (i = 0; i < DCTSIZE2; i++) coef_bit_ptr[ci][i] = -1;
    }
    ...
    static void jpeg_idct_islow(jpeg_decompress_struct cinfo, jpeg_component_info compptr, short[] coef_block, byte[][] output_buf, int output_buf_offset, int output_col) {
		 ...
		 */
            if (inptr[DCTSIZE * 1 + inptr_offset] == 0 && inptr[DCTSIZE * 2 + inptr_offset] == 0 && inptr[DCTSIZE * 3 + inptr_offset] == 0 && inptr[DCTSIZE * 4 + inptr_offset] == 0 && inptr[DCTSIZE * 5 + inptr_offset] == 0 && inptr[DCTSIZE * 6 + inptr_offset] == 0 && inptr[DCTSIZE * 7 + inptr_offset] == 0) {
                /* AC terms all zero */
		 ...
		 */
            ...
            if (wsptr[1 + wsptr_offset] == 0 && wsptr[2 + wsptr_offset] == 0 && wsptr[3 + wsptr_offset] == 0 && wsptr[4 + wsptr_offset] == 0 && wsptr[5 + wsptr_offset] == 0 && wsptr[6 + wsptr_offset] == 0 && wsptr[7 + wsptr_offset] == 0) {
                /* AC terms all zero */
                //			#define DESCALE(x,n)	RIGHT_SHIFT((x) + (ONE << ((n)-1)), n)
                byte dcval = range_limit[range_limit_offset + ((((wsptr[0 + wsptr_offset]) + (1 << ((PASS1_BITS + 3) - 1))) >> PASS1_BITS + 3) & RANGE_MASK)];
    ...
    static void jcopy_sample_rows(byte[][] input_array, int source_row, byte[][] output_array, int dest_row, int num_rows, int num_cols) /* Copy some rows of samples from one place to another.
 ...
 */
    ...
    static boolean jpeg_resync_to_restart(jpeg_decompress_struct cinfo, int desired) {
        ...
        for (; ; ) {
            ...
            if (marker < M_RST0 || marker > M_RST7)
                action = 3;
            else /* valid non-restart marker */
            {
                if (marker == (M_RST0 + ((desired + 1) & 7)) || marker == (M_RST0 + ((desired + 2) & 7)))
                    action = 3;
                else /* one of the next two expected restarts */
                if (marker == (M_RST0 + ((desired - 1) & 7)) || marker == (M_RST0 + ((desired - 2) & 7)))
                    action = 2;
                else
    ...
    static boolean jpeg_fill_bit_buffer(bitread_working_state state, int get_buffer, int bits_left, int nbits) /* Load up the bit buffer to a depth of at least nbits */
    {
 ...
 */
        ...
        if (!main.buffer_full) {
            int result = switch(cinfo.coef.decompress_data) {
                case DECOMPRESS_DATA ->
                    decompress_data(cinfo, main.xbuffer[main.whichptr], main.xbuffer_offset[main.whichptr]);
                case DECOMPRESS_SMOOTH_DATA ->
                    decompress_smooth_data(cinfo, main.xbuffer[main.whichptr], main.xbuffer_offset[main.whichptr]);
                case DECOMPRESS_ONEPASS ->
                    decompress_onepass(cinfo, main.xbuffer[main.whichptr], main.xbuffer_offset[main.whichptr]);
                default ->
                    0;
            };
            if (result == 0)
    ...
    static void process_data_simple_main(jpeg_decompress_struct cinfo, byte[][] output_buf, int[] out_row_ctr, int out_rows_avail) {
        ...
        if (!main.buffer_full) {
            int result = switch(cinfo.coef.decompress_data) {
                case DECOMPRESS_DATA ->
                    decompress_data(cinfo, main.buffer, main.buffer_offset);
                case DECOMPRESS_SMOOTH_DATA ->
                    decompress_smooth_data(cinfo, main.buffer, main.buffer_offset);
                case DECOMPRESS_ONEPASS ->
                    decompress_onepass(cinfo, main.buffer, main.buffer_offset);
                default ->
                    0;
            };
            if (result == 0)
    ...
    static boolean get_dht(jpeg_decompress_struct cinfo) /* Process a DHT marker */
    {
        int length;
        byte[] bits = new byte[17];
        byte[] huffval = new byte[256];
    ...
    static boolean get_dqt(jpeg_decompress_struct cinfo) /* Process a DQT marker */
    {
        int length;
        int n, i, prec;
        int tmp;
    ...
    static boolean get_dri(jpeg_decompress_struct cinfo) /* Process a DRI marker */
    {
        int length;
        int tmp;
        if (cinfo.bytes_offset == cinfo.bytes_in_buffer)
    ...
    static boolean get_dac(jpeg_decompress_struct cinfo) /* Process a DAC marker */
    {
        int length;
        int index, val;
        if (cinfo.bytes_offset == cinfo.bytes_in_buffer)
    ...
    static boolean get_sos(jpeg_decompress_struct cinfo) /* Process a SOS marker */
    {
        int length;
        int i, ci, n, c, cc;
    ...
    static boolean skip_variable(jpeg_decompress_struct cinfo) /* Skip over an unknown or uninteresting variable-length marker */
    {
        int length;
        if (cinfo.bytes_offset == cinfo.bytes_in_buffer)
            fill_input_buffer(cinfo);
    ...
    static boolean get_interesting_appn(jpeg_decompress_struct cinfo) /* Process an APP0 or APP14 marker without saving it */
    {
        int length;
 ...
 */
    {
        ...
        if (datalen >= APP0_DATA_LEN && (data[0] & 0xFF) == 0x4A && (data[1] & 0xFF) == 0x46 && (data[2] & 0xFF) == 0x49 && (data[3] & 0xFF) == 0x46 && (data[4] & 0xFF) == 0) {
 ...
 */
    {
        int /*version, flags0, flags1, */
        transform;
        if (datalen >= APP14_DATA_LEN && (data[0] & 0xFF) == 0x41 && (data[1] & 0xFF) == 0x64 && (data[2] & 0xFF) == 0x6F && (data[3] & 0xFF) == 0x62 && (data[4] & 0xFF) == 0x65) {
    ...
    static ImageData[] loadFromByteStream(InputStream inputStream, ImageLoader loader) {
        ...
        ImageData imageData = ImageData.internal_new(cinfo.output_width, cinfo.output_height, palette.isDirect ? 24 : 8, palette, scanlinePad, data, 0, null, null, -1, -1, SWT.IMAGE_JPEG, 0, 0, 0, 0);
        if (cinfo.buffered_image) {
            boolean done;
            do {
