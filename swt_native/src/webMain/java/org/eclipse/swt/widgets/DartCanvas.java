...
public class DartCanvas extends DartComposite implements ICanvas {
    ...
    boolean imeInComposition() {
        return ime != null && ((DartIME) ime.getImpl()).isInlineEnabled() && ((DartIME) ime.getImpl()).startOffset != -1;
    }
    ...
    public void scroll(int destX, int destY, int x, int y, int width, int height, boolean all) {
        ...
        boolean redraw = control != null && ((DartControl) control.getImpl()).backgroundImage != null;
        if (!redraw)
            redraw = hasRegion();
        if (!redraw)
            redraw = isObscured();
        if (!redraw && sourceRect.intersects(clientRect)) {
            getShell().getImpl().setScrolling();
            redraw = !update(all);
        }
        if (redraw) {
        } else {
    ...
    void setOpenGLContext(Object value) {
        ...
        {
            ((DartShell) shell.getImpl()).glContextCount--;
        }
        shell.getImpl().updateOpaque();
    }
    ...
    public void redraw() {
        super.redraw();
        if (this.getClass() == DartCanvas.class)
