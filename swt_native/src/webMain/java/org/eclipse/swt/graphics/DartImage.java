...
public final class DartImage extends DartResource implements Drawable, IImage {
    ...
    public boolean equals(Object object) {
        ...
        if (!(object instanceof Image image))
            return false;
        if (device != ((DartResource) image.getImpl()).device || alphaInfo_100.transparentPixel != ((DartImage) image.getImpl()).alphaInfo_100.transparentPixel)
            return false;
        if (imageDataProvider != null && ((DartImage) image.getImpl()).imageDataProvider != null) {
            ...
            return styleFlag == ((DartImage) image.getImpl()).styleFlag && imageGcDrawer.equals(((DartImage) image.getImpl()).imageGcDrawer) && width == ((DartImage) image.getImpl()).width && height == ((DartImage) image.getImpl()).height;
        } else {
        }
        return false;
    }
    ...
    public long internal_new_GC(GCData data) {
        ...
        try {
            if (data != null) {
                ...
                data.device = device;
                data.font = ((DartDevice) device.getImpl()).systemFont;
    ...
    private class CachedImageAtSize {
        ...
        private boolean isReusable(int width, int height) {
            return image != null && ((DartImage) image.getImpl()).height == height && ((DartImage) image.getImpl()).width == width;
        }
