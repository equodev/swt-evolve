...
class SliderSerializeTest extends SerializeTestBase {

    @Test
    void should_serialize_empty_Slider() {
        Slider w = new Slider(shell(), SWT.NONE);
        String json = serialize(w);
        JsonMapAssert assertJ = assertThatJson(json).isObject();
        assertJ.containsEntry("id", w.hashCode())
               .containsEntry("swt", "Slider");
    }
    ...
    void should_serialize_filled_Slider() {
        ...
        assertJ.satisfies(node("thumb").equalsTo(w.getThumb(), orAbsentIf0));
    ...
    @Test
    void should_name_every_change_Slider() {
        Slider w = new Slider(shell(), SWT.NONE);
        assertNamesEveryChange(w);
    }

    VSlider value(Slider w) {
        return ((DartSlider) w.getImpl()).getValue();
    }
}
