...
public class FontSizeTest extends SizeAssert {
    ...
    /**
     * Seed for the font sizes and texts this class fuzzes over. Random per run on purpose — the
     * point is to cover different sizes across runs — but printed, and settable with
     * {@code -Dfontsize.seed=<n>}, so a run that goes red can be repeated with the same cases.
     * Without that the sizes that failed are gone by the next invocation.
     */
    private static final long SEED = Long.getLong("fontsize.seed", new Random().nextLong());

    @BeforeAll
    void announceSeed() {
        System.out.println("FontSizeTest seed: " + SEED + "  (reproduce with -Dfontsize.seed=" + SEED + ")");
    }

    public IntStream sizes() {
        return IntStream.concat(IntStream.of(10), new Random(SEED).ints(4,50).filter(i -> i != 10)
                .distinct()
                .limit(4)
                .sorted());
    }
