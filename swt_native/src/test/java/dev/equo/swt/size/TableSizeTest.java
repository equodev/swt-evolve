...
@Tag("flutter-it")
@Tag("metal")
class TableSizeTest extends SizeTestBase {
    ...
    void header_height_matches_flutter(boolean headerVisible, int columnCount) {
        Table w = new Table(dartShell(), SWT.NONE);
        DartTable impl = (DartTable) w.getImpl();
        impl.setHeaderVisible(headerVisible);
        for (int i = 0; i < columnCount; i++) new TableColumn(w, SWT.NONE);
        int javaHeight = TableSizes.getHeaderHeight(impl);
        ...
        if (javaHeight == 0) {
            assertThat(header == null || header.y() == 0).as("header not rendered when hidden")
                                                         .isTrue();
        } else {
            assertThat(header).as("namedSizes.header")
                              .isNotNull();
            assertThat(header.y()).as("header height")
                                  .isEqualTo((double) javaHeight);
        }
    }
    ...
    void full_height_matches_flutter(boolean headerVisible, int columnCount, int itemCount) {
        ...
        impl.setHeaderVisible(headerVisible);
        for (int i = 0; i < columnCount; i++) new TableColumn(w, SWT.NONE);
        for (int i = 0; i < itemCount; i++) new TableItem(w, SWT.NONE);
        int javaHeight = TableSizes.getPreferredHeight(impl);
        CompletableFuture<Measure> result = flutter.measureUnbounded(impl);
        Measure measure = assertCompletes(flutter, result);
        assertThat(measure.widget.y).as("widget height")
                                    .isEqualTo(javaHeight);
    }
