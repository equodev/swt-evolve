    ...
    public Optional<String> openDialog() {
		 ...
		 */
            if (!name.contains("*.")) {
                ...
                int[] result = OS.readRegistryDwords(OS.HKEY_CURRENT_USER, "Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\Advanced", "HideFileExt");
        ...
        if ((style & (SWT.APPLICATION_MODAL | SWT.SYSTEM_MODAL)) != 0) {
            oldModal = ((DartDisplay) display.getImpl()).getModalDialog();
            ((DartDisplay) display.getImpl()).setModalDialog(this.getApi());
        }
        ///* Open the dialog */display.externalEventLoop = true;
        ;
        ...
        if ((style & (SWT.APPLICATION_MODAL | SWT.SYSTEM_MODAL)) != 0) {
            ((DartDisplay) display.getImpl()).setModalDialog(oldModal);
        }
