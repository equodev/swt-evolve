    ...
    protected void _hookEvents() {
        ...
        FlutterBridge.on(this, "CloseWindow", "close", e -> {
            getDisplay().asyncExec(() -> {
                if (isDisposed())
                    return;
                org.eclipse.swt.browser.WindowEvent event = new org.eclipse.swt.browser.WindowEvent(getApi());
        ...
        FlutterBridge.on(this, "VisibilityWindow", "hide", e -> {
            getDisplay().asyncExec(() -> {
                if (isDisposed())
                    return;
                org.eclipse.swt.browser.WindowEvent event = new org.eclipse.swt.browser.WindowEvent(getApi()); // org.eclipse.swt.browser.VisibilityWindowListener
