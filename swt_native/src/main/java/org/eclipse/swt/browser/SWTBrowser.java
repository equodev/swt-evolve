 ...
 */
public class SWTBrowser extends SWTComposite implements IBrowser {
     ...
     * @since 3.0
     */
    public SWTBrowser(SWTComposite parent, int style) {
        super(checkParent(parent), checkStyle(style));
        userStyle = style;
        String platform = SWT.getPlatform();
        if ("gtk".equals(platform)) {
            //$NON-NLS-1$
            ((SWTDisplay) (parent.getDisplay())).setData(NO_INPUT_METHOD, null);
        }
        style = getStyle();
    ...
    static SWTComposite checkParent(SWTComposite parent) {
        String platform = SWT.getPlatform();
        //$NON-NLS-1$
        if (!"gtk".equals(platform))
            return parent;
