...
class ViewFormLayout extends Layout {
    ...
    protected Point computeSize(IComposite composite, int wHint, int hHint, boolean flushCache) {
        SWTViewForm form = (SWTViewForm) ((SWTViewForm) composite);
        SWTControl left = (SWTControl) (form.getTopLeft());
        SWTControl center = (SWTControl) (form.getTopCenter());
        SWTControl right = (SWTControl) (form.getTopRight());
        SWTControl content = (SWTControl) (form.getContent());
    ...
    protected void layout(IComposite composite, boolean flushCache) {
        ...
        if (form.separateTopCenter || minTopWidth > rect.width) {
            ...
            if (left != null && !left.isDisposed()) {
                top = true;
                Rectangle trim = left instanceof SWTComposite ? ((SWTComposite) left).computeTrim(0, 0, 0, 0) : new Rectangle(0, 0, 0, 0);
