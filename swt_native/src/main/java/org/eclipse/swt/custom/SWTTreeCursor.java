 ...
 * @since 3.8
 */
public class SWTTreeCursor extends SWTCanvas implements ITreeCursor {

    SWTTree tree;

    SWTTreeItem row;

    SWTTreeColumn column;
     ...
     * @see Widget#getStyle()
     */
    public SWTTreeCursor(SWTTree parent, int style) {
        super(parent, style);
        tree = parent;
        setBackground(null);
        setForeground(null);
        listener = event -> {
			 ...
			 */
                ...
                SWTTreeItem current = row;
                SWTTreeItem parentItem = ((SWTTreeItem) (row.getParentItem()));
                while (parentItem != null && !parentItem.getExpanded()) {
                    current = parentItem;
                    parentItem = ((SWTTreeItem) (current.getParentItem()));
                }
                if (current != row) {
        ...
        disposeItemListener = event -> {
            SWTTreeItem currentItem = row;
            while (currentItem != null) {
                currentItem.removeListener(SWT.Dispose, disposeItemListener);
                currentItem = ((SWTTreeItem) (currentItem.getParentItem()));
            }
            SWTTreeItem disposedItem = (SWTTreeItem) event.widget.delegate;
            SWTTreeItem parentItem = ((SWTTreeItem) (disposedItem.getParentItem()));
            if (parentItem != null) {
                ...
                if (tree.getItemCount() == 1) {
                    ...
                    SWTTreeItem newFocus = null;
                    int rowIndex = tree.indexOf(disposedItem);
                    if (rowIndex != 0) {
                        SWTTreeItem previousItem = ((SWTTreeItem) (tree.getItem(rowIndex - 1)));
                        if (!previousItem.isDisposed()) {
                            newFocus = previousItem;
                        }
                    }
                    if (newFocus == null && rowIndex + 1 < tree.getItemCount()) {
                        SWTTreeItem nextItem = ((SWTTreeItem) (tree.getItem(rowIndex + 1)));
            ...
            if (column != null) {
                if (tree.getColumnCount() == 1) {
                    ...
                    if (positionIndex == columnOrder.length - 1) {
                        setRowColumn(row, ((SWTTreeColumn) (tree.getColumn(columnOrder[positionIndex - 1]))), true);
                    } else {
                        setRowColumn(row, ((SWTTreeColumn) (tree.getColumn(columnOrder[positionIndex + 1]))), true);
                    }
                }
        ...
        SWTScrollBar hBar = ((SWTScrollBar) (tree.getHorizontalBar()));
        if (hBar != null) {
            hBar.addListener(SWT.Selection, resizeListener);
        }
        SWTScrollBar vBar = ((SWTScrollBar) (tree.getVerticalBar()));
        if (vBar != null) {
    ...
    SWTTreeItem findItem(SWTTreeItem[] items, Point pt) {
        ...
        int index = end / 2;
        while (end - start > 1) {
            SWTTreeItem currentItem = (SWTTreeItem) (items[index]);
    ...
    public Color getBackground() {
        checkWidget();
        if (background == null) {
            return ((SWTDisplay) (getDisplay())).getSystemColor(BACKGROUND);
        }
        return background;
    }
    ...
    public Color getForeground() {
        checkWidget();
        if (foreground == null) {
            return ((SWTDisplay) (getDisplay())).getSystemColor(FOREGROUND);
        }
        return foreground;
    }
    ...
    SWTTreeItem getNextItem(SWTTreeItem item) {
        if (item == null)
            return null;
        ...
        while (parentItem != null) {
            ...
            if (index < parentItem.getItemCount() - 1) {
                return ((SWTTreeItem) (parentItem.getItem(index + 1)));
            }
            item = parentItem;
            parentItem = ((SWTTreeItem) (item.getParentItem()));
        }
        ...
        if (index == tree.getItemCount() - 1)
            return null;
        return ((SWTTreeItem) (tree.getItem(index + 1)));
    }

    SWTTreeItem getPreviousItem(SWTTreeItem item) {
        if (item == null)
            return null;
        SWTTreeItem parentItem = (SWTTreeItem) (item.getParentItem());
        if (parentItem == null) {
    ...
    void keyDown(Event event) {
        ...
        switch(event.keyCode) {
            case SWT.ARROW_UP:
                SWTTreeItem previousItem = (SWTTreeItem) (getPreviousItem(row));
                if (previousItem != null) {
                    setRowColumn(previousItem, column, true);
                }
                break;
            case SWT.ARROW_DOWN:
                SWTTreeItem nextItem = (SWTTreeItem) (getNextItem(row));
                if (nextItem != null) {
            ...
            case SWT.ARROW_RIGHT:
                {
                    ...
                    SWTTreeItem parentRow = (SWTTreeItem) (row.getParentItem());
                    int rowIndex = tree.indexOf(row);
                    if (event.keyCode == leadKey) {
                        if (parentRow != null) {
                            setRowColumn(row, ((SWTTreeColumn) (tree.getColumn(columnOrder[Math.max(0, index - 1)]))), true);
                        } else {
                            setRowColumn(rowIndex, columnOrder[Math.max(0, index - 1)], true);
                        }
                    } else {
                        if (parentRow != null) {
                            setRowColumn(row, ((SWTTreeColumn) (tree.getColumn(columnOrder[Math.min(columnCount - 1, index + 1)]))), true);
                        } else {
            ...
            case SWT.PAGE_UP:
                {
                    ...
                    if (index - page <= 0) {
                        SWTTreeItem first = (SWTTreeItem) (tree.getItem(0));
    ...
    void setRowColumn(int row, int column, boolean notify) {
        SWTTreeItem item = (SWTTreeItem) (row == -1 ? null : tree.getItem(row));
        SWTTreeColumn col = (SWTTreeColumn) (column == -1 || tree.getColumnCount() == 0 ? null : tree.getColumn(column));
        setRowColumn(item, col, notify);
    }

    void setRowColumn(SWTTreeItem row, SWTTreeColumn column, boolean notify) {
        if (this.row != null && this.row != row) {
            SWTTreeItem currentItem = this.row;
            while (currentItem != null) {
                currentItem.removeListener(SWT.Dispose, disposeItemListener);
                currentItem = ((SWTTreeItem) (currentItem.getParentItem()));
            }
        ...
        if (row != null) {
            if (this.row != row) {
                this.row = row;
                SWTTreeItem currentItem = (SWTTreeItem) (row);
                while (currentItem != null) {
     ...
     * </ul>
     */
    public void setSelection(ITreeItem row_, int column) {
        SWTTreeItem row = (SWTTreeItem) row_;
        checkWidget();
        int columnCount = tree.getColumnCount();
        int maxColumnIndex = columnCount == 0 ? 0 : columnCount - 1;
        ...
        SWTTreeColumn col = (SWTTreeColumn) (tree.getColumnCount() == 0 ? null : tree.getColumn(column));
    ...
    void treeCollapse(Event event) {
        if (row == null)
            return;
        SWTTreeItem root = (SWTTreeItem) ((SWTTreeItem) (SWTTreeItem) (event.item != null ? event.item.delegate : null));
        SWTTreeItem parentItem = (SWTTreeItem) (row.getParentItem());
        while (parentItem != null) {
            if (parentItem == root) {
            ...
            parentItem = ((SWTTreeItem) (parentItem.getParentItem()));
        }
    ...
    void treeMouseDown(Event event) {
        ...
        SWTTreeItem item = (SWTTreeItem) (tree.getItem(pt));
        if (item == null && (tree.getStyle() & SWT.FULL_SELECTION) == 0) {
            SWTTreeItem currentItem = (SWTTreeItem) (tree.getTopItem());
            SWTTreeItem parentItem = (SWTTreeItem) (currentItem.getParentItem());
            while (parentItem != null) {
                currentItem = parentItem;
                parentItem = ((SWTTreeItem) (currentItem.getParentItem()));
            }
        ...
        if (item == null)
            return;
        SWTTreeColumn newColumn = (SWTTreeColumn) (null);
        ...
        if (columnCount > 0) {
            for (int i = 0; i < columnCount; i++) {
                ...
                if (rect.contains(pt)) {
                    newColumn = ((SWTTreeColumn) (tree.getColumn(i)));
                    break;
                }
            }
            if (newColumn == null) {
                newColumn = ((SWTTreeColumn) (tree.getColumn(0)));
            }
        }
