 ...
 */
public class SWTCCombo extends SWTComposite implements ICCombo {
     ...
     * @see Widget#getStyle()
     */
    public SWTCCombo(SWTComposite parent, int style) {
        super(parent, style = checkStyle(style));
        _shell = ((SWTShell) (super.getShell()));
        listener = event -> {
            if (isDisposed())
                return;
            if (Widget.getInstance(popup) == event.widget) {
                popupEvent(event);
                return;
            }
            if (Widget.getInstance(text) == event.widget) {
                textEvent(event);
                return;
            }
            if (Widget.getInstance(list) == event.widget) {
                listEvent(event);
                return;
            }
            if (Widget.getInstance(arrow) == event.widget) {
                arrowEvent(event);
                return;
            }
            if (Widget.getInstance(SWTCCombo.this) == event.widget) {
                comboEvent(event);
                return;
            }
            if (Widget.getInstance(((SWTShell) (getShell()))) == event.widget) {
                ((SWTDisplay) (getDisplay())).asyncExec(() -> {
                    if (isDisposed())
                        return;
        ...
        if ((style & SWT.FLAT) != 0)
            arrowStyle |= SWT.FLAT;
        arrow = new SWTButton(this, arrowStyle);
        filter = event -> {
            if (isDisposed())
                return;
        ...
        for (int comboEvent : comboEvents) this.addListener(comboEvent, listener);
        int[] arrowEvents = { SWT.DragDetect, SWT.MouseDown, SWT.MouseEnter, SWT.MouseExit, SWT.MouseHover, SWT.MouseMove, SWT.MouseUp, SWT.MouseWheel, SWT.Selection, SWT.FocusIn };
        for (int arrowEvent : arrowEvents) arrow.addListener(arrowEvent, listener);
        createPopup(null, -1);
        if ((style & SWT.SIMPLE) == 0) {
            int itemHeight = list.getItemHeight();
            if (itemHeight != 0) {
                int maxHeight = ((SWTMonitor) (getMonitor())).getClientArea().height / 3;
    ...
    void createText(int comboStyle) {
        ...
        text = new SWTText(this, textStyle);
        if (textValue != null) {
        ...
        int[] textEvents = { SWT.DefaultSelection, SWT.DragDetect, SWT.KeyDown, SWT.KeyUp, SWT.MenuDetect, SWT.Modify, SWT.MouseDown, SWT.MouseUp, SWT.MouseDoubleClick, SWT.MouseEnter, SWT.MouseExit, SWT.MouseHover, SWT.MouseMove, SWT.MouseWheel, SWT.Traverse, SWT.FocusIn, SWT.Verify };
        for (int textEvent : textEvents) text.addListener(textEvent, listener);
    }
    ...
    void arrowEvent(Event event) {
        switch(event.type) {
            ...
            case SWT.MouseHover:
                {
                    Point pt = ((SWTDisplay) (getDisplay())).map(arrow, this, event.x, event.y);
                    event.x = pt.x;
                    event.y = pt.y;
    ...
    void createPopup(String[] items, int selectionIndex) {
        ...
        popup = new SWTShell(((SWTShell) (getShell())), SWT.NO_TRIM | SWT.ON_TOP);
        int style = getStyle();
        ...
        list = new SWTList(popup, listStyle);
        if (font != null)
            list.setFont(font);
        if (foreground != null)
        ...
        for (int popupEvent : popupEvents) popup.addListener(popupEvent, listener);
    ...
    void dropDown(boolean drop) {
        ...
        if (!isVisible())
            return;
        if (((SWTShell) (getShell())) != ((SWTComposite) (popup.getParent()))) {
        ...
        Rectangle displayRect = ((SWTMonitor) (getMonitor())).getClientArea();
        ...
        Rectangle parentRect = display.map(((SWTComposite) (getParent())), null, getBounds());
	 ...
	 */
        SWTScrollBar hBar = (SWTScrollBar) (list.getHorizontalBar());
 ...
 */
    ...
    public IShell getShell() {
        checkWidget();
        SWTShell shell = (SWTShell) (super.getShell());
        if (shell != _shell) {
    ...
    void handleScroll(Event event) {
        SWTScrollBar scrollBar = (SWTScrollBar) ((SWTScrollBar) (SWTScrollBar) (event.widget != null ? event.widget.delegate : null));
        SWTControl scrollableParent = (SWTControl) (scrollBar.getParent());
    ...
    void initAccessible() {
        ...
        getAccessible().addAccessibleControlListener(new AccessibleControlAdapter() {
            ...
            public void getLocation(AccessibleControlEvent e) {
                ...
                Point pt = ((SWTComposite) (getParent())).toDisplay(location.x, location.y);
                e.x = pt.x;
                e.y = pt.y;
    ...
    public boolean isFocusControl() {
        checkWidget();
        Predicate<SWTControl> checkFocusControl = control -> (control != null && !control.isDisposed() && control.isFocusControl());
    ...
    boolean isParentScrolling(SWTControl scrollableParent) {
        SWTControl parent = (SWTControl) (this.getParent());
        while (parent != null) {
            if (parent.equals(scrollableParent))
                return true;
            parent = ((SWTComposite) (parent.getParent()));
        }
        return false;
    }
    ...
    void listEvent(Event event) {
        switch(event.type) {
			 ...
			 */
                    Point point = arrow.toControl(((SWTDisplay) (getDisplay())).getCursorLocation());
                    Point size = arrow.getSize();
                    Rectangle rect = new Rectangle(0, 0, size.x, size.y);
                    if (rect.contains(point)) {
                        boolean comboShellActivated = ((SWTShell) (getDisplay().getActiveShell())) == ((SWTShell) (getShell()));
    ...
    void popupEvent(Event event) {
        switch(event.type) {
            case SWT.Paint:
                ...
                Rectangle listRect = list.getBounds();
                Color black = ((SWTDisplay) (getDisplay())).getSystemColor(SWT.COLOR_BLACK);
    ...
    void textEvent(Event event) {
        switch(event.type) {
            ...
            case SWT.MouseHover:
                {
                    Point pt = ((SWTDisplay) (getDisplay())).map(text, this, event.x, event.y);
                    event.x = pt.x;
                    event.y = pt.y;
                    notifyListeners(event.type, event);
                    event.type = SWT.None;
            ...
            case SWT.MenuDetect:
                {
                    ...
                    if (event.detail == SWT.MENU_KEYBOARD) {
                        Point pt = ((SWTDisplay) (getDisplay())).map(text, null, text.getCaretLocation());
                        e.x = pt.x;
                        e.y = pt.y;
                    }
     ...
     */
    public static void updateAndRefreshChildren(SWTCCombo combo, Consumer<SWTWidget> childUpdater) {
