...
public class SWTShell extends SWTDecorations implements IShell {
     ...
     * </ul>
     */
    public SWTShell(SWTDisplay display) {
     ...
     * @see SWT#SHEET
     */
    public SWTShell(SWTDisplay display, int style) {
        this(display, null, style, 0, false);
    }

    SWTShell(SWTDisplay display, SWTShell parent, int style, long handle, boolean embedded) {
        super();
        checkSubclass();
        if (display == null)
     ...
     * </ul>
     */
    public SWTShell(SWTShell parent) {
     ...
     * @see SWT#SHEET
     */
    public SWTShell(SWTShell parent, int style) {
     ...
     */
    public static SWTShell gtk_new(SWTDisplay display, long handle) {
        return new SWTShell(display, null, SWT.NO_TRIM, handle, true);
    }
     ...
     * @since 3.3
     */
    public static SWTShell internal_new(SWTDisplay display, long handle) {
        return new SWTShell(display, null, SWT.NO_TRIM, handle, false);
    }

    static int checkStyle(SWTShell parent, int style) {
    ...
    void addToolTip(SWTToolTip toolTip) {
        if (toolTips == null)
            toolTips = new SWTToolTip[4];
        ...
        SWTToolTip[] newToolTips = new SWTToolTip[toolTips.length + 4];
	...
	*/
    ...
    void bringToTop(boolean force) {
        ...
        SWTShell activeShell = display.activeShell;
        if (activeShell == this)
            return;
        if (!force) {
            if (activeShell == null)
	...
	*/
    ...
    void center() {
        ...
        Rectangle monitorRect = DPIUtil.autoScaleUp(((SWTMonitor) (parent.getMonitor())).getClientArea());
    ...
    void createHandle(int index) {
        ...
        if (shellHandle == 0) {
            ...
            if (isChildShell) {
				 ...
				 */
                    if (OS.isWayland()) {
                        SWTComposite topLevelParent = (SWTComposite) (parent);
                        while (topLevelParent != null && (topLevelParent.style & SWT.ON_TOP) != 0) {
                            topLevelParent = ((SWTComposite) (parent.getParent()));
                        }
	...
	*/
    ...
    @Override
    SWTControl findBackgroundControl() {
        return (state & BACKGROUND) != 0 || backgroundImage != null ? this : null;
    }

    @Override
    SWTComposite findDeferredControl() {
    ...
    void hookEvents() {
        super.hookEvents();
        OS.g_signal_connect(shellHandle, OS.dpi_changed, display.notifyProc, SWTWidget.DPI_CHANGED);
        if (GTK.GTK4) {
            ...
            OS.g_signal_connect(shellHandle, OS.notify_default_height, display.notifyProc, SWTWidget.NOTIFY_DEFAULT_HEIGHT);
            OS.g_signal_connect(shellHandle, OS.notify_default_width, display.notifyProc, SWTWidget.NOTIFY_DEFAULT_WIDTH);
            OS.g_signal_connect(shellHandle, OS.notify_maximized, display.notifyProc, SWTWidget.NOTIFY_MAXIMIZED);
        } else {
    ...
    void fixActiveShell() {
        ...
        if (display.activeShell == this && (style & SWT.ON_TOP) != 0) {
            SWTShell shell = (SWTShell) (null);
            if (parent != null && parent.isVisible())
                shell = ((SWTShell) (parent.getShell()));
    ...
    void fixShell(SWTShell newShell, SWTControl control) {
        if (this == newShell)
            return;
    ...
    SWTShell getModalShell() {
        SWTShell shell = (SWTShell) (null);
        SWTShell[] modalShells = display.modalShells;
        if (modalShells != null) {
            int bits = SWT.APPLICATION_MODAL | SWT.SYSTEM_MODAL;
            int index = modalShells.length;
            while (--index >= 0) {
                SWTShell modal = (SWTShell) (modalShells[index]);
                if (modal != null) {
                    if ((modal.style & bits) != 0) {
                        SWTControl control = (SWTControl) (this);
                        while (control != null) {
                            if (control == modal)
                                break;
    ...
    public IShell[] getShells() {
        ...
        int count = 0;
        SWTShell[] shells = (SWTShell[]) (display.getShells());
        for (SWTShell activeshell : shells) {
            SWTControl shell = (SWTControl) (activeshell);
            do {
                shell = ((SWTComposite) (shell.getParent()));
            } while (shell != null && shell != this);
            if (shell == this)
                count++;
        }
        int index = 0;
        SWTShell[] result = new SWTShell[count];
        for (SWTShell activeshell : shells) {
            SWTControl shell = (SWTControl) (activeshell);
            do {
                shell = ((SWTComposite) (shell.getParent()));
    ...
    long gtk_focus(long widget, long directionType) {
        switch((int) directionType) {
            ...
            case GTK.GTK_DIR_TAB_BACKWARD:
                SWTControl control = (SWTControl) (display.getFocusControl());
                if (control != null) {
                    if ((control.state & CANVAS) != 0 && (control.style & SWT.EMBEDDED) != 0 && ((SWTShell) (control.getShell())) == this) {
    ...
    long gtk_key_press_event(long widget, long event) {
        if (widget == shellHandle) {
            ...
            if (menuBar != null && !menuBar.isDisposed()) {
                SWTControl focusControl = (SWTControl) (display.getFocusControl());
	 ...
	 */
        ...
        if ((!resized || oldWidth != width || oldHeight != height) && (OS.isWayland() ? ((style & SWT.RESIZE) != 0) : true)) {
            oldWidth = width;
            oldHeight = height;
	 ...
	 */
        ...
        if (restored) {
            SWTControl focusControl = (SWTControl) (display.getFocusControl());
            if (focusControl instanceof SWTButton && (focusControl.style & SWT.PUSH) != 0) {
                restored = false;
            }
        }
        if (!restored) {
    ...
    void removeTooTip(SWTToolTip toolTip) {
        if (toolTips == null)
            return;
        for (int i = 0; i < toolTips.length; i++) {
    ...
    void reskinChildren(int flags) {
        ...
        if (toolTips != null) {
            for (int i = 0; i < toolTips.length; i++) {
                SWTToolTip toolTip = (SWTToolTip) (toolTips[i]);
                if (toolTip != null)
    ...
    void setActiveControl(SWTControl control) {
        setActiveControl(control, SWT.None);
    }

    void setActiveControl(SWTControl control, int type) {
	...
	* control.
	*/
        SWTControl[] activate = (SWTControl[]) ((control == null) ? new SWTControl[0] : control.getPath());
        SWTControl[] deactivate = (SWTControl[]) ((lastActive == null) ? new SWTControl[0] : lastActive.getPath());
        lastActive = control;
	...
	*/
    ...
    public void setEnabled(boolean enabled) {
        ...
        if (((state & DISABLED) == 0) == enabled)
            return;
        SWTDisplay display = this.display;
        SWTControl control = (SWTControl) (null);
        boolean fixFocus = false;
        if (!enabled) {
            if (display.focusEvent != SWT.FocusOut) {
                control = ((SWTControl) (display.getFocusControl()));
    ...
    @Override
    public void setMenuBar(IMenu menu_) {
 ...
 */
	...
	*/
    ...
    public void setVisible(boolean visible) {
		 ...
		 */
            if (parent != null && ((SWTShell) (parent.getShell())).getFullScreen()) {
		...
		*/
		 ...
		 */
            ...
            if (!(!GTK.GTK4 && OS.isX11() && GTK.GTK_IS_PLUG(shellHandle))) {
                display.dispatchEvents = new int[] { GDK.GDK_EXPOSE, GDK.GDK_FOCUS_CHANGE, GDK.GDK_CONFIGURE, GDK.GDK_MAP, GDK.GDK_UNMAP, GDK.GDK_NO_EXPOSE, GDK.GDK_WINDOW_STATE };
                SWTDisplay display = this.display;
                display.putGdkEvents();
                boolean iconic = false;
                SWTShell shell = (SWTShell) (parent != null ? parent.getShell() : null);
                do {
                    if (GTK.GTK4) {
	...
	*/
    ...
    int trimHeight() {
        ...
        if (hasTitle) {
            if (hasResize)
                return display.trimHeights[SWTDisplay.TRIM_TITLE_RESIZE];
            if (hasBorder)
                return display.trimHeights[SWTDisplay.TRIM_TITLE_BORDER];
            return display.trimHeights[SWTDisplay.TRIM_TITLE];
        }
        if (hasResize)
            return display.trimHeights[SWTDisplay.TRIM_RESIZE];
        if (hasBorder)
            return display.trimHeights[SWTDisplay.TRIM_BORDER];
        return display.trimHeights[SWTDisplay.TRIM_NONE];
    }
    ...
    int trimWidth() {
        ...
        if (hasTitle) {
            if (hasResize)
                return display.trimWidths[SWTDisplay.TRIM_TITLE_RESIZE];
            if (hasBorder)
                return display.trimWidths[SWTDisplay.TRIM_TITLE_BORDER];
            return display.trimWidths[SWTDisplay.TRIM_TITLE];
        }
        if (hasResize)
            return display.trimWidths[SWTDisplay.TRIM_RESIZE];
        if (hasBorder)
            return display.trimWidths[SWTDisplay.TRIM_BORDER];
        return display.trimWidths[SWTDisplay.TRIM_NONE];
    }
    ...
    void updateModal() {
        ...
        if (display.getModalDialog() == null) {
            SWTShell modal = (SWTShell) (getModalShell());
            int mask = SWT.PRIMARY_MODAL | SWT.APPLICATION_MODAL | SWT.SYSTEM_MODAL;
            SWTComposite shell = (SWTComposite) (null);
            if (modal == null) {
                if ((style & mask) != 0) {
                    shell = this;
                    /*
            ...
            SWTComposite topModalShell = (SWTComposite) (shell);
            while (shell != null) {
                if ((shell.style & mask) == 0) {
                    group = ((SWTShell) (shell.getShell())).group;
                    break;
                }
                topModalShell = shell;
                shell = shell.parent;
		...
		*/
            if (group == 0 && topModalShell != null)
                group = ((SWTShell) (topModalShell.getShell())).group;
        }
        if (group == 0) {
            /*
    ...
    void updateMinimized(boolean minimized) {
        ...
        for (int i = 0; i < shells.length; i++) {
            ...
            while (shell != null && shell != this && !shell.isUndecorated()) {
                shell = (SWTShell) ((SWTComposite) (shell.getParent()));
            }
    ...
    void deregister() {
        super.deregister();
        SWTWidget disposed = (SWTWidget) (display.removeWidget(shellHandle));
        if (shellHandle != 0 && !(disposed instanceof SWTShell)) {
	...
	*/
    ...
    Rectangle getBoundsInPixels() {
        checkWidget();
        int[] x = new int[1], y = new int[1];
        if ((state & SWTWidget.DISPOSE_SENT) == 0) {
            if (GTK.GTK4) {
                // TODO: GTK4 GtkWindow no longer has the ability to get position
            } else {
                GTK3.gtk_window_get_position(shellHandle, x, y);
