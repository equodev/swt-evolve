...
public abstract class SWTWidget implements IWidget {
     ...
     * @see #getStyle
     */
    public SWTWidget(SWTWidget parent, int style) {
        checkSubclass();
        checkParent(parent);
        this.style = style;
    ...
    void checkOrientation(SWTWidget parent) {
        style &= ~SWT.MIRRORED;
     ...
     * </ul>
     */
    void checkParent(SWTWidget parent) {
        if (parent == null)
            error(SWT.ERROR_NULL_ARGUMENT);
 ...
 */
    ...
    void hookEvents() {
        if (handle != 0) {
            OS.g_signal_connect(handle, OS.dpi_changed, display.notifyProc, SWTWidget.DPI_CHANGED);
        }
    }
 ...
 */
    ...
    char[] sendIMKeyEvent(int type, long event, char[] chars) {
        ...
        long ptr = 0;
        if (event == 0) {
            long controller = ((SWTControl) (SWTControl.getControl(this.handle))).keyController;
    ...
    void notifyCreationTracker() {
        if (WidgetSpy.isEnabled) {
            WidgetSpy.getInstance().widgetCreated(Widget.getInstance(this));
        }
    }

    void notifyDisposalTracker() {
        if (WidgetSpy.isEnabled) {
            WidgetSpy.getInstance().widgetDisposed(Widget.getInstance(this));
        }
    }
