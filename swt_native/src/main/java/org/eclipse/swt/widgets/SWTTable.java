...
public class SWTTable extends SWTComposite implements ITable {
    ...
    SWTTableColumn[] columns;

    SWTTableItem currentItem;

    SWTTableColumn sortColumn;
     ...
     * @see Widget#getStyle
     */
    public SWTTable(SWTComposite parent, int style) {
    ...
    SWTTableItem _getItem(int index) {
        if ((style & SWT.VIRTUAL) == 0)
            return items[index];
        if (items[index] != null)
            return items[index];
        return items[index] = new SWTTableItem(this, SWT.NONE, index, false);
    }
	...
	*/
    ...
    long cellDataProc(long tree_column, long cell, long tree_model, long iter, long data) {
        ...
        SWTTableItem item = (SWTTableItem) (_getItem(index[0]));
        ...
        if (columnCount == 0) {
            ...
            SWTTableColumn column = (SWTTableColumn) ((SWTTableColumn) display.getWidget(tree_column));
            if (column != null) {
    ...
    boolean checkData(SWTTableItem item) {
        if (item.cached)
            return true;
        if ((style & SWT.VIRTUAL) != 0) {
            item.cached = true;
	...
	*/
    ...
    public void clear(int index) {
        ...
        SWTTableItem item = (SWTTableItem) (items[index]);
        if (item != null)
            item.clear();
    }
    ...
    public void clear(int start, int end) {
        ...
        if (start == 0 && end == itemCount - 1) {
            ...
            for (int i = start; i <= end; i++) {
                SWTTableItem item = (SWTTableItem) (items[i]);
                if (item != null)
                    item.clear();
            }
        }
    ...
    public void clear(int[] indices) {
        ...
        for (int i = 0; i < indices.length; i++) {
            SWTTableItem item = (SWTTableItem) (items[indices[i]]);
            if (item != null)
                item.clear();
        }
    }
	 ...
	 */
        if (firstCompute) {
            for (int x = 0; x < columns.length; x++) {
                SWTTableColumn column = (SWTTableColumn) (columns[x]);
                if (column != null)
    ...
    void createColumn(SWTTableColumn column, int index) {
        ...
        if (index == 0 && columnCount > 0) {
            SWTTableColumn checkColumn = (SWTTableColumn) (columns[0]);
	...
	*/
    ...
    void createItem(SWTTableColumn column, int index) {
        ...
        if (columnCount == columns.length) {
            SWTTableColumn[] newColumns = new SWTTableColumn[columns.length + 4];
    ...
    void createItem(SWTTableItem item, int index) {
        if (!(0 <= index && index <= itemCount))
            error(SWT.ERROR_INVALID_RANGE);
        if (itemCount == items.length) {
            int length = drawCount <= 0 ? items.length + 4 : Math.max(4, items.length * 3 / 2);
            SWTTableItem[] newItems = new SWTTableItem[length];
	...
	*/
    ...
    void createWidget(int index) {
        ...
        items = new SWTTableItem[4];
        columns = new SWTTableColumn[4];
    ...
    void destroyItem(SWTTableColumn column) {
        int index = 0;
    ...
    void destroyItem(SWTTableItem item) {
        int index = 0;
    ...
    public ITableColumn[] getColumns() {
        checkWidget();
        SWTTableColumn[] result = new SWTTableColumn[columnCount];
    ...
    SWTTableItem getFocusItem() {
        long[] path = new long[1];
        GTK.gtk_tree_view_get_cursor(handle, path, null);
        if (path[0] == 0)
            return null;
        SWTTableItem item = (SWTTableItem) (null);
     ...
     * </ul>
     */
    public ITableItem getItem(int index) {
        checkWidget();
        if (!(0 <= index && index < itemCount))
            error(SWT.ERROR_INVALID_RANGE);
     ...
     * </ul>
     */
    public ITableItem getItem(Point point) {
        checkWidget();
        return getItemInPixels(DPIUtil.autoScaleUp(point));
    }

    SWTTableItem getItemInPixels(Point point) {
        checkWidget();
        if (point == null)
            error(SWT.ERROR_NULL_ARGUMENT);
        long[] path = new long[1];
     ...
     * </ul>
     */
    public ITableItem[] getItems() {
        checkWidget();
        SWTTableItem[] result = new SWTTableItem[itemCount];
        if ((style & SWT.VIRTUAL) != 0) {
            for (int i = 0; i < itemCount; i++) {
                result[i] = _getItem(i);
            }
     ...
     * </ul>
     */
    public ITableItem[] getSelection() {
        checkWidget();
        long selection = GTK.gtk_tree_view_get_selection(handle);
        ...
        if (list != 0) {
            ...
            OS.g_list_free(originalList);
            SWTTableItem[] result = new SWTTableItem[length];
            for (int i = 0; i < result.length; i++) result[i] = _getItem(treeSelection[i]);
            return result;
        }
        return new SWTTableItem[0];
    }
	...
	*/
    ...
    long gtk_key_press_event(long widget, long event) {
        ...
        switch(key[0]) {
            ...
            case GDK.GDK_space:
                if ((style & SWT.CHECK) != 0) {
                    SWTTableItem[] selected = (SWTTableItem[]) (getSelection());
    ...
    private void toggleItemAndSendEvent(SWTTableItem item) {
    ...
    void sendTreeDefaultSelection() {
        ...
        SWTTableItem tableItem = (SWTTableItem) (getFocusItem());
        if (tableItem == null)
            return;
        Event event = new Event();
        event.item = Widget.getInstance(tableItem);
    ...
    long gtk_changed(long widget) {
        SWTTableItem item = (SWTTableItem) (getFocusItem());
        if (item != null) {
            Event event = new Event();
            event.item = Widget.getInstance(item);
            sendSelectionEvent(SWT.Selection, event, false);
     ...
     * </ul>
     */
    public int indexOf(ITableColumn column_) {
        SWTTableColumn column = (SWTTableColumn) column_;
        checkWidget();
        if (column == null)
            error(SWT.ERROR_NULL_ARGUMENT);
        for (int i = 0; i < columnCount; i++) {
     ...
     * </ul>
     */
    public int indexOf(ITableItem item_) {
	 ...
	 */
        ...
        if (headerVisible && noChildDrawing && wasScrolled) {
            for (SWTTableColumn column : columns) {
                if (column != null) {
    ...
    void recreateRenderers() {
        ...
        if (columnCount == 0) {
            createRenderers(GTK.gtk_tree_view_get_column(handle, 0), SWTTable.FIRST_COLUMN, true, 0);
        } else {
            for (int i = 0; i < columnCount; i++) {
                SWTTableColumn column = (SWTTableColumn) (columns[i]);
    ...
    void rendererRender(long cell, long cr, long snapshot, long widget, long background_area, long cell_area, long expose_area, long flags) {
        ...
        if (item != null) {
            if (GTK.GTK_IS_CELL_RENDERER_TOGGLE(cell) || (columnIndex != 0 || (style & SWT.CHECK) == 0)) {
                ...
                if (ptr[0] == 0) {
                    int modelIndex = columnCount == 0 ? SWTTable.FIRST_COLUMN : columns[columnIndex].modelIndex;
    ...
    public void select(int index) {
        ...
        long selection = GTK.gtk_tree_view_get_selection(handle);
        OS.g_signal_handlers_block_matched(selection, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, CHANGED);
        SWTTableItem item = (SWTTableItem) (_getItem(index));
	...
	*/
    ...
    void setFontDescription(long font) {
        ...
        SWTTableColumn[] columns = (SWTTableColumn[]) (getColumns());
    ...
    public void setItemCount(int count) {
        ...
        if (isVirtual) {
            ...
            for (int i = itemCount; i < count; i++) {
                new SWTTableItem(this, SWT.NONE, i, true);
            }
        }
        if (!isVirtual)
            setRedraw(true);
    ...
    void setScrollWidth(long column, SWTTableItem item) {
     ...
     * @since 3.2
     */
    public void setSortColumn(ITableColumn column_) {
        SWTTableColumn column = (SWTTableColumn) column_;
        checkWidget();
        if (column != null && column.isDisposed())
            error(SWT.ERROR_INVALID_ARGUMENT);
     ...
     * @since 3.2
     */
    public void setSelection(ITableItem item_) {
        SWTTableItem item = (SWTTableItem) item_;
        if (item == null)
            error(SWT.ERROR_NULL_ARGUMENT);
        setSelection(new SWTTableItem[] { item });
    }
     ...
     * @since 3.0
     */
    public void showColumn(ITableColumn column_) {
        SWTTableColumn column = (SWTTableColumn) column_;
        checkWidget();
        if (column == null)
            error(SWT.ERROR_NULL_ARGUMENT);
	...
	*/
     ...
     */
    public void showItem(ITableItem item_) {
        SWTTableItem item = (SWTTableItem) item_;
        checkWidget();
        if (item == null)
            error(SWT.ERROR_NULL_ARGUMENT);
    ...
    public void showSelection() {
        checkWidget();
        SWTTableItem[] selection = (SWTTableItem[]) (getSelection());
        if (selection.length == 0)
            return;
        SWTTableItem item = (SWTTableItem) (selection[0]);
        showItem(item.handle);
    }
