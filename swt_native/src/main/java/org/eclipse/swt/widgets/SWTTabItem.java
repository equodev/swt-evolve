...
public class SWTTabItem extends SWTItem implements ITabItem {
     ...
     * @see Widget#getStyle
     */
    public SWTTabItem(SWTTabFolder parent, int style) {
        super(parent, style);
        this.parent = parent;
     ...
     * @see Widget#getStyle
     */
    public SWTTabItem(SWTTabFolder parent, int style, int index) {
        super(parent, style);
        this.parent = parent;
        createWidget(index);
    }
    ...
    void release(boolean destroy) {
        ...
        if (control != null && !control.isDisposed()) {
            SWTControl.gtk_widget_reparent(control, parent.parentingHandle());
        }
    ...
    public void setControl(IControl control_) {
        ...
        if (GTK.GTK4) {
            ...
            SWTControl oldControl = this.control, newControl = (SWTControl) (control);
            this.control = control;
            int index = parent.indexOf(this), selectionIndex = parent.getSelectionIndex();
            if (index != selectionIndex) {
                if (newControl != null) {
                    if (selectionIndex != -1) {
                        SWTControl selectedControl = (SWTControl) (parent.getItem(selectionIndex).getControl());
