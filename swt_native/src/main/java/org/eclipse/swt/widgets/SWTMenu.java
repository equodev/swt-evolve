...
public class SWTMenu extends SWTWidget implements IMenu {
    ...
    SWTMenuItem cascade, selectedItem;

    SWTDecorations parent;
    ...
    class Section {
        ...
        LinkedList<SWTMenuItem> sectionItems;

        private SWTMenuItem separator;
        ...
        public Section(SWTMenuItem separator) {
            this();
        ...
        public int getItemPosition(IMenuItem item__) {
            SWTMenuItem item_ = (SWTMenuItem) item__;
            SWTMenuItem item = (SWTMenuItem) item_;
     ...
     * @see Widget#getStyle
     */
    public SWTMenu(SWTControl parent) {
     ...
     * @see Widget#getStyle
     */
    public SWTMenu(SWTDecorations parent, int style) {
     ...
     * @see Widget#getStyle
     */
    public SWTMenu(SWTMenuItem parentItem) {
        this(checkNull(parentItem).parent);
    }

    static SWTControl checkNull(SWTControl control) {
        if (control == null)
            SWT.error(SWT.ERROR_NULL_ARGUMENT);
        return control;
    }

    static SWTMenu checkNull(SWTMenu menu) {
        if (menu == null)
            SWT.error(SWT.ERROR_NULL_ARGUMENT);
        return menu;
    }

    static SWTMenuItem checkNull(SWTMenuItem item) {
        if (item == null)
            SWT.error(SWT.ERROR_NULL_ARGUMENT);
        return item;
    }
    ...
    boolean ableToSetLocation() {
        if (!OS.isX11() && !((SWTShell) (getShell())).getVisible()) {
            return false;
        }
        return hasLocation;
    }
			...
			*/
                ...
                if (ableToSetLocation()) {
                    if (GTK.GTK4) {
						 ...
						 */
                            ...
                            long gdkResource = GTK3.gtk_widget_get_window(((SWTShell) (getShell())).topHandle());
                        ...
                        GTK3.gtk_menu_popup_at_rect(handle, event.window, rect, GDK.GDK_GRAVITY_NORTH_WEST, GDK.GDK_GRAVITY_NORTH_WEST, eventPtr);
				 ...
				 */
                    ...
                    if (eventPtr == 0) {
                        ...
                        if (OS.isX11()) {
                            event.window = OS.g_object_ref(GTK3.gtk_widget_get_window(((SWTShell) (getShell())).handle));
                        }
                        event.device = GDK.gdk_get_pointer(GDK.gdk_display_get_default());
                        event.time = display.getLastEventTime();
                        GTK3.memmove(eventPtr, event, GdkEventButton.sizeof);
    ...
    void addAccelerators(long accelGroup) {
        SWTMenuItem[] items = (SWTMenuItem[]) (getItems());
        for (int i = 0; i < items.length; i++) {
            SWTMenuItem item = (SWTMenuItem) (items[i]);
    ...
    void createHandle(int index) {
        ...
        if (GTK.GTK4) {
            ...
            if (actionGroup == 0)
                error(SWT.ERROR_NO_HANDLES);
            long shellHandle = ((SWTShell) (parent.getShell())).topHandle();
    ...
    public IMenuItem getItem(int index) {
        ...
        if (GTK.GTK4) {
            ...
            if (data == 0)
                error(SWT.ERROR_CANNOT_GET_ITEM);
            return (SWTMenuItem) display.getWidget(data);
        }
    }
    ...
    public IMenuItem[] getItems() {
        checkWidget();
        if (GTK.GTK4) {
            return items.toArray(new SWTMenuItem[items.size()]);
        } else {
            long list = GTK3.gtk_container_get_children(handle);
            if (list == 0)
                return new SWTMenuItem[0];
            long originalList = list;
            int count = OS.g_list_length(list);
            SWTMenuItem[] items = new SWTMenuItem[count];
            int index = 0;
            for (int i = 0; i < count; i++) {
                long data = OS.g_list_data(list);
                SWTMenuItem item = (SWTMenuItem) ((SWTMenuItem) display.getWidget(data));
                if (item != null)
                    items[index++] = item;
            ...
            if (index != items.length) {
                SWTMenuItem[] newItems = new SWTMenuItem[index];
    ...
    public IMenu getParentMenu() {
        ...
        if (cascade == null)
            return null;
        return ((SWTMenu) (cascade.getParent()));
    }
    ...
    public IShell getShell() {
        checkWidget();
        return ((SWTShell) (parent.getShell()));
    }
    ...
    long gtk_hide(long widget) {
        if ((style & SWT.POP_UP) != 0) {
            if (display.activeShell != null) {
                display.activeShell = ((SWTShell) (getShell()));
     ...
     * </ul>
     */
    public int indexOf(IMenuItem item__) {
        SWTMenuItem item_ = (SWTMenuItem) item__;
        SWTMenuItem item = (SWTMenuItem) item_;
        checkWidget();
        if (item == null)
    ...
    public boolean isEnabled() {
        checkWidget();
        SWTMenu parentMenu = (SWTMenu) (getParentMenu());
        if (parentMenu == null) {
     ...
     * </ul>
     */
    public void setDefaultItem(IMenuItem item__) {
        SWTMenuItem item_ = (SWTMenuItem) item__;
        SWTMenuItem item = (SWTMenuItem) item_;
        checkWidget();
    }
