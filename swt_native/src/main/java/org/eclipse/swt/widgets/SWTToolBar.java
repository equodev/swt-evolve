...
public class SWTToolBar extends SWTComposite implements IToolBar {
    ...
    SWTToolItem currentFocusItem;

    SWTToolItem[] tabItemList;
     ...
     * @see Widget#getStyle()
     */
    public SWTToolBar(SWTComposite parent, int style) {
	...
	*/
    ...
    SWTWidget computeTabGroup() {
        ...
        while (index >= 0) {
            SWTToolItem item = (SWTToolItem) (items[index]);
            if (item.isTabGroup())
                return item;
            index--;
        }
    ...
    SWTWidget[] computeTabList() {
        ...
        if (!isTabGroup() || !isEnabled() || !isVisible())
            return result;
        SWTToolItem[] list = (SWTToolItem[]) (tabList != null ? _getTabItemList() : items);
        for (int i = 0; i < list.length; i++) {
            SWTToolItem child = (SWTToolItem) (list[i]);
    ...
    SWTToolItem getItemInPixels(Point point) {
        if (point == null)
            error(SWT.ERROR_NULL_ARGUMENT);
        SWTToolItem[] items = (SWTToolItem[]) (getItems());
        for (int i = 0; i < items.length; i++) {
            if (items[i].getBoundsInPixels().contains(point))
                return items[i];
        }
        return null;
    }
    ...
    SWTToolItem[] _getItems() {
        if (GTK.GTK4) {
            ArrayList<SWTToolItem> childrenList = new ArrayList<>();
            for (long child = GTK4.gtk_widget_get_first_child(handle); child != 0; child = GTK4.gtk_widget_get_next_sibling(child)) {
                SWTWidget childWidget = (SWTWidget) (display.getWidget(child));
                if (childWidget != null) {
                    childrenList.add((SWTToolItem) childWidget);
                }
            }
            return childrenList.toArray(new SWTToolItem[childrenList.size()]);
        } else {
            long list = GTK3.gtk_container_get_children(handle);
            if (list == 0)
                return new SWTToolItem[0];
            int count = OS.g_list_length(list);
            SWTToolItem[] items = new SWTToolItem[count];
            long originalList = list;
            int index = 0;
            for (int i = 0; i < count; i++) {
                long data = OS.g_list_data(list);
                SWTWidget widget = (SWTWidget) (display.getWidget(data));
                if (widget != null)
                    items[index++] = (SWTToolItem) widget;
                list = OS.g_list_next(list);
            }
            OS.g_list_free(originalList);
            if (index != items.length) {
                SWTToolItem[] newItems = new SWTToolItem[index];
    ...
    SWTToolItem[] _getTabItemList() {
        ...
        if (count == tabItemList.length)
            return tabItemList;
        SWTToolItem[] newList = new SWTToolItem[count];
        int index = 0;
        for (int i = 0; i < tabItemList.length; i++) {
            if (!tabItemList[i].isDisposed()) {
    ...
    user_data) {
        ...
        SWTToolItem item = (SWTToolItem) ((SWTToolItem) display.getWidget(user_data));
        if (item != null) {
            return ((SWTToolBar) (item.getParent())).menuItemSelected(widget, item);
        }
        return 0;
    }

    long menuItemSelected(long widget, SWTToolItem item) {
    ...
    /*public*/
    void setTabItemList(SWTToolItem[] tabList) {
        checkWidget();
        if (tabList != null) {
            for (int i = 0; i < tabList.length; i++) {
                SWTToolItem item = (SWTToolItem) (tabList[i]);
                if (item == null)
            ...
            SWTToolItem[] newList = new SWTToolItem[tabList.length];
