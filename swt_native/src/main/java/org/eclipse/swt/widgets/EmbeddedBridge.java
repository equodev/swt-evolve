...
public abstract class EmbeddedBridge extends FlutterBridge {
    ...
    public EmbeddedBridge(DartWidget widget) {
    ...
    /**
     * Embedded {@code of(DartWidget)} body, shared by the per-OS {@code SwtEmbeddedBridge.of}
     * factories. The concrete is constructed by {@code factory} so that this base (in {@code main},
     * compiled into the web jar too) never names the native-only {@code SwtEmbeddedBridge}.
     */
    protected static EmbeddedBridge create(DartWidget widget, java.util.function.Supplier<EmbeddedBridge> factory) {
...
//            System.out.println("Cached theme " + cachedTheme);
     ...
     * Analogous to how DartWidget.register() creates the per-widget embedded bridge.
     *
...
//        System.out.println("Color from parent: " + parentBackgroundColor);
        ...
        context = FlutterNative.initialize(comm().getPort(), parentHandle, id(control), widgetName(control), theme, backgroundColorInt, parentBackgroundColorInt, 0, 0);

        long view = FlutterNative.getView(context);
    ...
    public void setBoundsCTabFolder(DartCTabFolder folder, Rectangle bounds, boolean viewOnly) {
        if (folder.getSelection() != null && folder.getSelection().getControl() != null && !(folder.getSelection().getControl().getImpl() instanceof DartControl)) {
            if (folder._onBottom()) {
                FlutterNative.setBounds(context, bounds.x, bounds.y, bounds.width, bounds.height, 0, bounds.height - 32, bounds.width, 32);
            } else {
                FlutterNative.setBounds(context, bounds.x, bounds.y, bounds.width, bounds.height, 0, 0, bounds.width, 32);
            }
        } else {
            FlutterNative.setBounds(context, bounds.x, bounds.y, bounds.width, bounds.height, 0, 0, bounds.width, bounds.height);
        }
    }
