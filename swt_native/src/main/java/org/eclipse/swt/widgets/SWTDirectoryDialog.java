...
public class SWTDirectoryDialog extends SWTDialog implements IDirectoryDialog {
     ...
     * </ul>
     */
    public SWTDirectoryDialog(SWTShell parent) {
     ...
     * </ul>
     */
    public SWTDirectoryDialog(SWTShell parent, int style) {
    ...
    Optional<String> openNativeChooserDialog() {
        ...
        long file = 0;
        if (GTK.GTK4) {
            file = SyncDialogUtil.run(Display.getInstance(display), asyncCallback -> GTK4.gtk_file_dialog_select_folder(handle, shellHandle, 0, asyncCallback, 0), asyncResult -> GTK4.gtk_file_dialog_select_folder_finish(handle, asyncResult, null));
