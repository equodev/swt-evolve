...
public class SWTTree extends SWTComposite implements ITree {
    ...
    SWTTreeColumn[] columns;

    SWTTreeColumn sortColumn;

    SWTTreeItem currentItem;
     ...
     * @see Widget#getStyle
     */
    public SWTTree(SWTComposite parent, int style) {
    ...
    SWTTreeItem _getItem(long iter) {
        ...
        items[id] = new SWTTreeItem(this, parentIter, SWT.NONE, indices[indices.length - 1], iter);
    ...
    SWTTreeItem _getItem(long parentIter, long iter, int index) {
        int id = getId(iter, true);
        if (items[id] != null)
            return items[id];
        return items[id] = new SWTTreeItem(this, parentIter, SWT.NONE, index, iter);
    }

    void reallocateIds(int newSize) {
        SWTTreeItem[] newItems = new SWTTreeItem[newSize];
	...
	*/
    ...
    long cellDataProc(long tree_column, long cell, long tree_model, long iter, long data) {
        if (cell == ignoreCell)
            return 0;
        SWTTreeItem item = (SWTTreeItem) (_getItem(iter));
        if (item != null)
        ...
        if (columnCount == 0) {
            ...
            SWTTreeColumn column = (SWTTreeColumn) ((SWTTreeColumn) display.getWidget(tree_column));
            if (column != null) {
    ...
    boolean checkData(SWTTreeItem item) {
        if (item.cached)
            return true;
        if ((style & SWT.VIRTUAL) != 0) {
            item.cached = true;
            SWTTreeItem parentItem = (SWTTreeItem) (item.getParentItem());
            Event event = new Event();
            event.item = Widget.getInstance(item);
	...
	*/
    ...
    void clear(long parentIter, int index, boolean all) {
        ...
        GTK.gtk_tree_model_get(modelHandle, iter, ID_COLUMN, value, -1);
        if (value[0] != -1) {
            SWTTreeItem item = (SWTTreeItem) (items[value[0]]);
            item.clear();
        }
        if (all)
            clearAll(all, iter);
	 ...
	 */
        if (!GTK.GTK4) {
            if (firstCompute) {
                for (SWTTreeColumn column : columns) {
                    if (column != null)
    ...
    void copyModel(long oldModel, int oldStart, long newModel, int newStart, long oldParent, long newParent, int modelLength) {
        ...
        if (GTK.gtk_tree_model_iter_children(oldModel, iter, oldParent)) {
            ...
            do {
                ...
                SWTTreeItem item = (SWTTreeItem) (null);
                if (index != -1) {
                    item = items[index];
                    if (item != null) {
    ...
    void createColumn(SWTTreeColumn column, int index) {
        /*
...
*/
        ...
        if (index == 0 && columnCount > 0) {
            SWTTreeColumn checkColumn = (SWTTreeColumn) (columns[0]);
	...
	*/
    ...
    void bindArrowKeyBindings() {
        ...
        GTK4.gtk_widget_class_add_binding_signal(GTK.GTK_WIDGET_GET_CLASS(handle), keyval[0], 0, Converter.javaStringToCString("expand-collapse-cursor-row"), Converter.javaStringToCString("(bbb)"), false, false, false);
        GTK.gtk_accelerator_parse(Converter.javaStringToCString("Right"), keyval, null);
        GTK4.gtk_widget_class_add_binding_signal(GTK.GTK_WIDGET_GET_CLASS(handle), keyval[0], 0, Converter.javaStringToCString("expand-collapse-cursor-row"), Converter.javaStringToCString("(bbb)"), false, true, false);
    }
    ...
    void createItem(SWTTreeColumn column, int index) {
        ...
        if (columnCount == columns.length) {
            SWTTreeColumn[] newColumns = new SWTTreeColumn[columns.length + 4];
        ...
        if (columnCount >= 1) {
            for (int i = 0; i < items.length; i++) {
                SWTTreeItem item = (SWTTreeItem) (items[i]);
                if (item != null) {
     ...
     */
    void createItem(SWTTreeItem item, long parentIter, int index) {
        /*
	...
	*/
    ...
    void createWidget(int index) {
        ...
        items = new SWTTreeItem[4];
        columns = new SWTTreeColumn[4];
        columnCount = 0;
     ...
     * @since 3.4
     */
    public void deselect(ITreeItem item_) {
    ...
    void destroyItem(SWTTreeColumn column) {
        int index = 0;
        ...
        if (columnCount == 0) {
            ...
            if (index == 0) {
                ...
                SWTTreeColumn firstColumn = (SWTTreeColumn) (columns[0]);
    ...
    void destroyItem(SWTTreeItem item) {
        long selection = GTK.gtk_tree_view_get_selection(handle);
        OS.g_signal_handlers_block_matched(selection, OS.G_SIGNAL_MATCH_DATA, 0, 0, 0, 0, CHANGED);
     ...
     * @since 3.1
     */
    public ITreeColumn getColumn(int index) {
        checkWidget();
        if (!(0 <= index && index < columnCount))
            error(SWT.ERROR_INVALID_RANGE);
     ...
     * @since 3.1
     */
    public ITreeColumn[] getColumns() {
        checkWidget();
        SWTTreeColumn[] result = new SWTTreeColumn[columnCount];
     ...
     * </ul>
     */
    public ITreeItem getItem(Point point) {
        checkWidget();
        return getItemInPixels(DPIUtil.autoScaleUp(point));
    }

    SWTTreeItem getItemInPixels(Point point) {
        checkWidget();
        if (point == null)
            error(SWT.ERROR_NULL_ARGUMENT);
        long[] path = new long[1];
    ...
    SWTTreeItem[] getItems(long parent) {
        ArrayList<SWTTreeItem> result = new ArrayList<>();
        long iter = OS.g_malloc(GTK.GtkTreeIter_sizeof());
        try {
        ...
        return result.toArray(new SWTTreeItem[result.size()]);
    }
    ...
    public ITreeItem[] getSelection() {
        ...
        if (list != 0) {
            ...
            SWTTreeItem[] treeSelection = new SWTTreeItem[count];
            int length = 0;
            ...
            OS.g_list_free(originalList);
            if (length < count) {
                SWTTreeItem[] temp = new SWTTreeItem[length];
                System.arraycopy(treeSelection, 0, temp, 0, length);
                treeSelection = temp;
            }
            return treeSelection;
        }
        return new SWTTreeItem[0];
    }
     ...
     * @since 3.2
     */
    public ITreeColumn getSortColumn() {
        checkWidget();
        return sortColumn;
    }
	 ...
	 */
        ...
        SWTTreeItem treeSelection = (SWTTreeItem) (null);
        if (list != 0) {
            long iter = OS.g_malloc(GTK.GtkTreeIter_sizeof());
            long data = OS.g_list_data(list);
            if (GTK.gtk_tree_model_get_iter(modelHandle, iter, data)) {
	 ...
	 */
        if ((state & DRAG_DETECT) != 0 && hooks(SWT.DragDetect) && OS.isWayland() && eventType == GDK.GDK_BUTTON_PRESS) {
            ...
            if (nextEvent == 0) {
                long[] path = new long[1];
                long selection = GTK.gtk_tree_view_get_selection(handle);
                if (GTK.gtk_tree_view_get_path_at_pos(handle, (int) eventX[0], (int) eventY[0], path, null, null, null) && path[0] != 0) {
	...
	*/
    ...
    void sendTreeDefaultSelection() {
        ...
        SWTTreeItem treeItem = (SWTTreeItem) (getFocusItem());
        if (treeItem == null)
            return;
        Event event = new Event();
        event.item = Widget.getInstance(treeItem);
    ...
    long gtk_button_release_event(long widget, long event) {
        ...
        long eventGdkResource = gdk_event_get_surface_or_window(event);
        if (GTK.GTK4) {
            if (eventGdkResource != gtk_widget_get_surface(handle))
                return 0;
        } else {
            if (eventGdkResource != GTK3.gtk_tree_view_get_bin_window(handle))
                return 0;
	 ...
	 */
        if ((state & DRAG_DETECT) != 0 && hooks(SWT.DragDetect) && OS.isWayland()) {
            ...
            if (GTK.gtk_tree_view_get_path_at_pos(handle, (int) eventX[0], (int) eventY[0], path, null, null, null) && path[0] != 0 && GTK.gtk_tree_selection_path_is_selected(selection, path[0])) {
    ...
    long gtk_changed(long widget) {
        SWTTreeItem item = (SWTTreeItem) (getFocusItem());
        if (item != null) {
            Event event = new Event();
            event.item = Widget.getInstance(item);
    ...
    void drawInheritedBackground(long cairo) {
        if ((state & PARENT_BACKGROUND) != 0 || backgroundImage != null) {
            SWTControl control = (SWTControl) (findBackgroundControl());
            if (control != null) {
    ...
    long gtk_motion_notify_event(long widget, long event) {
        if (GTK.GTK4) {
            ...
            if (surface != gtk_widget_get_surface(handle))
                return 0;
        } else {
	...
	*/
        ...
        if (index[0] >= items.length)
            return 0;
        SWTTreeItem item = (SWTTreeItem) (items[index[0]]);
        if (item == null)
            return 0;
	...
	*/
     ...
     * @since 3.1
     */
    public int indexOf(ITreeColumn column_) {
        SWTTreeColumn column = (SWTTreeColumn) column_;
        checkWidget();
        if (column == null)
            error(SWT.ERROR_NULL_ARGUMENT);
        for (int i = 0; i < columnCount; i++) {
     ...
     * @since 3.1
     */
    public int indexOf(ITreeItem item_) {
        SWTTreeItem item = (SWTTreeItem) item_;
        checkWidget();
        if (item == null)
            error(SWT.ERROR_NULL_ARGUMENT);
    ...
    void recreateRenderers() {
        ...
        if (columnCount == 0) {
            createRenderers(GTK.gtk_tree_view_get_column(handle, 0), SWTTree.FIRST_COLUMN, true, 0);
        } else {
            for (int i = 0; i < columnCount; i++) {
                SWTTreeColumn column = (SWTTreeColumn) (columns[i]);
    ...
    void releaseItem(SWTTreeItem item, boolean release) {
        int[] index = new int[1];
    ...
    void remove(long parentIter, int start, int end) {
        ...
        try {
            for (int i = start; i <= end; i++) {
                ...
                int[] value = new int[1];
                GTK.gtk_tree_model_get(modelHandle, iter, ID_COLUMN, value, -1);
                SWTTreeItem item = (SWTTreeItem) (value[0] != -1 ? items[value[0]] : null);
                if (item != null && !item.isDisposed()) {
                    /*
    ...
    void rendererRender(long cell, long cr, long snapshot, long widget, long background_area, long cell_area, long expose_area, long flags) {
        ...
        if (item != null) {
            if (GTK.GTK_IS_CELL_RENDERER_TOGGLE(cell) || (columnIndex != 0 || (style & SWT.CHECK) == 0)) {
                ...
                if (ptr[0] == 0) {
                    int modelIndex = columnCount == 0 ? SWTTree.FIRST_COLUMN : columns[columnIndex].modelIndex;
     ...
     *  </ul>
     */
    public void setInsertMark(ITreeItem item_, boolean before) {
        SWTTreeItem item = (SWTTreeItem) item_;
        checkWidget();
        if (item == null) {
    ...
    void setItemCount(long parentIter, int count) {
        ...
        if (isVirtual) {
            ...
            for (int i = itemCount; i < count; i++) {
                new SWTTreeItem(this, parentIter, SWT.NONE, itemCount, 0);
            }
        }
        if (!isVirtual)
            setRedraw(true);
     ...
     * @since 3.4
     */
    public void select(ITreeItem item_) {
        SWTTreeItem item = (SWTTreeItem) item_;
        checkWidget();
        if (item == null)
            error(SWT.ERROR_NULL_ARGUMENT);
	...
	*/
    ...
    @Override
    void setParentGdkResource(SWTControl child) {
        /*
    ...
    void setScrollWidth(long column, SWTTreeItem item) {
     ...
     * @since 3.2
     */
    public void setSelection(ITreeItem item_) {
        SWTTreeItem item = (SWTTreeItem) item_;
        checkWidget();
        if (item == null)
            error(SWT.ERROR_NULL_ARGUMENT);
        setSelection(new SWTTreeItem[] { item });
    }
     ...
     * @since 3.2
     */
    public void setSortColumn(ITreeColumn column_) {
        SWTTreeColumn column = (SWTTreeColumn) column_;
        checkWidget();
        if (column != null && column.isDisposed())
            error(SWT.ERROR_INVALID_ARGUMENT);
     ...
     * @since 2.1
     */
    public void setTopItem(ITreeItem item_) {
        SWTTreeItem item = (SWTTreeItem) item_;
        /*
     ...
     * @since 3.1
     */
    public void showColumn(ITreeColumn column_) {
        SWTTreeColumn column = (SWTTreeColumn) column_;
        checkWidget();
        if (column == null)
            error(SWT.ERROR_NULL_ARGUMENT);
	...
	*/
     ...
     */
    public void showItem(ITreeItem item_) {
        SWTTreeItem item = (SWTTreeItem) item_;
        checkWidget();
        if (item == null)
            error(SWT.ERROR_NULL_ARGUMENT);
    ...
    @Override
    void updateScrollBarValue(SWTScrollBar bar) {
