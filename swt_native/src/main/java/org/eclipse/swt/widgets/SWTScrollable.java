...
public abstract class SWTScrollable extends SWTControl implements IScrollable {
    ...
    SWTScrollBar horizontalBar, verticalBar;
     ...
     * @see Widget#getStyle
     */
    public SWTScrollable(SWTComposite parent, int style) {
        super(parent, style);
    }
    ...
    SWTScrollBar createScrollBar(int style) {
        if (scrolledHandle == 0)
            return null;
        SWTScrollBar bar = new SWTScrollBar();
        bar.parent = this;
        bar.style = style;
        bar.display = display;
        bar.state |= HANDLE;
    ...
    void destroyScrollBar(SWTScrollBar bar) {
