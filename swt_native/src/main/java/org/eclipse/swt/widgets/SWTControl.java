...
public abstract class SWTControl extends SWTWidget implements Drawable, IControl {
     ...
     * @see Widget#getStyle
     */
    public SWTControl(SWTComposite parent, int style) {
        super(parent, style);
        this.parent = parent;
        createWidget(0);
    }
    ...
    void drawBackground(SWTControl control, long gdkResource, long cr, int x, int y, int width, int height) {
        long cairo = 0;
        long region = 0;
        if (GTK.GTK4) {
    ...
    void fixFocus(SWTControl focusControl) {
        SWTShell shell = (SWTShell) (getShell());
	...
	*/
    ...
    boolean hasFocus() {
        return this == ((SWTControl) (display.getFocusControl()));
    }
    ...
    void printWindow(boolean first, SWTControl control, GC gc, long drawable, int depth, long window, int x, int y) {
        /*
    ...
    SWTWidget[] computeTabList() {
        if (isTabGroup()) {
            if (getVisible() && getEnabled()) {
                return new SWTWidget[] { this };
            }
        }
        return new SWTWidget[0];
    }

    SWTControl computeTabRoot() {
        SWTControl[] tabList = (SWTControl[]) (parent._getTabList());
        if (tabList != null) {
            int index = 0;
    ...
    void checkBackground() {
        ...
        SWTComposite composite = (SWTComposite) (parent);
        do {
    ...
    int setBounds(int x, int y, int width, int height, boolean move, boolean resize) {
		...
		*/
		 ...
		 */
			 ...
			 */
                if (focusControl != null && ((SWTControl) (display.getFocusControl())) != focusControl) {
	...
	*/
    ...
    @Override
    boolean isActive() {
        return ((SWTShell) (getShell())).getModalShell() == null && display.getModalDialog() == null;
    }
 ...
 */
     ...
     */
    public void moveAbove(IControl control_) {
    ...
    Point getControlOrigin() {
        ...
        boolean success = GTK4.gtk_widget_translate_coordinates(fixedHandle, ((SWTShell) (getShell())).shellHandle, 0, 0, originX, originY);
    ...
    void addRelation(SWTControl control) {
    }
 ...
 */
    ...
    boolean dragDetect(int x, int y, boolean filter, boolean dragOnTimeout, boolean[] consume) {
        ...
        if (OS.isWayland()) {
            ...
            if (GTK3.gtk_drag_check_threshold(handle, (int) startX[0], (int) startY[0], (int) startX[0] + (int) offsetX[0], (int) startY[0] + (int) offsetY[0])) {
                return true;
            }
            return false;
        } else {
			...
			*/
                ...
                while (System.currentTimeMillis() < timeout) {
                    ...
                    if (eventPtr != 0) {
                        ...
                        dragging = GTK3.gtk_drag_check_threshold(handle, startPos.x, startPos.y, currPos.x, currPos.y);
                        if (dragging)
                            break;
                    }
                }
    ...
    SWTMenu[] findMenus(SWTControl control) {
        if (menu != null && this != control)
            return new SWTMenu[] { menu };
        return new SWTMenu[0];
    }

    void fixChildren(SWTShell newShell, SWTShell oldShell, SWTDecorations newDecorations, SWTDecorations oldDecorations, SWTMenu[] menus) {
     ...
     */
    static void gtk_widget_reparent(SWTControl control, long newParentHandle) {
    ...
    public boolean forceFocus() {
        ...
        if (!isEnabled() || !isVisible())
            return false;
        if (((SWTShell) (display.getActiveShell())) != shell && !SWTDisplay.isActivateShellOnForceFocus())
            return false;
    ...
    public IMonitor getMonitor() {
        checkWidget();
        SWTMonitor[] monitors = (SWTMonitor[]) (display.getMonitors());
        ...
        return ((SWTMonitor) (display.getPrimaryMonitor()));
    }
    ...
    SWTControl[] getPath() {
        ...
        control = this;
        SWTControl[] result = new SWTControl[count];
        while (control != shell) {
	...
	*/
        SWTShell shell = (SWTShell) (_getShell());
	 ...
	 */
        ...
        long toolHandle = ((SWTShell) (getShell())).handle;
    ...
    long gtk_focus_in_event(long widget, long event) {
        // widget could be disposed at this point
        if (handle != 0) {
            SWTControl oldControl = display.imControl;
            if (oldControl != this) {
    ...
    long gtk_key_press_event(long widget, long event) {
		...
		*/
            if (((SWTShell) (display.getActiveShell())) == null) {
                if (filterKey(event))
                    return 1;
            }
            return 0;
    ...
    void gtk4_leave_event(long controller, long event) {
        ...
        if (sendLeaveNotify() || ((SWTControl) (display.getCursorControl())) == null) {
	...
	*/
    ...
    boolean isFocusAncestor(SWTControl control) {
    ...
    public boolean isFocusControl() {
        checkWidget();
        SWTControl focusControl = display.focusControl;
    ...
    public void requestLayout() {
        ((SWTShell) (getShell())).layout(new SWTControl[] { this }, SWT.DEFER);
    }
    ...
    @Override
    void releaseWidget() {
        boolean hadFocus = ((SWTControl) (display.getFocusControl())) == this;
        super.releaseWidget();
        if (hadFocus)
            fixFocus(this);
        if (display.currentControl == this)
 ...
 */
    ...
    static SWTControl getControl(long handle) {
    ...
    public boolean setParent(IComposite parent_) {
        ...
        releaseParent();
        SWTShell newShell = (SWTShell) (parent.getShell()), oldShell = (SWTShell) (getShell());
        SWTDecorations newDecorations = (SWTDecorations) (parent.menuShell()), oldDecorations = (SWTDecorations) (menuShell());
        SWTMenu[] menus = (SWTMenu[]) (oldShell.findMenus(this));
	...
	*/
    ...
    void setToolTipText(SWTShell shell, String newString) {
        /*
	...
	*/
    ...
    boolean translateMnemonic(Event event, SWTControl control) {
        if (control == this)
            return false;
    ...
    boolean translateMnemonic(int keyval, long event) {
        ...
        SWTDecorations shell = (SWTDecorations) (menuShell());
	...
	*/
    ...
    boolean traverseGroup(boolean next) {
        SWTControl root = (SWTControl) (computeTabRoot());
        SWTWidget group = (SWTWidget) (computeTabGroup());
        SWTWidget[] list = (SWTWidget[]) (root.computeTabList());
	...
	*/
        ...
        while ((index = ((index + offset + length) % length)) != start) {
            SWTWidget widget = (SWTWidget) (list[index]);
	...
	*/
        ...
        int start = index, offset = (next) ? 1 : -1;
        while ((index = (index + offset + length) % length) != start) {
            SWTControl child = (SWTControl) (children[index]);
