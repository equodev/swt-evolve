...
public class SWTDateTime extends SWTComposite implements IDateTime {
     ...
     * @see Widget#getStyle
     */
    public SWTDateTime(SWTComposite parent, int style) {
	...
	*/
    ...
    void createDropDownButton() {
        down = new SWTButton(this, SWT.ARROW | SWT.DOWN);
        ...
        popupListener = event -> {
            if (event.widget == Widget.getInstance(popupShell)) {
                popupShellEvent(event);
                return;
            }
            if (event.widget == Widget.getInstance(popupCalendar)) {
                popupCalendarEvent(event);
                return;
            }
            if (event.widget == Widget.getInstance(SWTDateTime.this)) {
                onDispose(event);
                return;
            }
            if (event.widget == Widget.getInstance(((SWTShell) (getShell())))) {
        ...
        popupFilter = event -> {
            SWTShell shell = (SWTShell) (((SWTControl) (SWTControl) (event.widget != null ? event.widget.delegate : null)).getShell());
            if (shell == ((SWTShell) (SWTDateTime.this.getShell()))) {
                handleFocus(SWT.FocusOut);
            }
        };
    }
    ...
    void createPopupShell(int year, int month, int day) {
        popupShell = new SWTShell(((SWTShell) (getShell())), SWT.NO_TRIM | SWT.ON_TOP);
        int popupStyle = SWT.CALENDAR;
        if (showWeekNumbers()) {
            popupStyle |= SWT.CALENDAR_WEEKNUMBERS;
        }
        popupCalendar = new SWTDateTime(popupShell, popupStyle);
        if (font != null)
    ...
    void dropDownCalendar(boolean drop) {
        ...
        setCurrentDate();
        if (((SWTShell) (getShell())) != ((SWTComposite) (popupShell.getParent()))) {
            recreateCalendar();
        }
        ...
        Rectangle coordsRelativeToScreen = display.mapInPixels(((SWTComposite) (getParent())), null, getBoundsInPixels());
        Rectangle displayRect = DPIUtil.autoScaleUp(((SWTMonitor) (getMonitor())).getClientArea());
 ...
 */
    ...
    void initAccessible() {
        ...
        accessible.addAccessibleControlListener(new AccessibleControlAdapter() {
            ...
            public void getLocation(AccessibleControlEvent e) {
                Rectangle rect = display.map(((SWTComposite) (getParent())), null, getBounds());
                e.x = rect.x;
                e.y = rect.y;
                e.width = rect.width;
    ...
    void popupCalendarEvent(Event event) {
        switch(event.type) {
            case SWT.Dispose:
                if (popupShell != null && !popupShell.isDisposed() && !isDisposed() && ((SWTShell) (getShell())) != ((SWTComposite) (popupShell.getParent()))) {
                    int year = popupCalendar.getYear();
                    int month = popupCalendar.getMonth();
                    int day = popupCalendar.getDay();
                    popupShell = null;
