 ...
 */
public class SWTDragSource extends SWTWidget implements IDragSource {
    ...
    static {
        if (GTK.GTK4) {
            dragBeginProc = new Callback(SWTDragSource.class, "dragBeginProc", void.class, new Type[] { long.class, long.class });
            dragPrepareProc = new Callback(SWTDragSource.class, "dragPrepareProc", long.class, new Type[] { long.class, double.class, double.class });
            dragEndProc = new Callback(SWTDragSource.class, "dragEndProc", void.class, new Type[] { long.class, long.class, boolean.class });
        } else {
            //$NON-NLS-1$
            DragBegin = new Callback(SWTDragSource.class, "DragBegin", 2);
            //$NON-NLS-1$
            DragGetData = new Callback(SWTDragSource.class, "DragGetData", 5);
            //$NON-NLS-1$
            DragEnd = new Callback(SWTDragSource.class, "DragEnd", 2);
            //$NON-NLS-1$
            DragDataDelete = new Callback(SWTDragSource.class, "DragDataDelete", 2);
        }
    }
     ...
     *  @see DND#DROP_LINK
     */
    public SWTDragSource(SWTControl control, int style) {
        ...
        if (GTK.GTK4) {
            ...
            controlListener = event -> {
                if (event.type == SWT.Dispose) {
                    if (!SWTDragSource.this.isDisposed()) {
                        SWTDragSource.this.dispose();
                    }
                }
    ...
    static void dragBeginProc(long source, long drag) {
        ...
        SWTDragSource dragSource = (SWTDragSource) (FindDragSource(widgetHandle));
        if (dragSource == null)
            return;
    ...
    static long DragBegin(long widget, long context) {
        SWTDragSource source = (SWTDragSource) (FindDragSource(widget));
        if (source == null)
            return 0;
    ...
    static SWTDragSource FindDragSource(long handle) {
        ...
        if (widget == null)
            return null;
        return (SWTDragSource) widget.getData(DND.DRAG_SOURCE_KEY);
    }
	 ...
	 */
        if (this.control instanceof SWTText) {
            DNDEvent event = new DNDEvent();
	 ...
	 */
		 ...
		 */
            if (this.control instanceof SWTTable || this.control instanceof SWTTree || this.control instanceof SWTList) {
