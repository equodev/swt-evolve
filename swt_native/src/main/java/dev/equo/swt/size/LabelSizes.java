...
public class LabelSizes {
    ...
    static Measure computeSizes(DartLabel widget, int wHint, int hHint, boolean changed) {
        ...
        if (hasFlags(style, (SWT.HORIZONTAL | SWT.SEPARATOR))) {
            width = wHint != SWT.DEFAULT ? wHint : HORIZONTAL_SEPARATOR.MIN_WIDTH;
            height = hHint != SWT.DEFAULT ? hHint : HORIZONTAL_SEPARATOR.MIN_HEIGHT;
        } else if (hasFlags(style, (SWT.VERTICAL | SWT.SEPARATOR))) {
            width = wHint != SWT.DEFAULT ? wHint : VERTICAL_SEPARATOR.MIN_WIDTH;
            height = hHint != SWT.DEFAULT ? hHint : VERTICAL_SEPARATOR.MIN_HEIGHT;
            ...
            m.image = computeImage(widget);
            width = wHint != SWT.DEFAULT ? wHint : Math.max((m.text.x() + m.image.x() + (m.image.x() > 0 && m.text.x() > 0 ? HORIZONTAL.IMAGE_SPACING : 0)) + ((m.text.x() > 0 || m.image.x() > 0) ? HORIZONTAL.HORIZONTAL_PADDING : 0), HORIZONTAL.MIN_WIDTH);
