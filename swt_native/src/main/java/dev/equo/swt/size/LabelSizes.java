...
public class LabelSizes {
    ...
    static Measure computeSizes(DartLabel widget, int wHint, int hHint, boolean changed) {
        ...
        if (hasFlags(style, (SWT.HORIZONTAL | SWT.SEPARATOR))) {
            ...
            m.text = computeText(widget, m, VERTICAL.EMPTY_TEXT_AFFECTS_SIZING, wHint);
            ...
            m.text = computeText(widget, m, HORIZONTAL.EMPTY_TEXT_AFFECTS_SIZING, wHint);
            m.image = computeImage(widget);
    ...
    private static PointD computeText(DartLabel widget, Measure m, boolean emptyTextAffectsSizing, int wHint) {
