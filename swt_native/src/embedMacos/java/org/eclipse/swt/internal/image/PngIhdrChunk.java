...
class PngIhdrChunk extends PngChunk {
    ...
    void validate(PngFileReadState readState, PngIhdrChunk headerChunk) {
        ...
        if (compressionMethod != 0)
            SWT.error(SWT.ERROR_INVALID_IMAGE);
        if (interlaceMethod != INTERLACE_METHOD_NONE && interlaceMethod != INTERLACE_METHOD_ADAM7) {
            SWT.error(SWT.ERROR_INVALID_IMAGE);
        }
    ...
    int getBitsPerPixel() {
        return switch(colorType) {
            ...
            case COLOR_TYPE_GRAYSCALE_WITH_ALPHA ->
                2 * bitDepth;
            case COLOR_TYPE_GRAYSCALE, COLOR_TYPE_PALETTE ->
                bitDepth;
            default ->
                {
                    SWT.error(SWT.ERROR_INVALID_IMAGE);
    ...
    int getSwtBitsPerPixel() {
        return switch(colorType) {
            case COLOR_TYPE_RGB_WITH_ALPHA, COLOR_TYPE_RGB, COLOR_TYPE_GRAYSCALE_WITH_ALPHA ->
                24;
            case COLOR_TYPE_GRAYSCALE, COLOR_TYPE_PALETTE ->
                Math.min(bitDepth, 8);
            default ->
    ...
    boolean usesDirectColor() {
        return switch(colorType) {
            case COLOR_TYPE_GRAYSCALE, COLOR_TYPE_GRAYSCALE_WITH_ALPHA, COLOR_TYPE_RGB, COLOR_TYPE_RGB_WITH_ALPHA ->
                true;
            default ->
                false;
        };
    ...
    PaletteData getPaletteData() {
        return switch(colorType) {
            ...
            case COLOR_TYPE_GRAYSCALE_WITH_ALPHA, COLOR_TYPE_RGB, COLOR_TYPE_RGB_WITH_ALPHA ->
