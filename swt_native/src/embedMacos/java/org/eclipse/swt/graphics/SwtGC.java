...
public final class SwtGC extends SwtResource implements IGC {
    ...
    NSAutoreleasePool checkGC(int mask) {
        ...
        if (!NSThread.isMainThread())
            pool = (NSAutoreleasePool) new NSAutoreleasePool().alloc().init();
        if (data.flippedContext != null && !getApi().handle.isEqual(NSGraphicsContext.currentContext())) {
            data.restoreContext = true;
            NSGraphicsContext.static_saveGraphicsState();
            NSGraphicsContext.setCurrentContext(getApi().handle);
        }
        if ((mask & (CLIPPING | TRANSFORM)) != 0) {
            NSView view = data.view;
            if ((data.state & CLIPPING) == 0 || (data.state & TRANSFORM) == 0 || (data.state & VISIBLE_REGION) == 0) {
                boolean antialias = getApi().handle.shouldAntialias();
                getApi().handle.restoreGraphicsState();
                getApi().handle.saveGraphicsState();
                getApi().handle.setShouldAntialias(antialias);
        ...
        OS.CGContextSetBlendMode(getApi().handle.graphicsPort(), data.xorMode ? OS.kCGBlendModeDifference : OS.kCGBlendModeNormal);
        ...
        if ((state & FOREGROUND) != 0) {
            ...
            if (pattern != null) {
                if (((SwtPattern) pattern.getImpl()).color != null)
                    ((SwtPattern) pattern.getImpl()).color.setStroke();
            } else {
        ...
        if ((state & FOREGROUND_FILL) != 0) {
            ...
            if (pattern != null) {
                if (((SwtPattern) pattern.getImpl()).color != null)
                    ((SwtPattern) pattern.getImpl()).color.setFill();
            } else {
                double[] color = data.foreground;
                if (data.fg != null)
                    data.fg.release();
    ...
    NSAttributedString createString(String string, int flags, boolean draw) {
        ...
        if (draw) {
            ...
            if (pattern != null) {
                if (((SwtPattern) pattern.getImpl()).color != null)
                    dict.setObject(((SwtPattern) pattern.getImpl()).color, OS.NSForegroundColorAttributeName);
            } else {
                NSColor fg = data.fg;
                if (fg == null) {
                    double[] color = data.foreground;
        ...
        if ((flags & SWT.DRAW_TAB) == 0) {
            dict.setObject(((SwtDevice) device.getImpl()).paragraphStyle, OS.NSParagraphStyleAttributeName);
        }
    ...
    void destroy() {
        ...
        /* Dispose the GC */
        if (drawable != null)
            drawable.internal_dispose_GC(getApi().handle.id, data);
        getApi().handle.restoreGraphicsState();
    ...
    public void drawArc(int x, int y, int width, int height, int startAngle, int arcAngle) {
        ...
        try {
            ...
            if (pattern != null && ((SwtPattern) pattern.getImpl()).gradient != null) {
    ...
    public void drawFocus(int x, int y, int width, int height) {
        ...
        try {
            ...
            OS.HIThemeDrawFocusRect(rect, true, getApi().handle.graphicsPort(), OS.kHIThemeOrientationNormal);
        } finally {
            uncheckGC(pool);
        }
    }
    ...
    public void drawText(String string, int x, int y, int flags) {
        ...
        try {
            ...
            getApi().handle.saveGraphicsState();
            getApi().handle.setShouldAntialias(mode);
    ...
    void fillPattern(NSBezierPath path, Pattern pattern) {
        ...
        start.x = ((SwtPattern) pattern.getImpl()).pt1.x;
        start.y = ((SwtPattern) pattern.getImpl()).pt1.y;
        NSPoint end = new NSPoint();
        end.x = ((SwtPattern) pattern.getImpl()).pt2.x;
        end.y = ((SwtPattern) pattern.getImpl()).pt2.y;
        double difx = end.x - start.x;
        double dify = end.y - start.y;
        if (difx == 0 && dify == 0) {
            double[] color = ((SwtPattern) pattern.getImpl()).color1;
        ...
        do {
            ...
            ((SwtPattern) pattern.getImpl()).gradient.drawFromPoint(start, end, 0);
            start.x = end.x;
            start.y = end.y;
    ...
    void strokePattern(NSBezierPath path, Pattern pattern) {
        ...
        long cgContext = getApi().handle.graphicsPort();
    ...
    public void getClipping(Region region) {
        ...
        try {
            ...
            if (data.clipPath != null) {
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    switch(element) {
                        case OS.NSMoveToBezierPathElement:
                            if (pointCount != 0)
                                ((SwtRegion) clipRgn.getImpl()).add(pointArray, pointCount);
                            pointCount = 0;
            ...
            if (data.inverseTransform != null) {
                ((SwtRegion) region.getImpl()).convertRgn(data.inverseTransform);
            }
        } finally {
            if (pool != null)
                pool.release();
    ...
    public FontMetrics getFontMetrics() {
        ...
        try {
            ...
            if (data.font.getImpl()._metrics() == null) {
                ...
                ((SwtFont) data.font.getImpl()).addTraits(dict);
    ...
    public int getInterpolation() {
        if (getApi().handle == null)
            SWT.error(SWT.ERROR_GRAPHIC_DISPOSED);
        int interpolation = (int) getApi().handle.imageInterpolation();
    ...
    public void setClipping(Region region) {
        ...
        if (!NSThread.isMainThread())
            pool = (NSAutoreleasePool) new NSAutoreleasePool().alloc().init();
        try {
            setClipping(region != null ? ((SwtRegion) region.getImpl()).getPath() : null);
        } finally {
            if (pool != null)
                pool.release();
        }
    ...
    public void setInterpolation(int interpolation) {
        ...
        getApi().handle.setImageInterpolation(quality);
    }
    ...
    void setPatternPhase(Pattern pattern) {
        if (((SwtPattern) pattern.getImpl()).image == null)
            return;
        NSPoint phase = new NSPoint();
        if (data.image != null) {
            phase.y += data.image.handle.size().height - ((SwtPattern) pattern.getImpl()).image.handle.size().height;
            ...
            phase.y += data.size.height - ((SwtPattern) pattern.getImpl()).image.handle.size().height;
        }
        getApi().handle.setPatternPhase(phase);
    }
