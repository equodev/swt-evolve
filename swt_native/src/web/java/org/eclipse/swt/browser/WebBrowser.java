...
abstract class WebBrowser {
	 ...
	 */
        for (BrowserFunction current : functions.values()) {
            if (((DartBrowserFunction) current.getImpl()).name.equals(((DartBrowserFunction) function.getImpl()).name)) {
                deregisterFunction(current);
                break;
            }
        }
        ((DartBrowserFunction) function.getImpl()).index = getNextFunctionIndex();
        registerFunction(function);
        StringBuilder functionBuffer = new StringBuilder(((DartBrowserFunction) function.getImpl()).name);
        //$NON-NLS-1$
        functionBuffer.append(" = function ");
        functionBuffer.append(((DartBrowserFunction) function.getImpl()).name);
        //$NON-NLS-1$
        functionBuffer.append("() {var result = callJava(");
        functionBuffer.append(((DartBrowserFunction) function.getImpl()).index);
        //$NON-NLS-1$
        functionBuffer.append(",'");
        functionBuffer.append(((DartBrowserFunction) function.getImpl()).token);
        //$NON-NLS-1$
        ...
        if (((DartBrowserFunction) function.getImpl()).top) {
            buffer.append(functionBuffer.toString());
        }
        //$NON-NLS-1$
        buffer.append("var frameIds = null;");
        if (((DartBrowserFunction) function.getImpl()).frameNames != null) {
            //$NON-NLS-1$
            buffer.append("frameIds = {");
            for (String frameName : ((DartBrowserFunction) function.getImpl()).frameNames) {
                buffer.append('\'');
                buffer.append(frameName);
                //$NON-NLS-1$
                buffer.append("':1,");
            }
            if (((DartBrowserFunction) function.getImpl()).frameNames.length > 0) {
        ...
        ((DartBrowserFunction) function.getImpl()).functionString = buffer.toString();
        nonBlockingExecute(((DartBrowserFunction) function.getImpl()).functionString);
    }
    ...
    void deregisterFunction(BrowserFunction function) {
        functions.remove(((DartBrowserFunction) function.getImpl()).index);
    }

    public void destroyFunction(BrowserFunction function) {
        String deleteString = getDeleteFunctionString(((DartBrowserFunction) function.getImpl()).name);
        //$NON-NLS-1$
    ...
    public Object evaluate(String script) throws SWTException {
        ...
        ((DartBrowserFunction) function.getImpl()).index = index;
        // Note, Webkit2 doesn't use 'isEvaluate' machinery because it doesn't use a function for evaluation.
        ((DartBrowserFunction) function.getImpl()).isEvaluate = true;
        registerFunction(function);
        ...
        //$NON-NLS-1$
        buffer.append(",'");
        buffer.append(((DartBrowserFunction) function.getImpl()).token);
        // $NON-NLS-1$
        buffer.append("', ['");
    ...
    void registerFunction(BrowserFunction function) {
        functions.put(((DartBrowserFunction) function.getImpl()).index, function);
    }
