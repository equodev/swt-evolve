...
abstract class StyledTextWriterBase extends TextWriter {
    ...
    public void writeLine(String line, int lineOffset) {
        ...
        int lineIndex = ((DartStyledText) styledText.getImpl()).content.getLineAtOffset(lineOffset);
        int lineAlignment;
        int lineIndent;
        boolean lineJustify;
        int verticalIndent;
        ...
        if (event != null) {
            ...
            verticalIndent = ((DartStyledText) styledText.getImpl()).renderer.getImpl().getLineVerticalIndent(lineIndex);
            lineAlignment = ((DartStyledText) styledText.getImpl()).renderer.getImpl().getLineAlignment(lineIndex, ((DartStyledText) styledText.getImpl()).alignment);
            lineIndent = ((DartStyledText) styledText.getImpl()).renderer.getImpl().getLineIndent(lineIndex, ((DartStyledText) styledText.getImpl()).indent);
            lineJustify = ((DartStyledText) styledText.getImpl()).renderer.getImpl().getLineJustify(lineIndex, ((DartStyledText) styledText.getImpl()).justify);
            ranges = ((DartStyledText) styledText.getImpl()).renderer.getImpl().getRanges(lineOffset, line.length());
            styles = ((DartStyledText) styledText.getImpl()).renderer.getImpl().getStyleRanges(lineOffset, line.length(), false);
        }
        if (styles == null) {
            styles = new StyleRange[0];
        }
        event = styledText.getImpl().getLineBackgroundData(lineOffset, line);
        Color lineBackground = (event != null && event.lineBackground != null) ? event.lineBackground : ((DartStyledText) styledText.getImpl()).renderer.getImpl().getLineBackground(lineIndex, null);
    ...
    void writeStyledLine(String line, int lineOffset, int[] ranges, StyleRange[] styles, Color lineBackground, int indent, int verticalIndent, int alignment, boolean justify) {
        ...
        // collect the length of the text we output (unescaped)
        int outTextLen = 0;
