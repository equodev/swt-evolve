 ...
 * @see ViewForm
 */
class DartViewFormLayout extends DartLayout implements IViewFormLayout {

    @Override
    public Point computeSize(Composite composite, int wHint, int hHint, boolean flushCache) {
        ViewForm form = (ViewForm) composite;
        Control left = ((DartViewForm) form.getImpl()).topLeft;
        Control center = ((DartViewForm) form.getImpl()).topCenter;
        Control right = ((DartViewForm) form.getImpl()).topRight;
        Control content = ((DartViewForm) form.getImpl()).content;
        ...
        if (((DartViewForm) form.getImpl()).separateTopCenter || (wHint != SWT.DEFAULT && leftSize.x + centerSize.x + rightSize.x > wHint)) {
    ...
    public void layout(Composite composite, boolean flushCache) {
        ...
        int minTopWidth = leftSize.x + centerSize.x + rightSize.x + 2 * form.marginWidth + 2 * ((DartViewForm) form.getImpl()).highlight;
        int count = -1;
        if (leftSize.x > 0)
            count++;
        if (centerSize.x > 0)
        ...
        int x = rect.x + rect.width - form.marginWidth - ((DartViewForm) form.getImpl()).highlight;
        int y = rect.y + form.marginHeight + ((DartViewForm) form.getImpl()).highlight;
        boolean top = false;
        if (((DartViewForm) form.getImpl()).separateTopCenter || minTopWidth > rect.width) {
            ...
            if (left != null && !left.isDisposed()) {
                ...
                int leftW = x - rect.x - form.marginWidth - ((DartViewForm) form.getImpl()).highlight - trim;
                leftSize = computeChildSize(left, leftW, SWT.DEFAULT, false);
                left.setBounds(rect.x + form.marginWidth + ((DartViewForm) form.getImpl()).highlight, y, leftSize.x, topHeight);
            }
            if (top)
            ...
            if (center != null && !center.isDisposed()) {
                ...
                int w = rect.width - 2 * form.marginWidth - 2 * ((DartViewForm) form.getImpl()).highlight - trim;
                Point size = computeChildSize(center, w, SWT.DEFAULT, false);
                if (size.x < centerSize.x) {
                    centerSize = size;
                }
                center.setBounds(rect.x + rect.width - form.marginWidth - ((DartViewForm) form.getImpl()).highlight - centerSize.x, y, centerSize.x, centerSize.y);
            ...
            if (left != null && !left.isDisposed()) {
                ...
                int w = x - rect.x - form.marginWidth - ((DartViewForm) form.getImpl()).highlight - trim.width;
        ...
        int oldSeperator = ((DartViewForm) form.getImpl()).separator;
        ((DartViewForm) form.getImpl()).separator = -1;
        if (content != null && !content.isDisposed()) {
            if (left != null || right != null || center != null) {
                ((DartViewForm) form.getImpl()).separator = y;
                y++;
            }
            content.setBounds(rect.x + form.marginWidth + ((DartViewForm) form.getImpl()).highlight, y, rect.width - 2 * form.marginWidth - 2 * ((DartViewForm) form.getImpl()).highlight, rect.y + rect.height - y - form.marginHeight - ((DartViewForm) form.getImpl()).highlight);
        }
        if (oldSeperator != ((DartViewForm) form.getImpl()).separator) {
            int t, b;
            if (oldSeperator == -1) {
                t = ((DartViewForm) form.getImpl()).separator;
                b = ((DartViewForm) form.getImpl()).separator + 1;
            } else if (((DartViewForm) form.getImpl()).separator == -1) {
                t = oldSeperator;
                b = oldSeperator + 1;
            } else {
                t = Math.min(((DartViewForm) form.getImpl()).separator, oldSeperator);
                b = Math.max(((DartViewForm) form.getImpl()).separator, oldSeperator);
            }
            form.redraw(((DartViewForm) form.getImpl()).borderLeft, t, form.getSize().x - ((DartViewForm) form.getImpl()).borderLeft - ((DartViewForm) form.getImpl()).borderRight, b - t, false);
        }
    }

    public ViewFormLayout getApi() {
        if (api == null)
            api = ViewFormLayout.createApi(this);
        return (ViewFormLayout) api;
    }

    public DartViewFormLayout(ViewFormLayout api) {
        super(api);
    }
}
