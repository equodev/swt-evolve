 ...
 * @see SashForm
 */
class DartSashFormLayout extends DartLayout implements ISashFormLayout {
    ...
    public Point computeSize(Composite composite, int wHint, int hHint, boolean flushCache) {
        ...
        if (ratios[maxIndex] > 0) {
            int sashwidth = ((DartSashForm) sashForm.getImpl()).sashes.length > 0 ? sashForm.SASH_WIDTH + ((DartSashForm) sashForm.getImpl()).sashes[0].getBorderWidth() * 2 : sashForm.SASH_WIDTH;
            if (vertical) {
    ...
    public void layout(Composite composite, boolean flushCache) {
        ...
        if (((DartSashForm) sashForm.getImpl()).controls.length == 0 && newControls.length == 0)
            return;
        ((DartSashForm) sashForm.getImpl()).controls = newControls;
        Control[] controls = ((DartSashForm) sashForm.getImpl()).controls;
        if (((DartSashForm) sashForm.getImpl()).maxControl != null && !((DartSashForm) sashForm.getImpl()).maxControl.isDisposed()) {
            for (Control control : controls) {
                if (control != ((DartSashForm) sashForm.getImpl()).maxControl) {
        ...
        if (((DartSashForm) sashForm.getImpl()).sashes.length < controls.length - 1) {
            Sash[] newSashes = new Sash[controls.length - 1];
            System.arraycopy(((DartSashForm) sashForm.getImpl()).sashes, 0, newSashes, 0, ((DartSashForm) sashForm.getImpl()).sashes.length);
            for (int i = ((DartSashForm) sashForm.getImpl()).sashes.length; i < newSashes.length; i++) {
        ...
        if (((DartSashForm) sashForm.getImpl()).sashes.length > controls.length - 1) {
            if (controls.length == 0) {
                for (Sash sash : ((DartSashForm) sashForm.getImpl()).sashes) {
                    sash.dispose();
                }
                ((DartSashForm) sashForm.getImpl()).sashes = new Sash[0];
            } else {
                Sash[] newSashes = new Sash[controls.length - 1];
                System.arraycopy(((DartSashForm) sashForm.getImpl()).sashes, 0, newSashes, 0, newSashes.length);
                for (int i = controls.length - 1; i < ((DartSashForm) sashForm.getImpl()).sashes.length; i++) {
                    ((DartSashForm) sashForm.getImpl()).sashes[i].dispose();
                }
                ((DartSashForm) sashForm.getImpl()).sashes = newSashes;
            }
        }
        if (controls.length == 0)
            return;
        Sash[] sashes = ((DartSashForm) sashForm.getImpl()).sashes;
        // get the ratios
    ...
    public SashFormLayout getApi() {
        if (api == null)
            api = SashFormLayout.createApi(this);
        return (SashFormLayout) api;
    }

    public DartSashFormLayout(SashFormLayout api) {
        super(api);
    }
}
