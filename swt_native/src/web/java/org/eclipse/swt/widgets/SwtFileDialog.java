...
public class SwtFileDialog extends SwtDialog implements IFileDialog {
    ...
    long _performKeyEquivalent(long id, long sel, long event) {
        ...
        if (window != null) {
            result = ((DartDisplay) ((DartWidget) parent.getImpl()).display.getImpl()).performKeyEquivalent(window, nsEvent);
        }
        return result ? 1 : 0;
    }
	 ...
	 */
        ...
        if (filterExtensions != null && filterExtensions.length != 0) {
            ...
            OS.object_setInstanceVariable(delegate.id, DartDisplay.SWT_OBJECT, jniRef);
    ...
    public long _method_overwriteExistingFileCheck() {
        return method_overwriteExistingFileCheck;
    }

    public long _methodImpl_overwriteExistingFileCheck() {
        return methodImpl_overwriteExistingFileCheck;
    }

    public long _method_performKeyEquivalent() {
        return method_performKeyEquivalent;
    }

    public long _methodImpl_performKeyEquivalent() {
        return methodImpl_performKeyEquivalent;
    }
