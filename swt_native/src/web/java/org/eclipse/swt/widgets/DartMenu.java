...
public class DartMenu extends DartWidget implements IMenu {
    ...
    public DartMenu(Decorations parent, int style, Menu api) {
        ...
        if (parent != null) {
            display = ((DartWidget) parent.getImpl()).display;
        } else {
    ...
    public DartMenu(Menu parentMenu, Menu api) {
        this(((DartMenu) checkNull(parentMenu).getImpl()).parent, SWT.DROP_DOWN, api);
    }
    ...
    DartMenu(Display display, Menu api) {
        super(api);
        if (display == null)
            display = DartDisplay.getCurrent();
        if (display == null)
    ...
    void checkParent(Widget parent) {
        ...
        if (parent == null && ((DartDisplay) DartDisplay.getDefault().getImpl()).appMenuBar == null)
    ...
    public void _setVisible(boolean visible) {
        if ((getApi().style & (SWT.BAR | SWT.DROP_DOWN)) != 0)
            return;
        TrayItem trayItem = ((DartDisplay) display.getImpl()).currentTrayItem;
        if (trayItem != null && visible) {
            ((DartTrayItem) trayItem.getImpl()).showMenu(this.getApi());
            return;
        }
        if (visible) {
            if (hasLocation) {
    ...
    public boolean getVisible() {
        ...
        if ((getApi().style & SWT.POP_UP) != 0) {
            Menu[] popups = ((DartDisplay) display.getImpl()).popups;
            if (popups != null) {
    ...
    public boolean isEnabled() {
        checkWidget();
        if (this.getApi() == ((DartDisplay) display.getImpl()).appMenuBar)
            return getEnabled();
        if (this.getApi() == ((DartDisplay) display.getImpl()).appMenu)
            return getEnabled();
    ...
    void releaseParent() {
        ...
        if ((getApi().style & SWT.BAR) != 0 && parent != null && this.getApi() == ((DartDecorations) parent.getImpl()).menuBar) {
    ...
    void releaseWidget() {
        super.releaseWidget();
        ((DartDisplay) display.getImpl()).removeMenu(this.getApi());
        parent = null;
    ...
    public void setVisible(boolean visible) {
        ...
        if (visible) {
            ...
            ((DartDisplay) display.getImpl()).removePopup(this.getApi());
            _setVisible(false);
        }
    }
