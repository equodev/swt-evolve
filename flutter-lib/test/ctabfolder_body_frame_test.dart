...
/// What `Sizes.CTAB_FOLDER_TAB_STRIP_HEIGHT` reserves for the strip.
...
void main() {
  testWidgets('the page starts one frame in from the folder\'s left edge', (tester) async {
    ...
    final border = ctabFolderBodyBorderWidth(SWT.NONE);

    expect(_bodyIn(tester).left, border,
  ...
  testWidgets('both sides and the bottom carry the same frame', (tester) async {
    ...
    expect(_folderWidth - body.right, border);
    expect(_folderHeight - body.bottom, border);
  });
  ...
  testWidgets('SWT.BORDER widens the frame, matching what Java reserves', (tester) async {
    ...
    final border = ctabFolderBodyBorderWidth(SWT.BORDER);

    expect(border, greaterThan(ctabFolderBodyBorderWidth(SWT.NONE)));
    final body = _bodyIn(tester);
    expect(body.left, border);
    expect(_folderWidth - body.right, border);
  });
