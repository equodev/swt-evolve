...
// `flutter test` parallelizes one file at a time, and the 200-odd cases here each render a widget
// and read its pixels back - enough to be the whole suite's tail on its own. The cases are split
// across sibling _test.dart files, which is the only way the runner spreads them over workers;
// `shard`/`shards` decide which ones each file claims.
void defineThemingConformance({int shard = 0, int shards = 1}) {
  var caseIndex = 0;
  bool mine() => caseIndex++ % shards == shard;
