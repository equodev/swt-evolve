...
List<Widget> getActions(
  ...
  return switch (style & cases) {
    ...
    (SWT.OK | SWT.CANCEL) => [
        createButton(context, labels?[SWT.OK.toString()] ?? "OK", SWT.OK),
        createFilledButton(
            context, labels?[SWT.CANCEL.toString()] ?? "CANCEL", SWT.CANCEL)
      ],
    (SWT.YES | SWT.NO) => [
    ...
    (SWT.RETRY | SWT.CANCEL) => [
        createButton(
            context, labels?[SWT.RETRY.toString()] ?? "RETRY", SWT.RETRY),
        createFilledButton(
            context, labels?[SWT.CANCEL.toString()] ?? "CANCEL", SWT.CANCEL)
      ],
    (SWT.ABORT | SWT.RETRY | SWT.IGNORE) => [
        createButton(
            context, labels?[SWT.ABORT.toString()] ?? "ABORT", SWT.ABORT),
        createButton(
            context, labels?[SWT.RETRY.toString()] ?? "RETRY", SWT.RETRY),
        createFilledButton(
            context, labels?[SWT.IGNORE.toString()] ?? "IGNORE", SWT.IGNORE)
      ],
    _ => [
