...
void setupCases(WidgetMeasurer measurer) {
  ...
  print('Generated ${measurer.testCases.length} TreeItem test cases');
}
...
MeasurementCase createCase(
  ...
  final (value, expectedComponents) = createVTreeItem(
    style,
    text,
    fontSize: fontSize,
    image: image,
  ...
  return MeasurementCase(
    ...
    fqn: 'org.eclipse.swt.widgets.TreeItem',
    expectedComponents: expectedComponents,
    widgetBuilder: (key) {
      getConfigFlags().use_swt_fonts = !useFontTheme;
      return TreeItemSwt(key: key, value: value);
    },
  );
}

(VTreeItem, Map<String, dynamic>) createVTreeItem(
  (String, int) style,
  String text, {
  int? fontSize,
  (int, int)? image,
